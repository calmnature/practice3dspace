...
import com.matrixone.apps.common.MeetingNotification;
...
public class emxMeetingBase_mxJPO extends emxDomainObject_mxJPO
{
   ...
   // public static final String ATTRIBUTE_MEETING_SITE_ID = PropertyUtil.getSchemaProperty("attribute_MeetingSiteID");//"Meeting Site ID";
        ...
        throws Exception
    {
        ...
        String sMeetingDateOrig         = busMeeting.getInfo(context,"attribute["+busMeeting.ATTRIBUTE_MEETING_STARTDATETIME+"]");
        ...
        patternType.addPattern(DomainConstants.TYPE_GROUP);
        patternType.addPattern(typeGroupProxy);
        ...
        typeSelects.addElement(busMeeting.SELECT_TYPE);
        typeSelects.addElement(busMeeting.SELECT_ID);
        ...
        try {
            ...
            StringList UniqueMembNames = new StringList();
            while (mapItr.hasNext())
            {
            	StringList membNames = new StringList();
                ...
                String sAttendeeType           = (String) map.get(busMeeting.SELECT_TYPE);
                String sAttendeeId           = (String) map.get(busMeeting.SELECT_ID);
                if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || sAttendeeType.equalsIgnoreCase(typeGroupProxy)){
                	DomainObject group = new DomainObject(sAttendeeId);
                	MapList listMembers = group.getRelatedObjects(context, DomainConstants.RELATIONSHIP_GROUPMEMBER, DomainConstants.TYPE_PERSON, new StringList(DomainConstants.SELECT_NAME), null, false, true, (short) 1, null, null, 0);
                	Iterator membersItr = listMembers.iterator();
                	while(membersItr.hasNext()) {
                		Map member = (Map) membersItr.next();
                		membNames.addElement((String) member.get(DomainConstants.SELECT_NAME));
                	}
                ...
                //To check whether email is enabled or not
                String mailEnabled = EnoviaResourceBundle.getProperty(context,"emxFramework.Meeting.EnableMails");
                boolean emailAlso = "true".equalsIgnoreCase(mailEnabled);
                ...
                // to set user wise preference date in notification mail and icon //
				String sMeetingDate = sMeetingDateOrig;
                try{
                	if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_PERSON)){
                	sMeetingDate = getFormattedDisplayMeetingStartDateTimeForUser(context, sAttendeeName, sMeetingDateOrig, userTZOffset);
                	}
				}catch(Exception e){}
                
                /*
                 * For handling scenario where User-Group is having only some of the members as Notified
                 */
                HashSet<String> attendeeNames = new HashSet<String>();
                if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || sAttendeeType.equalsIgnoreCase(typeGroupProxy)) {
                	if(!sAttributeValue.equalsIgnoreCase("Notified") && sAttributeValue.startsWith("Notified")) {
                		String names[] = sAttributeValue.split("\\|");
                		
                		for(int i=1; i<names.length; ++i)
                			attendeeNames.add(names[i]);
						
						sAttributeValue = "Partial";
                	}
                }         
                
                String[] messageKeys = {"host", "date", "note"};
                String[] messageValues = {sOwner, sMeetingDate, sMeetingNote};
                
                //To send mail to newly Added Attendee
                if ("Add".equals(sAttributeValue) || (UIUtil.isNullOrEmpty(sAttributeValue) && sOwner.equals(sAttendeeName)))
                {
                    ...
                    if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || sAttendeeType.equalsIgnoreCase(typeGroupProxy)){
                    	for(int i=0; i< membNames.size(); i++){
                    		 mailArguments[0] = membNames.get(i);
                    		 try{
                             	sMeetingDate = getFormattedDisplayMeetingStartDateTimeForUser(context, membNames.get(i), sMeetingDateOrig, userTZOffset);
             				}catch(Exception e){}
                    		 
                    		 mailArguments[8] = sMeetingDate;
                    		 messageValues[1] = mailArguments[8];
                    		 
                    		//Calling method to send Mail
                    		 if(!UniqueMembNames.contains(membNames.get(i))){
                    		 //mailUtil.sendNotificationToUser(context,mailArguments,emailAlso);
                    		 mailUtil.sendNotification(context, new StringList(mailArguments[0]), null, null,
                    				 					mailArguments[1], new String[0], new String[0],
                    				 					mailArguments[3], messageKeys, messageValues,
                    				 					new StringList(mailArguments[11]), null, null, emailAlso);
                             UniqueMembNames.addElement(membNames.get(i));
                    		 }
                    	}
                    }else{
                    	//Calling method to send Mail
                    	if(!UniqueMembNames.contains(sAttendeeName)){
                    	//mailUtil.sendNotificationToUser(context,mailArguments,emailAlso);
                    	mailUtil.sendNotification(context, new StringList(mailArguments[0]), null, null,
			 					mailArguments[1], new String[0], new String[0],
			 					mailArguments[3], messageKeys, messageValues,
			 					new StringList(mailArguments[11]), null, null, emailAlso);
                        UniqueMembNames.addElement(sAttendeeName);
                    	}
                    }
                  ...
                  else if ("Remove".equals(sAttributeValue) )
                {
                  ...
                  else if ("Notified".equals(sAttributeValue) || "Partial".equals(sAttributeValue))
                {
                    //Construct String array to send mail notification
                    String [] mailArguments=new String [12];
                    mailArguments[0] = sAttendeeName;
                    ...
                    if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || sAttendeeType.equalsIgnoreCase(typeGroupProxy)){
                    	for(int i=0; i< membNames.size(); i++){
                    		...
                    		//Calling method to send Mail
                    		 if(!UniqueMembNames.contains(membNames.get(i))){
                    			 if("Partial".equals(sAttributeValue) && !attendeeNames.contains(membNames.get(i))) {
                    				 mailArguments[1] = "emxFramework.ProgramObject.eServicecommonTrigaNotifyMeetingInvitation_if.Subject";
                                     mailArguments[3] = "emxFramework.ProgramObject.eServicecommonTrigaNotifyMeetingInvitation_if.Message";
                                     //mailUtil.sendNotificationToUser(context,mailArguments,emailAlso);
                                     mailUtil.sendNotification(context, new StringList(mailArguments[0]), null, null,
   ...
   public MapList getMeetings(matrix.db.Context context, String[] args) throws Exception
   {
       ...
       String strContextId = (String)requestMap.get("objectId");
		String ATTRIBUTE_CO_OWNER = PropertyUtil.getSchemaProperty(context, "attribute_MeetingCoowners");
       ...
       expandTypeWhere.append("('").append(DomainObject.SELECT_OWNER).append("' == '").append(busPerson.getName());
		...
		expandTypeWhere.append("( ").append(SYMB_QUOTE).append(context.getUser()).append(SYMB_QUOTE).append(" matchlist ").append(" attribute[" + ATTRIBUTE_CO_OWNER + "] ").append(" \"~\" )");
		expandTypeWhere.append(" || '");
  ...
  public String getMeetingContext(Context context,String[] args) throws Exception {
      ...
      //While exporting mode is empty and reportFormat ="CSV".
        ...
        selects.add("type.kindof["+ DomainConstants.TYPE_PROJECT_TEMPLATE +"]");
        selects.add("type.kindof["+ DomainConstants.TYPE_PROGRAM +"]");
        selects.add("type.kindof["+ DomainConstants.TYPE_PROJECT_CONCEPT +"]");

        if("create".equals(strMode)){
        	Map meetingMap = ObjectDom.getInfo(context,selects);
        	String contextName = (String)meetingMap.get(DomainConstants.SELECT_NAME);
    		String objectTitle = (String)meetingMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
    	    boolean isProjectType =  "true".equalsIgnoreCase((String)meetingMap.get("type.kindof["+ DomainConstants.TYPE_PROJECT_SPACE +"]")) || "true".equalsIgnoreCase((String)meetingMap.get("type.kindof["+ DomainConstants.TYPE_PROJECT_TEMPLATE +"]")) || "true".equalsIgnoreCase((String)meetingMap.get("type.kindof["+ DomainConstants.TYPE_PROGRAM +"]")) || "true".equalsIgnoreCase((String)meetingMap.get("type.kindof["+ DomainConstants.TYPE_PROJECT_CONCEPT +"]")); 
     
        	if((DomainObject.TYPE_WORKSPACE).equals(meetingMap.get(DomainConstants.SELECT_TYPE)) ||
        			com.matrixone.apps.domain.util.mxType.isOfParentType(context,(String)meetingMap.get(DomainConstants.SELECT_TYPE),com.matrixone.apps.domain.DomainObject.TYPE_WORKSPACE_VAULT) || (isProjectType && objectTitle != null && !"".equalsIgnoreCase(objectTitle))) {
        		contextName = (String)meetingMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);                  		
        	}
            return contextName;
        }else {
            ...
            Map contextObj = getMeetingContextObject(context, ObjectDom, selects, true);
            if (contextObj == null)
            	return "";
            String contextName = (String)contextObj.get(DomainConstants.SELECT_NAME);
        	if((DomainObject.TYPE_WORKSPACE).equals(contextObj.get(DomainConstants.SELECT_TYPE)) ||
        			com.matrixone.apps.domain.util.mxType.isOfParentType(context,(String)contextObj.get(DomainConstants.SELECT_TYPE),com.matrixone.apps.domain.DomainObject.TYPE_WORKSPACE_VAULT)) {
        		contextName = (String)contextObj.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);                  		
        	}
            //return  contextObj == null ? "" : contextName;
  ...
  public String getMeetingStartTime(Context context,String[] args) throws Exception {
    ...
    private String getFormattedDisplayMeetingStartDateTimeForUser(Context context, String user, String sMeetingDateOrig, double clientTZOffset) throws FrameworkException {
    ...
    private String getFormattedMeetingStartTime(Context context, Meeting meetingObj, double clientTZOffset, boolean trimSeconds) throws FrameworkException {
        StringList formattedDisplayDateTimeList = FrameworkUtil.split(getFormattedDisplayMeetingStartDateTime(context, meetingObj, clientTZOffset).replace("\u202F", " "), " "); //Replacing Narrow Non-Breaking space character (introduced in Java 21) with space as breaks the Date String processing
...
//Addition End:30-Apr-2010:di1:R210:PRG:Meeting Usability
        ...
        for (Iterator iter = agendaItemList.iterator(); iter.hasNext();) {
           ...
           // String levelId = (String) agendaItemMap.get(SELECT_LEVEL);
//Added:30-Apr-2010:di1:R210:PRG:Meeting Usability
...
//Addition End:30-Apr-2010:di1:R210:PRG:Meeting Usability
            ...
            if(index == -1)
            {
              ...
              //  newMap.put(SELECT_LEVEL, levelId);
//Added:30-Apr-2010:di1:R210:PRG:Meeting Usability
...
//Addition End:30-Apr-2010:di1:R210:PRG:Meeting Usability
		...
		MapList coowedList = getCoownedMeetings(context,  select);

         StringList allIds = new StringList();
		for(int i=0; i< MeetingList.size(); i++)
        {
            Map idMap = (Map) MeetingList.get(i);
            allIds.add((String)idMap.get(DomainConstants.SELECT_ID));
        }

        for(int j=0; j< coowedList.size(); j++)
        {
            Map idMap = (Map) coowedList.get(j);
            if(!allIds.contains( (String)idMap.get(DomainConstants.SELECT_ID))){
            	idMap.put("relationship", DomainConstants.RELATIONSHIP_ASSIGNED_MEETINGS);
            	MeetingList.add(idMap);
            }
        }
            return MeetingList;
	 ...
	 /**
     * Method to get the Meetings.
     *
     * @param context the eMatrix <code>Context</code> object
     * @param strWhereCondition - which holds the where expression for query
     * @param objectSelects - which holds selectables for query
     * @return Maplist - containing issue ids.
     * @throws Exception if the operation fails
     * @since AEF 421Hf14
     */
     protected MapList getCoownedMeetings(
             Context context,
    	 ...
    	 DomainObject personDom = DomainObject.newInstance(context, personid);
    	 String ATTRIBUTE_CO_OWNER = PropertyUtil.getSchemaProperty(context, "attribute_MeetingCoowners");
    	 StringBuffer expandTypeWhere = new StringBuffer(100);
    	 expandTypeWhere.append("( ").append(SYMB_QUOTE).append(context.getUser()).append(SYMB_QUOTE).append(" matchlist ").append(" attribute[" + ATTRIBUTE_CO_OWNER + "] ").append(" \"~\" )");

    	 MapList MeetingList = personDom.getRelatedObjects(context,
    			 relPattern.toString(),               //String relPattern
    			 DomainConstants.TYPE_MEETING,              //String typePattern
      ...
      Map contextObjectMap = getMeetingContextObject(context, objDom, selects, false);
        ...
        if (contextObjectMap == null) // Context object might have
          ...
          objContext.add("");
                	...
                	String contextName = (String)contextObjectMap.get(DomainConstants.SELECT_NAME);
                	if((DomainObject.TYPE_WORKSPACE).equals(contextObjectMap.get(DomainConstants.SELECT_TYPE)) ||
                			com.matrixone.apps.domain.util.mxType.isOfParentType(context,(String)contextObjectMap.get(DomainConstants.SELECT_TYPE),com.matrixone.apps.domain.DomainObject.TYPE_WORKSPACE_VAULT)) {
                		contextName = (String)contextObjectMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);                  		
                	}
                	objContext.add(contextName);
                }
...
//Added:30-Apr-2010:di1:R210:PRG:Meeting Usability
        ...
        if (TYPE_MEETING.equals(strobjType)) {
          ...
          if((DomainObject.TYPE_WORKSPACE).equals(contextObjectMap.get(DomainConstants.SELECT_TYPE)) ||
      			com.matrixone.apps.domain.util.mxType.isOfParentType(context,(String)contextObjectMap.get(DomainConstants.SELECT_TYPE),com.matrixone.apps.domain.DomainObject.TYPE_WORKSPACE_VAULT)) {
        	  Context = (String)contextObjectMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);                  		
      	}
...
//Addition End:30-Apr-2010:di1:R210:PRG:Meeting Usability
  ...
  public boolean checkEditMeeting(Context context, String[] args) throws Exception{
    ...
    boolean hasModifyAccess=MeetingDom.getAccessMask(context).hasModifyAccess();
  ...
  public void promoteMeeting(Context context, String[] args) throws Exception {
    ...
    private void checksToDeleteMeetings(Context context, String strObjectId, String[] meetingIds) throws FrameworkException, Exception {
		...
		String ATTRIBUTE_MEETING_CO_OWNER = PropertyUtil.getSchemaProperty(context,"attribute_MeetingCoowners");
        ...
        for (int i = 0; i < meetingIds.length; i++) {
            ...
            objsel.add("attribute["+ATTRIBUTE_MEETING_CO_OWNER+"]");
            objsel.add("current.access[delete]");
            Map meetingObjSelValues = meetingObj.getInfo(context , objsel);
            String meetingCoowners = (String) meetingObjSelValues.get("attribute["+ATTRIBUTE_MEETING_CO_OWNER+"]");
			StringList coownerList = FrameworkUtil.split(meetingCoowners, "~");
			if(!(loginUserName.equals(meetingObjSelValues.get(DomainObject.SELECT_OWNER)) || coownerList.contains(loginUserName))) {
				nonHostMeetings.add((String)meetingObjSelValues.get(DomainObject.SELECT_NAME));
			}
            String hasDeleteAccess = (String) meetingObjSelValues.get("current.access[delete]");
            ...
            if("Scheduled".equals(strMeetingState) || "In Progress".equals(strMeetingState)) {
                startedStateMeetings.add((String)meetingObjSelValues.get(DomainObject.SELECT_NAME));
            }
            if(!"true".equalsIgnoreCase(hasDeleteAccess)) {
                nonHostMeetings.add((String)meetingObjSelValues.get(DomainObject.SELECT_NAME));
            }
        }
        ...
        if(nonHostMeetings.size() != 0 || startedStateMeetings.size() != 0) {
            ...
            if(nonHostMeetings.size() != 0) {
                err.append("\\n").append(EnoviaResourceBundle.getProperty(context,"emxComponentsStringResource", context.getLocale(),"emxComponents.Common.NoDeleteAccess"));
   ...
   public MapList getMeetingttendee(Context context, String[] args)throws Exception
    {
        ...
        typePattern.addPattern(DomainConstants.TYPE_GROUP);
        ...
        selectTypeStmts.add(MeetingObj.SELECT_ATTRIBUTE_TITLE);
  ...
  public static MapList findTypesForAgendaItems(
   ...
   throws Exception{
       ...
       if("None".equals(strMeetingType)){
    	   boolean hasPromoteAccess=meetingDom.getAccessMask(context).hasPromoteAccess();
           if(!"Complete".equals(strCurrentState)&& hasPromoteAccess){
   ...
   throws Exception{
       ...
       selects.addElement(DomainConstants.SELECT_OWNER);
       selects.addElement(DomainConstants.SELECT_CURRENT);
       selects.addElement("attribute["+ATTRIBUTE_MEETING_TYPE+"]");
       selects.addElement("current.access[promote]");
       java.util.Map meetingInfoList= (java.util.Map)meetingDom.getInfo(context,selects);
       String strMeetingOwner = (String) meetingInfoList.get(DomainConstants.SELECT_OWNER);
       strCurrentState = (String) meetingInfoList.get(DomainConstants.SELECT_CURRENT);
       String strMeetingType = (String) meetingInfoList.get("attribute["+ATTRIBUTE_MEETING_TYPE+"]");
       String strPromoteAccess = (String) meetingInfoList.get("current.access[promote]");
       if(!"None".equals(strMeetingType)){
           if("Scheduled".equals(strCurrentState)){
               //Logged in person Owner
               if("true".equalsIgnoreCase(strPromoteAccess)){
       ...
       for (int i = 0; i < attributeRange.size(); i++) {
           if("3D Visual Meeting".equals(attributeRange.get(i))){
               if(isViewerIntegrationInstalled()){
            	   rangeMeetingTypeDisplay.add((String)attributeDisplayRange.get(i));
            	   rangeMeetingTypeActual.add((String)attributeRange.get(i));
               }
           }
           else{
        	   rangeMeetingTypeDisplay.add((String)attributeDisplayRange.get(i));
   ...
   static public boolean isViewerIntegrationInstalled()
    {
        try
        {
          ...
          String strClass = "com.cimmetry.servlet.jakarta.VueServlet";
...
public boolean isCreateAndDeleteAgendaItemEnabled(Context context, String[] args)throws Exception
  {
    ...
    String[] states=new String[]{STATE_MEETING_CREATE, STATE_MEETING_IN_PROGRESS, STATE_MEETING_SCHEDULED};
      HashMap programMap = (HashMap)JPO.unpackArgs(args);
      String strMeetingId = (String)programMap.get("objectId");
      Meeting meetingObj = new Meeting(strMeetingId);
...
public static String getCompanyFieldHTMLOutput(Context context, String[] args) throws Exception{
  ...
  private static boolean isMeetingHost(Context context, Meeting meetingObj) throws Exception {
		...
		StringList selects = new StringList();
		selects.addElement(DomainConstants.SELECT_OWNER);
		selects.addElement("attribute["+ATTRIBUTE_MEETING_CO_OWNER+"]");

		Map meetingInfo = meetingObj.getInfo(context, selects);
		String owner = (String) meetingInfo.get(DomainConstants.SELECT_OWNER);
		String coowners = (String) meetingInfo.get("attribute["+ATTRIBUTE_MEETING_CO_OWNER+"]");
		if(context.getUser().equals(owner) || (UIUtil.isNotNullAndNotEmpty(coowners) && coowners.contains(context.getUser()))){
			return true;
		}
		return false;
  }
...
/*      if(!isMeetingHost(context, meetingObj))
  ...
  public Map meetingAgendaItemEditPreProcess(Context context, String[] args) throws Exception {
	  ...
	  boolean hasModifyAccess=meeting.getAccessMask(context).hasModifyAccess();
      Map returnMap = new HashMap();
  ...
  public boolean isMeetingAttendee( Context context, String[] args ) throws Exception {
    ...
    try {
      ...
      if(doMeeting.getType(context).equals(DomainObject.TYPE_MEETING)){
				String ATTRIBUTE_CO_OWNER = PropertyUtil.getSchemaProperty(context, "attribute_MeetingCoowners");
				String coowners = doMeeting.getInfo(context, "attribute["+ATTRIBUTE_CO_OWNER+"]");
				if(UIUtil.isNotNullAndNotEmpty(coowners) && coowners.contains(context.getUser())){
					return true;
				}
	...
	public static boolean canRemoveAttendees( Context context, String[] args ) throws Exception {
		Map programMap         = (HashMap) JPO.unpackArgs( args );
		String sMeetingId      = (String) programMap.get( "objectId" );
		Meeting meetingObj = new Meeting(sMeetingId);
		if(!"Create".equals(getMeetingState(context, meetingObj))){
			return false;
		}
		if(!isMeetingHost(context, meetingObj)){
			return false;
    ...
    try {
      ...
      for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) {
        ...
        if (RELATIONSHIP_AGENDA_ITEM.equals(strColumnType)) {
          columnName.append("<img title='"+strColumnName+"' src='./images/iconSmallAgendaItem.png' border='0' valign='absmiddle'/> ");
          columnName.append(strColumnName);
        } else if (TYPE_DECISION.equals(strColumnType)) {
          columnName.append("<img title='"+strColumnName+"' src='./images/iconSmallDecision.png' border='0' valign='absmiddle'/> ");
          columnName.append(strColumnName);
        } else if (TYPE_PERSON.equals(strColumnType)) {
          columnName.append("<img title='"+strColumnName+"' src='./images/iconSmallPerson.png' border='0' valign='absmiddle'/> ");
          columnName.append(strColumnName);
        } else {
          ...
          columnName.append("')\">");
          columnName.append("<img src='./images/iconSmallMeeting.png' border='0' valign='absmiddle'/>  ");
...
try {
 ...
 /**
  ...
  public static StringList getCellAccessforMeetingSummary(
	...
	@com.matrixone.apps.framework.ui.ProgramCallable
	public String getMeetingCoownersList(Context context, String[] args) throws Exception {
		String roleList = null;
		StringBuffer rolesBuf = new StringBuffer();
		boolean iscloud = UINavigatorUtil.isCloud(context);
		if (!iscloud) {
		 Role roleObj = new Role("Global User");
         roleObj.open(context);
         RoleList slChild = roleObj.getChildren(context, true);
         String sTempRoles = slChild.toString().replace("[", "").replace("]", "").replace(", ", ",");
 		if(UIUtil.isNotNullAndNotEmpty(sTempRoles)){
 			sTempRoles = sTempRoles.concat(",");
 			rolesBuf.append(sTempRoles);   
 		}
 		rolesBuf.append("Global User,role_VPLMExperimenter,role_3DSRestrictedContributor,role_3DSRestrictedOwner,role_VPLMAdmin,role_VPLMProjectAdministrator"); 
		}else{
		rolesBuf.append("role_VPLMExperimenter,role_3DSRestrictedContributor,role_3DSRestrictedOwner,role_VPLMAdmin,role_VPLMProjectAdministrator"); 
		}
 		return "USERROLE="+rolesBuf.toString();
	}
  ...
  public StringList getAgendaResponsiblePersonIncludeIDs(Context context, String[] args) throws FrameworkException {
      try {
            ...
            for (int i = 0; i < attendees.size(); i++) {
                ...
                ids.add((String)attendee.get(SELECT_ID));
            }
            return ids;
        } catch (Exception e) {
  ...
  public String getMeetingDate(Context context, String[] args) throws FrameworkException {
	...
	/** to show form field editable or non-editable
	 * @param context
	 * @param args
	 * @return
	 * @throws Exception
	 */
	public static boolean canEditCoownersField(Context context, String args[]) throws Exception {
		HashMap inputMap = (HashMap)JPO.unpackArgs(args);
		...
		DomainObject meetingObject = new DomainObject(objectId);
		String hasChangeSOV = meetingObject.getInfo(context, DomainConstants.SELECT_HAS_CHANGESOV_ACCESS);
	...
	public static StringList showCoownerFieldEditable(Context context, String args[])throws Exception
	{
		HashMap inputMap = (HashMap)JPO.unpackArgs(args);
		...
		while (objectItr.hasNext())
		{
			Map curObjectMap = (Map) objectItr.next();
			String curObjectID = (String) curObjectMap.get("id");
			DomainObject meetingObject = DomainObject.newInstance(context,curObjectID);
			String hasChangeSOV = meetingObject.getInfo(context, DomainConstants.SELECT_HAS_CHANGESOV_ACCESS);

			if(hasChangeSOV.equalsIgnoreCase("true"))
			{
				returnStringList.addElement("true");
			}
			else
			...
			throws Exception
	{
		...
		{
			Meeting.setCoOwners(context, strObjId[0], strOldCoOwners, strNewCoOwners);
		}
	}
	@com.matrixone.apps.framework.ui.PostProcessCallable
	public void updateCoownersOwnership(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		HashMap requestMap = (HashMap) programMap.get("requestMap");
		String sMeetingId = (String) paramMap.get("objectId");
		String coowers		= (String) requestMap.get("CoOwners");
		if(UIUtil.isNotNullAndNotEmpty(coowers)){
			Meeting meetingObj = new Meeting(sMeetingId);
			StringList selects = new StringList(2);
			selects.add(SELECT_NAME);
			selects.add(SELECT_TYPE);
			Map meetingInfo = meetingObj.getInfo(context, selects);
			StringList tempValues = com.matrixone.apps.domain.util.FrameworkUtil.split(coowers, "|");
			tempValues.remove(context.getUser());
			meetingObj.updateCoOwnersOwnership(context, sMeetingId, new StringList(), tempValues);
			meetingObj.addAttendees(context, tempValues);
			Meeting.sendNotificationToCoowners(context, sMeetingId, (String) meetingInfo.get(SELECT_NAME), (String) meetingInfo.get(SELECT_TYPE), tempValues);
		}
	}
	public void updateCoownersField(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		HashMap requestMap = (HashMap) programMap.get("requestMap");
		String sMeetingId = (String) paramMap.get("objectId");
		String coowers		= (String) paramMap.get("New Value");
		String ATTRIBUTE_CO_OWNER = PropertyUtil.getSchemaProperty(context, "attribute_MeetingCoowners");
		Meeting meetingObj = new Meeting(sMeetingId);
		StringList selects = new StringList(2);
		selects.add(SELECT_NAME);
		selects.add(SELECT_TYPE);
		selects.add("attribute["+ATTRIBUTE_CO_OWNER+"]");
		Map meetingInfo = meetingObj.getInfo(context, selects);
		String oldCoowner = (String) meetingInfo.get("attribute["+ATTRIBUTE_CO_OWNER+"]");	
		Meeting.setCoOwners(context, sMeetingId, oldCoowner, coowers);
	}
	
	/**
	 * Invoked by the 3DNotification trigger on Meeting state promote
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args holds a String array containing the following entries: meetingID, meetingState
	 * @throws Exception if operation fails
	 */
	public void send3DNotificationOnPromote(Context context, String[] args) throws Exception {

		String meetingId =  args[0];
		String meetingState = args[1];
		
		HashMap<String, List<String>> meetingAttendees = getMeetingAttendeesAndRelId(context, meetingId, true);
		
		List<String> attendeeList = new ArrayList(meetingAttendees.keySet());

		List<String> notifiedAttendees = new ArrayList<String>();
		
		MeetingNotification notifyObj = new MeetingNotification();
		
		List<String> newAttendees = new ArrayList<String>();
		
		if(meetingState.equals(STATE_MEETING_SCHEDULED)) {
			
			for(String attendee : attendeeList) {  
				List<String> relIdList = meetingAttendees.get(attendee);

				String str[] = relIdList.toArray(new String[relIdList.size()]);
				StringList relSelect = new StringList("attribute["+DomainConstants.ATTRIBUTE_MEETING_ATTENDEE+"]");
				MapList relData = DomainRelationship.getInfo(context, str, relSelect);

				Iterator relDataItr = relData.iterator();
				while (relDataItr.hasNext()) {
					Map relDatamap = (Map)relDataItr.next();
					String attribData = (String) relDatamap.get("attribute["+DomainConstants.ATTRIBUTE_MEETING_ATTENDEE+"]");
					
					/*
					 * attribData can have values like "Add", "Notified", "Notified|usrn1|usrn2|usrn3|......."
					 * where each usrn is the username of the Notified person from a user group
					 */
					if(attribData.equals("Notified")) {
						notifiedAttendees.add(attendee);
						break;
					}						
					else if(attribData.startsWith("Notified")) {
						String attribDataValues[] = attribData.split("\\|");
				    	
				    	HashSet<String> notifiedData = new HashSet<String>();				    	
				    	for(int i=1; i<attribDataValues.length; ++i) 
				    		notifiedData.add(attribDataValues[i]);
				    	
				    	if(notifiedData.contains(attendee)) {
				    		notifiedAttendees.add(attendee);
							break;
				    	}				    		
					}   
				...
				if(!notifiedAttendees.contains(attendee))
					newAttendees.add(attendee);
			}		
			
			sendNotificationAndMail(context, meetingId, newAttendees, "Promote");
			
			HashMap<String,String> relIdAttribMap = getMeetingRelIds(context, meetingId);
			List<String> relIdList = new ArrayList(relIdAttribMap.keySet());
			
			DomainRelationship Obj;
			
	    	for(String relId : relIdList) {  
				String notifyStatus = relIdAttribMap.get(relId);
				
				if(!notifyStatus.equals("Notified")) {
					Obj = DomainRelationship.newInstance(context,relId);
					
					try {
						ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
		                Obj.setAttributeValue(context,DomainConstants.ATTRIBUTE_MEETING_ATTENDEE ,"Notified");
		            } catch (Exception e) {
		            	System.out.println("Error updating Meeting Attribute.");
		            	e.printStackTrace();
		            } finally {
		                ContextUtil.popContext(context);
		            }
			...
			notifiedAttendees = attendeeList;
		
		sendNotificationAndMail(context, meetingId, notifiedAttendees, "PromoteNotified");
		
	}
	
	/**
	 * Invoked by the 3DNotification trigger on adding new attendee to Meeting
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args holds a String array containing the following entries: meetingID, attendeeId, attendeeType, relationshipId
	 * @throws Exception if operation fails
	 */
	public void send3DNotificationOnAddAttendee(Context context, String[] args) throws Exception {

		String meetingId =  args[0];
    	String attendeeId = args[1];
    	String attendeeType = args[2];
    	String relId = args[3];
    	
    	Meeting busMeeting = new Meeting(meetingId);
        busMeeting.open(context);

        String meetingState  = (String)busMeeting.getInfo(context,DomainObject.SELECT_CURRENT);
        
        if(meetingState.equals(STATE_MEETING_SCHEDULED) || meetingState.equals(STATE_MEETING_IN_PROGRESS)) {     
        	List<String> addedAttendees = new ArrayList<String>();
        	
        	HashMap<String, List<String>> meetingAttendees;
        	
        	List<String> toList = new ArrayList<String>();
        	
        	if(attendeeType.equalsIgnoreCase(DomainConstants.TYPE_PERSON)) {
            	Person person = new Person(attendeeId);
    			String attendeeName = (String)person.getInfo(context, DomainConstants.SELECT_NAME);
    			addedAttendees.add(attendeeName);
    			
    			meetingAttendees = getMeetingAttendeesAndRelId(context, meetingId, false);
    			
    			if(!meetingAttendees.containsKey(attendeeName))
    				toList.add(attendeeName);
        	}
        	else {
        		addedAttendees = getUserGroupAttendees(context, attendeeId, attendeeType);
        		meetingAttendees = getMeetingAttendeesAndRelId(context, meetingId, true);
        		
        		for(String attendee : addedAttendees) {
	        		if(meetingAttendees.containsKey(attendee)) {
	        			int count = meetingAttendees.get(attendee).size();
        	...
        	if(!toList.isEmpty()) {
        		sendNotificationAndMail(context, meetingId, toList, "Add");
        	}
        	
        	DomainRelationship Obj = DomainRelationship.newInstance(context,relId);			
	...
	/**
	 * Invoked by the 3DNotification trigger on removing attendee from Meeting
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args holds a String array containing the following entries: meetingID, attendeeId, attendeeType, relationshipId
	 * @throws Exception if operation fails
	 */
	public void send3DNotificationOnRemoveAttendee(Context context, String[] args) throws Exception {
		...
		DomainRelationship Obj = DomainRelationship.newInstance(context,relId);
		String notifyStatus = Obj.getAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE);
				
		if(!notifyStatus.startsWith("Notified"))
			return;
	    ...
	    if(attendeeType.equalsIgnoreCase(DomainConstants.TYPE_PERSON)) {
	    	...
	    	if(!meetingAttendees.containsKey(attendeeName))
				...
				if(meetingState.equals(STATE_MEETING_CREATE)) {
					List<String> relIdList = meetingAttendees.get(attendeeName);
					
					for(String relationId : relIdList) {
						if(relId.equals(relationId))
							continue;
						
						Obj = DomainRelationship.newInstance(context,relationId);
						String attributeValue = Obj.getAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE);
	
						/*
						 * attributeValue can have values like "Add", "Notified", "Notified|usrn1|usrn2|usrn3|......."
						 * where each usrn is the username of the Notified person from a user group
						 */
						
						/*
						 * To handle below scenario:
						 * UserGroup UG1 contains Person P1, P2, P3
						 * P1 added to meeting in Scheduled/In-Work (P1 relationship attribute set as 'Notified')
						 * Meeting demoted to Draft
						 * UG1 added to meeting in Draft & then P1 removed from meeting
						 * As P1 still has presence in meeting thru UG1, P1 is not notified for removal from meeting
						 * However, next if UG1 is removed, then as UG1 was added in Draft state, the relationship attribute 
						 * was not updated as "Notified" yet leading to no notification being sent to P1 about removal from meeting.
						 * **Meeting Attribute has type string with maxlength 0 meaning unlimited; tested vid 10000 chars
						 */
						try {
							if(attributeValue.equals("Notified"))
									continue;
							else if(attributeValue.startsWith("Notified")) {
								String temp[] = attributeValue.split("\\|");
								List<String> attribValueList = new ArrayList<String>(Arrays.asList(temp));
								attribValueList.remove(0);

								if(!attribValueList.contains(attendeeName)) {
									try {
										ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
						                Obj.setAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE, attributeValue+"|"+attendeeName);
						            } catch (Exception e) {
						                throw e;
						            } finally {
						                ContextUtil.popContext(context);
								...
								try {
									ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
					                Obj.setAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE, "Notified"+"|"+attendeeName);
					            } catch (Exception e) {
					                throw e;
					            } finally {
					                ContextUtil.popContext(context);
						...
						catch(Exception e) { System.out.println("Error updating Meeting Attribute."); e.printStackTrace(); }
					}
				}
	    	}
	    }
	    else {
	    	List<String> removedAttendees = getUserGroupAttendees(context, attendeeId, attendeeType);
	        
	    	String attribValues[] = notifyStatus.split("\\|");
	    	
	    	HashSet<String> notifiedAttendees = new HashSet<String>();
	    	
	    	for(int i=1; i<attribValues.length; ++i)
	    		notifiedAttendees.add(attribValues[i]);
			...
			for(String attendee : removedAttendees) {
				...
				if(count == 1 && (notifyStatus.equals("Notified") || notifiedAttendees.contains(attendee)))
					toList.add(attendee);		
				else {
					Meeting busMeeting = new Meeting(meetingId);
					busMeeting.open(context);
					...
					if(meetingState.equals(STATE_MEETING_CREATE)) {
						...
						for(String relationId : relIdList) {
							...
							StringList relSelect = new StringList(DomainConstants.SELECT_FROM_TYPE);
							relSelect.add("attribute["+DomainConstants.ATTRIBUTE_MEETING_ATTENDEE+"]");
	
							Obj = DomainRelationship.newInstance(context,relationId);
							Hashtable data = Obj.getRelationshipData(context, relSelect); 
							
							StringList tempList = (StringList) data.get(DomainConstants.SELECT_FROM_TYPE);
							String fromType = (String) tempList.get(0);
							tempList = (StringList) data.get("attribute["+DomainConstants.ATTRIBUTE_MEETING_ATTENDEE+"]");
							String attributeValue = (String) tempList.get(0);
							...
							if(fromType.equalsIgnoreCase(DomainConstants.TYPE_PERSON)) {
								if(!attributeValue.equals("Notified")) {
									try {
										ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
						                Obj.setAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE, "Notified");
								...
								try {
									if(attributeValue.equals("Notified"))
										...
										if(!attribValueList.contains(attendee)) {
											try {
												ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
								                Obj.setAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE, attributeValue+"|"+attendee);
								            } catch (Exception e) {
								                throw e;
								            } finally {
								                ContextUtil.popContext(context);
										...
										try {
											ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), null, context.getVault().getName());
							                Obj.setAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_ATTENDEE, "Notified"+"|"+attendee);
							            } catch (Exception e) {
							                throw e;
							            } finally {
							                ContextUtil.popContext(context);
		...
		if(!toList.isEmpty()) {
			sendNotificationAndMail(context, meetingId, toList, "Remove");	
		}	
		
	}
	
	/**
	 * Invoked by the 3DNotification trigger on adding attendees as Co-owner to Meeting
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args holds a HashMap containing meetingId, list of co-owners added
	 * @throws Exception if operation fails
	 */
	public void send3DNotificationOnAddCoowners(Context context, String args[]) throws Exception {
	
		HashMap programMap = (HashMap)JPO.unpackArgs(args);
        String meetingId = (String)programMap.get("meetingId");
        StringList coowners = (StringList)programMap.get("coowners");        
		
		List<String> toList = new ArrayList<String>(coowners.toList());
		
		sendNotificationAndMail(context, meetingId, toList, "Add Co-owner");
		
	}

	/**
	 * Invoked by any of the 3DNotification public methods to fetch & prepare Meeting info
	 * which is to be sent to respective method of MeetingNotification.java for further processing
	 * @param context the eMatrix <code>Context</code> object.
	 * @param meetingId holds the id of Meeting
	 * @param toList holds the list of attendee username to whom 3DNotification is to be sent
	 * @param event holds the type of 3DNotification event for Meeting such as Promote, Add, Remove
	 * 
	 */
	private void sendNotificationAndMail(Context context, String meetingId, List<String> toList, String event) throws Exception {
		
		DomainObject meetingObj = DomainObject.newInstance(context, meetingId);
		...
		slBusSelect.add(DomainConstants.SELECT_ORIGINATOR);
		slBusSelect.add(DomainConstants.SELECT_CURRENT);
		
		Map mapObjectInfo = meetingObj.getInfo(context, slBusSelect);

		String physicalId = (String) mapObjectInfo.get(DomainConstants.SELECT_PHYSICAL_ID);
		String title = (String) mapObjectInfo.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
		String desc = (String) mapObjectInfo.get(DomainConstants.SELECT_DESCRIPTION);
		String owner =  (String) mapObjectInfo.get(DomainConstants.SELECT_OWNER);
		String originator = (String) mapObjectInfo.get(DomainConstants.SELECT_ORIGINATOR);
		String currentState = (String) mapObjectInfo.get(DomainConstants.SELECT_CURRENT);
		
		String meetingDateTime = meetingObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_STARTDATETIME);
		String duration = meetingObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_DURATION);        		
		String location = meetingObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_MEETING_LOCATION);
		
		Map<String, String> meetingInfo = new HashMap<>();
		meetingInfo.put(MeetingNotification.MEETING_ID, physicalId); //Physical ID needed; Meeting object ID lands on Meeting summary page
		meetingInfo.put(MeetingNotification.MEETING_TITLE, title); 
		meetingInfo.put(MeetingNotification.CURRENT_STATE, currentState);
		meetingInfo.put(MeetingNotification.MEETING_DESC, desc);
		meetingInfo.put(MeetingNotification.OWNER_NAME, owner);
		meetingInfo.put(MeetingNotification.MEETING_LOCATION, location);
		...
		for(String attendee: toList) {
			tempList = new ArrayList<String>();
			tempList.add(attendee);
			
			String dateTime = getFormattedDisplayMeetingStartDateTimeForUser(context, attendee, meetingDateTime, userTZOffset);
			dateTime = dateTime.replace("\u202F", " "); //Replacing Narrow Non-Breaking space character (introduced in Java 21) with space as breaks the Date String processing
			String startDate = getStartDate(context, attendee, dateTime, userTZOffset);
			String startTime = getStartTime(dateTime);
			String endTime = calcEndTime(dateTime, duration);

			meetingInfo.put(MeetingNotification.MEETING_DATE, startDate);
			meetingInfo.put(MeetingNotification.MEETING_START_TIME, startTime);
			meetingInfo.put(MeetingNotification.MEETING_END_TIME, endTime);
			
			if(event.indexOf("Promote") != -1) {
				//To identify whether notification is to be sent to new OR notified attendees
				if(event.equals("PromoteNotified"))
					meetingInfo.put("NOTIFIED", "true");
				notifyObj.sendMeetingPromote3DNotification(context, meetingInfo, tempList);
			}
			else if(event.equals("Add"))
				notifyObj.sendMeetingAddAttendee3DNotification(context, meetingInfo, tempList);
			else if(event.equals("Remove"))
				notifyObj.sendMeetingRemoveAttendee3DNotification(context, meetingInfo, tempList);
			else if(event.equals("Add Co-owner"))
				notifyObj.sendMeetingAddCoowner3DNotification(context, meetingInfo, tempList);
		}
	}

	/**
	 * Method to get start date of Meeting in standard format
	 */
	private String getStartDate(Context context, String user, String meetingDate, double userTZOffset) throws FrameworkException {
		
		try {
			/*
			 * MMM is Month, dd is Day, yyyy is year, w is Week number, E is day name
			 * Eg. Jul 09 2023 27 Fri
			*/
			SimpleDateFormat dateFormat = new SimpleDateFormat("MMM dd yyyy w E");
			
			Date startDateTime = eMatrixDateFormat.getJavaDate(meetingDate);
			String startDate = dateFormat.format(startDateTime);
			
			SimpleDateFormat currDateFormat = new SimpleDateFormat("MM/dd/yyyy hh:mm:ss a");
			Date today = new Date();
			String todayDate = currDateFormat.format(today);
			String currDate = getFormattedDisplayMeetingStartDateTimeForUser(context, user, todayDate, userTZOffset);
			currDate = currDate.replace("\u202F", " "); //Replacing Narrow Non-Breaking space character (introduced in Java 21) with space as breaks the Date String processing
			today = eMatrixDateFormat.getJavaDate(currDate);
			String currentDate = dateFormat.format(today);
			
			String startDateData[] = startDate.split(" ");
			String currentDateData[] = currentDate.split(" ");			
			
			String startDateMonth = startDateData[0];
			String currentDateMonth = currentDateData[0];			
			
			String startDateDay = startDateData[1];
			String currentDateDay = currentDateData[1];
			
			String startDateYear = startDateData[2];
			String currentDateYear = currentDateData[2];
			
			int startDateWeek = Integer.parseInt(startDateData[3]);
			int currentDateWeek = Integer.parseInt(currentDateData[3]);
			
			String day = startDateData[4];
			...
			cal.add(Calendar.DATE,-2); //Get previous day's Date; -2 as we stored next day in previous step
			...
			if(startDate.equalsIgnoreCase(currentDate))
				displayDate = "Today";
			else if(startDate.equalsIgnoreCase(tomorrowDate))
				displayDate = "Tomorrow";
			else if(startDate.equalsIgnoreCase(yesterdayDate))
				displayDate = "Yesterday";
			else if(startDateYear.equalsIgnoreCase(currentDateYear)) {
				displayDate = startDateMonth + " " + startDateDay;
				
				if(startDateWeek == currentDateWeek)
	                displayDate = day + ", " + displayDate;
			}
			else
				displayDate = startDateMonth + " " + startDateDay + " " + startDateYear;
	...
	/**
	 * Method to get start time of Meeting in standard format
	 */
	private String getStartTime(String meetingDate) throws FrameworkException {
	...
	/**
	 * Method to calculate end time of Meeting by adding the duration to the start date
	 */
	private String calcEndTime(String meetingDate, String duration) throws FrameworkException  {
		
		try {
			Date startDateTime = eMatrixDateFormat.getJavaDate(meetingDate);
			
			Date updatedDate = new Date(startDateTime.getTime()
								+ ((int) Math.round(Double.parseDouble(duration)) * ONE_MINUTE_IN_MILLIS));
			
			SimpleDateFormat dateFormat = new SimpleDateFormat(eMatrixDateFormat.getEMatrixDateFormat(),Locale.US);
			String endDateString = dateFormat.format(updatedDate);
			
			Date endDateTime = dateFormat.parse(endDateString);
	...
	/**
	 * Method to fetch the unique map of Meeting Attendee & list of respective attendee relationship to Meeting
	 */
	private HashMap<String, List<String>> getMeetingAttendeesAndRelId(Context context, String meetingId, boolean selectPersonAndGroup) throws Exception {
		
		String typeGroupProxy = PropertyUtil.getSchemaProperty(context,"type_GroupProxy");
		
		Pattern patternType = new Pattern(DomainConstants.TYPE_GROUP);
        patternType.addPattern(typeGroupProxy);
		
		if(selectPersonAndGroup)
        ...
        HashMap<String, List<String>> attendees = new HashMap<String, List<String>>();
        
        //Iterating the maplist to get the Attendee's detail
        DomainObject busMeeting     = DomainObject.newInstance(context,meetingId);
        
    	//To get all members connected to the Meeting
        MapList mapPerson = busMeeting.getRelatedObjects(context, patternRelationship.getPattern(), 
        								patternType.getPattern(), typeSelects, relSelects,
        								true, false, //boolean getTo, boolean getFrom 
                                        (short)1, "", "", //short recurseToLevel, expandTypeWhere, String relationshipWhere,
                                        null, null, null); //Pattern includeType, Pattern includeRelationship
        
        Iterator personMapItr = mapPerson.iterator();
        
        while (personMapItr.hasNext())
        {
        	Map map                     = (Map)personMapItr.next();
            String sRelationshipId      = (String) map.get(busMeeting.SELECT_RELATIONSHIP_ID);
            String sAttendeeName        = (String) map.get(busMeeting.SELECT_NAME);
            String sAttendeeType        = (String) map.get(busMeeting.SELECT_TYPE);
            String sAttendeeId          = (String) map.get(busMeeting.SELECT_ID);
            ...
            if(sAttendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || sAttendeeType.equalsIgnoreCase(typeGroupProxy)){
            	...
            	while(membersItr.hasNext()) {
            		...
            		String attendeeName = ((String) member.get(DomainConstants.SELECT_NAME)); 
            		
            		List<String> relIds = new ArrayList<String>();
            		
            		if(attendees.containsKey(attendeeName)) {
            			relIds = attendees.get(attendeeName);
            			relIds.add(sRelationshipId);
            		}
            		else
            			relIds.add(sRelationshipId);
            		
            		attendees.put(attendeeName, relIds);
            	}
            }
            else {
            	List<String> relIds = new ArrayList<String>();
        		...
        		if(attendees.containsKey(sAttendeeName)) {
        			relIds = attendees.get(sAttendeeName);
        			relIds.add(sRelationshipId);
        		}
        		else
        			relIds.add(sRelationshipId);
        		
        		attendees.put(sAttendeeName, relIds);
            }
        }   	
	...
	/**
	 * Method to get all the relationship id to a Meeting
	 */
	private HashMap<String,String> getMeetingRelIds(Context context, String meetingId) throws Exception {
        ...
        HashMap<String,String> relIdAttribMap = new HashMap<String,String>();
    	...
    	//To get all members connected to the Meeting
        MapList mapPerson = busMeeting.getRelatedObjects(context, patternRelationship.getPattern(), 
        								patternType.getPattern(), null, relSelects,
        								true, false, //boolean getTo, boolean getFrom 
                                        (short)1, "", "", //short recurseToLevel, expandTypeWhere, String relationshipWhere,
                                        null, null, null); //Pattern includeType, Pattern includeRelationship
        ...
        while (personMapItr.hasNext()) {
        	Map map                     = (Map)personMapItr.next();
            String sRelationshipId      = (String) map.get(busMeeting.SELECT_RELATIONSHIP_ID);
            String sRelAttrib      		= (String) map.get("attribute["+DomainConstants.ATTRIBUTE_MEETING_ATTENDEE+"]");
            relIdAttribMap.put(sRelationshipId, sRelAttrib);
        }    	

        return relIdAttribMap;  
	}
	
	/**
	 * Method to fetch all the members of a User Group
	 */
	private List<String> getUserGroupAttendees(Context context, String attendeeId, String attendeeType) throws Exception {
		
		String typeGroupProxy = PropertyUtil.getSchemaProperty(context,"type_GroupProxy");
		
		List<String> toList = new ArrayList<String>();
		
		if(attendeeType.equalsIgnoreCase(DomainConstants.TYPE_GROUP) || attendeeType.equalsIgnoreCase(typeGroupProxy)) {		
			DomainObject group = new DomainObject(attendeeId);
			...
			while(membersItr.hasNext()) {
	    		Map member = (Map) membersItr.next();    		
	    		String name = (String) member.get(DomainConstants.SELECT_NAME);
	    		toList.add(name);
	    	}
		}
