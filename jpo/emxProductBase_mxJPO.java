  ...
  */
  ...
  /**
    ...
    */
	...
	private static final String derivedAbstractToSelectable = "to[" + DerivationUtil.RELATIONSHIP_DERIVED_ABSTRACT + "].id";
	private static final String mainDerivedToSelectable = "to[" + DerivationUtil.RELATIONSHIP_MAIN_DERIVED + "].id";
	private static final String derivedToSelectable = "to[" + DerivationUtil.RELATIONSHIP_DERIVED + "].id";
    /**
    * Default Constructor.
    *
  ...
  * @param args holds no arguments
    ...
    */
    protected MapList findProducts(Context context, String whereCondition, String ownerCondition)  throws Exception
    {
        ...
        String SELECT_PRODUCTS_FROM_NAME =  "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS+ "].from.name"; 
        objectList.add(SELECT_PRODUCTS_FROM_NAME);
      ...
      sbWhereExp.append(strCompany);
        ...
        //MapList relBusObjList = findObjects(context,strType,null,null,ownerCondition,null,DomainConstants.EMPTY_STRING,true,objectList);
        
        //For Bad Query issue, findObjects() API replaced with getRelatedObjects() with some changes. 
        DomainObject companyObj = new DomainObject(strCompany);
        String strObjWhere = null;
        if(ProductLineCommon.isNotNull(ownerCondition)){
        	strObjWhere = "owner == "+ownerCondition;
        }
        String rels = ProductLineConstants.RELATIONSHIP_COMPANY_PRODUCT+","+ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY;
        MapList relBusObjList = companyObj.getRelatedObjects(context,rels,ProductLineConstants.TYPE_PRODUCTS,objectList,null,false,true,(short)0,strObjWhere,null,(short)0,false,
                true,(short)0,null,null,null,null);
        
        StringList lstPVSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, ProductLineConstants.TYPE_PRODUCT_VARIANT);
        Map currentMap = null;
        for(int i = 0; i < relBusObjList.size(); i++)
        {
        	currentMap = (Map)relBusObjList.get(i);
        	String strObjType = (String)currentMap.get(DomainConstants.SELECT_TYPE);
        	if(lstPVSubTypes.contains(strObjType)){
        		relBusObjList.remove(currentMap);
        		i--;
        	}
        }
...
/**
    ...
    public int checkPreviousRevisionForProductPromote(Context context, String args[])  throws Exception
    {
        ...
        String objectId = args[0];
       	...
       	if (ProductLineCommon.isNotNull(strParentId)) {
       		...
       		if(domObj.isKindOf(context, ProductLineConstants.TYPE_PRODUCTS)){
       			// TODO - Need to revisit the below code 
       			if (!"Release".equalsIgnoreCase(strState) && !"Obsolete".equalsIgnoreCase(strState)) {
                    String language = context.getSession().getLanguage();
                    //Alert message formulated to display the error message.
       		...
       		else if (!Product.isFrozenState(context, strParentId)) {
                String language = context.getSession().getLanguage();
                //Alert message formulated to display the error message.
                String strAlertMessage = EnoviaResourceBundle.getProperty(context,SUITE_KEY,"emxProduct.Alert.DerivationStateCheckFailed",language);
                //Alert message is explicitly thrown to the front end.
    ...
    */
    ...
    else
    {
    	...
    	mArgs.put("paramString",strOldProjectId);
    	String[] newArgs = JPO.packArgs(mArgs);
        /*bHasReadAccess = ${CLASS:emxProduct}.hasAccessOnProject(context,strOldProjectId);*/
    ...
    */
    ...
    protected List currentPhaseProcess(
        ...
        StringList lstProdSelects = new StringList(sbGovProjSelect.toString());
        ...
        StringList lstTaskObjSelects = new StringList();
  ...
  /**
    ...
    public List getGoverningProject(
        ...
        StringList lstProdSelects = new StringList(sbGovProjIdSelect.toString());
    ...
    public MapList getProductsForRoadmap(
        ...
        String RowIds = (String) programMap.get("RowIds");
        ...
        String[] strSourceObjectId = RowIds.split(",");
    ...
    public Vector getGoverningProjectIcon(Context context, String[] args)
        ...
        StringList lstObjectSelects = new StringList(sbSelectGoverning.toString());
    ...
    try{
                ...
                /*lstAllObjList = ${CLASS:emxPLCCommonBase}.setLevelsInMapList((MapList)lstAllObjList,ProductLineConstants.TYPE_FEATURE_LIST);*/
                Map mapArgs = new HashMap();
        		Map pMap = new HashMap();
        		pMap.put("relObjIdList", (MapList)lstAllObjList);
        		pMap.put("strIntermediateTypes", ProductLineConstants.TYPE_FEATURE_LIST);
        		mapArgs.put("paramMap", pMap);
        		String[] newJPOArgs = JPO.packArgs(mapArgs);
        		lstAllObjList = (List) JPO.invoke(context, "emxPLCCommonBase", null, "setLevelsInMapList", newJPOArgs, List.class);
    ...
    **/
        ...
        public List getLevel(Context context,String[] args) throws Exception
            {
                StringList lstLevel = new StringList();
         ...
         public MapList getAllProjects(
        		...
        		try{
        	      ...
        	      //  sbWhereExpression.append(", ");
        	        ...
        	        lstMultiValueList.add("to[" +ProductLineConstants.RELATIONSHIP_RELATED_PROJECTS + "].from.type");
        	        
        	        
        	        if(!lstMultiValueList.contains("to[" +ProductLineConstants.RELATIONSHIP_RELATED_PROJECTS + "].from.id")){
        	        	lstMultiValueList.add("to[" +ProductLineConstants.RELATIONSHIP_RELATED_PROJECTS + "].from.id");						
					}
        	       ...
        	       // if a subproject is connected to Product, it has "related projects relationship" so it will comming in result
        			...
        			StringList slExclusionTypes = new StringList();
        			boolean isNotsubproject = true;
        			boolean isRoadMapProject = true;
        			String strExclusionType = EnoviaResourceBundle.getProperty(context,"emxProduct.RoadMapProject.ExcludeTypes");
            	    if(ProductLineCommon.isNotNull(strExclusionType) && !strExclusionType.equals("")){
            			StringList exclusionTypes = FrameworkUtil.splitString(strExclusionType,",");
            			for(int i=0;i<exclusionTypes.size();i++){
            				StringList exclusionSubTypes = ProductLineUtil.getChildTypesIncludingSelf(context, PropertyUtil.getSchemaProperty(context,(String) exclusionTypes.get(i)));
            				slExclusionTypes.addAll(exclusionSubTypes);
            			}            	    	
            	    }
        			...
        			for (Iterator iterator = tempList.iterator(); iterator
						...
						if(ProductLineCommon.isNotNull(strType) && slExclusionTypes.contains(strType)){
							isRoadMapProject = false;
						}

						if(isNotsubproject && isRoadMapProject){
							projList.add(objMap);
						}
...
/*        	 if (MULTI_VALUE_LIST.contains("to["+ProductLineConstants.RELATIONSHIP_RELATED_PROJECTS+"].from.id")){
    ...
    protected static List getStringList(MapList lstProjectList,String strKey){
        StringList lstIdList  = new StringList();
     ...
     */
        ...
        StringList lstProjSelects = new StringList();
     ...
     */
                            ...
                            String[] args) throws Exception
        {
            ...
            StringList lstProgSelects = new StringList();
    ...
    public List getSelectedTasks(Context context, String[] args)
        ...
        StringList lstObjSelects = new StringList(DomainConstants.SELECT_TYPE);
  ...
  public Map getRelAttribsRange(Context context, String[] args,String strAttributeName) throws Exception
   ...
   /** This method gets the object Structure List for the context Product object.This method gets invoked
  	    ...
  	    String derivationType = null;
   	    if (objectType != null && strRelPattern.equals(ProductLineConstants.RELATIONSHIP_PRODUCTS)) {
            ...
            try {
             ...
             /*productStructList = (MapList) ${CLASS:emxPLCCommon}.getStructureListForType(context, args);*/
             productStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
        }
    ...
    public List getProjectProgram(
        ...
        StringList lstProjSelects = new StringList(sbProgIdSelect.toString());
...
/*End of Add by Sandeep, Enovia MatrixOne for Bug # 310542*/
     ...
     * This method is used to return the boolean True or False for the Access on the Project of the context user.
     * @param context The ematrix context object.
     * @param strProjectId String packed in JPO Args containing the ID of the Project.
     * @return boolean true or false
     * @since JPOToJARConversion
     */
    public static boolean hasAccessOnProject(Context context,String[] strProjectIdInArgs) throws Exception
    {
    	Map paramMap = (HashMap) JPO.unpackArgs(strProjectIdInArgs);
    	String strProjectId = (String) paramMap.get("paramString");
        return hasAccessOnProject(context, strProjectId);
    }
    ...
    */
    public String getHTMLForGovProject(Context context, String[] args) throws Exception{
        ...
        String reportFormat = (String)mpRequest.get("reportFormat");
            ...
            !strMode.equalsIgnoreCase("null") && strMode.equalsIgnoreCase("edit"))
        {
            ...
            try
            {
                if (strGovProjId==null || strGovProjId.equals("null") || strGovProjId.equals("") )
                	...
                	/*bHasReadAccess = ${CLASS:emxProduct}.hasAccessOnProject(context,strGovProjId);*/
                	Map mArgs = new HashMap();
                	mArgs.put("paramString",strGovProjId);
                	String[] newArgs = JPO.packArgs(mArgs);
                	bHasReadAccess = (boolean) JPO.invoke(context, "emxProduct", null, "hasAccessOnProject", newArgs, Boolean.class);
                }                    
            } catch (Exception e) {
            ...
            if (!bHasReadAccess && "edit".equalsIgnoreCase(strMode))
            {
...
//                sbBuffer.append("OID");
            ...
            if(strGovProjName!=null && !strGovProjName.equals("") && !strGovProjName.equalsIgnoreCase("null")) {
                ...
                if("CSV".equalsIgnoreCase(reportFormat)){
                	sbBuffer.append(strGovProjName);
                	return sbBuffer.toString();
                }else if(!bHasReadAccess && "edit".equalsIgnoreCase(strMode)) {
                	...
                	sbHref.append("<A HREF=\"../common/emxTree.jsp?objectId=");
                    sbHref.append(XSSUtil.encodeForHTMLAttribute(context,strGovProjId));
                    sbHref.append("&mode=replace");
                    sbHref.append("&AppendParameters=true");
                    sbHref.append("&reloadAfterChange=true");
                    sbHref.append("\" target=\"_blank\" class=\"object\">");
  ...
  throws Exception {
	  int result =0;
	  // if this is an IPMLCommandName 3dxml Import, skip it
      String IPMLCommandName = PropertyUtil.getGlobalRPEValue(context, "IPMLCommandName");
      if("Import".equalsIgnoreCase(IPMLCommandName)){return 0;}      
   ...
   *   "cannot access jakarta.servlet.ServletContext".
 ...
 {	 		
	...
	public void copyProductAttributesToModel(Context context,String[] args) throws Exception {
		...
		if (mpProd!=null && mpProd.size()!=0) {
		    ...
		    strListCopyDetails.add(DomainObject.SELECT_PROJECT);
		    strListCopyDetails.add("altowner1");
			Map mapModAttr = domProd.getInfo(context, strListCopyDetails);
			String strObjProject = (String) mapModAttr.get(DomainObject.SELECT_PROJECT);
		    String strObjectOrg = (String) mapModAttr.get("altowner1");
		    System.out.println("----------------> In copyProductAttributesToModel");
			mapModAttr.remove(DomainObject.SELECT_ID);
			mapModAttr.remove(DomainObject.SELECT_TYPE);
			mapModAttr.remove(DomainObject.SELECT_PROJECT);
			mapModAttr.remove("altowner1");
			...
			for (int j=0; j<mLstModelRelatedData.size();j++) {
				...
				if (strDirection.equalsIgnoreCase("to")) {
        			System.out.println("Connecting to relationship: " + strRelName + " rtConRelType.toString(): " + rtConRelType.toString() + " ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY: " +  ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY);
					if(!ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY.equalsIgnoreCase(rtConRelType.toString())) {
			...
			domModel.setPrimaryOwnership(context, strObjProject, strObjectOrg, false);
	...
	throws Exception{
		...
		DomainObject dom = new DomainObject(strNewOrganizationId);
		if(!dom.exists(context)){
			 StringBuffer where = new StringBuffer(16);
		   	 where.append("name=='").append(strNewOrganizationId).append('\'');
		   	 MapList organizationList = DomainObject.findObjects(context,DomainConstants.TYPE_ORGANIZATION,DomainConstants.QUERY_WILDCARD,where.toString(),StringList.create(DomainConstants.SELECT_ID));
		   	 Map organizationMap= (Map)organizationList.get(0);
		   	 strNewOrganizationId = (String)organizationMap.get(DomainConstants.SELECT_ID);
		}
     ...
     */
     public Vector getColumnModelValues(Context context, String[] args) throws Exception {
		...
		String SELECT_MAIN_PRODUCT_FROM_NAME =  "to[" + ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT + "].from.name";
		String SELECT_PRODUCTS_FROM_NAME =  "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS+ "].from.name"; 

         // Process for each row in the table
         for (int i = 0; i < lstProductList.size(); i++) {
			if(((Map) lstProductList.get(i)).containsKey(SELECT_PRODUCTS_FROM_NAME) ||
					((Map) lstProductList.get(i)).containsKey(SELECT_MAIN_PRODUCT_FROM_NAME)){
				String strModelName = "";
				if(((Map) lstProductList.get(i)).containsKey(SELECT_PRODUCTS_FROM_NAME)){
					strModelName = (String) (((Map) lstProductList.get(i))).get(SELECT_PRODUCTS_FROM_NAME);
				}
				else if (((Map) lstProductList.get(i)).containsKey(SELECT_MAIN_PRODUCT_FROM_NAME)){
					strModelName = (String) ((Map) lstProductList.get(i)).get(SELECT_MAIN_PRODUCT_FROM_NAME);
				}
				else{
					strModelName = "";
				}
     ...
     */
     public Vector getColumnDerivationType(Context context, String[] args) throws Exception {
         ...
         for (int i = 0; i < lstProductList.size(); i++) {
     	    // Get the object ID
			//oidsArray[i] = (String) ((Map) lstProductList.get(i)).get(DomainConstants.SELECT_ID);
			String objType = ((Map) lstProductList.get(i)).get(DomainConstants.SELECT_TYPE).toString();
			if(ProductLineUtil.getChildTypesIncludingSelf(context, ProductLineConstants.TYPE_PRODUCTS).contains(objType)){
			if(((Map) lstProductList.get(i)).containsKey("relationship")){
				String relType = ((Map) lstProductList.get(i)).get("relationship").toString();
				if(ProductLineConstants.RELATIONSHIP_DERIVED.equals(relType)){
					columnValues.add(strDerivationType);
				} else if (ProductLineConstants.RELATIONSHIP_MAIN_DERIVED.equals(relType)) {
					columnValues.add(strRevisionType);
				} else {
					columnValues.add("");
				}
			}else{
				String objId = ((Map) lstProductList.get(i)).get(DomainConstants.SELECT_ID).toString();
				DomainObject domObject = new DomainObject(objId);
				String revisionDerivationInfo = domObject.getInfo(context, "to[" + DerivationUtil.RELATIONSHIP_DERIVED_ABSTRACT + "].type",false);
				if(ProductLineConstants.RELATIONSHIP_DERIVED.equals(revisionDerivationInfo)){
         	   columnValues.add(strDerivationType);
				} else if (ProductLineConstants.RELATIONSHIP_MAIN_DERIVED.equals(revisionDerivationInfo)) {
         	   columnValues.add(strRevisionType);
            } else {
         	   columnValues.add("");
            }
                 ...
                 if (reportFormat != null && "CSV".equalsIgnoreCase(reportFormat)) {
 	            	  ...
 	            	  strNewIcon =
 	                         "<img src=\"../common/images/"
 	                                ...
 	                                + "TITLE=\""
 	                                + " "
 	                                + strTooltipDerivationCue//XSSOK
 	                                + "\""
 	                                + "/>";
 	             }
		//String[] oidsArray = new String[lstProductList.size()];
		for (int i = 0; i < lstProductList.size(); i++) {
			// Get the object ID			
			//oidsArray[i] = (String) ((Map) lstProductList.get(i)).get(DomainConstants.SELECT_ID);
			String objType = ((Map) lstProductList.get(i)).get(DomainConstants.SELECT_TYPE).toString();
			...
			if(((Map) lstProductList.get(i)).containsKey("relationship")){
				...
				DomainObject domObject = new DomainObject(objId);
				String revisionDerivationInfo = domObject.getInfo(context, "to[" + DerivationUtil.RELATIONSHIP_DERIVED_ABSTRACT + "].type",false);
				if (null!=revisionDerivationInfo &&  ProductLineConstants.RELATIONSHIP_DERIVED.equals(revisionDerivationInfo)) {			
					columnValues.add(strNewIcon);
             } else {
          	     columnValues.add("");
             }
    ...
    */
    public Vector getColumnLatestCue(Context context, String[] args) throws Exception {
        ...
        StringList slObjSelects1 = new StringList();
        slObjSelects1.add(ProductLineConstants.SELECT_CURRENT);
        slObjSelects1.add("from["+ProductLineConstants.RELATIONSHIP_MAIN_DERIVED+"].to.current");
        slObjSelects1.add(derivedAbstractToSelectable);
        ...
        MapList mpProdList = DomainObject.getInfo(context, oidsArray , slObjSelects1);
               
        Iterator itrMpProdList = mpProdList.iterator();
        while(itrMpProdList.hasNext()){
            boolean isLatest = false;
            boolean isLatestReleased = false;
            
            Map revisionDerivationInfo = (Map) itrMpProdList.next();
            
            // If the current object is a Derivation, it does not get Latest or Latest Released Flag.
            if (revisionDerivationInfo.containsKey(mainDerivedToSelectable)) {
	               
	            //check if n+1 revision exist and its current state
	            if(ProductLineConstants.STATE_RELEASE
	            		.equals(revisionDerivationInfo.get(ProductLineConstants.SELECT_CURRENT).toString())){
	            	isLatestReleased = true;
	            	if (revisionDerivationInfo.containsKey("from["+ProductLineConstants.RELATIONSHIP_MAIN_DERIVED+"].to.current")) {
		            	String revisionState = revisionDerivationInfo.get("from["+ProductLineConstants.RELATIONSHIP_MAIN_DERIVED+"].to.current").toString();
		            	if(ProductLineConstants.STATE_RELEASE.equals(revisionState)){
                ...
                if (reportFormat != null && "CSV".equalsIgnoreCase(reportFormat)) {
                    ...
                    strNewIcon =
  	                       "<img src=\"../common/images/"
  	                               ...
  	                               + "TITLE=\""
  	                               + " "
  	                               + strTooltipLatestReleasedCue//XSSOK
  	                               + "\""
  	                               + "/>";
                }
                columnValues.add(strNewIcon);
    ...
    */
    public Vector getColumnDerivationLevel(Context context, String[] args) throws Exception {
        ...
        StringList lstSelect = new StringList(2);
        ...
        for (int j = 0; j < lstProductList.size(); j++) {
            ...
            String strMessage = "";
      	    if (ProductLineCommon.isNotNull(strDerivationLevel)) {
            	  ...
            	  if (stringKeyLevel.equalsIgnoreCase(strMessage)) {
    					Map mArgs = new HashMap();
    					mArgs.put("paramString", strType);
    					String[] newArgs = JPO.packArgs(mArgs);
    					String objectParentType = (String) JPO.invoke(context, "emxPLCCommon", null, "getParentType",
    							newArgs, String.class);
    					strSymbolicType = FrameworkUtil.getAliasForAdmin(context,DomainConstants.SELECT_TYPE, objectParentType, true);
    					stringKeyLevel = "DerivationLevel." + strSymbolicType + ".Display." + strDerivationLevel;
    					strMessage = EnoviaResourceBundle.getProperty(context,strSuiteKey,stringKeyLevel.toString(),context.getSession().getLanguage());					
    				}
            }
    ...
    */
    public Vector getColumnDerivedFrom(Context context, String[] args) throws Exception {
        ...
        // Add to list of Object Selectables.
        StringList lstDerivedSelects = new StringList(4);
		...
		lstDerivedSelects.add("to["+ProductLineConstants.RELATIONSHIP_MAIN_DERIVED+"].from.attribute[Title]");
		lstDerivedSelects.add("to["+ProductLineConstants.RELATIONSHIP_DERIVED+"].from.name");
        ...
        for (int j = 0; j < lstProductList.size(); j++) {
			...
			Map prdMap = ((Map)lstProductList.get(j));
			strType = (String)prdMap.get(DomainObject.SELECT_TYPE);
			
			if (!ProductLineUtil.getChildTypesIncludingSelf(context, ProductLineConstants.TYPE_PRODUCTS).contains(strType)) {
                columnValues.add("");
                continue;
			}else if(prdMap.get(strDerivedIdSelect) != null ){
				strDFId = (String) prdMap.get(strDerivedIdSelect);

				if(prdMap.get(strDerivedNameSelect) != null)
					strDFName = (String) prdMap.get(strDerivedNameSelect);

				if(strDFName.isEmpty() && prdMap.get("to[Derived].from.name") != null)
					strDFName = (String) prdMap.get("to[Derived].from.name");

				strDFRevision = (String) prdMap.get(strDerivedRevisionSelect);
			}else if(prdMap.get(strMainDerivedIdSelect) != null){
				strDFId = (String) prdMap.get(strMainDerivedIdSelect);

				if(prdMap.get(strMainDerivedNameSelect) != null)
					strDFName = (String) prdMap.get(strMainDerivedNameSelect);
        	
				if(strDFName.isEmpty() && prdMap.get("to[Main Derived].from.attribute[Title]") != null)
					strDFName = (String) prdMap.get("to[Main Derived].from.attribute[Title]");

				strDFRevision = (String) prdMap.get(strMainDerivedRevisionSelect);
			} else {
        	
				//Get the Derived From for all the Products
				BusinessObjectWithSelectList lstProductData = BusinessObject
						.getSelectBusinessObjectData(context, new String[]{(String)prdMap.get(DomainObject.SELECT_ID)}, (StringList) lstDerivedSelects);
      	    // Get the ID.  We check the Derived relationship first, and if not found, we'll
      	    // check the Main Derived select string.
				strDFId = lstProductData.getElement(0).getSelectData(strDerivedIdSelect);
				if (strDFId.isEmpty()) {
					strDFId = lstProductData.getElement(0).getSelectData(strMainDerivedIdSelect);
      	    }

      	    // Get the Name.  We check the Derived relationship first, and if not found, we'll
      	    // check the Main Derived select string.
				strDFName = lstProductData.getElement(0).getSelectData(strDerivedNameSelect);
				if (strDFName.isEmpty()) {
					strDFName = lstProductData.getElement(0).getSelectData(strMainDerivedNameSelect);
      	    }
				if(strDFName.isEmpty()){
					strDFName = lstProductData.getElement(0).getSelectData("to[Main Derived].from.attribute[Title]");
      	    }
				if(strDFName.isEmpty()){
					strDFName = lstProductData.getElement(0).getSelectData("to[Derived].from.name");
    	    }

      	    // Get the Revision.  We check the Derived relationship first, and if not found, we'll
      	    // check the Main Derived select string.

				strDFRevision = lstProductData.getElement(0).getSelectData(strDerivedRevisionSelect);
				if (strDFRevision.isEmpty()) {
					strDFRevision = lstProductData.getElement(0).getSelectData(strMainDerivedRevisionSelect);
      	    }
        ...
        try {
		    ...
		    if ( parentInfo == null ) {
				...
				String strId = (String)parentInfo.get(DomainObject.SELECT_ID);
  		        ...
  		        String strMarketingName = (String)parentInfo.get("attribute[Marketing Name]");
			    ...
			    derivedFromName.append("', '800', '700', 'true', 'popup')\">");
  				...
  				derivedFromName.append(XSSUtil.encodeForHTML(context, strMarketingName));
            ...
            String strSymbolicType = FrameworkUtil.getAliasForAdmin(context,DomainConstants.SELECT_TYPE, strType, true);
	 	    String strDerivationLevel = getFormDerivedAttribute(context, objectId, ProductLineConstants.SELECT_ATTRIBUTE_DERIVATION_LEVEL);
       	    ...
       	    if (ProductLineCommon.isNotNull(strDerivationLevel)) {
             	 ...
             	 if (stringKeyLevel.equalsIgnoreCase(derivationLevelValue)) {
  					Map mArgs = new HashMap();
  					mArgs.put("paramString", strType);
  					String[] newArgs = JPO.packArgs(mArgs);
  					String objectParentType = (String) JPO.invoke(context, "emxPLCCommon", null, "getParentType",
    ...
    */
	...
	public String createProductRevisionForEC(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			if (productMap != null && productMap.size() > 0) {
				...
				HashMap nodeAttrs = DerivationUtil.createDerivedNode(context, strDerivedFromID, strDerivationLevel, strType, strDerivationType);
	...
	public Map createProductDerivation(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			String strCompanyNameDisplay = (String) programMap.get("CompanyName");
			String strDesignResponsibilityDisplay = (String) programMap.get("DesignResponsibility");		//43469.25675.38908.52750 will show id when updated else Title
			String strCompanyNameDisplayOID = (String) programMap.get("CompanyNameOID");
			String strDesignResponsibilityDisplayOID = (String) programMap.get("DesignResponsibilityOID");	//Supplier1 will show Name when updated else empty
			String strCompanyNameActual = (String) programMap.get("CompanyNameDisplay");					
			String strDesignResponsibilityActual = (String) programMap.get("DesignResponsibilityDisplay");	//SupplierLIP will always show Title
			String strUpdatedCompanyId = "";
			String strUpdatedDesignResponsibilityId = "";
			
			//Check if the Company is updated while MV creation
			if(!("".equals(strCompanyNameDisplayOID) || strCompanyNameDisplayOID == null)) {
				System.out.println("Company Name is updated");
				strUpdatedCompanyId = strCompanyNameDisplay;
				
			}
			
			//Check if the Design Responsibility is updated while MV creation
			if(!("".equals(strDesignResponsibilityDisplayOID) || strDesignResponsibilityDisplayOID == null)) {
				System.out.println("Design Responsibility Name is updated");
				strUpdatedDesignResponsibilityId = strDesignResponsibilityDisplay;
			...
			String strparentOID = (String) programMap.get("modelID");
			...
			if (strDerivationLevel == null || (strDerivationLevel != null && strDerivationLevel.trim().isEmpty())) {//Setting Derivation Level to Level0 if null or not set
		...
		*/
		...
		String strProductId = "";
    		...
    		//------------------------------------------------------------
    		//START Modifications for Switching To Modeler's createProductDerivation API 
    		//------------------------------------------------------------
    		
    		com.dassault_systemes.enovia.productline.modeler.Product prod = new com.dassault_systemes.enovia.productline.modeler.Product();
			
			if ((strparentOID != null) && (strDerivedFromID == null || strDerivedFromID.trim().isEmpty())) {
				//Handles the creation of first(root) MV under existing Model
				Model m = new Model(strparentOID);
				System.out.println("------------------>In createRevisionableProduct");
				strProductId = m.createRevisionableProduct(context, strType, strName, strRevision, strPolicy,
						strCompanyId, strMarketingName, true);
				com.dassault_systemes.enovia.governance.modeler.GovernanceModelerCommon modelerGovProduct = new com.dassault_systemes.enovia.governance.modeler.GovernanceModelerCommon(
						strProductId);
				modelerGovProduct.setDetails(context, objAttributeMap);
			} else {
				//Handles two cases, the creation of:	a.model revision (when model exists(strparentOID is not null) and root MV exists(strDerivedFromID)	b.new MV under new Model
				System.out.println("------------------>In createProductDerivation");
				if(!("".equals(strUpdatedCompanyId))) {
					System.out.println("creating MV with updated strUpdatedCompanyId");
					strProductId = prod.createProductDerivation(context, strType, strName, strRevision, strPolicy, strOwner,
							strDescription, objAttributeMap, strDerivationType, strDerivedFromID, strDerivationLevel, strUpdatedCompanyId);
					
				}else {
					System.out.println("creating MV with non-updated strUpdatedCompanyId");
					strProductId = prod.createProductDerivation(context, strType, strName, strRevision, strPolicy, strOwner,
							strDescription, objAttributeMap, strDerivationType, strDerivedFromID, strDerivationLevel);
				}
			...
			DomainObject toObject = DomainObject.newInstance(context, strProductId);
	        //String strProductDSI	d = toObject.getInfo(context, "to["+ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY+"].from.id");
	        String strSelectable = "to[" + ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT + "].from.id";
	        StringList selectList = new StringList("to["+ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY+"].from.id");
	        selectList.add(strSelectable);
	        Map prdMap = toObject.getInfo(context, selectList);
	        String strProductDSId = (String) prdMap.get("to["+ProductLineConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY+"].from.id");
	        if(strparentOID == null){
	        	//Fetching Model Id from GetInfo of newly created Product if it is null. This is needed for connection of Design Responsibility
				strparentOID = (String) prdMap.get(strSelectable);
				}
	        //------------------------------------------------------------
    		//END Modifications for Switching To Modeler's createProductDerivation API
    		//------------------------------------------------------------
			//Connect the Design Responsibility on create of the Product Revison/Derivation
			if(!ProductLineCommon.isNotNull(strProductDSId))
			{
				DomainObject domParentObject = DomainObject.newInstance(context, strparentOID);
		        String strNewOrganizationId = domParentObject.getInfo(context, "altowner1");
				DomainObject dom = new DomainObject(strNewOrganizationId);
				if(!dom.exists(context)){
					 StringBuffer where = new StringBuffer(16);
				   	 where.append("name=='").append(strNewOrganizationId).append('\'');
				...
				if(!("".equals(strUpdatedDesignResponsibilityId) || strUpdatedDesignResponsibilityId == null)){
        			System.out.println("----------------->strUpdatedDesignResponsibilityId: " + strUpdatedDesignResponsibilityId);
					DomainObject fromObject = DomainObject.newInstance(context, strUpdatedDesignResponsibilityId);
					//Changing the context to super user
					ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), "", "");
					DomainRelationship.connect(
							context,
				...
				}else if(ProductLineCommon.isNotNull(strNewOrganizationId)){
					DomainObject fromObject = DomainObject.newInstance(context, strNewOrganizationId);
					//Changing the context to super user
					ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), "", "");
					DomainRelationship.connect(
							context,
	...
	public String connectProductDerivationToModel(Context context, String[] args)
		...
		try {
    		...
    		// PropertyUtil.setGlobalRPEValue(context,"CONTEXT_CREATE","True");
	...
	throws FrameworkException
    {
		...
		try {
			...
			String relId = new String();
			if(mapParentDetail!=null && mapParentDetail.containsKey(DomainRelationship.SELECT_ID)){
				relId = (String) mapParentDetail.get(DomainRelationship.SELECT_ID);
			}
	    ...
	    try {
			...
			if (ProductLineCommon.isNotNull(strProductLineId)) {
	            if (reportFormat.equals("DEFAULT")) {
		  	    	...
		  	    	sbHead.append("'");
		  	    	sbHead.append(", '800', '700', 'false', 'popup')\">");
    	            ...
    	            strProductLine = sbHref.toString();
		      	    ...
		      	    // Part 1: Print Plain Text of the name and revision.
		      	    strProductLine = strProductLineName;
                }
	       }
     ...
     public void createModelOnClone(Context context,String args[]) throws Exception
     {
    	try{
    		...
    		if(!cloneContext.equals("") && cloneContext!=null && cloneContext.equalsIgnoreCase("TRUE")){
		    		...
		    		int i;
		    		MapList mlNewObjectId = new DomainObject().findObjects(context, objectType, objectNewName, objectRev, "*", "*", "", false, selectId);
  ...
  public Map createModelAndConnectToProductLine(Context context, String[] args) throws Exception {
    ...
    /*
     * 
     * Visible only when model is accessible.
     */
    public boolean isVisibleCreateRevision(Context context, String[] args) throws Exception
    {
    	//The packed argument send from the JPO invoke method is unpacked to retrive the HashMap.
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        //The object id of the context object is retrieved from the HashMap using the appropriate key.
        String strObjectId = (String)  programMap.get("objectId");
        Product thisProduct = new Product(strObjectId);
		String strModelId = thisProduct.getModelId(context);
		//If model is not visible return false
		if(strModelId != null){
			return true;
		}
