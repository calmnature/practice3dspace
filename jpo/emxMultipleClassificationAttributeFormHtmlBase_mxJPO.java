...
public class emxMultipleClassificationAttributeFormHtmlBase_mxJPO extends
    ...
    protected Map _agNameTitle  = new HashMap();
    ...
    public String getFieldViewHTML(String attributeName, String attributeValue)
        ...
        String attributeNameFromPropertiesFile = EnoviaResourceBundle.getProperty(_context,
                "emxFrameworkStringResource",new Locale(_lang),attributeProperty);
...
//      field.put("field_display_value", attributeValue);
        field.put("label", attributeNameFromPropertiesFile);
    ...
    public String getFieldEditHTML(String attributeName, String attributeValue)
		...
		// if value type of attribute is range value, setting validate function
		if(attribType.isRangeVal()) {
			if ("real".equalsIgnoreCase(dataType) || "integer".equalsIgnoreCase(dataType) ) {
				settings.put(SETTING_VALIDATE, "checkRangeValue");
			}
		}
...
//      field.put("field_display_value", attributeValue);
    ...
    public String getClassificationHeadingHTML(String interfaceName) {
    	...
    	try {
    		classificationName = new DomainObject(interfaceName).getInfo(_context, SELECT_ATTRIBUTE_TITLE);
		} catch (Exception e) {}
    	if(classificationName == null || classificationName.trim().isEmpty())
        	classificationName = (String) ((StringList) _if2clsInfo.get(interfaceName)).get(1);
        StringBuffer buf = new StringBuffer();
        buf.append("<TR><TD colspan='");
        buf.append(_maxCol);
        buf.append("' class='heading1'>");
        buf.append(XSSUtil.encodeForXML(_context, classificationName));
        buf.append("</TD></TR>");
        return buf.toString();
    }
     ...
     * @throws Exception if the operation fails
     */
    public String getAttributeGroupHeadingHTML(String attributeGroupName) throws FrameworkException {
        String name;
        if(LibraryUtil.isInterfaceVisible(_context, attributeGroupName)){ //bae3 hiddenAG
	        try {
        	...
        	name = EnoviaResourceBundle.getProperty(_context, "emxLibraryCentralStringResource", new Locale(_lang),"emxLibraryCentral.Common.ClassificationAttributes");//bae3 hiddenAG
        }
        StringBuffer buf = new StringBuffer();
        buf.append("<TR><TD colspan='");
        buf.append(_maxCol);
        buf.append("' class='heading2'>");
        buf.append(getAttributeGroupTitle(name));
        buf.append("</TD></TR>");
        return buf.toString();
    }
    private String getAttributeGroupTitle(String attributeGroupName) throws FrameworkException {
        String agTitle = "";
        for (Object key : _agNameTitle.keySet()) {
        	if(key.equals(attributeGroupName)) {
        		agTitle = (String) _agNameTitle.get(key);
        	}
            
        }
        if(UIUtil.isNullOrEmpty(agTitle)) {
        	agTitle = attributeGroupName;
        }
		return agTitle;
    }
    /**
    ...
    public String getAttributeGroupHTML(String mode, String attributeGroupName)
        ...
        if (!access.equals("#DENIED!") && !access.isEmpty()) // bae3 lets not show only headers
        {
        StringBuffer buf = new StringBuffer(
...
//              ${CLASS:emxLibraryCentralUtil}.commaPipeQueryToMap(_context, 
...
//                                                              "|"
//                                                              );
              //Changes done to hide the hidden attributes on UI for properties page of IP Security Classes.
              StringList selectables  = new StringList();
              paramMap.put("classItf", allAttributeGroups.get(i));
              paramMap.put("selectData", selectables);
              String[] params = JPO.packArgs(paramMap);
              MapList classificationAttributes = JPO.invoke(context,LibraryCentralResourceConstants.CLASSIFICATION_ATTRIBUTES_JPO, null,"getAttributeDetailsOfClassItf",params,MapList.class);
              StringList attributeNameList = new StringList();
              for (int j = 0; j < classificationAttributes.size(); j++) {
                  HashMap attribute = (HashMap) classificationAttributes.get(j);
                  attributeNameList.add((String) attribute.get("name"));
              }
              _ag2attrs.put(allAttributeGroups.get(i), attributeNameList);
          }
   ...
   /**
    ...
    public void updateFields(Context context, String[] args) throws Exception {
		...
		while (it2.hasNext()) {
			...
			if (key.startsWith("#") && key.indexOf(",units_")==-1 && key.indexOf("_tmp")==-1
				...
				if(strUnitOfMeasure!=null && !"null".equalsIgnoreCase(strUnitOfMeasure) && !"".equals(strUnitOfMeasure))
				{
					...
					{
						//IR-830017-3DEXPERIENCER2021x - no need to update the attribute if its old value and new one are empty
						try{
							AttributeType myAttrrType = new AttributeType(attributeName);
							myAttrrType.open(context);
							boolean isMultival = myAttrrType.isMultiVal();
							Dimension dimension = myAttrrType.getDimension(context);
							myAttrrType.close(context);
							if(isMultival && dimension != null){
								String oldVal = getInfo(context, "attribute["+attributeName+"]");
								if(oldVal.isEmpty()){
									continue;
								}
							}						
						}catch(Exception e){
							System.out.println("Error occurred while checking the old value="+e.getMessage());
						}
						value="0";
					}
		...
		while(itr.hasNext()){
			...
			if(key.endsWith("_order")){ //IR-830017-3DEXPERIENCER2021x (temp fix, BPS sends new param XX_order for some attributes, why?)
				String result   = MqlUtil.mqlCommand(context, "list attribute $1", true, key);
				if(result.isEmpty())
					continue;
			}
			...
			newAttrrType.open(context);
			String dataType = newAttrrType.getDataType();
			if((newAttrrType.isRangeVal()) && (dataType.equalsIgnoreCase(FORMAT_REAL) || dataType.equalsIgnoreCase(FORMAT_INTEGER))) {
				String attributeValue = (String)value;
    			if(null != attributeValue && !attributeValue.equals("")){
      	           Attribute attr = new Attribute(newAttrrType,"");
     	           ...
     	           newAttrrType.close(context); 
     	           AttributeList attributesList = new AttributeList();
     	           attributesList.add(attr);
     	           setAttributes(context, attributesList);
    			}
    			// changes added by DMA8 for IR-591119-3DEXPERIENCER2017x end.
			} else {
	...
	private void createMultiValuesMap(String attributeName, String value, HashMap newAttrValuesMap) {
		...
		if(attributeName.indexOf("mva")!=-1){
			...
			//if the newAttrValuesMap already has a Map, meaning mva attribute value and attribute is already present in the Map 
                ...
                // IR-505169-3DEXPERIENCER2018x start.
				if(existingValue instanceof String)
				{
					HashMap map=new HashMap();
					map.put(Integer.parseInt(index), value);
    ...
    throws Exception {
        ...
        if(selectedClassIds.length() > 0 )
        {
            ...
            String getAgAttrsCmd        = "print interface \"$1\" select name $2 attribute dump $3 recordsep $4";
            for(int i = 0; i < allAttributeGroups.size(); i++) {
                commaPipeQueryToMapForAGTitle(_context, 
                                                                getAgAttrsCmd, 
                                                                true, 
                                                                _ag2attrs,
                                                                _agNameTitle,
                                                                (String)allAttributeGroups.get(i),
                                                                "property["+LibraryCentralResourceConstants.TITLE+"].value",
                                                                ",",
                                                                "|"
                                                                );
            }
            ...
            while (iterator.hasNext()) {
                ...
                for (int i = 0; i < slAttributes.size() ; i++) {
                    ...
                    //changes for New IR - 514442
                    String SELECT_ATTRIBUTE_TITLE = "attribute[" + attribute + "]";
                    defValue = attribute.getDefaultValue(context);
                    if(defValue.equals("") || defValue.equals(null)){
                    	_attr2val.put((String)slAttributes.get(i),obj.getInfo(context, SELECT_ATTRIBUTE_TITLE));
                    }else{
    ...
    private static  StringList commaPipeQueryToMapForAGTitle(Context context, String cmd,
            boolean asRoot, Map mapOut, Map nameToTitle, String... vArgs) throws FrameworkException {
        ...
        while (recordsDataIter.hasNext()) {
            ...
            String title = UIUtil.isNotNullAndNotEmpty(fields.get(1))? (String)fields.get(1): name;
            //title = AttributeGroup.formatTitle(title);
            titles.add(name);
            if (nameToTitle != null) {
                nameToTitle.put(name, title);
                if (mapOut != null && UIUtil.isNullOrEmpty(title)) {
                    fields.subList(0, 1).clear();
                    mapOut.put(name, fields);
                }
                else {
             	   fields.subList(0, 2).clear();
                    mapOut.put(name, fields);
                }
            }
