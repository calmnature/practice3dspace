...
public class emxNotificationUtilBase_mxJPO  extends emxUtil_mxJPO {
	...
	throws Exception {
		...
		objectNotification(context,objectId,notificationName,payload,null);
	}

	public static void objectNotification(Context context,
			String objectId, String notificationName, Map payload,String notifyType)
	throws Exception {
		...
		if(objectId != null && objectId.length() > 0)
		{
			...
			try
			{
				...
				if(isExists)
					objectRelationshipNotification(context, objectId, OBJECT, notificationName, payload,notifyType);
				else
				{
					strResult =  MqlUtil.mqlCommand(context,"print connection $1 select $2 dump $3",objectId,"name","|");					
					objectRelationshipNotification(context, objectId, RELATIONSHIP, notificationName, payload,notifyType);
				}
	...
	public static void objectRelationshipNotification(Context context,
			String id, String idType, String notificationName, Map payload) throws Exception {
		objectRelationshipNotification(context,id,idType,notificationName,payload,null);
	}

	public static void objectRelationshipNotification(Context context,
			String id, String idType, String notificationName, Map payload,String notifyType)
	throws Exception {
		...
		for (int j = 0; j < notificationWithSelectList.size(); j++)
		{
			...
			if (bows.getSelectData((String)_sl.elementAt(0)).compareTo(_stateActive) == 0)
			{
				...
				boolean sendIconMail = false;
				if(UIUtil.isNullOrEmpty(notifyType) || "both".equalsIgnoreCase(notifyType)){
					sendMail = true;
					sendIconMail = true;
				}else if("email".equalsIgnoreCase(notifyType)){
					sendMail = true;
				}else {
					sendIconMail = true;
				}
				// Pass results to sendNotification through  IconMail
				if((finalToIconMailList.size()>0 || finalCcIconMailList.size()>0 || finalBccIconMailList.size()>0) && sendIconMail) {
				...
				if((finalToEmailList.size()>0 || finalCcEmailList.size()>0 || finalBccEmailList.size()>0) && sendMail) {
	...
	throws Exception {
		createNotification(context,objectId,toList,ccList,
				subjectKey, messageKey,
				idSelectList, basePropFile,
				toSelectList, ccSelectList,
				filter, urlSuffix, fromAgent,
				messageJPOMethod, filterJPOMethod,both);
	}
	public static void createNotification(Context context,
			String objectId, StringList toList, StringList ccList,
			String subjectKey, String messageKey,
			...
			StringList toSelectList, StringList ccSelectList,
			String filter, String urlSuffix, String fromAgent,
			String messageJPOMethod, String filterJPOMethod,String notifyType)
	throws Exception {
		...
		sendNotification(context, objectId, finalToList, finalCcList, null,
				...
				from, messageJPOMethod,notifyType);
	}
	...
	public static void sendNotification(Context context, String objectId,
			...
			String messageJPOMethod)throws Exception {
		sendNotification(context, objectId,
				toList, ccList, bccList,
				subjectKey, messageKey, objectIdList,
				basePropFile, urlSuffix, fromAgent,
				messageJPOMethod,both);
	}
	...
	public static void sendNotification(Context context, String objectId,
			...
			String subjectKey, String messageKey, StringList objectIdList,
			String basePropFile, String urlSuffix, String fromAgent,
			String messageJPOMethod,String notifyType)
	throws Exception {
		...
		if (baseURL != null && !"".equals(baseURL)) {
			if (objectIdList != null && objectIdList.size() > 0) {
				...
				while (i.hasNext()) {
					urlsBuffer.append(MailUtil.getObjectLinkURI(context, baseURL, (String) i.next()));
		...
		while (itr.hasNext()) {
			...
			sendMail(context, to, cc, bcc, subject, message.toString(), objectIdList, fromAgent,notifyType);
		}
	}
	...
	throws Exception {
		...
		if (baseURL != null && !"".equals(baseURL)) {
			...
			else
			{
				...
				if(idType.equals("object"))
				{
					urlsBuffer.append(MailUtil.getObjectLinkURI(context, baseURL, id));
				}
				else
				{
					urlsBuffer.append("relationshipID=");
        ...
        while (itr.hasNext()) {
            ...
            Map objInfo = dObj.getInfo(context, objectSelects);
				...
				objName = (String)objInfo.get(DomainConstants.SELECT_NAME);
				objRevision =  (String)objInfo.get(DomainConstants.SELECT_REVISION);
            //Supported types for Mail listener
			String strTypeList =  EnoviaResourceBundle.getProperty(context, "emxFramework.emailListener.Notification_support_types");
            StringTokenizer st = new StringTokenizer(strTypeList, ",");
			while (st.hasMoreTokens()) {
				typeList.add(st.nextToken());
			}
			if(typeList.contains(objType)) {
				formattedSubject = subject +" | "+ objType +", "+objName+", "+ objRevision;
			}
			}
			Vector<Locale> locales = new Vector<Locale>();
			locales.add(locale);
			...
			if((notifyType.equalsIgnoreCase(email)) && (fromAgent.equalsIgnoreCase(subscriptionAgent) || (replyTo!=null && ((String) replyTo.elementAt(0)).equalsIgnoreCase(subscriptionAgent) )))
			{
				...
				if(formattedSubject.equals(subject) && "object".equals(idType)) {
	            	formattedSubject = subject+" | "+objType+", "+objName+", "+objRevision;
	            }
			...
			if (preprocessJPOMethod != null && preprocessJPOMethod.length() > 0) {
				...
				try
				{
					...
					try {
						...
						String subjectFromJPO = (String) resultsMap.get("subject");
						if(subjectFromJPO.equals(subject)) {
							//subject = formattedSubject;
						}else {
							if(typeList.contains(objType) && "object".equals(idType)) {
								formattedSubject = subjectFromJPO +" | "+ objType +", "+objName+", "+ objRevision;
							} else {
								formattedSubject = subjectFromJPO;
							}
            ...
            while (itrFreq.hasNext()) {
                ...
                if (toFreq.size() > 0 || ccFreq.size() > 0 || bccFreq.size() > 0) {
                	...
                	if (frequency.equalsIgnoreCase("immediately")) {
		                sendJavaMail(context, toFreq, ccFreq, bccFreq, formattedSubject, messageText, messageHTML, 
                		...
                		if (spools != null && spools.size() > 0) {
			                spoolJavaMail(context, toFreq, ccFreq, bccFreq, formattedSubject, messageText, messageHTML, 
	...
	protected static void sendMail(Context context, StringList toList,
		...
		sendMail(context, toList,
				ccList, bccList, subject,
	...
	protected static void sendMail(Context context, StringList toList,
			StringList ccList, StringList bccList, String subject,
			String message, StringList objectIdList,
			String fromAgent,String notifyType) throws Exception {
		...
		boolean sendEmailAndIconMail = false;
		if(UIUtil.isNullOrEmpty(notifyType) || both.equalsIgnoreCase(notifyType)){
			sendEmailAndIconMail = true;
		}
		// Set the subject and send the iconmail.
		mail.send(context, subject,sendEmailAndIconMail);
	...
	protected static String getMessage(Context context, String id, String idType,
		...
		if (basePropFileName == null || basePropFileName.equals("")) {
			...
			message = MessageUtil.getString(context,basePropFileName, messageKey, null,
					locale);
		}
	...
	public static String getObjectLinkHTMLFromMap(Context context, String[] args) throws Exception {
		Map params = (Map) JPO.unpackArgs(args);
		return getObjectLinkHTML(context, (String) params.get(DomainConstants.SELECT_NAME), (String) params.get(DomainConstants.SELECT_ID));
	}
	...
	public static String getObjectLink(Context context, String objectId) throws Exception {
		...
		String link = "";
		String baseURL = emxMailUtil_mxJPO.getBaseURL(context, null);
		if(UIUtil.isNotNullAndNotEmpty(baseURL)) {
			link = MailUtil.getObjectLinkURI(context, baseURL, objectId);
		}
		
		return (link);
	}
	/**
	 * This method call the object notification when the event is change owner
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args holds the following input arguments:
	 *        objectId - a String holding the id of an object to be used for
	...
	public static int objectNotificationForChangeOwner(Context context, String[] args)
	throws Exception {
		if (args == null || args.length < 2) {
			throw (new IllegalArgumentException());
		}
		...
		String objectId = args[index++];	
		String notificationName = args[index++];
		String kindOfOwner = args[index++];
			
		if("APPObjectOwnerChangedEvent".equals(notificationName) && "owner".equals(kindOfOwner)){
			String arguments[] = new String[args.length];
			for (int i=0; i < args.length && UIUtil.isNotNullAndNotEmpty(args[i]) && !kindOfOwner.equals(args[i]);  i++)
			{
				arguments[i]=args[i];		   
			}
			objectNotification(context, arguments);
		}	
		return 0;
	}
