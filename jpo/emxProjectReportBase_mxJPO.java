...
@com.matrixone.apps.framework.ui.ProgramCallable
  ...
  String[] args) throws Exception
  {
    ...
    strlBusSelects.add(DomainConstants.SELECT_NAME);
    strlBusSelects.add("attribute[PLMEntity.V_Name]");
    //To display names in name column according to type 
    strlBusSelects.add(ProgramCentralConstants.SELECT_IS_KINDOF_VPMREFERENCE);
    ...
    String strTypeVPMReference = (String) PropertyUtil.getSchemaProperty(context,"type_VPMReference");
    ...
    patTypePattern.addPattern(strTypeVPMReference);
...
//    }
	...
	* @param context the eMatrix <code>Context</code> object
	* @param args holds the HashMap containg the following arguments:
	*        objectId - the context governed items name
	* @return List of governed item name  
	* @throws Exception if the operation fails
	*/
@com.matrixone.apps.framework.ui.ProgramCallable
	public Vector getGovernedItemsName (Context context, String[] args) throws Exception{
		 Map hmpParamMap = (HashMap)JPO.unpackArgs(args);
		 MapList objectList  = (MapList) hmpParamMap.get("objectList");
		 
		 Vector governedItemsNameList = new Vector();
		 int size = objectList.size();
		 for (int i = 0; i < size; i++) {
	           Map mapObject = (Map) objectList.get(i);
	           String name = (String)mapObject.get(DomainConstants.SELECT_NAME);
	           if(mapObject.containsKey(ProgramCentralConstants.SELECT_IS_KINDOF_VPMREFERENCE) && "TRUE".equalsIgnoreCase((String)mapObject.get(ProgramCentralConstants.SELECT_IS_KINDOF_VPMREFERENCE))) {
	           	name = (String)mapObject.get("attribute[PLMEntity.V_Name]");
	           }
	           governedItemsNameList.add(name);
	           
	       }	
		 return governedItemsNameList;
	}
  /**
...
@com.matrixone.apps.framework.ui.ProgramCallable
  ...
  {  
	  ...
	  if(ProgramCentralUtil.isNullString(strAllFolderIds)) {
		  strAllFolderIds = (String) CacheUtil.getCacheObject(context, "dpm_contentReport_selectedIds");
	  }
	  CacheUtil.removeCacheObject(context, "dpm_contentReport_selectedIds");
	  StringList slFolderIds =FrameworkUtil.split(strAllFolderIds.trim(), ",");
	  ...
	  int iSelectedFolderListSize = slFolderIds.size();  
		...
		CacheUtil.setCacheObject(context, "dpm_contentReport_AllFolderIds", allFolderList);
  ...
  */
	...
	public StringList getDocumentFolderPath(Context context, String[] args)throws Exception {
		...
		Map hmpProgramMap = (HashMap) JPO.unpackArgs(args);
      String strPrinterFriendly = (String) ((Map)hmpProgramMap.get("paramList")).get("printerFriendly");
      ...
      String strSelectedIds = (String) ((Map)hmpProgramMap.get("paramList")).get("selectedIds");
		...
		String strResult = MqlUtil.mqlCommand(context, strMQL, true, DomainConstants.TYPE_CONTROLLED_FOLDER, "derivative", "|");
		...
		String	_superUser = PropertyUtil.getSchemaProperty(context, "person_UserAgent");
		try
		{
			if (!_superUser.equals(contextUser)) {
				ContextUtil.pushContext(context, _superUser, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);				
				isContextPushed = true;
			}
		
		StringList selectedBookMarkIds = (StringList) CacheUtil.getCacheObject(context, "dpm_contentReport_AllFolderIds");
		CacheUtil.removeCacheObject(context, "dpm_contentReport_AllFolderIds");
		...
		for(BusinessObjectWithSelectItr itr1= new BusinessObjectWithSelectItr(folderContentWithSelectList); itr1.next();)
      {
			...
			if(null != slFolderContentsIds) {
				...
				for(int i=0; i < folderDocumentListSize; i++)
              {
					...
					//IR-767023-Skip the iteration if the Bookmark Folder is not added on the same Project.
					if(!selectedBookMarkIds.contains(sFoldId)) continue;
					...
					if(slControlledFolderTypeHierarchy.contains(sFoldType) || DomainConstants.TYPE_WORKSPACE_VAULT.equalsIgnoreCase(sFoldType)){
						sFoldTitle = (String)slFolderContentsTitles.get(i);
					}
					if(strPrinterFriendly.equalsIgnoreCase("false")) {
                  if(strReportFormat.equals("CSV") || strReportFormat.equals("Text")){
                	   if(slControlledFolderTypeHierarchy.contains(sFoldType) || DomainConstants.TYPE_WORKSPACE_VAULT.equalsIgnoreCase(sFoldType)){
  ...
  */
    ...
    double dblCompletedTasks = 0.0;
		...
		Map mapRequestMap = (Map) hmpProgramMap.get("requestMap");
		String reportFormat=(String) mapRequestMap.get("reportFormat");
    ...
    if(mapDisply != null)
    {
    	if("CSV".equalsIgnoreCase(reportFormat)|| "Text".equalsIgnoreCase(reportFormat))
        {
...
/**
  ...
  */
  ...
  /**
   * Displays the files attached for a document
   *
   * @param context the eMatrix <code>Context</code> object
   * @param args holds the HashMap containg the following arguments:
   * objectList - Maplist containing information about the documents
   * @return StringList containing the files (names)
   * @throws Exception if the operation fails
   * @since PMC 10.6SP2
   */
@SuppressWarnings({ "rawtypes", "unchecked" })
public StringList getDocumentFolderFiles(Context context, String[] args) throws Exception {

    StringList documentFileList = new StringList();
    Map hmpProgramMap = (HashMap) JPO.unpackArgs(args);
    MapList maplObjectList = (MapList) hmpProgramMap.get("objectList");
    Map paramList = (Map) hmpProgramMap.get("paramList");

    StringBuffer sbfFolderFileURL = new StringBuffer();

    String strPrinterFriendly = (String) paramList.get("printerFriendly");
    if (strPrinterFriendly == null || strPrinterFriendly.isEmpty()) {
        strPrinterFriendly = "false";
    }
    ...
    for (int i = 0; i < size; i++) {
        Map objectMap = (Map) maplObjectList.get(i);
        arrFolders[i] = (String) objectMap.get(SELECT_ID);
    }

    String SELECT_DOCUMENT_FILES = "from[Active Version].to.attribute[Title].value";
    String SELECT_DOCUMENTS_IDS  = "to[" + DomainConstants.RELATIONSHIP_VAULTED_OBJECTS_REV2 + "].from.id";
    String SELECT_FILE_IDS       = "from[Active Version].to.id";

    StringList strlTypeSelects = new StringList();
    strlTypeSelects.add(SELECT_DOCUMENT_FILES);
    strlTypeSelects.add(SELECT_DOCUMENTS_IDS);
    strlTypeSelects.add(SELECT_FILE_IDS);
    ...
    try {
        if (!superUser.equals(contextUser)) {
            ContextUtil.pushContext(context, superUser,
                    DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
            isContextPushed = true;
        }

        BusinessObjectWithSelectList folderContentList =
                BusinessObject.getSelectBusinessObjectData(context, arrFolders, strlTypeSelects);

        for (BusinessObjectWithSelectItr itr =
                     new BusinessObjectWithSelectItr(folderContentList); itr.next();) {

            BusinessObjectWithSelect bows = itr.obj();

            StringList slFolderFiles = bows.getSelectDataList(SELECT_DOCUMENT_FILES);
            StringList slFileIds     = bows.getSelectDataList(SELECT_FILE_IDS);

            sbfFolderFileURL.setLength(0);

            if (slFolderFiles != null && slFileIds != null) {
                int noOfFiles = slFileIds.size();

                for (int j = 0; j < noOfFiles; j++) {

                    if ("false".equalsIgnoreCase(strPrinterFriendly)) {

                        if ("CSV".equals(strReportFormat) || "Text".equals(strReportFormat)) {

                            sbfFolderFileURL.append(slFolderFiles.get(j));
                            if (j < noOfFiles - 1) {
                                sbfFolderFileURL.append(" ");
                            }

                        } else {

                            sbfFolderFileURL.append("<a href=javascript:emxTableColumnLinkClick(")
                                    .append("'../components/emxCommonDocumentPreCheckout.jsp?action=download")
                                    .append("&appProcessPage=&emxSuiteDirectory=programcentral")
                                    .append("&relId=null&jsTreeID=null")
                                    .append("&suiteKey=ProgramCentral&objectId=")
                                    .append(slFileIds.get(j))
                                    .append("',700,600);>")
                                    .append(XSSUtil.encodeForHTML(context, slFolderFiles.get(j)))
                                    .append("</a><br>");
                        }

                    } else {
                        sbfFolderFileURL.append(
                                XSSUtil.encodeForHTML(context, slFolderFiles.get(j)));
                    }
                }
            }

            documentFileList.add(sbfFolderFileURL.toString());
        }
