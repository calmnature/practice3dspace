...
{
	private static final boolean IS_CLOUD_INSTALL = UIUtil.isNotNullAndNotEmpty(System.getenv("DS_IN_CLOUD"));
	private static final boolean IS_3DDRIVE_INSTALL = UIUtil.isNotNullAndNotEmpty(System.getenv("DSY_INSTALL_ONLY3DDrive"));
    ...
    private static final String RENAME_SUFIX = "_";
	...
	private static final String KEY_INSTALL_ON = "InstallOn";
	private static final String KEY_INSTALL_ON_CLOUD = "Cloud";
	private static final String KEY_INSTALL_ON_3DDRIVE = "3DDrive";
	private static final String KEY_INSTALL_ON_PREMISE = "Premise";
    ...
    /**
	 * Check if schema needs to be installed or not.
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @throws Exception if the operation fails
	 * @since BPS 2022x
	 */
	public boolean isInstallable(Context context)
			throws Exception
	{
		// get root
		Element root = schemaDefinitionFile.getRootElement();
		// get symbolic name text
		String installOn = root.getChildText(KEY_INSTALL_ON);
		if (installOn == null) {
			installOn = "";
		} else {
			installOn = installOn.trim();
		}
		// by default install
		boolean ret = true;
		// if undefined or Premise then install
		if (installOn.length() == 0) {
			ret = true;
		} else if (IS_CLOUD_INSTALL && !installOn.contains(KEY_INSTALL_ON_CLOUD)) {
			ret = false;
		} else if(IS_3DDRIVE_INSTALL && !installOn.contains(KEY_INSTALL_ON_3DDRIVE)) {
			ret = false;
		}
		return ret;
	}
        ...
        throws Exception
    {
        ...
        if (sAdminType.equals("role") || sAdminType.equals("group"))
        {
            ...
            if(sAdminType.equals("role"))
            {
            	aKeys.add("parent");
            	aKeys.add("remove parent");
            }
            sNewCmd = removeUnresolvedSchemaReferences(context, sAdminSymbolicName, aKeys, sInputCmd);
        }
        ...
        if (sAdminType.equals("policy"))
        {
            ...
            aKeys.add("store");
			aKeys.add("approve");
			aKeys.add("add approve");
			aKeys.add("remove approve");
			aKeys.add("ignore");
			aKeys.add("add ignore");
			aKeys.add("remove ignore");
			aKeys.add("reject");
			aKeys.add("add reject");
			aKeys.add("remove reject");

            sNewCmd = removeUnresolvedSchemaReferences(context, sAdminSymbolicName, aKeys, sInputCmd);
            sNewCmd = removeUnresolvedUsers(context, sAdminSymbolicName, sName, sNewCmd);
            sNewCmd = resolveNewStates(context, sName, sNewCmd);
        }

		// if admin type is rule then look for users
		if (sAdminType.equals("rule"))
		{
			sNewCmd = removeUnresolvedUsers(context, sAdminSymbolicName, sName, sInputCmd);
		}

        // if admin type is application then look for all admin types
        if (sAdminType.equals("application") || sAdminType.equals("package"))
        {
            ArrayList aKeys = new ArrayList();
    ...
    private static String resolveNewStates(Context context, String sName, String cmd) throws Exception {
    	String newCmd = cmd;
    	String result = emxInstallUtil_mxJPO.executeMQLCommand(context, "list policy " + "\"" + sName + "\"");
    	if (sName.equals(result)) {
    		result = emxInstallUtil_mxJPO.executeMQLCommand(context, "print policy " + "\"" + sName + "\" select allstate state dump");

    		result = result.replaceFirst("FALSE,", "");
    		result = result.replaceFirst("TRUE", "allstate");

    		List states = Arrays.asList(result.split(","));
    		ArrayList keyWords = new ArrayList();
    		keyWords.add("add state");
    		keyWords.add("add allstate");
            ArrayList aCmd = getWordsFromCmd(context, cmd, keyWords);

            for (int i = 0; i < aCmd.size(); i++) {
            	String sCmdWord = (String)aCmd.get(i);
            	if ("add state".equals(sCmdWord)) {
                	String state = (String)aCmd.get(i + 1);
                	if (states.contains(state)) {
                		aCmd.set(i, "state");
                	}
            	}
            	if ("add allstate".equals(sCmdWord)) {
                	if (states.contains("allstate")) {
                		aCmd.set(i, "allstate");
                	}
            	}
            }
            newCmd = String.join(" ", aCmd);
    	}
        ...
        throws Exception
    {
		...
		aFieldKeyWords.add("add login user");
		aFieldKeyWords.add("remove login user");
		aFieldKeyWords.add("login user");
		aFieldKeyWords.add("add login");
		aFieldKeyWords.add("remove login");
		aFieldKeyWords.add("login");
 		aFieldKeyWords.add("remove revoke user");
 		aFieldKeyWords.add("remove revoke login user");
        ...
        for (int i = 0; i < aCmd.size(); i++)
        {
            ...
            for (int ii = 0; ii < aFieldKeyWords.size(); ii++)
            {
				...
				if (sToken.equals(sFieldKeyWord) && !((String)aCmd.get(i - 1)).equals("route") && !((String)aCmd.get(i + 1)).equals("owner") && !((String)aCmd.get(i + 1)).equals("\"owner\""))
                {
                    String sFilter = null;
					...
					for (int j = 0; j < 5 && i+2 <= aCmd.size() - 1; j++) {
						...
						if (sFilterKey.startsWith("org") || sFilterKey.startsWith("proj") || sFilterKey.equals("owner") || sFilterKey.equals("reserve") || sFilterKey.equals("maturity")) {
        ...
        throws Exception
    {
        ...
        sCmd = sCmd.replaceAll("\\s{2,}", " ").trim();

        // remove trailing semi-colon ;
        if (sCmd.endsWith(";"))
            sCmd = sCmd.substring(0, sCmd.length() - 1);

	if (sCmd.startsWith("add relationship")) {
		sCmd= sCmd.replaceFirst("add relationship", "add rel");
	}
	if (sInputCmd.startsWith("modify relationship")) {
		sCmd= sCmd.replaceFirst("modify relationship", "modify rel");
	}
        ...
        for (int i = 0; i < aCmd.size(); i++)
        {
            ...
            for (int ii = 0; ii < aFieldKeyWords.size(); ii++)
            {
                ...
                if (sToken.equals(sFieldKeyWord))
                {
                	...
                	String sAdminType = sAdminSymbolicName.substring(0, sAdminSymbolicName.indexOf('_'));
					if ((sAdminType.equals("role") && sToken.equals("remove parent") && (i == aCmd.size()-1))){
                    ...
                    if (aSyms.size() > 0)
                    {
                        ...
                        if (!sAdminType.equals("package") && (sToken.startsWith("member") || sToken.startsWith("add member")))
                        {
        ...
        throws Exception
    {
        ...
        if (sAdminType.equals("relationship"))
        {
            ...
            aKeys.put("add rule", "add rule");
            aKeys.put("remove rule", "remove rule");
            aKeys.put("rule", "add rule");
		...
		// if admin is rule then look for user
		if (sAdminType.equals("rule"))
		{
			aNewCmd = new ArrayList();
			aNewCmd.add(resolveUsers(context, sAdminSymbolicName, sName, aInputCmd));
		}
                ...
                for (int iii = 0; iii < aUnresolvedRef.size(); iii++)
                {
                    ...
                    if(sCmdWord.indexOf(KEY_SYMBOLIC_NAME_START + sUnresolvedRef + KEY_SYMBOLIC_NAME_END) >= 0)
                    {
                        ...
                        else
                        {
                           ...
                           if (!sAdminType.equals("package") && (sKey.startsWith("member") || sKey.startsWith("add member")))
                           {
                    ...
                    else
                    {
                        ...
                        if(sCmdWord.indexOf(KEY_SYMBOLIC_NAME_START + sUnresolvedRef + KEY_SYMBOLIC_NAME_END) >= 0)
                        {
							...
							for (int j = 0; j < 5 && found; j++) {
								if (aCmd.size() - 1 > ii + 2) {
									if (((String)aCmd.get(ii + 2)).startsWith("proj")) {
										sufixCmd += (String)aCmd.get(++ii) + " " + "project ";
										ii++;
										continue;
									}
									if (((String)aCmd.get(ii + 2)).startsWith("org")) {
