...
public class emxOrganizationBase_mxJPO extends emxDomainObject_mxJPO
{
    ...
    throws Exception
    {
        ...
        String YES   =EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource",new Locale(languageStr),"emxComponents.Common.Yes");
        String NO    =EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource",new Locale(languageStr),"emxComponents.Common.No");
    ...
    throws Exception
    {
        ...
        try {
            String[] personIds= new String[relBusObjPageList.size()];
            
            for (int i = 0; i < relBusObjPageList.size(); i++) {
                personIds[i] = (String)((Hashtable)relBusObjPageList.get(i)).get(DomainConstants.SELECT_NAME);
                Iterator it = PersonUtil.getAssignments(context,personIds[i]).iterator();
                HashSet<String> rolesList = new HashSet<String>();
                
                while(it.hasNext()){
                	String roleName = it.next().toString() ; 
                	if((roleName.matches("([\\w\\s+]+)\\.([\\w\\s+-]+)\\.([\\w\\s+-]+)") || roleName.equalsIgnoreCase(personIds[i]))){
                		continue;
                	...
                	roleName = i18nNow.getRoleI18NString(roleName.trim() ,strLanguage);
                	rolesList.add(roleName);	
                }
                columnValues.add(rolesList.stream().collect(Collectors.joining(", ")));  
            }
        }
    ...
    public Vector getLicenseType(Context context, String[] args)throws Exception
    {
        ...
        try {
            for (int i = 0; i < objPageList.size(); i++) {
                ...
                if(iCasualHours > 0){
                	columnValues.add(EnoviaResourceBundle.getProperty(context, "emxPersonOrgModelStringResource", new Locale(context.getSession().getLanguage()), "emxComponents.Common.Licensing.Casual"));
                }else{
                	columnValues.add(EnoviaResourceBundle.getProperty(context, "emxPersonOrgModelStringResource", new Locale(context.getSession().getLanguage()), "emxComponents.Common.Licensing.Full"));
                }
            }
        }
        catch (Exception Ex){
        ...
        finally
        {
            if(flag==true)
            {
                emxContextUtil_mxJPO.mqlNotice(context,
                		EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource", new Locale(context.getSession().getLanguage()),"emxComponents.Common.UnregisteredRoles"));
            }
        }
        return returnValue;
    }
    ...
    */
    ...
    public MapList getCollaborationRequests(Context context, String[] args) throws Exception
    {
        ...
        if(strListOrgRep != null && (size = strListOrgRep.size()) > 0)
        {
            ...
            StringList orgSelects = new StringList();
            orgSelects.add(DomainConstants.SELECT_ATTRIBUTE_TITLE);
            Map orgMap = domRepOrg.getInfo(context,orgSelects);
            String strRepOrgName    = (String)orgMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
            String strRepOrgType    = (String)orgMap.get(DomainConstants.SELECT_TYPE);
            ...
            strBusSelects.add(DomainConstants.SELECT_ATTRIBUTE_TITLE);
            ...
            while (listItr.hasNext())
            {
                ...
                dataMap.put("SELECT_TO_NAME",dataMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
    ...
    */
    public Vector getToOrgForCollaborationRequestsSummary(Context context,String args[]) throws Exception
    {
        ...
        if(objList != null && (listSize = objList.size()) > 0 )
        {
            ...
            String strCompanyImage = "<img src=\"../common/images/iconCompany16.png\" border=\"0\" align=\"absmiddle\">";
            String strBusinessUnitImage = "<img src=\"../common/images/iconBusinessUnit16.png\" border=\"0\" align=\"absmiddle\">";
              ...
              if(sRelationship != null && !"null".equals(sRelationship) && !"".equals(sRelationship))
              {
                    ...
                    if(sBuffer.length() > 0)
                    {
                        ...
                        if(mapList != null && (size = mapList.size()) > 0)
                        {
                           ...
                           strOutput = EnoviaResourceBundle.getProperty(context,"emxPersonOrgModelStringResource", new Locale(langStr),sMessage);
                           intOutput = 1;
                        }
                    }//end of if
              }
    ...
    */
    ...
    public int createAdminUserOrganizationForOrganization(Context context,String[] args) throws Exception
    {
        ...
        try
        {
            if(!"".equals(name))
            {
                String exists = MqlUtil.mqlCommand(context, "list role $1", name);
                if (!exists.isEmpty()) {
                    return 0;
                }
                ContextUtil.pushContext(context);
                ...
                String sCommand = "modify role $1 property $2 value $3 property $4 value $5 property $6 value $7";
            	StringList mqlCommandParams = new StringList();
            	mqlCommandParams.add(name);
            	mqlCommandParams.add("application");
            	mqlCommandParams.add("Framework");
            	mqlCommandParams.add("version");
            	mqlCommandParams.add(serverVersion);
            	mqlCommandParams.add("original name");
            	mqlCommandParams.add(name);
            	String mqlOutput = MqlUtil.mqlCommand(context, sCommand,mqlCommandParams);
                ContextUtil.commitTransaction(context);
            }
        }
        catch (Exception ex)
    ...
    public int inheritParentAdminUserOrganizationOfOrganization(Context context, String[] args) throws Exception
    {
        ...
        try
        {
            if(!"".equals(fromName) && !"".equals(toName))
            {
                ContextUtil.pushContext(context);
                contextPushed=true;         
                String sCommand = "modify role $1 asanorg parent $2";
                MqlUtil.mqlCommand(context, sCommand, toName, fromName);
            }
        }
        catch (Exception ex)
        {
        ...
        while (mapListItr.hasNext())
        {
                ...
                if( (existingRoles.indexOf("~"+sLeadRoles[iLeadRole]+"~")!= -1) && !(perMap.get("to.id").equals(personDo.getInfo(context,DomainConstants.SELECT_ID))))
            {
                    iRolCount = 1 ;
                        ...
                        iReturn += strrole+" ";
                        iReturn +=loc.GetString("emxPersonOrgModelStringResource",strLanguage, "emxComponents.Common.isLeadRoleAssigned")+" ";
...
//Added:Nov 6, 2008:OEF:R208:PRG Adv Resource Planning
   ...
   public StringList getExcludeOIDForResourceManagerSearch(Context context, String args[]) throws Exception
   {
       try
       {
          ...
          // String strVaultPattern = context.getVault().getName();
           ...
           List<String> slActiveRMList = new ArrayList<String>();
           ...
           String strSecuredVPMProjectAdmin = PropertyUtil.getSchemaProperty(context,"role_3DSRestrictedOwner");
           ...
           }else if(null != strSecuredVPMProjectAdmin && !"null".equalsIgnoreCase(strSecuredVPMProjectAdmin.trim()) && !"".equalsIgnoreCase(strSecuredVPMProjectAdmin.trim())){
        	   String sCommandStatement = "print role $1 select $2 dump $3";
        	   String strVPLMProjectAdminUsers =  MqlUtil.mqlCommand(context, sCommandStatement,strSecuredVPMProjectAdmin, "person",",");
        	   slVPLMProjectAdminUserList = FrameworkUtil.split(strVPLMProjectAdminUsers, ",");
           }
   ...
   /**
    ...
    public String getFieldResourceManagersData (Context context, String[] args) throws MatrixException
    {
    	...
    	{
    		System.out.println("Error: "+iaexp.toString());
...
//End:R208:PRG Adv Resource Planning
    ...
    public Map getCountryChooserDetailsForHTMLDisplay(Context context, String[] args) throws Exception
    {
		...
		List manualEntList = (List) details.get("manualEntryList");     
        manualEntryList = EnoviaResourceBundle.getFrameworkStringResourceProperty(context, "emxFramework.Range.Country.~~~AllowFreeFormEntry~~~",context.getLocale());
        details.put("manualEntryList", new StringList(manualEntryList));           
     ...
     */
     ...
     public MapList getFormatstoAdd(Context context, String[] args) throws FrameworkException {
         try {
             ...
             // get the where expression for the type 
 	    	 String strWhereExpression = FrameworkUtil.getWhereExpressionForMql(context,"",TYPE_FILE_FORMAT) ;
     ...
     public MapList selectionOwningOrganizationExpandProgram(Context context, String[] args) throws FrameworkException {
         try {
             ...
             Integer expandLevelInt = 1;
             if(UIUtil.isNotNullAndNotEmpty(expandLevel)) {
            	 if(expandLevel.equalsIgnoreCase("All")) {
            		 expandLevelInt = 0;
            	 }else {
            		 expandLevelInt = Integer.parseInt(expandLevel);
            	 } 
             }
             ...
             organizations.addAll(org.getBusinessUnitsDepartmentsAndPlants(context, expandLevelInt, selectables));
...
//             organizations.addAll(plant.getPlants(context, "emxPlant", "getPlants", orgId, selectables, "on")) ;
	...
	/**
	 * Method to exclude the current org from search
	 * @param context The Matrix Context
	 * @param args
	 * @return
	 * @throws Exception
	 */
	@com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	public StringList excludeCurrentOrganization(Context context, String[] args)
	throws Exception
	{
		Map programMap = (Map) JPO.unpackArgs(args);
	...
	public int addTitleOnOrgAdminRole(Context context, String[] args) throws Exception{
		boolean companyRename = false;
    	try {
    		String isCompanyRenameEnabled = EnoviaResourceBundle.getProperty(context, "emxFramework.Organization.EnableNewAPIs");
    		if(isCompanyRenameEnabled.equalsIgnoreCase("true")) {
    			companyRename = true;
    		}
    	} catch(Exception e) {}
    	String name =args[0];
        ...
        if(companyRename) {
        	String id=args[1];
        	DomainObject orgobj = new DomainObject(id);
        	title = orgobj.getInfo(context,DomainConstants.SELECT_ATTRIBUTE_TITLE);
        }
		boolean contextPushed = false;
		try {
			if(UIUtil.isNotNullAndNotEmpty(title)) {
				...
				contextPushed=true;
				ContextUtil.startTransaction(context,false);
				String sCommand = "modify role $1 property $2 value $3";
				StringList mqlCommandParams = new StringList();
				mqlCommandParams.add(name);
				mqlCommandParams.add("Title");
				mqlCommandParams.add(title);
				String mqlOutput = MqlUtil.mqlCommand(context, sCommand,mqlCommandParams);
				ContextUtil.commitTransaction(context);
			}
	...
	public int updateAdminRoleTitle(Context context, String[] args) throws Exception{
		String name = args[0];
		...
		boolean contextPushed = false;
		try {
			if(attrName.equals(DomainConstants.ATTRIBUTE_TITLE) && UIUtil.isNotNullAndNotEmpty(title)) {
				ContextUtil.pushContext(context);
				contextPushed=true;
				ContextUtil.startTransaction(context,false);
				String sCommand = "modify role $1 property $2 value $3";
	...
	public int isOrgTitleNotUnique(Context context, String[] args)  throws Exception {
		String attrName = args[0];
		String title = args[1];
		String type = DomainConstants.TYPE_ORGANIZATION; 
        String objectWhere = DomainConstants.SELECT_ATTRIBUTE_TITLE+"==\""+title+"\"";
        ...
        if(attrName.equals(DomainConstants.ATTRIBUTE_TITLE) && UIUtil.isNotNullAndNotEmpty(title)) {
	        ...
	        if(compList.size() > 0) {
	        	ContextUtil.abortTransaction(context);
	        	String titleNotUniqueMsg = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", context.getLocale(), "emxComponents.Profiling.OrganizationAlreadyExists");
				emxContextUtil_mxJPO.mqlNotice(context,titleNotUniqueMsg);
	            return 1;
		    }else {
		    	return 0;
		    }
