...
/*commented for the bug no 337605 - End */
    ...
    throws Exception
    {
		...
		String personCompanyTitle = "to[Employee].from.attribute[Title]";
        ...
        try
        {
			...
			memberSelects.add(personCompanyTitle);
            ...
            while (projectMemberListItr.hasNext())
            {
				...
				String OrgnizationTitle = (String) objectMap.get(personCompanyTitle);
				...
				objectMap.put("OrgnizationTitle",OrgnizationTitle);
    ...
    throws Exception
    {
        ...
        try
        {
            ...
            while (objectListIterator.hasNext())
            {
                ...
                String title = (String) objectMap.get("OrgnizationTitle"); // To return Company Title instead of Name
                
                if(ProgramCentralUtil.isNullString(title)) {
                    title = organization;
                }
                vectCompany.add(title);
            }
        }
        catch (Exception ex)
        {
...
//      Added for the Bug 310681 End
     ...
     * @since PRG V6R2011
     */
    public boolean isSecurityContextAccessEnabled(Context context, String[] args)
      throws Exception
    {
...
//    		e.printStackTrace();   
    		DebugUtil.debug("isSecurityContextAccessEnabled "+ e.getMessage());
    	}
    	return true;
    }
...
//			exp.printStackTrace();   
			DebugUtil.debug("getTimesheetColumnData "+ exp.getMessage());
			throw exp;
		}
	}
    ...
    public boolean hasAccessToModifyProject(Context context,String[]args)throws Exception
    {
    	...
    	if (access) {
			String result = MqlUtil.mqlCommand(context,"print bus $1 select $2 dump", projectId, "current.access[modify]");
			access = "true".equalsIgnoreCase(result);
    	}

    	return access;
    }

	private static Map getCellLevelEditAccessBlock(Context context,String id) throws MatrixException {
		...
		BusinessObjectWithSelectList bwsl = 
				ProgramCentralUtil.getObjectWithSelectList(context, new String[] {id}, selects);
		
		BusinessObjectWithSelect bws = bwsl.get(0);
		Map objMap = FrameworkUtil.toMap(bws);
		objMap.put("id", id);
    ...
    public static StringList getCellLevelEditAccess(Context context, String args[])throws Exception
    {
		...
		int accessListLen 	= objectList.size();
		
		StringList accessList = new StringList(accessListLen);
		...
		//Added new logic to disable cell edit for collabspace leader on same collab+org
		
		MapList collabspacesMap =new MapList(accessListLen);
		for (int i=0; i < accessListLen; i++) {
			Map map = (Map) objectList.get(i);
			String name = (String) map.get("name");
			String org = (String) map.get("org");
			String project = (String) map.get("project");

			if (ProgramCentralUtil.isNotNullString(org)) { //Added for performance improvement
				collabspacesMap.add(map);
			}
		}
		...
		Map projAccessMap = null;
		
		for (Object obj : objectList) {
			Map objMap = (Map) obj;
			String curObjectID = (String) objMap.get("id");
			String disableSelection = (String)objMap.get("disableSelection");
			if("true".equalsIgnoreCase(disableSelection)) {
				accessList.add("false");
			}else {
				StringList valueList = StringUtil.split(curObjectID, ":");
				if(valueList.size() > 1 ) {
					String projId = valueList.get(0);
					if(projAccessMap == null || projAccessMap.isEmpty()) {
						projAccessMap = getCellLevelEditAccessBlock(context, projId);
					}else {
						String id = (String) projAccessMap.get("id");
						if(!projId.equals(id)) {
							projAccessMap = getCellLevelEditAccessBlock(context, projId);
						}
					}
					objectOrg 		= (String)projAccessMap.get("organization");
					objectProject 	= (String)projAccessMap.get("project");
					owner 			= (String)projAccessMap.get(DomainConstants.SELECT_OWNER);
					chnageOwnerAccess = (String)projAccessMap.get("current.access[changesov]");
				}
				...
				if("false".equalsIgnoreCase(chnageOwnerAccess)) {
					...
					boolean isPerson = project.contains("_PRJ") && (DomainConstants.EMPTY_STRING.equals(objMap.get("org")));
					if(isPerson) {
						int index = project.lastIndexOf("_");
						String personName = project.substring(0, index);
						StringList assignProductList = new StringList();
						String isValidAssignment = "true";
						String assignment = ProgramCentralConstants.EMPTY_STRING;
						String memberAccess = ProgramCentralConstants.EMPTY_STRING;

						if( loggedInUser.equals(personName) || personName.equals(owner)) {
							accessList.add("false");
						} else {
							try{
								...
								int collabListLen = collabspacesMap.size();
								for (int j = 0; j < collabListLen; j++) {
									Map collabMap = (Map) collabspacesMap.get(j);
									memberAccess = (String) collabMap.get("access");

									String name = (String) collabMap.get(ProgramCentralConstants.SELECT_NAME);
									if (name != null && name.contains("GLOBAL")) {
										leader = "Project Lead";
									} else {
										leader = "VPLMProjectLeader";
									}
									assignment = "assignment[ctx::" + leader + "." + name + "]";

									isValidAssignment = MqlUtil.mqlCommand(context, "print person $1 select $2 dump", false, personName, assignment);
...
//								e.printStackTrace();   
								DebugUtil.debug("getCellLevelEditAccess "+ e.getMessage());
							}
							if(assignProductList.contains(ProgramCentralConstants.DPS_LICENSE_ARRAY[0]) || assignProductList.contains(ProgramCentralConstants.PRG_LICENSE_ARRAY[0])){
								if("true".equalsIgnoreCase(isValidAssignment) && ("All".equalsIgnoreCase(memberAccess) || "Project Lead".equalsIgnoreCase(memberAccess))){
									accessList.add("false");
								}else{
									accessList.add("true");	
								}
					...
					} else if(!DomainConstants.EMPTY_STRING.equals(objMap.get("org")) || !DomainConstants.EMPTY_STRING.equals(objMap.get("project"))) {

						boolean org = ((String) objMap.get("org")).equals(objectOrg);
						boolean proj = ((String) objMap.get("project")).equals(objectProject);
						if(org && proj) {
							accessList.add("false");
						} else {
							accessList.add("true");
		...
		DebugUtil.debug("Total time taken by getCellLevelEditAccess().........................."+(System.currentTimeMillis()-start));
    ...
    /**
     ...
     public StringList getProjectMemberRoleColumnEditAccess(Context context,String args[]) throws Exception {
 		...
 		for(int iterator = 0; iterator < objectMap.size(); iterator++)
 		{
			...
			returnStringList.add(String.valueOf(editAccess));		
 		}
 		return returnStringList;
 	}
 	...
 	static public MapList getObjectAccessList(Context context, String[] args) throws Exception {
		...
		//String projectOwner = domainObject.getInfo(context, DomainConstants.SELECT_OWNER);
		//projectOwner += "_PRJ";
...
//			String personName = mapObjects.get("name");
//
...
//			} 
//			//Enable selection for ProjectOwner.
//			if(ProgramCentralUtil.isNotNullString(personName) && personName.equals(projectOwner)) {
//				{
...
//		}
	...
	/**
     * Creates command PMCAddMember dynamically from DomainAccessAddMember.
     * @param context The eMatrix <code>Context</code> object.
     ...
     */
 	...
 	@com.matrixone.apps.framework.ui.ProgramCallable
 	 public  Map createDynamicPMCAddMember (Context context, String[]args )throws Exception
	    {
	      ...
	      if(ProgramCentralUtil.isNotNullString(objectId)){
	      	    ...
	      	    Map domainAcessAddMemberCmdMap = ProgramCentralUtil.createDynamicCommand(context,"DomainAccessAddMember",uiMenu,true);
	      		Map mapSetting = (Map)domainAcessAddMemberCmdMap.get("settings");
	      		String sHref = (String)domainAcessAddMemberCmdMap.get("href");

	      		String  cmdExpression = (String) mapSetting.get("Access Expression");
	      		if(ProgramCentralUtil.isNullString(cmdExpression)){
	      			cmdExpression = "current.access[fromconnect]||current.access[changesov]";
	      		}
	      		
	      		String mqlQuery = "eval expr $1 on bus $2";
	      		String expResult = MqlUtil.mqlCommand(context, false, false, mqlQuery, false,cmdExpression,objectId);
	      		
	      		if("true".equalsIgnoreCase(expResult)){
	      			mapSetting.put("Access Expression",expResult.toLowerCase());
	      		}else{
	      			mapSetting.put("Access Expression","false");
	      		}

	      		String strRegSuite ="ProgramCentral";
	      		mapSetting.put("Registered Suite",strRegSuite);
	      		
				//Added to avoid 8k error while searching person for project
				if(sHref != null && sHref.contains("&field=")){
					sHref = FrameworkUtil.findAndReplace(sHref, "&field=TYPES=type_Person:CURRENT=policy_Person.state_Active", "&fieldQueryProgram=emxProjectMember:getFieldQueryForProjectMember");
				}
				
				if(sHref != null && sHref.contains("&excludeOIDprogram=")){
					sHref = FrameworkUtil.findAndReplace(sHref, "&excludeOIDprogram=emxDomainAccess:getExcludePersonList", "");
				}//End

	      		if(sHref != null && sHref.contains("&submitURL=")){
	     			sHref = FrameworkUtil.findAndReplace(sHref, "&submitURL=${COMMON_DIR}/emxDomainAccessProcess.jsp&cmdName=addPerson", "&submitURL=../programcentral/emxProgramCentralUtil.jsp?mode=addMember");
				}
	     			
	      		if(sHref != null && sHref.contains("&jpoName=")){
	      			sHref = FrameworkUtil.findAndReplace(sHref,"&jpoName=emxDomainAccess", "");
	     			
	     		}
	      		if(sHref != null && sHref.contains("&methodName=")){
	      			sHref = FrameworkUtil.findAndReplace(sHref,"&methodName=addMember", "");
	     			
	     		}
	
	      		domainAcessAddMemberCmdMap.put("settings", mapSetting);  		
	      		domainAcessAddMemberCmdMap.put("href", sHref);
	      		domainAcessAddMemberCmdMap.put("name", "PMCAddMember");
	      		categoryMapList.add(domainAcessAddMemberCmdMap);	
      		
	      		}
	      retunMap.put("Children",categoryMapList) ;
		  } catch (Exception e) {
		  	throw new MatrixException(e);
		  }
		  return retunMap;
 	...
 	/**
      * Creates command PMCDeleteAccess dynamically from DomainAccessAddMember.
      * @param context The eMatrix <code>Context</code> object.
      * @param args holds information about object.
      * @return command setting info map.
      * @throws Exception if operation fails.
      */
 	 ...
 	 @com.matrixone.apps.framework.ui.ProgramCallable
  	 public  Map createDynamicPMCDeleteAccess (Context context, String[]args )throws Exception
 	    {
  		 MapList categoryMapList = new MapList();
  		 Map retunMap=new HashMap<>();
	       try {
	       ...
	       if(ProgramCentralUtil.isNotNullString(objectId)){
	       	    ...
	       	    Map domainAcessDeleteAccessCmdMap = ProgramCentralUtil.createDynamicCommand(context,"DomainAccessDeleteAccess",uiMenu,true);
	       		Map mapSetting = (Map)domainAcessDeleteAccessCmdMap.get("settings");
	      		...
	      		if("true".equalsIgnoreCase(expResult)){
	       		...
	       		String sHref="../programcentral/emxProgramCentralUtil.jsp?mode=deleteMember";
	       		String strRegSuite ="ProgramCentral";
	       		mapSetting.put("Registered Suite",strRegSuite);
	       		String confirmMessage = EnoviaResourceBundle.getProperty(context, strRegSuite, "emxProgramcentral.Delete.ConfirmMsg", strLang);
	       		mapSetting.put("Confirm Message",confirmMessage);
	       		domainAcessDeleteAccessCmdMap.put("settings", mapSetting);  		
	       		domainAcessDeleteAccessCmdMap.put("href", sHref);
	       		domainAcessDeleteAccessCmdMap.put("name", "PMCDeleteAccess");
	       		categoryMapList.add(domainAcessDeleteAccessCmdMap);
 	...
 	/**
      * Creates command PMCAddSecurityContext dynamically from DomainAccessAddSecurityContext.
      * @param context The eMatrix <code>Context</code> object.
      * @param args holds information about object.
      * @return command setting info map.
      * @throws Exception if operation fails.
      */
  	 @com.matrixone.apps.framework.ui.ProgramCallable
  	 public  Map createDynamicPMCAddSecurityContext (Context context, String[]args )throws Exception
  	 {
  		 MapList categoryMapList = new MapList();
  		 Map retunMap=new HashMap<>();
  		 try {
  			 ...
  			 if(ProgramCentralUtil.isNotNullString(objectId)){
  				 ...
  				 Map domainAcessAddSecurityContextCmdMap = ProgramCentralUtil.createDynamicCommand(context,"DomainAccessAddSecurityContext",uiMenu,true);
  				 Map mapSetting = (Map)domainAcessAddSecurityContextCmdMap.get("settings");
  				 String sHref = (String)domainAcessAddSecurityContextCmdMap.get("href");
  				 ...
  				 if(sHref != null && sHref.contains("&submitURL=")){
  					 sHref = FrameworkUtil.findAndReplace(sHref, "&submitURL=${COMMON_DIR}/emxDomainAccessProcess.jsp", "&submitURL=../common/emxDomainAccessProcess.jsp");
 	}

  				 domainAcessAddSecurityContextCmdMap.put("settings", mapSetting);  	
  				 domainAcessAddSecurityContextCmdMap.put("href", sHref);
  				 domainAcessAddSecurityContextCmdMap.put("name", "PMCAddSecurityContext");
  				 categoryMapList.add(domainAcessAddSecurityContextCmdMap);
  			 }
  			 retunMap.put("Children",categoryMapList) ;
  		 } catch (Exception e) {
  			 throw new MatrixException(e);
  	...
  	/**
      * Creates command PMCAddUserGroups dynamically from DomainAccessAddUserGroups.
      * @param context The eMatrix <code>Context</code> object.
      * @param args holds information about object.
      * @return command setting info map.
      * @throws Exception if operation fails.
      */
  	 @com.matrixone.apps.framework.ui.ProgramCallable
  	 public  Map createDynamicPMCAddUserGroups (Context context, String[]args )throws Exception
  	 {
  		 MapList categoryMapList = new MapList();
  		 Map retunMap=new HashMap<>();
  		 try {
  			 ...
  			 if(ProgramCentralUtil.isNotNullString(objectId)){
  				 ...
  				 Map domainAcessAddSecurityContextCmdMap = ProgramCentralUtil.createDynamicCommand(context,"DomainAccessAddUserGroups",uiMenu,true);
  				 Map mapSetting = (Map)domainAcessAddSecurityContextCmdMap.get("settings");
  				 String sHref = (String)domainAcessAddSecurityContextCmdMap.get("href");

  				 String  cmdExpression = (String) mapSetting.get("Access Expression");
  				 if(ProgramCentralUtil.isNullString(cmdExpression)){
  					 cmdExpression = "current.access[changeowner]||current.access[changesov]";
  				 }
  				 ...
  				 domainAcessAddSecurityContextCmdMap.put("settings", mapSetting);  	
  				 domainAcessAddSecurityContextCmdMap.put("href", sHref);
  				 domainAcessAddSecurityContextCmdMap.put("name", "PMCAddUserGroups");
  				 categoryMapList.add(domainAcessAddSecurityContextCmdMap);
  			 }
  			 retunMap.put("Children",categoryMapList) ;
  		 } catch (Exception e) {
  	...
  	/**
      * Creates command PMCAddUserGroupsCloud dynamically from DomainAccessAddUserGroupsCloud.
      * @param context The eMatrix <code>Context</code> object.
      * @param args holds information about object.
      * @return command setting info map.
      * @throws Exception if operation fails.
      */
  	 @com.matrixone.apps.framework.ui.ProgramCallable
  	 public  Map createDynamicPMCAddUserGroupsCloud (Context context, String[]args )throws Exception
  	 {
  		 MapList categoryMapList = new MapList();
  		 Map retunMap=new HashMap<>();
  		 try {
  			 ...
  			 if(ProgramCentralUtil.isNotNullString(objectId)){
  				 ...
  				 Map domainAcessAddSecurityContextCmdMap = ProgramCentralUtil.createDynamicCommand(context,"DomainAccessAddUserGroupsCloud",uiMenu,true);
  				 Map mapSetting = (Map)domainAcessAddSecurityContextCmdMap.get("settings");
  				 String sHref = (String)domainAcessAddSecurityContextCmdMap.get("href");
  				 ...
  				 domainAcessAddSecurityContextCmdMap.put("settings", mapSetting);  	
  				 domainAcessAddSecurityContextCmdMap.put("href", sHref);
  				 domainAcessAddSecurityContextCmdMap.put("name", "PMCAddUserGroupsCloud");
  				 categoryMapList.add(domainAcessAddSecurityContextCmdMap);
  			 }
  			 retunMap.put("Children",categoryMapList) ;
  		 } catch (Exception e) {
	...
	@com.matrixone.apps.framework.ui.ProgramCallable
	public String getFieldQueryForProjectMember(Context context, String[] args) throws Exception {
		try {
			HashMap paramMap = (HashMap) JPO.unpackArgs(args);
			String projectId = (String) paramMap.get("objectId");
			//String mqlCmd = "print bus $1 select $2 dump";
			//String projName = MqlUtil.mqlCommand(context, mqlCmd, objectId, "name");
			return "TYPES=type_Person:CURRENT=policy_Person.state_Active:MEMBER_ID!="+projectId;
		} catch (Exception ex) {
//			ex.printStackTrace();   
			DebugUtil.debug("getFieldQueryForProjectMember "+ ex.getMessage());
			throw ex;
		}
	}
	
	@com.matrixone.apps.framework.ui.ProgramCallable
public MapList getAllMembers(Context context, String[] args) throws Exception {
    try {
        MapList uniqueMembers = new MapList();
        ...
        if (strProjectId != null && !strProjectId.isEmpty()) {
            ...
            String organization = (String) projectInfo.get("organization");
            String collabSpace = (String) projectInfo.get("project");

            String[] securityContexts = new String[] {
                "VPLMProjectLeader." + organization + "." + collabSpace,
                "3DSRestrictedLeader." + organization + "." + collabSpace
            };

            for (String securityContext : securityContexts) {
                String mqlCmd = "temp query bus $1 $2 $3 select $4 dump $5";
                String mqlResult = MqlUtil.mqlCommand(context, mqlCmd, "*", securityContext, "*", "physicalid", "|");
                String physicalId = null;
                if (mqlResult != null && !mqlResult.isEmpty()) {
                    String[] resultParts = mqlResult.split("\\|");
                    physicalId = resultParts[resultParts.length - 1].trim();
                }
                if (physicalId != null && !physicalId.isEmpty()) {
                    // Get Group URIs
                    String mqlCmd1 = "print bus $1 select $2 dump $3";
                    String selectExp = "to[Group Assigned Security Context].from.attribute[Group URI]";
                    String getGroupURI = MqlUtil.mqlCommand(context, mqlCmd1, physicalId, selectExp, ",");

                    if (getGroupURI != null && !getGroupURI.isEmpty()) {
                        String[] groupURIs = getGroupURI.split(",");

                        for (String groupURI : groupURIs) {
                            groupURI = groupURI.trim();
                            Dataobject groupURIObject = new Dataobject();
                            DataelementMapAdapter.setDataelementValue(groupURIObject, "groupURI", groupURI);

                            String groupId = com.dassault_systemes.enovia.tskv2.TaskService.getGroupId(context, groupURIObject);

                            if (groupId != null && !groupId.isEmpty()) {
                                MapList groupMemberList =
                                    com.matrixone.apps.program.Task.getUserGroupMembersList(context, groupId);

                                if (groupMemberList != null && !groupMemberList.isEmpty()) {
                                    for (int i = 0; i < groupMemberList.size(); i++) {
                                        Map member = (Map) groupMemberList.get(i);
                                        String pid = (String) member.get("physicalid");

                                        if (pid != null && !pid.isEmpty() && uniquePhysicalIds.add(pid)) {
        ...
        System.out.println("Final uniqueMembers: " + uniqueMembers);
        return uniqueMembers;

    } catch (Exception ex) {
        DebugUtil.debug("getAllMembers() Exception: " + ex.getMessage());
        ex.printStackTrace();
        throw ex;
    }
}
