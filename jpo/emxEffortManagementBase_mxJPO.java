...
public class emxEffortManagementBase_mxJPO
{
	...
	private static final String EFFORT_ORIGINATOR = "from["+DomainConstants.RELATIONSHIP_HAS_EFFORTS+"].to."+DomainConstants.SELECT_ORIGINATOR;  // new code
	private static final String EFFORT_ID = "from["+DomainConstants.RELATIONSHIP_HAS_EFFORTS+"].to.id"; // new code

	private final String type_TaskManagement=(String)PropertyUtil.getSchemaProperty("type_TaskManagement");
	private final String type_effort=(String)PropertyUtil.getSchemaProperty("type_Effort");
	private final String attribute_Duration=(String)PropertyUtil.getSchemaProperty("attribute_TaskEstimatedDuration");
	private final String attribute_TaskFinishDate=(String)PropertyUtil.getSchemaProperty("attribute_TaskEstimatedFinishDate");
	private final String relPattern=(String)PropertyUtil.getSchemaProperty("relationship_hasEfforts");
	
	private final String type_Person=(String)PropertyUtil.getSchemaProperty("type_Person");
	private final String rel_assigned=(String)PropertyUtil.getSchemaProperty("relationship_AssignedTasks");
	private final String attribute_Originator=(String)PropertyUtil.getSchemaProperty("attribute_Originator");
	private final String attribute_WeekEndingDate=(String)PropertyUtil.getSchemaProperty("attribute_WeekEndingDate");
	private final String attribute_RemainingEffort=(String)PropertyUtil.getSchemaProperty("attribute_RemainingEffort");
	private final String separator = FrameworkProperties.getProperty("emxFramework.DecimalSymbol");
	...
	public Vector getName(Context context, String args[])
		...
		if(ProgramCentralUtil.isNullString(exportFormat)) {
			exportFormat= strPrinterFriendly;
		}
		...
		for(int i=0;i<objectListSize;i++)
		{
			...
			Map map = (Map) taskProjectInfoMapList.get(i);
			...
			String taskObjType = (String) map.get(DomainConstants.SELECT_TYPE);
			String isKindOfProjectSpace = (String) map.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
			String title = (String) map.get(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
			...
			if ("TRUE".equalsIgnoreCase(isKindOfProjectSpace) && ProgramCentralUtil.isNotNullString(title)) {
				strName = title;
			}
			...
			String url = "../common/emxNavigator.jsp?objectId="+taskId;
			...
			if (blDeletedTask){
				...
				if(!isPrinterFriendly){
					...
					sBuff.append("<a  href=\""+url);                       
						...
						sBuff.append("\" target=\"_blank\" title=\""+encodedName);
						sBuff.append("\">");
						sBuff.append(encodedName);
				...
				if(!("Exists".equalsIgnoreCase(sState)&&TYPE_EFFORT.equalsIgnoreCase(taskObjType))){
			...
			{					
				if( strName!=null &&!"null".equals(strName) ) {				
					...
					sbPersonLink.append(XSSUtil.encodeForXML(context,strUserFullName));
	...
	public double getTaskPlannedEffort (Context context, String taskId)
		...
		// IR-1152637 - Consider Calendar for planned and Remaining Efforts
		Task task = (Task) DomainObject.newInstance(context,DomainConstants.TYPE_TASK,"Program");
        task.setId(taskId);
        String workingTimePerDay = "480";
        String durationInHours = "";
        int workingTimePerDayInHours = 0;
        String taskEstimatedDuration = "";
        try {
			StringList busSelect = new StringList();
	        busSelect.add(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_DURATION);
	        busSelect.add(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_DURATION_IU);
			Map taskInfo = task.getInfo(context, busSelect);
			Double duration = 0.0;
			if(taskInfo != null && !taskInfo.isEmpty()) {
				taskEstimatedDuration = (String) taskInfo.get(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_DURATION);
				String inputUnit = (String) taskInfo.get(ProgramCentralConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_DURATION_IU);
				if("d".equalsIgnoreCase(inputUnit)){
					WorkCalendar workCalendar = task.getSchedulingCalendar(context);
					if(workCalendar != null) {
			        	 workingTimePerDay = workCalendar.getInfo(context, "attribute[Working Time Per Day].value");
			        }
				}
			}
			workingTimePerDayInHours = Integer.parseInt(workingTimePerDay)/60;
			duration = Double.parseDouble(taskEstimatedDuration) * workingTimePerDayInHours;
			durationInHours = String.valueOf(duration);
		}catch(Exception e) {
			e.printStackTrace();
			workingTimePerDayInHours = 8;
		}
		//String mqlStr = "print bus $1 select $2 dump $3";
		//String task_duration = bus.getAttributeValue(context,attribute_Duration);
		
		//PRG:RG6:R213:Mql Injection:parameterized Mql:18-Oct-2011:start
//		String sCommandStatement = "print bus $1 select $2 dump";
//		resultinHours =  MqlUtil.mqlCommand(context, sCommandStatement,taskId, "attribute["+attribute_Duration+"].unitvalue[h]");
		//PRG:RG6:R213:Mql Injection:parameterized Mql:18-Oct-2011:End
		...
		if(taskVec.size()==0)
		{
			...
			return (Task.parseToDouble(durationInHours)); 
		...
		for (int i = 0; i < taskVec.size(); i++)
		{
			...
			{ 
				...
				iDuration = Task.parseToDouble(taskEstimatedDuration.trim());
				iPercentAllocation = Task.parseToDouble(allo.trim());
				if(iPercentAllocation >0){
					//iAllocation = (((8 * iDuration)/100)*iPercentAllocation);
					iAllocation = (((workingTimePerDayInHours * iDuration)/100)*iPercentAllocation);     	 
				}
				durationInHours = iAllocation + "";
				dTemp = Task.parseToDouble(durationInHours);
				dArr[i] = dTemp;
			}

			if(allo!=null && allo.trim().length() > 0 && durationInHours!=null && durationInHours.trim().length() > 0 )
			{
				//Double allocation = new Double(allo);
				Double allocation  = Task.parseToDouble(allo);
				double alloc = allocation.doubleValue()/100;

				d_item= Task.parseToDouble(durationInHours); 
	...
	public Vector getPredictedEndDateHTML(Context context, String args[])
		...
		String onSchedule = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                "emxProgramCentral.Common.OnSchedule", lang);
        String overdue = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                "emxProgramCentral.Common.Overdue", lang);
        String updateTaskEstimatedFinishDate = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                "emxProgramCentral.Common.UpdateTaskEstimatedFinishDate", lang);
		...
		for (int i = 0; i < relBusObjPageList.size(); i++)
		{
			...
			if(sType != null && sType.equalsIgnoreCase(type_effort)){
				...
				//statusGif = "<img src=\"../common/images/iconStatusGreen.gif\" border=\"0\" alt=\"On Schedule\" />";
				statusGif = "<img src=\"../common/images/iconStatusGreen.gif\" border=\"0\" alt=\""+onSchedule+"\" title=\""+onSchedule+"\" />";
				...
				if(rm_effort!=0){
					...
					if(islatest && NextDueDate.after(nDay))
					{
						...
						//Added:27-July-10:vf2:R210:PRG Bug 062016
						//statusGif = "<img src=\"../common/images/iconStatusRed.gif\" border=\"0\" alt=\"Overdue\" />";
						statusGif = "<img src=\"../common/images/iconStatusRed.gif\" border=\"0\" alt=\""+overdue+"\" title=\""+overdue+"\"/>";
						//End:27-July-10:vf2:R210:PRG Bug 062016
					}
					else if(!islatest)
					{
			...
			else if(sType != null && sType.equalsIgnoreCase(DomainConstants.TYPE_PERSON))
			{
				...
				for(BusinessObjectWithSelectItr itr= new BusinessObjectWithSelectItr(withSelectList); itr.next();)
				{
					...
					taskVec = getEffortIds(context,sEffortId,relPattern,type_TaskManagement,true,false,(short)1);
				...
				if (isLead)
				{
					...
					if(!(estimated_date.equals(predicted_date)))
					{
						...
						String PageHeading = EnoviaResourceBundle.getProperty(context, "ProgramCentral", 
								"emxProgramCentral.Effort.ConfirmDateUpdate", context.getSession().getLanguage());
						//Added:27-July-10:vf2:R210:PRG Bug 062016
						StringBuffer sbApproverLink = new StringBuffer();                           
						sbApproverLink.append("<a href=\"javascript:emxTableColumnLinkClick('" +

								com.matrixone.apps.domain.util.XSSUtil.encodeForURL("../common/emxForm.jsp?form=PMCTaskEstimatedFinishDateUpdateConfirmation&postProcessURL=../programcentral/emxProjectManagementUtil.jsp?mode=EffortEstimatedEndDate&objectId="+id+"&editLink=true&targetLocation=slidein")+"&amp;formHeader="+ XSSUtil.encodeForHTML(context,PageHeading));
								sbApproverLink.append("', '', '', 'true', 'slidein', '','','','','400')\">");						
						//sbApproverLink.append("<img border=\"0\" src=\"../common/images/iconStatusCheckmark.gif\" alt=\"Update Task Estimated Finish Date\"></img>");                                                         
						sbApproverLink.append("<img border=\"0\" src=\"../common/images/iconStatusCheckmark.gif\" alt=\""+updateTaskEstimatedFinishDate+"\" title=\""+updateTaskEstimatedFinishDate+"\"></img>");                                                         
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getMon(Context context, String args[])throws Exception {
		
		return getDay(context, args, "attribute_Monday");
			}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getTue(Context context, String args[]) throws Exception {
		return getDay(context, args, "attribute_Tuesday");
			}
	...
	public Vector getWeekEnding(Context context, String args[])
  	...
  	/**
	  *This Access Function checks access for Week Ending column
	  *	 
	  * @param context the eMatrix <code>Context</code> object
	  * @param args holds the HashMap containg the following arguments:
      * @return It returns false for Tasks opened in the Structured View when "Effort View" is enabled
      * @throws Exception if the operation fails
	  */
   	public boolean hasAccessToWeekEndingDate(Context context, String args[]) throws Exception
  	{
  		HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String tableMenu = (String) programMap.get("tableMenu");
        if(ProgramCentralUtil.isNotNullString(tableMenu) && "PMCWBSTableMenu".equalsIgnoreCase(tableMenu)){
        	return false;
        }
        return true;
  	}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getWed(Context context, String args[]) throws Exception {
		return getDay(context, args, "attribute_Wednesday");
			}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getThu(Context context, String args[]) throws Exception {
		return getDay(context, args, "attribute_Thursday");
			}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getFri(Context context, String args[]) throws Exception	{
		return getDay(context, args, "attribute_Friday");
			}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getSat(Context context, String args[]) throws Exception {
		return getDay(context, args, "attribute_Saturday");
			}
	 ...
	 * @since PMC 10.5.1.2
	 */
	public Vector getSun(Context context, String args[]) throws Exception {
		return getDay(context, args, "attribute_Sunday");
			}
	...
	public Date getTaskPredictedEndDate (Context context,
		...
		DomainObject dm = DomainObject.newInstance(context, objectId);
		String task_weekendingdate = dm.getAttributeValue(context,attribute_TaskFinishDate);
		MapList ml = dm.getRelatedObjects(context,
				PropertyUtil.getSchemaProperty( context, "relationship_hasEfforts" ),
				PropertyUtil.getSchemaProperty( context, "type_Effort" ),
				busSelects,
	...
	public MapList getProjectTaskList (Context context,String[] args)throws Exception
	{
		...
		String ScheduleEffortView = (String) paramMap.get("ScheduleEffortView");


		task.setId(objectId);
		String projectPALId = task.getInfo(context, ProgramCentralConstants.SELECT_PAL_PHYSICALID_FROM_PROJECT); 
		...
		StringList busSelects = new StringList(1);
		busSelects.add(task.SELECT_ID);
		busSelects.add(task.SELECT_PAL_PHYSICALID_FROM_PROJECT);
		busSelects.add(ProgramCentralConstants.SELECT_KINDOF_TASKMANAGEMENT);
		busSelects.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
		if (nExpandLevel!= 1) {
		busSelects.add(EFFORT_ORIGINATOR);
		busSelects.add(EFFORT_ID);
		}
		...
		for(int j = 0; j < mlist.size(); j++){				
			...
			//VUR1 : IR-748710 : for Schedule Vide : Effort View, removing subproject's tasks.
			if(ScheduleEffortView != null && ScheduleEffortView.equalsIgnoreCase("true") && strExpandLevel.equalsIgnoreCase("ALL")) {
				String taskPALId = (String) ((map.get(task.SELECT_PAL_PHYSICALID_FROM_PROJECT)) == null ? (map.get(task.SELECT_PAL_PHYSICALID_FROM_TASK)) : (map.get(task.SELECT_PAL_PHYSICALID_FROM_PROJECT)) );					
				if( !projectPALId.equalsIgnoreCase(taskPALId) && "TRUE".equalsIgnoreCase( (String) map.get(ProgramCentralConstants.SELECT_KINDOF_TASKMANAGEMENT))) {
					mlist.remove(j);
					j--;
				}				
			} 
		...
		if (mlist != null && mlist.size() > 0)
		{
			...
			for (itr = mlist.iterator(); itr.hasNext(); )
			{
				...
				if (eid != null && eid.trim().length() > 0 )
				{
					...
					effortMap.put(DomainConstants.SELECT_RELATIONSHIP_ID,sRelId);
					effortMap.put(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE, map.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE));
					objList.add(effortMap);
				}
				
								
				if (nExpandLevel != 1 ) { // added if block to list task assignees when expand level is more than one
				StringList originatorList	= new StringList();
				StringList effortList = new StringList();
				Object origList = map.get(EFFORT_ORIGINATOR);
				Object effList = map.get(EFFORT_ID);
				if (effList instanceof StringList){
					originatorList =(StringList) map.get(EFFORT_ORIGINATOR);
					effortList =(StringList) map.get(EFFORT_ID);
		        } else if (effList instanceof String) {
					originatorList.add((String) map.get(EFFORT_ORIGINATOR));
					effortList.add((String) map.get(EFFORT_ID));
		        }
					if (null != effortList)
						{			
							Map mAllPersonsMap = new HashMap(); 
							for (int i=0 ,effSize = effortList.size();i<effSize;i++)
							{
												
								String sUSerEffortId = (String) effortList.get(i);
								String sPersonId = (String) originatorList.get(i);
								String sPersonObjId =PersonUtil.getPersonObjectID(context, sPersonId);

								
								if(!mAllPersonsMap.containsKey(sPersonId))
								{   
									int newLevel = Integer.parseInt(sLevel)+1;
									Map mUserMap = new HashMap();
									mUserMap.put("level",String.valueOf(newLevel));							
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getProjectTaskListSubmitted (Context context,String[] args) throws Exception {
		return getEffortForProjectTaskList(context, args, ProgramCentralConstants.STATE_EFFORT_SUBMIT);
					}
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getProjectTaskListPending (Context context,String[] args) throws Exception {
		return getEffortForProjectTaskList(context, args, ProgramCentralConstants.STATE_EFFORT_REJECTED);
					}
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getProjectTaskListApproved (Context context, String[] args) throws Exception {
		return getEffortForProjectTaskList(context, args, ProgramCentralConstants.STATE_EFFORT_APPROVED);
					}
	...
	public boolean showEffortFilter(Context context, String args[]) throws MatrixException
	{
		...
		try {
			...
			if(selectedTable != null && selectedTable.contains("PMCProjectTaskEffort")){
				canShow = true; 
			}
		} catch(Exception e) {
	...
	//---------------------------------New APIs to optimize the performance------------------------------
		...
		public void updateEstDate(Context context, String args[]) throws Exception {
			com.matrixone.apps.program.Task task = (com.matrixone.apps.program.Task) DomainObject.newInstance(context, DomainConstants.TYPE_TASK, "PROGRAM");

			HashMap programMap = (HashMap) JPO.unpackArgs(args);
			HashMap objectMap = (HashMap) programMap.get("requestMap");
			Locale locale = (Locale)objectMap.get("localeObj");

			HashMap paramMap = (HashMap) programMap.get("paramMap");
			String objectId=(String)paramMap.get("objectId");
			task.setId(objectId);
			// String attribute_EstimatedFinishDate=(String)PropertyUtil.getSchemaProperty(context,"attribute_TaskEstimatedFinishDate");    

			String new_date = (String)paramMap.get( "New Value");		
			double clientTZOffset = (new Double((String)objectMap.get("timeZone"))).doubleValue();
			String new_date_input = eMatrixDateFormat.getFormattedInputDate(context,new_date, clientTZOffset,locale);		

			try{                            

				task.updateFinishDate(context,new Date(new_date_input));

			}catch(Exception ex){
				//System.out.println("An Error occurred while processing you request:::"+ex.toString());
				ex.printStackTrace();				
			}

		}
		/**
	 * Gets the effort values for each effort object for dayName
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args holds the HashMap containg the following arguments:
	 *    objectList - MapList containing the Object Id of task and effort
	 ...
	 * @since PMC 10.5.1.2
	 */
	private Vector getDay(Context context, String args[], String dayName)throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		MapList relBusObjPageList =(MapList)programMap.get("objectList");
		HashMap paramMap = (HashMap)programMap.get("paramList");
		Vector columnValues = new Vector(relBusObjPageList.size());
		...
		for (int i = 0; i < relBusObjPageList.size(); i++) {
			...
			String sType = bus.getTypeName();
			if(sType != null && sType.equalsIgnoreCase(type_effort)){
				dbl_mon = getEffortDaily(context, id, dayName);
				...
				dbl_mon = getTaskDaily(context, effortIds, dayName);
				eff_val = "" + Math.round(dbl_mon);
				columnValues.add(eff_val);
			}
		}
	...
	/**
	 * Gets the list of efforts objects which are in given effortState
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args holds the HashMap containg the following arguments:
	 *    objectId - the context task object Id
	 ...
	 * @since PMC 10.5.1.2
	 */
	public MapList getEffortForProjectTaskList(Context context, String[] args, String effortState) throws Exception {
		HashMap paramMap = (HashMap)JPO.unpackArgs(args);
		com.matrixone.apps.program.Task task = (com.matrixone.apps.program.Task) DomainObject.newInstance(context, DomainConstants.TYPE_TASK, "PROGRAM");
		String objectId = (String) paramMap.get("objectId");
		task.setId(objectId);
		...
		if (mlist != null)
		{
			...
			while (mapItr.hasNext())
			{
				...
				if (id != null && id.trim().length() > 0 )
				{
					...
					vec = getEffortIds(context,id,relPattern,type_effort,false,true,(short)1);
