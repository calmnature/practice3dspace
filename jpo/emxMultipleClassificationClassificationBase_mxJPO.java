...
/**
    ...
    public void addAttributeGroup(Context context, String grpName)
        ...
        try {
        	...
        	if(hasLibrarianRole)
        	{
        		...
        		// if(UIUtil.isNotNullAndNotEmpty(grpName) && !grpName.contains(ClassificationConstants.ATTRIBUTE_GROUP_NAME_SUFFIX) && !grpName.contains("Generated")) {
        		// 	grpName = grpName +ClassificationConstants.ATTRIBUTE_GROUP_NAME_SUFFIX;
        		// }
    ...
    public HashMap reparent(Context context, String objectId, String oldParent, String newParent, boolean carryOverInheritedAttributeGroups) throws FrameworkException, Exception
    {
        ...
        if(newParent!=null)
        {
            String newParentUsage=MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", newParent, "attribute["+LibraryCentralConstants.ATTRIBUTE_ClassificationUsage+"]");
            String classUsage=MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", objectId, "attribute["+LibraryCentralConstants.ATTRIBUTE_ClassificationUsage+"]");
            if(!newParentUsage.equalsIgnoreCase(classUsage))
            {
               throw new Exception(EnoviaResourceBundle.getProperty(context,"emxLibraryCentralStringResource",new Locale(context.getSession().getLanguage()),"emxMultipleClassification.Error.MoveClassUsageMismatch")); //FUN113089 do not allow move class if usage value is different
            }
        }
        try 
        {
        	...
        	if (newParent != null) 
        	{
        		context.setCustomData("operation", "move");
            ...
            /*String usage=MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", newParent, "attribute["+LibraryCentralConstants.ATTRIBUTE_ClassificationUsage+"]");
            if(!(usage==null || usage.isEmpty()))
            {
                String usageInterfacePresent=MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", objectId, "interface["+USAGE_INTERFACE_NAME+"]");
                if("TRUE".equalsIgnoreCase(usageInterfacePresent))
                {
                    MqlUtil.mqlCommand(context,"modify bus $1 $2 $3", objectId, LibraryCentralConstants.ATTRIBUTE_ClassificationUsage,usage);
                }
                else
                {
                    LibraryUtil.addUsageInterface(context,objectId,usage,null);
                }
            }
            else
            {
                MqlUtil.mqlCommand(context,"modify bus $1 remove interface $2", objectId, LibraryCentralConstants.USAGE_INTERFACE_NAME);
            }*/
        	

        	//TO invalidate VPLM Cache
            try{
				if(this.isKindOf(context, LibraryCentralConstants.TYPE_GENERAL_CLASS))
					LibraryUtil.invalidateVPLMCatalogueCache(context);
            }catch (MatrixException e){
            	e.printStackTrace();
                throw (new FrameworkException(e));
    ...
    /**
	 * Same method as previous, only difference is called from within Widget.
	 * NOTE: Through this method we no longer perform carryOver of attributes here. That is taken care of in the WebService.
     */
	public HashMap reparentInWidget(Context context, String objectId, String oldParent, String newParent) throws FrameworkException, Exception
    {
        dbg_break();
		StringList busSelects = new StringList();
		Map resultMap = null;
        ...
        try 
        {
        	StringList myAttributeGroups = getAttributeGroups(context, false);
        	// disconnect the Classification from its parent	
        	busSelects.addElement("id");
        	context.setCustomData("operation", "move");
        	resultMap = this.getRelatedObject(context,RELATIONSHIP_SUBCLASS, false, busSelects, null);
        	...
        	//Get Interface of NewParent
    		String[] parent = {newParent,"attribute["+DomainConstants.ATTRIBUTE_MXSYSINTERFACE+"].value"};
    		String newParentInterface = MqlUtil.mqlCommand(context,"print bus $1 select $2 dump",true,parent);
        	myAttributeGroups.addElement(newParentInterface);
	...
	/**
     * 	Same method as previous, only difference is method called from within Widget
     */
    public HashMap reparentInWidget(Context context, String[] args) throws FrameworkException,Exception 
    {
        if (args == null || args.length < 1) {
            throw (new IllegalArgumentException());
        }
        ...
        String objectId = (String) map.get("objectId");
        String oldParent = (String) map.get("oldParent");
        return (HashMap) reparentInWidget(context, objectId, oldParent, (String)map.get("newParent"));
    }
     ...
     * @throws FrameworkException if the operation fails
     */
    public String getRecursiveEndItemCount(Context context)
            throws FrameworkException {
        //changing from int to String for IR-672340-3DEXPERIENCER2020x
        return getAttributeValue(context, ATTRIBUTE_COUNT);
    }
     ...
     * @throws FrameworkException if the operation fails
     */
    public String getRecursiveEndItemCount(Context context, String[] args)
    throws FrameworkException {
        //changing from int to String for IR-672340-3DEXPERIENCER2020x
    ...
    public HashMap reclassify(Context context, String[] args) throws FrameworkException {
        ...
        try {
            ...
            {
				// Changes added by PSA11 start(IR-532214-3DEXPERIENCER2018x).
    ...
    */
    ...
    throws Exception {
        ...
        while(iterator.hasNext()){
            ...
            if(agVisible(context, attrGrpName)){
	...
	private static boolean agVisible(Context context, String ag) throws FrameworkException {
		try {
			String cmd      = "print interface $1 select hidden dump" ;
			String result   = MqlUtil.mqlCommand(context, cmd, ag).trim(); // trim the newline
    ...
    public static MapList getAttributeGroupInheritedFrom(Context context,
        ...
        try {
            ...
            while (recordIter.hasNext()) {
                ...
                //for auto-name :: use Title
                String title = new DomainObject(id).getInfo(context, SELECT_ATTRIBUTE_TITLE);
                        ...
                        if(idOfConnCls.equals(clsid)){
                            ...
                            if(!isprinterFriendly) {
                                href = "<a href=\"javascript:showModalDialog("
                                    ...
                                    + "'860','520');\"" + ">" + XSSUtil.encodeForXML(context, nameOfConnCls) + "</a>";
                            } else {
                                href = nameOfConnCls;
                            }
                        }
    ...
    */
    public static Vector isAttributeGroupInherited(Context context, String[] args)
      ...
      public static Vector findAllParentClassificationsInCurrentTaxonomy(Context context, String strParentId, String strChildId) throws FrameworkException
      {
        ...
        strMQLBuffer.append(" recurse to all select bus $3 where $4 dump $5");
        String strResult            = MqlUtil.mqlCommand(context, strMQLBuffer.toString(), true,
                                                            strParentId,
                                                            strRelSubclass,
                                                            "id",
                                                            "from["+strRelClassifiedItem+"].to.id=="+strChildId.trim()+"",
                                                            "|"
                                                        );
    ...
    public File exportLibrary(Context context, String args[]) throws FrameworkException{
        ...
        try{
            ...
            //This check is added for Import/Export functionality of IP Protection and Export Control
            boolean fromIPProtection=Boolean.parseBoolean((String) programMap.get("fromIPProtection"));
            ...
            if(objectType.equals(TYPE_GENERAL_LIBRARY))
                libXML=classification.createTaxonomyXML(context,objectId,GENERAL_LIBRARY_ELEMENT_NAME, false, fromIPProtection);
            else if(objectType.equals(TYPE_PART_LIBRARY))
                libXML=classification.createTaxonomyXML(context, objectId, PART_LIBRARY_ELEMENT_NAME, false, fromIPProtection);
            else if(objectType.equals(TYPE_DOCUMENT_LIBRARY))
                libXML=classification.createTaxonomyXML(context, objectId, DOCUMENT_LIBRARY_ELEMENT_NAME, false, fromIPProtection);
            else{
                ...
                libXML=classification.createTaxonomyXML(context, objectId, aliasName, false, fromIPProtection);
            }
            Iterator itr=subClassesList.iterator();
            while(itr.hasNext()){
                Map subClassMap=(Map)itr.next();
                String generalClassId=(String)subClassMap.get(SELECT_ID);
                Element generalclassXML =getAttributeGroupandSubClassesforGL(context, generalClassId, false, fromIPProtection);
            ...
            gLibExportFile = new java.io.File(workspace, getLibraryExportFileName(context, objectId));
    ...
    private String getLibraryExportFileName(Context context, String objectId) throws Exception{
    	Calendar calen=Calendar.getInstance();
        int date=calen.get(Calendar.DAY_OF_MONTH);
        int month=calen.get(Calendar.MONTH)+1;
        int yr=calen.get(Calendar.YEAR);
		
		// Changes added by PSA11 start(IR-478456-3DEXPERIENCER2018x).
		// Changes added by PSA11 start(IR-536976-3DEXPERIENCER2018x).
     ...
     * @throws FrameworkException
     */
    public Element getAttributeGroupandSubClassesforGL(Context context,String classId, boolean forWidget, boolean fromIPProtection) throws FrameworkException{
        ...
        try{
            ...
            if(typeName.equals(TYPE_GENERAL_CLASS))
                classXML=classification.createTaxonomyXML(context, classId,GENERAL_CLASS_ELEMENT_NAME, forWidget, fromIPProtection);
            else if(typeName.equals(TYPE_PART_FAMILY))
                classXML=classification.createTaxonomyXML(context, classId,PART_FAMILY_ELEMENT_NAME, false, fromIPProtection);
            else if(typeName.equals(TYPE_DOCUMENT_FAMILY))
                classXML=classification.createTaxonomyXML(context, classId,DOCUMENT_FAMILY_ELEMENT_NAME, false, fromIPProtection);
            else{
                ...
                aliasName=aliasName.substring(aliasName.indexOf("_")+1, aliasName.length());
                classXML=classification.createTaxonomyXML(context, classId,aliasName, forWidget, fromIPProtection);
            }
            ...
            while(itr.hasNext()){
                Map subClassMap=(Map)itr.next();
                String generalClassId=(String)subClassMap.get(SELECT_ID);
                Element subClassesXML=getAttributeGroupandSubClassesforGL(context, generalClassId, forWidget, fromIPProtection);
