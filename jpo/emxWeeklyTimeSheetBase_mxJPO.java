...
public class emxWeeklyTimeSheetBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    private static final String SELECT_EFFORT_PARENT_TASK_ID = "to[" + RELATIONSHIP_HAS_EFFORTS + "].from.id";
    /** value for efforts total effort. */
    private static final String SELECT_TASK_TO_EFFORT_TOTAL_EFFORT = "from["+RELATIONSHIP_HAS_EFFORTS+"].to.attribute["+
            ATTRIBUTE_TOTAL_EFFORT+"].value";
    /** value for effort originator. */
    private static final String SELECT_TASK_TO_EFFORT_ORIGINATOR = "from["+RELATIONSHIP_HAS_EFFORTS+"].to.attribute["+
            DomainConstants.ATTRIBUTE_ORIGINATOR+"].value";
    /** value for effort week ending data. */
    private static final String SELECT_TASK_TO_EFFORT_WEEK_END_DATE = "from["+RELATIONSHIP_HAS_EFFORTS+"].to.attribute["+
            ATTRIBUTE_WEEK_ENDING_DATE+"].value";
    /** value for task parent id through relationship subtask. */
    private static final String SELECT_TASK_PARENT_ID = "to[" + RELATIONSHIP_SUBTASK + "].from.id";
    /** value for task parent type through relationship subtask. */
    private static final String SELECT_PARENT_TYPE = "to[" + RELATIONSHIP_SUBTASK + "].from.type";
    /** used to check parent type is project. */
    private static final String SELECT_IS_PARENT_TYPE_KINDOF_PROJECT = "to[" + RELATIONSHIP_SUBTASK + "].from.type.kindof[" + TYPE_PROJECT_SPACE + "]";
    /** used to check parent type is phase. */
    private static final String SELECT_IS_PARENT_TYPE_KINDOF_PHASE = "to[" + RELATIONSHIP_SUBTASK + "].from.type.kindof[" + TYPE_PHASE + "]";
    /** value for Effort state through relationship subtask. */
    private static final String SELECT_EFFORT_CURRENT_STATE = "from[" + RELATIONSHIP_HAS_EFFORTS + "].to.current";
    ...
    /** value of PAL ID of Task of an Effort **/
    public static final String SELECT_PAL_OF_TASK_OF_EFFORT = "to[hasEfforts].from.to["+ RELATIONSHIP_PROJECT_ACCESS_KEY + "].from.id";

    /** used to check parent type is weekly timesheet. */
    private static final String SELECT_KINDOF_WEEKLY_TIMESHEET = "type.kindof[" + TYPE_WEEKLY_TIMESHEET + "]";
    /**
     * Constructor
     *
     * @param context
            ...
            if(domObj.isKindOf(context,
                ...
                for(int i=0; i<modSize; i++)
                {
                    ...
                    if(mp!=null && mp.size()>0) {
                        projectId = (String)mp.get(DomainConstants.SELECT_ID);
                        mp.put("level", "1");
                    } else {
                        nonProjectTask = true;
                        mEffort.put("isSimpleTask", "true");
                        mEffort.put("level", "1");
                    }
                    String strType = "";
    ...
    public Vector getWeekNo(Context context, String[] args) throws Exception {
        ...
        while (objectListIterator.hasNext()) {
            ...
            String weekEndDate = (String) mapObject.get(SELECT_ATTRIBUTE_ENDING_DATE);
            Date weekEndingDate = eMatrixDateFormat.getJavaDate(weekEndDate);

            Calendar cal = Calendar.getInstance();
            cal.setTime(weekEndingDate);

            // Both of these are defaults
            cal.setFirstDayOfWeek(Calendar.MONDAY);
            cal.setMinimalDaysInFirstWeek(4);
            Date tempDate = cal.getTime();

            int strWeekNo = cal.get(Calendar.WEEK_OF_YEAR);
    ...
    public MapList getSubmittedTimesheetForApprover(Context context, String[] args) throws Exception {
        ...
        String invokeFromODTFile    = (String)programMap.get("invokeFrom"); //Added for ODT
        String ODT_TEST_CASE = "TestCase";

        MapList returnList=new MapList();
        // IR-1192787-3DEXPERIENCER2024x
        // add try/catch with push context
        // maybe the user does not have access to the project or timesheet
        ...
        try {
            ...
            isContextPushed = true;
                                    ...
                                    String objectWhereSimpleTask = "(current == Assign || current == Active || current == Review || current == Complete) && (!from[Subtask]) && (!to[Project Access Key]) && (from[hasEfforts])";
			//Member's Projects 
            ...
            //Get Assigned Non-Project Tasks for the logged in User > Adding support for Non-Project Tasks with IR-1133908
            MapList assignedNonProjectTaskList = ProgramCentralUtil.getAssignments(context, person, true/*personAssignments*/, true/*groupAssignments*/,"Task,Phase", projectInfo, projectInfo, objectWhereSimpleTask);

            String taskIds[] = new String[assignedNonProjectTaskList.size()];
            for(int i =0,count= assignedNonProjectTaskList.size();i<count;i++) {
                Map simpleTask = (Map)assignedNonProjectTaskList.get(i);
                String taskId = (String) simpleTask.get(SELECT_ID);
                taskIds[i] = taskId;
            }
            BusinessObjectWithSelectList taskWithSelectList = new BusinessObjectWithSelectList();
            StringList busSelectItems = new StringList();
            busSelectItems.add("from[hasEfforts].to.id"); //get EFFORTS
            busSelectItems.add("from[hasEfforts].to.to[Effort].from.id");//get TIMESHEET for which the effort is submitted
            busSelectItems.add("from[hasEfforts].to.to[Effort].from.current");//get CURRENT state of the timeSheets
            taskWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, taskIds, busSelectItems);

            Set effortsSetForNonProjectTasks = new HashSet(); // Will hold efforts for submitted Timesheets
            Set submittedTimesheetSetForNonProjectTasks = new HashSet();// Will hold all the Submitted Timesheets
            for(int j=0,taskWithSelectListSize=taskWithSelectList.size();j<taskWithSelectListSize;j++) {
                BusinessObjectWithSelect bws = taskWithSelectList.getElement(j);
                StringList effortIdList             = bws.getSelectDataList("from[hasEfforts].to.id");
                StringList timesheetIdList          = bws.getSelectDataList("from[hasEfforts].to.to[Effort].from.id");
                StringList timesheetCurrentState    = bws.getSelectDataList("from[hasEfforts].to.to[Effort].from.current");
                // IR-1192787-3DEXPERIENCER2024x
                // add if check for null timesheetCurrentState
                // maybe the user does not have access to the project or timesheet
                if (timesheetCurrentState != null) {
                    int size = timesheetCurrentState.size();
                    for(int k=0;k < size ; k++) {
                        String timesheetState = timesheetCurrentState.get(k);
                        if ("Submit".equalsIgnoreCase(timesheetState)) {
                            if(effortIdList != null && effortIdList.size()>0) {
                                effortsSetForNonProjectTasks.addAll(effortIdList);
                            }
                            if(timesheetIdList != null && size >0) {
                                submittedTimesheetSetForNonProjectTasks.add(timesheetIdList.get(k));
                            }
                        }
                    }
                }
            ...
            StringList timesheetSelectable = new StringList();
            timesheetSelectable.add(SELECT_NAME);
            timesheetSelectable.add(SELECT_ID);
            timesheetSelectable.add("attribute[Week Ending Date].value");

            StringList stateList=new StringList();
            stateList.add(ProgramCentralConstants.STATE_EFFORT_SUBMIT);

            String[] timeSheetIdArray = new String[submittedTimesheetSetForNonProjectTasks.size()];
            timeSheetIdArray = (String[]) submittedTimesheetSetForNonProjectTasks.toArray(timeSheetIdArray);

            BusinessObjectWithSelectList timeSheetWithSelectList = new BusinessObjectWithSelectList();
            timeSheetWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, timeSheetIdArray,timesheetSelectable);
            MapList timesheetInfoList = FrameworkUtil.toMapList(timeSheetWithSelectList);

            MapList weeklytimeSheetInfoForNonProjectTasks = new MapList();
            MapList effortListOfSimpleTask1 = new MapList();
            for(int i=0 , count = timesheetInfoList.size();i<count;i++) {
                int effortCount = 0;
                WeeklyTimesheet weeklytimesheet = new WeeklyTimesheet();
                Map weeklyTimesheetInfo =  (Map) timesheetInfoList.get(i);
                String timeSheetId = (String) weeklyTimesheetInfo.get(SELECT_ID);
                weeklytimesheet.setId(timeSheetId);
                effortListOfSimpleTask1=weeklytimesheet.getEfforts(context, stateList, null, null, new StringList(SELECT_ID));
                for(int j=0, k =effortListOfSimpleTask1.size() ;j<k;j++) {
                    Map effortMap=(Map)effortListOfSimpleTask1.get(j);
                    String effortid=(String)effortMap.get(SELECT_ID);
                        if(effortsSetForNonProjectTasks.contains(effortid)) {
                            effortCount++;
                            }
                        }
                if(effortCount>0) {
                    weeklyTimesheetInfo.put("level", "1");
                    weeklytimeSheetInfoForNonProjectTasks.add(weeklyTimesheetInfo);
                }
            }

            StringList projectIdList = new StringList();
            for(int i=0, memberProjectSize=memberProject.size();i<memberProjectSize;i++) {
            ...
            String objIds[] = new String[projectIdList.size()];
            projectIdList.toArray(objIds);

            StringList busSelect = new StringList("from[Effort].to.id");

            BusinessObjectWithSelectList objectWithSelectList = new BusinessObjectWithSelectList();
            ...
            for(int j=0,objectWithSelectListSize=objectWithSelectList.size();j<objectWithSelectListSize;j++) {
                BusinessObjectWithSelect bws = objectWithSelectList.getElement(j);
                StringList effortIdList         = bws.getSelectDataList("from[Effort].to.id");
                if(effortIdList != null && effortIdList.size()>0) {
                    relatedEffortID.addAll(effortIdList);
                }
            }
    ...
    //      strList.add(ProgramCentralConstants.STATE_EFFORT_SUBMIT);
            ...
            int weeklytimeSheetSize =weeklytimeSheet.size();
            if(relatedEffort.size()>0 || weeklytimeSheetSize >0) {
                WeeklyTimesheet weeklyimtsheet = new WeeklyTimesheet();
                for(int k=0;k<weeklytimeSheetSize;k++) {
                    int count=0;
                    Map WeekltTimesheetMap=(Map)weeklytimeSheet.get(k);
                    String id=(String)WeekltTimesheetMap.get(SELECT_ID);
                    weeklyimtsheet.setId(id);
                    //effortList=weeklyimtsheet.getEfforts(context, strList, null, null);
                    effortList=weeklyimtsheet.getEfforts(context, stateList, null, null, new StringList(SELECT_ID));
                    int effortListSize = effortList.size();
                    for(int j=0;j<effortListSize;j++) {
            ...
            //Remove duplicate timesheets in case of mixed cases
            int weeklytimeSheetInfoSize = weeklytimeSheetInfoForNonProjectTasks.size();
            int returnListSize = returnList.size();
            if (weeklytimeSheetInfoSize > 0 && returnListSize > 0) {
                for (int i=0,k = weeklytimeSheetInfoForNonProjectTasks.size();i<k;i++) {
                    int timesheetCount = 0;
                    Map simpleTaskTimeSheetMap = (Map) weeklytimeSheetInfoForNonProjectTasks.get(i);
                    String simpletTaskTimeSheetId = (String) simpleTaskTimeSheetMap.get(SELECT_ID);
                    for(int j=0,m = returnList.size();j<m;j++) {
                        Map ProjectTimeSheetMap = (Map) returnList.get(j);
                        String projectTimeSheetId = (String) ProjectTimeSheetMap.get(SELECT_ID);
                        if(simpletTaskTimeSheetId.equalsIgnoreCase(projectTimeSheetId)) {
                            timesheetCount++;
                            break;
                        }
                    }
                    if(timesheetCount == 0) {
                        returnList.add(simpleTaskTimeSheetMap);
                    }

                }
            }
            else if(returnListSize == 0 && weeklytimeSheetInfoSize > 0) {
                returnList.addAll(weeklytimeSheetInfoForNonProjectTasks);
            }
        } catch (Exception ex) {
            //ex.printStackTrace();  
            DebugUtil.debug("getSubmittedTimesheetForApprover "+ ex.getMessage());
            throw ex;
        }
        finally {
            if (isContextPushed)
        ...
        // System.out.println("getSubmittedTimesheetForApprover time : "+(end-start));
        return returnList;
}
    /**
     * This method used to list Weekly Timesheet which are in Submitted state for approver.
...
//          sbWhere.append("attribute["+DomainConstants.ATTRIBUTE_PROJECT_ACCESS+"] ~~ \""+ROLE_PROJECT_OWNER+"\"");
            ...
            for(int j=0,projecListSize = projecList.size();j<projecListSize;j++){
                Map mpProject = (Map)projecList.get(j);
                if("TRUE".equalsIgnoreCase((String)mpProject.get("current.access[modify]")) || vRoles.contains(strResourceManager))
                {
                String strProjectId = (String)mpProject.get(DomainConstants.SELECT_ID);
                MapList mlTimesheets = getTimesheetsToApprove(context,strProjectId,strState);
                for(int k=0,mlTimesheetsSize = mlTimesheets.size();k<mlTimesheetsSize;k++){
                    timesheetToApprove.add(mlTimesheets.get(k));
                }
            }
            }
            StringList strList = new StringList();
            for(int m=0,timesheetToApproveSize=timesheetToApprove.size();m<timesheetToApproveSize;m++) {
    ...
    public Vector getStatusIcon(Context context, String[] args)
        ...
        try {
            ...
            for(int i=0;i<size;i++){
                Map timeSheetMap = (Map) objectList.get(i);
                objIdArr[i] = (String) timeSheetMap.get(DomainObject.SELECT_ID);
            }

            StringList busSelect = new StringList(WeeklyTimesheet.SELECT_CURRENT);
            BusinessObjectWithSelectList objectWithSelectList = null;
            if(ODT_TEST_CASE.equalsIgnoreCase(invokeFromODTFile)){ //Added for ODT usage
                objectWithSelectList =
                        ProgramCentralUtil.getObjectWithSelectList(context, objIdArr, busSelect,false);
            }else{
                objectWithSelectList =
                        ProgramCentralUtil.getObjectWithSelectList(context, objIdArr, busSelect);
            }

            for(int i=0;i<size;i++) {
                String statusGif = ProgramCentralConstants.EMPTY_STRING;
                BusinessObjectWithSelect bws = objectWithSelectList.getElement(i);
                String timesheetStatus = bws.getSelectData(WeeklyTimesheet.SELECT_CURRENT);
                if("CSV".equalsIgnoreCase(exportFormat) || "HTML".equalsIgnoreCase(exportFormat)){
                    statusGif=timesheetStatus;
                }
                else{
    ...
    public String getTimesheetName(Context context, String[] args) throws MatrixException {
        ...
        try {
            ...
            if(!"null".equals(copyObjectId) && null!= copyObjectId && !"".equals(copyObjectId)){
                ...
                name = MqlUtil.mqlCommand(context, mqlCmd, copyObjectId, DomainConstants.SELECT_NAME);
    ...
    public Vector getTimeSheetOwnerName(Context context, String[] args)
        ...
        try {
            ...
            String SELECT_PERSON_ID = "to["+RELATIONSHIP_WEEKLY_TIMESHEET+"].from.id";
            StringList busSelect = new StringList(SELECT_PERSON_ID);
            ...
            for(int i=0;i<size;i++) {
                BusinessObjectWithSelect bws = objectWithSelectList.getElement(i);
                strPerosnId         = bws.getSelectData(SELECT_PERSON_ID);
                dobj.setId(bws.getObjectId(context));

                StringBuffer sbownerLink = new StringBuffer();
                String ownerName = PersonUtil.getFullName(context,dobj.getOwner(context).toString());
                if("CSV".equalsIgnoreCase(exportFormat) || "HTML".equalsIgnoreCase(exportFormat)){
                    sbownerLink.append(ownerName);
                }else {
...
//              sbownerLink.append(XSSUtil.encodeForHTML(context,ownerName)); //doesn't support characters such as á,é,í,ó,ú,à,è,ì,ò,ù,â,ê,î,ô,û,ç,ñ  
				sbownerLink.append(XSSUtil.encodeForXML(context,ownerName));
        ...
        try{
            ...
            for(int i=0;i<mpEfforts.size();i++){
                ...
                if(d==0.0){
                    ...
                    //Added for Non-Project Tasks
                    try {
                        if(ProgramCentralUtil.isNullString(strProject) || "".equalsIgnoreCase(strProject)) {
                        ...
                        Object oTaskName = taskMap.get(SELECT_NAME);
                        strTaskWithZeroEffort.add(String.valueOf(oTaskName));
                        continue;
                    }finally {
                        if(isContextPushed) {
                            ContextUtil.popContext(context);
    ...
    public void submit(Context context, String[] args) throws MatrixException{
        try {
			...
			//START DK for IR-1187939-3DEXPERIENCER2021x
   		boolean isContextPushed = false;
			...
			DomainObject objPrj = DomainObject.newInstance(context);
			DomainObject domObj = DomainObject.newInstance(context);
			//END DK for IR-1187939-3DEXPERIENCER2021x
                        ...
                        && !strType.equals("null"))
                {
                    if(strType.equalsIgnoreCase(DomainConstants.TYPE_PROJECT_SPACE)){
                        if(strProjectLead.equalsIgnoreCase(strPreference)){
                        	//IR-1238375 - Cannot submit weekly timesheet after rejection
							ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
							isContextPushed = true;
...
//                                isContextPushed = true;
			...
			//e.printStackTrace();   
			DebugUtil.debug("submit "+ e.getMessage());
                            ...
                            if(orgList!=null && orgList.size()>0) {
                                        ...
                                        && !strOrgId.equals("null"))
                                {
                                    for(int k=0;k<slOrg.size();k++){
										domObj.setId(slOrg.get(k).toString());
		...
		//	e.printStackTrace(); 
			DebugUtil.debug("submit 2 "+ e.getMessage());
			throw new MatrixException(e);
                                            }finally {
			if (isContextPushed)
			{
			...
			//e.printStackTrace();  
			DebugUtil.debug("submit 3 "+ e.getMessage());
			throw new MatrixException(e);
        }
    }
        ...
        try {
            ...
            if(objTimesheeDomainObject.isKindOf(context,
                ...
                if(strState.equalsIgnoreCase(STATE_WEEKLY_TIMESHEET_APPROVED)){
                    ...
                    MqlUtil.mqlCommand(context, "notice $1", sErrMsg);
                }
            }
            ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
            try {
    ...
    private static ConcurrentHashMap<String, Map<String, String>> _cache = new ConcurrentHashMap<String, Map<String, String>>();
    private static final String GLOBAL_TENANT_UID = "GLOBAL_TENANT";
    /**
     * @param context - logged in person context
     * @return Tenant name from context
     */
    public static String getTenant(Context context) {
        String tenantUID = context.getTenantUid();
        if (UIUtil.isNullOrEmpty(tenantUID)) {
            tenantUID = GLOBAL_TENANT_UID;
        }
        return tenantUID;
    }
    /**
     * @param context - logged in person context
     * @param key - name of expression (Not symbolic name)
     * @param reload - If cache needs to be reloaded
     * @return Value of the expression as String
     */
    public static String getCache(Context context, String key, boolean reload) {
        try {
            String tenantUID = getTenant(context);
            
            // If tenant cache is absent add it otherwise get previous cache.
            _cache.putIfAbsent(tenantUID, new ConcurrentHashMap<String, String>());
            Map<String, String> tenantCache = _cache.get(tenantUID);
            
            // if reload then remove any previous cache
            if (reload) {
                tenantCache.remove(key);
            }
            
            // if cache is not present for the key add it.
            // If expression doesn't exists or if value is not boolean then expression value is false by default.
            return tenantCache.computeIfAbsent(key,  (k) -> {
                String ret = "";
                try {
                    String output = MqlUtil.mqlCommand(context, "list expression $1", k);
                    if (k.equals(output)) {
                        ret = MqlUtil.mqlCommand(context, "print expression $1 select $2 dump $3", k, "value", "|");
                    } else {
                    	//System.out.println(String.format("Missing expression - %s ", k));
                    }
                } catch (Exception e) {
                    //e.printStackTrace();  
                    DebugUtil.debug("getCache "+ e.getMessage());
                }
                return ret;
            });
        } catch (Exception ex) {
            //ex.printStackTrace();  
            DebugUtil.debug("getCache 2 "+ ex.getMessage());
            throw ex;
        }
    }
    ...
    public void approve(Context context, String[] args) throws Exception{
        try {
			...
			String strDisableTaskSynch = getCache(context, "DPM_DisableTaskSynchFromTimesheet", false);
			ContextUtil.startTransaction(context, true);
			boolean isContextPushed = false;
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            String strTimesheetId = (String)programMap.get("objectId");
			...
			if(objTimesheeDomainObject.isKindOf(context, TYPE_WEEKLY_TIMESHEET)) {
                if(STATE_WEEKLY_TIMESHEET_SUBMIT.equalsIgnoreCase(strState)) {
                  StringList busSelects =new StringList(SELECT_ID);
                      busSelects.add(SELECT_CURRENT);
                      busSelects.add(SELECT_TASK_ID);
                      busSelects.add(SELECT_PAL_OF_TASK_OF_EFFORT);

                    WeeklyTimesheet wt = new WeeklyTimesheet(strTimesheetId);
                    //MapList mpEfforts = wt.getEfforts(context, null, null, null);
                    MapList mpEfforts = wt.getEfforts(context, null, null, null,busSelects);
                ...
                try {
					...
					//e.printStackTrace();  
					DebugUtil.debug("approve "+ e.getMessage());
			...
			}else if(objTimesheeDomainObject.isKindOf(context, TYPE_EFFORT)){
                if(strState.equalsIgnoreCase(STATE_WEEKLY_TIMESHEET_SUBMIT)){
                    objTimesheeDomainObject.promote(context);
					StringList busSelectList = new StringList(2);
                    busSelectList.add(SELECT_TASK_ID);
                    busSelectList.add("to[hasEfforts].from.to[Project Access Key]");
                    ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
					isContextPushed = true;
                    try {
						if( UIUtil.isNullOrEmpty(strDisableTaskSynch) || !"true".equalsIgnoreCase(strDisableTaskSynch) ) {
                        ...
                        Map effortInfo = objTimesheeDomainObject.getInfo(context,busSelectList);
                        String taskId = (String) effortInfo.get(SELECT_TASK_ID);

						//renamed getPercentageComplete(context,taskId,strEffortId);
                        setTaskPercentageCompleteForEffort(context,taskId,strTimesheetId);
						
						//check if the task for which effort submitted has parent
						String pal = (String) effortInfo.get("to[hasEfforts].from.to[Project Access Key]");						
						if(pal != null && !"false".equalsIgnoreCase(pal)) {
                        com.matrixone.apps.common.Task task = new com.matrixone.apps.common.Task(taskId);
							...
							sl.add(SELECT_ID);
							Map projectMap = task.getProject(context, sl);
							...
							if(ProgramCentralConstants.PROJECT_SCHEDULE_AUTO.equalsIgnoreCase(projectSchedule)){
								if(ODT_TEST_CASE.equalsIgnoreCase(invokeFromODTFile)) {
									...
									TaskDateRollup.rolloutProject(context,new StringList(projectId),false);
								}else {
						...
						//e.printStackTrace();   
						DebugUtil.debug("approve 2 "+ e.getMessage());
						throw new FrameworkException(e);
					}
                    finally {
                ...
                try {
					...
					//e.printStackTrace();   
					DebugUtil.debug("approve 3 "+ e.getMessage());
					throw new FrameworkException(e);
                } finally {
					if (isContextPushed)
					{
			...
			ContextUtil.abortTransaction(context);
			//e.printStackTrace();   
			DebugUtil.debug("approve 4 "+ e.getMessage());
			throw new FrameworkException(e);
        }
    }
		...
		//try{
            ...
            for(int i=0;i<mpEfforts.size();i++){
                ...
                if(isApprover) {
					...
					//ContextUtil.commitTransaction(context);
            //to improve performance, same is executed through trigger
                /*
                    try{
                        ProgramCentralUtil.pushUserContext(context);
                        PropertyUtil.setGlobalRPEValue(context,"PERCENTAGE_COMPLETE", "true");
                        String taskId = (String)effort.get(SELECT_TASK_ID);
                        Task task = new Task(taskId);
                        String PalID = (String)effort.get(SELECT_PAL_OF_TASK_OF_EFFORT );
						//renamed getPercentageComplete(context,taskId,strEffortId);
						setTaskPercentageCompleteForEffort(context,taskId,strEffortId);
    ...
    public int triggerApproveEntireTimesheet(Context context, String[] args) throws Exception{
        try {
			...
			//e.printStackTrace();  
			DebugUtil.debug("triggerApproveEntireTimesheet "+ e.getMessage());
            throw new FrameworkException(e.getMessage());
        }
    }
    ...
    public int triggerApproveTimesheet(Context context, String[] args) throws Exception{
        try {
                ...
                for(int i=0;i<mpEfforts.size();i++){
                    ...
                    if(isApprover) {
                        ...
                        try {
							...
							PropertyUtil.setGlobalRPEValue(context,"PERCENTAGE_COMPLETE", "true");
							PropertyUtil.setGlobalRPEValue(context,"NO_CRITICAL_TASK_NOTIFICATION", "true");
                            Map effortInfo = objEffort.getInfo(context, busSelectList);
                            String taskId = (String) effortInfo.get(SELECT_TASK_ID);
							//renamed getPercentageComplete(context,taskId,strEffortId);
							setTaskPercentageCompleteForEffort(context,taskId,strEffortId);
							//check if the task for which effort submitted has parent
							...
							//e.printStackTrace();  
							DebugUtil.debug("triggerApproveTimesheet "+ e.getMessage());
							throw new FrameworkException(e);
                            }
						finally {
							PropertyUtil.setGlobalRPEValue(context,"PERCENTAGE_COMPLETE", "false");
			...
			//e.printStackTrace();  
			DebugUtil.debug("triggerApproveTimesheet 2 "+ e.getMessage());
            throw new FrameworkException(e.getMessage());
        }
    }
    /**
    ...
    public StringList addTask(Context context, String[] args) throws MatrixException
    {
        ...
        /* Commenting method emxWeeklyTimeSheet:addTask as it not being used currently.
         * Previously it was configured on command PMCWeeklyTimesheetAddTask.
         *
        try
        {
    ...
    public MapList expandProject(Context context, String[] args) throws MatrixException{
        ...
        /* Commented method emxWeeklyTimeSheet : expandProject as it not being used currently.
         * Previously it was configured on command PMCWeeklyTimesheetAddTask as expandProgram.
         *
        try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public MapList expandProjects(Context context, String[] args) throws MatrixException{
        ...
        try {
            ...
            String personId = (String)programMap.get("parentId");
            Task task = new Task();
            task.setId(taskId);
            boolean isProjectSpace = false;
            if(task.isKindOf(context, DomainConstants.TYPE_PROJECT_SPACE)){
            ...
            String strPreference = task.getAttributeValue(context, ATTRIBUTE_EFFORT_SUBMISSION);
            Person person;
            if (programMap.containsKey("fromApprover")) { //to check if expandProjects Method is called from To Approve table
                person = new Person(personId);
            }
            else {
            person = new Person(PersonUtil.getPersonObjectID(context));
            }
            StringList task_busSelects =  new StringList(6);
            task_busSelects.add(task.SELECT_ID);
            task_busSelects.add(task.SELECT_NAME);
    ...
    public Vector getDayEffort(Context context, String[] args, String strAttributeName) throws MatrixException
    {
        ...
        try
        {
            ...
            while(objectListIterator.hasNext())
            {
                ...
                String isSimpleTask = (String) mapObject.get("isSimpleTask");
                if("TRUE".equalsIgnoreCase(isSimpleTask)) {
                    sLevel = Integer.toString(Integer.parseInt(sLevel)+1);
                }
                String strEffort = "";
                if("2".equalsIgnoreCase(sLevel))
                {
                    ...
                    if(mProjectEffortMap.size()!=0 && mProjectEffortMap.containsKey(sParentId)){
    ...
    public Vector getTotalHrsForTimesheet(Context context,String[] args) throws MatrixException {
        ...
        try {
            ...
            while (objectListIterator.hasNext()) {
                ...
                if(timesheetId != null) {
                    MapList effortsMapList = new MapList();
                    weeklyTimesheet.setId(timesheetId);
                    ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
                    try {
                    StringList busSelect =new StringList(SELECT_TASK_ID);
                    busSelect.add("attribute["+ATTRIBUTE_TOTAL_EFFORT+"]");
                    //effortsMapList = weeklyTimesheet.getEfforts(context, null, null, null);
                    effortsMapList = weeklyTimesheet.getEfforts(context, null, null, null,busSelect);
                    }finally {
                        ContextUtil.popContext(context);
                    }
                    if(effortsMapList != null) {
                        double totalEffort = 0.0;
                        for(int i = 0,effortsMapListSize = effortsMapList.size(); i<effortsMapListSize; i++){
                            Map effort = new HashMap();
                            effort = (Map)effortsMapList.get(i);
        ...
        try {
            ...
            while (objectListIterator.hasNext()) {
                ...
                if(timesheetId != null) {
                    ...
                    ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
                    try {
                        //effortMapList = weeklyTimesheet.getEfforts(context, strStateList, null, null);
                        effortMapList = weeklyTimesheet.getEfforts(context, strStateList, null, null, new StringList("attribute["+ATTRIBUTE_TOTAL_EFFORT+"]"));
                    } finally {
                        ContextUtil.popContext(context);
                    }
                    if(effortMapList != null) {
                        double totalEffort = 0.0;
                        for(int i = 0,effortsSize = effortMapList.size(); i<effortsSize; i++){
                            Map effort = new HashMap();
                            effort = (Map)effortMapList.get(i);
                            totalEffort = totalEffort + Task.parseToDouble((String)effort.get("attribute["+ATTRIBUTE_TOTAL_EFFORT+"]"));
                        }
                        String strTotalEffort = Double.toString(totalEffort);
                        if(strTotalEffort.contains("."))
    ...
    public StringList checObjectType(Context context,String[] args)
        ...
        {
            //e.printStackTrace();   
            DebugUtil.debug("checObjectType "+ e.getMessage());
            throw e;
        }
            }
    ...
    public StringList checObjectTypeForRemainingEfforts(Context context,String[] args)
        ...
        {
            //e.printStackTrace();   
            DebugUtil.debug("checObjectTypeForRemainingEfforts "+ e.getMessage());
            throw e;
        }
            }
    ...
    public StringList checObjectTypeForApprover(Context context,String[] args)
        ...
        {
            //e.printStackTrace();   
            DebugUtil.debug("checObjectTypeForApprover "+ e.getMessage());
            throw e;
        }
            }
    ...
    public String getMonthName(Context context,int nCurrentMonthValue)  throws Exception {
        try {
            ...
            //exp.printStackTrace();   
            DebugUtil.debug("getMonthName "+ exp.getMessage());
            throw exp;
        }
    }
    ...
    public MapList getPhaseChildLaborReport(Context context, String[] args) throws Exception
    {
        ...
        {
            //exp.printStackTrace();  
            DebugUtil.debug("getPhaseChildLaborReport "+ exp.getMessage());
                    ...
                    if (mapTaskInfo != null && !slTasksAdded.contains(strTaskId))
                    {
                        ...
                        if(mapTaskInfo.containsKey(SELECT_TASK_TO_EFFORT_TOTAL_EFFORT))
                        {
                            ...
                            else if (mapTaskInfo.get(SELECT_TASK_TO_EFFORT_TOTAL_EFFORT).getClass().getName().equals("matrix.util.StringList"))
                            {
                                ...
                                List<StringBuffer> slSepDateAndEffort = new ArrayList<StringBuffer>();
    ...
    private MapList getAllTasksForLaborReportForProject(Context context, String strProjectId,String strSelYear, String strYearType,double dbTotal,String strPersonName) throws MatrixException
    {
        try {
            ...
            int mlTasksSize = mlTasks.size();
            if(mlTasksSize>0){
                for(int i=0;i<mlTasksSize;i++) {
                    StringBuffer sbDateAndEffort = new StringBuffer();
                    ...
                    if(taskMap.get(SELECT_EFFORT_CURRENT_STATE).getClass().getName().equals("matrix.util.StringList")){
                        ...
                        if(strStateList!=null) {
                            ...
                            int strStateListSize = strStateList.size();
                            for(int j=0;j<strStateListSize;j++) {
    ...
    private MapList getEffortsForLaborReportForProject(Context context, String strProjectId,String strSelYear, String strYearType) throws MatrixException
    {
        try {
            ...
            //long end = System.currentTimeMillis();
            //System.out.println("getEffortsForLaborReportForProject time : "+(end-start));
            return mlEfforts;
        }
        catch (Exception exp)
        {
    ...
    private double getTotalEffortForWeek(Context context,String effortId, String totalEffort,String strSelYear, String strYearType) throws Exception{
        ...
        if(effortId!=null && effortId.trim().length()>0){
            ...
            busSelect.add("attribute["+ATTRIBUTE_WEEK_ENDING_DATE+"]");
            busSelect.add("attribute[Sunday]");
            busSelect.add("attribute[Monday]");
            busSelect.add("attribute[Tuesday]");
            busSelect.add("attribute[Wednesday]");
            busSelect.add("attribute[Thursday]");
            busSelect.add("attribute[Friday]");
            busSelect.add("attribute[Saturday]");

            Map effortInfo = effortDObj.getInfo(context, busSelect);
            String wkEndingDate = (String) effortInfo.get("attribute["+ATTRIBUTE_WEEK_ENDING_DATE+"]");
            ...
            if(calWkEndDate.after(calStartDate) ||calWkEndDate.compareTo(calStartDate)==0){
                ...
                String dayEffort =  (String) effortInfo.get("attribute["+day+"]");
                strEffortList.add(dayEffort);
                StringList daysEfforts = new StringList();
                for(int j=0;j<6;j++){
                    //MODIFIED::PA4:13-Jun-2011:IR-104838V6R2012x:START
                    ...
                    if(calpDate.after(calStartDate) ||calpDate.compareTo(calStartDate)==0){
                        ...
                        //daysEfforts.add("attribute["+day1+"]");
                        strEffortList.add((String)effortInfo.get("attribute["+day1+"]"));
                        wkEndDate = pdate;
                    } else {
                        break;
                    }
        ...
        int effortListSize = strEffortList.size();
        for(int k=0;k<effortListSize;k++){
    ...
    private StringBuffer getWeekData(Context context, String effortId,String strSelYear, String strYearType,String strPersonName, boolean updateApprovedEffort) throws Exception {
        ...
        if(effortId!=null && effortId.trim().length()>0){
            ...
            StringList busSelect = new StringList();
            busSelect.add("attribute["+ATTRIBUTE_WEEK_ENDING_DATE+"]");
            busSelect.add("attribute["+ATTRIBUTE_ORIGINATOR+"]");
            busSelect.add("attribute[Sunday]");
            busSelect.add("attribute[Monday]");
            busSelect.add("attribute[Tuesday]");
            busSelect.add("attribute[Wednesday]");
            ...
            Map effortInfo = dObjEffort.getInfo(context, busSelect);

            //wkEndingDate = dObjEffort.getAttributeValue(context, ATTRIBUTE_WEEK_ENDING_DATE);
            wkEndingDate = (String) effortInfo.get("attribute["+ATTRIBUTE_WEEK_ENDING_DATE+"]");
            ...
            if(strPersonName == null){
                    ...
                    for(int j=0;j<6;j++){
                        ...
                        if(calpDate.after(calStartDate) ||calpDate.compareTo(calStartDate)==0){
                            if((calpDate.before(calFinishDate) || calpDate.compareTo(calFinishDate) == 0))
                            {
                                ...
                                String day1 = getDayName(dayNo);
                                //String strEffortValue =  dObjEffort.getAttributeValue(context, day1);
                                String strEffortValue =  (String) effortInfo.get("attribute["+day1+"]");
                ...
                //String originator = dObjEffort.getAttributeValue(context,     );
                String originator = (String) effortInfo.get("attribute["+ATTRIBUTE_ORIGINATOR+"]");
    ...
    private double getTotalForReport(StringBuffer sbEffortAndDate) {
        ...
        if((sbEffortAndDate != null && !"".equals(sbEffortAndDate.toString()) && !"null".equals(sbEffortAndDate))) {
            ...
            int slDateEffortSize = slDateEffort.size()-1;
            for(int k=0;k<slDateEffortSize;k++){
                String strEffortKeyValue = (String)slDateEffort.get(k);
                String[] strWeekDate = strEffortKeyValue.split("\\=");
    ...
    public Vector getColumnNameData(Context context, String[] args)
        ...
        try {
            ...
            //exp.printStackTrace();  
            DebugUtil.debug("getColumnNameData "+ exp.getMessage());
            throw exp;
        }
    ...
    public Vector getColumnTypeData(Context context, String[] args)
        ...
        try {
            ...
            //exp.printStackTrace(); 
            DebugUtil.debug("getColumnTypeData "+ exp.getMessage());
            throw exp;
        }
    }
    ...
    public Vector getProjectEfforts(Context context, String[] args)
        ...
        try {
            ...
            //exp.printStackTrace();   
            DebugUtil.debug("getProjectEfforts "+ exp.getMessage());
            throw exp;
        }
    }
    ...
    public MapList getDynamicLaborReport (Context context, String[] args) throws Exception
    {
        ...
        try {
            ...
            for(int l =0; l < intervals.length ; l++)
            {
                ...
                if("Monthly".equalsIgnoreCase(strViewSelected))
                {
                ...
                else
                {
                    ...
                    mapColumn.put("label", strWK+week_no);
            ...
            //e.printStackTrace();   
            DebugUtil.debug("getDynamicLaborReport "+ e.getMessage());
        }
        return mlColumns;
    }
    ...
    public MapList getMembershipChildLaborReport(Context context, String[] args) throws Exception
    {
        ...
        try
        {
            ...
            int strProjectIdArrayLength = strProjectIdArray.length;
            for (int i = 0; i < strProjectIdArrayLength; i++) {
        ...
        {
            //exp.printStackTrace();   
            DebugUtil.debug("getMembershipChildLaborReport "+ exp.getMessage());
            throw new MatrixException(exp);
        }
        // long end = System.currentTimeMillis();
        // System.out.println("start expand labor report complete time "+(end-start));
        return mlTaskList ;
    }
    ...
    private MapList getTaskFromProject(Context context, String strProjectId, String strPersonName, MapList mpPersonsForProject, int ssize, String strSelYear, String strYearType) throws Exception {
        ...
        try {
            ...
            //e.printStackTrace();
            DebugUtil.debug("getTaskFromProject "+ e.getMessage());
        }
     ...
     * @throws Exception if operation fails
     */
    // Commented method as not being used anymore.
...
//      return mpChildTasks;
    ...
    private MapList getCompanyMembers(Context context, String strCompanyId)throws MatrixException {
        ...
        try {
            ...
            //e.printStackTrace();  
            DebugUtil.debug("getCompanyMembers "+ e.getMessage());
        }
        return mpMembers;
    }
    ...
    private MapList getResourceManagersAndCompanyList(Context context, String strProjectId)throws MatrixException {
        ...
        try {
            ...
            //e.printStackTrace();   
            DebugUtil.debug("getResourceManagersAndCompanyList "+ e.getMessage());
        }
    ...
    private MapList getPersonForProject(Context context,
        ...
        try {
            ...
            //e.printStackTrace(); 
            DebugUtil.debug("getPersonForProject "+ e.getMessage());
        }
    ...
    public MapList displayTimesheetTasks(Context context, String[] args,StringList filterState)
        ...
        try{
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            String strPersonId = PersonUtil.getPersonObjectID(context);
            String timesheetId = (String) paramMap.get("timesheetID");
            String objectId = (String) paramMap.get("objectId");
            paramMap.put("fromApprover","true");  //to let expandProjects know that it has been called from To Approve table
            DomainObject domObj = DomainObject.newInstance(context, objectId);
            String shasEffort_relationship = PropertyUtil.getSchemaProperty(context,"relationship_hasEfforts");
            MapList mlEfforts = new MapList();
            MapList mlModifiedEffortsList = new MapList();
            ...
            strSelect.add(DomainConstants.SELECT_KINDOF_PROJECT_SPACE);
            strSelect.add(SELECT_KINDOF_WEEKLY_TIMESHEET);
            strSelect.add(DomainConstants.SELECT_TYPE);

            Map map = domObj.getInfo(context, strSelect);

            String isKindOfProjectSpace = (String) map.get(DomainConstants.SELECT_KINDOF_PROJECT_SPACE);
            String type = (String) map.get(DomainConstants.SELECT_TYPE);
            String isKindOfWeeklyTimesheet = (String) map.get(SELECT_KINDOF_WEEKLY_TIMESHEET);
            if("TRUE".equalsIgnoreCase(isKindOfWeeklyTimesheet) || TYPE_PERSON.equalsIgnoreCase(type)) {
                ...
                for(int i=0; i<modSize; i++){
                    Map mEffort = new HashMap();
                    boolean isNonProjectTask = false;
                    mEffort = (Map)mlModifiedEffortsList.get(i);
                    String strTaskId = (String)mEffort.get("to["+shasEffort_relationship+"].from.id");
                    String strTaskType = (String)mEffort.get("to["+shasEffort_relationship+"].from.type");
                    String strEffortId = (String)mEffort.get(DomainConstants.SELECT_ID);
                    ...
                    if(timesheetTasks.size()>0){
                        for(int j=0;j<timesheetTasks.size();j++){
                            Map mpProject = (Map)timesheetTasks.get(j);
                            String strProjectId = (String)mpProject.get(DomainConstants.SELECT_ID);
                            if(strProjectId.equalsIgnoreCase(projectId) && ProgramCentralUtil.isNotNullString(projectId)){
                                index = j;
                                isSameProject = true;
                                break;
                            }
    ...
    public MapList getApprover(Context context,String effortId) throws MatrixException {
        ...
        boolean ifLoginUserHasModifyAccessOnSimpleTask = false;
        boolean ifLoginUserHasModifyAccess = false; 
        
        String personId = PersonUtil.getPersonObjectID(context);
        String loggedInUser = context.getUser();
        
        String strProjectLead = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Common.ProjectLead", context.getSession().getLanguage());
        String strResourceMgr = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Common.ResourceManager", context.getSession().getLanguage());
        String strPreference = null;
        String strApprover = EnoviaResourceBundle.getProperty(context, "emxProgramCentral.WeeklyTimesheet.ApproverSelection");
        final String PROJECT_LEAD_ACCESS =  "Project Lead";
        ...
        String mqlCmd = "print bus $1 select $2 dump ";
		String relEffortToProject = "to["+RELATIONSHIP_HAS_EFFORTS+"].from.to["+RELATIONSHIP_PROJECT_ACCESS_KEY+"].from.from["+RELATIONSHIP_PROJECT_ACCESS_LIST +"].to.id" ;
		String strProjectId = MqlUtil.mqlCommand(context, true, true, mqlCmd, true,effortId,relEffortToProject);  
        
	       if(strProjectId.equalsIgnoreCase(DomainConstants.EMPTY_STRING))	{
	    	   //Simple task
	        	String relEffortToTaskHasAccess = "to["+RELATIONSHIP_HAS_EFFORTS+"].from.current.access[modify]" ;
				String hasModifyAccess = MqlUtil.mqlCommand(context, mqlCmd ,effortId,relEffortToTaskHasAccess);
				if("true".equalsIgnoreCase(hasModifyAccess)) {
					Map nonProjectTaskApproverMap=new HashMap();
	        		nonProjectTaskApproverMap.put(DomainConstants.SELECT_ID, personId);
	                nonProjectTaskApproverMap.put(DomainConstants.SELECT_NAME, loggedInUser);
	                approverMapList.add(nonProjectTaskApproverMap);
	            }
	        	...
	        	try {		
	        		String relEffortToProjectHasAccess = "to["+RELATIONSHIP_HAS_EFFORTS+"].from.to["+RELATIONSHIP_PROJECT_ACCESS_KEY+"].from.from["+RELATIONSHIP_PROJECT_ACCESS_LIST +"].to.current.access[modify]";
	    			String hasModifyAccess = MqlUtil.mqlCommand(context, mqlCmd ,effortId,relEffortToProjectHasAccess);
	    			if("true".equalsIgnoreCase(hasModifyAccess)) {
	    				ifLoginUserHasModifyAccess = true;
	    				}
	        	}catch(Exception e) {
					//TODO 
					}
        ...
        try {
            ...
            try {
                ...
                if(strPreference.equals(strProjectLead) == true) {
                	boolean isLoggedInUserInApproverList = false; 
                    ...
                    while(itr.hasNext())
                    {
                        ...
                        if(ProgramCentralUtil.isNotNullString(name) && isPersonOwnership.equalsIgnoreCase("true")){
                            ...
                            if(id.equalsIgnoreCase(personId)) {
								isLoggedInUserInApproverList = true;
							}
                            String access=(String)list.get(DomainAccess.KEY_ACCESS_GRANTED);
                            if(PROJECT_LEAD_ACCESS.equalsIgnoreCase(access)){
                    ...
                    // if logged in user hass access to approve the effort
					if(!isLoggedInUserInApproverList && ifLoginUserHasModifyAccess) {			
								Map map=new HashMap();
								map.put(DomainConstants.SELECT_ID, personId);
								map.put(DomainConstants.SELECT_NAME,loggedInUser);
								approverMapList.add(map);
    ...
    public Vector getState(Context context, String[] args) throws MatrixException {
        ...
        try {
            ...
            StringList busSelect = new StringList(WeeklyTimesheet.SELECT_CURRENT);
            busSelect.add(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
            busSelect.add(SELECT_POLICY);
            BusinessObjectWithSelectList objectWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, objIdArr, busSelect,true);

            Map mapTimesheetInfo = new LinkedHashMap();
            for (BusinessObjectWithSelectItr objectWithSelectItr = new BusinessObjectWithSelectItr(objectWithSelectList); objectWithSelectItr.next();) {
                ...
                for (Iterator itrSelectables = busSelect.iterator(); itrSelectables.hasNext();) {
                    String strSelectable = (String)itrSelectables.next();
                    mapTask.put(strSelectable, objectWithSelect.getSelectData(strSelectable));
                }
                mapTimesheetInfo.put(objectWithSelect.getObjectId(context), mapTask);
            }
            ...
            for(int i = 0; i<size;i++) {
                ...
                String objectId = (String)mapObject.get(DomainConstants.SELECT_ID);
                String isTotalRow = (String)mapObject.get("isTotalRow");
                Map timesheetmap = (Map)mapTimesheetInfo.get(objectId);
                ...
                if (!"true".equalsIgnoreCase(isTotalRow))
                {
                        ...
                        if("TRUE".equalsIgnoreCase(timesheetmap.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE).toString())){
                                strState = "";
                            }
                            else{
                            strState = (String)timesheetmap.get(WeeklyTimesheet.SELECT_CURRENT);
                            }
                            ...
                            String sObjectPolicy = (String)timesheetmap.get(SELECT_POLICY);
    ...
    public MapList displayPersonForLaborReportByPerson(Context context, String[] args)
        ...
        try{
            ...
            if(ProgramCentralUtil.isNotNullString(strTimesheetRowId)){
                personIdList = strTimesheetRowId.split(",");

                for(int i=0;i<personIdList.length;i++){
                    Map mpPerson = new HashMap();
                    personId     = (String)personIdList[i];
            ...
            for(int j=0;j<timesheetTasks.size();j++){
                ...
                MapList ml = DomainObject.findObjects(context,TYPE_EFFORT,QUERY_WILDCARD,"-",
                        QUERY_WILDCARD,null,sbWhere.toString(),false,objectSelects);
                StringBuffer sbDateAndEffort = new StringBuffer();
            ...
            //e.printStackTrace(); 
            DebugUtil.debug("displayPersonForLaborReportByPerson "+ e.getMessage());
            throw e;
        }
        return timesheetTasks;
    }
    ...
    public MapList displayProjectsForLaborReportByPhase(Context context, String[] args)
        ...
        try{
            ...
            //e.printStackTrace(); 
            DebugUtil.debug("displayProjectsForLaborReportByPhase "+ e.getMessage());
            throw e;
        }
        HashMap hmTemp = new HashMap();
        hmTemp.put("expandMultiLevelsJPO","true");
            ...
            if(ProgramCentralUtil.isNotNullString(effortId)) {
                ...
                try {
                    ...
                    if(effortVecSize == 1) {
                        ...
                        if((currentIndex != -1)  && (dblValues.length != 0)) {
     ...
     * @author vf2
     */
	private void setTaskPercentageCompleteForEffort(Context context,String taskId,String strEffortId) throws MatrixException {
        try {
            String[] args = new String[0];
            emxEffortManagementBase_mxJPO emxEffortJPO = new emxEffortManagementBase_mxJPO(context,args);
			StringList slList = new StringList(2);
			slList.add(DomainConstants.SELECT_TYPE);
			slList.add("attribute[" + ATTRIBUTE_PERCENT_COMPLETE + "]");
			DomainObject doTask  = DomainObject.newInstance(context, taskId);
			Map mapTaskInfo = doTask.getInfo(context, slList);
			String strType = (String)mapTaskInfo.get(DomainConstants.SELECT_TYPE);
			String  strPercentage = (String)mapTaskInfo.get("attribute[" + ATTRIBUTE_PERCENT_COMPLETE + "]");
            double perComplete = Task.parseToDouble(strPercentage);
			if(perComplete < 100 && strType != null){
					...
					MapList subtaskList = doTask.getRelatedObjects(context,
                                RELATIONSHIP_SUBTASK,
				...
				doTask.setAttributeValue(context, ATTRIBUTE_PERCENT_COMPLETE, srem_task);
            }
        } catch(Exception e) {
			//e.printStackTrace(); 
			DebugUtil.debug("setTaskPercentageCompleteForEffort "+ e.getMessage());
			throw new MatrixException(e);
        }
    }
    ...
    public Vector getName(Context context,String[] args) throws MatrixException {
        ...
        String strCurrentState =ProgramCentralConstants.EMPTY_STRING;
        boolean hideLifecyleHeader = false;
        try {
            HashMap paramMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList)paramMap.get("objectList");
            ...
            for(int i=0;i<objectList.size();i++) {
                ...
                if(isTotalRow!=null && "true".equals(isTotalRow)) {
                    ...
                    if(objectId != null && objectId.trim().length()>0){
                        ...
                        selectObjList.add(SELECT_CURRENT);
                        selectObjList.add(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
                        selectObjList.add(ProgramCentralConstants.SELECT_IS_PROJECT_CONCEPT);
                        selectObjList.add(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
                        selectObjList.add(ProgramCentralConstants.SELECT_IS_PROJECT_TEMPLATE);
                        ...
                        try {
			                ...
			                String sIsProjectConcept = (String)map.get(ProgramCentralConstants.SELECT_IS_PROJECT_CONCEPT);
			                String sIsProjectSpace = (String)map.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
			                String sIsProjectTemplate = (String)map.get(ProgramCentralConstants.SELECT_IS_PROJECT_TEMPLATE);

			                boolean isProjectType = "true".equalsIgnoreCase(sIsProjectConcept) || "true".equalsIgnoreCase(sIsProjectSpace) || "true".equalsIgnoreCase(sIsProjectTemplate);
							strName = ProgramCentralUtil.getTitleForProjectType(context, map, isProjectType);
                            strType = (String)map.get(SELECT_TYPE);
                            strCurrentState = (String)map.get(SELECT_CURRENT);

                            if(!(STATE_WEEKLY_TIMESHEET_EXISTS.equalsIgnoreCase(strCurrentState)||
                                    TYPE_PROJECT_SPACE.equals(strType)&&
                                    STATE_PROJECT_SPACE_ACTIVE.equalsIgnoreCase(strCurrentState))){
                                hideLifecyleHeader = true;
                            }
                            ...
                            if(domObj.isKindOf(context, TYPE_EFFORT)){
                                ...
                                if(isCurrentlyExported){ // [MODIFIED::PRG:RG6:Dec 29, 2010:IR-083876V6R2012:R211]
                                    ...
                                    if(hideLifecyleHeader){
                                    sbName.append("&amp;headerLifecycle=hide");
                                    }
                                ...
                                if(isCurrentlyExported){
                                    ...
                                    if(hideLifecyleHeader){
                                        sbNameLink.append("&amp;headerLifecycle=hide");
                                        }
        ...
        try {
            ...
            for(int i=0;i<mapListBusiUnit.size();i++){
                ...
                if(!sObjectlist.contains(busiUnitMap.get(DomainConstants.SELECT_ID))) {
                    sObjectlist.add(String.valueOf(busiUnitMap.get(DomainConstants.SELECT_ID)));
                }
            }
        } catch(Exception e) {
            throw new MatrixException(e.getMessage());
    ...
    private StringList getProjectOwnerList(Context context, String projectId) throws MatrixException {
        ...
        if(projectId != null && projectId.trim().length() > 0) {
            ...
            for(int i=0;i<mpList.size();i++) {
                ...
                Object oProjectId = ProjectMap.get(DomainConstants.SELECT_ID);
                projList.add(String.valueOf(oProjectId));
            }
        }
        return projList;
    }
    ...
    /*  Commented method emxWeeklyTimeSheet : changeApprover as it not being used currently.
     *
     * try {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
    ...
    public StringList getTaskWithZeroEffort(Context context,String[] args) throws Exception{
        ...
        try{
            ...
            for(int i=0;i<mpEfforts.size();i++){
                ...
                Map taskMap =  objTask.getInfo(context, strObjectList);
                if(d==0.0){
                    Object oName = taskMap.get(SELECT_NAME);
                    strTaskWithZeroEffort.add(String.valueOf(oName));
                }
            }
            if(mpEfforts.size()==strTaskWithZeroEffort.size()){
            ...
            //e.printStackTrace();   
            DebugUtil.debug("getTaskWithZeroEffort "+ e.getMessage());
            throw new Exception(e.getMessage());
        }
    ...
    public int triggerconnectMemberResourcePoolOrProject(Context context,String[] args) throws Exception {
        ...
        try{
            ...
            if(ProgramCentralUtil.isNotNullString(fromId) && ProgramCentralUtil.isNotNullString(personId)) {
                ...
                String SELECT_KINDOF_ORGANIZATION = "type.kindof[" + TYPE_ORGANIZATION+ "]";
                
                StringList selectList = new StringList();
                selectList.add(SELECT_KINDOF_ORGANIZATION);
                ...
                Map<String,String> domainObjectInfoMap = domObj.getInfo(context, selectList);
                
                boolean isKindofOrganization = "TRUE".equalsIgnoreCase(domainObjectInfoMap.get(SELECT_KINDOF_ORGANIZATION));
                boolean isKindofProjectSpace = "TRUE".equalsIgnoreCase(domainObjectInfoMap.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE));
                boolean isKindofExperiment = "TRUE".equalsIgnoreCase(domainObjectInfoMap.get(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT));
                
                if((SEELCT_APPROVER_RESOURCE_MANAGER.equals(strApprover) && isKindofOrganization) ||
                        (SEELCT_APPROVER_PROJECT_LEAD.equals(strApprover) && (isKindofProjectSpace && !isKindofExperiment))) {
                    //connectTimesheetAndDeptOrBUnit(context,fromId,personId);
                	DomainObject domObjPerson = newInstance(context,personId);
                	boolean  isKindofPerson = domObjPerson.isKindOf(context,TYPE_PERSON);
                	connectTimesheetAndDeptOrBUnit(context, fromId, personId, domObj, domObjPerson, isKindofProjectSpace, isKindofPerson);
                }
                result = 0;
            }
            return result;
            ...
            //e.printStackTrace(); 
            DebugUtil.debug("triggerconnectMemberResourcePoolOrProject "+ e.getMessage());  
            throw new Exception(e.getMessage());
        }
    ...
    public int triggerdisconnectMemberResourcePoolOrProject(Context context,String[] args) throws Exception {
        ...
        try{
            ...
            //e.printStackTrace();  
            DebugUtil.debug("triggerdisconnectMemberResourcePoolOrProject "+ e.getMessage());  
            throw new Exception(e.getMessage());
        }
    }
    ...
    public void connectTimesheetAndDeptOrBUnit(Context context, String deptOrBUnitId,String personId) throws Exception {
    	try {
            if(ProgramCentralUtil.isNotNullString(deptOrBUnitId) && ProgramCentralUtil.isNotNullString(personId)) {
            	...
            	DomainObject domObjDeptOrBUnit = newInstance(context,deptOrBUnitId);
            	DomainObject domObjPerson = newInstance(context,personId);
            	
            	boolean  isKindofProjectSpace = domObjDeptOrBUnit.isKindOf(context, TYPE_PROJECT_SPACE);
            	boolean  isKindofPerson = domObjPerson.isKindOf(context,TYPE_PERSON);
            	
            	connectTimesheetAndDeptOrBUnit(context, deptOrBUnitId, personId, domObjDeptOrBUnit, domObjPerson, isKindofProjectSpace, isKindofPerson);
            }
    	} catch(Exception e) {
            throw e;
     	}
    }
    public void connectTimesheetAndDeptOrBUnit(Context context, String deptOrBUnitId,String personId, DomainObject domObjDeptOrBUnit, DomainObject domObjPerson, boolean isKindofProjectSpace, boolean isKindofPerson) throws Exception {
        try {
            StringList stateList = new StringList();
            ...
            int timesheetCount = 0;
            int size = 0;
			//DomainObject domObj = newInstance(context,deptOrBUnitId);
			//DomainObject domObjPerson = newInstance(context,personId);
            //if(ProgramCentralUtil.isNotNullString(deptOrBUnitId) && ProgramCentralUtil.isNotNullString(personId)) {
				if(!(isKindofProjectSpace && isKindofPerson)){
    ...
    public StringList getDeptAndBusinessUnitIds(Context context,String orgID,Person person) throws MatrixException {
        ...
        try {
            ...
            for(int i=0;i<mapListBusiUnit.size();i++){
                Map busiUnitMap = (Map)mapListBusiUnit.get(i);
                if(!sObjectlist.contains(busiUnitMap.get(DomainConstants.SELECT_ID))) {
                    Object oId = busiUnitMap.get(DomainConstants.SELECT_ID);
                    sObjectlist.add(String.valueOf(oId));
                }
            }
        } catch(Exception e) {
            throw new MatrixException(e.getMessage());
    ...
    /**
     * To display WeeklyTimeSheet command of MyDesk menu. Logged in person atleast have ProjectUser,
     * Resource Manager,VPLMProjectLeader,ExternalProjectLeader or VPLMViewer role
     * @param context the eMatrix <code>Context</code> object
     * @param args containing object related parameters
    ...
    public boolean hasAccessOnWeeklyTimeSheetCmd(Context context, String[] args) throws MatrixException
    {
        ...
        String sResourceManager=ProgramCentralConstants.ROLE_RESOURCE_MANAGER;
        String strVPLMleader=ProgramCentralConstants.ROLE_VPLM_PROJECT_LEADER;
        String strVPLMviewer=ProgramCentralConstants.ROLE_VPLM_VIEWER;

        try{
            Vector vRoles = PersonUtil.getUserRoles(context);
            if(vRoles.contains(sResourceManager)||vRoles.contains(sProjectUser) || vRoles.contains(sExternalProjectUser)
                    ||vRoles.contains(strVPLMleader)||vRoles.contains(strVPLMviewer))
            {
                hasAccess = true;
            }
        }catch(Exception e){
    ...
    /**
     * To display WeeklyTimeSheetApprove command . Logged in person atleast have ProjectUser role or
     ...
     * @throws MatrixException
     */
    public boolean hasAccessOnPMCWeeklyTimeSheetApproveCmd(Context context, String[] args) throws MatrixException
    {
        boolean hasAccess = false;
        String strResourceManager=ProgramCentralConstants.ROLE_RESOURCE_MANAGER;
        String strVPLMleader=ProgramCentralConstants.ROLE_VPLM_PROJECT_LEADER;
        ...
        try{
            Vector vRoles = PersonUtil.getUserRoles(context);
            String assignment = PersonUtil.getSecurityContextAssignmentsForDisplay(context).toString(); // Used getSecurityContextAssignmentsForDisplay to get Title instead of Name
            String strApprover = EnoviaResourceBundle.getProperty(context, "emxProgramCentral.WeeklyTimesheet.ApproverSelection");

            //if("R".equalsIgnoreCase(strApprover) && vRoles.contains(strResourceManager))
            if("R".equalsIgnoreCase(strApprover))
            {
                if (vRoles.contains(strResourceManager)) {
                hasAccess = true;
                } else {
                    String loggedinUserObjectId = PersonUtil.getPersonObjectID(context);
                    String mqlCmd = "print bus $1 select $2 dump $3";
                    String mqlResult = MqlUtil.mqlCommand(context, true, true, mqlCmd, true, loggedinUserObjectId,
                            "to[" + RELATIONSHIP_RESOURCE_MANAGER + "]", "|");
                    if ("TRUE".equalsIgnoreCase(mqlResult)) {
                        hasAccess = true;
                    }
                }

            } else if("P".equalsIgnoreCase(strApprover) && (vRoles.contains(strProjectLead) || vRoles.contains(strExternalProjectLead) || assignment.contains(strVPLMleader)))
            {
                hasAccess = true;
            }
        }catch(Exception e){
    ...
    public StringList getTimeSheetName (Context context, String[] args)throws Exception
    {
        ...
        try{
            ...
            String exportFormat = (String)mParamList.get("exportFormat");
            Locale locale = (Locale) mParamList.get("localeObj");
            DateFormat formatter = DateFormat.getDateInstance(DateFormat.SHORT, locale);
            for(int i=0,size=objectList.size();i<size;i++) {
                ...
                sbURL.append("\">");
                sbURL.append(XSSUtil.encodeForXML(context,timeSheetName));
                sbURL.append("</a>");
    ...
    public MapList getDynamicWeekColumns(Context context, String[] args) throws MatrixException
    {
        try {
            ...
            for (int cnt=1; cnt<=8; cnt++) {
                ...
                mapSettings.put("Edit Access Function", "hasTaskEffortEditAccess");
                mapSettings.put("Edit Access Program", "emxWeeklyTimeSheet");
				mapSettings.put("Validate", "isValidEfforts");
    ...
    public void updateEffortsAgainstAssignment(Context context, String[] args) throws Exception{
        ...
        try{
            ...
            if(effort < 0)
                ...
                String sErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                        "emxProgramCentral.WeeklyTimeSheet.Effort.validateEffortEntry", context.getSession().getLanguage());
                MqlUtil.mqlCommand(context, "error $1", sErrMsg);
                return;
            }
    ...
    /**
     * This method will return logged in user's assigned tasks or phases based on the Effort Submission attribute set on the parent project.
     * It will return the leaf level tasks and phases. Tasks and Phases from Complete, Archive and Cancel projects will be excluded.
     * @param context the ENOVIA Context object
     * @param args reuqest arguments
     * @return leaf level tasks or phases assigned to a logged in user based on Effort Submission attribute set on the parent project.
     * @throws Exception if operation fails.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public MapList getAssignedTasks(Context context, String[] args) throws Exception {
        MapList taskList = new MapList();
        final String PARENT_PROJECT_EFFORT_SUBMISSION = ProgramCentralConstants.SELECT_PROJECT_OBJECT +"."+"attribute["+ATTRIBUTE_EFFORT_SUBMISSION+"]";
        String strPersonId = PersonUtil.getPersonObjectID(context);
        Person person = new Person(strPersonId);

        StringList slTaskSelectList = new StringList();
        slTaskSelectList.add(SELECT_ID);
        slTaskSelectList.add(ProgramCentralConstants.SELECT_KINDOF_TASK);
        slTaskSelectList.add(ProgramCentralConstants.SELECT_IS_PHASE);
        slTaskSelectList.add(ProgramCentralConstants.SELECT_PROJECT_NAME);
        slTaskSelectList.add(ProgramCentralConstants.SELECT_PROJECT_TYPE);
		slTaskSelectList.add(ProgramCentralConstants.SELECT_PROJECT_TITLE);
        //slTaskSelectList.add(PARENT_PROJECT_EFFORT_SUBMISSION);
        slTaskSelectList.add(ProgramCentralConstants.SELECT_IS_SUMMARY_TASK);

        String typePattern = TYPE_TASK+","+TYPE_PHASE;
        StringBuilder sbObjectWhere = new StringBuilder();
        sbObjectWhere.append("(current ~~ ");
        sbObjectWhere.append(ProgramCentralConstants.STATE_PROJECT_TASK_ASSIGN);
        sbObjectWhere.append(" || current ~~ ");
        sbObjectWhere.append(ProgramCentralConstants.STATE_PROJECT_TASK_ACTIVE);
        sbObjectWhere.append(" || current ~~ ");
        sbObjectWhere.append(ProgramCentralConstants.STATE_PROJECT_TASK_REVIEW);
        sbObjectWhere.append(" || current ~~ ");
        sbObjectWhere.append(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE);
        sbObjectWhere.append(")");

        sbObjectWhere.append(" && (");
        sbObjectWhere.append("from[Subtask] ~~ False");
        sbObjectWhere.append(" )");

        sbObjectWhere.append(" && !(");
        sbObjectWhere.append(ProgramCentralConstants.SELECT_PROJECT_STATE + "==" + ProgramCentralConstants.STATE_PROJECT_SPACE_COMPLETE);
        sbObjectWhere.append(" || " + ProgramCentralConstants.SELECT_PROJECT_STATE + "==" + ProgramCentralConstants.STATE_PROJECT_SPACE_ARCHIVE);
        sbObjectWhere.append(" || " + ProgramCentralConstants.SELECT_PROJECT_STATE + "== "+ ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_CANCEL);
        sbObjectWhere.append(" )");

        sbObjectWhere.append(" && !(");
        sbObjectWhere.append(ProgramCentralConstants.SELECT_PROJECT_TYPE + "== '" + ProgramCentralConstants.TYPE_EXPERIMENT+"'");
        sbObjectWhere.append(" || " + ProgramCentralConstants.SELECT_PROJECT_TYPE + "== '" + ProgramCentralConstants.TYPE_PROJECT_BASELINE+"'");
        sbObjectWhere.append(" )");


        String objectWhere = sbObjectWhere.toString();

        MapList assignedTaskList = new MapList();
        ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"),DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);

        try{
            assignedTaskList=ProgramCentralUtil.getAssignments(context, person, true/*personAssignments*/, true/*groupAssignments*/,typePattern, slTaskSelectList, slTaskSelectList, objectWhere);
        }finally{
            ContextUtil.popContext(context);
        }


        Iterator assignedTaskListIterator = assignedTaskList.iterator();
        while(assignedTaskListIterator.hasNext()){
            Map taskInfo = (Map)assignedTaskListIterator.next();
            //String parentProjectEffortSubmission = (String)taskInfo.get(PARENT_PROJECT_EFFORT_SUBMISSION);
            String isKindOfTask = (String)taskInfo.get(ProgramCentralConstants.SELECT_KINDOF_TASK);
            String isKindOfPhase = (String)taskInfo.get(ProgramCentralConstants.SELECT_IS_PHASE);
            String isSummaryTask = (String)taskInfo.get(ProgramCentralConstants.SELECT_IS_SUMMARY_TASK);

            if("False".equalsIgnoreCase(isSummaryTask) && ("True".equalsIgnoreCase(isKindOfPhase) || "True".equalsIgnoreCase(isKindOfTask))){
                taskInfo.put("hasChildren", "false"); // To remove expand icon in search
                taskList.add(taskInfo);
            } /*
                 * else if("Task".equalsIgnoreCase(parentProjectEffortSubmission) &&
                 * "True".equalsIgnoreCase(isKindOfTask)){ taskList.add(taskInfo); }
                 */
        }
    ...
    /**
     * This method will return the parent project name list.
     * @param context the ENOVIA Context object
     * @param args reuqest arguments
     * @return the parent project name list.
     * @throws Exception if operation fails.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public StringList getProjectName(Context context, String[] args) throws Exception {
        ...
        String typeIcon = EMPTY_STRING;
        Map<String,String> typeIconMap = new HashMap<>();

        while(objectListIterator.hasNext()){
            Map objectInfo = (Map)objectListIterator.next();
            String projectName = (String)objectInfo.get(ProgramCentralConstants.SELECT_PROJECT_NAME);
			String projectTitle = (String)objectInfo.get(ProgramCentralConstants.SELECT_PROJECT_TITLE);
			if (ProgramCentralUtil.isNotNullString(projectTitle)) {
            ...
            if(!ProgramCentralUtil.isNullString(projectName)) {
                ...
                String symbolicNameOfType = FrameworkUtil.getAliasForAdmin(context, "Type", projectType, true);
                if(typeIconMap.containsKey(projectType)){
                    typeIcon = typeIconMap.get(projectType);
                } else {
                    try{
                        typeIcon = EnoviaResourceBundle.getProperty(context, "emxFramework.smallIcon." + symbolicNameOfType);
                    }catch(Exception e){
                        typeIcon  = EnoviaResourceBundle.getProperty(context, "emxFramework.smallIcon.type_ProjectSpace");
                    }

                    typeIconMap.put(projectType, typeIcon);
                }
                projectName = "<img src=\"../common/images/"+typeIcon+"\"/>" + XSSUtil.encodeForXML(context,projectName);
            }else {
                projectName = "";
            }
    ...
    /**
     * This method will return the access list for edit task efforts.
     * @param context the ENOVIA Context object
     * @param args reuqest arguments
     * @return effort access list.
     * @throws Exception if operation fails.
     */
    @com.matrixone.apps.framework.ui.ProgramCallable
    public StringList hasTaskEffortEditAccess(Context context, String [] args) throws Exception {
        StringList accessList = new StringList();
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        MapList objectList = (MapList) programMap.get("objectList");
        Map requestMap = (Map) programMap.get("requestMap");
        String selectedProgramLabel = (String)requestMap.get("selectedProgramLabel");
        ...
        while(objectListIterator.hasNext()){
            Map objectInfo = (Map)objectListIterator.next();
            String isSummaryTask = (String)objectInfo.get(ProgramCentralConstants.SELECT_SUMMARY);
            accessList.add("TRUE".equalsIgnoreCase(isSummaryTask) ? "FALSE" : "TRUE");
            }
        }
        return accessList;
    }
