...
//import matrix.db.MQLCommand;	//mkcsc
...
public class IEFCreateDesignObject_mxJPO
{
	...
	private HashMap autonameHashMap		= new HashMap();
	private	String sObjGeneratorType = "";
	private String resultTrue = "true|";
	...
	public IEFCreateDesignObject_mxJPO (Context context, String[] args) throws Exception
    {
		...
		sObjGeneratorType = MCADMxUtil.getActualNameForAEFData(context, "type_eServiceObjectGenerator");
    }
	...
	public String createDesignObject(Context context, String[] args) throws MCADException
	{
		...
		try
		{
			...
			//HashMap returnPolicyValueMap = new HashMap();		//mkcsc
			...
			else
			{
				...
				if(globalConfigObject.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE))
				{
					...
					if(instanceNameOfTemplate != null && instanceNameOfTemplate.contains("-"))
					{
						...
						{
							autoNameSeries = instanceNameOfTemplate.substring(0,indexForAutoName+1);	
						}
					}
					//IR-572532 : For Autoname series, other than A,B,C,D size
					if(autonameHashMap != null) {
						if(!autonameHashMap.isEmpty() && autonameHashMap.containsKey(instanceType)) 
							autonameSeriesForInstance = (String)autonameHashMap.get(instanceType);
						else		
						{
							String args1[] = new String[4];
							args1[0] = sObjGeneratorType;
							args1[1] = symbolicBaseInstanceTypeName;
							args1[2] = "*";
							args1[3] = "attribute[eService Name Prefix].value=='"+autoNameSeries+"'";
							String strResult = util.executeMQL(context, "temp query  bus $1 $2 $3 where $4 select revision", args1);
						
							if(strResult.startsWith(resultTrue))
							{
								strResult = strResult.substring(5);
								StringTokenizer st = new StringTokenizer(strResult, "=");  
								String autonameRevision = "";
								int count = 1;
								while(st.hasMoreElements()) {
									autonameRevision = st.nextToken().trim();
									if(count == 2) {
											autonameHashMap.put(instanceType, autonameRevision);
											autonameSeriesForInstance = autonameRevision;	
										}
									count++;
								}
							}	
					...
					String sourceAttributeValue = integrationName+"|R428";
			...
			//IR-515338, Modify Attribute was called, before the values were set. Hence shifted setAttributes() function below.
			String attrNameSource		= MCADMxUtil.getActualNameForAEFData(context, "attribute_Source");
			String sourceAttributeValue = integrationName+"|R428";
			...
			if(globalConfigObject.isTypeOfClass(cadType, MCADAppletServletProtocol.TYPE_FAMILY_LIKE))
			{
				...
				//String instanceName  = instanceDesignObject.getName();	//mkcsc
...
//				retVal = "true|" + designObjectID + "|" + designObjName + "|" + cadType;
    ...
    private void setAttributes(Context context, BusinessObject designObject, String isRootObject, HashMap attrNameValMap) throws MCADException
    {
		try
		{
			attrNameValMap.remove("IEF-StartDesignTemplateSites");
			attrNameValMap.remove("IEFStartDesignJPO");
			attrNameValMap.remove("IEF-StartDesignForm");
			...
			if(attrNameValMap != null && attrNameValMap.size() > 0 )
			{
				...
				while(itr.hasNext())
				{
					...
					//IR-515338: designObject.setDescription(attrValue) is deprecated method. Hence replaced the method. Similar for setOwner() method.
					if(attrName.equals("$$Description$$"))
					{
						designObject.setDescription(context,attrValue);
						designObject.update(context);
					}
					else if(attrName.equals("$$Owner$$"))
					{
						designObject.setOwner(context,attrValue);
						designObject.update(context);
					}
					...
					//rgh1 changes for DMC and 3D Space attribute ===========================
					else if(generalUtil.doesAttributeExistsOnType(context, designObjectType, attrName) || generalUtil.isMappedInterfaceAttribute( attrName) || generalUtil.doesAttrExistsOnRACEInterface(context, designObjectType, attrName,cache))
					{
