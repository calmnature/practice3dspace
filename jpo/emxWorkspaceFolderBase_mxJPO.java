...
public class emxWorkspaceFolderBase_mxJPO extends emxWorkspaceConstants_mxJPO
{
    ...
    {
        super(context, args);
        sbProjectMember.append(DomainConstants.SELECT_NAME);
        sbAttrCreateFolder.append(attributeBracket).append(DomainObject.ATTRIBUTE_CREATE_FOLDER).append("].value");
        sbAttrProjectAccess.append(attributeBracket).append(DomainObject.ATTRIBUTE_PROJECT_ACCESS).append("].value");
        sbContentAttr.append(attributeBracket).append(DomainObject.ATTRIBUTE_COUNT).append(closeBracket);
    }
...
//      Added for Bug 372630 starts
   ...
   public Vector showContent(Context context, String[] args)
     throws Exception
    {
        ...
        {
            HashMap programMap = (HashMap) JPO.unpackArgs(args);
            MapList objList = (MapList)programMap.get(OBJECT_LIST);
  ...
  public Vector showCheckbox(Context context, String[] args)
    ...
    private boolean checkAccess(Context context, String objectId) throws Exception
    {
          try{
            ...
            sbWhere.append(sbProjectMember.toString()).append("=='").append(person.getName(context)).append("'");
            DomainObject dom = new DomainObject(objectId);
            if(!"Workspace".equals(dom.getType(context)))
            {
    ...
    static public String getProjectId(matrix.db.Context context, String folderId) throws MatrixException {
      ...
      String strProjectType  = PropertyUtil.getSchemaProperty(context,"type_ProjectManagement");
      ...
      String strWorkspaceTemplate  = PropertyUtil.getSchemaProperty(context,"type_WorkspaceTemplate");
      ...
      typePattern.addPattern(strWorkspaceTemplate);
    ...
    public boolean showEditAndSubscriptionCommand(matrix.db.Context context,String[] args)throws Exception
    {
        ...
        if(FrameworkUtil.isWsInArchiveState(context, workspaceFolder)) {
    		return boolChecking;
    	}
 ...
 public Object constructWorkspaceFolderAccessList(Context context,String[] args,boolean bPersonFlag,boolean bBuyerFlag,boolean bRoleFlag) throws Exception
  {
      ...
      HashMap programMap = (HashMap) JPO.unpackArgs(args);
      String language = (String) programMap.get(LANGUAGE_STR);
      ...
      if(DomainConstants.TYPE_WORKSPACE.equals(sTypeName))
      {
      ...
      sAttSelProjectAccessTemp.append(DomainConstants.ATTRIBUTE_PROJECT_ACCESS).append(strVal);
      ...
      sAttSelCreateRouteTemp.append(sAttrCreateRoute).append(strVal);
      String sAttSelCreateRoute = sAttSelCreateRouteTemp.toString();

      StringBuffer sAttSelCreateFolderTemp  = new StringBuffer(strAttr);
      sAttSelCreateFolderTemp.append(sAttrCreateFolder).append(strVal);
      String sAttSelCreateFolder = sAttSelCreateFolderTemp.toString();

      StringBuffer sSelOrgNameTemp          = new StringBuffer(strTo);
      sSelOrgNameTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append("].from.to[").append(DomainConstants.RELATIONSHIP_EMPLOYEE).append("].from.name");
      String sSelOrgName = sSelOrgNameTemp.toString();

      StringBuffer sSelOrgIdTemp            = new StringBuffer(strTo);
      sSelOrgIdTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append("].from.to[").append(DomainConstants.RELATIONSHIP_EMPLOYEE).append("].from.id");
      ...
      sSelPersonLastNameTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append(strFrom).append(com.matrixone.apps.common.Person.SELECT_LAST_NAME).append(".value");
      String sSelPersonLastName = sSelPersonLastNameTemp.toString();

      StringBuffer sSelPersonFirstNameTemp  = new StringBuffer(strTo);
      sSelPersonFirstNameTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append(strFrom).append(com.matrixone.apps.common.Person.SELECT_FIRST_NAME).append(".value");

      String sSelPersonFirstName = sSelPersonFirstNameTemp.toString();
      StringBuffer sSelPersonNameTemp       = new StringBuffer(strTo);
      sSelPersonNameTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append("].from.name");

      String sSelPersonName = sSelPersonNameTemp.toString();
      StringBuffer sAssignBuyerDeskTemp     = new StringBuffer(strTo);
      sAssignBuyerDeskTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append("].from.from[").append(DomainConstants.RELATIONSHIP_ASSIGNED_BUYER).append("].to.id");
      String sAssignBuyerDesk = sAssignBuyerDeskTemp.toString();

      StringBuffer sStateTemp               = new StringBuffer(strTo);
      sStateTemp.append(DomainConstants.RELATIONSHIP_PROJECT_MEMBERSHIP).append(strFrom).append(DomainConstants.SELECT_CURRENT);
      String sState = sStateTemp.toString();

      StringBuffer sBuyerDeskTemp           = new StringBuffer("from[");
      sBuyerDeskTemp.append(DomainConstants.RELATIONSHIP_WORKSPACE_BUYER_DESK).append("].to.id");
      ...
      while(mapItr.hasNext())
     {
      ...
      if("Active".equals(strState) && (bPersonFlag || bBuyerFlag)) {
        hashMap = new HashMap();
        ...
        if(!"Project Lead".equals((String)map.get(sAttSelProjectAccess)))
        {
 ...
 public Vector showPersonName(Context context,String[] args) throws Exception {
     ...
     HashMap programMap=(HashMap)JPO.unpackArgs(args);
     HashMap paramMap=(HashMap)programMap.get(PARAM_LIST);
     ...
     String imgPerson = "<img src='images/iconPerson16.png' border='0' id=''/>";
     ...
     for(Iterator objectListItr=objectList.iterator();objectListItr.hasNext();)  {
         ...
         if(isTextReport) {
             ...
             memberDisplayStrBuffer.append(isRoleType ? imgRole : imgPerson).append("&nbsp;").append(XSSUtil.encodeForHTML(context, sDisplayPersonName));
         } else {
             String sPersonId               = (String)mapAccess.get("PersonId");
             String sOrgId                  = (String)mapAccess.get("OrgId");

             StringBuffer buffer = new StringBuffer(200);
             buffer.append("emxTree.jsp?AppendParameters=true&mode=insert").append("&objectId=").append(XSSUtil.encodeForJavaScript(context, sPersonId)).append(XSSUtil.encodeForJavaScript(context, jsTreeID));
             buffer.append("&workspaceId=").append(XSSUtil.encodeForJavaScript(context, workspaceId)).append(XSSUtil.encodeForJavaScript(context, folderId)).append("&OrgId=").append(XSSUtil.encodeForJavaScript(context, sOrgId));

             memberDisplayStrBuffer.append("<a href=\"").append(buffer).append("\" target=\"content\"  class='object'>");
             memberDisplayStrBuffer.append(imgPerson).append("</a>").append("&nbsp;").append("<a href=\"").append(buffer);
             memberDisplayStrBuffer.append("\" target=\"content\"  class='object'>").append(XSSUtil.encodeForHTML(context, sDisplayPersonName));
 ...
 /**
  ...
  public Vector getInheritedAccesses(Context context,String[] args) throws Exception
  {
     ...
     MapList objectList=(MapList)programMap.get(OBJECT_LIST);
     HashMap paramMap=(HashMap)programMap.get(PARAM_LIST);
     String languageStr = (String)paramMap.get(LANGUAGE_STR);
    ...
    public Vector getAvailableAccesses(Context context,String[] args) throws Exception
  {
    ...
    if (stCheckNull != null && !stCheckNull.equals("null") && !"".equals(stCheckNull))
    {
                              ...
                              AccessUtil accessUtil)
{
    ...
    try {
      ...
      if(inheritAccess != null) {
       ...
       sInheritAccess = accessUtil.NONE;
       if(sAccessString == null || "".equals(sAccessString) || accessUtil.NONE.equals(sAccessString)){
         sAccessString = "&nbsp";
       }
    }
                                                 ...
                                                 String grantee) throws Exception
{
  ...
  private boolean isAssignedBuyer(StringList BuyerDeskListId,String WSBuyerDeskId) {
    ...
    public boolean isNotVersionObject (matrix.db.Context context, String[] args) throws Exception
    {
        ...
        return !("true".equalsIgnoreCase(objIsVersion));
    }
  ...
  public static boolean hasMoveAccess(Context context, String args[]) throws Exception
  {
      ...
      if(BaseObject.TYPE_WORKSPACE.equals(sTypeName)) {
          workspace = (Workspace)DomainObject.newInstance(context, objectId);
        } else if(BaseObject.TYPE_WORKSPACE_VAULT.equals(sTypeName)) {
  ...
  public static boolean hasUnlinkAccess(Context context, String args[]) throws Exception
  {
	...
	public static String getVaultDisplayName(Context context, String[] args) throws Exception
	{
		...
		try
		{
			...
			String title = (String) objMap.get(SELECT_ATTRIBUTE_TITLE);
			lable = title + "(" + count +")";
			if(! UIUtil.isNullOrEmpty(relId) && ! "undefined".equalsIgnoreCase(relId))
			{
	...
	public static Vector isLinkedFolder(Context context, String[] args) throws Exception
	{
		...
		try
		{
			...
			for (int i=0; i<objectList.size(); i++)
			{
				...
				if( RELATIONSHIP_LINKED_FOLDERS.equals(relName) )
				{
					linkedVector.add("<img border='0' src='../common/images/iconSmallLinkedObject.gif' alt='"+linked+"'></img>");
				} else {
					linkedVector.add("");
				}
			}
       ...
       *		This method uses old deprecated schema.  Only use if you are using ProjectMember objects for Members.
       * @param context the eMatrix <code>Context</code> object
       * @param args holds the objectId
       * @return boolean true if has the access else returns false
       * @throws Exception if the operation fails
       * @since TC X+3
       * @deprecated Use method WorkspaceVault:hasCreateAccess(Context context, String objectId)
       */
      public boolean hasCreateAccess(Context context, String args[]) throws Exception
      {
          try {
              ...
              StringBuffer sbWhere = new StringBuffer(sbProjectMember.length()+24);
              sbWhere.append(sbProjectMember.toString()).append("=='").append(context.getUser()).append("'");
      ...
      public void folderCreateProcess(Context context, String[] args)throws Exception
      {
          ...
          try{
           ...
           // strProjectRevision                 += "-" + boProject.getName();
            ...
            if(!bExists) {
              boNewCategory.create(context, strPolicyProject);
...
//     Added:12-Apr-2010:lvc:V6R2011x:TMC Wellness Plan
       ...
       public void subfolderCreateProcess(Context context, String[] args)throws Exception
       {
           ...
           try {
            ...
            if("true".equalsIgnoreCase(i18NSubFolderName))
            {
            ...
            //<--!Modied for the Bug No:339243 08/06/2007 10:00AM End-->
               ...
               boNewSubCategory.create(context, workspace.POLICY_PROJECT);
...
//     Added:12-Apr-2010:lvc:V6R2011x:TMC Wellness Plan
       ...
       public String getParentForSubVault(Context context, String[] args)throws Exception
       {
           ...
           HashMap requestMap = (HashMap)programMap.get("requestMap");
           String objectId = (String)requestMap.get("objectId");
           DomainObject domObjFolder          = DomainObject.newInstance(context, objectId);
           String strCategoryName = domObjFolder.getInfo(context,DomainConstants.SELECT_ATTRIBUTE_TITLE); 
		/*
		 * BusinessObject boFolder = new BusinessObject(objectId);
		 * boFolder.open(context); String strCategoryName = boFolder.getName();
		 * boFolder.close(context);
		 */
           strParent = "<img src=\"images/iconSmallFolder.png\" border=\"0\" id=\"\"/>"+ XSSUtil.encodeForHTML(context,strCategoryName);
...
//     Added:08-Apr-2010:lvc:V6R2011x:TMC Wellness Plan
       ...
       public void folderCloneProcess(Context context, String[] args)throws Exception
       {
           ...
           if(UIUtil.isNullOrEmpty(strFolderName)) {
           ...
           if(UIUtil.isNullOrEmpty(strFolderDesc)) {
               strFolderDesc ="";
           }
           ...
           if(!"".equals(sTempId) && BaseObject.TYPE_PROJECT.equals(ParentObject.getType(context))) {
       ...
       public String getCloneName(Context context, String[] args)throws Exception
       {
           ...
           String strCloneName = domObj.getInfo(context, DomainConstants.SELECT_ATTRIBUTE_TITLE);
		/*
		 * BusinessObject boFolder = new BusinessObject(objectId);
		 * boFolder.open(context); String strCloneName = boFolder.getName();
		 * boFolder.close(context);
		 */
       ...
       static public void CloneFolders(matrix.db.Context context, String sfolderId,String sCloneId,String strFolderName) throws MatrixException {
           ...
           while(relWSelectSubCategoryItr.next()) {
             ...
             strProjectRevision = boProject.getRevision();
             //String folderAutoNameId = autoName(context, session, strProjectVaultType,strProjectRevision, "policy_Project", strProjectVault);
 ...
 /**
       ...
       public MapList getPredefinedFolderName(Context context,String[] args)throws Exception {
          ...
          // if (emxPage.isNewQuery()) {
             ...
             while(relWSelectProjectVaultItr.next()) {
               ...
               String strVaultName         = (String)vaultHashTable.get(DomainConstants.SELECT_NAME);
   ...
   public Vector getFolderNames(Context context, String[] args) throws Exception
   {
       ...
       String folderIcon = "<img src='images/iconSmallFolder.png' border='0' id=''></img>";
       while(itr.hasNext()){
           Map map = (Map)itr.next();
           StringBuffer buffer = new StringBuffer(100);
           buffer.append(folderIcon).append(XSSUtil.encodeForHTML(context,(String)map.get(DomainConstants.SELECT_NAME)));
   ...
   public Map getContentRemoveOptionRange(Context context, String[] args) throws FrameworkException {
       ...
       try {
           ...
           String sLanguage = (String) paramMap.get(LANGUAGE_STR);
           ...
           if(bActivateDSFA) {
               message.append(' ').append(i18Now.getI18nString("emxTeamCentral.DeleteContent.VCDatabaseMessage", bundle, sLanguage));
           }
           ...
           if(bActivateDSFA) {
               message.append(' ').append(i18Now.getI18nString("emxTeamCentral.DeleteContent.VCRestoreMessage", bundle, sLanguage));
           }
           displayValues.add(message.toString());
   ...
   public Map removeFolderContentPreProcess(Context context, String[] args) throws FrameworkException {
       ...
       try {
           ...
           String sLanguage = (String) requestMap.get(LANGUAGE_STR);
           ...
           sbSelRtId.append(DomainObject.RELATIONSHIP_OBJECT_ROUTE).append("].to.id");

           StringBuffer sbLockedSelect = new StringBuffer(56);
           sbLockedSelect.append("relationship[").append(CommonDocument.RELATIONSHIP_ACTIVE_VERSION).append("].to.locked");
           ...
           mulValSelects.add(sbSelRtId.toString());
           MapList accessMapList = DomainObject.getInfo(context,documentId,selDocumentList, mulValSelects);
   ...
   public Map createWorkspaceFolderProcess(Context context,String[] args) throws Exception
   {
       ...
       String languageStr    = (String) programMap.get(LANGUAGE_STR);
       ...
       try{
         ...
         if(!bExists) {
        	 ...
        	 wVault.createObject(context, strTypefolder, strFolderName, strProjectRevision, strPolicyProject, context.getVault().getName());
        	 try {
			 ...
			 //set state of new Folder to InWork to match readability of previous policy
			 //allowing to be done here as this is only called from Team Create Form
			 wVault.promote(context);
	...
	public Map getRangeValuesFolderAccessTypeCreateForm(Context context, String[] args)throws Exception {
	    ...
	    for (int i = 0; i < attributeRange.size(); i++) {
	        if(WorkspaceVault.ATTRIBUTE_ACCESSTYPE_SPECIFIC.equals(attributeRange.get(i))){
	        	...
	        	rangeAccessTypeActual.add((String)attributeRange.get(i));
	        }else{
 ...
 public StringList getWorkspaceVaultAddSecurityContextIncludeIDs(Context context, String[] args) throws Exception
	...
	@com.matrixone.apps.framework.ui.IncludeOIDProgramCallable
    public StringList getWorkspaceVaultAddUserGroupIncludeIDs(Context context, String[] args) throws Exception
	{
     HashMap programMap         = (HashMap) JPO.unpackArgs(args);
     String objectId            = (String) programMap.get("objectId");

     return emxDomainAccessBase_mxJPO.getInclusionList(context, WorkspaceVault.getWorkspaceVaultParentId(context,objectId), "GroupProxy");
	}

	@com.matrixone.apps.framework.ui.ProgramCallable
	public static MapList getSecurityContextOIDsList(Context context, String args[]) throws Exception {
	 ...
	 String workspaceId = WorkspaceVault.getWorkspaceVaultParentId(context,objectId);

	 long startTime = System.currentTimeMillis();
	 MapList accessList = new MapList();
	 MapList parentIdAccessList = new MapList();
	 ...
	 if(UIUtil.isNotNullAndNotEmpty(workspaceId)){
		 parentIdAccessList = DomainAccess.getAccessSummaryList(context, workspaceId);
	 }
	 ...
	 for (int i = 0; i < accessList.size(); i++) {
		 ...
		 String tempName = (String)accessMap.get(DomainConstants.SELECT_NAME);
		 if(!tempName.endsWith("_PRJ")){
			 scList.add(tempName);
		 }            
	 }
	 StringBuffer sbf = new StringBuffer();
	 for (int i = 0; i < parentIdAccessList.size(); i++) {
		 Map accessMap = (Map) parentIdAccessList.get(i);
		 String tempName = (String)accessMap.get(DomainConstants.SELECT_NAME);
		 if(!tempName.endsWith("_PRJ")){
			 if(!scList.contains(tempName)){
				sbf.append("(name ~~ \"*."+tempName+"\") ||");
			 }
		 }            
	 }
	 if(sbf.length() > 2){
	 ...
	 whereClause = sbf.toString();
	 if(whereClause.isEmpty()){
		 return new MapList();
	 }
	// System.out.println("whereClause: " + whereClause);

	// System.out.println("Time for DomainAccess.getAccessSummaryList(): " + (System.currentTimeMillis()-startTime));
	...
	// System.out.println("Time for DomainObject.findObjects(): " + (System.currentTimeMillis()-startTime));
	 
	//System.out.println("securityContextList: " + securityContextList);
	
	MapList includeSecContextIdList = new MapList();
    HashSet uniqueOrgProjComboList = new HashSet();
...
//System.out.println("Time for removing duplicate combo(): " + (System.currentTimeMillis()-startTime));
 ...
 public void deleteWorkspaceFolderAccess(Context context, String[] args) throws Exception
 {
     ...
     try
     {
         ...
         for(int i =0; i<ids.length;i++)
         {
             ...
             if( idList.size() >2 )
             {
                 ...
                 if(UIUtil.isNotNullAndNotEmpty(strProject)  && strProject.endsWith("_PRJ"))
                 {
     ...
     public Vector getFolderDisplayName(Context context, String[] args) throws Exception{
     	try
         {
             ...
             while(objectListItr.hasNext())
             {
                ...
                sFolderName   = (String)objectMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
                if(UIUtil.isNullOrEmpty(sFolderName)) {
		...
		try {
			...
			if ("owner".equals(kindOfOwner)) {
				...
				DomainAccess.createObjectOwnership(context, strObjectId,
						com.matrixone.apps.domain.util.PersonUtil.getPersonObjectID(context, newOwner), defaultAccess,
	...
	public int updateOwnershipObjects(Context context, String[] args) throws Exception
     {
    	 String objectId = args[0];
		 String previousOwner = args[1];
		 String newOwner = args[2];
		 if(UIUtil.isNotNullAndNotEmpty(newOwner)){//Folder is being created, so no need to update ownership here.
		 String previousOwnerId = com.matrixone.apps.domain.util.PersonUtil.getPersonObjectID(context, previousOwner);
		 String newOwnerId = com.matrixone.apps.domain.util.PersonUtil.getPersonObjectID(context, newOwner);
		 DomainAccess.createObjectOwnership(context, objectId, newOwnerId, "Full", DomainAccess.COMMENT_MULTIPLE_OWNERSHIP);
		 DomainAccess.createObjectOwnership(context, objectId, previousOwnerId, "Read", DomainAccess.COMMENT_MULTIPLE_OWNERSHIP, true, true);
		 }
		return 0;
     }
}
