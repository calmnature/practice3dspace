...
public class emxCommonUtilBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	*/
	...
	/**
	* Used in User Groups access to promote Group state to active on CreateAction Trigger based on the expression value
	*
	* @param context the eMatrix <code>Context</code> object
	* @param args contains current state and group ID
	*
	* @return integer
	* @throws Exception if the operation fails
	*/
	public int promoteToActiveState(Context context, String[] args) throws Exception {
		String expressionCache = (String)CacheUtil.getCacheObject(context, "usergroup.allowPrivateUserGroup");
		if(UIUtil.isNullOrEmpty(expressionCache)) {
			final String expName = "AllowPrivateUserGroup";
			String expMQLCmd = "list expression $1 select value dump";
			String allowPrivate = MqlUtil.mqlCommand(context, expMQLCmd, expName);
			expressionCache = allowPrivate;
			CacheUtil.setCacheObject(context, "usergroup.allowPrivateUserGroup", allowPrivate);
		}
		String _GROUP_ACTIVE_ = "Active";
    	String _GROUP_PRIVATE_ = "Private";
   		String gpCurrent = args[1];
		String groupId = args[0];
		if((UIUtil.isNullOrEmpty(expressionCache) || "False".equalsIgnoreCase(expressionCache)) && _GROUP_PRIVATE_.equalsIgnoreCase(gpCurrent)) {
			System.out.println(gpCurrent+" - promoteToAciveState - "+groupId);
			try {
				ContextUtil.startTransaction(context, true);
				DomainObject userGroup = new DomainObject(groupId);
				userGroup.setState(context, _GROUP_ACTIVE_);
