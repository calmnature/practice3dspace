...
public class emxValidationBase_mxJPO extends DomainObject
{
	...
	public Map<String, String> validateName(Context context,String []args)throws Exception
	{
        ...
        String name = ((String) programMap.get("Name")).trim();
		...
		invalidCharacters = invalidCharacters.contains(ProgramCentralConstants.SPACE) ? invalidCharacters.replace(ProgramCentralConstants.SPACE, ProgramCentralConstants.EMPTY_STRING) : invalidCharacters;
		String valid = "true";
	...
	public Map<String, String> validateAssignee(Context context,String []args)throws Exception
	{
		...
		String error = DomainObject.EMPTY_STRING;
		String valid = "true";
		String errorMessage = EnoviaResourceBundle.getProperty(context,"ProgramCentral","emxProgramCentral.ProjectImport.InvalidUser", context.getSession().getLanguage());
		...
		if(personInfo == null || personInfo.isEmpty()){
			...
			// RVW: Changed findObjects to pass revison pattern to resolve MX_CHECK_BAD_QUERY warning/error (see IR-666140-3DEXPERIENCER2019x)
			//
				...
				if(assigneeList != null && !assigneeList.isEmpty()){
					for(int i=0; i<assigneeList.size();i++){
						...
						if(personInfoMap != null){
							...
							error = " \""+assigneeName.trim()+ "\" " + errorMessage;
							valid = "false";
						}
					}
...
public Map<String, String> validateDescription(Context context, String[] args) throws Exception {
    Map<String, String> returnMap = new HashMap<>();

    Map<String, String> programMap = JPO.unpackArgs(args);
    String description = programMap.get("Description");

    String error = DomainObject.EMPTY_STRING;
    String invalidCharacters = EnoviaResourceBundle.getProperty(context, "emxFramework.Javascript.BadChars");
    invalidCharacters = invalidCharacters.contains(ProgramCentralConstants.SPACE)
            ? invalidCharacters.replace(ProgramCentralConstants.SPACE, ProgramCentralConstants.EMPTY_STRING)
            : invalidCharacters;

    StringBuilder invalidCharsFound = new StringBuilder();
    String valid = "true";

    
    for (int i = 0; i < invalidCharacters.length(); i++) {
        char testChar = invalidCharacters.charAt(i);
        if (description.indexOf(testChar) >= 0) {
            if (invalidCharsFound.indexOf(String.valueOf(testChar)) == -1) { 
                invalidCharsFound.append(testChar).append(" ");
            }
            valid = "false";
        }
    }
    ...
    if (invalidCharsFound.length() > 0) {
        error = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.ProjectImport.InvalidCharacterInDescription", context.getSession().getLanguage());
        error += " '" + invalidCharsFound.toString().trim() + "'.";
    }
	 ...
	 * @throws Exception if operation fails.
	 */
	public Map<String, String> validateTaskPriority(Context context, String[] args) throws Exception {
    Map<String, String> returnMap = new HashMap<String, String>();
    Map<?, ?> programMap = JPO.unpackArgs(args);
    //System.out.println("programMap :" + programMap);

    String Priority = ((String) programMap.get("Task Priority"));
    ...
    StringList allowedPriorities = new StringList();
    allowedPriorities.add("High");
    allowedPriorities.add("Medium");
    allowedPriorities.add("Low");
    allowedPriorities.add("Urgent");
    allowedPriorities.add("high");
    allowedPriorities.add("medium");
    allowedPriorities.add("low");
    allowedPriorities.add("urgent");
    ...
    if (!allowedPriorities.contains(Priority)) {
        error = "Invalid task priority value: '" + Priority + "'. Allowed values are High, Medium, Low, Urgent.";
        valid = "false";
    } else {
        // Character-level validation
        for (int i = 0; i < invalidCharacters.length(); i++) {
            char testChar = invalidCharacters.charAt(i);
            if (Priority.indexOf(testChar) >= 0) {
                if (invalidCharsFound.indexOf(String.valueOf(testChar)) == -1) {
                    invalidCharsFound.append(testChar).append(" ");
                }
                valid = "false";
        ...
        if (invalidCharsFound.length() > 0) {
            error = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.ProjectImport.InvalidCharacterInTaskPriority", context.getSession().getLanguage());
            error += " '" + invalidCharsFound.toString().trim() + "'.";
        }
    }
