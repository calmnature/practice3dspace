...
/**
 * The <code>emxLibraryCentralCommonBase.java</code> class contains utility
 * methods for getting data using configurable tables in Library Central.
 *
...
public class emxLibraryCentralCommonBase_mxJPO extends emxDomainObject_mxJPO implements LibraryCentralConstants {
	public static final Map<String, String> symbolicNames = Collections.unmodifiableMap(new HashMap<String, String>() {
		{
			put("classificationUsage", "");
			put("attribute_PartFamilySequencePattern", "Part Family Sequence Pattern");
			put("attribute_PartFamilyPrefixPattern", "Part Family Prefix Pattern");
			put("attribute_Description", "");
			put("attribute_Approver", "Approver");
			put("attribute_PartFamilyPatternSeparator", "Part Family Pattern Separator");
			put("attribute_PartFamilySuffixPattern", "Part Family Suffix Pattern");
			put("RestrictSecurityRulesInClass", "");
			put("attribute_PartFamilyBaseNumber", "Part Family Base Number");
			put("attribute_Owner", "");
	...
	/**
	 * Entry point for business object creation. The required arguments are defined
	 * in <code>getRequiredCreateArgKeys ()</code> Warning: do to the static nature
	 * of this system if sub-classes need to modified implementation details, this
	 * function must be overridden.
	 *
	 * @param context the eMatrix <code>Context</code> user's context
	 * @param args    contains a Map with the following entries: type - a String of
	 *                the Object Type name - a String of the Object Name policy - a
	 *                String of the Object Policy
	...
	public String createObject(Context context, String[] args) throws IllegalArgumentException, FrameworkException {
		...
		String classificationUsage = null;
		Map attributeMap = null;
		Map specificationMap = null;
		String restrictSecurityRulesInClass = null;
		try {
			...
			classificationUsage = (String) argMap.get(LibraryCentralConstants.JPO_ARGS_CLASSIFICATION_USAGE);
			...
			restrictSecurityRulesInClass = (String) argMap
					.get(LibraryCentralConstants.ATTRIBUTE_RestrictSecurityRulesInClass);
			attributeMap.put(LibraryCentralConstants.ATTRIBUTE_RestrictSecurityRulesInClass,
					restrictSecurityRulesInClass);
		} catch (Exception e) {
		...
		objectId = createObject(context, type, name, revision, policy, vault, title, attributeMap, specificationMap,
				classificationUsage);
	 ...
	 * @throws com.matrixone.framework.util.FrameworkException if a major failure
	 *                                                         occurs
	 */
	protected String createObject(Context context, String type, String name, String revision, String policy,
			String vault, String title, Map attributeMap, Map specificationMap, String classificationUsage)
		...
		try {
			...
			String restrictSecurityRulesInClass = (String) attributeMap
					.get(LibraryCentralConstants.ATTRIBUTE_RestrictSecurityRulesInClass);
			busObj = createBusObject(context, type, name, revision, policy, vault, owner, description, title,
					classificationUsage, restrictSecurityRulesInClass);
	 ...
	 * @throws Exception if the operation fails
	 */
	protected BusinessObject createBusObject(Context context, String type, String name, String revision, String policy,
			String vault, String owner, String description, String title, String classificationUsage,
			String restrictSecurityRulesInClass) throws Exception {
		...
		if ((LibraryCentralConstants.TYPE_GENERAL_LIBRARY).equals(type)) {
			busObj.setAttributeValue(context, ATTRIBUTE_LibraryUsage, classificationUsage);
		} else if ((LibraryCentralConstants.TYPE_GENERAL_CLASS).equals(type)) {
			busObj.setAttributeValue(context, ATTRIBUTE_ClassUsage, classificationUsage);
		}
		if ((LibraryCentralConstants.TYPE_EXPORT_CONTROL_CLASS).equals(type)
				|| (LibraryCentralConstants.TYPE_IP_CONTROL_CLASS).equals(type)
				|| (LibraryCentralConstants.TYPE_SECURITY_CONTROL_CLASS).equals(type)) {
			busObj.setAttributeValue(context, ATTRIBUTE_RESTRICT_SECURITY_RULES_IN_CLASS, restrictSecurityRulesInClass);
		}
	...
	/**
	 * Performs additional operations here.After basic object creation or changes
	 * are complete.
	 *
	 * @param context          the eMatrix <code>Context</code> object
	 * @param busObj
	 * @param owner            - owner of object to create
	 * @param attributeMap     - map containing custom attributes, description, &
	 *                         owner
	 * @param specificationMap - map containing other required information for clone
	 * @throws Exception if a major failure occurs
	 */
	protected void finishBusObject(Context context, BusinessObject busObj, String owner, Map attributeMap,
			Map specificationMap) throws Exception {
		...
		if (obj != null) {
			...
			String containingRel = (String) specificationMap
					.get(LibraryCentralConstants.JPO_SPEC_CONTAINING_RELATIONSHIP);
		...
		// Commenting below grantOwnerAccess line for IR-657786-3DEXPERIENCER2020x -
		// Imported Library Structure are not seen.
		// The reason is objects with grants are not indexed, their iov and dov are
		// empty and they will not be shown
		// in Global search or CV servlet or any other place using indexed data (widget
		// etc.)
	...
	protected void setCustomAttributes(Context context, BusinessObject busObj, Map attrMap) throws Exception {
		...
		while (itr.hasNext()) {
			...
			if (symbolicNames.containsKey(key)) {
				symbolicAttrName = symbolicNames.get(key);
			} else {
				symbolicAttrName = PropertyUtil.getSchemaProperty(context, key);
			}

			if (UIUtil.isNotNullAndNotEmpty(symbolicAttrName)) {
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: objectid - objectid id
	 *                list - list of ids to connect to
	 * @throws Exception if a major failure occurs
	 */
	 ...
	 * @throws Exception if the operation fails
	 */
	protected void addToContainingObject(Context context, DomainObject domainObj, String[] containingObjIds,
			String containingRel) throws Exception {

		if (containingRel != null && !containingRel.equals("null") && containingObjIds != null
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: objectId - objectId
	 *                folderId - folder to connect to user list - users to send
	 *                messages to
	 * @throws Exception if a major failure occurs
	 */
	...
	protected void addToFolder(Context context, DomainObject domainObj, String folderId) throws Exception {
		...
		if (folderId != null && folderId.length() > 0 && !folderId.equals("null")) {
			...
			while (st.hasMoreTokens()) {
				...
				sendEventIconMail(context, LibraryCentralConstants.EVENT_CONTENT_ADDED, new DCWorkspaceVault(temp),
						objID);
	 ...
	 * 
	 * @param objectId The Matrix ObjectId of the object for which notification
	 * should be sent.
	 * 
	 * @param specificationMap A specification map, containing the keys for the
	 * message (JPO_SPEC_NOTIFY_MESSAGE) and the users to whom the notification is
	 * to be sent (JPO_SPEC_NOTIFY_TO_USERS).
	 * 
	 * @throws Exception if the operation fails
	 */
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: subject - subject message
	 *                - message to send user list - users to send messages to
	 * @throws Exception if a major failure occurs
	 */
	 ...
	 * @throws Exception if the operation fails
	 */
	protected void notifyUsers(Context context, BusinessObject busObj, String subject, String message,
			String toUsersList) throws Exception {
		...
		if (toUsersList != null && toUsersList.length() > 0 && !toUsersList.equals("null")) {
			...
			if (subject == null || subject.length() == 0) {
				...
				subject = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strNotificationSub, null,
						locale);
			}
			...
			if (message == null || message.length() == 0) {
				...
				String msg = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strNotificationMessage,
						null, locale);
				...
				String autoName = emxMailUtil_mxJPO.getString("emxDocumentCentralStringResource", strAutoName, null,
						locale);

				int autoNameIndex = message.indexOf(autoName);

				if (autoNameIndex != -1) {
					message = message.substring(0, autoNameIndex) + name
							+ message.substring(autoNameIndex + autoName.length());
				}
			} // else

			MailUtil.sendNotification(context, toList, null, null, subject, null, null, message, null, null, objList,
					companyName);
		}
	}
	...
	/**
	 * Helper function which provides all required argument keys for the create
	 * operation. Specifically, the following Strings are required:
	 * <code>LibraryCentralConstants.JPO_ARGS_TYPE,
	...
	static public String[] getRequiredCreateArgKeys() {
		String[] reqdArgKeys = new String[] { LibraryCentralConstants.JPO_ARGS_TYPE,
				LibraryCentralConstants.JPO_ARGS_REVISION, LibraryCentralConstants.JPO_ARGS_POLICY,
				LibraryCentralConstants.JPO_ARGS_VAULT, LibraryCentralConstants.JPO_ARGS_ATTR_MAP,
	...
	/**
	 * This method validates that all required argument keys are contained in the
	 * argMap.
	 *
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: childIds - a String array of
	 *                children object ids relationship - the relationship to connect
	 *                the new object with parentId - the id of the parent Object to
	 *                connect with
	 * @returns nothing
	 * @throws Exception if the operation fails
	 */
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: objId - object id
	...
	public String deleteLCObjects(Context context, String[] packedArgs) throws Exception {
		...
		try {
			...
			if (access.hasDeleteAccess()) {
				MapList mapResultList = doObj.getRelatedObjects(context, strRelationPattern,
						LibraryCentralConstants.QUERY_WILDCARD, selSelect, null, false, true, (short) 1, null, null, 1);
				if (mapResultList.isEmpty()) {
					String strInterface = doObj.getAttributeValue(context,
							LibraryCentralConstants.ATTRIBUTE_MXSYS_INTERFACE);
					String strTypeName = doObj.getInfo(context, SELECT_TYPE);
					kindOfGenLibOrClass = doObj.isKindOf(context, LibraryCentralConstants.TYPE_GENERAL_CLASS)
							|| doObj.isKindOf(context, LibraryCentralConstants.TYPE_GENERAL_LIBRARY);
					// delete Library Feature Reference first to avoid its policy issues
					// then delete the lib object and then only delete the interface to avoid its
					// trigger issues
					ContextUtil.startTransaction(context, true);
					strResult = "";
	...
	private void deleteLibraryFeatureReference(Context context, String strPhyID) throws Exception {
		try {
			ContextUtil.pushContext(context);
			MapList mapList = DomainObject.findObjects(context, TYPE_LIBRARY_FEATURE_REFERENCE, strPhyID,
					QUERY_WILDCARD, QUERY_WILDCARD, QUERY_WILDCARD, "", false, new StringList(DomainObject.SELECT_ID));
			if (!mapList.isEmpty()) {
	...
	public int checkForContainerObjectsBeforeDelete(Context context, String[] args) throws Exception {
		...
		strRelation.append(LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM);
		strRelation.append(",");
		strRelation.append(LibraryCentralConstants.RELATIONSHIP_STANDARD_CLASSIFIED_ITEM);
		Pattern patternRelation = new Pattern(strRelation.toString());
		String strRelationPattern = patternRelation.getPattern();
		...
		if (!mapResultList.isEmpty()) {
			...
			String stdMsg = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(languageStr), "emxDocumentCentral.Deleted.NotAbleToDeleteMessage");
			String typeNLS = EnoviaResourceBundle.getAdminI18NString(context, "Type",
					(String) resultMap.get(DomainObject.SELECT_TYPE), languageStr);
	...
	public int deleteLCObjectInterface(Context context, String[] args) throws Exception {
		...
		String generatedInterfaceName = "Generated" + strPhyID;
		String mqlResult = MqlUtil.mqlCommand(context, "list interface $1", strPhyID + "," + generatedInterfaceName);
		if (UIUtil.isNotNullAndNotEmpty(mqlResult)) {

			String[] result = mqlResult.split("\n");

			if (result.length > 1) {
				String isHidden = MqlUtil.mqlCommand(context, "print interface $1 select $2 dump",
						generatedInterfaceName, "hidden");
				if ("TRUE".equalsIgnoreCase(isHidden)) {
					MqlUtil.mqlCommand(context, "delete interface $1", true, generatedInterfaceName);
				}
			}
			if (strPhyID.equals(result[0])) {
				MqlUtil.mqlCommand(context, "delete interface $1", true, strPhyID);
			}

			// TO invalidate VPLM Cache
			String kindOfGenClass = MqlUtil.mqlCommand(context, "print type $1 select $2 dump", true, strTypeName,
					"kindof[" + TYPE_GENERAL_CLASS + "]");
			if (kindOfGenClass.equalsIgnoreCase("TRUE")) {
				LibraryUtil.invalidateVPLMCatalogueCache(context);
			} else {
				String kindOfGenLib = MqlUtil.mqlCommand(context, "print type $1 select $2 dump", true, strTypeName,
						"kindof[" + TYPE_GENERAL_LIBRARY + "]");
	...
	public String removeObjects(Context context, String[] packedArgs) throws Exception {
		...
		try {
			...
			for (int i = 0; i < iSize; i++) {
				...
				try {
					MapList resultList = (MapList) tempChildObj.getRelatedObjects(context,
							strRelationhipPattern.toString(), LibraryCentralConstants.QUERY_WILDCARD, slSelAttrs, null,
					...
					if (resultList.size() == 0) {
						...
						if (searchType != null && searchType.equalsIgnoreCase("All Levels")) {
							...
							String strResult = MqlUtil.mqlCommand(context, strQuery, parentId, sRelSubclass,
									sRelClassifiedItem, "id",
			...
			objectsNotRemoved += "|"
					+ (String) MqlUtil.mqlCommand(context, strQuery, parentId, sRelSubclass, "id", ",");
			if (ValidationFailed) {
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: type - the object type
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param holds   the following input arguments: 1 - object ID 2 - event the
	 *                event name
	 * @throws Exception if the operation fails
	 */
	public void classifiedItemContentModify(matrix.db.Context context, String[] args) throws Exception {
		...
		try {
			...
			if (objectId != null && !"".equals(objectId) && !"null".equals(objectId)) {
				...
				mListParent = (MapList) domainObj.getRelatedObjects(context,
						DomainConstants.RELATIONSHIP_CLASSIFIED_ITEM, LibraryCentralConstants.QUERY_WILDCARD,
						slSelAttrs, null, true, false, (short) 0, null, null);
	...
	/**
	 * To restrict connecting any from item on Subclass relation to have only
	 * limited to types e.g Part Library can have Part Family(and its derivatives)
	 * as subclass Any classification type (General Class, Part Family and
	 * Manfacturing Partfamily) can have a same type or its derivatives as Subclass
	 * i.e. General Class can have an other General Class as subclass Part Family
	 * can have Part Family and Manfacturing Partfamily as subclass Manfacturing
	 * Partfamily can have an other Manfacturing Partfamily as subclass Libraries
	 * (General Library and Part Library) can have any classification type as
	 * subclass So no need to put 'Libraries' in return Map, i.e. if any from item
	 * is not included in return Map all items on to side can connect as Subclass to
	 * that from side item.
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds no arguments
	 * @return Map with key = fromside item (symbolic name) on subclass relation and
	 *         value = set (of symbolic names) of types that can be subclass for
	 *         that from item
	 * @throws Exception if the operation fails
	 */

	public Map getConditionalAllowedClassesInSubclassRelation(Context context, String[] args) throws Exception {
		Map relFromTypes = LibraryCentralCommon.getTypesFromRelation(context,
				LibraryCentralConstants.RELATIONSHIP_SUBCLASS, true);
		...
		for (Iterator iter = relFromTypes.keySet().iterator(); iter.hasNext();) {
			...
			// For General Class, Part Family and Manfacturing Partfamily only the same
			// class can be added as allowed class
			...
			if (LibraryCentralCommon.isClassificationType(context, buName)) {
				...
				allowedTypeMap.put(from, LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_PART_FAMILY, true));
			} else if (buName.equals(LibraryCentralConstants.TYPE_LIBRARY)) {
				// For Document Library only Document Family is allowed.
				allowedTypeMap.put(from, LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_DOCUMENT_FAMILY, true));
			}
		}
	...
	/**
	 * To restrict connecting any from item on Classified Item relation to have only
	 * limited to types e.g Part Family (and its derivates) can have only Part (and
	 * its derivatives) as its end items
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds no argument
	 * @return Map with key = fromside item (symbolic name) on ClassifiedItem
	 *         relation and value = set (of symbolic names) of types that can be
	 *         subclass for that from item
	 * @throws Exception if the operation fails
	 */

	public Map getConditionalAllowedEndItemsInClassifiedItemRelation(Context context, String[] args) throws Exception {
		Map relFromTypes = LibraryCentralCommon.getTypesFromRelation(context,
				LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM, true);
		Map allowedTypeMap = new HashMap();
		for (Iterator iter = relFromTypes.keySet().iterator(); iter.hasNext();) {
			String from = (String) iter.next();
			BusinessType bu = (BusinessType) relFromTypes.get(from);
			...
			if (buName.equals(LibraryCentralConstants.TYPE_PART_FAMILY)
				...
				String SUPPLIER_EQUIVALENT_PART = PropertyUtil.getSchemaProperty(context,
						"type_SupplierEquivalentPart");
				Set derivativeSymbolicNames = LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_PART, true);
				derivativeSymbolicNames.addAll(
						LibraryCentralCommon.getDerivativesSymbolicNames(context, SUPPLIER_EQUIVALENT_PART, true));
				...
				Set derivativeSymbolicNames = LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_GENERIC_DOCUMENT, true);
				derivativeSymbolicNames.addAll(LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_DOCUMENT, true));
				...
				derivativeSymbolicNames.addAll(
						LibraryCentralCommon.getDerivativesSymbolicNames(context, type_ControlledDocument, true));
				allowedTypeMap.put(from, derivativeSymbolicNames);
				...
				Map relToTypes = LibraryCentralCommon.getTypesFromRelation(context,
						LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM, false);
				Set toTypeNamesOnRel = relToTypes.keySet();
				toTypeNamesOnRel.removeAll(LibraryCentralCommon.getDerivativesSymbolicNames(context,
						LibraryCentralConstants.TYPE_DOCUMENT_SHEET, true));
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - policy
	...
	/**
	 * This method checks whether a given type is Document Central Type i.e.
	 * Document library/Book shelf/ Book
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - type
	...
	public boolean isOrphanAllowed(Context context, String[] args) throws Exception {
		String OrphanSettings = EnoviaResourceBundle.getProperty(context,
	...
	/**
	 * This method returns true if orphan object can be created and the given type
	 * is DocumentCentralType
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	...
	/**
	 * This method returns true if orphan object can not be created and the given
	 * type is DocumentCentralType
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	...
	/**
	 * This method returns true if orphan object can not be created and the given
	 * type is not a DocumentCentralType
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	...
	public String getCreateInTypes(Context context, String[] args) throws Exception {
		...
		try {
			...
			if (allowedParentsMap.isEmpty() || allowedParentsMap.get(type) == null) {
				baseType = "null";
			} else {
				baseType = ((String) allowedParentsMap.get(type)).trim();
			}

			if (null != baseType && "null".equalsIgnoreCase(baseType)
					&& baseType.lastIndexOf(",") == baseType.length()) {
	...
	/**
	 * This method returns exclude OIDs (for which user does not have form connect
	 * access)
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - field - create In type
	 *                and it should be like TYPE=Document Library
	...
	public StringList getCreateInExcludeOIDs(Context context, String[] args) throws Exception {
		...
		try {
			...
			MapList mapList = DomainObject.findObjects(context, createInType, null, null, null, "*",
					whereExp.toString(), true, new StringList(DomainObject.SELECT_ID));
	...
	public HashMap getVaultList(Context context, String[] args) throws Exception {
		...
		try {
			...
			for (int i = 0; i < vaultsArray.length; i++) {
				...
				fieldDisplayChoices.add(UINavigatorUtil.getAdminI18NString(JPO_ARGS_VAULT, vaultsArray[i],
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - type 1 - languagestr
	...
	public String getNotificationMessage(Context context, String[] args) throws Exception {
		...
		String[] keys = new String[] { DomainConstants.SELECT_TYPE, DomainConstants.SELECT_NAME, USER,
				DomainConstants.SELECT_TYPE };
		String[] values = new String[] { type, name, user, type };
		String notMessage = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
				new Locale(languagestr), "emxDocumentCentral.DocumentCentralCommon.NotificationMessage");
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - TypeActual 1 - Name 2
	 *                - Owner 3 - Policy
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 -
	 *                launguagestr
	...
	public String getLockStatus(Context context, String[] args) throws Exception {
		...
		try {
			...
			String lockedByMessage = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(languagestr), "emxDocumentCentral.Common.LockedBy");
			if (locked != null && !"null".equals(locked) && "true".equalsIgnoreCase(locked)) {
				...
				sbReturnString.append(lockedByMessage).append(" ").append(XSSUtil.encodeForHTML(context, lockedBy))
						.append("'>");
	...
	/**
	 * This method sets the Create In Default value when the form is launched from
	 * any parent
	 *
	 * @param context the eMatrix <code>Context</code> object
	...
	public String setCreateInDefaultValue(Context context, String[] args) throws Exception {
		...
		try {
			...
			sbReturnString.append("  document.forms[\'emxCreateForm\'].elements[\'Create In\'].value =\""
					+ XSSUtil.encodeForJavaScript(context, objectName) + "\"; ");
			sbReturnString.append("  document.forms[\'emxCreateForm\'].elements[\'Create InDisplay\'].value =\""
					+ XSSUtil.encodeForJavaScript(context, objectName) + "\"; ");
			sbReturnString.append("  document.forms[\'emxCreateForm\'].elements[\'Create InOID\'].value =\""
	...
	/**
	 * This method updates Create In Field connects the created object with
	 * seleceted container Id
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 - New OID
	 *                (i.e value of Create In Field)
	 * @throws Exception if the operation fails
	 */
	public void updateCreateInField(Context context, String[] args) throws Exception {
		try {
			...
			String[] containingObjIdsArray = (String[]) slContainingObjIds
					.toArray(new String[slContainingObjIds.size()]);
	...
	/**
	 * This method updates Folder field connects the created object with selected
	 * folder
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 - New OID
	 *                (i.e value of fodler Id)
	 * @throws Exception if the operation fails
	 * 
	 */
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 - New
	 *                Value (i.e notify users) 2 - launguagestr 3 - Message
	 * @throws Exception if the operation fails
	 */
	public void updateNotifyField(Context context, String[] args) throws Exception {
		try {
			...
			DomainObject domainObj = new DomainObject(objectId);
			String subject = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(languagestr), "emxDocumentCentral.Notify.Create.Subject");
	...
	/**
	 * This method updates Title field If it is Empty with Object Name The updation
	 * of Title field is done in updateNotifyField
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	 * @throws Exception if the operation fails
	...
	/**
	 * This method updates Message field The updation of Message field is done in
	 * updateNotifyField
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object ID 1 - vault 2
	 *                - name 3 - type
	...
	/**
	 * This method creates the List of Folder Objects To be Displayed in Add To
	 * Folder Option
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	...
	public StringList getFolders(Context context, String[] args) throws Exception {
		...
		MapList mpList = myVault.findObjects(context, PropertyUtil.getSchemaProperty(context, "type_ProjectVault"), "*",
				"", "*", "*", where.toString(), false, strList);
	...
	/**
	 * This method checks if the Object has Connection directly or indirectly with
	 * WorkSpace Vault
	 * 
	 * @param context the eMatrix <code>Context</code> object
	...
	public boolean isDCFolder(Context context, String id) throws Exception {
		...
		String vaultId = (String) dmObj.getInfo(context,
				"to[" + DomainObject.RELATIONSHIP_WORKSPACE_VAULTS + "].from.id");
		if (vaultId == null) {
	...
	/**
	 * This method gets the attribute Range value for Gloabal read and returns the
	 * Corresponding Value ad the Display Value
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments
	...
	public HashMap getFolderSecurityOptions(Context context, String[] args) throws Exception {
		...
		try {
			...
			fieldDisplayChoices.add(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(context.getSession().getLanguage()),
					"emxDocumentCentral.CreateFolder.GlobalReadAccess"));
			fieldChoices.add("False");
			fieldDisplayChoices.add(EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(context.getSession().getLanguage()), "emxDocumentCentral.CreateFolder.Private"));
			rangeMap.put("field_choices", fieldChoices);
			rangeMap.put("field_display_choices", fieldDisplayChoices);
		} catch (Exception ex) {
			throw new FrameworkException(ex.toString());
	...
	/**
	 * This method gets the attribute value for Gloabal read and returns the
	 * Corresponding String for Display
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object ID 1 -
	 *                languageStr
	...
	public static String getFolderSecurityValue(Context context, String[] args) throws Exception {
		...
		if (attrGlobalReadValue != null && !"null".equalsIgnoreCase(attrGlobalReadValue)
				&& attrGlobalReadValue.equalsIgnoreCase("True")) {
			attrGlobalReadValue = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(context.getSession().getLanguage()), "emxDocumentCentral.CreateFolder.Global");
		} else {
			attrGlobalReadValue = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
					new Locale(context.getSession().getLanguage()), "emxDocumentCentral.CreateFolder.Private");
		}
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - mode
	...
	public static String getVCSelector(Context context, String[] args) throws Exception {
		...
		return ("true".equalsIgnoreCase(vcfile) ? (String) objectDetailsMap.get("vcfile[1].specifier")
				: (String) objectDetailsMap.get("vcfolder[1].config"));
	}
	...
	public static String getVCServer(Context context, String[] args) throws Exception {
		...
		String vcfile = (String) objectDetailsMap.get("vcfile");
		String vcfolder = (String) objectDetailsMap.get("vcfolder");
		return ("true".equalsIgnoreCase(vcfile) ? (String) objectDetailsMap.get("vcfile[1].store")
				: (String) objectDetailsMap.get("vcfolder[1].store"));
	}
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - ObjectId 1 -
	 *                newObjectId
	 * @return boolean - isVersionable
	 * @throws Exception if the operation fails
	 */
	...
	public static void connectDocumentSheetToGenericDocument(Context context, String[] args) throws Exception {
		try {
			...
			if (!UIUtil.isNullOrEmpty(objectId) && !UIUtil.isNullOrEmpty(parentObjectId)) {
				...
				DomainRelationship.connect(context, dparentObject, LibraryCentralConstants.RELATIONSHIP_DOCUMENT_SHEETS,
						dNewObject);
			}
		} catch (Exception ex) {
			throw new FrameworkException(ex.toString());
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - requestMap
	...
	public Map createLBCObject(Context context, String[] args) throws Exception {
		...
		try {
			...
			/*
			 * try{
			 * if(type.equalsIgnoreCase(LibraryCentralConstants.TYPE_GENERAL_LIBRARY)||type.
			 * equalsIgnoreCase(LibraryCentralConstants.TYPE_GENERAL_CLASS))
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 1 - object Is
	 * @throws Exception if the operation fails
	 */
	@com.matrixone.apps.framework.ui.PostProcessCallable
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: contentIDs - contentIds
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: type - type
	 * @throws Exception if a major failure occurs
	 */
	public HashMap getPolicies(Context context, String[] args) throws Exception {
		...
		PolicyList allPolicyList = boType.getAllPoliciesForPerson(context).filterHidden();
		...
		while (policyItr.next()) {
			...
			if ((typeString.equals(TYPE_LIBRARY) && policyName.equals(POLICY_LIBRARIES))
					|| (typeString.equals(TYPE_DOCUMENT_FAMILY)
							&& policyName.equals(LibraryCentralConstants.POLICY_CLASSIFICATION))
	...
	/***
	 * Trigger method, which would be called for every create of Subclass
	 * relationship. During the connection, child Objects revision would be compared
	 * with the parent objects physicalid, if its same, then the revision would not
	 * be changed to physicalid of the parent. Otherwise it would be changed. During
	 * creation of object, if there exists a parent, the object revision would be
	 * changed to physicalid of the parent,if no parent exist, the revision would
	 * not be changed.
	 * 
	 * @param context
	 * @param args
	...
	/***
	 * Changes the revision of the object to the PhysicalId of the parent object.
	 * Since changing the revision alone is not permitted, we are changing the name
	 * of the object also with a "~" and reverting back to the original name.
	 * 
	 * @param context           the eMatrix <code>Context</code> object
	 * @param childId,          the objectId whose revision needs to be changed to
	 *                          PhysicalId
	 * @param objectName,       the name of the object
	 * @param parentPhysicalId, physicalId of the parent Object
	 * @throws FrameworkException,if the operation fails
	 */
	public void changeRevisiontoPhysicalId(Context context, String childId, String objectName, String parentPhysicalId)
			throws FrameworkException, Exception {
		String errorMessage = EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
				new Locale(context.getSession().getLanguage()), "emxMultipleClassification.AddExisting.Error");
		try {
			...
			MqlUtil.mqlCommand(context, mqlerrorQuery, errorMessage, objectName,
					new DomainObject(childId).getInfo(context, SELECT_TYPE));
			err.printStackTrace();
		}
	}
	...
	/**
	 * This method returns the vault associated with the context's Person this is
	 * temporarily added to replace ${CLASS:emxDocumentCentralRootBase}:systemVaults()
	 * 
	 * @param context
	 * @param args
	 * @return
	...
	/***
	 * This method return all the Subclasses for a Library. Used during classify or
	 * Reclassify on End Items
	 * 
	 * @param context
	 * @param args
	 * @return
	...
	public String getAllSubClassforLibrary(Context context, String args[]) throws FrameworkException {
		...
		try {
			...
			DomainObject domObj = new DomainObject(objectId);
			MapList mList = null;
			mList = domObj.getRelatedObjects(context, LibraryCentralConstants.RELATIONSHIP_SUBCLASS, "*", objectSelects,
					relselects, true, // boolean getTo,
	...
	/**
	 * This method checks if the Document Object is locked Used as check trigger in
	 * Generic Document lifecycle changes
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    an array of String arguments for this method 0 - objectId, the
	 *                id of this Generic Document
	...
	public int isLocked(Context context, String[] args) throws Exception {
		try {
			...
			MapList activeVersionsList = domainObj.getRelatedObjects(context, rel_ActiveVersion, "*", objectSelects,
					null, false, true, new Short("1"), "locked == true", "", 0);
			if (activeVersionsList != null && activeVersionsList.size() != 0) {
				...
				String errorString = "\\n" + EnoviaResourceBundle.getProperty(context,
						"emxLibraryCentralStringResource", new Locale(context.getSession().getLanguage()),
						"emxLibraryCentral.Document.FilesLockedMsg");
	...
	/***
	 * This is an Update Function used for updating values of Classification
	 * Attributes during Create For example: useful in Part or Generic Document
	 * create forms Adding as part of IR-652485-3DEXPERIENCER2020x :: BAE3 ::
	 * 
	 * @param context
	 * @param args
	 * @throws Exception
	 */
	public void updateClassificationAttributeForCreateForm(Context context, String[] args) throws Exception {
		HashMap argsMap = (HashMap) JPO.unpackArgs(args);
		...
		if (UIUtil.isNotNullAndNotEmpty(newObjectId)) {
			...
			if (UIUtil.isNotNullAndNotEmpty(attributeName)) {
				...
				if (attributeValueNonEmpty) {
					if (("timestamp").equals(attributeDataType)) {
						try {
							...
							attributeFormattedValue = eMatrixDateFormat.getFormattedInputDate(context, attributeValue,
									iClientTimeOffset, locale);
							if (UIUtil.isNotNullAndNotEmpty(attributeFormattedValue))
								attributeValue = attributeFormattedValue;
						} catch (Exception e) {
							/*
							 * in some cases even though exception occurs we need to continue with original
							 * attributeValue,hence not throwing exception here
							 */
						}
					}
				}
				...
				if (UOMUtil.isAssociatedWithDimension(context, attributeName) && (attributeValueNonEmpty)
						&& !(attributeType.isMultiVal())) {
					allAttrsinAGMap.put(attributeName,
							attributeValue + " " + ((String[]) requestMap.get("units_" + attributeFieldName))[0]);
				}
				...
				// checking if attribute is of type multival
					...
					if (strOrder != null && strOrder.length > 0 && UIUtil.isNotNullAndNotEmpty(strOrder[0])) {
						...
						while (tokens.hasMoreTokens()) {
							...
							if (mutliAttrValArray != null && mutliAttrValArray.length > 0
									&& UIUtil.isNotNullAndNotEmpty(mutliAttrValArray[0])) {
								if (dimFlag)
				...
				} else if ((attributeType.isRangeVal())
						&& (("real").equals(attributeDataType) || ("integer").equals(attributeDataType))) {
	...
	/***
	 * This is a range value Update Function used for updating rangeValue type of
	 * Classification Attributes during Edit
	 * 
	 * @param context
	 * @param args
	 * @throws Exception
	 */
	public void UpdateRangeValueFunction(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		String objectId = (String) paramMap.get(OBJECT_ID);
		...
		if (programMap.containsKey("fieldMap")) {
			...
			attributeName = attributeName.substring(attributeName.indexOf(LibraryCentralConstants.IPC_DELIMITER_PIPE)
					+ LibraryCentralConstants.IPC_DELIMITER_PIPE.length());
		}
	...
	public Boolean isENGInstalled(Context context, String[] args) {
		boolean engInstalled = FrameworkUtil.isSuiteRegistered(context, "appVersionX-BOMEngineering", false, null,
				null);
		return engInstalled;
	}
	 ...
	 * @return Boolean depending on if the context user has the Library User or
	 *         similar role or not.
	 * @throws Exception if the operation fails.
	...
	public Boolean hasLibraryViewerRole(Context context, String[] args) throws Exception {
		...
		boolean isVPLMViewer = personObj.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_VPLMViewer"));
		...
		return Boolean.valueOf(isLibraryUser || isAuthor || isLimitedAuthor || isReleaseManager || isReviewer
				|| isVPLMAdmin || isVPLMViewer || is3DSRestrictedReader);
	}
	 ...
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    contains a packed HashMap
	 * @return Boolean depending on if the context user has the Record Retention
	 *         Manager Role.
	 * @throws Exception if the operation fails.
	 * @since CommonComponents 11.0
	 */
	...
	/**
	 * This Method returns the field parameter to emxFullSearch.jsp to choose Valid
	 * Approvers
	 * 
	 * @param context
	 * @param args
	 ...
	 *
	 * @param context :
	 * @param args    : contains HashMap Object which in turn has either a Library
	 *                or Class Info
	 *
	...
	public String createLBCObjectUsingREST(Context context, String[] args) throws Exception {
		try {
			...
			String classificationUsage = objMap.get(LibraryCentralConstants.JPO_ARGS_CLASSIFICATION_USAGE);
			if (UIUtil.isNullOrEmpty(classificationUsage))
				classificationUsage = LibraryCentralConstants.ATTRIBUTEVALUE_ClassificationUsage_Definition;
			...
			if (obj.isKindof(context, objMap.get(LibraryCentralConstants.TYPE_ATTRIBUTE),
					LibraryCentralConstants.TYPE_LIBRARIES)) {
			...
			} else if (obj.isKindof(context, objMap.get(LibraryCentralConstants.TYPE_ATTRIBUTE),
			...
			if (!getRevision || !bo.exists(context)) {
				...
				// LibraryUtil.addUsageInterface(context,objectId,usage,parentId); //ISCAR
				// FUN113089
				DomainObject domObj = new DomainObject(objectId);

				if (type.equals(LibraryCentralConstants.TYPE_GENERAL_CLASS))
					domObj.setAttributeValue(context, LibraryCentralConstants.ATTRIBUTE_ClassUsage,
							classificationUsage);
				if (type.equals(LibraryCentralConstants.TYPE_GENERAL_LIBRARY))
					domObj.setAttributeValue(context, LibraryCentralConstants.ATTRIBUTE_LibraryUsage,
							classificationUsage);
	...
	public void updateLBCObject(Context context, String[] args) throws Exception {
		try {
			...
			if (null != libMap.get(LibraryCentralConstants.SELECT_ID)
					&& !libMap.get(LibraryCentralConstants.SELECT_ID).isEmpty()) {
				...
				if (null != libMap.get(LibraryCentralConstants.SELECT_OWNER)) {
					/*
					 * Person person = new Person(); boolean exists =
					 * person.doesPersonExists(context, libMap.get("owner")); if(exists) {
					 */
				...
				if (null != libMap.get(LibraryCentralConstants.TITLE)) {
					obj.setAttributeValue(context, LibraryCentralConstants.SELECT_TITLE,
							libMap.get(LibraryCentralConstants.TITLE));
				}
	...
	/*
	 * public static int checkAllowedItem(Context context, String[] args) throws
	 * Exception { int strResult = 1; try{ if(args.length > 0) { String
	 * strToObjectId = args[1]; String strFromObjectId = args[0]; String
	 * relationName = args[2]; Map alloweditem = new HashMap(); DomainObject toobj =
	 * new DomainObject(strToObjectId); DomainObject fromobj = new
	 * DomainObject(strFromObjectId); String totype =
	 * UICache.getSymbolicName(context, toobj.getInfo(context,
	 * DomainConstants.SELECT_TYPE), "type"); String fromtype =
	 * UICache.getSymbolicName(context, fromobj.getInfo(context,
	 * DomainConstants.SELECT_TYPE), "type"); //fromobj.isKindOf(context, type);
	 * if(relationName.equals(LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM))
	 * { alloweditem = LibraryCentralCommon.getAllowedEndItemsMap(context); }else {
	 * alloweditem = LibraryCentralCommon.getAllowedClassesMap(context); }
	 * if(alloweditem.containsKey(fromtype)) { String allowedtype =
	 * (String)alloweditem.get(fromtype); StringList allowedtypelist = new
	 * StringList(allowedtype.split(",")); if(allowedtypelist.contains(totype)) {
	 * strResult = 0; return strResult; }else { for(Iterator iter =
	 * allowedtypelist.iterator(); iter.hasNext();) { String allowedname =
	 * (String)iter.next(); String allowednameactual =
	 * PropertyUtil.getSchemaProperty(context,allowedname);
	 * if(toobj.isKindOf(context, allowednameactual)) { strResult = 0; return
	 * strResult; } } } } } }catch(Exception ex) { ex.printStackTrace(System.out);
	 * throw ex; } return strResult; }
	 */

	/***
	 * This Function returns True showGranteeAccessCategory is set to true in
	 * emxLibraryCentral.properties, otherwise False
	 * 
	 * @param context
	 * @param args
	 * @throws Exception
	...
	public Boolean showGranteeAccessCategory(Context context, String[] args) {
		...
		try {
			showGranteeAccessCategory = EnoviaResourceBundle.getProperty(context,
					"emxLibraryCentral.showGranteeAccessCategory");
	...
	// KDR2
	/***
	 * This Function executes when user update any column value of an attribute on
	 * object
	 * 
	 * @param context
	 * @param args
	...
	public Vector getLabel(Context context, String[] args) throws Exception {
		...
		try {
			...
			// Fetching required attributes of the objectids using
			// getSelectBusinessObjectData
			BusinessObjectWithSelectList bosAttributeList = BusinessObject.getSelectBusinessObjectData(context,
					objIdArray, selectStmts, false);
