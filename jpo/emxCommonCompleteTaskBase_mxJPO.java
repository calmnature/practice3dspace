...
public class emxCommonCompleteTaskBase_mxJPO
{
	...
	public static int completeTask (Context context, String args[]) throws Exception
	{
		...
		//Done mbs21 Declared a global variable attrMap to store the attribute changed and set it to inboxTask 
		//attrMap is not used anywhere in this file
		...
		try {
			...
			String sNotPromotedObjsChangeRelated="";
			String sNotPromotedProcessObject = "";
			...
			StringBuffer sBufNoPromotedObjects =new StringBuffer();
			StringBuffer sBufNoPromotedProcessObjects =new StringBuffer();
			StringBuffer sBufNotPromotedObjectsNotification =new StringBuffer();
			StringBuffer sBufNotPromotedProcessObjectsNotification =new StringBuffer();
           ...
           if (!bConsiderAdhocRoutes.equals("TRUE")) {
			...
			RouteModeler.addTitleSelectables(objectSelects);
			...
			for(int i = 0; i < routeContentList.size(); i++) {
				...
				sRouteContent3DTitles.append(RouteModeler.getDisplayTitle(routeContent));
					...
					if("Yes".equalsIgnoreCase(sRouteDemoteAction) ) {
						for(int count=0; count<routeContentList.size();count++) {
							...
							for(int i=0; i<ObjectList.size() ; i++) {
								...
								String baseState = (String) objectsMap.get("attribute["+sAttRouteBaseState+"].value");
								String basePolicy= (String) objectsMap.get("attribute["+sAttRouteBasePolicy+"].value");
			...
			if (!"Reject".equals(sApprovalStatus))
			{
				...
				for(String sTaskId: tasksToBeCompletedList){
					...
					// mbs21 3 setAttribute value???
					//Done mbs21 changed from 3 setAttributeValue to 1 setAttributeValue
					attrMap.clear();
					attrMap.put(sAttActualCompletionDate, sDate);
					attrMap.put(sAttApprovalStatus, "None");
					attrMap.put(sAttComments, EnoviaResourceBundle.getProperty(context,"emxFrameworkStringResource", strLocale, "emxFramework.ProgramObject.eServicecommonCompleteTask.AutoCompleteCommment")+" "+sTaskTitle);
					inboxTaskToComplete.setAttributeValues(context,attrMap);
					...
					if (sState.equals(sStateAssigned) || sState.equals(sStateReview))
					{
 						...
 						if ((sProcessionRule.toLowerCase()).equals("any")  && !"Auto Complete".equals(sApprovalStatus) )
						{
							...
							if (curRouteSequence.equalsIgnoreCase(sRouteSequence)) {
								...
								//Locale strLocale = context.getLocale();
								DomainObject inboxTaskToComplete = new DomainObject(sTaskId);
								//Done mbs21 changed 3 setAttributeValue to a single setAttributeValues
								attrMap.clear();
								attrMap.put(sAttActualCompletionDate, sDate);
								attrMap.put(sAttApprovalStatus, "None");
								attrMap.put(sAttComments, EnoviaResourceBundle.getProperty(context,"emxFrameworkStringResource", strLocale, "emxFramework.ProgramObject.eServicecommonCompleteTask.AutoCompleteCommment")+" "+sTaskTitle);
				...
				if (bFound == 0 && sRouteStatus.equals("Started")) {
					...
					if (outStr1 == 0) {
						...
						boolean isChangeObjectPresent = false;
							for(String value: respone.values()){
								if(UIUtil.isNotNullAndNotEmpty(value) && !"None".equalsIgnoreCase(value)){
									isChangeObjectPresent = true;
								}
							}

						boolean isProcessControlled = false;
						Map<String, String> processControlledResponse = new HashMap<>();
						if(UINavigatorUtil.isCloud(context)) {
							processControlledResponse = RouteModeler.getProcessControlledObject(context, docs);
							for(String value: processControlledResponse.values()){
								if(UIUtil.isNotNullAndNotEmpty(value) && "TRUE".equalsIgnoreCase(value)){
									isProcessControlled = true;
								}
							}
						}
						...
						if (sRouteCompletionAction.equals("Promote Connected Object")) {
							...
							for(int count=0; count<routeContentList.size();count++) {
								...
								if (bPromoteObject == 1) {
									...
									if("none".equalsIgnoreCase(respone.get(physicalId)) || "none".equalsIgnoreCase(respone.get(sObjId))){
										if(currState.equals(sObjCurrent) && UIUtil.isNotNullAndNotEmpty(targetState)){
											if("TRUE".equalsIgnoreCase(isPLMEntity)) {
												...
												Map<String, String> contentInfoForRule = new HashMap<String, String>();
												contentIdList.add((String)object.get(DomainConstants.SELECT_PHYSICAL_ID));
												if(contentIdList.size() > 0) {
													//to handle notify use-case
													String fromPage = com.matrixone.apps.domain.util.PropertyUtil.getGlobalRPEValue(context, "PROMOTE_ROUTE_CONTENT_TASK_COMPLETION");
													if("true".equalsIgnoreCase(fromPage)) {
														continue;
													}else {
														com.matrixone.apps.domain.util.PropertyUtil.setGlobalRPEValue(context, "PROMOTE_ROUTE_CONTENT_TASK_COMPLETION", "true");
														//contentInfoForRule = LifeCycleUtil.checkAndCreateRouteOnObjects(context, contentIdList);
														//check added for handling branching use-cases IR-1484773-3DEXPERIENCER2026x
														contentInfoForRule = LifeCycleUtil.checkAndCreateRouteOnObjects(context, (String)object.get(DomainConstants.SELECT_PHYSICAL_ID),currState, targetState );
													}
													
												}
												com.matrixone.apps.domain.util.PropertyUtil.unsetGlobalRPEValue(context, "PROMOTE_ROUTE_CONTENT_TASK_COMPLETION");
												//to Avoid deadlock. throwing the exception
												if(!contentInfoForRule.isEmpty()) {
													String hasException =contentInfoForRule.get("ERROR_ROUTE_CREATION");
													if(UIUtil.isNotNullAndNotEmpty(hasException)) {
												...
												if("true".equalsIgnoreCase(FrameworkUtil.getChangeMaturityControlValue(context))) {
													context.setCustomData("route_content_promotion","true");
												}
												com.matrixone.apps.common.RouteIRPCContentPromote.changeMaturityIRPCContents(context, physicalId, currState, targetState, sRoutePhysicalId);
//												if("true".equalsIgnoreCase(FrameworkUtil.getChangeMaturityControlValue(context))) {
//													context.removeFromCustomData("route_content_promotion");
//												}
											}else {
												if(!enforceSignature) {
...
//													context.removeFromCustomData("route_content_promotion");
									...
									} else if(UINavigatorUtil.isCloud(context) && "TRUE".equalsIgnoreCase(processControlledResponse.get(physicalId))){
										
										sBufNoPromotedProcessObjects.append(sObjType+" ");
										sBufNoPromotedProcessObjects.append(sObjName+" ");
										sBufNoPromotedProcessObjects.append(sObjRev+"\n");
										if(sBufNotPromotedProcessObjectsNotification.length() > 0){
											sBufNotPromotedProcessObjectsNotification.append(", ");
										}
										sBufNotPromotedProcessObjectsNotification.append(sObjName);
									} else {
										sBufNoPromotedObjects.append(sObjType+" ");
										sBufNoPromotedObjects.append(sObjName+" ");
										sBufNoPromotedObjects.append(sObjRev+"\n");
										if(sBufNotPromotedObjectsNotification.length() > 0){
											sBufNotPromotedObjectsNotification.append(", ");
										}
										sBufNotPromotedObjectsNotification.append(RouteModeler.getDisplayTitle(object));
									}
								}
							}//for
							...
							sNotPromotedObjsChangeRelated = sBufNoPromotedObjects.toString();
							sNotPromotedProcessObject = sBufNoPromotedProcessObjects.toString();
							...
							if ("".equals(sObjectsNotSatisfied) && "".equals(sRoutesInProcess) && "".equalsIgnoreCase(sNotPromotedObjsChangeRelated) && "".equalsIgnoreCase(sNotPromotedProcessObject)) {
...
//								taskAndRouteInfo.put(RouteTaskNotification.PROMOTED_OBJECT, sBufPromotedObjectsNotification.toString());
								...
								if(isChangeObjectPresent && !sBufNotPromotedObjectsNotification.isEmpty()) {
									taskAndRouteInfo.put(RouteTaskNotification.NOT_PROMOTED_CHANGE_RELATED_OBJECT, sBufNotPromotedObjectsNotification.toString());
								}
								if(isProcessControlled && !sBufNotPromotedProcessObjectsNotification.isEmpty()) {
									taskAndRouteInfo.put(RouteTaskNotification.NOT_PROMOTED_PROCESS_OBJECT, sBufNotPromotedProcessObjectsNotification.toString());
								}
...
//													context.removeFromCustomData("route_content_promotion");
										...
										sBufNotPromotedObjectsNotification.append(sObjName);
									}
								}
							}//for
							...
							if (!"".equals(sPromotedObjects) && isContentPromoted == true && "".equalsIgnoreCase(sNotPromotedObjsChangeRelated) && "".equalsIgnoreCase(sNotPromotedProcessObject)) {
								...
								objectIdList.add(sRouteId);
								mailUtil.sendNotification(context, toList, null, null, "emxFramework.ProgramObject.eServicecommonCompleteTask.SubjectComplete", null, null, "emxFramework.ProgramObject.eServicecommonCompleteTask.MessageCompletePromoteConnectedObject", messageKeys, messageValues, objectIdList, null,"both".equalsIgnoreCase(notifyType));
							} else if(!"".equalsIgnoreCase(sRoutesInProcess) || !"".equalsIgnoreCase(sNotPromotedObjsChangeRelated)){
								if ("".equals(sRoutesInProcess)) {
									arguments = new String[3];
									arguments[0]  = "emxFramework.ProgramObject.eServicecommonCompleteTask.None";
									arguments[1]  = "0";
								...
								StringList toList = new StringList();
								toList.add(sOwner);
								String[] messageKeys = {"RType","RName","RRev","RInProcess","PromotedObj"};
								String[] messageValues = {sRouteType,sRouteName,sRouteRev,sRoutesInProcess,sPromotedObjects};
								mailUtil.sendNotification(context, toList, null, null, "emxFramework.ProgramObject.eServicecommonCompleteTask.SubjectNotComplete", null, null, "emxFramework.ProgramObject.eServicecommonCompleteTask.MessageNotComplete", messageKeys, messageValues, null, null,"both".equalsIgnoreCase(notifyType));
								Map<String, String> taskAndRouteInfo = new HashMap<>();
								taskAndRouteInfo.put(RouteTaskNotification.ROUTE_ID, sRoutePhysicalId);
								taskAndRouteInfo.put(RouteTaskNotification.ROUTE_NAME, sRouteTitle);
				...
				//System.out.println("############## Before delete subroute : "+ subRouteToBeDeleted.size());
				//TODO mbs21 why triggerActionPromoteOnReviewStateDeleteSubRoutes trigger when we are deleting subroute here only???
			...
			if(isRouteCompleted){
				...
				//System.out.println("###################### isContentPromoted : " + isContentPromoted);
				//TODO mbs21 send routeInfo from here itself, dont add getInfo there!!
				//TODO mbs21 check if srouteId and the Route object has same srouteId if so, then pass it, also check if all attributeValues are matched
				...
				//System.out.println("######################### sRouteId : " + sRouteId + "routeId : " + routeId);
			}
		} catch(Exception e) {
			...
			if(UIUtil.isNotNullAndNotEmpty(routeId)) {
				...
				//Since routeId and sRouteId are same, why cant we send sRouteId object here instead of one more db call???
	...
	private static void sendNotification(Context context, String sRouteActionOfTask, String ObjectId,String sTaskPhysicalId ,String sRouteId,String sRoutePhysicalId,String routeOwner,String sApprovalStatus, String sRouteName, String sLastName, String sFirstName, String sPerson,String sType,String sName,String sTaskTitle,String sRev,String sRouteType,String sRouteRev, String dislpayCompletionDateUsersTZ, String sComments,String sReviewersComments, String sRouteLink, String sInboxTaskLink, int routeContentListSize, StringBuffer sRouteContent,StringBuffer sRouteContentIds, StringBuffer sRouteContentTitles, StringBuffer sRouteContentTypes, StringBuffer sRouteContentDescription,String sRouteContent3D,String sReviewTask,String sTaskAssignee, String sRouteTitle, String resourceReference,  String sTaskName) throws Exception {
		...
		if("Auto Complete".equals(sApprovalStatus)) {
			...
			if("USERGROUPOWNER".equals(sTaskAssignee)) {
				...
				java.util.List<String> tempList = new ArrayList<String>(Arrays.asList(mailList.split(",")));
