...
public class emxSubscriptionUtilBase_mxJPO extends emxDomainObject_mxJPO
{
           ...
           throws Exception
        {
	    	...
	    	strRelWhere = FrameworkUtil.getWhereExpressionForMql(context,strRelWhere,DomainConstants.TYPE_PERSON) ;
        ...
        public static Document prepareMailXML(Context context, Map headerInfo, Map bodyInfo, Map footerInfo) throws Exception
        {
            ...
            if (headerInfo != null && headerInfo.size() > 0)
            {
                ...
                if (headerText != null)
                {
                    ...
                    setElementContent(context, header, XSSUtil.encodeForXML(context,headerText));
                    headerData.addContent(header);
                }
                String creatorText = (String)headerInfo.get("creatorText");
                if (creatorText != null)
                {
                	if(creatorText.contains("&"))
                    {
                    	creatorText = creatorText.replace("&", XSSUtil.encodeForXML(context,"&"));
                    }
					String messageType = (String)headerInfo.get("messageType");
					if(creatorText.contains("<") && !("html".equalsIgnoreCase(messageType)))                	

                    {
                    	creatorText = creatorText.replaceAll("<", XSSUtil.encodeForXML(context,"<"));
                    }
            ...
            if (footerInfo != null && footerInfo.size() > 0)
            {
                ...
                for (int i = 0; dataLineList != null && i < dataLineList.size() ; i++)
                {
                    ...
                    dataLineText = dataLineText.replace("&","&amp;");
