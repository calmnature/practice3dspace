...
import com.dassault_systemes.enovia.issue.factory.XIssueFactory;
import com.dassault_systemes.enovia.issue.query.IXBrowseIssue;
import com.dassault_systemes.enovia.issue.query.IXIssueQuery;
import com.dassault_systemes.enovia.issue.query.IXIssueQueryParameters;
import com.dassault_systemes.enovia.issue.query.clause.XIssueQuerySource;
import com.dassault_systemes.enovia.issue.query.clause.XIssueStatusClauses;
import com.dassault_systemes.enovia.issue.service.IXIssueServices;
...
public class emxProgramCentralUtilBase_mxJPO
{
    ...
    public emxProgramCentralUtilBase_mxJPO () throws Exception {
    	super();
    }
    /**
...
//        hashMap.put("Action", "refresh");
    ...
    */
    ...
    private  static String TYPE_URL = null;
    ...
    static {
         ...
         TYPE_URL = PropertyUtil.getSchemaProperty("type_URL");
   ...
   public boolean SetPOVToParentPOV(Context context, String[] args) throws Exception
   {
      ...
      try{
         ...
         String bisFromRiskManagementType = DomainConstants.EMPTY_STRING;
         ...
         String bisToUrlType = DomainConstants.EMPTY_STRING;
         ...
         String SELECT_KINDOF_URL = "type.kindof[" + TYPE_URL+ "]";
         ...
         strFromList.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_CONCEPT);
         strFromList.add(ProgramCentralConstants.SELECT_KINDOF_TASKMANAGEMENT);
         strFromList.add(ProgramCentralConstants.SELECT_IS_KINDOF_RISKMANAGEMENT);
         // RVW: This should be added on toList, not fromList
         // strFromList.add(ProgramCentralConstants.SELECT_KINDOF_REQUIREMENT_SPEICIFICATION);
         ...
         bisFromRiskManagementType = (String)mpObjectInfo.get(ProgramCentralConstants.SELECT_IS_KINDOF_RISKMANAGEMENT);
         ...
         strToList.add(SELECT_KINDOF_URL);
         ...
         strToList.add(ProgramCentralConstants.SELECT_ATTRIBUTE_ACCESS_TYPE);

         //  IR-715553-3DEXPERIENCER2020x
         //  RVW: Modified to add type.kindof(Requirement Specification) to toList
         //
         strToList.add(SELECT_KINDOF_PART_SPECIFICATION);
         strToList.add(ProgramCentralConstants.SELECT_KINDOF_REQUIREMENT_SPEICIFICATION);
         ...
         bisToUrlType = (String)mpObjectInfo.get(SELECT_KINDOF_URL);
         ...
         String bisToRequirementSpecificationType = (String)mpObjectInfo.get(ProgramCentralConstants.SELECT_KINDOF_REQUIREMENT_SPEICIFICATION);

         // We no longer want to re-stamp type DOCUMENTS
        ...
        //
        if ("true".equalsIgnoreCase(bisToPartSpecificationType) || (bisToProxyPolicy.equalsIgnoreCase(drawingPrintPolicy)) || "true".equalsIgnoreCase(bisToRequirementSpecificationType))
            return true;
         ...
         {
            if("true".equalsIgnoreCase(bisToUrlType)) {
                String allowPOVStamping = PropertyUtil.getRPEValue(context, "MX_ALLOW_POV_STAMPING", true);
                if("true".equals(allowPOVStamping)) {
                    if("true".equalsIgnoreCase(bisFromProjectSpaceType) || "true".equalsIgnoreCase(bisFromTaskManagementType) || "true".equalsIgnoreCase(bisFromRiskManagementType)) {
                ...
                for (Iterator itrParent = mlParent.iterator(); itrParent.hasNext();)
                {
                    ...
                    if(mapParent != null)
                    {
                        ...
                        if("TRUE".equalsIgnoreCase(isProjectSpace) || "TRUE".equalsIgnoreCase(isProjectConcept) || "TRUE".equalsIgnoreCase(isProjectTemplate))
                        {
                             ...
                             * Earlier this responsibility was managed by BPS trigger.
                             */
                            String attrToAccessType = (String)mpObjectInfo.get(ProgramCentralConstants.SELECT_ATTRIBUTE_ACCESS_TYPE);
                            if((attrToAccessType!= null) && "Specific".equals(attrToAccessType)) {
                                ...
                                if (!UIUtil.isNullOrEmpty(result)) {
                                    ...
                                    DomainAccess.createObjectOwnership(context, toId, "", personName, accessName, null);
                                }
                            }
...
//            ex.printStackTrace();
    ...
    public void sendTaskMailHelper(Context context, String[] args) throws Exception {
        ...
        String whereClause = "type.kindof[Person]==TRUE || type.kindof[Group]==TRUE  || type.kindof[Group Proxy]==TRUE";
                String mqlResult = MqlUtil.mqlCommand(context, "temp query bus $1 $2 $3 where $4 select $5 dump", "*", sAssigneeName, "*", whereClause, "type");
                ...
                if(!this.isNotify(context, sAssigneeName)) {
                    continue;
                        ...
                        String groupWhereClause = "type.kindof[Group]==TRUE  || type.kindof[Group Proxy]==TRUE";
                        String userGroupInfo =
                                MqlUtil.mqlCommand(context, "temp query bus $1 $2 $3 where $4 select $5 dump", "*", sAssigneeName, "*", groupWhereClause, "physicalid");
...
//                      System.out.println("Simulia URL: " + objeNameURL);
                ...
                if (sDescription != null && !sDescription.isEmpty()) {
                    ...
                    String encodedDescription = XSSUtil.encodeForXML(context, sDescription);
                    String[] messageValuesForDescription = {encodedDescription  };
            ...
            String sEnoviaURLString = this.getEnoviaURLwithHTMLFormatting(context, sArgsArr);
                ...
                DebugUtil.debug("sendNotificationAssignee "+ ex.getMessage());
                throw ex;
            }finally{
                if(contextPushed){
                    ContextUtil.popContext(context);
        ...
        try {
            ...
            if(ProgramCentralUtil.isNotNullString(objectId)){
                ...
                try{
                    ...
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_NAME);
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_ID);
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_CONCEPT);
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_TEMPLATE);
                    selectList.add(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+DomainConstants.SELECT_ATTRIBUTE_TITLE);

                    ProgramCentralUtil.pushUserContext(context);

                    Map<String,String> taskInfoMap = taskObj.getInfo(context, selectList);

                    String isKindOfProjectSpace = (String) taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
                    String isKindOfProjectConcept = (String) taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_CONCEPT);
                    String isKindOfProjectTemplate = (String) taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+ProgramCentralConstants.SELECT_KINDOF_PROJECT_TEMPLATE);
                    //boolean isProjectType = "true".equalsIgnoreCase(isKindOfProjectSpace) || "true".equalsIgnoreCase(isKindOfProjectConcept) || "true".equalsIgnoreCase(isKindOfProjectTemplate);
                    prjName =  taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_NAME);
                    if (ProgramCentralUtil.isNotNullString(taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+DomainConstants.SELECT_ATTRIBUTE_TITLE))) {
                    	prjName =taskInfoMap.get(ProgramCentralConstants.SELECT_PROJECT_OBJECT+"."+DomainConstants.SELECT_ATTRIBUTE_TITLE);
                        }                    
 ...
 //     System.out.println(sbURL.toString());
    ...
    /**
     ...
     protected MapList getAllProjectIssues(Context context, String[] args, String Filter) throws Exception
     {
        ...
        HashMap programMap = (HashMap) JPO.unpackArgs(args);
        String objectId = (String) programMap.get("objectId");
        String isCallForProgramRIO = (String) programMap.get("isCallForProgramRIO");
        DebugUtil.debug("isCallForProgramRIO... "+ isCallForProgramRIO);
        ...
        //START:IR-1268305-3DEXPERIENCER2024x : Use index query of Issue to list in Program-->RIO-->Issues.
         //Also, split affected items list in chunk of 100, it was breaking if affected items number is more than 300.
        if("true".equalsIgnoreCase(isCallForProgramRIO)) {
            DebugUtil.debug("Loading Indexed issues for Program-->RIO-->Issues..." + taskIds.size());
            //System.out.println("****Loading Indexed issues for Program-->RIO-->Issues**** " + taskIds.size());
	        
	        List<String> allAffectedItemList = new ArrayList<>();
	        allAffectedItemList.addAll(taskIds);
            int affectedItemSize = allAffectedItemList.size();
            int chunkSize = 200;
            for (int i=0, j=allAffectedItemList.size(); i<j; i += chunkSize) {
            	//This is for last chunk if affected items are less than 200.
            	int maxIndex = Math.min(affectedItemSize, i + chunkSize);
            	List<String> affectedItemListChunk = allAffectedItemList.subList(i,maxIndex);
            	
        if("Active".equalsIgnoreCase(Filter)){
            		MapList issueMapList = getAllIssues(context, affectedItemListChunk, true, false);
            		returnIssueList.addAll(issueMapList);
        }else if("Closed".equalsIgnoreCase(Filter)){
            		MapList issueMapList = getAllIssues(context, affectedItemListChunk, false, true);
            		returnIssueList.addAll(issueMapList);
        }else if("All".equalsIgnoreCase(Filter)){
            		MapList issueMapList = getAllIssues(context, affectedItemListChunk, true, true);
            		returnIssueList.addAll(issueMapList);
            	}
        }
        //END:IR-1268305-3DEXPERIENCER2024x
        } else {
        ...
        MapList uniqueIssueList = new MapList();
        Set<String> issueIdsSet = new HashSet<String>();

        for(int i=0, size=returnIssueList.size(); i<size;i++){
            Map issueMap = (Map) returnIssueList.get(i);
            String issueId = (String) issueMap.get(DomainConstants.SELECT_ID);
            if(!issueIdsSet.contains(issueId)){
                uniqueIssueList.add(issueMap);
    ...
    /**
     * This method is copied from BPS Issue.java to improve the performance by making following changes.
     *
     * 1:Search for index data than real: issueQueryParameters.setSource(XIssueQuerySource.INDEX)
     * 2:Add multiple affected items to the query than one at a time:  issueQueryParameters.addAffectedItemPhysicalIdsSequence(idList)
     *
     * @param context
     * @param affectedItemIdList
     * @param active
     * @param closed
     * @return
     * @throws Exception
     */
    private static MapList getAllIssues(Context context, Collection<String> affectedItemIdCollection , boolean active , boolean closed) throws Exception {

        MapList issues = new MapList();
        try {
             IXIssueQueryParameters issueQueryParameters = XIssueFactory.createIssueFactory().createIssueQueryParameters();
             issueQueryParameters.setSource(XIssueQuerySource.INDEX);
             if(active) issueQueryParameters.addStatus(XIssueStatusClauses.ACTIVE);
             if(closed) issueQueryParameters.addStatus(XIssueStatusClauses.INACTIVE);

             //Add multiple affected items to the query than one at a time to improve the performance.
             //Query will hit DB once for all the affected items.
             for(String affectedItemId : affectedItemIdCollection) {
                 List<String> idList = new ArrayList<String>();
                 idList.add(affectedItemId);
                 issueQueryParameters.addAffectedItemPhysicalIdsSequence(idList);
             }

             issues = getIssuesByQuery(context, issueQueryParameters);
         } catch (Exception e) {
        	 e.printStackTrace();
             throw new FrameworkException(e);
         }
    ...
    private static MapList getIssuesByQuery(Context context,IXIssueQueryParameters queryParameters)  throws Exception{
         MapList issues = new MapList();
         IXIssueServices services = XIssueFactory.createIssueFactory();
         IXIssueQuery issueQuery = services.createIssueQuery(context, queryParameters);
         List<IXBrowseIssue> BrowserIssues = new ArrayList<IXBrowseIssue>();

         for(int i = 1; i <= issueQuery.getNumberOfPages(); i++)
             BrowserIssues.addAll(issueQuery.getPageContent(context, i));

         for(IXBrowseIssue browserIssue : BrowserIssues) {
             Map issue = new HashMap();
             String issueId = browserIssue.getM1Id();
             if(DomainObject.exists(context, issueId)) {
            	 issue.put(DomainConstants.SELECT_ID,issueId);
             issues.add(issue);
             } else {
            	 //System.out.println("Issue id doesn't exist..." + issueId);
             }
         }
         return issues;
    }
    ...
    /**
     * This method is executed to check for isLastVersion (replacement for V_isLastVersion deprecated attribute) for VPMReference objects.
     *
     * @param context the eMatrix <code>Context</code> object
     * @param args holds follwing arguments
     *            objectId- Object id of the document
    ...
    public boolean isLastVersion(Context context, String[] args) throws Exception
    {
        boolean retVal=false;
        try
        {
            ...
            retVal = ProgramCentralUtil.isLastVersion(context, objectId);
        } catch (Exception ex) {
            DebugUtil.debug("isLastVersion "+ ex.getMessage());
            throw ex;
        }
        finally
        {
