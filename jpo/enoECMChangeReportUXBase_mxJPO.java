...
import com.dassault_systemes.enovia.changeorder.interfaces.IChangeOrderServices.ChangeIdCardENUM;
...
public class enoECMChangeReportUXBase_mxJPO extends emxDomainObject_mxJPO {
     ...
     **
     */
    public enoECMChangeReportUXBase_mxJPO(Context context, String[] args) throws Exception {
        super(context, args);
    ...
    public String generateCOHtmlSummaryReport(Context context, String args[]) throws Exception {
        ...
        try {
        	...
        	boolean bIsChangeRequestType = domChange.isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST);
...
/*
			...
			if (bIsChangeRequestType) {
                summaryReport.append("<table width=\"100%\"><tr><td class=\"subhead\"><h2>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Command.AffectedItems") + "</h2></td></tr></table>");
                summaryReport.append(getCRAffectedItemsSummaryDetails(context, args));
            }

            summaryReport.append("<table width=\"100%\"><tr><td class=\"subhead\"><h2>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Command.Attachments") + "</h2></td></tr></table>");
            ...
            if (bIsChangeRequestType) {
                ...
                summaryReport.append(getChangeOrdersOfChangeRequest(context, args));

                summaryReport.append("<table width=\"100%\"><tr><td class=\"subhead\"><h2>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Command.ChangeActions") + "</h2></td></tr></table>");
                summaryReport.append(getChangeActionsOfChangeRequest(context, args));
            }
    ...
    public String getChangeOrders(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            while (objItr.hasNext()) {
                ...
                strChangeCoordinator = ChangeUXUtil.isNullOrEmpty(strChangeCoordinator) ? "" : strChangeCoordinator;
                imgRefType = UINavigatorUtil.getTypeIconProperty(context, (String) ecrMap.get(SELECT_TYPE));
                if (imgRefType == null || imgRefType.length() == 0) {
                    imgRefType = "iconSmallDefault.gif";
                }
                ...
                if (tempObj instanceof StringList) {
                    ...
                    caName = ChangeUXUtil.isNullOrEmpty(caName) ? "" : caName;
                    sbChangeOrders.append(caName);
                }
...
/*
    public String getCOAffectedItemsSummaryDetails(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            Map partMap = null;
            HashMap < String, String > requestChangeRangeMap = getRequestChangeRangeMap(context);
            ...
            while (proposedChangeItr.hasNext()) {
                ...
                String partRequestedChange = null;
                ...
                if (requestChangeRangeMap != null && !requestChangeRangeMap.isEmpty()) {
                    if (partMap.get(DomainConstants.ATTRIBUTE_REQUESTED_CHANGE) != null) {
                        partRequestedChange = requestChangeRangeMap.get(((String) partMap.get(DomainConstants.ATTRIBUTE_REQUESTED_CHANGE)).trim());
                    }

                }

                if (partRequestedChange == null) {
                    partRequestedChange = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(),
                        "emxFramework.Range.Requested_Change." + ((String)(String) partMap.get(DomainConstants.ATTRIBUTE_REQUESTED_CHANGE)).replaceAll(" ", "_"));
                }
                ...
                newParts.append("<td>" + EnoviaResourceBundle.getStateI18NString(context, (String) partMap.get(SELECT_POLICY), (String) partMap.get(SELECT_CURRENT), strLanguage) + "</td>");
                newParts.append("<td>" + partRequestedChange + "</td>");
                newParts.append("</tr>");
...
*/
    /*Create a Dictionary over range-values of Field-Choice values and Field-Display values*/
    public HashMap < String, String > getRequestChangeRangeMap(Context context) throws MatrixException {
        HashMap < String, String > requestedChangeMap = null;

        HashMap reqChangeDescrLists = JPO.invoke(context, "enoECMChangeAction", null, "displayRequestedChangeRangeValues", new String[0], HashMap.class);

        StringList listChoices = (StringList) reqChangeDescrLists.get("field_choices");
        StringList listDispChoices = (StringList) reqChangeDescrLists.get("field_display_choices");

        if (listChoices != null && !listChoices.isEmpty() &&
            listDispChoices != null && !listDispChoices.isEmpty() && listChoices.size() == listDispChoices.size()) {
            requestedChangeMap = new HashMap < String, String > ();
            for (int idx = 0; idx < listChoices.size(); ++idx) {
                requestedChangeMap.put(listChoices.get(idx).trim(), listDispChoices.get(idx).trim());
            }
        }
    ...
    public String getCABasicInfo(Context context, String args[]) throws Exception {
		...
		String ATTRIBUTE_TITLE = PropertyUtil.getSchemaProperty(context, "attribute_Title");
		String orgTitle = "organization.property["+ATTRIBUTE_TITLE+"].value";
        ...
        objectSelects.add(orgTitle);
        objectSelects.add(ChangeConstants.SELECT_INTERFACE_CHANGE_ON_HOLD);

		//DomainObject.MULTI_VALUE_LIST.add("from[" + sRelTechAssignee + "].to.name");
		//DomainObject.MULTI_VALUE_LIST.add("from[" + sRelChangeReviewer + "].to.name");
        //Map attributeMap = getInfo(context, objectSelects);
		//DomainObject.MULTI_VALUE_LIST.remove("from[" + sRelTechAssignee + "].to.name");
		//DomainObject.MULTI_VALUE_LIST.remove("from[" + sRelChangeReviewer + "].to.name");
		StringList lstMultiValueList = new StringList();
		lstMultiValueList.add("from[" + sRelTechAssignee + "].to.name");
        lstMultiValueList.add("from[" + sRelChangeReviewer + "].to.name");
		Map attributeMap = getInfo(context, objectSelects, lstMultiValueList);

        String attrName = null;
        String attrValue = null;
        String sCurrentState = (String) attributeMap.get(SELECT_CURRENT);
        String sIsCAOnHold = (String)attributeMap.get(ChangeConstants.SELECT_INTERFACE_CHANGE_ON_HOLD);
        ...
        if("TRUE".equalsIgnoreCase(sIsCAOnHold))
            returnString.append("<tr><td class=\"label\">"+EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(),"EnterpriseChangeMgt.Label.State")+":</td><td class=\"inputField\">"+i18nNow.getStateI18NString((String)attributeMap.get(SELECT_POLICY),(String)attributeMap.get(SELECT_CURRENT),strLanguage)+"["+ EnoviaResourceBundle.getProperty(context, SUITE_KEY,"EnterpriseChangeMgt.Label.hold",strLanguage) +"]</td></tr>");
        else
        ...
        returnString.append("<td><table width=\"100%\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" >");
        returnString.append("<tr><td class=\"label\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Common.Description") + ":</td><td class=\"inputField\"><pre>" + XSSUtil.encodeForHTML(context, ecoDesc) + "&nbsp;</pre></td></tr>");
        ...
        while (ecoAttrListItr.hasNext()) {
            ...
            if (!EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.Originator").equals(attrName)) {
				...
				Object data = attrMap.get(attrName);
	    		if(data!= null && data instanceof StringList){
	    			StringList attrValueList = (StringList) data;
	    			returnString.append("<td class=\"inputField\">");
	    			for(int index=0;index<attrValueList.size();index++) {
	    				String attrValueFromList = (String)attrValueList.get(index);
	    				returnString.append(XSSUtil.encodeForHTML(context, i18nNow.getRangeI18NString(attrName, attrValueFromList, strLanguage)));
	    				if(!((attrValueList.size()-1)==index)){
                ...
                returnString.append("<td class=\"inputField\">" + XSSUtil.encodeForHTML(context, i18nNow.getRangeI18NString(attrName, attrValue, strLanguage)) + "</td>");
				}
            }
        }
        returnString.append("</tr>");
...
*/
		StringBuffer techAssignee = new StringBuffer();
		StringBuffer techSrAssignee = new StringBuffer();
		StringList techAssignees = (StringList) attributeMap.get("from[" + sRelTechAssignee + "].to.name");
		StringList techSrAssignees = (StringList) attributeMap.get("from[" + sRelChangeReviewer + "].to.name");
		
		if(techAssignees != null){
			for(int j = 0; j < techAssignees.size(); j++)
			{
				techAssignee.append(PersonUtil.getFullName(context, techAssignees.get(j)));
				techAssignee.append("<BR>");
			}
		}
		
		if(techSrAssignees != null){
			for(int j = 0; j < techSrAssignees.size(); j++)
			{
				techSrAssignee.append(PersonUtil.getFullName(context, techSrAssignees.get(j)));
				techSrAssignee.append("<BR>");
			}
		}
        returnString.append("<tr><td class=\"label\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.ResponsibleOrganisation") + ":</td><td class=\"inputField\">" + (String) attributeMap.get(orgTitle) + "</td></tr>");
    ...
    throws Exception {
        ...
        try {
            ...
            while (objItr.hasNext()) {
                ...
                String sReqChangeVal = null;
                String sAttrReqChange = PropertyUtil.getSchemaProperty(context,
                    "attribute_RequestedChange");

                if (requestChangeRangeMap != null && !requestChangeRangeMap.isEmpty()) {
                    if (proposedChangeMap.get(sAttrReqChange) != null) {
                        sReqChangeVal = requestChangeRangeMap.get(((String) proposedChangeMap.get(sAttrReqChange)).trim());
                    }
                }

                if (sReqChangeVal == null) {
                    sReqChangeVal = FrameworkUtil.findAndReplace((String) proposedChangeMap.get(sAttrReqChange), " ", "_");
                    sReqChangeVal = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource", context.getLocale(), "emxFramework.Range.Requested_Change." + sReqChangeVal);
                }
    ...
    public String getCOBasicInfo(Context context, String args[]) throws Exception {
		...
		String sRelChangeOrder = PropertyUtil.getSchemaProperty(context, "relationship_ChangeImplementation");
        //String sRelResponsibleOrganization = SELECT_ORGANIZATION;
		String orgTitle = "organization.property["+ATTRIBUTE_TITLE+"].value";
        ...
        boolean bIsChangeRequestType = isKindOf(context, ChangeConstants.TYPE_CHANGE_REQUEST);
        String strExcludeAttributes = null;
        
        //For new informed users paradigm
        List<String> lsInformedUsers = new ArrayList<String>();
        if(bIsChangeRequestType) {
        	strExcludeAttributes = EnoviaResourceBundle.getProperty(context, "EnterpriseChangeMgt.ChangeRequest.SummaryReport.ExcludeAttributes");
        	
        	ChangeRequest changeRequest = new ChangeRequest(objectId);
        	lsInformedUsers = changeRequest.getFollowers(context);
        }else {
			IChangeOrderServices changeOrderFactory = ChangeOrderFactory.createChangeOrderFactory();
		    IChangeOrder iChangeOrder = changeOrderFactory.retrieveChangeOrderFromDatabase(context, this);
		    lsInformedUsers = iChangeOrder.getFollowers(context);
        }
        ...
        while (ecoAttrListItr.hasNext()) {
            ...
            returnString.append("<tr>");
            if (!EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.Originator").equals(attrName) && !(UIUtil.isNotNullAndNotEmpty(strExcludeAttributes) && strExcludeAttributes.contains(attrName))) {
		...
		StringList relSelectStmts = new StringList("attribute["+ATTRIBUTE_ROUTE_BASE_PURPOSE+"]");

        mapRouteTemplate = getRelatedObjects(context,
            DomainConstants.RELATIONSHIP_OBJECT_ROUTE, DomainConstants.TYPE_ROUTE_TEMPLATE,
            selectStmts, relSelectStmts, false, true, (short) 1, null, null,0);
        ...
        if (isKindOf(context, ChangeConstants.TYPE_CHANGE_ORDER)) {
			...
			*/
			...
			StringList strTempList = getInfoList(context, "to[" + sRelChangeOrder + "].from.name");
			String strChange = strTempList.join(",");
		...
		//Commented for IR-885174
        //returnString.append("<tr><td class=\"label\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.Policy") + "</strong>:&nbsp;</td><td class=\"inputField\">" + sPolicy + "</td></tr>");

        if(sPolicy.equals(ChangeConstants.FORMAL_CHANGE))
        ...
        if(lsInformedUsers.size() > 0) {
        	returnString.append("<tr><td class=\"label\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.FollowerList") + "</strong>:&nbsp;</td>");
        	returnString.append("<td class=\"inputField\">");
        	for(int i=0 ; i<lsInformedUsers.size(); i++ ) {
        		String strInformedUser = lsInformedUsers.get(i);
        		String strInformedUserFullName = PersonUtil.getFullName(context, strInformedUser);       		
        		returnString.append(XSSUtil.encodeForHTML(context, strInformedUserFullName));
        		if(!((lsInformedUsers.size()-1)==i))
        			returnString.append("<br/>");
        	}
        ...
        try {
            ...
            if (!COInfoList.isEmpty()) {
                ...
                while (sItr.hasNext()) {
                    ...
                    if (!mxType.isOfParentType(context, type, ChangeConstants.TYPE_CHANGE_PROXY)) {

                        sbEditIcon.append("<a href=\"javaScript:emxTableColumnLinkClick('");
                        sbEditIcon.append("../enterprisechangemgtapp/emxECMSummaryReportFS.jsp?objectId=");
                        ...
                        sbEditIcon.append("\"" + XSSUtil.encodeForXML(context, strSummaryReport) + "\"");
                        sbEditIcon.append("/></a>");
                        //String strGeneratePDF = FrameworkProperties.getProperty(context, "EnterpriseChangeMgt.Change.ViewPdfSummary");

                        if ("true".equalsIgnoreCase(strGeneratePDF)) {
                            sbEditIcon.append("<a href=\"javaScript:emxTableColumnLinkClick('");
                            //sbEditIcon.append("../enterprisechangemgtapp/emxECMPDFSummaryReportFS.jsp?objectId=");
                            sbEditIcon.append("../enterprisechangemgtapp/emxECMPdfSummaryReportFS.jsp?objectId=");
                            sbEditIcon.append(XSSUtil.encodeForHTMLAttribute(context, objectId));
                            sbEditIcon.append("&amp;suiteKey=EnterpriseChangeMgt");
    ...
    /**
     * Get affected items of Change Request for Summary Report
     *
     * @param context the eMatrix <code>Context</code> object.
     * @param args holds objectId.
     * @return String.
     * @throws Exception If the operation fails.
     */    
    public String getCRAffectedItemsSummaryDetails(Context context, String[] args) throws Exception {
    	StringBuffer sbAffectedItems = new StringBuffer();
        try {
            String objectId = args[0];           
            String strLanguage = context.getSession().getLanguage();
    		...
    		List<IAffectedItem> lsAffectedItem = iChangeRequest.getAffectedItems(context, true);

    		sbAffectedItems.append("<table width=\"100%\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" >");
    		sbAffectedItems.append("<tr>");
    		sbAffectedItems.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.NAME") + "</th>");
    		sbAffectedItems.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.TYPE") + "</th>");
    		sbAffectedItems.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.REVISION") + "</th>");
    		sbAffectedItems.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.State") + "</th>");
    		sbAffectedItems.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.Owner") + "</th>");
    		sbAffectedItems.append("</tr>");

    		StringList slAffectedItems = new StringList();
			int lsAffectedItemSize = lsAffectedItem.size();
			for(int i=0 ; i<lsAffectedItemSize; i++){
				IAffectedItem iAffectedItem = lsAffectedItem.get(i);
				slAffectedItems.add(iAffectedItem.getPhysicalId());
			}
			...
			MapList mlAffectedItemInfo = DomainObject.getInfo(context, slAffectedItems.toStringArray(), slObjSelect);
            Iterator itrAffectedItemInfo = mlAffectedItemInfo.iterator();
            while (itrAffectedItemInfo.hasNext()) {
                Map mpAffectedItemInfo = (Map) itrAffectedItemInfo.next();
                String strType = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_TYPE);
                String strName = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_NAME);
                String strRevision = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_REVISION);
                String strPolicy = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_POLICY);
                String strCurrent = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_CURRENT);
                String strOwner = (String) mpAffectedItemInfo.get(DomainConstants.SELECT_OWNER);
                String strOwnerFullName = PersonUtil.getFullName(context, strOwner);
                
                String imgAffectedItemType = UINavigatorUtil.getTypeIconProperty(context, strType);

               if (UIUtil.isNullOrEmpty(imgAffectedItemType)) {
                	imgAffectedItemType = "iconSmallDefault.gif";
                }
                
                sbAffectedItems.append("<tr>");
                sbAffectedItems.append("<td><img src=\"../common/images/" + imgAffectedItemType + "\" border=\"0\" alt=\"*\">&nbsp;" + XSSUtil.encodeForHTML(context, strName) + "&nbsp;</td>");
                sbAffectedItems.append("<td>" + EnoviaResourceBundle.getTypeI18NString(context, strType, strLanguage) + "</td>");
                sbAffectedItems.append("<td>" + strRevision + "</td>");
                sbAffectedItems.append("<td>" + EnoviaResourceBundle.getStateI18NString(context, strPolicy, strCurrent, strLanguage) + "</td>");
                sbAffectedItems.append("<td>" + XSSUtil.encodeForHTML(context, strOwnerFullName) + "</td>");
                sbAffectedItems.append("</tr>");                
            } 

            if (lsAffectedItemSize == 0) {
            	sbAffectedItems.append("<tr><td colspan=\"12\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Change.NoAffectedItemsConnected") + "</td></tr>");
            }
        } catch (FrameworkException Ex) {
            throw Ex;
        }

        sbAffectedItems.append("</table>");
        return sbAffectedItems.toString();
    }

    /**
     * Get change orders of Change Request for Summary Report
     *
     * @param context the eMatrix <code>Context</code> object.
     * @param args holds objectId.
     * @return String.
     * @throws Exception If the operation fails.
     */     
    public String getChangeOrdersOfChangeRequest(Context context, String[] args) throws Exception {
        StringBuffer sbChangeOrders = new StringBuffer();
        try {
            String strLanguage = context.getSession().getLanguage();
            String strChangeRequestId = args[0];
    		...
    		IChangeRequestServices changeRequestFactory = ChangeRequestFactory.createChangeRequestFactory();
    		IChangeRequest iChangeRequest = changeRequestFactory.retrieveChangeRequestFromDatabase(context, domChange);  
    		List<String> lsChangeOrderId = iChangeRequest.getImplementingChangeOrderIdentifiers(context);
    		
			List<ChangeIdCardENUM> changeIdCardSelectables = new ArrayList<ChangeIdCardENUM>();
	        changeIdCardSelectables.add(ChangeIdCardENUM.NAMESELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.TYPESELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.POLICYSELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.OWNERSELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.CURRENTSELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.COTITLESELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.CHANGECOORDINATORPERSONSELECTABLE);
			
	        Map<String, BusinessObjectWithSelect> mpChangeInfo = changeOrderFactory.getChangeIdCard(context, lsChangeOrderId, changeIdCardSelectables);
	        
            sbChangeOrders.append("<table width=\"100%\" border=\"0\" cellpadding=\"3\" cellspacing=\"0\" >");
            sbChangeOrders.append("<tr>");
            sbChangeOrders.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.NAME") + "</th>");
            sbChangeOrders.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.Title") + "</th>");
            sbChangeOrders.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Label.OWNER") + "</th>");
            sbChangeOrders.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.ColumnHeader.ChangeCoordinator") + "</th>");
            sbChangeOrders.append("<th>" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Common.State") + "</th>");
            sbChangeOrders.append("</tr>");
		    ...
		    for(String key : setMpChangeKeys) {
		    	...
		    	String strType = busSelectData.getSelectData(ChangeIdCardENUM.TYPESELECTABLE.toString());
		    	String strName = busSelectData.getSelectData(ChangeIdCardENUM.NAMESELECTABLE.toString());
		    	String strPolicy = busSelectData.getSelectData(ChangeIdCardENUM.POLICYSELECTABLE.toString());
		    	String strCurrent = busSelectData.getSelectData(ChangeIdCardENUM.CURRENTSELECTABLE.toString());
		    	String strTitle = busSelectData.getSelectData(ChangeIdCardENUM.COTITLESELECTABLE.toString());
		    	String strCoordinator = busSelectData.getSelectData(ChangeIdCardENUM.CHANGECOORDINATORPERSONSELECTABLE.toString());
		    	String strCoordinatorFullName = PersonUtil.getFullName(context, strCoordinator);
		    	String strOwner= busSelectData.getSelectData(ChangeIdCardENUM.OWNERSELECTABLE.toString());
		    	String strOwnerFullName = PersonUtil.getFullName(context, strOwner);

                String imgChangeOrderType = UINavigatorUtil.getTypeIconProperty(context, strType);
                if (UIUtil.isNullOrEmpty(imgChangeOrderType)) {
                	imgChangeOrderType = "iconSmallDefault.gif";
                }

                sbChangeOrders.append("<tr>");
                sbChangeOrders.append("<td><img src=\"../common/images/" + imgChangeOrderType + "\" border=\"0\" alt=\"*\">&nbsp;" + XSSUtil.encodeForHTML(context, strName) + "&nbsp;</td>");
                sbChangeOrders.append("<td>" + XSSUtil.encodeForHTML(context, strTitle) + "&nbsp;</td>");
                sbChangeOrders.append("<td>" + XSSUtil.encodeForHTML(context, strOwnerFullName) + "&nbsp;</td>");
                sbChangeOrders.append("<td>" + XSSUtil.encodeForHTML(context, strCoordinatorFullName) + "&nbsp;</td>");
                sbChangeOrders.append("<td>" + i18nNow.getStateI18NString(strPolicy, strCurrent, strLanguage) + "&nbsp;</td>");
                sbChangeOrders.append("</tr>");
            }
            if (lsChangeOrderId.size() == 0) {
                sbChangeOrders.append("<tr><td colspan=\"5\">" + EnoviaResourceBundle.getProperty(context, "emxEnterpriseChangeMgtStringResource", context.getLocale(), "EnterpriseChangeMgt.Change.NoChangeOrderConnected") + "</td></tr>");
            }
        } catch (FrameworkException Ex) {
            throw Ex;
    ...
    public String getChangeActionsOfChangeRequest(Context context, String[] args) throws Exception {
        ...
        try {
    		...
    		IChangeRequestServices changeRequestFactory = ChangeRequestFactory.createChangeRequestFactory();
    		IChangeRequest iChangeRequest = changeRequestFactory.retrieveChangeRequestFromDatabase(context, domChange);  
    		List<String> lsChangeActionId = iChangeRequest.getImplementingChangeActionIdentifiers(context);
	        ...
	        changeIdCardSelectables.add(ChangeIdCardENUM.OWNERSELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.CURRENTSELECTABLE);
	        changeIdCardSelectables.add(ChangeIdCardENUM.CATITLESELECTABLE);
		    ...
		    for(String key : setMpChangeKeys) {
		    	...
		    	String strPolicy = busSelectData.getSelectData(ChangeIdCardENUM.POLICYSELECTABLE.toString());
		    	String strCurrent = busSelectData.getSelectData(ChangeIdCardENUM.CURRENTSELECTABLE.toString());
		    	String strTitle = busSelectData.getSelectData(ChangeIdCardENUM.CATITLESELECTABLE.toString());
		    	String strOwner= busSelectData.getSelectData(ChangeIdCardENUM.OWNERSELECTABLE.toString());
		    	String strOwnerFullName = PersonUtil.getFullName(context, strOwner);
