...
// IR-043921V6R2011 - Changes END
            ...
            else
            {
                ...
                if(toList.size() >= 1 && ccList.size() >= 1) {
					...
					MailUtil.sendMessage(context, toList, ccList, null, strSubject, strBody + strComments, new StringList(strTaskId),"true".equalsIgnoreCase(mailEnabled));
						 ...
						 String SELECT_ROUTE_TASK_PRIORITY = "attribute[" + PropertyUtil.getSchemaProperty(context, "attribute_RouteTaskPriority") + "]";
	                     ...
	                     taskSelectList.add(SELECT_ROUTE_TASK_PRIORITY);
					taskSelectList.add(DomainConstants.SELECT_PHYSICAL_ID);
					...
					if(!isGroupRoleIncluded) {
			            ...
			            taskAndRouteInfo.put(RouteTaskNotification.ROUTE_TASK_PRIORITY, (String) taskMap.get(SELECT_ROUTE_TASK_PRIORITY));
    ...
    public  String createRouteFromTemplateForCA(Context context, String[] args) throws Exception {
    	...
    	boolean startRoute = true;
    	if(params.containsKey("startRoute")) {
    		try {
    			startRoute = Boolean.valueOf((Boolean) params.get("startRoute"));
    		}catch(Exception ex) {}
    	}
    	Map objectRouteRelAttributes = (Map) params.get("objectRouteRelAttributes"); 
    	return addApproverTaskFromTemplate(context, strObjectId, strState, strRouteTemplateId, true,Boolean.parseBoolean(isTrue),routeCompletionAction,contextProject,contextOrg,objectRouteRelAttributes, startRoute);
    }
    ...
    public  String addApproverTaskFromTemplate(Context context, String contextObjectId, String contextObjectState, String routeTemplateId, boolean forceTemplate,boolean sendMail, String routeCompletionAction,String contextProject, String contextOrg, Map objectRouteRelAttributes ) throws Exception {
    	return addApproverTaskFromTemplate( context,  contextObjectId,  contextObjectState,  routeTemplateId,  forceTemplate, sendMail,  routeCompletionAction, contextProject, contextOrg, objectRouteRelAttributes, true );
    }
     ...
     * @throws Exception
     */
    public  String addApproverTaskFromTemplate(Context context, String contextObjectId, String contextObjectState, String routeTemplateId, boolean forceTemplate,boolean sendMail, String routeCompletionAction,String contextProject, String contextOrg, Map objectRouteRelAttributes, boolean startRoute) throws Exception {
        try {
            //int nReturnCode = 0;
            ContextUtil.startTransaction(context, true);
...
//          For Bug 347000
            ...
            slBusSelect.add("state");

            DomainObject dmoParentObject = new DomainObject(contextObjectId);
            Hashtable mapObjectInfo            = (Hashtable) dmoParentObject.getInfo(context, slBusSelect);
            ...
            String strPhysicalId = (String)mapObjectInfo.get(DomainObject.SELECT_PHYSICAL_ID);
            StringList states = (StringList)mapObjectInfo.get("state");
         	String targetState = "";
         	if(UIUtil.isNotNullAndNotEmpty(strCURRENT)) {
         		int indexOfCurrentState = states.indexOf(strCURRENT);
            ...
            String routeTitle = (String)routeTemplateMap.get(DomainConstants.SELECT_ATTRIBUTE_TITLE);
            ...
            if (mlRoutes.size() == 0 || "True".equalsIgnoreCase(sRouteChooseUserGroup)) {
    			...
    			if(context.getUser().equals(userAgentName)) {
    				userAgentName = PropertyUtil.getGlobalRPEValue(context,"APPLIES_TO_CONTENT_OWNER");
    				if(!UIUtil.isNotNullAndNotEmpty(userAgentName)) {
    					userAgentName = PropertyUtil.getGlobalRPEValue(context,ContextUtil.MX_LOGGED_IN_USER_NAME);
    				}
    			}else {
    				userAgentName = context.getUser();
    			}
                com.matrixone.apps.common.Person objPerson = com.matrixone.apps.common.Person.getPerson(context,userAgentName);
                ...
                strRouteName = FrameworkUtil.autoName(context,
                                                      ...
                                                      true,
                                                      true);
                String contextVault = context.getVault().getName();
                String vault = null;
                if("ADMINISTRATION".equals(contextVault)) {
                	vault = dmoParentObject.getInfo(context, DomainConstants.SELECT_VAULT);
                }
          
                route.createObject(context, DomainConstants.TYPE_ROUTE, strRouteName, null, DomainObject.POLICY_ROUTE, vault);
                ...
                for (Iterator itrTasks = mlTasks.iterator(); itrTasks.hasNext();) {
                    ...
                    String currentState = (String)mapTask.get(DomainObject.SELECT_CURRENT);
                 ...
                 // Copy all the attributes from route template Route Node relationship to Route
                    ...
                    if((DomainObject.TYPE_ROUTE_TASK_USER).equals(strUserType)){
                    	...
                    	if("TRUE".equalsIgnoreCase(routeOwnerChoice)) {
    						toObject = new DomainObject(PersonUtil.getPersonObjectID(context, userAgentName));
    					}else {
                        //Normal task
                ...
                try {
                    ...
                    boolean accessRuleEvaluation = true;
		         	if(UIUtil.isNotNullAndNotEmpty(targetState) && UIUtil.isNotNullAndNotEmpty(strCURRENT) && "true".equalsIgnoreCase(FrameworkUtil.getChangeMaturityControlValue(context))) {
		         		accessRuleEvaluation = AccessControlUtility.evaluatePromoteAccessRules(context, strPhysicalId, strCURRENT, targetState);
		         	}
					if("true".equalsIgnoreCase(hasPromoteAccess) && Route.isStateBlockingAllowed(context, strTypeName) && !"true".equalsIgnoreCase(isPLMEntity) && accessRuleEvaluation){						  
                ...
                // Start the route if this is the current state of the object
                if (strCurrentStateOfObject.equals(contextObjectState) && startRoute) {
					if (routeNodeChooseUGFlag) {
			...
			if(routeNodeChooseUGFlag) {
				RouteTaskNotification notif = RouteTaskNotification.getInstance(context, "");
				notif.sendUserGroupRouteNotStarted3DNotification(context,(String)context.getUser());
			}
                    ...
                    //START BUG 346838
						...
						if ("Stopped".equals(strCurrentRouteStatus)) {
							...
							/* if(strRouteOwner.equals(contextUser)){
							 ...
							 if(UIUtil.isNotNullAndNotEmpty(strAttrRouteAction) && "Comment".equalsIgnoreCase(strAttrRouteAction)) {
								 vecResult.add(STRING_AWAITING_COMMENT);
							 }	else  if(UIUtil.isNotNullAndNotEmpty(strAttrRouteAction) && "Approve".equalsIgnoreCase(strAttrRouteAction)) {
							vecResult.add(STRING_AWAITING_APPROVAL);
						}
                        	//}
                         }
