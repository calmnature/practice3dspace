 ...
 *  @quickreview KIE1 ZUD 16:11:15 : IR-484273-3DEXPERIENCER2018xR419-FUN055836: Parameter Evaluation Curve is KO with preferences field selection.
 ...
 *  @quickreview HAT1 ZUD 17:06:08 : IR-510765-3DEXPERIENCER2018x: R419-STP: Use Case and Test Case Properties Page is  Not Editable With Double Click. Method added: runJavascriptLoader(). 
 *  @quickreview KIE1 ZUD 17:08:23 : IR-542976-3DEXPERIENCER2018x: R419-STP: Parameter value is not getting copied in Test Exicution copied parameters. 
 *  @quickreview QYG  ZUD 17:12:09 : IR-561869-3DEXPERIENCER2018x: Wrong user id is shown as acting on a Requirement Spec in its history.
 *  @quickreview KIE1 ZUD 17:12:11 : IR-553171-3DEXPERIENCER2018x: Test Case Parameter Last Measured Value does not get sorted or listed as per Parameter
*/
...
//End of Add by Vibhu, Enovia MatrixOne for Bug 303269 on 28 April 05
     ...
     */
     ...
     /**
      * This method added for JPO To JAR conversion
      * @param context    the eMatrix Context object.
      * @param args       args that top level parent contains type to be sent
      * @return String    the top level parent type
      * @throws Exception if the operation fails
      */
      public static String getParentType(Context context, String[] args)
			throws Exception {
		Map paramMap = (HashMap) JPO.unpackArgs(args);
		String type = (String) paramMap.get("paramString");
		return getParentType(context, type);
	}
        /**
...
//Added by Enovia MatrixOne for Performance Bug- Where used, on 23 Jan 06
    ...
    DomainObject domainObject = newInstance(context,strObjectId);
           ...
           /*The code below fetches the to side objects(till level 1) which are
            ...
            StringList slObjSelListCopy = new StringList();
 ...
 return  (MapList)relObjIdList ;
    ...
    public static Vector getEditIcon(Context context,String[] args) throws Exception
        {
                ...
                while(objectListItr.hasNext())
                {
                    ...
                    // ++ KIE1 added for Name non editable form for Requirement Specification 
                    if(dObj.isKindOf(context, "Requirement Specification"))
                    {
                    	strBaseType = strType;
                    }
                    //--KIE1
                          ...
                          strReportFormat.equals("")==false))
                        {
                             ...
                             /*End of Modify:Ramandeep,Enovia MatrixOne Bug#300035 3/11/2005*/
                              ...
                              else if (strType.equalsIgnoreCase(strTypeInclusionRule))
                              {
                               ...
                               StringList lstRuleSelects = new StringList();
    ...
    public static boolean boolContains(String strParent,String strPattern)
     ...
     /**
      * This method is added for JPO to JAR Conversion and is used to set the level for all the objects in a
        MapList as seen by the user by ignoring the Intermediate Objects.
      * @param List relObjIdList - contains the parent MapList
      * @param String strIntermediateTypes - comma seperated string of
        intermediate types
      ...
      * @throws Exception if the operation fails
      **/
     public static List setLevelsInMapList(Context context, String[] args)
      throws Exception
      {
    	HashMap programMap = (HashMap) JPO.unpackArgs(args);
 		HashMap paramMap = (HashMap) programMap.get("paramMap");
 		MapList relObjIdList = (MapList) paramMap.get("relObjIdList");
 		String strIntermediateTypes = (String) paramMap.get("strIntermediateTypes");
 		
      return  setLevelsInMapList(relObjIdList, strIntermediateTypes);
      }//end of the method
        /**
          ...
          /* Start  - Added  by Amarpreet Singh 3dPLM for Checking the access to Edit Design Responsibility*/
            try
            {
                if ( (strRDOId==null || strRDOId.equals("")) &&
                    ...
                    /*boolean hasAccessOnProject = ${CLASS:emxProduct}.hasAccessOnProject(context,strRDOId);*/
                    Map mArgs = new HashMap();
                	mArgs.put("paramString",strRDOId);
                	String[] newArgs = JPO.packArgs(mArgs);
                	boolean hasAccessOnProject = (boolean) JPO.invoke(context, "emxProduct", null, "hasAccessOnProject", newArgs, Boolean.class);
          ...
          /* End - Added  by Amarpreet Singh 3dPLM for Checking the access to Edit Design Responsibility*/
            ...
            if (!bHasReadAccess)
            {
                if (strRDOType!=null && !strRDOType.equals("") && !strRDOType.equalsIgnoreCase("null")){
                	...
                	strTypeIcon = UINavigatorUtil.getTypeIconProperty(context,strRDOType);
...
//                sbBuffer.append("OID");
            ...
            if(strRDOName!=null && !strRDOName.equals("") && !strRDOName.equalsIgnoreCase("null")){
                ...
                try
                {
                    ...
                    /*bHasReadAccess = ${CLASS:emxProduct}.hasAccessOnProject(context,strRDOId);*/
                    Map mArgs = new HashMap();
                	mArgs.put("paramString",strRDOId);
                	String[] newArgs = JPO.packArgs(mArgs);
                	bHasReadAccess = (boolean) JPO.invoke(context, "emxProduct", null, "hasAccessOnProject", newArgs, Boolean.class);
                } catch (Exception e) {
                    bHasReadAccess = false;
                }
	...
	public int updateDesignResponsibility(Context context, String[] args)
		...
		HashMap requestMap = (HashMap) programMap.get("requestMap");
		String strMode = ((String[])requestMap.get("mode"))[0];
		...
		lstSelect.add(DomainObject.SELECT_PROJECT);
		lstSelect.add(DomainObject.SELECT_ORGANIZATION);
		lstSelect.add("organization.property[Title].value");
		Map mapPRJORG = domObj.getInfo(context, lstSelect);
		String strObjProject = (String)mapPRJORG.get(DomainObject.SELECT_PROJECT);
		String strObjOrganization = (String)mapPRJORG.get(DomainObject.SELECT_ORGANIZATION);
		String strObjOrganizationTitle = (String)mapPRJORG.get("organization.property[Title].value");
		
		String strNewOrganizationOID = (String) paramMap.get("New Value");
		if (strNewOrganizationOID == null) {
			strNewOrganizationOID = "";
		}
		String strNewProject = "";
		if(requestMap!=null && requestMap.containsKey("CollabSpace")){
			String[] strFieldName =(String[]) requestMap.get("CollabSpace");
			if(strFieldName!=null){
				strNewProject=strFieldName[0];
		...
		DomainObject domObjOrgnization = new DomainObject(
				strNewOrganizationOID);
		String strNewOrganizationName = strNewOrganizationOID;
		boolean isExists=domObjOrgnization.exists(context);// meaninig it is RDO ID and NOT the random name/empty value 
		if(isExists){
			  strNewOrganizationName = domObjOrgnization.getInfo(context,DomainObject.SELECT_NAME);//getting Name from RDO ID
		}
		// If Project and Organization on object is same with selected Project and Organization then to skip setting of Project and Organization.		
		if(strObjProject.equalsIgnoreCase(strNewProject) && (strObjOrganization.equalsIgnoreCase(strNewOrganizationName) || strObjOrganizationTitle.equalsIgnoreCase(strNewOrganizationName))){
			return 0;
		}
		//		
		
		if ("".equals(strNewOrganizationName) && "".equalsIgnoreCase(strNewProject)) 
		{
			...
			domObj.setPrimaryOwnership(context, defaultProj, defaultOrg, false);
		}
		else
		{	
			if("edit".equalsIgnoreCase(strMode)){
			  domObj.setPrimaryOwnership(context, strObjProject, strNewOrganizationName, false);
			}else{
              if(!isExists){//for RDO ID case- we are retrieving Name - else other cases it is always Title , so need to get Name for it
			   String strNameFromTitle=Organization.getCompanyNameFromTitle(context,strNewOrganizationName);//getDesignResponsibilityValue add title for first load and NOT Name
			   if(UIUtil.isNotNullAndNotEmpty(strNameFromTitle)){//if not empty - then it is title to name
					strNewOrganizationName=strNameFromTitle;
			   }
			  }
			  domObj.setPrimaryOwnership(context, strNewProject, strNewOrganizationName, false);
			}
		}
		return 0;
         ...
         if(strNextState.equals(strStateRelease)){
            ...
            if(lstECList!=null && !lstECList.isEmpty()){
                ...
                if(lstECList!=null && !lstECList.isEmpty()){
                    ...
                    /*${CLASS:emxContextUtil}.mqlNotice(context,strErrorMsg);*/
...
//End of Add by Enovia MatrixOne for EC bug on 18 Mar 2005
    ...
    **/
    ...
    public List getNameRev (Context context, String[] args) throws Exception{
        ...
        StringList lstNameRev = new StringList();
...
//                                            + "].from.id");
                ...
                if (EnoviaResourceBundle.getProperty(context,"eServiceSuiteProductLine.ProductBuildAddExisting.DisplayOrphanBuilds")
					...
					String[] strBuildIdArr = new String[lstBuildList.size()];

                    for (int i = 0; i < lstBuildList.size(); i++) {
                        Map productMap = (Map) lstBuildList.get(i);
                        strBuildId = (String) productMap.get("id");
                        strBuildIdArr[i] = strBuildId;
                    }
                    Map returnMapCheck = new HashMap();
                    returnMapCheck = showBuildsInProductContextMultiple(context, strParentModelID, strBuildIdArr);
                    for (int i = 0; i < returnMapCheck.size(); i++) {
                    	String strBuildIdTemp = strBuildIdArr[i];
                    	String returnValueFromMap = (String) returnMapCheck.get(strBuildIdTemp);                    	
                    	if(returnValueFromMap.equalsIgnoreCase("false")) {
                    		tempStrList.add(strBuildIdTemp);
                    	}
                    }
     ...
     */
     ...
     /**
      * This method is added as part of JPO TO JAR Conversion This will return the ID's of all product context IPUs
      * @param context - Matrix context
      * @param strProdId - Product ID
      * @return Stringlist
      * @throws Exception
      */
      public static StringList getContextIPUBuilds(Context context,  String[] strProdIdInArgs) throws Exception {
    	 Map paramMap = (HashMap) JPO.unpackArgs(strProdIdInArgs);
  		 String strProdId = (String) paramMap.get("paramString");
         return getContextIPUBuilds(context,  strProdId);
     }     
    ...
    */
     ...
     * @param context the eMatrix <code>Context</code> object.
     * @param strProdModelInfo contains model Id of the selected product.
     * @param strBuildIdArr contains array of Build Ids
     * @return boolean.
     * @since ProductLine X5
     * @throws Exception if the operation fails.
    */
    private Map showBuildsInProductContextMultiple(Context context, String strProdModelInfo,
            String[] strBuildIdArr) {
        try {
            //DomainObject objBuild = new DomainObject(strBuildId);
            StringList selectables = new StringList(2);
            selectables.add("to["
            ...
            selectables.add("to["
                    ...
                    + "].from.id");
            //Map map = objBuild.getInfo(context, selectables);
            MapList maplistMultipleBuilds = DomainObject.getInfo(context, strBuildIdArr, selectables);
            Map buildMap = new HashMap();
            for(int k=0;k<maplistMultipleBuilds.size();k++){
				Map map = (Map) maplistMultipleBuilds.get(k);
				String strBuildId = strBuildIdArr[k];
				String strProdInfo = (String)map.get("to["
	                    + ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD
	                    + "].from.id");
	            String strBuildModelInfo =(String) map.get("to["
	            ...
	            buildMap.put(strBuildId, returnValue);
			}
            return buildMap;         
     ...
     */
     ...
     /**
		...
		*/
		...
		//Adding this check for checking inter-mapped CompanyId and name and then setting the company id
		if ((strNewCompanyName != null && strNewCompanyName.length() != 0) && (strNewCompanyId != null && strNewCompanyId.length() != 0)) {
			//-----------------------------Getting Company Title------------------------------------------//	        
        	StringList strCompanyDetailsSelects = new StringList(DomainConstants.SELECT_NAME);
        	strCompanyDetailsSelects.add(ProductLineConstants.SELECT_ATTRIBUTE_TITLE);
        	DomainObject companyObject = new DomainObject(strNewCompanyName);
        	if(companyObject.exists(context)) {
        		Map companyObjectMap = companyObject.getInfo(context, strCompanyDetailsSelects);
                String strUpdatedCompanyTitle = (String) companyObjectMap.get(ProductLineConstants.SELECT_ATTRIBUTE_TITLE);
                String strUpdatedCompanyName = (String) companyObjectMap.get(DomainConstants.SELECT_NAME);            
    	        //-----------------------------End of Getting Company Title-----------------------------------//
                //-----------------------------Setting newCompanyId if a match -------------------------------//
                System.out.println("------------------>Just Before interchanging Company ID and CompanyName : strUpdatedCompanyTitle: " + strUpdatedCompanyTitle + " strUpdatedCompanyName: " + strUpdatedCompanyName);
                if (strNewCompanyId.equalsIgnoreCase(strUpdatedCompanyTitle) || strNewCompanyId.equalsIgnoreCase(strUpdatedCompanyName)) { //Here strNewCompanyName has ID and strNewCompanyId has Title/Name
                	System.out.println("------------------>Switching Company ID and CompanyName");
                	strNewCompanyId = strNewCompanyName;
                }
        	}        	
		}
		...
		if (strNewCompanyId == null || strNewCompanyId.length() == 0) {
	        ...
	        //-----------------------------Getting Company Title------------------------------------------//
            DomainObject companyObject = new DomainObject(companyId);
            String strUserCompanyTitle = companyObject.getInfo(context, ProductLineConstants.SELECT_ATTRIBUTE_TITLE);
	        //-----------------------------End of Getting Company Title-----------------------------------//
	        //if(strNewCompanyName.equals(person.getCompany(context).getName())){ 
	        if(strNewCompanyName.equals(strUserCompanyTitle) || strNewCompanyName.equals(person.getCompany(context).getName())){
			// setId(companyId);
		...
		try {
			if (strOldCompanyId == null
				...
				/*bHasReadAccess = ${CLASS:emxProduct}.hasAccessOnProject(context,
						strOldCompanyId);*/
			Map mArgs = new HashMap();
        	mArgs.put("paramString",strOldCompanyId);
        	String[] newArgs = JPO.packArgs(mArgs);
        	bHasReadAccess = (boolean) JPO.invoke(context, "emxProduct", null, "hasAccessOnProject", newArgs, Boolean.class);
			}
		} catch (Exception e) {
...
/**
    ...
    public void autoPromoteMajorCollaborativeMode(Context context, String args[]) throws Exception {
        ...
        if ("true".equalsIgnoreCase(autoPrmote)) {
            ...
            boolean runAsSuperUser = false; //IR-561869-3DEXPERIENCER2018x
 ...
 // ++ KIE1 : HL Parameter under Test Execution
   	public StringList getParameterLastMeasuredValue(Context iContext, String[] iArgs) throws Exception
   	{
		...
		MapList paraList 		    = (MapList) programMap.get("objectList");
        ...
        for(int i =0 ; i<paraList.size(); i++)
        {
        	...
        	if(returnMap != null && returnMap.size() > 0 && returnMap.containsKey(mapforsort.get("id"))){
        		strReturnValue = returnMap.get(mapforsort.get("id")).toString();
        	}
        	returnList.add(strReturnValue);
        }
		return returnList;
				...
				for(int j = 0; j < paraList.size(); j++)
				{
					...
					if(parentParamid.equals(paramRole_split[0]))
					{
						...
						else
				    	{
							...
							if(!strParameterValue.equals("null"))
							{
								// added for valuation type multi values with dropdown on Test Case context
								if(valuationType.equals(PLMParm_ValuationType.MULTI))
								{
									String multiVal[] = strParameterValue.split(",");
				        			String displayValue ="<select>";
				        			...
				        			{
				        				DivDecoratedText +="<div id='contentCell' class='cke_contents cke_reset'>";
										DivDecoratedText +="<a href=\"javascript:showModalDialog('../productline/PLCParameterUtil.jsp?Mode=getHighChart&amp;objectId="+parentParamid+"&amp;parentOID="+ParentId+"')\">"+displayValue+"</a>";
 	 ...
 	 public String getTestExecutionName(Context context, String[] args) throws Exception
 	 {
		...
		if(lastTestExecutionObjMapList != null && !lastTestExecutionObjMapList.isEmpty())
		{
			...
			for(int i = 0; i<lastTestExecutionObjMapList.size(); i++)
			{
				...
				if(("false".equalsIgnoreCase(ParEvolutionWithTitle) || ParEvolutionWithTitle.equalsIgnoreCase("")) && ("false".equalsIgnoreCase(ParEvolutionWithEndDate) || ParEvolutionWithEndDate.equalsIgnoreCase("")))
				{
    ...
    { 
    	try
    	{
    		...
    		{
    			// IR-654920
    			IKweUnit kweCurrUnit = kweDico.findUnitBySymbol(context, displayUnit);
    			double iUnit =0.0;
    			double MKSVal = 0.0;
    			//Convert Min Value
    			if( 0!=paramValues[0].compareTo(""))
    			{
    				iUnit = Double.parseDouble(paramValues[0]);
    				MKSVal = kweCurrUnit.convert(iUnit);
    				paramValues[0]=""+MKSVal;
    			}
    			// Convert  Value
    			try{
    				if(0!=paramValues[1].compareTo(""))
    				{
    					iUnit = Double.parseDouble(paramValues[1]);    			
    					MKSVal = kweCurrUnit.convert(iUnit);
    					paramValues[1]=""+MKSVal;
    				}
    			}catch(Exception e)
    			{
    				if(valuationType.equals(PLMParm_ValuationType.MULTI)){
    					...
    					{
    						iUnit = Double.parseDouble(multiVal[i]);
    						MKSVal = kweCurrUnit.convert(iUnit);
    						paramValues[1]+= MKSVal +",";
    					}
    				}  
    			...
    			//Convert Max Value
    			if(0!=paramValues[2].compareTo(""))
    			{
    				iUnit = Double.parseDouble(paramValues[2]);
    				MKSVal = kweCurrUnit.convert(iUnit);
    				paramValues[2]=""+MKSVal;
    			}
    		}
    		...
    		if(paraType.equals(ParameterTypes.Boolean)){
    			...
    			if(valuationType.equals(PLMParm_ValuationType.MULTI))
    			{
    				...
    				for (int i=1; i <= multiVal.length; i++)
    					...
    					listValues.InsertItem(i, kweFacto.createString(context, multiVal[i-1].trim()));

    				plmPara.setMultiValues(context, listValues);
    				
    			}else{
    				plmPara.valuate(context, stringValue.trim());	
    			}
    		}else{
        		...
        		// ++setting multivalues for parameter under Test Execution
        		if(valuationType.equals(PLMParm_ValuationType.MULTI))
				{
	        		String[] multiVal = paramValues[1].split(",");
	        		for (int i=1; i <= multiVal.length; i++)
	...
	public  String lastCompletedTestExecutionStatus(Context context, String[] args) 
	{
		...
		try 
		{
			...
			if(testExecutionList != null && testExecutionList.size() > 0)
			{
	...
	/**
	 * Display Default Design Responsibility value in the WebForm for Design Responsibility field
	 * for Copy object case- it should be context object Design Responsibility will be displayed.
	 * 
	 * @param Context context
	 * @param args[] - arguments provided
	 * @return Organization/altowner1 value for context user
	 * @throws Exception
	 * @since R420
	 */
    public static String getDesignResponsibilityValue(Context context, String[] args)
    		throws Exception {
    	String strDesignResponsibility = "";
    	try {
    		HashMap programMap = (HashMap) JPO.unpackArgs(args);
    		HashMap requestMap = (HashMap) programMap.get("requestMap");
    		strDesignResponsibility=PersonUtil.getDefaultOrganization(context, context.getUser());
    		strDesignResponsibility = Organization.getCompanyTitleFromName(context, strDesignResponsibility);
    		
    		
    		String strCopyObjId = (String) requestMap.get("copyObjectId");
    		if (UIUtil.isNotNullAndNotEmpty(strCopyObjId)){ // if copy case.
    			DomainObject objDerivedFrom = new DomainObject(strCopyObjId);
    			//String copyRDO = objDerivedFrom.getInfo(context,"altowner1");
    			String copyRDO = objDerivedFrom.getInfo(context, ProductLineConstants.SELECT_ATTRIBUTE_TITLE);
    			if(UIUtil.isNotNullAndNotEmpty(copyRDO)){
    				strDesignResponsibility=copyRDO;
    			}
    		}
    	...
    	return strDesignResponsibility;
    }
    
    
    /**
     * Get Collab space available. Range function
     * @param context the eMatrix <code>Context</code> object
     * @param args
     *        0 - HashMap containing one Map entry for the key PARAM_MAP
     *          This Map contains the arguments passed to the jsp which called this method.
    ...
    public Map getCollabSpace(Context context, String[] args)
    		throws Exception {
        ...
        /**
     * get Collabspace Title (if empty Collabspace Name) for the object.
     *  
     * @param context
     * @param args
     * @return
     * @throws FrameworkException
     */
    public String getCollabSpaceFieldValue(Context context, String[] args) throws FrameworkException {
    	String strCollabSpaceValue = "";
    	try {
    		HashMap programMap = (HashMap) JPO.unpackArgs(args);
    		HashMap requestMap = (HashMap) programMap.get("requestMap");
    		String objectId = (String) requestMap.get("objectId");
    		...
    		StringList slObjectSelectsList = new StringList();
    		slObjectSelectsList.add("project");
    		slObjectSelectsList.add("project.property[Title].value");
    		Map mapCSDetails =   dObj.getInfo(context,slObjectSelectsList);
    		strCSName=(String)mapCSDetails.get("project");
    		strCSTitle = (String)mapCSDetails.get("project.property[Title].value");
    		if (ProductLineCommon.isNotNull(strCSTitle)) { //with Common Space Collab space has issue in Title/title value
    			strCollabSpaceValue=strCSTitle;
    		}else {
    			strCollabSpaceValue=strCSName;
    		}
    	...
    	return strCollabSpaceValue;
    }
    /**
     * Set Collab space on Object created on project/altowner2.
     * @param context the eMatrix <code>Context</code> object
     * @param args
     *        0 - HashMap containing one Map entry for the key PARAM_MAP
     *          This Map contains the arguments passed to the jsp which called this method.
     ...
     * @since R420
     */
     public int updateCollabSpace(Context context, String[] args)
			throws Exception {
		Map programMap = (HashMap) JPO.unpackArgs(args);
		Map paramMap = (HashMap) programMap.get("paramMap");
		HashMap requestMap = (HashMap) programMap.get("requestMap");
...
// ++ HAT1 ZUD IR-510765-3DEXPERIENCER2018x ++
	/**
	 * On double click on the web form, it will be opened in Edit mode.
	 * @param Context context
	 * @param args[] - arguments provided
	 * @return Organization/altowner1 value for context user
	 * @throws Exception
...
//                "$('.field').dblclick(function() { " +
    ...
    /** This is a wrapper API over the isFrozenState() API which is defined in ProductLineUtil bean.
     *  This is used as Access Function.
     *  
     *  @param context
     ...
     *  @since R421        
     */ 
    public boolean isFrozenState(Context context, String[] args) throws Exception
    {
    	boolean showUXComp = true;
    	...
    	try 
    	{
			...
			if(ProductLineCommon.isNotNull(objectId)){
				// To call isFrozenState() ProductLineUtil bean API
				isFrozenState = ProductLineUtil.isFrozenState(context, objectId);
			}
			if(isFrozenState){
				showUXComp = false;
			}
    ...
    /**This is Access Function which is used to Show or Hide Resource Rules Functionality UX. 
     * Depending on the value of MQL Expression ResourceRuleActivation.
     * And Checks for Mobile Mode And FTR/PLC License.
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression ResourceRuleActivation is Active.
     *                 and if Mobile Mode is Disabled AND User has either of Licenses (FTR or PLC).
     *         false - If the value of the MQL Expression ResourceRuleActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showResourceRulesUX(Context context, String[] args)
    {
    	boolean showResourceRulesUX = false;
    	try 
    	{
    		//Check for MQL expression value.
    		String strExpression = "ResourceRuleActivation";
			String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
			if("Active".equalsIgnoreCase(strExprValue))
			{
				showResourceRulesUX = true;
			}
    		
    		//Check for Mobile Mode.
    		if(showResourceRulesUX){
    			boolean isMobileModeDisabled = isMobileModeDisabled(context, args);
    			showResourceRulesUX = isMobileModeDisabled;
    		}
     ...
     * @return
     *         true  - If the value of the MQL Expression ResourceRuleActivation is Active.
     *                 and if Mobile Mode is Enabled OR User don't have either of Licenses (FTR or PLC).
     *         false - If the value of the MQL Expression ResourceRuleActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showResourceRulesUXMobile(Context context, String[] args)
    {
    	boolean showResourceRulesUX = false;
    	try 
    	{
    		...
    		//Check for Mobile Mode.
    		if(showResourceRulesUX){
    			boolean isMobileModeEnabled = isMobileModeEnabled(context, args);
    			showResourceRulesUX = isMobileModeEnabled;
    		}
    ...
    /**This is Access Function which is used to Show or Hide Product Compatibility Rules Functionality UX. 
     * Depending on the value of MQL Expression PCRActivation.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression PCRActivation is Active.
     *         false - If the value of the MQL Expression PCRActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showPCRulesUX(Context context, String[] args) throws Exception
    {
    	boolean showPCRulesUX = false;
    	try 
    	{
    		String strExpression = "PCRActivation";
			String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
			if("Active".equalsIgnoreCase(strExprValue))
			{
				showPCRulesUX = true;
    	...
    	return showPCRulesUX;
    }
    
    /**This is Access Function which is used to Show or Hide Product Variants Functionality UX. 
     * Depending on the value of MQL Expression ProductVariantActivation and Mobile mode.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression ProductVariantActivation is Active AND mobile mode is disabled.
     *         false - If the value of the MQL Expression ProductVariantActivation is NotActive OR Mobile mode is enabled.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showProductVariantsUX(Context context, String[] args)
    {
    	boolean showProductVariantsUX = false;
    	try 
    	{
    		String strExpression = "ProductVariantActivation";
			String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
			if("Active".equalsIgnoreCase(strExprValue))
			{
				showProductVariantsUX = true;
			}
			
    		//Check for Mobile Mode.
    		if(showProductVariantsUX){
    			boolean isMobileModeDisabled = isMobileModeDisabled(context, args);
    			showProductVariantsUX = isMobileModeDisabled;
    		}
		} 
    	catch (Exception e) 
    	{
     ...
     * 
     * @return
     *         true  - If the value of the MQL Expression ProductVariantActivation is Active AND Mobile Mode is enabled.
     *         false - If the value of the MQL Expression ProductVariantActivation is NotActive OR mobile mode is disabled.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showProductVariantsUXMobile(Context context, String[] args)
    {
    	boolean showProductVariantsUX = false;
    	try 
    	{
    		...
    		if(showProductVariantsUX){
    			boolean isMobileModeEnabled = isMobileModeEnabled(context, args);
    			showProductVariantsUX = isMobileModeEnabled;
    		}
		} 
    	catch (Exception e) 
    	{
    ...
    /**This is Access Function which is used to Show or Hide Manufacturing Features Functionality UX. 
     * Depending on the value of MQL Expression ManufacturingFeatureActivation.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression ManufacturingFeatureActivation is Active.
     *         false - If the value of the MQL Expression ManufacturingFeatureActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showManufacturingFeaturesUX(Context context, String[] args) throws Exception
    {
    	boolean showManufacturingFeaturesUX = false;
    	try 
    	{
    		String strExpression = "ManufacturingFeatureActivation";
			String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
			if("Active".equalsIgnoreCase(strExprValue))
			{
				showManufacturingFeaturesUX = true;
			}
		} 
    	catch (Exception e) 
    	{
    	...
    	return showManufacturingFeaturesUX;
    }
    
    /**This is Access Function which is used to Show or Hide Manufacturing Features Functionality UX. 
     * Depending on the value of MQL Expression ManufacturingFeatureActivation and Mobile Mode.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression ManufacturingFeatureActivation is Active AND Mobile mode is Enabled.
     *         false - If the value of the MQL Expression ManufacturingFeatureActivation is NotActive OR Mobile mode is Disabled.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showMFMobileEnabled(Context context, String[] args) throws Exception
    {
    	boolean bDisplay = false;
    	try 
    	{
    		//get MQL Expression ManufacturingFeatureActivation value
    		boolean showManufacturingFeaturesUX = showManufacturingFeaturesUX(context,args);
			
    		//Check for Mobile Mode.
    		if(showManufacturingFeaturesUX){
    			boolean isMobileModeEnabled = isMobileModeEnabled(context, args);
    			bDisplay = isMobileModeEnabled;
    		}
		} 
    	catch (Exception e) 
    	{
     ...
     * 
     * @return
     *         true  - If the value of the MQL Expression ManufacturingFeatureActivation is Active AND Mobile Mode is Disabled.
     *         false - If the value of the MQL Expression ManufacturingFeatureActivation is NotActive OR Mobile mode is Enabled.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showMFMobileDisabled(Context context, String[] args) throws Exception
    {
    	boolean bDisplay = false;
    	try 
    	{
    ...
    /**This is Access Function which is used to Show or Hide Selection Type field or column on Logical Feature UX. 
     * Depending on the value of MQL Expression LogicalSelectionTypeActivation.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression LogicalSelectionTypeActivation is Active.
     *         false - If the value of the MQL Expression LogicalSelectionTypeActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public boolean showSelectionTypeUX(Context context, String[] args) throws Exception
    {
    	boolean showSelectionTypeUX = false;
    	try 
    	{
    		String strExpression = "LogicalSelectionTypeActivation";
    		String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
    		if("Active".equalsIgnoreCase(strExprValue))
			{
				showSelectionTypeUX = true;
			}
		} 
    	catch (Exception e) 
    	{
    	...
    	return showSelectionTypeUX;
    }
    
    /**This is Access Function which is used to Show or Hide Design in Rule Classification drop down on Create Boolean Compatibility Rule form UX.
     * Also used to show or hide Validate Design Rules functionality UX on Product Configuration > Properties Page. 
     * Depending on the value of MQL Expression DesignRuleActivation.
     * 
     * @param context
     *        ematrix context object
     * @param agrs
     * 
     * @return
     *         true  - If the value of the MQL Expression DesignRuleActivation is Active.
     *         false - If the value of the MQL Expression DesignRuleActivation is NotActive.
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
     */
    public static boolean showDesignRuleUX(Context context, String[] args) throws Exception
    {
    	boolean showDesignRuleUX = false;
    	try 
    	{
    		String strExpression = "DesignRuleActivation";
    		String strExprValue = ProductLineUtil.getExpressionValue(context, strExpression);
			if("Active".equalsIgnoreCase(strExprValue))
			{
				showDesignRuleUX = true;
			}
		} 
    	catch (Exception e) 
    	{
    	...
    	return showDesignRuleUX;
    }

	
	
	 /**This is used to get Revision Value from database
     *
     * @param context
     *        ematrix context object
     * @param args
     *        Type whose revision is to be fetched
     * @return
     *         Value of given policy - 1,2,3,.../A,B,C,...
     * @throws
     *        FrameworkException
     * @since
     *        R421 (R2019x)
	*/
	 public Object getFirstRevision(Context context, String[] args) throws Exception {
    	...
    	if("null".equalsIgnoreCase(strSymbolicPolicy)) {
    		strSymbolicPolicy = (String)requestMap.get("Policy");
    	}
    	strPolicy=strSymbolicPolicy;
    	
    	if(!"null".equalsIgnoreCase(strSymbolicPolicy) && strSymbolicPolicy.startsWith("policy_"))
    	strPolicy = PropertyUtil.getSchemaProperty(context, strSymbolicPolicy);
    	
    	if("null".equalsIgnoreCase(strPolicy) || "".equalsIgnoreCase(strPolicy))
    	{
    		if(!"null".equalsIgnoreCase(strSymbolicType) && strSymbolicType.startsWith("type_"))
    	    	strType = PropertyUtil.getSchemaProperty(context, strSymbolicType);
    			
    		DomainObject obj = new DomainObject();
    		strPolicy = obj.getDefaultPolicy(context, strType);
    	}
	 ...
	 /**This is used to Hide the policy field on PLCreateTestCase form
     *
     * @param context
     *        ematrix context object
	 * @return
    ...
    public boolean isModelDefinitionTransitionEnabled(Context context, String []args) throws Exception {
		return getFTRModelDefinitionTransitionStatus(context).equals("ENABLED");
	}
	
	public boolean isModelDefinitionTransitionDisabled(Context context, String []args) throws Exception {
		return getFTRModelDefinitionTransitionStatus(context).equals("DISABLED");
    ...
    /**This is used to update search criteria 
     *
     * @param context
     *        ematrix context object
	 * @return
	 ...
	 */
    @com.matrixone.apps.framework.ui.ProgramCallable
	public String getFieldQueryForModelBuild(Context context, String[] args) throws Exception {
		try {
			HashMap paramMap = (HashMap) JPO.unpackArgs(args);
			String productId = (String) paramMap.get("objectId");// Model Version ID
			DomainObject domParentProduct = new DomainObject(productId);
			String strParentModelID = domParentProduct.getInfo(context,
                        "to[" + ProductLineConstants.RELATIONSHIP_PRODUCTS
                                + "].from.physicalid");// Model ID
			if(strParentModelID == null || "".equals(strParentModelID)){
                                strParentModelID = domParentProduct
                                .getInfo(context,
                                        "to[" + ProductLineConstants.RELATIONSHIP_MAIN_PRODUCT
			...
			return "TYPES=type_Builds:HAS_PRODUCT_BUILD=FALSE:HAS_PRODUCT_CONFIGURATION_BUILD=FALSE:PLC_MODEL_PID="+strParentModelID;
		} catch (Exception ex) {
            throw new FrameworkException(ex);
		}
	}
