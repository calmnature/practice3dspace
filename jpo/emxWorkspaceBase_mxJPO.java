...
public class emxWorkspaceBase_mxJPO extends emxWorkspaceConstants_mxJPO
{
      ...
      Exception
  {
       ...
       if(objType != null && objType.equals(DomainConstants.TYPE_PROJECT_VAULT)) {
    	   boolean boolChecking = false;
           if(FrameworkUtil.isWsInArchiveState(context, domObj)) {
       			result = boolChecking;
       	   }
       }      
  ...
  @com.matrixone.apps.framework.ui.ProgramCallable
  public Object getLinkWorkspaces(Context context, String[] args)
      throws Exception, MatrixException
  {
      try
      {
          return getMyDeskWorkspace(context,args,"addLinkWorkspace");
      }
      catch (Exception ex)
      {
          throw ex;
    ...
    public Object getMyDeskWorkspace(Context context, String[] args,String sFtr) throws Exception {
    	...
    	if (newSecurityModel != null && "false".equals(newSecurityModel)) {
    		...
    		if (sFtr.equals("Active")) {
            queryTypeWhere = "('" + DomainObject.SELECT_CURRENT + "' == 'Active')";
    		} else if (sFtr.equals("addLinkWorkspace")) {
    			queryTypeWhere = "('" + DomainObject.SELECT_CURRENT + "' == 'Active' || '"+ DomainObject.SELECT_CURRENT + "' == 'Create' )";
    		}
    	}
...
//    context.getSession().putValue("error.message", exp.getMessage());
 ...
 public Vector showCreateRouteIcon(Context context,String[] args) throws Exception
 {
        ...
        for(Iterator objectListItr=objectList.iterator();objectListItr.hasNext();)
        {
         ...
         {
            createRouteIcons.add("<img border=\"0\" src=\"images/iconSmallRoute.png\" alt=\""+EnoviaResourceBundle.getProperty(context, stringResFileId, new Locale(languageStr), "emxTeamCentral.Access.CreateRoute")+"\" title=\""+EnoviaResourceBundle.getProperty(context,stringResFileId, new Locale(languageStr), "emxTeamCentral.Access.CreateRoute")+"\" />");
         }
         else
         {
 ...
 public Vector showCreateFolderIcon(Context context,String[] args) throws Exception
 {
        ...
        for(Iterator objectListItr=objectList.iterator();objectListItr.hasNext();)
        {
         ...
         {
            createFolderIcons.add("<img border=\"0\" src=\"images/iconSmallFolder.png\" alt=\""+EnoviaResourceBundle.getProperty(context,stringResFileId, new Locale(languageStr), "emxTeamCentral.CreateFolder.Tooltip")+"\" title=\""+EnoviaResourceBundle.getProperty(context, stringResFileId, new Locale(languageStr), "emxTeamCentral.CreateFolder.Tooltip")+"\" />");
         }
         else
         {
 ...
 public Vector getAccessOrganizations(Context context,String[] args) throws Exception
{
 ...
 public String getBuyerDesk (Context context,String[] args)throws Exception
    ...
    public MapList getWorkspaceVaults (Context context, String[] args) throws Exception
    {
        ...
        String selection = "single";
        String accessCheck = "current.access[read,modify,checkout,checkin,lock,revise,fromconnect,toconnect,show]";
        if( "addLinkFolder".equals(menuAction) )
        {
            selection = "multiple";
        ...
        {
            accessCheck = "current.access[fromconnect]";
    ...
    */
    ...
    /**
     * getRouteScopeWorkspaces - Displays the workspaces within a Route's scope
     * @param context the eMatrix <code>Context</code> object
     * @return String
     * @throws Exception if the operation fails
     * @since R420
     ...
     */
     ...
     public Vector showUserLastNameFirstName(Context context,String[] args) throws Exception
     {
        ...
        for(Iterator objectListItr=objectList.iterator();objectListItr.hasNext();)
        {
           ...
           {
               treeUrl = "<img src='images/iconPerson16.png' border='0' id=''/>"+sDisplayPersonName;
    ...
    throws FrameworkException
{
  ...
  public MapList getDisucssionFindfilesResult(Context context, String[] args) throws Exception
  {
      ...
      vaultNames.addElement(vaultString);
        ...
        mulValSelects.add(selVaultId);
        mulValSelects.add(selVaultNames);
        //get the details of the document objects for the current page
        paginatedMapList = DomainObject.getInfo(context, (String [])docIdList.toArray(new String []{}), selects, mulValSelects);
  ...
  {
      StringBuffer sb = new StringBuffer();
      sb.append("<img border=0 src=../common/images/iconPerson16.png></img>&nbsp;");
...
//    Bug No 312917 - End
  ...
  public Map createWorkspaceProcess(Context context,String[] args) throws MatrixException
  {
      try{
          Map programMap = (Map) JPO.unpackArgs(args);
          String strWSName = (String) programMap.get("Title");
          if(UIUtil.isNullOrEmpty(strWSName)){
			   strWSName = (String) programMap.get("Name");
		  }
          ...
          WorkspaceCreateMdl worksCreateM = new WorkspaceCreateMdl();
			/*
			 * if(strWSName.length()>(Integer.parseInt(MAX_LENGTH))) { String
			 * strLengthMessage =
			 * UINavigatorUtil.getI18nString("emxTeamCentral.NameLength.Message",
			 * "emxTeamCentralStringResource",langStr); String strChars =
			 * UINavigatorUtil.getI18nString("emxTeamCentral.NameLength.NumChars",
			 * "emxTeamCentralStringResource",langStr);
          ...
          //objectId = TeamUtil.autoRevision(context, HttpServletRequest , strProjectType, strWSName ,WorkspaceObj.POLICY_PROJECT, strCompanyVault);
        	  ...
        	  attributes.put(ATTRIBUTE_TITLE, strWSName);
        	  objectId = worksCreateM.createAndconnectWorkspaceTemplate(context,  DomainConstants.TYPE_WORKSPACE, "", WorkspaceObj.POLICY_PROJECT, attributes,  strWSDes,sTemplateId);
          ...
          if(!UIUtil.isNullOrEmpty(sTemplateId)) {
              ...
              DomainObject workspaceTemplateObj = DomainObject.newInstance(context,sTemplateId,DomainConstants.WORKSPACEMDL);
              worksCreateM.connectWorkspaceTemplate(context,  workspaceTemplateObj, true, true);
              //WorkspaceObj.connectWorkspaceTemplate(context,workspaceTemplateObj,true,true,strCompanyVault,true);
            }
          */
          if ( strBuyerDeskId != null && !"".equals(strBuyerDeskId) ) {
			...
			/*
			 * }else { retMap.put("ErrorMessage", EnoviaResourceBundle.getProperty(context,
			 * "emxTeamCentralStringResource", context.getLocale(),
			 * "emxTeamCentral.Common.Workspace") + " " + strWSName + " " +
			 * EnoviaResourceBundle.getProperty(context, "emxTeamCentralStringResource",
			 * context.getLocale(), "emxTeamCentral.Common.AlreadyExists")); return retMap;
			 * }
			 */
    	  ...
    	  //added for IR-1305574-3DEXPERIENCER2025x
		 String msg = e.getMessage();	 
          if (msg.contains("No create access to business object")) {
              msg = EnoviaResourceBundle.getProperty(context,
                                                     "FolderNGServices",
                                                     context.getLocale(),
                                                     "Creation.AccessError.message");  
          } 
  ...
  public void connectMember(matrix.db.Context context, String[] args) throws Exception
  {
      try
      {
    	  ...
    	  if(e.getMessage().indexOf("fromconnect") > 0){
    		  String sMsg = EnoviaResourceBundle.getProperty(context,"emxComponentsStringResource", new Locale(context.getSession().getLanguage()),"emxComponents.ObjectAccess.NoAccessToAddRemove");
...
public static int updateCountOnCreateAndDelete(Context context, String[] args) throws Exception
	{
		...
		DomainObject projObj = new DomainObject(projectId);
		projObj.lockForUpdate(context);
		int count = Integer.parseInt(projObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_COUNT));
        try {
			ContextUtil.pushContext(context);
			if(UIUtil.isNotNullAndNotEmpty(event) && "Create".equalsIgnoreCase(event)) {
				projObj.setAttributeValue(context, DomainConstants.ATTRIBUTE_COUNT, String.valueOf(++count));
		...
		MapList projExpList = mxBus.expand(context,
	            projObj, DomainConstants.RELATIONSHIP_SUB_VAULTS, "*",
	            true, false, 0,
	            busSelects, null,
	            null, null,
	            null, null);	
		
		DomainObject folderObj = null;
		try {
			ContextUtil.pushContext(context);
			for(Map objMap : (List<Map>)projExpList) { 
				String objId = (String) objMap.get(DomainConstants.SELECT_ID); 
				folderObj = new DomainObject(objId);
				folderObj.lockForUpdate(context);
				count = Integer.parseInt(folderObj.getAttributeValue(context, DomainConstants.ATTRIBUTE_COUNT));
				try {
					if(UIUtil.isNotNullAndNotEmpty(event) && "Create".equalsIgnoreCase(event)) {
						folderObj.setAttributeValue(context, DomainConstants.ATTRIBUTE_COUNT, String.valueOf(++count));
					} else if("Delete".equalsIgnoreCase(event)){
						folderObj.setAttributeValue(context, DomainConstants.ATTRIBUTE_COUNT, String.valueOf(--count));
					}	
				} catch(Exception e) {
...
/**
 * Trigger method called on create and delete checks when adding or removing Workspace or Folder content or SubFolders
 * The connection or removal of content or sub Folders will be blocked if the parent object is at the specified 
 * state or higher.
 * @param context the eMatrix <code>Context</code> object
 * @param args String array of input arguments
 * 		[0]=from object id
 * 		[1]=to object id
 * 		[2]=Baseline blocking state
 * @return int where 0 is success (allow) and 1 is failure (block)
 * @throws Exception if the operation fails
 */
   
static final Map<String, String> stateMapping = new HashMap<String, String>() {{
    put("PRIVATE", "state_Create");
    put("INWORK", "state_Active");
    put("FROZEN", "state_Review");
    put("RELEASED", "state_Complete");
    put("OBSOLETE", "state_Archive");
}};
...
public static int checkAllowChangeConnection(Context context, String[] args) throws Exception
{
	...
	String toObjId = args[3];
	String blockingState = args[4];
	String event = args[5];
	String fromType = args[6];
	
	//System.out.println("**********INSIDE checkAllowChangeConnection...args.");
	//System.out.println("**********relId=" + relId);
	//System.out.println("**********relType=" + relType);
	//System.out.println("**********fromObjectId=" + fromObjId);
	//System.out.println("**********toObjId=" + toObjId);
	//System.out.println("**********blockingState=" + blockingState);
	//System.out.println("**********event="+event);
	//System.out.println("**********fromType="+fromType);

    try {
    	DomainObject domFromObj = DomainObject.newInstance(context, fromObjId);
    	...
    	Map selectMap = domFromObj.getInfo(context, selectList);

    	//only perform check if from object is using Workspace policy
    	if (DomainConstants.POLICY_PROJECT.equalsIgnoreCase((String)selectMap.get(DomainConstants.SELECT_POLICY)))
    	{
    		//We are allowing Content to be deleted as Content is under the control of the Content owner
    		//if this event is Delete and the TO type is not a Workspace Vault, allow the check
    		//if (event != null && "Delete".equalsIgnoreCase(event))
    		//{
    		//	DomainObject domToObj = DomainObject.newInstance(context, toObjId);
    		//	if (!domToObj.isKindOf(context, DomainConstants.TYPE_PROJECT_VAULT))
    		//	{
    		//		System.out.println("**********returning 0 as EVENT is DELETE and TYPE is NOT WV");
    		//		return 0;
    		//	}
    		//}

    		//get state that maps to baseline blockingState
    		//TODO for now state mapping is in class, this should be added as properties on policy
    		String sTargetStateSymb = stateMapping.get(blockingState);
    		String sTargetState = PropertyUtil.getSchemaProperty(context,"policy", DomainConstants.POLICY_PROJECT, sTargetStateSymb);
    		//System.out.println("**********sTargetState="+sTargetState);

    		if (checkObjState(context, domFromObj, sTargetState, LT) != 0) // if current state is not less than target state - do not allow
    		{
    			//System.out.println("**********fail checkState returning 1");
    			String sMsg = "";
    			if (relType!=null && DomainConstants.RELATIONSHIP_VAULTED_DOCUMENTS.equalsIgnoreCase(relType))
    			{
    				sMsg = EnoviaResourceBundle.getProperty(context,"emxTeamCentralStringResource", new Locale(context.getSession().getLanguage()),"emxTeamCentral.Workspace.MsgChangeContentFrozen");
    			}
    			else
    			{
    				sMsg = EnoviaResourceBundle.getProperty(context,"emxTeamCentralStringResource", new Locale(context.getSession().getLanguage()),"emxTeamCentral.Workspace.MsgChangeStructureFrozen");
    			}
    			emxContextUtil_mxJPO.mqlNotice(context,sMsg);
    			return 1; //current state is at or past target state, or error - block
    		}
    	}
		
		//Need to also block add\delete of Data Vaults\Sub Vaults if context user does not have 'modify' access (either thru role or Sharing) on FROM object
		if (relType !=null && (DomainConstants.RELATIONSHIP_SUBVAULTS.equalsIgnoreCase(relType) || DomainConstants.RELATIONSHIP_PROJECT_VAULTS.equalsIgnoreCase(relType) ) )
		{
			String hasModify = (String)selectMap.get("current.access[modify]");
			if ("False".equalsIgnoreCase(hasModify))
			{
	    		//System.out.println("**********no modify returning 1");
	    		String sMsg = EnoviaResourceBundle.getProperty(context,"emxTeamCentralStringResource", new Locale(context.getSession().getLanguage()),"emxTeamCentral.Workspace.MsgNoChangeStructureAccess");
	    		emxContextUtil_mxJPO.mqlNotice(context,sMsg);
				return 1; // no modify access on parent object - block connect/disconnect
			}
		}
		//System.out.println("**********reached end returning intReturn="+intReturn);
		return intReturn;
...
protected static int checkObjState(matrix.db.Context context, DomainObject domObj, String sTargetState, int comparisonOperator)
{
	try
	{
		String sCurrentState = domObj.getInfo(context, DomainConstants.SELECT_CURRENT);
		StringList strListChildStates = domObj.getInfoList(context,DomainConstants.SELECT_STATES);
...
public Map createWorkspaceFolderProcess(Context context,String[] args) throws Exception
{
    ...
    String strFolderName  = (String) programMap.get("TitleDisplay");
    String languageStr    = (String) programMap.get("languageStr");
    ...
    try{
      ...
      String strFolderOwnership = "context";
			/*
			 * if (workspace.isFolderExists(context, strFolderName)) { bExists = true;
			 * String i18NCategory =
			 * EnoviaResourceBundle.getProperty(context,"emxTeamCentralStringResource", new
			 * Locale(languageStr),"emxTeamCentral.AddCategories.Category"); String
			 * i18NNotUnique =
			 * EnoviaResourceBundle.getProperty(context,"emxTeamCentralStringResource", new
			 * Locale(languageStr),"emxTeamCentral.AddCategories.NotUnique");
			 * retMap.put("ErrorMessage", i18NCategory +"  "+ strFolderName +"  "+
			 * i18NNotUnique); return retMap; }
			 */
      //if(!bExists) {
     	 WorkspaceVault wVault =  new WorkspaceVault();
     	 Map<Object, Object> attributes = new HashMap<Object, Object>();
	  	 attributes.put(ATTRIBUTE_TITLE, strFolderName);
	  	...
	  	WorkspaceCreateMdl workspaceValutMod = new WorkspaceCreateMdl();
	  	 String workpsaceObjId = workspaceValutMod.createVault(context, strTypefolder, null, strPolicyProject, null, attributes, "", true);
	  	 wVault.setId(workpsaceObjId);
