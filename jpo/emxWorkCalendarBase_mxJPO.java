...
public class emxWorkCalendarBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	/**
	 * Return a collection of HTML formatted string containing Location names. 
	 * @param context the ENOVIA <code>Context</code> object.
	 * @param args JPO methods arguments.
	 * @return a StringList of Location details.
	 * @throws FrameworkException
	 */
	
	public StringList getCalendarLocationsList(Context context , String[] args) throws FrameworkException{
		try{
			Map programMap = (Map) JPO.unpackArgs(args);
			Map requestMap = (Map) programMap.get("requestMap");
			...
			String SELECT_LOCATION_ID = "to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.id";
			String SELECT_LOCATION_NAME = "to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.name";
			String SELECT_LOCATION_TYPE = "to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.type";
			String SELECT_LOCATION_RELID="to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.to["+DomainConstants.RELATIONSHIP_ORGANIZATION_LOCATION+"].id";
			StringList busSelects = new StringList(3);
			busSelects.add(SELECT_LOCATION_ID);
			busSelects.add(SELECT_LOCATION_NAME);
			busSelects.add(SELECT_LOCATION_TYPE);
			busSelects.add(SELECT_ID);
			busSelects.add(SELECT_LOCATION_RELID);


			Map mapCalendarInfo = new LinkedHashMap();
			BusinessObjectWithSelectList objectWithSelectList = null;
			if("TestCase".equalsIgnoreCase(invokeFrom)) { //Added for ODT
				objectWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, strObjectIds, busSelects,true);
			}else {
				objectWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, strObjectIds, busSelects);
			}
			...
			for (BusinessObjectWithSelectItr objectWithSelectItr = new BusinessObjectWithSelectItr(objectWithSelectList); objectWithSelectItr.next();) {
				...
				Map mapCalendar = new HashMap();
				for (Iterator itrSelectables = busSelects.iterator(); itrSelectables.hasNext();) {
					String strSelectable = (String)itrSelectables.next();
					mapCalendar.put(strSelectable, objectWithSelect.getSelectDataList(strSelectable));
				}
				mapCalendarInfo.put(objectWithSelect.getSelectData(SELECT_ID), mapCalendar);
			}
			...
			while(objectIdItr.hasNext()){
				...
				String calendarId = (String)((Map)objectIdItr.next()).get(ProgramCentralConstants.SELECT_ID);

				Map objectInfo = (Map)mapCalendarInfo.get(calendarId);
				StringBuffer sbOutput = new StringBuffer();
				String strOuput =DomainConstants.EMPTY_STRING;

				if(objectInfo != null) {

					StringList typeList			= (StringList)objectInfo.get(SELECT_LOCATION_TYPE);
					if(typeList != null && typeList.contains(ProgramCentralConstants.TYPE_LOCATION)) {
						StringList nameList			= (StringList)objectInfo.get(SELECT_LOCATION_NAME);
						StringList idList			= (StringList)objectInfo.get(SELECT_LOCATION_ID);
						StringList relIds			=(StringList)objectInfo.get(SELECT_LOCATION_RELID);

						for(int i=0, typeListSize=typeList.size();i<typeListSize;i++) {
							if(ProgramCentralConstants.TYPE_LOCATION.equalsIgnoreCase(typeList.get(i))) {
								String locationName = nameList.get(i);
								locationName=XSSUtil.encodeForHTMLAttribute(context,locationName);
								String locationId = idList.get(i);
								String relId = relIds.get(0);
								sbOutput.append("<a href ='javascript:showModalDialog(\"");
								sbOutput.append("../common/emxTree.jsp?objectId=");
			                    sbOutput.append(XSSUtil.encodeForURL(context,locationId));
			                    sbOutput.append("&amp;relId=");
			                    sbOutput.append(XSSUtil.encodeForURL(context,relId));
			                    sbOutput.append("\", \"875\", \"550\", \"false\", \"popup\")'>");
			                    sbOutput.append(locationName);
			                    sbOutput.append("</a>");
								sbOutput.append("<br/>");
								strOuput = sbOutput.toString();
							}
	...
	public StringList getCalendarWorkweekInXML (Context context, String[] args) throws Exception{
		...
		try {
			...
			int dayOfWeekIndex;
			   ...
			   String[] strCalendarIds = new String[objectListSize];
			    for (int i = 0; i < objectListSize; i++) {
			           Map calendarInfo = (Map) objectList.get(i);
			           String calendarId = (String) calendarInfo.get(DomainObject.SELECT_ID);
			           if(calendarId!=null && !calendarId.isEmpty()) {
			        	   strCalendarIds[i] = calendarId;
			           }
		        }

			    /*StringList slBusSelect = new StringList(2);
		        slBusSelect.add("attribute[Working Days Per Week]");
		        slBusSelect.add("attribute[Working Days Per Month]");
		        BusinessObjectWithSelectList objectWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, strCalendarIds, slBusSelect,true);
		        for(int i=0, bsize = objectWithSelectList.size(); i <bsize ; i++){
		            ...
		            String workingDaysPerWeek =  objectWithSelect.getSelectData("attribute[Working Days Per Week]");
		            String workingDaysPerMonth =  objectWithSelect.getSelectData("attribute[Working Days Per Month]");
		           
		            Map calendarInfo =(Map) objectList.get(i);	              
		            if(workingDaysPerMonth!=null & !workingDaysPerMonth.isEmpty())
		            	calendarInfo.put("workingDaysPerMonth", workingDaysPerMonth);
		            if(workingDaysPerWeek!=null & !workingDaysPerWeek.isEmpty())
		            	calendarInfo.put("workingDaysPerWeek", workingDaysPerWeek);
		        }*/
	
			for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
				Map calendarInfo = (Map) iterator.next();
				String calendarId = (String) calendarInfo.get(DomainObject.SELECT_ID);
				//String workingDaysPerWeek = (String)calendarInfo.get("workingDaysPerWeek");
				//String workingDaysPerMonth =(String) calendarInfo.get("workingDaysPerMonth");
				...
				for (Iterator itrWorkweek = mlWorkweek.iterator(); itrWorkweek.hasNext();) {
					Map weekDayInfo = (Map) itrWorkweek.next();
					String workingTimes =  (String) (weekDayInfo.get(WorkCalendar.SELECT_WORKING_TIMES));
					
					String workStartTime = "0800";
					String workFinishTime = "1700";
					String firstRecessStartTime = "1200";
					String lastRecessFinishTime = "1300";
					if(ProgramCentralUtil.isNotNullString(workingTimes)) {
						...
						for (int i = 0, size = workingHours.size(); i < size; i++) {
							...
							if(i==0){
								workStartTime = startFinishTime.get(0);
								firstRecessStartTime = size >1 ? startFinishTime.get(1) : EMPTY_STRING;
							}
							if(i==size-1){
								workFinishTime = startFinishTime.get(1);
								lastRecessFinishTime = size >1 ? startFinishTime.get(0) :  EMPTY_STRING;
							}
					...
					//sbWorkweek.append(" workingDaysPerWeek=\""+workingDaysPerWeek+"\"");
					//sbWorkweek.append(" workingDaysPerMonth=\""+workingDaysPerMonth+"\"");
					sbWorkweek.append(" oid=\""+(String)weekDayInfo.get("id") + "\" >");				
					sbWorkweek.append("<Work_StartTime>" + workStartTime +"</Work_StartTime>");
					sbWorkweek.append("<Work_FinishTime>" + workFinishTime +"</Work_FinishTime>");
					sbWorkweek.append("<Lunch_StartTime>" + firstRecessStartTime +"</Lunch_StartTime>");					
					sbWorkweek.append("<Lunch_FinishTime>" + lastRecessFinishTime +"</Lunch_FinishTime>");	
					sbWorkweek.append("<workingTimes>" + workingTimes +"</workingTimes>");
					sbWorkweek.append("</WeekDay>");
				}
				slWorkweek.add(sbWorkweek.toString());
			}
	...
	/**
	 * Gets Calendar's working hours per day list. Used in Calendar Listing page under Company
	 * @param context the ENOVIA <code>Context</code> object.
	 * @param args JPO methods arguments.
	 * @return Standard working hour in string format.
	 * @throws FrameworkException
	 */
	public StringList getCalendarStandardWorkList(Context context, String[] args) throws FrameworkException{
		try{
			Map programMap = (Map) JPO.unpackArgs(args);
			...
			busSelects.add("attribute[Working Time Per Day]");
			busSelects.add(SELECT_ID);
			Map mapCalendarInfo = new LinkedHashMap();
			BusinessObjectWithSelectList objectWithSelectList = null;
			if("TestCase".equalsIgnoreCase(invokeFrom)) { //Added for ODT
			...
			for (BusinessObjectWithSelectItr objectWithSelectItr = new BusinessObjectWithSelectItr(objectWithSelectList); objectWithSelectItr.next();) {
				...
				for (Iterator itrSelectables = busSelects.iterator(); itrSelectables.hasNext();) {
					String strSelectable = (String)itrSelectables.next();
					mapCalendar.put(strSelectable, objectWithSelect.getSelectData(strSelectable));
				}
				mapCalendarInfo.put(objectWithSelect.getSelectData(SELECT_ID), mapCalendar);
			}
			...
			for (int i=0;i<size;i++) {
				Map calendarInfo = (Map) objectList.get(i);
				String calendarId = (String) calendarInfo.get(DomainObject.SELECT_ID);
				Map calendarInfoMap = (Map)mapCalendarInfo.get(calendarId);
				String sWork			= (String)calendarInfoMap.get("attribute[Working Time Per Day]");
				double dWork = Task.parseToDouble(sWork);
				dWork = (dWork / 60);
				slWorkweek.add((dWork + DomainConstants.EMPTY_STRING));
			}
			return slWorkweek;
		}catch(Exception e){
			throw new FrameworkException(e);
	 ...
	 @com.matrixone.apps.framework.ui.PostProcessCallable
	    ...
	    /**
	     * This method is to update the name of calendar
	     * @param context
	     * @param args
	     * @throws Exception
	     */
		public void updateCalendarName(Context context, String[] args)throws Exception, MatrixException {
			Map programMap = (Map)JPO.unpackArgs(args);
			Map paramMap   = (Map)programMap.get("paramMap");
			String objectId       = (String)paramMap.get("objectId");
			String strNewValue = (String)paramMap.get("New Value");
			String companyId = PersonUtil.getUserCompanyId(context);
			boolean isCalendarNameNotUnique = WorkCalendar.checkForUniqueCalendar(context, companyId, strNewValue);

			try
			{
				if (!isCalendarNameNotUnique) {
					WorkCalendar wc = new WorkCalendar(objectId);
					wc.setName(context, strNewValue);
				}else {
						String errMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Calendar.UniqueNameMessage", context.getSession().getLanguage());
						throw new MatrixException(errMessage.toString());
				}
			}
			catch(Exception e)
			{
	    ...
	    /**
	     * This method is to update the description of calendar
	     * @param context
	     * @param args
	     * @throws Exception
	     */
		public void updateCalendarDescription(Context context, String[] args)throws Exception, MatrixException {
			Map programMap = (Map)JPO.unpackArgs(args);
			Map paramMap   = (Map)programMap.get("paramMap");
			String objectId       = (String)paramMap.get("objectId");
			String strNewValue = (String)paramMap.get("New Value");
			...
			{
				WorkCalendar wc = new WorkCalendar(objectId);
				wc.setDescription(context, strNewValue);
			}
			catch(Exception e)
			{
				throw new MatrixException(e.toString());
		...
		public boolean isHostOrOrganizationMember(Context context, String[] args) throws Exception{
	      	HashMap programMap = (HashMap) JPO.unpackArgs(args);
	     	String calendarId = (String) programMap.get("objectId");
	     	
	     	DomainObject calendarObj = newInstance(context, calendarId);
	     	String orgId = calendarObj.getInfo(context, "to[Company Calendar].from.id");  
	 		...
	 		try{
	 			...
	 			DomainObject personObj = newInstance(context, personId);
	 	        String selectUserCompanyID = "to[" + PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_relationship_Employee) + "].from.id";
	 	        personOrgId = personObj.getInfo(context, selectUserCompanyID);  
	 		}catch(Exception e){
	 			throw new Exception(e); 
	 		}finally{
	 			ContextUtil.popContext(context);
	 		}		       
	        if(hostId.equals(personOrgId)){  			//if the person is the host company member
	 			return true; 
	 		}
	 		else if(orgId.equals(personOrgId)){ 		//if the orgId is the user orgId
	 			return true;
	 		}
	 		else{
	 			return false;
		...
		public StringList getCalendarLocationsTypeList(Context context , String[] args) throws FrameworkException{
			try{
				Map programMap = (Map) JPO.unpackArgs(args);
				Map requestMap = (Map) programMap.get("requestMap");
				...
				String sRelOrgLocation   = PropertyUtil.getSchemaProperty(context, "relationship_OrganizationLocation");
				String sRelAttrBillingAddress   = PropertyUtil.getSchemaProperty(context, "attribute_BillingAddress");
	            String sRelAttrHeadquartersSite     = PropertyUtil.getSchemaProperty(context, "attribute_HeadquartersSite");
	            String sRelAttrManufacturingSite    = PropertyUtil.getSchemaProperty(context, "attribute_ManufacturingSite");
	            ...
	            String i18nsRelAttrHeadquarters     = i18nNow.getAttributeI18NString(sRelAttrHeadquartersSite,languageStr);
	            String i18nsRelAttrManufacturing    = i18nNow.getAttributeI18NString(sRelAttrManufacturingSite,languageStr);
	            String i18nsRelAttrShippingAddress  = i18nNow.getAttributeI18NString(sRelAttrShippingAddress,languageStr);
	            
				String attr = "to[Organization Location].attribute[Headquarters Site]";

				String SELECT_BillingAddress = "to["+sRelOrgLocation+"].attribute["+sRelAttrBillingAddress+"]";
				String SELECT_HeadquartersSite = "to["+sRelOrgLocation+"].attribute["+sRelAttrHeadquartersSite+"]";
				String SELECT_ManufacturingSite = "to["+sRelOrgLocation+"].attribute["+sRelAttrManufacturingSite+"]";
				String SELECT_ShippingAddress = "to["+sRelOrgLocation+"].attribute["+sRelAttrShippingAddress+"]";
				
				String SELECT_LOCATION_NAME = "to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.name";
				String SELECT_LOCATION_TYPE = "to["+ProgramCentralConstants.RELATIONSHIP_CALENDAR+"].from.type";
				StringList busSelects = new StringList(3);
				busSelects.add(SELECT_BillingAddress);
				busSelects.add(SELECT_HeadquartersSite);
				busSelects.add(SELECT_ManufacturingSite);
				busSelects.add(SELECT_ShippingAddress);
				busSelects.add(SELECT_ID);
				...
				while(objectIdItr.hasNext()){
					...
					if(objectInfo != null) {
						...
						if("Yes".equals(objectInfo.get(SELECT_BillingAddress)))
		                {
							sbOutput.append(XSSUtil.encodeForHTML(context,i18nsRelAttrBillingAddress));
		                }
		                if("Yes".equals(objectInfo.get(SELECT_HeadquartersSite)))
		                {
		                    if(sbOutput.length() > 0)
		                    {
		                    	sbOutput.append("<br/>");
		                    }
		                    sbOutput.append(XSSUtil.encodeForHTML(context,i18nsRelAttrHeadquarters));
		                }
		                if("Yes".equals(objectInfo.get(SELECT_ManufacturingSite)))
		                {
		                    if(sbOutput.length() > 0)
		                    {
		                    	sbOutput.append("<br/>");
		                    }
		                    sbOutput.append(XSSUtil.encodeForHTML(context,i18nsRelAttrManufacturing));
		                }
		                if("Yes".equals(objectInfo.get(SELECT_ShippingAddress)))
		                {
		                    if(sbOutput.length() > 0)
		                    {
		                    	sbOutput.append("<br/>");
		                    }
		                    sbOutput.append(XSSUtil.encodeForHTML(context,i18nsRelAttrShippingAddress));
		                }
		                sbOutput.append(" ");
					}
					slWorkweek.add(sbOutput.toString());
				}
...
@com.matrixone.apps.framework.ui.ProgramCallable
	public Vector getLocationNameColumnData(Context context , String[] args)throws Exception{
			Map programMap 		= (Map)JPO.unpackArgs(args);
			MapList objectList 	= (MapList) programMap.get("objectList");
			Map paramMap = (Map) programMap.get("paramList");
			String disableHyperlink 	= (String)paramMap.get("disableHyperLink");
			int size=objectList.size();
			String[] locationIds=new String[size]; 
			Vector columnValues = new Vector(objectList.size());
			BusinessObjectWithSelectList bwsl=null;
			
			if(!"true".equalsIgnoreCase(disableHyperlink)){
				Iterator objectIdItr = objectList.iterator();
				StringList slBusSelect = new StringList(13);
				slBusSelect.add("to["+DomainConstants.RELATIONSHIP_ORGANIZATION_LOCATION+"].id");
				
				for (int i = 0; i < size; i++) {
					Map mapObject = (Map) objectList.get(i);
					String taskId = (String) mapObject.get(DomainObject.SELECT_ID);
					locationIds[i] = taskId;
				}
				 bwsl = ProgramCentralUtil.getObjectWithSelectList(context, locationIds, slBusSelect);
			}
			...
			for(int i=0;i<size;i++){
				...
				String locationId = (String)objectInfo.get(SELECT_ID);
				locationId = XSSUtil.encodeForURL(context,locationId);
				
				if(!"true".equalsIgnoreCase(disableHyperlink)){
					BusinessObjectWithSelect bws = bwsl.getElement(i);
					strName=XSSUtil.encodeForHTMLAttribute(context,strName);
					String relId = bws.getSelectData("to["+DomainConstants.RELATIONSHIP_ORGANIZATION_LOCATION+"].id");
					sBuff.append("<a href ='javascript:showModalDialog(\"");
					sBuff.append("../common/emxTree.jsp?objectId=");
					sBuff.append(XSSUtil.encodeForURL(context,locationId));
                    sBuff.append("&amp;relId=");
                    sBuff.append(XSSUtil.encodeForURL(context,relId));
                    sBuff.append("\", \"875\", \"550\", \"false\", \"popup\")'>");
