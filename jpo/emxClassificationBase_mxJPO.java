...
public class emxClassificationBase_mxJPO extends emxLibraryCentralCommon_mxJPO {
	 ...
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: 0 - strPartId The id of
	 *                the part 1- strparentObj Id The id of the parent object 2 -
	 *                strPartType The type of the part 3 - strName The name of the
	 *                part 4 - strPartRevision The revision of the part 5 -
	 *                strPartPolicy The policy of the part 6 - strDescription The
	 *                description of the part
	 * @return a Map containing the part object and boolean state whether the part
	 *         exists or not
	...
	public static Map createAndConnectPart(Context context, String[] args) throws FrameworkException {
		try {
			...
			return createAndConnectPart(context, strPartId, strparentObjId, strPartType, strName, strPartRevision,
					strPartPolicy, strDescription);
		} catch (Exception e) {
	...
	public static Map createAndConnectPart(Context context, String strPartId, String strparentObjId, String strPartType,
			String strName, String strPartRevision, String strPartPolicy, String strDescription)
			throws FrameworkException {

		boolean bPartExists = false;
		Map partMap = new HashMap();
		try {
			BusinessObject busPart = new BusinessObject(strPartType, strName, strPartRevision,
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: 0 - parentId is the id of
	 *                the parent object 1 - childIds is the array of the child
	 *                objects to be added
	...
	public static String addEndItems(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			if (null != childIds && childIds.length != 0) {
				resultString = addEndItems(context, parentId, childIds);
			}
		} catch (Exception e) {
			throw new FrameworkException(e);
		}
	 ...
	 * @throws FrameworkException if the operation fails
	 */
	public static String addEndItems(Context context, String classId, String[] childIds) throws FrameworkException {
		try {
			DomainObject classObj = new DomainObject(classId);

			StringList selects = new StringList();
			selects.add(SELECT_TYPE);
			selects.add("attribute[" + LibraryCentralConstants.ATTRIBUTE_ClassUsage + "]");
			selects.add("type.kindof[" + LibraryCentralConstants.TYPE_CLASSIFICATION + "]");
			Map map = (Map) classObj.getInfo(context, selects);
			String objectType = (String) map.get(SELECT_TYPE);
			String usage = (String) map.get("attribute[" + LibraryCentralConstants.ATTRIBUTE_ClassUsage + "]");
			boolean isKindOfClasssification = "TRUE".equalsIgnoreCase(
					((String) map.get("type.kindof[" + LibraryCentralConstants.TYPE_CLASSIFICATION + "]")));
			String workSpaceVault = PropertyUtil.getSchemaProperty(context, "type_ProjectVault");
			String relType = ClassificationUtils.getRelationshipFromUsage(usage);

			// In case of Adding DC Types to workSpace Vault

			if (objectType.equals(workSpaceVault)) {
				String[] folderIds = new String[1];
				folderIds[0] = classId;
				String objNameNotAdded = DCWorkspaceVault.addToFolders(context, folderIds, childIds);
			} else if (isKindOfClasssification) {
				// IR-869153-3DEXPERIENCER2022x stop classifying already classified
				// LibraryFeaturePort
				for (String childId : childIds) {
					...
					if (doObj.isKindOf(context, sTypeToblock)) {
						...
						String isClassifiedPort = MqlUtil.mqlCommand(context, "print bus $1 select $2 $3 dump", childId,
								"to[Classified Item]", "to[Standard Classified Item]");

						if (isClassifiedPort.toUpperCase().contains("TRUE")) {
							String resourceBundle = "emxLibraryCentral";
							String strLanguage = context.getSession().getLanguage();
							String errMsg = EnoviaResourceBundle.getProperty(context, resourceBundle,
									new Locale(strLanguage), "emxLibraryCentral.LibraryFeaturePort.AccessNotAllowed");
							throw new FrameworkException(errMsg);
						}

					}
				}
				DomainRelationship.connect(context, classObj, relType, true, childIds);
			}
			String strQuery = "expand bus $1 to relationship $2 recurse to all select bus $3 dump $4";

			String strResult = MqlUtil.mqlCommand(context, strQuery, classId,
					LibraryCentralConstants.RELATIONSHIP_SUBCLASS, "id", ",");
			return strResult;
		} catch (Exception e) {
			throw new FrameworkException(e);
		}
	...
	public MapList getClassificationIds(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			classificationIds = (MapList) domObj.getRelatedObjects(context, relPattern.getPattern(), // relationship
																										// pattern
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - objectList
	 * @return StringList - Classification Path List
	 * @throws Exception if the operation fails
	 */
		...
		try {
			...
			while (objectListItr.hasNext()) {
				...
				MapList parentClasses = (MapList) domObj.getRelatedObjects(context,
						...
						null, // relationship where
						0); // limit
				parentClasses = parentClasses.sortStructure(context, DomainConstants.SELECT_LEVEL, "descending",
				...
				while (itr.hasNext()) {
					...
					elemHtmlList.add(renderPathElem(context,
							(UIUtil.isNullOrEmpty(parentClassTitle)) ? parentclassName : parentClassTitle,
							parentClassId, reportFormat));
				}
				elemHtmlList.add(renderPathElem(context, classTitle, ObjectId, reportFormat));
				classficationPaths
						.add(pathWrapperOpen + FrameworkUtil.join(elemHtmlList, elemSeperator) + pathWrapperClose);
			}
		} catch (Exception err) {
	...
	public String renderPathElem(Context context, String strName, String strObjectId, String reportFormat)
		...
		/*
		 * If the Library/Class name contains "&" character, we need to replace it with
		 * the code "amp;" because getXML() of BPS code base doesn't support the "&"
		 * character. IR-261969
		 */
		/*
		 * Removed changes "&" character replaced with "amp;" as BPS supporting &
		 * character now
		 */
		/*
		 * removed the encodeForHTML wrap, as it is messing up when french chars are
		 * present in the object name BPS does this again for programHTML fields inside
		 * SB code, which causing double encoding IR-261969
		 */
		return "<a href=\"javascript:void(0)\" onClick=\"javascript:showModalDialog('../common/emxTree.jsp?objectId="
				+ XSSUtil.encodeForJavaScript(context, strObjectId) + "','860','520');\" >"
	...
	public StringList getClassificationExclusionIds(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			String objectId = (String) paramMap.get("objectId");
			DomainObject domObj = new DomainObject(objectId);
			slExclusionIds = domObj.getInfoList(context,
					"to[" + LibraryCentralConstants.RELATIONSHIP_CLASSIFIED_ITEM + "].from.id");

			// PSA11 - IR-664909-3DEXPERIENCER2020x -Start
			// StringBuffer sbWhereExpression = new
			// StringBuffer("current.access[fromconnect]~~FALSE");
			StringBuffer sbWhereExpression = new StringBuffer("current.access[fromconnect] == TRUE");
			sbWhereExpression.append(" && (policy == " + LibraryCentralConstants.POLICY_CLASSIFICATION
					+ " || policy == \"" + LibraryCentralConstants.POLICY_DOCUMENT_CLASSIFICATION + "\""
					+ " || policy == \"" + LibraryCentralConstants.POLICY_LIBRARIES + "\")");
			sbWhereExpression.append(" && (attribute[" + LibraryCentralConstants.ATTRIBUTE_ClassUsage + "] == \""
					+ LibraryCentralConstants.ATTRIBUTEVALUE_ClassificationUsage_Standard + "\" || attribute["
					+ LibraryCentralConstants.ATTRIBUTE_LibraryUsage + "] == \""
					+ LibraryCentralConstants.ATTRIBUTEVALUE_ClassificationUsage_Standard + "\")");
			System.out.println("sbWhereExpression====" + sbWhereExpression);
			...
			// MapList mlExcludeOIDs = DomainObject.findObjects(context, types[i], "*",
			// sbWhereExpression.toString(), objSelects);
			MapList mlExcludeOIDs = DomainObject.findObjects(context, commaSeparatedTypeList, "*",
					sbWhereExpression.toString(), objSelects);
	...
	/**
	 * This method returns search query for Library field during
	 * classification/reClassification
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 - rowIds
	...
	/**
	 * This method returns search query for Class field during
	 * classification/reClassification
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - object Id 1 - rowIds
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments 0 - objectId 1 - rowIds 2
	 *                - ClassOID
	 * @throws Exception if the operation fails
	 */
	public void updateClassField(Context context, String[] args) throws Exception {
		...
		if (classificationMode != null && !"null".equals(classificationMode)
				&& "classification".equals(classificationMode)) {
			for (int i = 0; i < slNewClassIds.size(); i++) {
				Classification.addEndItems(context, (String) slNewClassIds.get(i), new String[] { objectId });
			}
		} else if (classificationMode != null && !"null".equals(classificationMode)
				&& "reClassification".equals(classificationMode)) {
			String oldClassId = extractVal(requestMap.get("selectedClassIds"));
			for (int i = 0; i < slNewClassIds.size(); i++) {
				oldClassId = (i == 0) ? oldClassId : null;
				Classification.reclassify(context, new String[] { objectId }, oldClassId,
						(String) slNewClassIds.get(i));
			}
		}
	}
	 ...
	 * requestMaps wherein each value is an array of strings, out of which we will
	 * always want the first element. Most other forms submit a string value for
	 * each param value. This method hides that difference.
	 *
	...
	/**
	 * Check if Classification of content is allowed.
	 * 
	 * @param context the eMatrix Context object.
	 * @param args[]  holds object and connection details: [0] : relationship ID [1]
	 *                : relationship type [2] : FROM object ID [3] : TO object ID
	 *                [4] : blocking state name [5] : EVENT [6] : FROM TYPE
	 * @return 0 if classification is allowed
	 * @throws Exception if the operation fails
	 * @since R2024x FD01
	 */
	public int checkAllowClassification(Context context, String[] args) throws Exception {
		int intReturn = 0;
		// String relType = args[1];
		String fromObjId = args[2];
		// String toObjId = args[3];
		String allowedState = args[4];
		// String eventType = args[5];
		String fromType = args[6];
		Boolean isContextPushed = false;

		// Check is only required for 'General Class'
		if (fromType.equals(LibraryCentralConstants.TYPE_GENERAL_CLASS)) {

			if (context.getUser().equals(_superUser)) {
				return intReturn;
			}

			try {
				/*
				 * Push context to retrieve state information, as context user may not have
				 * access to retrieve policy/state information.
				 */
				ContextUtil.pushContext(context);
				isContextPushed = true;
				DomainObject fromObject = DomainObject.newInstance(context, fromObjId);
				String currentState = fromObject.getInfo(context, SELECT_CURRENT);
				if (!allowedState.equals(currentState)) {
					emxContextUtil_mxJPO.mqlNotice(context,
							EnoviaResourceBundle.getProperty(context, "emxLibraryCentralStringResource",
									context.getLocale(), "emxLibraryCentral.Message.MsgClassifyNotAllowed"));
					intReturn = 1;
				}
			} catch (Exception e) {
				throw e;
