...
public class emxMemberRelationshipBase_mxJPO extends com.matrixone.apps.common.MemberRelationship
{
    ...
    public void triggerCreateAction(Context context, String[] args) throws Exception {
        ...
        //Added new logic to add member as lead for same collabspace leader
        if(Boolean.valueOf(isKindOfProjectTemplate)){
        	access = "Project Lead";   //user will be added as Leader for co-owner irrespective of role
        }else{
        	
        	MapList collabspacesMap = (MapList) CacheUtil.getCacheObject(context, fromObjectId);
        	if(collabspacesMap == null || collabspacesMap.isEmpty()){
        		 MapList projectAccessList = DomainAccess.getAccessSummaryList(context, fromObjectId); 
        		 collabspacesMap = new MapList(projectAccessList.size());
        		
                  int accessListLen = projectAccessList.size();
                  for (int i=0; i < accessListLen; i++) {
                      Map map = (Map) projectAccessList.get(i);
                      String name = (String) map.get("name");
                      String org = (String) map.get("org");
                      String project = (String) map.get("project");
                      if (ProgramCentralUtil.isNotNullString(org) || ProgramCentralUtil.isNotNullString(project)) {
                    	  collabspacesMap.add(map);
                      }
                  }
        	}
        	
            if(collabspacesMap!=null){
            	
            	  String assignedRolesAll = MqlUtil.mqlCommand(context, "print person $1 select $2 dump", true, personName, "assignment");
            	  StringList assignedRolesAllList = FrameworkUtil.split(assignedRolesAll, ","); 
            	  ...
            	  for (int j=0; j < collabListLen; j++) {
					  ...
					  String memberAccess = (String)collabMap.get("access");
					  ...
					  String assignment = "ctx::"+ leader+ "."+ name +""; 
					 
					  if(assignedRolesAllList.contains(assignment) && ("All".equalsIgnoreCase(memberAccess) || "Project Lead".equalsIgnoreCase(memberAccess))){
					  	access = "Project Lead";
					  	break;
					  }
            	  }
				  CacheUtil.setCacheObject(context, fromObjectId, collabspacesMap);
            }
        }
        ...
        if(Boolean.valueOf(isKindOfProjectSpace) || Boolean.valueOf(isKindOfProjectConcept) 
			...
			String physicalId = MqlUtil.mqlCommand(context,mqlCmd , fromObjectId,ProgramCentralConstants.SELECT_PHYSICALID ,"|");
			String projectAccess =(String)CacheUtil.getCacheObject(context, "MemberModificationFor_"+physicalId+"_For_"+contextUser);
			if (!"Project Lead".equalsIgnoreCase(access) && "Project Lead".equalsIgnoreCase(projectAccess) ) {
            	access = projectAccess;
			}
            DomainAccess.createObjectOwnership(context, fromObjectId, personId, access, DomainAccess.COMMENT_MULTIPLE_OWNERSHIP,true/*replace*/);   //Will replace existing ownership if already present - IR-1258855
        }
    ...
    public void triggerDeleteAction(Context context, String[] args) throws Exception {
        ...
        if("true".equalsIgnoreCase(isKindOfProjectSpace) || "true".equalsIgnoreCase(isKindOfProjectConcept)
		    ...
		    if("true".equalsIgnoreCase(isKindOfProjectTemplate)){
