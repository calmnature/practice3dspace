...
public class emxProjectHoldAndCancelBase_mxJPO extends emxDomainObject_mxJPO
{
    ...
    throws Exception
    {
        ...
        objectSelects.add("attribute[Route Activity State]");
        objectSelects.add("attribute[Route Status]");
        
		StringList relSelects = new StringList(1);
        relSelects.add("attribute[Route Base State]");
		
        MapList relatedRoutes = gateObj.getRelatedObjects(context, 
                DomainConstants.RELATIONSHIP_OBJECT_ROUTE, //Relationship type 
                DomainConstants.TYPE_ROUTE, //objects type
                objectSelects, //objectSelectable
            ...
            String errMsg1 = MessageUtil.getMessage(context, null,"emxProgramCentral.Gate.errMsg1", messageValues, null, context.getLocale(), "emxProgramCentralStringResource");
                ...
                //Added Newly
                    ...
                    if(relatedRoutes != null && !relatedRoutes.isEmpty()) {
                        for(int i=0;i<relatedRoutes.size();i++){
                            Map tempMap = (Hashtable) relatedRoutes.get(i);
                            //String state = (String) tempMap.get(SELECT_CURRENT);
                            //String routeStatus = (String) tempMap.get("attribute[Route Status]");
                            String routeBaseState = (String) tempMap.get("attribute[Route Base State]");
                            if ("state_Review".equalsIgnoreCase(routeBaseState)) {
                            	mlRoutes.add(tempMap);                            	
                            }
                        }
                        if(mlRoutes.size() > 0) {
                        	return;
            ...
            if (relatedDecisions != null && !relatedDecisions.isEmpty()){
                ...
                if(!"JSP".equalsIgnoreCase(invokedFrom)){
                    ...
                    //
                	CacheUtil.removeCacheObject(context, loggedInUser+"_PromotedTaskIdList");
    ...
    throws MatrixException{
        try{
			...
			String projectTitle= (String) objMap.get(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
            ...
            if (memberList.size() > 0)
            {
                ...
                String i18NewValue = EnoviaResourceBundle.getStateI18NString(context, policyStrNew, newValue, context.getLocale().getLanguage());
                        ...
                        String mKey[] = {"ProjectType", "projectTitle","projectRevision","From","To"};
                String mValue[] = {projectType, projectTitle,projectRevision,i18OldValue,i18NewValue};
    ...
    public boolean isApproveEnabled(Context context,String[] args) throws Exception
    {
        ...
        StringList objectSelects = new StringList();
        objectSelects.add("from[Object Route].to.id");
        objectSelects.add("from[Object Route].attribute[Route Base State]");
        ...
        boolean hasConnectedRoute = false;
    	Map map 	= taskObj.getInfo(context, objectSelects);
    	String projectId 	= (String) map.get(ProgramCentralConstants.SELECT_PROJECT_ID);
    	String routeId 	= (String) map.get("from[Object Route].to.id");
    	String routeBaseState 	= (String) map.get("from[Object Route].attribute[Route Base State]");
    	if(ProgramCentralUtil.isNotNullString(routeId) && "state_Review".equalsIgnoreCase(routeBaseState)) {
    		hasConnectedRoute = true;
    	}
        boolean hasAccess   = hasAccess(context, projectId, objectId,"Approve");

        return hasAccess && !hasConnectedRoute;
    }
