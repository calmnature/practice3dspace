...
public class emxProjectBaselineBase_mxJPO extends DomainObject
{
	...
	public MapList getWBSTasks(Context context, String[] args ) throws Exception{
		...
		String relPattern = ProgramCentralConstants.RELATIONSHIP_SUBTASK;
			...
			//Description Column value addition by AUY2
			objectSelects.addElement(ProgramCentralConstants.SELECT_DESCRIPTION);
	...
	public StringList getNameColumn(Context context,String[]args)throws Exception
	{
		try{
			...
			for(int i=0; i<objectInfoList.size();i++){
				...
				String name = ProgramCentralUtil.getTitleForProjectType(context, taskMap, isKindOfProjectSpace);
				...
				sbLink.append(XSSUtil.encodeForXML(context, name));
				sbLink.append("</a>");
	...
	public StringList getCurrentEstimatedDuration(Context context, String[]args)throws MatrixException{
		try{
			...
			String strI18Weeks = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Weeks", ctxLang);
			String strI18Months = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Months", ctxLang);
			...
			for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
				...
				if("h".equalsIgnoreCase(durationUnit)){
					...
					_duration = _duration / 5;	//1 week = 5 days
					duration = Double.toString(_duration);
					duration= duration + ProgramCentralConstants.SPACE + strI18Weeks;
				} else if("mon".equalsIgnoreCase(durationUnit)){
					_duration = _duration / 20;	//1 month = 20 days
					duration = Double.toString(_duration);
					duration= duration + ProgramCentralConstants.SPACE + strI18Months;
				}else {
	...
	public StringList getTaskDependencyColumn(Context context, String[] args) throws Exception
	{
		...
		selectables.add(ProgramCentralConstants.SELECT_PROJECT_ACCESS_KEY_PHYSICAL_ID_FOR_PREDECESSOR); //External Task/project dependency
		...
		for(int j=0, bsize = bwsl.size(); j < bsize; j++)
		{
			...
			predecessorIds = bws.getSelectDataList(ProgramCentralConstants.SELECT_PREDECESSOR_PHYSICAL_IDS);//getSelectableValues(bws, Task.SELECT_PREDECESSOR_IDS, predecessorIds);
			...
			{
				predecessorTypes = bws.getSelectDataList(Task.SELECT_PREDECESSOR_TYPES);//getSelectableValues(bws, Task.SELECT_PREDECESSOR_TYPES, predecessorTypes);
				predecessorLagTimes = bws.getSelectDataList(ProgramCentralConstants.SELECT_PREDECESSOR_LAG_TIME_INPUT);//getSelectableValues(bws, ProgramCentralConstants.SELECT_PREDECESSOR_LAG_TIME_INPUT, predecessorLagTimes);
				predecessorLagTimeUnits = bws.getSelectDataList(ProgramCentralConstants.SELECT_PREDECESSOR_LAG_TIME_UNITS);//getSelectableValues(bws, ProgramCentralConstants.SELECT_PREDECESSOR_LAG_TIME_UNITS, predecessorLagTimeUnits);
				...
				for (int i=0; i < depedencyList.size(); i++)
				{
					...
					if (predecessorProjectType == null)
					{
                        ...
                        externalProjectSelectableList.add("from[" + RELATIONSHIP_PROJECT_ACCESS_LIST + "].to.attribute[Title]");
						externalProjectSelectableList.add("|");
						
						String sCommandStatement = "print bus $1 select $2 $3 $4 $5 dump $6";
						...
						palMapCache.put("title:"+ projectAccessKeyIdForPredecessor, projectInfo.get(3));
