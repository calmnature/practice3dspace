...
public class emxCommonDocumentBase_mxJPO extends emxDocument_mxJPO {
	...
	/**
	 * This is method is used block delete if web edition is active
	 * this method need to be called from delete action trigger from any DOCUMENTS
	 * type object
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds no arguments
	 *                args[0] should be ${OBJECTID}
	 *                args[1] should be ${POLICY}
	 ...
	 * @grade 0
	 */
	public static int blockDeleteDocumentWhenWebEditionActive(Context context, String[] args) throws Exception {
		try {
			boolean idWebEditionOn = false;
			int status = 0;
			String docId = args[0];
			String policy = args[1];
			String fileId = "";
			String strDeleteErrorWhenWebEditionOn = "";
			...
			if (tenant == null || tenant.isEmpty() || tenant.equalsIgnoreCase( "OnPremise")) {
				System.out.println("tenant is OnPremise");
			}
			else{	
				...
				if(policy.equals("Version")){
					fileId = docId;
					docId = MqlUtil.mqlCommand(context, "print bus $1 select $2  dump ",
						fileId, "to[" + RELATIONSHIP_ACTIVE_VERSION + "].from.physicalid");
				}
				try{
					if(UIUtil.isNotNullAndNotEmpty(docId)){
						Class<?> lClass = Class.forName("com.dassault_systemes.enovia.document.Document");
						Object inst = lClass.newInstance();
						Object[] lMethodArgumentArray = new Object[3];
						lMethodArgumentArray[0] = context;
						lMethodArgumentArray[1] = docId;
						lMethodArgumentArray[2] = fileId;
						Method method = lClass.getMethod("blockDeleteOnWebEdition", matrix.db.Context.class, String.class,String.class);
						idWebEditionOn = (boolean) method.invoke(inst, lMethodArgumentArray);
					        strDeleteErrorWhenWebEditionOn = EnoviaResourceBundle.getProperty(context, EMXCOMPONENTS_STR_RESOURCE,
							new Locale(context.getSession().getLanguage()), "emxComponents.Common.DeleteActionErrorMessageAsWebEditionSessionActive");
					}		
				}catch (Exception e) {
					idWebEditionOn = false;
				}
				
				if(idWebEditionOn)
				{
					emxContextUtilBase_mxJPO.mqlNotice(context, strDeleteErrorWhenWebEditionOn);
					status = 1;
				}	
			}
	...
	public void autoPromoteCollaborativeMode(Context context, String args[]) throws Exception {
		if (CommonDocument.isDocumentStateInWorkOnCreation(context)) {
			...
			if ("Revision".equalsIgnoreCase(event)) {
				String newRev = MqlUtil.mqlCommand(context, "print bus $1 select $2 $3 dump $4", objectId, "last.id",
						"last.policy", "|");
	...
	public int blockPromotionForLockedDocuments(Context context, String[] args) throws Exception {
		...
		String RELATIONSHIP_ACTIVE_VERSION = PropertyUtil.getSchemaProperty(context,
			DomainObject.SYMBOLIC_relationship_ActiveVersion);
			String strPromoteErrorWhenWebEditionOn ="";
		String fileId = "";
		...
		if(tenant == null || tenant.isEmpty() || tenant.equalsIgnoreCase( "OnPremise")) {
			...
			try{
				if(UIUtil.isNotNullAndNotEmpty(oid)){
					...
					Object[] lMethodArgumentArray = new Object[3];
					lMethodArgumentArray[0] = context;
					lMethodArgumentArray[1] = oid;
					lMethodArgumentArray[2] = fileId;
					Method method = lClass.getMethod("blockDeleteOnWebEdition", matrix.db.Context.class, String.class,String.class);
					idWebEditionOn = (boolean) method.invoke(inst, lMethodArgumentArray);
												
					System.out.println("idWebEditionOn : "   +  idWebEditionOn);
					
					 strPromoteErrorWhenWebEditionOn = EnoviaResourceBundle.getProperty(context, EMXCOMPONENTS_STR_RESOURCE,
						new Locale(context.getSession().getLanguage()), "emxComponents.Common.PromoteActionErrorMessageAsWebEditionSessionActive");
				}		
			}catch (Exception e) {
				idWebEditionOn = false;
			}
			if(idWebEditionOn)
			{
				emxContextUtilBase_mxJPO.mqlNotice(context, strPromoteErrorWhenWebEditionOn);
				status = 1;
			}	
		}
	...
	public void getInformationOfDocument(Context context, String[] args) throws Exception {
		try{	
			System.out.println("getInformationOfDocument");
			String objectId = args[0];
			String masterID = "";
			String objPolicy = "";
			String fileId = "";
			...
			String RELATIONSHIP_ACTIVE_VERSION = PropertyUtil.getSchemaProperty(context,
				DomainObject.SYMBOLIC_relationship_ActiveVersion);
			String strEventPublishing=context.getCustomData("EventPublishing");
			...
			if (args.length > 1) {
				...
				if (objPolicy.equalsIgnoreCase("Version") && objectId != null && !"".equals(objectId)) 
				{
					...
					masterID = versionObject.getInfo(context, "last.to[" + RELATIONSHIP_ACTIVE_VERSION + "].from.physicalid");
					boolean publishEvent=(strEventPublishing == null || strEventPublishing.isEmpty() || !strEventPublishing.equalsIgnoreCase("false")) ? true : false;
					System.out.println("publishEvent" + publishEvent);
					if(!(publishEvent))
					{
						//DomainObject versionObject = DomainObject.newInstance(context, objectId);
						//masterID = versionObject.getInfo(context, "last.to[" + RELATIONSHIP_ACTIVE_VERSION + "].from.physicalid");
						if(!UIUtil.isNullOrEmpty(masterID))
						{
							...
							objectInfoString = "objectInfo_".concat(masterID);
							String objectInfo = context.getCustomData(objectInfoString);
							...
							if(!UIUtil.isNullOrEmpty(objectInfo)){
								...
								if(!(slObjectInfo.size()<3)){
									...
									if((fileId != null && !"".equals(fileId))){
									  ...
									  context.setCustomData(objectInfoString, objPolicy + "|" + masterID +"|"+ fileId );
									}
									else{
					...
					{	
						...
						fileInfoString = "fileDocInfo_".concat(fileId);
						context.setCustomData(fileInfoString ,  masterID +"|"+ fileId);
					}
				}
			...
			System.out.println("Failed for objectid : " + args[0]);
			e.printStackTrace();
		}
	}
	public void publishCheckinEvent(Context context, String[] args) throws Exception {
		...
		if(!(strdisableTriggerForDocumentImportOrCreateScenario==null||strdisableTriggerForDocumentImportOrCreateScenario.isEmpty())){ 
			try{
				...
				e.printStackTrace();
				System.out.println("Failed in publishCheckinEvent ");
				System.out.println("Failed for objectid : " + args[0]);
			}
		}else{
	...
	public void publishCheckinEventActual(Context context, String[] args) throws Exception {
		System.out.println("publishCheckinEventActual");
		...
		if(!("fileOnDemand".equals(eventType)))
		{
			...
			{	
				eventType = "deleted";
				objectInfoString ="objectInfo_".concat(objectId);
				String objectInfo = context.getCustomData(objectInfoString);
				System.out.println("objectInfo in  document release : "   + objectInfo);
				StringList slObjectInfo = FrameworkUtil.split(objectInfo, "|");
				if(!(slObjectInfo.size()<3)){
					fileId = slObjectInfo.get(2);
					System.out.println("fileId in document release : "   + fileId);
				}
				context.removeFromCustomData(objectInfoString);
			...
			}else  
			{
				...
				if(publishEvent)
				{
					fileInfoString = "fileDocInfo_".concat(objectId);
					String fileObjInfo = context.getCustomData(fileInfoString);
					StringList getFileInfo = FrameworkUtil.split(fileObjInfo, "|");
					System.out.println("getFileInfo" + getFileInfo);
					if(!(getFileInfo.size()<2)){
						objectId = getFileInfo.get(0);
						fileId = getFileInfo.get(1);
						System.out.println("fileId in version policy  : "   + fileId);
					}
					context.removeFromCustomData(fileInfoString);
				}
			}
		}else if("fileOnDemand".equals(eventType))
		{
			//policy  = MqlUtil.mqlCommand(context, "print bus $1 select $2  dump ", objectId,"policy");
	...
	public void getDocumentCheckinInfo (Context context, String[] args) throws Exception {
		try{
			...
			if (mlist != null && mlist.size() > 0) {
				...
				context.setCustomData("checkinActionInfoForThumbnail", objectId + "|" +  fileId  );
			}
		}catch(Exception ex){
	...
	public void getFileInfoForEventPubishing (Context context, String[] args) throws Exception {
		try{
			...
			if(! policy .equals("Version"))
			{
				...
				context.setCustomData("DOC_REVISE_OR_COPY_FOR_THUMBNAIL", objectId );
			}
		}catch(Exception ex){
			 System.out.println(ex.getMessage());
		}
	...
	public void generateThumbnailOnTransaction(Context context, String[] args) throws Exception {
        ...
        String RELATIONSHIP_ACTIVE_VERSION = PropertyUtil.getSchemaProperty(context,
				DomainObject.SYMBOLIC_relationship_ActiveVersion);
        String checkinActionInfoForThumbnail = context.getCustomData("checkinActionInfoForThumbnail");
		String DOC_REVISE_OR_COPY_FOR_THUMBNAIL = context.getCustomData("DOC_REVISE_OR_COPY_FOR_THUMBNAIL");
		if (UIUtil.isNullOrEmpty(args[0])) {
			return;
		}
        
        if (UIUtil.isNotNullAndNotEmpty(checkinActionInfoForThumbnail))
			{
				StringList slObjectInfo = FrameworkUtil.split(checkinActionInfoForThumbnail, "|");
				objectId = slObjectInfo.get(0);
				fileId = slObjectInfo.get(1);
				checkinDone = true;
				context.removeFromCustomData("checkinActionInfoForThumbnail");
			}
		else if (UIUtil.isNotNullAndNotEmpty(DOC_REVISE_OR_COPY_FOR_THUMBNAIL))
			{
				objectId = DOC_REVISE_OR_COPY_FOR_THUMBNAIL;
				fileId = MqlUtil.mqlCommand(context, "print bus $1 select $2  dump $3 ",
				objectId, "from[" + RELATIONSHIP_ACTIVE_VERSION + "].to.physicalid",",");
					
				if (UIUtil.isNotNullAndNotEmpty(fileId))
				{     System.out.println("check HCY5 inside elseif inner IF");
					checkinDone = true;
				}
				context.removeFromCustomData("DOC_REVISE_OR_COPY_FOR_THUMBNAIL");
			}
		 ...
		 ArrayList fileIdList = new ArrayList();
                ...
                if (UIUtil.isNotNullAndNotEmpty(fileId)) {
                    ...
                    for (int i = 0; i < fileIdList.size(); i++) {
                        String fileIdInfo = (String) fileIdList.get(i);
                        try {
                            String fileName = MqlUtil.mqlCommand(context, "print bus $1 select $2  dump ", fileIdInfo, "attribute[Title]");
                            ...
                            _log.error("Error executing MQL command: {}", e.getMessage());
                            throw e;
                        }
                    }
                    checkinDone = false;
                } else {
                    _log.error("fileId is null or empty, cannot split.");
