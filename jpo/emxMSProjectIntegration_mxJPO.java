 ...
 **  @quickReview 17:10:18 ACE2 IR-557097-3DEXPERIENCER2017x: Calendar exception does not get added to MS Project 
 **   + Handle server date formatting
 *
 */
...
public class emxMSProjectIntegration_mxJPO extends ProjectSpace 
{
	...
	public emxMSProjectIntegration_mxJPO () { super();   /* Do nothing */ }
	...
	public emxMSProjectIntegration_mxJPO(Context context, String[] args) throws Exception 
	{
		...
		projectSelectable = new StringList();//SetProjectTasksMapping(context, false);
		taskSelectable = new StringList();//SetProjectTasksMapping(context, true);
	}
		...
		try
		{
			...
			switch(commandName)
			{
				...
				case "calendarExceptions":
					responseElement = CalendarExceptions(commandElement);
					break;
	...
	public MapList getAllUserProjects(Context context, String[] args) throws Exception
	{
		...
		String busWhere = "type!='" + ProgramCentralConstants.TYPE_PROJECT_SNAPSHOT+"'&&type!='"+ProgramCentralConstants.TYPE_PROJECT_BASELINE+"'&&type!="+ProgramCentralConstants.TYPE_EXPERIMENT; 
		//return getProjects(context, Person.getPerson(context), new Pattern(TYPE_PROJECT_SPACE).getPattern(), busSelects, null, busWhere, DomainConstants.EMPTY_STRING, true);
		return getProjects(context, Person.getPerson(context), new Pattern(TYPE_PROJECT_SPACE).getPattern(), busSelects, null, busWhere, DomainConstants.EMPTY_STRING, true,true);
	}
	...
	/**
	 * Get calendar exception details
	 */
	public Element CalendarExceptions(Element commandElement) throws Exception 
	{
		WorkCalendar workCalendar;
		Element responseElement = new Element("CalendarList");

		for (Element calendar : (List<Element>)commandElement.getChildren("Calendar")) 
		{
			String calendarId = calendar.getText();
			Element calendarElement = AddElementWithAttributes(responseElement, "Calendar", "oid", calendarId);
			Element calendarExceptions = AddElements(calendarElement, "ExceptionList");

			for (Map exceptionMap : (List<Map>)new WorkCalendar(calendarId).getExceptions(context)) 
			{
				Element calendarException = AddElementWithAttributes(calendarExceptions, "Exception", 
					"oid", (String)exceptionMap.get(DomainObject.SELECT_ID),
					"title", (String)exceptionMap.get(WorkCalendar.SELECT_TITLE), 
					"exceptionType", (String)exceptionMap.get("attribute[Calendar Exception Type]"), 
					"frequency", FormatAsProjectString(exceptionMap.get(WorkCalendar.SELECT_FREQUENCY))
				);

				AddElements(calendarException,
					"Start_Date",  FormatAsProjectDate(exceptionMap.get(WorkCalendar.SELECT_START_DATE)),
					"End_Date", FormatAsProjectDate(exceptionMap.get(WorkCalendar.SELECT_END_DATE))
				);

				AddElements(calendarException,
					"Day_Of_Week", FormatAsProjectString(exceptionMap.get("attribute[Day Of Week]")),
					"Days_Of_Week", FormatAsProjectString(exceptionMap.get("attribute[Days Of Week]")),
					"Day_Of_Month", FormatAsProjectString(exceptionMap.get("attribute[Day Of Month]")),
					"Week_Of_Month", FormatAsProjectString(exceptionMap.get("attribute[Week Of Month]")),
					"Month_Of_Year", FormatAsProjectString(exceptionMap.get("attribute[Month Of Year]"))
				);
				...
				String workingTimes =  FormatAsProjectString(exceptionMap.get(WorkCalendar.SELECT_WORKING_TIMES));
				...
				AddElements(calendarException,
						"Work_StartTime", FormatAsProjectString(workStartTime),
						"Work_FinishTime", FormatAsProjectString(workFinishTime),
						"Lunch_StartTime", FormatAsProjectString(firstRecessStartTime),
						"Lunch_FinishTime", FormatAsProjectString(lastRecessFinishTime),
						"Working_Times", FormatAsProjectString(workingTimes)
					);
			}
		}
...
*/
	...
	/*private void SetProjectDetails(Element projectElement, Map detailsMap, int parentIndex, boolean isSummary) throws Exception
	{
		...
		if (isSummary)
		{
			...
			{	
				AddElements(projectElement, "ActualStart", FormatAsProjectDate(detailsMap.get(SELECT_TASK_ACTUAL_START_DATE)));
				AddElements(projectElement, "ActualFinish", FormatAsProjectDate(detailsMap.get(SELECT_TASK_ACTUAL_FINISH_DATE)));
	...
	private void UpdateTaskData(String projectType, HashMap<String, String> resourceUIDs) throws Exception
	{
		...
		multiValueSelectables.add(SELECT_TASK_ASSIGNEE_ID);
		multiValueSelectables.add(SELECT_ASSIGNEE_ALLOCATION);
		
		DecimalFormat unitsFormat = new DecimalFormat("0.##");
	
		for (Map taskDetailsMap : (List<Map>)DomainObject.newInstance(context).getInfo(context, taskIdElementMap.keySet().toArray(new String[taskIdElementMap.size()]), busSelects, multiValueSelectables)) 
		{
	...
	private String ExecuteMergeProject(Element projectElement, boolean isRootProject, Map<String, String> createdIDs, String userLanguage, String deletedTasks, String deletedProjects, ArrayList<String> invalidResources, boolean parentExists) throws Exception
	{
		...
		for(Map settings : projectColAttrMap.values()) 
		{
			...
			String columnValue = GetElementValue(projectElement, columnName);
			 ...
			 if (null == columnValue || (!existingProject && columnValue.isEmpty()) || (existingProject ? basicfieldsExisting : basicfields).contains(columnName) || "true".equals(settings.get("MPIIgnoreForServer")) || columnName.equals("WBS") || columnName.equals("OutlineNumber"))
			{ 
				continue; 
			}
			...
			if(columnName.equals("ScheduleFrom")) 
			{
				...
				attributes.put(ATTRIBUTE_DEFAULT_CONSTRAINT_TYPE, columnValue.equals("True") ? "As Soon As Possible" : "As Late As Possible");
	...
	private void ExecuteMergeTask(Element taskElement, Map<String, String> createdIDs, String taskType, boolean existingProject, boolean isRootProject) throws Exception
	{
		try
		{
			...
			for(Map settings : taskBusColAttrMap.values()) 
			{
				...
				if ("user".equals(type) && null != columnValue) { columnValue = GetUserFromLastNameFirstName(columnValue, null, null);  }
				...
				if(null != columnValue && (existingTask || !columnValue.isEmpty()) && !skip && null != (selectable = (String)settings.get("BusinessExpression")))
				{
					...
					if ("current".equals(selectable)) { skip = true; if (!columnValue.isEmpty()) { try { task.setState(context, columnValue); } catch(Exception ex) {  throw columnValue.equals(task.STATE_PROJECT_SPACE_COMPLETE) ? new Exception(ex.getMessage() + "\nNotice: Cannot set state " + columnValue + "; Please Take appropriate actions manually to Complete the Gate") : ex; } } }
...
/*	private void GetTypeSpecificTaskSelectables(String typeName, String isExperiment, StringList taskBusSelect, StringList taskRelSelect) throws Exception
	 ...
	 */
	/*private void DeleteTasks(String[] removedList, String parentId, StringList busSelects , boolean isProject) throws Exception 
	{
	...
	private Element AddElementWithAttributes(Element parent, String name, String... attributeData)
	{
		Element element = AddElements(parent, name);

		for (int i = 0; i < attributeData.length; i += 2)
		{
			if (i + 1 < attributeData.length) { element.setAttribute(attributeData[i], attributeData[i + 1]); }
		}
	...
	{
		boolean existingProject = null != projectId && !projectId.isEmpty() && "true".equalsIgnoreCase(MqlUtil.mqlCommand(context, "print bus $1 select exists dump",  projectId));
	...
	private String FormatAsProjectDate(Object dateContent)
	{
		return (null == dateContent || ((String)dateContent).isEmpty()) ? DomainConstants.EMPTY_STRING : dateFormat.format(MATRIX_DATE_FORMAT.parse((String)dateContent, new java.text.ParsePosition(0)));
	}

	private String FormatAsProjectString(Object content)
	{
		return content != null ? (String)content : DomainConstants.EMPTY_STRING;
	}
	...
	/*private String GetUserFromLastNameFirstName(String userLastNameFirstName, String uid, ArrayList<String> invalidResources) throws Exception 
	{
		...
		if (null == userNameIdMap.get(userLastNameFirstName) && userLastNameFirstName.indexOf(" ") != -1) 
		{
			...
			for (Map user : (List<Map>)findObjects(context, TYPE_PERSON, "*", "*", "*", "*" , whereClause, false, busSelects)) 
			{
