...
public class emxBuildBase_mxJPO extends emxProductFile_mxJPO {
	...
	/**
	 * This method added for JPO To JAR Conversion as an overloaded API. It will Get the list of all builds under a context.
	 *
	 * @param context
	 *            the eMatrix <code>Context</code> object
	 * @param args
	 *            holds the HashMap containing the following arguments: paramList
	 *            - HashMap containing the Object Id and relationship name
	 * @param strWhereCondition
	 *            a String containing the where condition
	 * @param strRelationshipName
	 ...
	 *             if the operation fails
	 */
	public List getRelatedBuilds(Context context, String[] packedArgs) throws Exception {
			
		HashMap programMap = (HashMap) JPO.unpackArgs(packedArgs);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		String strWhereCondition = (String) paramMap.get("strWhereCondition");
		String strRelationshipName = (String) paramMap.get("strRelationshipName");
		String[] args = (String[]) paramMap.get("args");
		List relObjIdList = getRelatedBuilds(context, strWhereCondition, strRelationshipName, args);
		// Returning the Maplist of Object Ids
		return relObjIdList;
	}
	...
	public static MapList getStructureList(Context context, String[] args)
    	...
    	mArgs.put("paramString", objectType);
    	String[] newArgs = JPO.packArgs(mArgs);
    	String objectParentType = (String) JPO.invoke(context, "emxPLCCommon", null, "getParentType", newArgs, String.class);
		/*String objectParentType = ${CLASS:emxPLCCommon}.getParentType(context,
				objectType);*/
		...
		if (objectParentType != null
			...
			/*buildStructList = (MapList) ${CLASS:emxPLCCommon}
					.getStructureListForType(context, args);*/
			buildStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
		}
	...
	public int updateProduct(Context context, String[] args) throws Exception {
		...
		try {
			...
			//IR-1350020-3DEXPERIENCER2023x - Unable to attach a Model Version to a Hardware build in the Released state from the interface
			//Need to push context on connect/disconnect/attribute update
			//Regarding states: the remove/deleteBuildRule has maturity check		
	...
	public StringList getAllContextBuilds(Context context, String[] strBuildIdInArgs) throws Exception {
		
		Map paramMap = (HashMap) JPO.unpackArgs(strBuildIdInArgs);
    	String strBuildId = (String) paramMap.get("paramString");
		DomainObject ctxtBuildObj = new DomainObject(strBuildId);
		
		return getAllContextBuilds(context,	ctxtBuildObj, new StringList());
	}
	...
	public StringList filterContextAndRelatedBuildsForIPU(Context context,
		...
		for (int i = 0; i < listIPU.size(); i++) {
			...
			if (((String) map.get(DomainConstants.SELECT_TYPE))
				...
				list.add(map.get(DomainConstants.SELECT_ID).toString());
			}
		}
		return list;
	}
	...
	public String getBuildProduct(Context context, String[] args)
		...
		if (strObjectId != null) {
			...
			strProductName = domBuild.getInfo(context, "to["
					+ ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD
					+ "].from.name");
			String strProductRevision = domBuild.getInfo(context, "to["
					+ ProductLineConstants.RELATIONSHIP_PRODUCT_BUILD
					+ "].from.revision");
			strProductName += " "+strProductRevision;
		...
		if ((strMode != null && !strMode.equals("")
			...
			if (strProductId != null && !strProductId.equals("")) {
				...
				sbBuffer.append("images/iconSmallModel.png");
				sbBuffer.append("\"</img>");
				sbBuffer.append("</A>");
	...
	public void assignBuildUnitNumberAndShorthandNotation(Context context,
		...
		try {
			...
			arg[0] = strModelId;
			/*${CLASS:emxModel} objModel = new ${CLASS:emxModel}(context, arg);*/
			DomainObject objModel =new DomainObject(strModelId);
			...
			if (strUnitNumber == null
				...
				/*int iUnitNumber = objModel.getLastUnitNumber(context,
						strModelId);*/
				Map mArgs = new HashMap();
		    	mArgs.put("paramString", strModelId);
		    	String[] newArgs = JPO.packArgs(mArgs);
				int iUnitNumber = JPO.invoke(context, "emxModel", null, "getLastUnitNumber", newArgs);
				...
				JPO.invoke(context, "emxModel", null, "setLastUnitNumber", arrParameters, null);
	...
	public int checkForBuildDeleteRules(Context context, String[] args)
		...
		// Added for Frozen State Mapping of Product function
		/*
		...
		*/
		...
		//FUN106899 - Manage Units definition: Builds are allowed to be removed from Review / Release Model Version
        String POLICY_PRODUCT = PropertyUtil.getSchemaProperty(context,"policy_Product");
        String sObsolete = FrameworkUtil.lookupStateName(context, POLICY_PRODUCT, "state_Obsolete");
		        
		if (sObsolete.equals(prodState)) {

			strProdAlertMessage = EnoviaResourceBundle.getProperty(context,SUITE_KEY,
					"emxProduct.Alert.DeleteUnits",language);
			// new MQLCommand().executeCommand(context, "notice '" +
			// strProdAlertMessage + "'");
	...
	public int checkForBuildRemoveRules(Context context, String[] args)
        ...
        */
		...
		if (isProdObsolete) {
			String language = context.getSession().getLanguage();
			strProdAlertMessage = EnoviaResourceBundle.getProperty(context,SUITE_KEY,
					"emxProduct.Alert.RemoveUnits",language);
			// new MQLCommand().executeCommand(context, "notice '" +
			// strProdAlertMessage + "'");
			MqlUtil.mqlCommand(context, "notice $1", strProdAlertMessage);
			return 1;
	...
	public StringList getSerializedParts(Context context, String[] args)
		...
		try {
			...
			String strPartMarkingFromID = "to["+ProductLineConstants.RELATIONSHIP_PART_MARKING +"].from.id";
			StringList objSelect = new StringList(1);
			objSelect.addElement(strPartMarkingFromID);
			
			MapList buildsList = DomainObject.findObjects(context,
					ProductLineConstants.RELATIONSHIP_PART_MARKING, DomainConstants.QUERY_WILDCARD,
					sWhereExp, objSelect, objSelect);
						
			for (int i = 0; i < buildsList.size(); i++) {
				Map buildMap = (Map) buildsList.get(i);
				Object obj = (Object) buildMap.get(strPartMarkingFromID);
				if(obj instanceof StringList){
					sList.addAll((StringList) obj);
				}
	...
	public boolean isCFPInstalled(Context context, String[] args)
			throws Exception {
		if(FrameworkUtil.isSuiteRegistered(context,
				"appVersionDMCPlanning", false, null, null) || FrameworkUtil.isSuiteRegistered(context,
	...
	public StringList isShorthandNotationEditable(Context context, String[] args)
		...
		try {
			...
			for (int i = 0; itr.hasNext(); i++) {
				...
				build.setId(id);
				resultList.add(String.valueOf(build.isProductBuild(context)));
			}
			return resultList;
		} catch (Exception e) {
			e.printStackTrace();
