...
public class emxProjectBudgetBase_mxJPO extends emxFinancialItemBase_mxJPO
{
	...
	public static final String SELECT_IS_TYPE_BUDGET = "from["+ RELATIONSHIP_PROJECT_FINANCIAL_ITEM+"].to.type.kindof["+TYPE_BUDGET+"]";
	...
	public void createTemplateBudget(Context context, String[] args)  throws Exception
	{
		try{
			...
			if(dmoProjectSpace.isKindOf(context, DomainObject.TYPE_PROJECT_TEMPLATE)){
				...
				// Commented out to the following lines as we are creating the ownership while connecting budget to Project Template through action trigger.
	...
	public void createProjectBudget(Context context, String[] args)  throws Exception
	{
		try
		{
			...
			//START : IR-1372841-3DEXPERIENCER2026x
			DomainObject dmoProjectSpace = DomainObject.newInstance(context,strProjectSpaceId);
			StringList isBudgetTypeList = dmoProjectSpace.getInfoList(context,SELECT_IS_TYPE_BUDGET) ;
			int countBudget = 0;
			for(String isBudget : isBudgetTypeList) {
				if("true".equalsIgnoreCase(isBudget)) {
					countBudget++;
				}
			}
			if(countBudget > 1) {
				...
				String sErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral", 
						"emxProgramCentral.ProjectBenefit.ProjectAlreadyHasBudget", strLanguage);
				throw new MatrixException(sErrMsg);
			}
			//END : IR-1372841-3DEXPERIENCER2026x
...
//			MqlUtil.mqlCommand(context, sCommandStatement,budgetId, budgetName,revision); 
	...
	public MapList getTableExpandChildTemplateBudgetData(Context context,String[] args) throws Exception
	{
		...
		try {
			Map programMap = (HashMap) JPO.unpackArgs(args);
			mlCostCategoryList = getTableTemplateBudgetData(context, args);
			if(mlCostCategoryList.size() > 0) {
				Map mpBudget = (Map)mlCostCategoryList.get(0);
				programMap.put("objectId",(String)(mpBudget.get(DomainConstants.SELECT_ID)));
				String[] arrJPOArguments    = new String[1];
				...
				mlCostItemCategoryList =  getTableExpandChildProjectBudgetBenefitData(context,args,true);
	...
	public String getTimeLineInterval(Context context, String args[],boolean istimeLineIntervalFrom) throws Exception
	{
		...
		if(mlPhaseList.size()>0) {
			strHTMLBuffer.append("<img src='../common/images/iconSmallCalendar.gif' border='0' valign='absmiddle' name='timelineImg' hidden='true'/>");
		}else {
			strHTMLBuffer.append("<img src='../common/images/iconSmallCalendar.gif' border='0' valign='absmiddle' name='timelineImg'/>");
		}
	...
	public Object getBudgetCostItemsRange(Context context,String[] args) throws Exception
	{
		...
		if(objectId != null && !"".equals(objectId)){
			...
			for (Iterator itr=costItemList.iterator();itr.hasNext();) 
			{
				...
				fieldRangeValues.add((String)objectMap.get(DomainObject.SELECT_ID));
...
//				fieldDisplayRangeValues.addElement(objectMap.get(DomainObject.SELECT_NAME));
				fieldRangeValues.add((String)objectMap.get(DomainObject.SELECT_ID));
				fieldDisplayRangeValues.add((String)objectMap.get(DomainObject.SELECT_NAME));
			}
			tempMap.put("field_choices", fieldRangeValues);
		...
		if(strTransactionId != null && !"".equals(strTransactionId)){
			...
			if(strCostItemId != null && !"".equals(strCostItemId) && !"null".equals(strCostItemId)){
				...
				if(CostItem.QUARTERLY.equals(strCostInterval)){
					...
					SimpleDateFormat sdf = new SimpleDateFormat(eMatrixDateFormat.getEMatrixDateFormat());
					strFormattedDisplayDate = sdf.format(cal.getTime());
					//strFormattedDisplayDate = dateFormat.format(cal.getTime());
					relWhere = CostItemIntervalRelationship.SELECT_INTERVAL_DATE + "== \""+ strFormattedDisplayDate + "\"";
...
tempStateList.add(ProjectTemplate.STATE_PROJECT_TEMPLATE_DRAFT);
		tempStateList.add(ProjectTemplate.STATE_PROJECT_TEMPLATE_IN_APPROVAL);
	...
	public boolean isDeleteCostItemAvailable(Context context,String args[]) throws Exception
	{
		...
		if(null != isRMB && "true".equalsIgnoreCase(isRMB.trim())){
			...
			if(ProgramCentralUtil.isNotNullString(sTempObjId)){
				...
				if(ProgramCentralUtil.isNotNullString(sBudgetObjId)){	
					...
					final String SELECT_IS_TYPE_PROJECT_SPACE = "to["+RELATIONSHIP_PROJECT_FINANCIAL_ITEM+"].from.type.kindof["+TYPE_PROJECT_SPACE+"]";
					
					StringList slBudgetSelects = new StringList();
					slBudgetSelects.add(SELECT_IS_BUDGET);
					slBudgetSelects.add(SELECT_PROJECT);
					slBudgetSelects.add(SELECT_IS_TYPE_PROJECT_SPACE);
					...
					if(null != mapBudgetInfo){
						...
						if(null != isBudget && "true".equalsIgnoreCase(isBudget.trim())){
							...
							isTypeProjectSpace = (String)mapBudgetInfo.get(SELECT_IS_TYPE_PROJECT_SPACE);
		...
		try{
			if(bBudget){
				...
				String  selecteView = (String)mapProgram.get(BUDGETVIEWFILTER);
				String type = dmoProject.getType(context);
				...
				//--- Changes for IR-622824 ---//
				//--  added to handle Delete cost item command for Project Space 
				if( (isTempEnforced == false) && (isTypeProjectSpace.equalsIgnoreCase("TRUE")) ){
					result = false;
				}else{
	...
	public boolean isAddCostItemAvailableTemplate(Context context,String args[]) throws Exception
	{
		...
		Map ObjectInfo = (Map)dObjTemplate.getInfo(context, busSelect);
		
		String state = (String)ObjectInfo.get(ProgramCentralConstants.SELECT_CURRENT);
		...
		ProjectTemplate projectTemplate = (ProjectTemplate)DomainObject.newInstance(context, DomainConstants.TYPE_PROJECT_TEMPLATE, DomainObject.PROGRAM);
 		...
 		boolean hasAccess = "True".equalsIgnoreCase(isProjectTemplate) && ("Draft".equalsIgnoreCase(state) || "In Approval".equalsIgnoreCase(state)) && isBudgetCreated(context,args) && hasEditAccess && isCtxUserOwnerOrCoOwner;
		return hasAccess;
	}
	...
	public StringList getTransactionValue(Context context, String[] args) throws MatrixException
	{
		...
		try {
			...
			String objectId = (String)paramMap.get("projectId");
			MapList objectList = (MapList) programMap.get("objectList");
			Currency currency = new Currency();
			...
			if(ProgramCentralUtil.isNullString(strPreferredCurrency) || "As Entered".equalsIgnoreCase(strPreferredCurrency)){
				strPreferredCurrency = strBaseCurrency;
			}
			...
			for (Iterator iterator = transactionAmountInfo.iterator(); iterator.hasNext();) {
				...
				try{
					dblTransactionAmount = convertAmount(context, dblTransactionAmount, strTransactionAmountUnit, strPreferredCurrency, new Date(),true);
				}catch(Exception e){
					isNoticeRequired = false;
				}
				strTransactionAmount = Currency.format(context, strPreferredCurrency, dblTransactionAmount);
	...
	public Map preProcessCheckForEdit (Context context, String[] args) throws Exception{
        ...
        if(ProgramCentralUtil.isNotNullString(language)) {
            language = context.getSession().getLanguage();
        }
		String errorMsg   = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.WBS.NoEdit", language);
	...
	/** To exclude the members who doesn't have DPM license in MultiOwnershipAccess page
     * @param context
     * @param args
    ...
    public StringList getExcludePersonList(Context context, String args[]) throws Exception {
    	...
    	emxDomainAccessBase_mxJPO domainAccessJPO = new emxDomainAccessBase_mxJPO(context, null);
    	StringList existingPersonList = domainAccessJPO.getExclusionList(context, args, "Person");
    	
    	//Exclude already existing user on MOA Page.
    	if(!existingPersonList.isEmpty()){
    		excludeIds.addAll(existingPersonList);
    	}
    	...
    	//Find all person 
    	MapList mlPersonList = DomainObject.findObjects(context,
    			"Person",
				null,
				null,
				busSelects);
    	
    	int personSize = mlPersonList.size();
    	for(int i=0; i<personSize; i++){
    		Map personMap = (Map) mlPersonList.get(i);
    		String personId = (String) personMap.get(SELECT_ID);
