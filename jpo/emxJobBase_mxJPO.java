...
public class emxJobBase_mxJPO extends emxDomainObject_mxJPO
{
    ...
    throws FrameworkException
  {
		...
		if(PersonUtil.isVPLMAdmin(context, PersonUtil.getActiveSecurityContext(context))){
        	user = "*";
        }
		...
		final String  sSuperUser      =  PropertyUtil.getSchemaProperty("person_UserAgent");
        ...
        {
            whereExp.append(" (owner!='creator' && owner!='"+sSuperUser+"')");
        }
		...
		String WhereExpress= whereExp.toString();
        WhereExpress = FrameworkUtil.getWhereExpressionForMql(context,WhereExpress,typeJob);
...
*/
    ...
    throws MatrixException
  {
    ...
    public Map getCommandData(Context context,String jobId,String commandName) throws Exception
    {
        ...
        resultMap.put("HREF",XSSUtil.encodeForURL(nextHREF));
    ...
    public void delete(Context context,String[] args) throws FrameworkException
    {
    	...
    	try
    	{
    		...
    		for(int index=0;index<jobIds.length;index++)
    		{
    			...
    			{
    				objectIdsNotDeleted.add((String)resMap.get(DomainConstants.SELECT_NAME));
    			}
    		}
