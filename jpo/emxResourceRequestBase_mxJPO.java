...
public class emxResourceRequestBase_mxJPO extends emxDomainObjectBase_mxJPO 
{
	...
	protected static final String SELECT_RESOURCE_POOL_TITLE = "from["+RELATIONSHIP_RESOURCE_POOL+"].to.attribute[Title]";
	...
	public static Map projectSpaceResourceRequestInfo = new HashMap();
	public static StringList updatedRequestList = new StringList();
	 ...
	 * @throws Exception if operation fails
	 */
	//not found on PMCResourceRequestLifecycleFilter, PMCResourcePlan, PMCResourceRequestPhaseTimelineFilter & present in mode=EditPagedelete on PMCEditPageDeleteRequest but command not found 
...
//			ResourceRequest.tableValidateCurrency(context,mlRequests);
	...
	private void getTableResourcePlanUpdatePersonFTEMap(Map mapRequestInfo) throws MatrixException {
		...
		if(mapRequestInfo.get(SELECT_PERSON_FTE_FROM_RESOURCE_REQUEST)!=null)
		{
			...
			StringList strPersonFTE = new StringList();
			if (mapRequestInfo.get(SELECT_PERSON_FTE_FROM_RESOURCE_REQUEST) instanceof String) {
				strPersonFTE.addElement((String)mapRequestInfo.get(SELECT_PERSON_FTE_FROM_RESOURCE_REQUEST));
			}else{
				strPersonFTE = (StringList)mapRequestInfo.get(SELECT_PERSON_FTE_FROM_RESOURCE_REQUEST);
			}
			...
			Map mapRequestInfo, FTE fte, String strResourcePlanPrefMode, String strObjectType, String strRequestId) throws MatrixException 
	{
		try {
			...
			{
				if(ProgramCentralUtil.isNotNullString((String) mapRequestInfo.get(SELECT_REL_ATTRIBUTE_FTE)))
				{
					...
					throws Exception, FrameworkException 
	{
		...
		busSelect.add(ProgramCentralConstants.SELECT_IS_PERSON);
		busSelect.add(SELECT_IS_RESOURCE_REQUEST);
		...
		busSelect.add(SELECT_RESOURCE_PLAN_FTE);
	...
	public MapList getTableResourcePlanTimelineRequestData(Context context, String[] args) throws Exception
	{
		try
		{
			...
			for (Iterator iterRequest = mlRequests.iterator(); iterRequest .hasNext();)
			{
				...
				mapRequestInfo.put("CurrentState", mapResourceRequestTranslatedStates.get(strStateValue));
				mapRequestInfo.put("ViewBy", strFilterValue);
				getTableResourcePlanUpdateFTEMap(context, strProjectID, strFilterValue,mapRequestInfo, fte, RESOURCE_PLAN_PREFERENCE_TIMELINE,TYPE_RESOURCE_REQUEST, strRequestId);	
				if(mapRequestInfo.get(SELECT_PERSON_ID_FROM_RESOURCE_REQUEST)!=null)
				{
					mapRequestInfo.put("PersonId", mapRequestInfo.get(SELECT_PERSON_ID_FROM_RESOURCE_REQUEST));
				}
	...
	{ 
		try
		{
			...
			final String SELECT_REL_RESOURCE_PLAN_ATTRIBUTE_FTE = "to[" + RELATIONSHIP_RESOURCE_PLAN + "].attribute[" + ATTRIBUTE_FTE + "].value";
			...
			slBusSelect.add(SELECT_IS_RESOURCE_REQUEST);
			...
			FTE fte = null;
			MapList mlSubmittedResourceRequestsList = new MapList();
			for (Iterator iterRequest = mlResourceRequestsList.iterator(); iterRequest .hasNext();)
			{
				...
				if(ProgramCentralUtil.isNotNullString(strStateValue) && !(strStateValue.equalsIgnoreCase(STATE_RESOURCE_REQUEST_CREATE)))
				{
					mlSubmittedResourceRequestsList.add(mapRequestInfo);
				}
				...
				mapRequestInfo.put("CurrentState", mapResourceRequestTranslatedStates.get(strStateValue));
				if(mapRequestInfo.get(SELECT_RESOURCE_PLAN_FTE) != null )
				{
					fte = FTE.getInstance(context, (String)mapRequestInfo.get(SELECT_RESOURCE_PLAN_FTE));
					mapRequestInfo.put("FTE", fte);
				}
				Object objPersonFTEType = null;
				if(mapRequestInfo.get(SELECT_PERSON_ID_FROM_RESOURCE_REQUEST)!=null)
			...
			ResourceRequest.tableValidateCurrency(context,mlSubmittedResourceRequestsList);				   

			return mlSubmittedResourceRequestsList;
		} 
		catch (Exception exp) 
		{
			throw new MatrixException(exp);
	...
	public Vector getColumnSkillCompetencyData(Context context, String[] args) throws Exception
	{
		...
		while (objectListIterator.hasNext())
		{
			...
			String isResourceRequest = (String) mapObject.get(SELECT_IS_RESOURCE_REQUEST);
			String isPerson = (String) mapObject.get(ProgramCentralConstants.SELECT_IS_PERSON);
			...
			else if ("true".equalsIgnoreCase(isResourceRequest))
			{
	...
	{
		projectSpaceResourceRequestInfo.clear();
		...
		if(!isTypeProjectTemplate && !RESOURCE_PLAN_PREFERENCE_PHASE.equalsIgnoreCase(strFilterValue))
		{
			...
			istypeProjectSpace = (boolean) mapDate.get("isTypeProjectSpace");
			Calendar calStartDate = Calendar.getInstance();
			...
			if("Monthly".equals(strTimeframeConfigName)){ 
				for(int i=1;i<13;i++){
					MonthName[i]= getMonthName(context,i);
				}
			}
	...
	public StringList getFTEStyleInfo(Context context, String[] args)  throws Exception 
	{
		try 
		{
			...
			{
				mapObjectInfo = (Map) objectList.get(i);
				Double dAllChildFTEValues = 0D;
				String strType = "";
				if(null!=mapObjectInfo)
				{
					strType = (String)mapObjectInfo.get(SELECT_TYPE);
					String strRequestState = (String)mapObjectInfo.get(SELECT_CURRENT);
					if(null != (mapObjectInfo.get(SELECT_RESOURCE_PLAN_FTE)) && !"".equals(mapObjectInfo.get(SELECT_RESOURCE_PLAN_FTE))){
						if(null != (mapObjectInfo.get("FTE")) && !"".equals(mapObjectInfo.get("FTE"))){
							fte =  (FTE) mapObjectInfo.get("FTE");
						}else{
						String strFTE = (String)mapObjectInfo.get(SELECT_RESOURCE_PLAN_FTE);
						fte = FTE.getInstance(context, strFTE);
						}
	...
	public Vector getColumnFTEData(Context context, String[] args)  throws Exception {
		try {
			...
			if("Costs".equalsIgnoreCase(strCostsFTEFilter)){
				int fteSize=getColumnFTECostValue(context,programMap).size();
				vecResult= (String[]) getColumnFTECostValue(context,programMap).toArray(new String[fteSize]);

			}
			else{
				int fteSize=getColumnOnlyFTEValue(context,programMap).size();
				vecResult=(String[]) getColumnOnlyFTEValue(context,programMap).toArray(new String[fteSize]);
			}

			int size=vecResult.length;
			String separator = EnoviaResourceBundle.getProperty(context, "emxFramework.DecimalSymbol");
			for(int i=0;i<size;i++)
			{
				String sRetValue=(String) vecResult[i];
				if(sRetValue.contains("."))
				{
					sRetValue = sRetValue.replace('.',separator.charAt(0));
                }
				returnVector.add(sRetValue);
				}            	
	...
	private MapList getResourceForExpand(Context context,String[] args) throws Exception
	{
		...
		for (Iterator itrRequestPerson = mlRequestPersonList.iterator(); itrRequestPerson.hasNext();)
		{
			...
			mapRequestPersonInfo.put(ProgramCentralConstants.SELECT_IS_PERSON, "true");
		}
	...
	public Vector getColumnStateData(Context context, String[] args)throws Exception 
	{
		try 
		{
			...
			for (Iterator itrStates = objectList.iterator(); itrStates.hasNext();) 
			{
				...
				{
					strPolicy = (String)mapRowData.get(SELECT_POLICY);
					strState = (String)mapRowData.get(SELECT_CURRENT);
					if(ProgramCentralUtil.isNullString(strState)||ProgramCentralUtil.isNullString(strPolicy)){
						DomainObject dmoObject = DomainObject.newInstance(context,strObjectId);
...
//		}
	...
	public Vector getColumnOrganisationData(Context context, String[] args)throws Exception 
	{
		try 
		{
			...
			StringList slOrganizationTitle = new StringList();
			...
			for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) 
			{
				...
				if(null!=mapOrgData)
				{
					...
					slOrganizationTitle = (StringList)mapOrgData.get("OrganizationTitle"); // To get the organization title instead of name
					...
					for(int i=0;i<nTotalResourcePools;i++)
					{
						...
						String organizationTitle = (String)slOrganizationTitle.get(i);
						...
						if(ProgramCentralUtil.isNullString(organizationTitle)) {
							organizationTitle = strOrganization;
						}
						...
						if(!isPrinterFriendly){
							...
							sbSubstanceLink.append(XSSUtil.encodeForXML(context,organizationTitle));
							sbSubstanceLink.append("</a>");
							slPersonResourcePools.add(sbSubstanceLink.toString());
						}else{
							slPersonResourcePools.add(sbSubstanceLink.append(organizationTitle).toString());
						}
	 ...
	 * @throws Exception if operation fails
	 */
	//program on command PMCCreateResourcePlanFromWBS but Access Expression of the command is false therefore commented out
	@com.matrixone.apps.framework.ui.ProgramCallable
...
//			} 
	 ...
	 * @throws Exception if operation fails
	 */
	//access expression on PMCCreateResourcePlanFromWBS is false and so table PMCResourceRequestCreateWBSTable is not visible
...
//			return new ResourceRequest(strResourceRequestId);
	 ...
	 * @since AEF 9.5.1.3
	 */
	public void notifySubmitRequest(Context context, String[] args)throws Exception {
		...
		StringList objectIdList = new StringList((String)objMap.get(SELECT_ID)); 
		
		String strResourcePoolID          = (String) objMap.get(SELECT_RESOURCE_POOL_ID);
		DomainObject dmoResourcePool = DomainObject.newInstance(context,strResourcePoolID);
		StringList slManagers = dmoResourcePool.getInfoList(context, SELECT_RESOURCE_MANAGER_NAME);
		...
		for(int i =0; i<slManagers.size(); i++){
			mailToList.add(slManagers.get(i));
		}
		...
		emxNotificationUtilBase_mxJPO.sendJavaMail(context, mailToList, null, null, sMailSubject, sMailMessage.toString(),
				null, fromAgent, null, objectIdList, "both");
	...
	public int notifyRejectRequest(Context context, String[] args) throws Exception {
		...
		if(strNextStateName.equals(strtRejectedState)){
			...
			try{
				...
				objectIdList.add((String)objMap.get(SELECT_ID));
			...
			emxNotificationUtilBase_mxJPO.sendJavaMail(context, mailToList, null, null, sMailSubject, sMailMessage.toString(),
					null, strPersonName, null, objectIdList, "both");
		}
	 ...
	 */
	//Project Role field / column on all components is disabled i.e. on PMCResourceRequestCreateForm, PMCProjectMemberSummary, PMCResourceRequestByPhaseCreateForm,
...
//			String strOuput = output.toString();
	...
	throws Exception, FrameworkException {
		...
		strHTMLBuffer.append("<input type='text' readonly='readonly' size='' name='").append(rowName).append("' value=\'"
				+ XSSUtil.encodeForXML(context,strDate) +"'/>");
...
//		select.add(DomainConstants.SELECT_ID);
//		select.add(DomainConstants.SELECT_NAME);
//		mapUserList =DomainObject.findObjects(context,PropertyUtil.getSchemaProperty(context,"type_BusinessSkill"),null,null,select);
//
...
//		}
	 ...
	 * @throws Exception if operation fails
	 */
	// code commented out as the function is on Discussion column of PMCResourceRequestSummaryTable and the column's Access Expression value is false
...
//				${CLASS:emxSubscriptionUtil} SubUtil = new ${CLASS:emxSubscriptionUtil}(context,args);
...
//		}
	...
	public Vector getColumnPersonIcon(Context context, String args[]) throws Exception
	{
		...
		for (Iterator itrTableRows = objectList.iterator(); itrTableRows.hasNext();)
		{
			...
			String isPerson = (String) mapObjectInfo.get(ProgramCentralConstants.SELECT_IS_PERSON);
			String isProjectSpace = (String) mapObjectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);

			if("true".equalsIgnoreCase(isProjectSpace)||"true".equalsIgnoreCase(isProjectSpace)){
				vecResult.add("");
			}
			else
			{
				strRequestId = (String) mapObjectInfo.get(ProgramCentralConstants.SELECT_ID);
				DomainObject dmoRequest = DomainObject.newInstance(context,strRequestId);
				...
				if(mlRequestPersonList.size()>0)
				{
					...
					for (int i=0;i<personInfoSize;i++)
					{
						...
						strNameBuffer.append(XSSUtil.encodeForXML(context,(PersonUtil.getFullName(context,(String)mapRequestPersonInfo.get(SELECT_NAME)))));
...
//				strBufferOrgText.append("&amp;checkStoredResult=true');\">");
...
//			}
	...
	public String removeAssignment(Context context,String[]args) throws Exception 
	{
		...
		{ 
			...
			if(!strRequestState.equalsIgnoreCase(DomainObject.STATE_RESOURCE_REQUEST_REQUESTED))
			{
				DomainObject dmoResourcePool = DomainObject.newInstance(context,
						strResourcePoolID);
				String busSelectsManager = SELECT_RESOURCE_MANAGER_NAME;
	...
	public void updateFTEColumnDataByPhase(Context context,String[]args) throws MatrixException 
	{
		try 
		{
			...
			try
			{
				...
				NumberFormat nFormater = NumberFormat.getInstance();
				Number numericValue =  nFormater.parse(strNewValue);
				strNewValue = numericValue.toString();
				nNewFTEValue = Task.parseToDouble(strNewValue);
			}
			catch (NumberFormatException e) {
				String sErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral", 
			...
			if( strRelId!= null && !"".equals(strRelId) && !"null".equals(strRelId)) {
				...
				if(ProgramCentralUtil.isNotNullString(strRequestID) && !updatedRequestList.contains(strRequestID)){
				updatedRequestList.add(strRequestID);
			}
...
//			}  		
		...
		try 
		{
			...
			if( strRelId!= null && !"".equals(strRelId) && !"null".equals(strRelId)) {
				...
				if(TYPE_PERSON.equals(strTypePerson)){
					isPerson = true;
					strRequestID         =(String)slRelFromId.get(0);
					strPersonId         =(String)slRelToId.get(0);  
			...
			if((TYPE_PERSON.equals(strTypePerson) && ProgramCentralUtil.isNotNullString(strPersonId)) || (!TYPE_PERSON.equals(strTypePerson) && ProgramCentralUtil.isNotNullString(strRequestID))){
				String strObjID   = strRequestID;
				// IR-1142961
				if(TYPE_PERSON.equals(strTypePerson)){
					strObjID   = strPersonId;
				}
				// End IR-1142961
				if(!updatedRequestList.contains(strObjID)) 
					updatedRequestList.add(strObjID  );
				projectSpaceResourceRequestInfo.remove(strObjID  +nYear+nTimeLine);
				projectSpaceResourceRequestInfo.remove(strObjID  +"0"+"0");  // added for update issue of Total column when FTE column is updated in Timeline View  of Resource Plan Table 
			}
	...
	public StringList getExcludeInActiveResourcePool(Context context, String args[]) throws Exception
	{
		...
		String resourceManagerWhere = "from["+RELATIONSHIP_RESOURCE_MANAGER+"]";
		String memberWhere="from["+RELATIONSHIP_MEMBER+"]";
		...
		whereExp = "(("+SELECT_CURRENT+"=="+strState+") || ((("+resourceManagerWhere+"==FALSE)) && ("+SELECT_TYPE+" != "+strPlantType+" )) || ";
		whereExp += "(("+SELECT_TYPE+"=="+strPlantType+") && ("+memberWhere+"==FALSE)))";
		
		mapUserList =DomainObject.findObjects(context,PropertyUtil.getSchemaProperty(context,"type_Organization"),null,whereExp,select);
...
//			}
	 ...
	 * 
	 */
	// code commented out as it is the update function on state column of PMCResourceRequestSummaryTable and the Editable value of the column is false
...
//				strRelWhere,0); //where clause to apply to relationship, can be empty ""
	...
	public void reuseProposedRequestByPL(Context context,String[]args) throws Exception 
	{
		this.demote(context);
	}
	...
	public StringList propose(Context context,String[] args) throws Exception
	{
		...
		if(!slAffectingRequests.isEmpty())
		{
			...
			//MqlUtil.mqlCommand(context, "notice \""+str+"\"");
			MqlUtil.mqlCommand(context, "notice $1", "\""+str+"\"");
		} 
		...
		try{
			...
			if(null!=strOrganizationOldValue && !"".equals(strOrganizationOldValue) && !"null".equals(strOrganizationOldValue))
			{
				...
				for (Iterator itrRequest = mlRequests.iterator(); itrRequest .hasNext();)
				{
					...
					{
						if(!strObjectType.equals(DomainConstants.TYPE_BUSINESS_SKILL) && (strRequestState.equals(DomainConstants.STATE_RESOURCE_REQUEST_CREATE) || strRequestState.equals(DomainConstants.STATE_RESOURCE_REQUEST_REJECTED)))
						{
			...
			{
				String mqlQuery = "Error ";
				
				for(int i=0; i<slInvalidSelection.size(); i++) {
					mqlQuery+= " $"+(i+1);
				}
				String strErrorMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.ResourceRequest.NotInDesiredState", context.getSession().getLanguage());
				//MqlUtil.mqlCommand(context, "Error" +slInvalidSelection+ "\""+strErrorMessage+"\""); //PRG:RG6:R213:Mql Injection:Static Mql:24-Oct-2011
				
				mqlQuery+= "$"+slInvalidSelection.size();
				slInvalidSelection.add(strErrorMessage);
				MqlUtil.mqlCommand(context, mqlQuery, slInvalidSelection); 
			}
		}         
		catch (Exception exp)
		{
	...
	public Map getMinMaxRequestDates(Context context,String strProjectId) throws Exception {
		try {
			...
			mapDate.put("isTypeProjectSpace",isTypeProjectSpace);
			return mapDate;
		} 
		catch (Exception exp)
		{
	...
	public Vector getColumnRequestNameData(Context context,String[] args) throws Exception
	{
		try
		{
			...
			boolean isPrinterFriendly = false;
			String strPrinterFriendly = (String)programMap.get("reportFormat");
			...
			for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) 
			{
				...
				strObjectName = (String) mapRowData.get(SELECT_NAME);
				strObjectId =(String) mapRowData.get(SELECT_ID);
				String isResourceRequest = (String) mapRowData.get(SELECT_IS_RESOURCE_REQUEST);
				String isPerson = (String) mapRowData.get(ProgramCentralConstants.SELECT_IS_PERSON);
				String isProjectSpace = (String) mapRowData.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
				String type=(String) mapRowData.get(SELECT_TYPE);
				StringBuffer sbSubstanceLink = new StringBuffer();

				String rootNode = (String) mapRowData.get("Root Node");
				if("true".equalsIgnoreCase(rootNode)||ProgramCentralUtil.isNullString(strObjectName)){
					dmoObject = DomainObject.newInstance(context,strObjectId);
					strObjectName = (String) dmoObject.getInfo(context,SELECT_NAME);
				}
				if("true".equalsIgnoreCase(isResourceRequest) || TYPE_RESOURCE_REQUEST.equalsIgnoreCase(type))
				{
					if(!isPrinterFriendly){
						sbSubstanceLink.append("<a href='../common/emxTree.jsp?objectId=");
						sbSubstanceLink.append(XSSUtil.encodeForURL(context,strObjectId));
						sbSubstanceLink.append("' class='object' target='content' >");
						sbSubstanceLink.append(XSSUtil.encodeForXML(context,strObjectName));
						sbSubstanceLink.append("</a>");
						slPersonResourcePools.add(sbSubstanceLink.toString());
					}else{
						slPersonResourcePools.add(sbSubstanceLink.append(strObjectName).toString());
					}

		    		vecResult.addElement(FrameworkUtil.join(slPersonResourcePools, ",  "));
				}
				else if("true".equalsIgnoreCase(isPerson))
				{
					...
					if(!isPrinterFriendly){
						...
						sbSubstanceLink.append(XSSUtil.encodeForURL(context,strObjectId));
						sbSubstanceLink.append("' class='object' target='content' >");
						sbSubstanceLink.append(XSSUtil.encodeForXML(context,strFullName));
						sbSubstanceLink.append("</a>");
						slPersonResourcePools.add(sbSubstanceLink.toString());
					}else{
						slPersonResourcePools.add(sbSubstanceLink.append(strFullName).toString());
					}
				...
				else if(ProgramCentralUtil.isNotNullString(strObjectName)){
					if(!isPrinterFriendly){
						sbSubstanceLink.append("<a href='../common/emxTree.jsp?objectId=");
						sbSubstanceLink.append(XSSUtil.encodeForURL(context,strObjectId));
						sbSubstanceLink.append("' class='object' target='content' >");
...
//			Locale locale = ProgramCentralUtil.getLocale(context);
//			String strLanguage 	  = locale.getLanguage();
//			String strCurrency = ${CLASS:emxProgramCentralUtil}.getCurrencySymbol(context, locale,ProgramCentralUtil.getCurrencyUnit(context));
//			StringBuffer	currencyBuffer	= new StringBuffer();
//			for (int idx = 0; idx < objectList.size(); idx++) {
//				currencyBuffer	= new StringBuffer();
//				currencyBuffer.append("<input type=\"textbox\"  value=\""+ ${CLASS:emxProgramCentralUtil}.getFormattedNumberValue(context,0)+ "\"  name=\"txtStandardCost"+idx+"\" ");
...
//			}
	...
	throws Exception 
	{
		...
		if(ProgramCentralUtil.isNotNullString(sPromoteMsg))
		{
			...
			MqlUtil.mqlCommand(context, "notice $1", sPromoteMsg);
			return TRIGGER_FAILURE;
		}
		return TRIGGER_SUCCESS;
	} 
	 ...
	 */
	//form PMCResourcePlanPreferencesForm & command PMCResourcePlanPreference not found
...
//			}
	 ...
	 * @throws Exception
	 */
	//command PMCResourcePlanPreference not found
...
//			}
	...
	public Vector getColumnFTECostValue(Context context, Map programMap)  throws Exception {
		try {
			...
			for (Iterator itrTableRows = objectList.iterator(); itrTableRows.hasNext();)
			{
				...
				if(projectSpaceResourceRequestInfo.get(strResourceReqId+nYear+nMonth)!=null){
					String vecResult1 = (String) projectSpaceResourceRequestInfo.get(strResourceReqId+nYear+nMonth);
					vecResult.add(vecResult1);
					continue;
				}
				...
				slSelects.add(SELECT_RESOURCE_PLAN_FTE);
				...
				else
				{
					if(isResourceRequest)
					{
						...
						if(!"Total".equalsIgnoreCase(strColumnName))
						{
							...
							String strFTE = (String)mapObjInfo.get(SELECT_RESOURCE_PLAN_FTE);
							...
							if(projectSpaceResourceRequestInfo.get(strResourceReqId+nYear+nMonth)==null){
								projectSpaceResourceRequestInfo.put(strResourceReqId+nYear+nMonth, format.format(fteValue));
							}
						}
					}
					else
					...
					if("Total".equalsIgnoreCase(strColumnName))
					{
						...
						if(projectSpaceResourceRequestInfo.get(strResourceReqId+nYear+nMonth)==null){
							projectSpaceResourceRequestInfo.put(strResourceReqId+nYear+nMonth, strTotalRowCost);
						}
					}					
				}			
			}			
	...
	public Vector getColumnOnlyFTEValue(Context context, Map programMap)  throws Exception {
		try {
			...
			if(updatedRequestList.size()>0){
				updateFTEValueMap(context , objectList,updatedRequestList);
				updatedRequestList.clear();
			}
			...
			for (int i=0;i<size;i++)
			{
				...
				if(projectSpaceResourceRequestInfo.get(strObjectId+nYear+nMonth)!=null){
					String vecResult1 = (String) projectSpaceResourceRequestInfo.get(strObjectId+nYear+nMonth);
					vecResult.add(vecResult1);

				}else{
					fteValue=0d; 
					String isResourceRequeststr = (String)mapObjectInfo.get(SELECT_IS_RESOURCE_REQUEST);
					String isPersonStr = (String)mapObjectInfo.get(ProgramCentralConstants.SELECT_IS_PERSON);
					String isProjectSpaceStr = (String)mapObjectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
					if(ProgramCentralUtil.isNullString(isResourceRequeststr)&&
							ProgramCentralUtil.isNullString(isPersonStr)&&
							ProgramCentralUtil.isNullString(isPersonStr)

							){
						StringList objectSelects = new StringList();
						objectSelects.add(ProgramCentralConstants.SELECT_IS_PERSON);
						objectSelects.add(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
						objectSelects.add(SELECT_IS_RESOURCE_REQUEST);

						Map objMap  =new HashMap();
						try {
							ProgramCentralUtil.pushUserContext(context);
							DomainObject do1 = new DomainObject(strObjectId);
							objMap = do1.getInfo(context, objectSelects);
							isResourceRequeststr = (String)objMap.get(SELECT_IS_RESOURCE_REQUEST);
							isPersonStr = (String)objMap.get(ProgramCentralConstants.SELECT_IS_PERSON);
							isProjectSpaceStr = (String)objMap.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
					...
					if("true".equalsIgnoreCase(isProjectSpaceStr))
					{
						MapList mpChildObjList = new MapList();
						int count1 = 0;
						mpChildObjList = (MapList)mapObjectInfo.get("children");
					...
					{ 
						if("true".equalsIgnoreCase(isResourceRequeststr))
						{
							if(!"Total".equalsIgnoreCase(strColumnName))
							{
								String strFTE = (String) mapObjectInfo.get(SELECT_RESOURCE_PLAN_FTE);
								...
								vecResult.add(format.format(fteValue));
								if(projectSpaceResourceRequestInfo.get(strObjectId+nYear+nMonth)==null){
									projectSpaceResourceRequestInfo.put(strObjectId+nYear+nMonth, format.format(fteValue));
								}
							}
						}
						else if ("true".equalsIgnoreCase(isPersonStr))
						{
							if(!"Total".equalsIgnoreCase(strColumnName))
							{
						...
						else
						{
							if(!"Total".equalsIgnoreCase(strColumnName))
							{
								...
								mapObjectInfo.put("totalRowCost",totalRowCost);
								if(projectSpaceResourceRequestInfo.get(strObjectId+nYear+nMonth)==null){
									projectSpaceResourceRequestInfo.put(strObjectId+nYear+nMonth, strTotalCost);
								}
							}
						...
						{ 
							...
							vecResult.add(strTotalRowCost);
							if(projectSpaceResourceRequestInfo.get(strObjectId+nYear+nMonth)==null){
								projectSpaceResourceRequestInfo.put(strObjectId+nYear+nMonth, strTotalRowCost);
							}
						}
					}
				}
	...
	private void updateFTEValueMap(Context context, MapList objectList, StringList updatedRequestList) throws FrameworkException {

		String [] objectidArray = new String[updatedRequestList.size()];

		for(int i =0;i<updatedRequestList.size();i++){
			objectidArray[i] = (String) updatedRequestList.get(i);
		}

		StringList objectSelects = new StringList();
		objectSelects.add(SELECT_RESOURCE_PLAN_FTE);
		objectSelects.add(DomainConstants.SELECT_ID);

		MapList updatedInfoList = new MapList();
		Map infoMap = new HashMap();
		try {
			ProgramCentralUtil.pushUserContext(context);
			updatedInfoList = DomainObject.getInfo(context, objectidArray, objectSelects);
		} finally {
			ProgramCentralUtil.popUserContext(context);
		}

		for(int i=0;i<updatedInfoList.size();i++){
			Map temp = (Map) updatedInfoList.get(i);
		...
		for(int i=0;i<objectList.size();i++){
			...
			if(infoMap.containsKey(id)){
				temp.putAll((Map) infoMap.get(id));
			}
		}
	}
		...
		try
		{
			...
			if(null!=slRequestIdList)
			{
				...
				for(BusinessObjectWithSelectItr itr= new BusinessObjectWithSelectItr(resourceRequestObjWithSelectList); itr.next();)
				{
					...
					mapRequestInfo.put(SELECT_CURRENT,strCurrentState);
					isResourceRequest = bows.getSelectData(SELECT_IS_RESOURCE_REQUEST);
					mapRequestInfo.put(SELECT_IS_RESOURCE_REQUEST,isResourceRequest);
				...
				if(null!=strViewBy && !"".equals(strViewBy)&&!"null".equalsIgnoreCase(strViewBy) && strViewBy.equalsIgnoreCase(RESOURCE_PLAN_PREFERENCE_PHASE))
				{
					...
					if("Total".equalsIgnoreCase(strColumnName))
					{
						...
						else if(mapObjectInfo.containsKey("children"))
						{
							...
							strTotalRowFTE=format.format(totalCost);
							strTotalRowFTE = strTotalRowFTE+"";
						vecResult.add(strTotalRowFTE);
							mapObjectInfo.put("totalRowCost",0.0);
						}
						else 
	...
	public StringList getFTEViewEditAccess(Context context,
		...
		for(int iterator = 0; iterator < listSize; iterator++)
		{
			...
			String isProjectSpace = (String) map.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
			if("true".equalsIgnoreCase(isProjectSpace))
				returnStringList.add("false");
			else
				returnStringList.add(String.valueOf(isAccessible));
		}
	...
	public StringList getCostViewEditAccess(Context context,
		...
		for (int i=0;i<listSize;i++) {
			Map mapObjInfo =(Map)objectMap.get(i);
			if(mapObjInfo != null){
				String sIsResourceRequestType = (String) mapObjInfo.get(SELECT_IS_RESOURCE_REQUEST);
				boolean isResourceRequest = ProgramCentralUtil.isNotNullString(sIsResourceRequestType) && "true".equalsIgnoreCase(sIsResourceRequestType)?true:false;
				if(isResourceRequest&&isAccessible)
				{
					...
					{
						returnStringList.add(Boolean.valueOf(isAccessible).toString());
					}
					else
					{
						returnStringList.add("false");
	...
	public String getResourcePoolEditTextBox(Context context,String[]args) throws Exception 
	{
		...
		StringList busSelect=new StringList();
		busSelect.add(SELECT_RESOURCE_POOL_NAME);
		busSelect.add(SELECT_RESOURCE_POOL_TITLE);
		busSelect.add(SELECT_RESOURCE_POOL_ID);
		Map mapResReqInfo = strResourceReqDomObj.getInfo(context, busSelect);        
		String title = (String)mapResReqInfo.get(SELECT_RESOURCE_POOL_TITLE);
		String name = (String)mapResReqInfo.get(SELECT_RESOURCE_POOL_NAME);
		if(ProgramCentralUtil.isNullString(title)) {
			title = name;
		}
		String strReturn = getResourcePoolStr(context,title,(String)mapResReqInfo.get(SELECT_RESOURCE_POOL_ID),false);
		return strReturn;
	}
	...
	private String getResourcePoolStr(Context context,String strName,String strId,boolean hasFields) throws Exception {
		...
		strOrgText.append("&amp;checkStoredResult=true');\">");
		strOrgText.append("</input>");
	...
	public void updateResourceRequestCost(Context context, String[] args) throws MatrixException 
	{
		try{
			...
			String strNewValue = (String) paramMap.get("New Value");
			//when strNewValue value is with currency symbol like $12.00, It is resulting into parsing //exception so need to replace currency symbol with blank("")
			strNewValue = strNewValue.replaceAll("[^\\d.,]", "");
	 ...
	 * @author NZF
	 */
	//function call commented out in this JPO
...
//			try {
//				ProgramCentralUtil.pushUserContext(context);
//				objectInfoList = DomainObject.getInfo(context, objectIdArray, objectSelects);
//			} finally {
//				ProgramCentralUtil.popUserContext(context);
//			}
//
...
//			{
//				Map map = (Map)objectInfoList.get(iterator);
//				String isProjectSpace = (String) map.get(ProgramCentralConstants.SELECT_IS_PROJECT_SPACE);
//
//				if("true".equalsIgnoreCase(isProjectSpace))
//				{
...
//			}
	...
	public StringList getResourcePlanColumnEditAccess(Context context,
		...
		{
			String access = (String)slCheckAccess.get(iterator);
			boolean checkAccess = new Boolean(access);
			Map mapObjInfo = (Map)objectMap.get(iterator);
			String sIsResourceRequestType = (String) mapObjInfo.get(SELECT_IS_RESOURCE_REQUEST);
			boolean isResourceRequest = ProgramCentralUtil.isNotNullString(sIsResourceRequestType) && "true".equalsIgnoreCase(sIsResourceRequestType)?true:false;
			if(isResourceRequest)
			{
...
//			}
	...
	//calling method's use not found
