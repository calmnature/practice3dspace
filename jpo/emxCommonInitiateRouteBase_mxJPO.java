...
import com.matrixone.apps.domain.DomainConstants.DOMAIN_ACCESS_MASK;
...
public class emxCommonInitiateRouteBase_mxJPO
{
	...
	public static int InitiateRoute (Context context,String args[])
	{
		...
		//making this a global variable to reduct db calls
//		String strTentativeCompletionDate = "";
		//adding this global variable
		String inboxTaskName = "";
		String routeOwningOrganization = "";
		...
		//TODO mbs21 remove this variable
		boolean areCompletionDateEqual = false;
		...
		try {
			...
			objectSelects.add(SELECT_OWNING_ORG_ID);
			...
			routeOwningOrganization = (String) routeMap.get(SELECT_OWNING_ORG_ID);
					...
					if (routeSequence == sRouteSequenceValue) {
						...
						if (mlOldTasks.size() > 0) {
							...
							if("Auto Complete".equals(sApprovalStatus)) {
								...
								if(!sRouteId.equalsIgnoreCase(connectedRouteId)) {
									try {
										...
										taskInfo = dmoNewTask.getInfo(context, slTaskSelect);
							...
							if (POLICY_INBOX_TASK_STATE_COMPLETE.equals(strOldTaskState)) {
								...
								taskInfo = dmoNewTask.getInfo(context, slTaskSelect, multiValueTaskSelect);
								sInboxTaskId = (String) taskInfo.get(DomainObject.SELECT_ID);
								sInboxTaskPhysicalId = (String) taskInfo.get(DomainConstants.SELECT_PHYSICAL_ID);
								DomainAccess.createObjectOwnership(context, sInboxTaskId, sRouteId,
										DomainAccess.COMMENT_MULTIPLE_OWNERSHIP); // updating Route Id on Inbox Task
									...
									if ("true".equalsIgnoreCase(sRoutePreserveOwner) && UIUtil.isNotNullAndNotEmpty(strTaskAssignTo) && !sPersonId.equalsIgnoreCase(strTaskAssignTo) && !sRouteId.equalsIgnoreCase(connectedRouteId)) {
								           ...
								           newRelId = MqlUtil.mqlCommand(context, "print connection $1 select $2 dump", newRelId, DomainConstants.SELECT_PHYSICAL_ID);
											...
											try {
												...
												String []coObjMap = routeObject.getConnectedObjectIds(context);
												//grant read access on content for User group 
												String contentAccess = RouteModeler.getContentAccess(context, DOMAIN_ACCESS_MASK.READ.toString());
												for (int j= 0; j < coObjMap.length; j++) {
													String defaultAccessMasks = DomainAccess.getPhysicalAccessMasks(context, sRouteId, contentAccess);
											    	DomainAccess.createObjectOwnership(context, (String) coObjMap[j], null, sPersonName, defaultAccessMasks, DomainAccess.COMMENT_MULTIPLE_OWNERSHIP_GRANTED +" " +sName, true);
								            	}
						...
						StringList accessNames = new StringList();
						//This change is made because of IR-1420812-3DEXPERIENCER2026x 
						// Problem was since the document owner not had the access of the document,
						// Doc Owner was unable to create the inbox Task, hence pushed the context and now he is able to create the Inbox Task
						try {
							ContextUtil.pushContext(context);
						...
						//Done added selects to select task name and scheduled completion date
						inboxTaskName = (String) inboxTask.getInfo(context, DomainObject.SELECT_NAME);
						...
						//done mbs21 moved this line below after computing the assigneeSetDue date completion date
//						inboxTask.setAttributeValues(context, attributesMap);
						...
						//done mbs21 moved the below 2 lines because there were redundant db calls for sScheduledCompletionDate
						...
						if (!"".equals(sAbsenceDelegate)
							...
							if (absenceMap.size() != 0) {
								...
								//Revoke access on content
								//Should not be Route owner
								//Check whether he owns other tasks or not
								if (UIUtil.isNotNullAndNotEmpty(sRouteOwner)
										&& !sRouteOwner.equalsIgnoreCase(oldTaskAssignee)) {
									List<String> assingeeArray = new ArrayList<>();
									for (int k = 0; k < routedNodeObjects.size(); k++) {
										Map routedNodeObject = (Map) routedNodeObjects.get(k);
										String name = (String) routedNodeObject.get("name");
										assingeeArray.add(name);
									}
									int count = 0;
									for (String name : assingeeArray) {
										if (name.equalsIgnoreCase(oldTaskAssignee)) {
											count++;
											if (count > 1)
												break;
										}
									...
									if (count == 1) {
										for (int i1 = 0; i1 < sRouteObjectsMaplist.size(); i1++) {
											...
											if (!(DomainObject.TYPE_WORKSPACE.equals(fromType)
													|| DomainObject.TYPE_WORKSPACE_VAULT.equals(fromType)
													|| DomainObject.TYPE_PROJECT_SPACE.equals(fromType)
													|| DomainObject.TYPE_PROJECT_CONCEPT.equals(fromType)
													|| DomainObject.TYPE_ROUTE.equals(fromType))) {
												DomainAccess.deleteObjectOwnership(context, sObjectId, null,
														oldTaskAssignee + "_PRJ",
														DomainAccess.COMMENT_MULTIPLE_OWNERSHIP_GRANTED + " " + sName,
														true);
											}
						...
						//TODO mbs21 why open and close context?? and does the above line make db call? becz we already have person object
						//done mbs21 commented the below code
//						personObject.open(context);
//						personObject.close(context);
						DomainRelationship.connect(context, inboxTask,
								sRelProjectTask, personObject);
						String grantAccessForNewUser = Route.getAccessbits(context,sRouteId,person.getInfo(context, DomainObject.SELECT_NAME));
						...
						if(UIUtil.isNotNullAndNotEmpty(sDelegatorName)) {
							try {
								ContextUtil.pushContext(context);
								String contentAccess = RouteModeler.getContentAccess(context, defaultAccessOnRoute);
								String defaultAccessMasks = DomainAccess.getPhysicalAccessMasks(context, sRouteId, contentAccess);
								for(int k=0; k< sRouteObjectsMaplist.size(); k++) {								  
									...
									if(!(DomainObject.TYPE_WORKSPACE.equals(fromType) || DomainObject.TYPE_WORKSPACE_VAULT.equals(fromType) || DomainObject.TYPE_PROJECT_SPACE .equals(fromType) || DomainObject.TYPE_PROJECT_CONCEPT.equals(fromType) || DomainObject.TYPE_ROUTE.equals(fromType))) {									
										DomainAccess.createObjectOwnership(context, (String)mapRouteObjects.get(DomainConstants.SELECT_ID) , null, sDelegatorName+"_PRJ", defaultAccessMasks, DomainAccess.COMMENT_MULTIPLE_OWNERSHIP_GRANTED +" " +sName, false);
									}									
								}
							}finally {
								ContextUtil.popContext(context);
						...
						if (!sRouteTaskUser.equals("")) {
							...
							//TODO mbs21 added selectable with route selects and got it in single hit
							
							String sOwnOrgId = routeOwningOrganization;
						...
						//TODO mbs21 can we just select name from line 1193 instead of inboxTask.getInfo????
						// done mbs21 changed to inboxTaskName taken from line 1165
						
                        String currentTaskContent = InboxTask.getTaskContent(context, sRouteId, inboxTaskName);
