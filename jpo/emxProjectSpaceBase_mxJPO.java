            ...
            while (objectListIterator.hasNext())
            {
                ...
                if ((programIds == null) || programIds.isEmpty())
                {
                    ...
                    for(int nCount=0;nCount<programIds.size();nCount++)
                    {
    		            ...
    		            objectSelects.add(ProgramCentralConstants.SELECT_NAME);
    		            
    					Map objectInfoMap = (Map) programDobj.getInfo(context, objectSelects);
    					System.out.println(objectInfoMap);
    					
    					boolean isKindOfProgram = "TRUE".equalsIgnoreCase((String) objectInfoMap.get(ProgramCentralConstants.SELECT_IS_PROGRAM));
    					
    					strProgramName = ProgramCentralUtil.getTitleForProjectType(context, objectInfoMap, isKindOfProgram);
...
//End:nr2:PRG:R210:03-06-:2010:For Project Gate Highlight
    ...
    public void modifyPrimaryOwnership(Context context, String[] args) throws Exception
    {
  	...
  	public Map createNewProject(Context context,String[]args)throws Exception
  	{
  		...
  		try{
			...
			String runInBackground= "false";
  			
  			
  			String copyFinancialData 		= (String)programMap.get("financialData");
			//Commented out for : FUN155106
  			//String copyFolderData 			= (String)programMap.get("folders");
			String copyBookmarkContent = (String)programMap.get("BookmarkContent");
            ...
            String openerframe 				= (String)programMap.get("openerFrame"); //Indicates the frame (generally command) from which the use case is performed.
			 runInBackground = (String)programMap.get("ExecuteInBackground"); 
			 
			String contextProjectId = DomainConstants.EMPTY_STRING; // When the Project Create menu used in related Projects or Program commands.
			String isContextKindOfProgram  = DomainConstants.EMPTY_STRING;
			if(ProgramCentralUtil.isNotNullString(objectId)) {
				contextProjectId = objectId;
				isContextKindOfProgram = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3",contextProjectId, ProgramCentralConstants.SELECT_IS_PROGRAM,"|");
				if("true".equalsIgnoreCase(isContextKindOfProgram)){
					programId = contextProjectId;
				}
			}
			
			String newProjectId 			= DomainConstants.EMPTY_STRING;						
			String newProjectIdToRunInBackground	 = 		DomainConstants.EMPTY_STRING;	
			if((ProgramCentralUtil.isNullString(objectId) && ProgramCentralUtil.isNotNullString(selectedProjectId)) || 
					(ProgramCentralUtil.isNotNullString(selectedProjectId) && ("Clone".equalsIgnoreCase(createProject) || "Template".equalsIgnoreCase(createProject)))){
			...
			//end  IR-528127-3DEXPERIENCER2018x
  			...
  			if ("Blank".equalsIgnoreCase(createProject) || 
  					"Import".equalsIgnoreCase(createProject) || "True".equalsIgnoreCase(runInBackground)) {
  			...
  			attributeMap.put(DomainObject.ATTRIBUTE_TITLE, projectName);
			basicProjectInfo.put("name", autoName);
            ...
            relatedProjectInfo.put("deliverableId", deliverabletId);
  			...
  			if("Clone".equalsIgnoreCase(createProject) || "Template".equalsIgnoreCase(createProject)){
				...
				if("True".equalsIgnoreCase(runInBackground)){
					newProject = project.createBlankProject(context, 
  						basicProjectInfo, 
  						attributeMap, 
  						relatedProjectInfo);
						newProjectIdToRunInBackground = newProject.getObjectId();
				}
				...
				if(ProgramCentralUtil.isNullString(runInBackground)){
				...
				ContentReplicateOptions selectedOptionForBookmarkContent = ContentReplicateOptions.COPY;
				 
				if (ProgramCentralUtil.isNullString(copyBookmarkContent) || "Reference".equalsIgnoreCase(copyBookmarkContent)) {
					selectedOptionForBookmarkContent = ContentReplicateOptions.CONNECT_EXISTING;
				}
				if (ProgramCentralUtil.isNullString(copyBookmarkContent) || "None".equalsIgnoreCase(copyBookmarkContent)) {
					selectedOptionForBookmarkContent = ContentReplicateOptions.IGNORE;
				}
				// projectDate date format required for both if-else condition
				projectDate = FormatUtil.getFormattedISODate(context, constraintDate.getTime(), null /*SimpleDateFormat*/);
  				...
  				if("Clone".equalsIgnoreCase(createProject)) {
					...
					// IR-1461676-3DEXPERIENCER2026x
					inputProject.setType(projectSpaceType);
					...
					args1.put("bookmarkContentReplicateOption" , selectedOptionForBookmarkContent.toString());
					args1.put("runInBackground" , runInBackground);
					args1.put("newProjectId" , newProjectIdToRunInBackground);
			
					Servicedata  response = ServiceBase.saveData(context, inputService, lParameters, 0);
					if("false".equalsIgnoreCase(runInBackground)){
					...
					String questionCommentValue 	= (String) CacheUtil.getCacheObject(context, loggedInUser+"_QuestionsResponseComment");
					if(ProgramCentralUtil.isNotNullString(questionResponseValue)){
						...
						StringList questionCommentValueList = FrameworkUtil.split(questionCommentValue, "|");
	  					for(int i=0;i<questionResponseValueList.size();i++){
	  						String questionRValue = (String)questionResponseValueList.get(i);
	  						String questionCValue = (String)questionCommentValueList.get(i);
	  						StringList questionActualRList = FrameworkUtil.split(questionRValue, "=");
	  						StringList questionActualCList = FrameworkUtil.split(questionCValue, "=");
							...
							if(questionActualCList.size() > 1){
								DataelementMapAdapter.setDataelementValue(question, "comment", questionActualCList.get(1));
							}
  		...
  		//AAR14 Reverting changes made for function FUN135520 as function is moved to 25x.FD02: Commenting out code
  			//basicProjectInfo.put("name", autoName);
  			basicProjectInfo.put("type", projectSpaceType);
  			basicProjectInfo.put("policy", projectPolicy);
  			basicProjectInfo.put("vault", projectVault);
			...
			if(ProgramCentralUtil.isNullString(sKeepSourceColors) || "false".equalsIgnoreCase(sKeepSourceColors)){
  			...
  			if("Blank".equalsIgnoreCase(createProject) || 
				...
				*/
				...
				if("PMCRelatedProjects".equalsIgnoreCase(openerframe)) {
					relatedProjectInfo.put("AddAsChild", "true"); //override the objectId with contextProjectId for RelatedProject Connection
					relatedProjectInfo.put("RelatedProjectId", contextProjectId);
					//Connect Source and Target Project as "Related Projects" only in case of PMCRelatedProjects frame, when target project is created/copied/imported/fromtemplate from Related Projects command.
					newProject.callUpdateProjectRelatedInfo(context,relatedProjectInfo, true);
				}else {
					//No need to connect Source and Target Project as "Related Projects" in case of Copy From Project use case. Hence, overloading the method to skip the connection by sending connection flag "false".
					newProject.callUpdateProjectRelatedInfo(context,relatedProjectInfo, false);
				}
  	 ...
  	 */
		...
		String ProjectMembers = EnoviaResourceBundle.getProperty(context, ProgramCentralConstants.PROGRAMCENTRAL, 
					"emxProgramCentral.Common.ProjectMembers", context.getSession().getLanguage());
						
		String assignees = EnoviaResourceBundle.getProperty(context, ProgramCentralConstants.PROGRAMCENTRAL, 
					"emxProgramCentral.partialSchedule.copyAssigneesOption", context.getSession().getLanguage());
		String deliverables = EnoviaResourceBundle.getProperty(context, ProgramCentralConstants.PROGRAMCENTRAL, 
					"emxProgramCentral.partialSchedule.copyDeliverablesOption", context.getSession().getLanguage());
		String passiveTasks = EnoviaResourceBundle.getProperty(context, ProgramCentralConstants.PROGRAMCENTRAL,
				"emxProgramCentral.PassiveSubtask.FlexibleLinks", context.getSession().getLanguage());
					
		boolean assigneeCheck = com.dassault_systemes.enovia.dpm.ProjectSpace.getExpressionValue(context, "DPM_CopyTaskAssignees", false);
		boolean deliverablesCheck = com.dassault_systemes.enovia.dpm.ProjectSpace.getExpressionValue(context, "DPM_CopyTaskDeliverables", false); 
		...
		sb.append("<input type=\"checkbox\" name=\"projectMembers\" value=\"true\" checked=\"true\" />");
		sb.append("<label for=\"projectMembers\">");
		sb.append(ProjectMembers);
		sb.append("</label>");
		sb.append("</td>");
		...
		sb.append("<td colspan=\"2\" width=\"95\">");
  		sb.append("<input type=\"checkbox\" name=\"assignees\" value='True'");
		if(assigneeCheck){
			sb.append(" checked=\""+assigneeCheck+"\"");
		}
		sb.append("/>");
  		sb.append("<label for=\"assignees\">");
  		sb.append(assignees);
  		sb.append("</label>");
  		sb.append("</td>");
        sb.append("</tr>");
        ...
        sb.append("<tr>");
        sb.append("<td colspan=\"2\" width=\"95\">");
		sb.append("<input type=\"checkbox\" name=\"deliverables\" value='True'");
		if(deliverablesCheck){
			sb.append(" checked=\""+deliverablesCheck+"\"");
		}
		sb.append("/>");
		sb.append("<label for=\"deliverables\">");
        ...
        sb.append("<tr>");
  		sb.append("<td colspan=\"2\" width=\"135\">");
  		sb.append("<input type=\"checkbox\" name=\"connectPassiveTasks\" value=\"True\" />");
  		sb.append("<label for=\"connectPassiveTasks\">");
  		sb.append(passiveTasks);
  		sb.append("</label>");
  		sb.append("</td>");
        sb.append("</tr>");
  	...
  	public String getCopyOptionsForTemplate(Context context,String[]args)throws Exception
  	{
		...
		sb.append("<td colspan=\"2\" width=\"110\">");
  		sb.append("<input type=\"checkbox\" name=\"financialData\" value=\"true\" checked=\"true\"/>");
  		sb.append("<label for=\"financialData\">");
  		sb.append(financialData);
  		sb.append("</label>");
		...
		sb.append("<td colspan=\"2\" width=\"110\" hidden=\"true\">");
  	...
  	private void removeImportCacheSetting(Context context)throws Exception
  	{
		...
		CacheUtil.removeCacheObject(context,"noMoreExpandTPColumn");
  	}
    ...
    public StringList getImportedObjectID(Context context,String[]args)throws Exception
    {
  	...
  	public MapList getTemplateWBS(Context context,String[]args)throws Exception
  	{
  		...
  		int taskExpandLevel = 1;
  		if(ProgramCentralUtil.isNotNullString((String) programMap.get("taskExpandLevel"))) {
  			taskExpandLevel = Integer.parseInt((String) programMap.get("taskExpandLevel"));
  		}
  		task.setId(templateId);

  		StringList busSelects = new StringList(10);
  		busSelects.add(SELECT_ID);
  		busSelects.add(SELECT_PHYSICAL_ID);
  		busSelects.add(SELECT_TYPE);
  		busSelects.add(SELECT_NAME);
  		...
  		MapList mapList = Task.getTasks(context, task, taskExpandLevel, busSelects, null, false, false);
  		...
  		String loggedInUser = context.getUser();
  		String questionResponse	= (String) CacheUtil.getCacheObject(context, loggedInUser+"_QuestionsResponse");
  		Map <String,String>questionResponseMap = new HashMap<String,String>();
  		...
  		for(int i=0;i<mapList.size();i++){
			...
			if("FALSE".equalsIgnoreCase(hasSubtask)){
  			...
  			if(ProgramCentralUtil.isNotNullString(taskTransfer)){
				...
				boolean addTaskToTaskList = false;
  				for(int itr=0; itr<sltaskTransfer.size() && itr<slquestionId.size(); itr++) {
					...
					if(taskTransferResponse.equalsIgnoreCase(questionResponseValue)){
						addTaskToTaskList = true;
						break;
					}
  				}
				if(addTaskToTaskList){
...
PropertyUtil.setGlobalRPEValue(context, "CopyColorAttribute", "true");
	...
	public static void copyPartialScheduleProcess(Context context,String[]args)throws Exception {
        ...
        String UseStartAndEndDatesAsEntered = (String) programMap.get("UseStartAndEndDatesAsEntered");
			String copyDeliverables = (String) programMap.get("copyDeliverables");
			String copyAssignees = (String) programMap.get("copyAssignees");
			String connectPassiveTasks = (String) programMap.get("connectPassiveTasks");
			StringList sourceTaskList = FrameworkUtil.split(sourceTasks, "|");
			String[] sourceTaskIds = new String[sourceTaskList.size()];
			boolean copyAssigneesFlag = false; 
			boolean copyDeliverablesFlag = false;

			String copyTaskPreferenceOption = "None"; // FUN156438:Setting on project to copy/refer/none task attachment 
			String copyTaskReferences = "false";
			...
			MapList objectInfoList = DomainObject.getInfo(context, sourceTaskIds, objectSelects);

			StringList partialTaskList = new StringList();
			
			final String TASK_TO_PROJECT_COPYTASKPREFERENCE = "to[Project Access Key].from.from[Project Access List].to." + ProgramCentralConstants.SELECT_COPYTASKPREFERENCE;
			
			DomainObject targetObj = new DomainObject(targetTaskId);
			...
			busSelect.add(TASK_TO_PROJECT_COPYTASKPREFERENCE);
			busSelect.add(ProgramCentralConstants.SELECT_COPYTASKPREFERENCE);
			Map targetProjectInfo = targetObj.getInfo(context, busSelect);
			String istargetKindOfTask = (String) targetProjectInfo.get(ProgramCentralConstants.SELECT_KINDOF_TASKMANAGEMENT);
			String targetProjectId = targetTaskId;
			//targetTaskId is checked objected id, but if it is kind of task, need to set targetProjectId with master project id 
			if("true".equalsIgnoreCase(istargetKindOfTask)) {
				targetProjectId = (String)targetProjectInfo.get(ProgramCentralConstants.SELECT_PROJECT_ID);
				copyTaskPreferenceOption = (String)targetProjectInfo.get(TASK_TO_PROJECT_COPYTASKPREFERENCE);
			}else {
				copyTaskPreferenceOption = (String)targetProjectInfo.get(ProgramCentralConstants.SELECT_COPYTASKPREFERENCE);

			}
			
			String scheduleFrom = new DomainObject(targetProjectId).getAttributeValue(context, ProgramCentralConstants.ATTRIBUTE_SCHEDULED_FROM);
			if (ProgramCentralUtil.isNullString(copyAssignees)) {
				copyAssignees = "false";
			}
			if (ProgramCentralUtil.isNullString(copyDeliverables)) {
			...
			args1.put("copyTaskDeliverables", copyDeliverables);
			args1.put("copyTaskAssignees", copyAssignees);
			args1.put("copyChecklists", "true"); // IR-1208101
			args1.put("UseStartAndEndDatesAsEntered", UseStartAndEndDatesAsEntered);
			args1.put("copyPartialSchedule", "true");
			args1.put("scheduleFrom", scheduleFrom);
			args1.put("copyPassiveTasks", connectPassiveTasks);
			if("true".equalsIgnoreCase(istargetKindOfTask)) {
				args1.put("copyPartialTargetTask", ProgramCentralUtil.getPhysicalId(context, targetTaskId));
			}
			if("Reference".equalsIgnoreCase(copyTaskPreferenceOption) || "Copy".equalsIgnoreCase(copyTaskPreferenceOption)) {
				ContentReplicateOptions selectedOptionForReferenceDocument = "Reference".equalsIgnoreCase(copyTaskPreferenceOption) ? ContentReplicateOptions.CONNECT_EXISTING : ContentReplicateOptions.COPY;
				copyTaskReferences = "true";
				args1.put("referencesReplicateOption", selectedOptionForReferenceDocument.toString());
			}
			args1.put("copyTaskReferences", copyTaskReferences); // FUN156438 
			...
			for (Iterator iterator = objectInfoList.iterator(); iterator.hasNext();) {
				...
				String taskTypeKindOfProjectSpace = (String) taskInfo
						.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
				String taskTypeKindOfProjectTemplate = (String) taskInfo
						.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_TEMPLATE);

			
				if ("true".equalsIgnoreCase(taskTypeKindOfProjectSpace)
						|| "true".equalsIgnoreCase(taskTypeKindOfProjectTemplate)) {
					Dataobject sourceProject = new Dataobject();
					sourceProject.setId(taskId);

					final Servicedata inputService = new Servicedata();
					inputService.getData().add(sourceProject);
					...
					if(ProgramCentralUtil.isNotNullString(questionResponseValue)){
						questionResponseMap = Question.getQuestionResponce(questionResponseValue);
						ArrayList<Dataobject> questions = new ArrayList<Dataobject>();
						for(Map.Entry<String, String> entry : questionResponseMap.entrySet()){							
							Dataobject question = new Dataobject();
							question.setId(new DomainObject(entry.getKey()).getPhysicalId(context));
							//question.setType(ProgramCentralConstants.TYPE_QUESTION);
							DataelementMapAdapter.setDataelementValue(question, "questionResponse", entry.getValue());
							DataelementMapAdapter.setDataelementValue(question, "id", entry.getKey());
							DataelementMapAdapter.setDataelementValue(question, "type", ProgramCentralConstants.TYPE_QUESTION);							
							questions.add(question);
						}
						RelateddataMapAdapter.addRelatedData(sourceProject, "questions", questions);
					}
					
					args1.put("copyPartialDestinationProject", targetProjectId); // Project id (in case if task checked, it's master project)					
					 Servicedata response = ServiceBase.saveData(context, inputService, lParameters, 0);
				} else {
			...
			if (partialTaskList.size() > 0) {				
				...
				final Servicedata inputService = new Servicedata();
				Dataobject targetProjectObj = new Dataobject();
				targetProjectObj.setId(targetProjectId);
				inputService.getData().add(targetProjectObj);
				...
				ArrayList<Dataobject> tasks = new ArrayList<Dataobject>();
				for(String taskId : partialTaskList){							
					Dataobject task = new Dataobject();
					task.setId(taskId);
					tasks.add(task);
				}
				RelateddataMapAdapter.addRelatedData(targetProjectObj, "cloneTasks", tasks);
							
				args1.put("copyPartialScheduleTasks", "true");
				Servicedata response = ServiceBase.saveData(context, inputService, lParameters, 0);
				String newProjectId = (String)response.getData().get(0).getId();
			}
		} catch(Exception exception) {
			System.out.println("Exception occured in emxProjectSpaceBase_mxJPO.copyPartialScheduleProcess()");
	...
	public String getProgramName(Context context, String[] args) throws Exception {
		...
		if(ProgramCentralUtil.isNotNullString(objectId)){
			StringList busSelects = new StringList();
			busSelects.add(SELECT_NAME);
			busSelects.add(SELECT_ATTRIBUTE_TITLE);
			
			MapList objInfoMapList = DomainObject.getInfo(context, new String[]{objectId}, busSelects);
			Map objInfoMap = (Map) objInfoMapList.get(0);
			programName = (String) objInfoMap.get(SELECT_ATTRIBUTE_TITLE);
...
//        	 mlMasterPlanGates = deliverablePlanGate.getMasterPlanGates(context, deliverableId, null);
   ...
   public boolean isAddingShadowGateEnabled(Context context,String args[]) throws Exception
   {
   	 ...
   	 public boolean getEnableApprovalViewAccess(Context context, String[] args) throws Exception {
   		 ...
   		 if(issueNameList != null){
   			 for(int i=0;i<issueNameList.size();i++){
   				 ...
   				 if(issueName.startsWith(ProgramCentralConstants.START_WITH_EXP) || issueName.startsWith(ProgramCentralConstants.START_WITH_SCN)){
    ...
    private boolean areDatesEqual(Date firstDate,Date secondDate){
	  ...
	  {
         try {
		     String strLoggedInUsersCS = PersonUtil.getActiveProjectTitle(context);
      ...
      public boolean hasAccessToRemoveGovernedItemsAction(Context context, String[] args) throws MatrixException {
	    ...
	    busSelects.add(SELECT_TASK_CONSTRAINT_TYPE);
			...
			busSelects.add(ProgramCentralConstants.SELECT_TASK_PRIORITY);
        ...
        } else if("w".equalsIgnoreCase(projectEstimatedDurationUnit)&& ProgramCentralUtil.isNotNullString(projectEstimatedDuration)){
			double duration = Task.parseToDouble(projectEstimatedDuration);    				
			duration = duration/5;
			duration = Task.parseToDouble(_df.format(duration));
			projectEstimatedDuration = String.valueOf(duration);
        } else if("mon".equalsIgnoreCase(projectEstimatedDurationUnit)&& ProgramCentralUtil.isNotNullString(projectEstimatedDuration)){
			double duration = Task.parseToDouble(projectEstimatedDuration);    				
        ...
        String name = (String) projectMap.get(SELECT_ATTRIBUTE_TITLE);
        if (ProgramCentralUtil.isNullString(name)) {
        	name = (String) projectMap.get(SELECT_NAME);
        }
		
		String projectDescription = (String) projectMap.get(SELECT_DESCRIPTION);
        
	    headerLine.replace(0, headerLine.length(), "0"); //WBS
	    headerLine.append(seperator);
	    headerLine.append(name); //Name
	    headerLine.append(seperator);
		...
		headerLine.append(seperator);
		headerLine.append(projectDescription);
	    taskList.add(headerLine.toString());
	    ...
	    taskSelects.add(SELECT_NAME);
            	...
            	if(ProgramCentralUtil.isNotNullString(projectObjectId) && projectObjectId.equals((String)listPredecessorProjectIds)) {
	    			...
	    			if (ProgramCentralUtil.isNotNullString((String)listLagTimes)
	    					&& ProgramCentralUtil.isNotNullString((String)listLagTimesDurationUnit)){
	    						...
	    						if("h".equalsIgnoreCase((String)listLagTimesDurationUnit) && ProgramCentralUtil.isNotNullString((String)listLagTimes)) {
	    							float lagTime = Float.parseFloat((String)listLagTimes)*8;           //lag time in days 
	    							lagTimeDays = String.valueOf(lagTime);
	    						} else if("w".equalsIgnoreCase((String)listLagTimesDurationUnit) && ProgramCentralUtil.isNotNullString((String)listLagTimes)) {
	    							float lagTime = Float.parseFloat((String)listLagTimes)/5;            
	    							lagTimeDays = String.valueOf(lagTime);
	    						} else if("mon".equalsIgnoreCase((String)listLagTimesDurationUnit) && ProgramCentralUtil.isNotNullString((String)listLagTimes)) {
	    							float lagTime = Float.parseFloat((String)listLagTimes)/20;           
	    							lagTimeDays = String.valueOf(lagTime);
	    						} else {
	            ...
	            for (int i =0; i<sl.size(); i++) {
	    			...
	    			if (predecessor != null) {
	    				...
	    				if (slag != null && !"".equals(slag)){
	    							...
	    							String lagTimeDays = EMPTY_STRING;
	    							if("h".equalsIgnoreCase(slagUnit.get(i)) && ProgramCentralUtil.isNotNullString((String)slag.get(i))) {
	    								float lagTime = Float.parseFloat((String)slag.get(i))*8;           //lag time in days 
	    								lagTimeDays = String.valueOf(lagTime);
	    							} else if("w".equalsIgnoreCase(slagUnit.get(i)) && ProgramCentralUtil.isNotNullString((String)slag.get(i))) {
	    								float lagTime = Float.parseFloat((String)slag.get(i))/5;           
	    								lagTimeDays = String.valueOf(lagTime);
	    							} else if("mon".equalsIgnoreCase(slagUnit.get(i)) && ProgramCentralUtil.isNotNullString((String)slag.get(i))) {
	    								float lagTime = Float.parseFloat((String)slag.get(i))/20;           
	    								lagTimeDays = String.valueOf(lagTime);
	    							} else {
            ...
            } else if("w".equalsIgnoreCase(taskEstDurationUnit)&& ProgramCentralUtil.isNotNullString(taskEstDuration)){
				double duration = Task.parseToDouble(taskEstDuration);    				
				duration = duration/5;
				duration = Task.parseToDouble(_df.format(duration));
				taskEstDuration = String.valueOf(duration);
            }else if("mon".equalsIgnoreCase(taskEstDurationUnit)&& ProgramCentralUtil.isNotNullString(taskEstDuration)){
				double duration = Task.parseToDouble(taskEstDuration);    				
				duration = duration/20;
				duration = Task.parseToDouble(_df.format(duration));
				taskEstDuration = String.valueOf(duration);
			...
			String taskDescrption = (String)thisTask.get(SELECT_DESCRIPTION);
			String  taskPriorityValue = (String)thisTask.get(ProgramCentralConstants.SELECT_TASK_PRIORITY);
	        StringBuffer thisLine = new StringBuffer();
	        thisLine.append(thisTask.get(SubtaskRelationship.SELECT_TASK_WBS) + seperator);
	        thisLine.append((ProgramCentralUtil.isNotNullString((String)thisTask.get(SELECT_ATTRIBUTE_TITLE))?thisTask.get(SELECT_ATTRIBUTE_TITLE):thisTask.get(SELECT_NAME)) + seperator);
	        ...
	        thisLine.append(taskConstraintDate + seperator);
			thisLine.append(taskDescrption + seperator);
			thisLine.append(taskPriorityValue);			
   ...
   @com.matrixone.apps.framework.ui.ProgramCallable
    ...
    private MapList getProjectList(Context context, String[] args, String Mode) throws MatrixException
    {
    	...
    	MapList activeProjectListSubset = new MapList();
    	StringList projectIdsToBeRemoved = new StringList();
    	StringList busSelects = new StringList(2);
    	busSelects.add(ProjectSpace.SELECT_ID);
    	busSelects.add(ProjectSpace.SELECT_NAME);
    	busSelects.add(SELECT_ATTRIBUTE_TASK_ESTIMATED_END_DATE);
    	...
    	try{
    		if("AssignedActive".equalsIgnoreCase(Mode)){
    			...
    			if("CredentialedActive".equalsIgnoreCase(Mode)) {
    				activeProjectListSubset = getActiveProjects(context, args);
    				
    				Iterator<Map> activeProjectListSubsetIterator = activeProjectListSubset.iterator();
	                while(activeProjectListSubsetIterator.hasNext()){
	                	Map projectInfo = activeProjectListSubsetIterator.next();
	                	projectIdsToBeRemoved.add((String)projectInfo.get(DomainConstants.SELECT_ID));
	                	
	                }
	                Iterator<Map> projectListIterator = projectList.iterator();
	                while(projectListIterator.hasNext()){
	                	Map projectInfo = projectListIterator.next();
	                	String id = (String)projectInfo.get(DomainConstants.SELECT_ID);
	                	if(projectIdsToBeRemoved.contains(id)){
	                		projectListIterator.remove();
	                    }
	                }
    		} 
		...
		@com.matrixone.apps.framework.ui.ProgramCallable
		public String getCopyTaskPreferenceField(Context context, String args[]) throws Exception {
			HashMap programMap = (HashMap) JPO.unpackArgs(args);
			HashMap requestMap = (HashMap) programMap.get("requestMap");
			String objectId = (String) requestMap.get("objectId");
			String strLanguage = (String) requestMap.get("languageStr");
			if (ProgramCentralUtil.isNullString(strLanguage)) {
				strLanguage = context.getSession().getLanguage();
			}
			String strMode = (String) requestMap.get("mode");

			String mqlQueryString = "print bus $1 select $2 dump";
			String strCopyTaskPreference = MqlUtil.mqlCommand(context, mqlQueryString, objectId,
					"attribute[Copy Task Preference]");

			String strAttributeInt = ProgramCentralConstants.ATTRIBUTE_COPYTASKPREFERENCE;
			AttributeType atrCopyTaskPreference = new AttributeType(strAttributeInt);
			atrCopyTaskPreference.open(context);
			StringList strCopyTaskPreferenceRangeList = atrCopyTaskPreference.getChoices(context);
			atrCopyTaskPreference.close(context);
			...
			if ("edit".equals(strMode)) {
				...
				for (String strCopyTaskPreferenceRange : strCopyTaskPreferenceRangeList) {
					strHTMLBuffer.append("<input type='radio' name='CopyTaskPreference' id='CopyTaskPreference' ");
					strHTMLBuffer.append("value ='" + strCopyTaskPreferenceRange + "' ");
					if (strCopyTaskPreference.equalsIgnoreCase(strCopyTaskPreferenceRange)) {
						strHTMLBuffer.append("checked='true' ");
					}
					strHTMLBuffer.append(" style=\"margin: 0 1px 4px 6px; \" />"); //To align with radio button

					strHTMLBuffer
							.append(i18nNow.getRangeI18NString(strAttributeInt, strCopyTaskPreferenceRange, strLanguage));
				}

			} else {
				strHTMLBuffer.append(i18nNow.getRangeI18NString(strAttributeInt, strCopyTaskPreference, strLanguage));
			}
			return strHTMLBuffer.toString();
		}

		public boolean updateCopyTaskPreference(Context context, String args[]) throws Exception {
			HashMap programMap = (HashMap) JPO.unpackArgs(args);
			...
			try {
				...
				String strCopyTaskPreference = objParent.getInfo(context, ProgramCentralConstants.SELECT_COPYTASKPREFERENCE);
				if (strCopyTaskPreference.equalsIgnoreCase(newValue)) {
					return true;
				}

				objParent.setAttributeValue(context, ProgramCentralConstants.ATTRIBUTE_COPYTASKPREFERENCE, newValue);

			} catch (Exception exp) {
				DebugUtil.debug("updateCopyTaskPreference " + exp.getMessage());
			}
			return true;
		...
		public boolean hasAccessToPMCCalculateFloatAndCriticalPathCommand(Context context,String args[]) throws Exception
		{
			...
			try {
				...
				excludeTableList.add("PMCWBSAssignmentAllocationViewTable");
...
/**
	    ...
	    public String getImportOption(Context context,String[]args)throws Exception{
	    	...
	    	if (ProgramCentralUtil.isNullString(parentOID)) {
	    		parentOID = selectedTaskId;
	    	}
	    	
	    	StringList objectSelects = new StringList();
            objectSelects.add(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
            objectSelects.add(ProgramCentralConstants.SELECT_IS_EXPERIMENT_TASK);
	    	
            MapList infoMapList = new MapList();
            Map taskInfoMap = new HashMap();
	    	
            if(ProgramCentralUtil.isNotNullString(parentOID)) {
	    	infoMapList = (MapList) Task.getInfo(context, new String[] {parentOID}, objectSelects);
	    	taskInfoMap = (Map) infoMapList.get(0);
            }
			...
			boolean assigneeCheck = com.dassault_systemes.enovia.dpm.ProjectSpace.getExpressionValue(context,
					"DPM_CopyTaskAssignees", false);
			boolean deliverablesCheck = com.dassault_systemes.enovia.dpm.ProjectSpace.getExpressionValue(context,
					"DPM_CopyTaskDeliverables", false);
			...
			if (isVisibleForTemplate) {
				if (!isCopyFromFileForm && !"Import".equalsIgnoreCase(ImportProject)) {
					...
					if (!"TRUE".equalsIgnoreCase((String)taskInfoMap.get(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT)) && !"TRUE".equalsIgnoreCase((String)taskInfoMap.get(SELECT_IS_EXPERIMENT_TASK))) {
					sb.append("<tr>");
			...
			else if(!isVisibleForTemplate && !isCopyFromFileForm){
				sb.append("<tr>");
				sb.append("<td  width=\"135\">");
				sb.append("<input type=\"checkbox\" name=\"deliverables\" value='True'");
				if (deliverablesCheck) {
     ...
     */ 
         public void createProjectFromProjectTemplate(Context context, String[] args) throws Exception {
    	   try {
    		...
    		new ProjectService().completePTCloningProcess(context,projectId,taskMapList,answerList,defaultTaskConstraintType, selectedOptionMap);
    	}catch(Exception e) {
    		e.printStackTrace();
    		throw e;
    	}
    ...
    public Vector getNameColumn (Context context, String[] args) throws Exception
    {
		...
		try {
         ...
         slBusSelect.add(DomainConstants.SELECT_NAME);
         slBusSelect.add(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
         slBusSelect.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
         ...
         for(int i=0; i<size; i++) {
             ...
             boolean isKindOfProjectSpace          = "TRUE".equalsIgnoreCase((String) objectWithSelect.getSelectData(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE));
             
//             if(ProgramCentralUtil.isNotNullString((String) objectWithSelect.getSelectData(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE)) && isKindOfProjectSpace) {
//            	 projectName = (String) objectWithSelect.getSelectData(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
//				}
             
             String projectName = ProgramCentralUtil.getTitleForProjectType(context, objectWithSelect, isKindOfProjectSpace);
	...
	public void promoteSuccessor3DNotification(Context context, String[] args) throws Exception {			
	 ...
	 public StringList getQuestionAnswerFromResponse(Context context, String[] args) throws Exception {	    			
		...
		if(jsonData.contains("QuestionInfo")){
			...
			for(int i=0;i<objectList.size();i++){
				Map questionMap = (Map)objectList.get(i);
				String questionId = (String)questionMap.get(DomainObject.SELECT_ID);
				String questionPhysicalIdIdId = (String)questionMap.get(ProgramCentralConstants.SELECT_PHYSICALID);
				StringBuilder sb = new StringBuilder();
					for(int jsonArrItr = 0; jsonArrItr<tempjsonArr.length(); jsonArrItr++){
					JSONObject tempJSONObj = tempjsonArr.getJSONObject(jsonArrItr);
					if(questionId.equalsIgnoreCase((String)tempJSONObj.getString("questionId")) || questionPhysicalIdIdId.equalsIgnoreCase((String)tempJSONObj.getString("questionId")))
					{
	 ...
	 public StringList getQuestionTasksFromResponse(Context context, String[] args) throws Exception {
		...
		if(ProgramCentralUtil.isNotNullString(selectedProjectId) && selectedProjectId != EMPTY_STRING){
			 ...
			 relIdTemp = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump",selectedProjectId,temp1);
    		...
    		String relWhere = EMPTY_STRING; 
			 ...
			 MapList relatedObjectsMapList = selectedProjectObj.getRelatedObjects(context,RELATIONSHIP_SUBTASK,"*",
		...
		if(ProgramCentralUtil.isNotNullString(relIdTemp) && relIdTemp != EMPTY_STRING){
			responseDataJSON = (String) DomainRelationship.getAttributeString(context, relIdTemp, "ProjectQuestionResponse");
				...
				for(int i=0;i<objectList.size();i++){
					...
					String qrListString = "";
					String questionId = (String)questionMap.get(DomainObject.SELECT_ID);
			 String questionPhysicalId = (String)questionMap.get(SELECT_PHYSICALID);
		...
		if(jsonData.contains("QuestionInfo")){
			...
			for(int jsonArrItr = 0; jsonArrItr<tempjsonArr.length(); jsonArrItr++){
				...
				String responseForQuestion = (String)tempJSONObj.getString("response");
					...
					StringBuilder sb = new StringBuilder();
							 if((questionId.equalsIgnoreCase((String)tempJSONObj.getString("questionId")) || questionPhysicalId.equalsIgnoreCase((String)tempJSONObj.getString("questionId"))) && sltaskResponse.size()==sltaskPhysicalIds.size())
					{
			...
			for(int jsonArrItr = 0; jsonArrItr<tempjsonArr.length(); jsonArrItr++){
				...
				String responseForQuestion = (String)tempJSONObj.getString("response");
					...
					StringBuilder sb = new StringBuilder();
						 if(questionId.equalsIgnoreCase((String)tempJSONObj.getString("questionId")) || questionPhysicalId.equalsIgnoreCase((String)tempJSONObj.getString("questionId")))
					{
						String connectedTaskNames = "";
			...
			busSelects.add("from[Question].to.physicalid");
	  		...
	  		String busWhere =  question.SELECT_HAS_ITEMS + "== True&&current==" + ActiveState;
				...
				for(int mapListItr = 0; mapListItr<questionList.size(); mapListItr++){
					...
					String questionPhysicalIdId = (String) m.get(ProgramCentralConstants.SELECT_PHYSICALID);
					if(responseDataOnRelationship != EMPTY_STRING && (responseDataOnRelationship.contains(questionId) || responseDataOnRelationship.contains(questionPhysicalIdId)))
					{
...
public String getForecastBasedOnValue(Context context, String args[]) throws MatrixException {
	    	String language = context.getSession().getLanguage();
	    	String ForecaseBasedOn = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.ForecastBasedOn.CurrentProgress", language);
	    	try {
	    		Map programMap = (Map) JPO.unpackArgs(args);
	    		Map paramMap = (Map)programMap.get("paramMap");
	    		String strId = (String) paramMap.get("objectId");
	    		...
	    		DomainObject project = DomainObject.newInstance(context, strId);
	    		String hasInterface = project.getInfo(context,"interface[DPMPlannedProgressForecast]");

	    		if("True".equalsIgnoreCase(hasInterface)) {
	    			ForecaseBasedOn = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.ForecastBasedOn.PlannedProgress", language);
	    		}
	    		return ForecaseBasedOn;
	    	} catch(Exception e) {
	    		throw new MatrixException(e);
	    	}
	    }
	    ...
	    @com.matrixone.apps.framework.ui.ProgramCallable
		public Map getForecastBasedOnValuesRange(Context context, String args[]) throws MatrixException{

			Map forecastBasedOnRange = new HashMap();
			String language = context.getSession().getLanguage();
			
			String i18CurrentProgress = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.ForecastBasedOn.CurrentProgress", language);
			String i18PlannedProgress = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.ForecastBasedOn.PlannedProgress", language);

			try {	
				StringList valueList = new StringList(2);
				valueList.add("CurrentProgress");
				valueList.add("PlannedProgress");
				
				StringList displayList = new StringList(2);
				displayList.add(i18CurrentProgress);
				displayList.add(i18PlannedProgress);
				
				forecastBasedOnRange.put("field_choices", valueList);
				forecastBasedOnRange.put("field_display_choices", displayList);
				
			}catch(Exception e){
				throw new MatrixException(e);
			}
			return forecastBasedOnRange;
		} 

	    public void updateForecastBasedOn(Context context, String args[]) throws MatrixException{
	    	try {
	    		Map programMap = (Map)JPO.unpackArgs(args);
	    		Map paramMap = (HashMap) programMap.get("paramMap");
	    		String val = (String) paramMap.get("New Value");
	    		...
	    		if(val.equalsIgnoreCase("CurrentProgress")) {
	    			MqlUtil.mqlCommand(context, "modify bus $1 remove $2 $3",  projectId, "interface", "DPMPlannedProgressForecast");
	    		}else {
	    			MqlUtil.mqlCommand(context, "modify bus $1 add $2 $3",  projectId, "interface", "DPMPlannedProgressForecast");
	    		}
	    ...
	    /**
	     * Create new Project Template with reference to Project Space
	     * @param context - The eMatrix <code>Context</code> object.
	     * @param args holds information about object.
	     * @return Map of newly created obejct id;
	     * @throws Exception if operation fails.
	     */
	    @com.matrixone.apps.framework.ui.CreateProcessCallable  
	    public Map createProjectTemplateFromPS(Context context, String args[]) throws Exception{
	    	Map programMap = (Map)JPO.unpackArgs(args);
	    	Map <String,String>returnMap = new HashMap();
	    	try {
	    		String selectedProject = (String)programMap.get("SeachProjectOID");
	    		String name = (String)programMap.get("Name");
		    	Dataobject sourceProject = new Dataobject();
				sourceProject.setId(selectedProject);  
				DataelementMapAdapter.setDataelementValue(sourceProject, "name",name);
								
				final Servicedata inputService = new Servicedata();
				inputService.getData().add(sourceProject);
				ServiceParameters lParameters = new ServiceParameters();
				final String serviceName = "dpm.projecttemplates";
				lParameters.setServiceName(serviceName + "/copy");			
				ArgMap args1 = new ArgMap();
				lParameters.setServiceArgs(args1);
				lParameters.setDefaultUpdateAction(UpdateActions.MODIFY);
				...
				Servicedata response = ServiceBase.saveData(context, inputService, lParameters, 0);		
				String newProjectTemplateId = (String)response.getData().get(0).getId();
				System.out.println("newProjectTemplateId :"+newProjectTemplateId);
				returnMap.put("id", newProjectTemplateId);
	    	}catch(Exception e){
	    		throw new MatrixException(e);
	    	}
	    	return returnMap;
	    ...
	    /**
	  	 ...
	  	 */
	  	@com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
		public StringList excludeExperimentBaselineInactiveProjects(Context context,String[]args)throws Exception
		{
			StringList slFinalList = new StringList();
			try{
				StringList slSelect = new StringList(2);
				...
				slSelect.add(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
				slSelect.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_BASELINE);
				slSelect.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
				slSelect.add(ProgramCentralConstants.SELECT_CURRENT);
				
				String typePattern = ProgramCentralConstants.TYPE_EXPERIMENT+","+ProgramCentralConstants.TYPE_PROJECT_BASELINE+","+ProgramCentralConstants.TYPE_PROJECT_SPACE;			
				MapList mpProjectList = DomainObject.findObjects(context,
						typePattern,
						null,
						null,
				...
				for (Iterator iterator = mpProjectList.iterator(); iterator.hasNext();) {
					...
					String isExpProject = (String)projectInfoMap.get(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
					String isProjectBaseline = (String)projectInfoMap.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_BASELINE);
					String isprojectSpace = (String)projectInfoMap.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
					String currentState = (String)projectInfoMap.get(ProgramCentralConstants.SELECT_CURRENT);
					
					if("true".equalsIgnoreCase(isExpProject) || "true".equalsIgnoreCase(isProjectBaseline) || 
							("true".equalsIgnoreCase(isprojectSpace) && (ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_HOLD.equalsIgnoreCase(currentState) ||
									ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_CANCEL.equalsIgnoreCase(currentState) || ProgramCentralConstants.STATE_PROJECT_SPACE_ARCHIVE.equalsIgnoreCase(currentState)))){
						slFinalList.addElement(objectId);
					}
				}
	    ...
	    /**
	     * Exports Multiple projects in zip file
	     * @param context the eMatrix <code>Context</code> object
	     * @param args
	     * @throws FoundationUserException,Exception
	     */

	    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	    public void exportProjectsProcess(Context context, String args[]) throws FoundationUserException, Exception {
			Map paramMap = (Map)JPO.unpackArgs(args);
			String[] objectIds = (String[]) paramMap.get("projectIds");
			String programId = (String) paramMap.get("programId");
			String extension = (String) paramMap.get("extension");
			String loggedInUser=(String) context.getUser();
			String language = (String) paramMap.get("language");
			String invokeFrom 	= (String)paramMap.get("invokeFrom"); //Added for OTD
			Map<String, Integer> projectNameCountMap = new HashMap<String, Integer> ();
			String zipFileName = "";
			String programName = DomainConstants.EMPTY_STRING;
		
			if (programId != null) {
				DomainObject programObj = new DomainObject(programId);
				programName = programObj.getName(context);
				zipFileName = "Program-";

			}
			zipFileName += "Projects.zip";
			...
			try (ZipOutputStream zos = new ZipOutputStream(fos)) {
				for (String PrjObjectId : objectIds) {
					Map projectParamMap = new HashMap();
					DomainObject selectedProject = new DomainObject(PrjObjectId);
					...
					if (!"True".equalsIgnoreCase(isProjectSpace)) {
						String errorMessage = ProgramCentralUtil.getPMCI18nString(context,
								"emxProgramCentral.Common.SELECT_PROJECT_SPACES", (String) paramMap.get("language"));
						throw new FoundationUserException(errorMessage);
					}

					projectParamMap.put("objectId", PrjObjectId);
					projectParamMap.put("exportFormat", paramMap.get("exportFormat"));
					projectParamMap.put("language", language);
					projectParamMap.put("exportSubProject", paramMap.get("exportSubProject"));

					StringList projectTasks = exportProjectTaskList(context, JPO.packArgs(projectParamMap));

					try {
						// creation of fileame, if duplicated names found, in that case appending
						// counter value to it
						fileName = fileName == null ? "" : fileName;
						if (projectNameCountMap.containsKey(fileName)) {
							Integer counter = projectNameCountMap.get(fileName);
							projectNameCountMap.put(fileName, counter + 1);
							fileName += "_" + counter;
						} else {
							projectNameCountMap.put(fileName, 1);
						}
						fileName += extension;

						File srcFile = new File(fileName);
						zos.putNextEntry(new ZipEntry(srcFile.getName()));
						for (String row : projectTasks) {
							row += "\n";
							byte[] rowBytes = row.getBytes();
							zos.write(rowBytes, 0, rowBytes.length);
						}
					} catch (Exception ex) {
						ex.printStackTrace();
					} finally {
			...
			//sending email of with zip file as an attachment
			Map mailMap = new HashMap();
			String subjectParamsValue = DomainConstants.EMPTY_STRING;
			if(ProgramCentralUtil.isNotNullString(programId) && !programId.isEmpty()){
				subjectParamsValue = programName;
			}
			else{
				subjectParamsValue = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Common.PROJECT_LIST",language);
			}
			
			
			String subjectMsg = "emxProgramCentral.Common.FILE_EXPORTED_SUBJECT";
	        String sKey[] = {"source"};
	        String sValue[] = {subjectParamsValue};

	        String companyName = null;
	        String subject  = emxProgramCentralUtilClass.getMessage(context,
	        		subjectMsg,
	                sKey,
	                sValue,
	                companyName);
			
			String[] toEmailAddress = new String[]{PersonUtil.getEmail(context)};
			String[] attachments = new String[]{zipFileName};
			String content = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Common.MAIL_FIND_ATTACHMENT",language).replaceAll("%FILE_NAME%",zipFileName);
			
			mailMap.put("subject", subject);
			mailMap.put("content", content);
			mailMap.put("toEmailAddress", toEmailAddress);
			mailMap.put("attachments", attachments);
			
			if(!"TestCase".equalsIgnoreCase(invokeFrom)) {
				ProgramCentralUtil.sendMail(context, subject, content, toEmailAddress, attachments);
			}
		}
	    
    /**
     * Trigger method to send notification to Person or User Group Members on adding that Person or User Group to a Project.
     * 
     * @param context
     * @param args
     * @throws Exception
     */
    public void notifyAddedMember(Context context, String[] args) throws Exception {
    	
    	String projectId = args[0];
    	String addedOwnership = args[1];
    	String customDataKey = "AccessChange_For_Project_"+projectId+"_For_"+addedOwnership;
    	boolean isCallForRemove = false;
    	
    	//START:IR-1366530-3DEXPERIENCER2025x
    	String projectMemberAccessChange = context.getCustomData(customDataKey);
    	
    	//Send notification if projectMemberAccessChange is null or empty. 
    	//customDataKey is set TRUE indicates that use access is changed in project and not addition of new project member.
    	if(ProgramCentralUtil.isNullString(projectMemberAccessChange)) {
    		notifyMember(context, projectId, addedOwnership, isCallForRemove);
    	}
    	//Clear customDataKey every time.
    	context.removeFromCustomData(customDataKey);
    	//END:IR-1366530-3DEXPERIENCER2025x
    }
    /**
     * Trigger method to send notification to Person or User Group Members on removal of that Person or User Group from a Project. 
     * @param context
     * @param args
     * @return
     * @throws Exception
     */
    public void notifyRemovedMember(Context context, String[] args) throws Exception {
    	
    	String projectId = args[0];
    	String removedOwnership = args[1];
    	boolean isCallForRemove = true;
    	
        	notifyMember(context, projectId, removedOwnership,isCallForRemove);
        }
    /**
     * This method sends 3D Notification to Person or User Group Members on addition or removal of that Person or User Group for a Project. 
     * 
     * @param context
     * @param projectId
     * @param memberName
     * @param isCallForRemove true indicates 
     * @return
     * @throws Exception
     */
    private void notifyMember(Context context, String projectId, String memberName,boolean isCallForRemove) throws Exception {
    	...
    	roleSelectList.add("isarole");
    	roleSelectList.add("isaprojectgroup");
    	roleSelectList.add("isaproject");
    	roleSelectList.add("parent");
    	roleSelectList.add("|");
    	...
    	userGroupSelectList.add(DomainConstants.SELECT_ATTRIBUTE_TITLE);
    	userGroupSelectList.add("|");
    	
    	try {
             //Person: list role p6e_PRJ select isarole isaprojectgroup isaproject parent;
    		 //User Group:list role auto_31736245626182 select isarole isaprojectgroup isaproject parent;
    		 //Security Context: list role Supplier select isarole isaprojectgroup isaproject parent;
             String result = MqlUtil.mqlCommand(context, "list role $1 select $2 $3 $4 $5 dump $6", true,roleSelectList);
             StringList resultList = FrameworkUtil.split(result, "|");
            
             String isarole = resultList.get(0);
             String isaprojectgroup = resultList.get(1);
             String isaproject = resultList.get(2);
             String parent = null;
             if(resultList.size()>= 4) {
            	 parent = resultList.get(3);
             }

             if("TRUE".equals(isarole)) {
            	  
            	 if("TRUE".equals(isaprojectgroup)) {//User Group
            		 
            		 boolean sendNotification = shouldSendNotification(context, projectId, memberName, isCallForRemove);
            		 ...
            		 String userGroupMQLResult =  MqlUtil.mqlCommand(context, mqlQuery, true,userGroupSelectList);
            			 ...
            			 if(userGroupInfoList.size() >= 5) {
            				 ...
            				 String userGroupTitle = userGroupInfoList.get(4);
            				 userGroupIdList.add(userGroupId);
            			 
            			 com.dassault_systemes.enovia.dpm.notification.NotificationUtil.sendProjectMemberNotification(context, projectId, userGroupTitle,userGroupIdList, personNameList, isCallForRemove);
            		 }
            		 }
            		 
            	 } else if("TRUE".equals(isaproject)) {
            		 
            		 if("User Projects".equals(parent) && memberName.contains("_PRJ")) {//Person
            			 
            			 boolean sendNotification = shouldSendNotification(context, projectId, memberName, isCallForRemove);
            			 
						if(sendNotification) {
							String personName = memberName.split("_")[0];
        		        	personNameList.add(personName);
            			 com.dassault_systemes.enovia.dpm.notification.NotificationUtil.sendProjectMemberNotification(context, projectId, null, userGroupIdList, personNameList, isCallForRemove);
        		        }
                	 } else {//Security Context
                		// TODO: Code to write for Security Context
                	 }
            	 }
            	 
             }
         } catch (Exception e) {
        	 DebugUtil.debug("notifyRemovedMember failed ..."+e);
        	 e.printStackTrace();
     	}
    }
    /**
     * This method always returns true for add/removal of Person or User Group as Project Member.
     * This method return false if Person or User Group access is changed from Reader to Leader or vice versa.
     * 
     * @param context
     * @param projectId
     * @param memberName
     * @param isCallForRemove
     * @param memberType
     * @return
     * @throws Exception
     */
    private boolean shouldSendNotification(Context context, String projectId, String memberName,boolean isCallForRemove) throws Exception {
    	
    	boolean shouldSendNotification = true;
    	
    	//return true for member add.
    	if(!isCallForRemove) {
    		return shouldSendNotification;
    	}
		String customDataKey = "AccessChange_For_Project_"+projectId+"_For_"+memberName;
		String mqlResult = "";
		 try {
			 //Person ownership = -|VPLMAdminUser_PRJ|Multiple Ownership For Object
			 //Group ownership = -|auto_11736493642793|Multiple Ownership For Object
			 //Security Context ownership = Corporate Plant|Private|Multiple Ownership For Object
			 mqlResult = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", true,projectId,"ownership","#");
		 } catch(Exception exception) {
			 exception.printStackTrace();
		 }
		 String[] ownershipStringArray = mqlResult.split("#");
		 for(String ownershipString :ownershipStringArray) {
			 if(ownershipString.contains(memberName)) {
				 //Don't send remove notification if member exists in the project.
				 //Add customData to be used in notifyAddedMember().
				 context.setCustomData(customDataKey,"TRUE");
		...
		@com.matrixone.apps.framework.ui.ProgramCallable
	    public String getBookmarkContentOptions(Context context, String args[]) throws Exception
		{
			StringBuffer strHTMLBuffer = new StringBuffer();

			String strLanguage=context.getSession().getLanguage();
			String docCopy = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.Common.Copy", strLanguage);
			String docConnect = EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.BookmarkContent.Reference", strLanguage);
			String docNone= EnoviaResourceBundle.getProperty(context,ProgramCentralConstants.PROGRAMCENTRAL, "emxProgramCentral.BookmarkContent.None", strLanguage);
			
			strHTMLBuffer.append("<input type='radio' name='BookmarkContent'  value='None' />");
			strHTMLBuffer.append(docNone);
			
			strHTMLBuffer.append("<input type='radio' name='BookmarkContent'  value='Copy' checked='true' />");
			strHTMLBuffer.append(docCopy);
			strHTMLBuffer.append("<input type='radio' name='BookmarkContent'  value='Reference' />");
			strHTMLBuffer.append(docConnect);
  	...
  	public StringList getImportedObjectPriority(Context context,String[]args)throws Exception
  	{
  		StringList importObjectPriorityList = new StringList();
  		try{
  			Map programMap = (Map)JPO.unpackArgs(args);
  			MapList objectList = (MapList)programMap.get("objectList");
  			...
  			String isStructureExpanded = (String)CacheUtil.getCacheObject(context, "noMoreExpandTPColumn") ;
			
  			if(ProgramCentralUtil.isNullString(isStructureExpanded)|| "true".equalsIgnoreCase(isRootNode)){  
  				for(int i=0;i<objSize;i++){
  					Map fileMap = (Map)objectList.get(i);
					
  					String taskPriority = (String)fileMap.get("Task Priority");
					
  					importObjectPriorityList.addElement(taskPriority);
  				}
  				CacheUtil.setCacheObject(context, "noMoreExpandTPColumn", "true");
  			}

  		}catch(Exception ex){
  			ex.printStackTrace();
  		}
  		return importObjectPriorityList;
  	}
	
		@com.matrixone.apps.framework.ui.ProgramCallable
		public String getExecuteInBackgroundOption(Context context, String[] args) throws Exception {				
		Map programMap          = JPO.unpackArgs(args);

	    	Map paramMap        = (Map) programMap.get("paramMap");
	    	Map requestMap        = (Map) programMap.get("requestMap");
	    	String ImportProject =(String) requestMap.get("createProject");
			String backgoundImportOption = EnoviaResourceBundle.getProperty(
            context,
            ProgramCentralConstants.PROGRAMCENTRAL,
            "emxProgramCentral.Common.ExecuteInBackground",
		...
		sb.append("<tr>");
		sb.append("<td colspan=\"2\" width=\"300\">");
		sb.append("<input type=\"checkbox\" id=\"ExecuteInBackground\" name=\"ExecuteInBackground\" value=\"true\" />");
		sb.append("</td>");
		sb.append("</tr>");
		sb.append("</table>");
	...
	@com.matrixone.apps.framework.ui.ProgramCallable
public String getProjectOwnerField(Context context, String[] args) throws Exception {
        HashMap programMap = (HashMap) JPO.unpackArgs(args);

    com.matrixone.apps.program.ProjectSpace projectSpace =
            (com.matrixone.apps.program.ProjectSpace) DomainObject.newInstance(
                    context,
                    DomainConstants.TYPE_PROJECT_SPACE,
                    DomainConstants.PROGRAM
            );
    ...
    Map theProjectMap = (Map) projectSpace.getInfo(context, selectables);

    String OwnerName = (String) theProjectMap.get(ProgramCentralConstants.SELECT_OWNER);
    String ownerId = person.getPerson(context, OwnerName).getId();
    ...
    String OwnerFullName = strLastName + " " + strFirstName;

    StringBuilder sb = new StringBuilder();

    if ("view".equalsIgnoreCase(strMode)) {
        sb.append(OwnerFullName != null ? OwnerFullName : "");
    }
    ...
    if ("edit".equalsIgnoreCase(strMode)) {
        ...
        sb.append("<input type='hidden' id='hiddenProjectMembers' name='hiddenProjectMembers' value='")
          .append(ownerId != null ? ownerId : "")
          .append("' />");

        sb.append("<input type='text' id='projectMembers' name='projectMembers' value='")
          .append(OwnerFullName != null ? OwnerFullName : "")
          .append("' readonly='true' />");


        String baseURL = "../common/emxFullSearch.jsp"
                + "?field=TYPES=type_Person:CURRENT=policy_Person.state_Active:TYPES=type_Person:USERROLE=Project Lead,VPLMProjectLeader,3DSRestrictedLeader"
                + "&form=PMCCommonPersonSearchForm"
                + "&selection=single"
                + "&fieldNameActual=hiddenProjectMembers"
                + "&fieldNameDisplay=projectMembers"
                + "&parentOID=" + objectId
                + "&suiteKey=ProgramCentral"
                + "&type=PERSON_CHOOSER"
                + "&table=PMCCommonPersonSearchTable"
                + "&objectId=" + objectId
                + "&chooserType=direct"
                + "&submitURL=../programcentral/emxProjectManagementUtil.jsp?mode=addMemberFromUserGroup"
                + "&suiteKey=ProgramCentral"
                + "&StringResourceFileId=emxProgramCentralStringResource";


        String iconPath1 = "../programcentral/images/I_CommonIcon_User_22.png";
        String toolTip1 = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                                    "emxProgramCentral.common.selectProjectOwner.tooltip", languageStr);
        sb.append("<a href=\"javascript:showFullSearchChooserInForm('")
          .append(baseURL)
          .append("','hiddenProjectMembers','600','600')\">")
          .append("<img src=\"").append(iconPath1).append("\" title=\"").append(toolTip1).append("\"/></a>");

        sb.append("</td>");

        sb.append("<td style='padding-left:20px;'>");
        String iconPath2 = "../programcentral/images/I_CommonIcon_UserGroup_22.png";
		String toolTip2 = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
                                    "emxProgramCentral.common.UserGroup.tooltip", languageStr);

        sb.append("<a href=\"javascript:showChooser('../common/emxIndentedTable.jsp?")
          .append("&table=PMCTransferAssignment")
          .append("&selection=single")
          .append("&header=emxProgramCentral.Common.Search")
          .append("&memberType=Person")
          .append("&program=emxProjectMember:getAllMembers")
          .append("&hideHeader=true")
          .append("&suiteKey=ProgramCentral")
          .append("&cancelLabel=emxProgramCentral.Common.Close")
          .append("&direction=from")
          .append("&objectId=").append(objectId)
          .append("&chooserType=group")
          .append("&submitURL=../programcentral/emxProjectManagementUtil.jsp?mode=addMemberFromUserGroup")
          .append("&suiteKey=ProgramCentral")
          .append("&StringResourceFileId=emxProgramCentralStringResource")
          .append("')\">")
          .append("<img src=\"").append(iconPath2).append("\" title=\"").append(toolTip2).append("\"/></a>");
...
public StringList getUserGroupsForMembers(Context context, String[] args) throws Exception {
    ...
    selects.add("id");
    selects.add("to[Group Member].from.attribute[Title]");
	
    String[] ids = new String[objectList.size()];
    for (int i = 0; i < objectList.size(); i++) {
        Map objMap = (Map) objectList.get(i);
        ids[i] = (String) objMap.get("id");
    }

    MapList infoList = DomainObject.getInfo(context, ids, selects);
    ...
    for (int i = 0; i < infoList.size(); i++) {
        ...
        Object groupObj = infoMap.get("to[Group Member].from.attribute[Title]");
        String groupTitles = "";
        if (groupObj instanceof StringList) {
            groupTitles = String.join(",", (StringList) groupObj);
        } else if (groupObj != null) {
            groupTitles = ((String) groupObj)
                    .replaceAll("[\\u0007]", ",")
                    .replaceAll(",+", ",")
                    .trim();
        }
        returnList.add(groupTitles);
    }
