...
<%
  com.matrixone.apps.program.Task task =
   ...
   if(relationshipIds.length > 0){
	...
	StringList tasksWithoutDeleteAccessList = (StringList)preProcessMap.get("tasksWithoutDeleteAccess");
    ...
    Map<String,String> projectScheduleMap = ProgramCentralUtil.getProjectSchedule(context, topId);
	   projectSchedule = (projectScheduleMap.get(topId) != null && !projectScheduleMap.get(topId).isEmpty()) ? projectScheduleMap.get(topId): ProgramCentralConstants.PROJECT_SCHEDULE_AUTO;
...
if(boolStructureBrowser){
    ...
    if(!tasksWithoutDeleteAccessList.isEmpty()){
    	String strTasksWithoutDeleteAccess = EnoviaResourceBundle.getProperty(context, "ProgramCentral", 
  			  "emxProgramCentral.Common.CannotDeleteTaskNoDeleteAccess", strLanguage);
        message = message+"\\n"+strTasksWithoutDeleteAccess+"\\n"+tasksWithoutDeleteAccessList;
		}
...
%>
    <script type="text/javaScript">
		var currentFrameName = "<%=currentframe%>";
		var displaytopFrame = findFrame(getTopWindow(), "detailsDisplay");
		if(null==displaytopFrame)
		{
			 displaytopFrame = findFrame(getTopWindow(), "portalDisplay");
		}
