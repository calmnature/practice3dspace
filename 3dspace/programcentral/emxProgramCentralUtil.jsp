...
<%
			...
			requestMap.put("timeZone", (String)session.getAttribute("timeZone"));
		    ...
		    String templateObjectId = (String)session.getAttribute("selectedTemplateId");
					...
					if(!isFileCurrpted){
						...
						if(ProjectSpacederivatives.contains(objectType)){
							...
							String objectStartDate = (String)importedObjectMap.get("Task Estimated Start Date");
							 String objectDescription = (String)importedObjectMap.get("description");
							 // Check if objectDescription(i.e from CSV )  is not null or not empty
							if (objectDescription != null && !objectDescription.trim().isEmpty()) {
								projectDescription = objectDescription.trim();
								}
			...
			if(ProgramCentralUtil.isNotNullString(selectedProjectId)){
				...
				MapList projectInfoMapList = project.getSelectedProjectInfo(context, sourceTaskIdArray,  (String)session.getAttribute("timeZone"), request.getLocale());				
			...
			String loggedinUser = context.getUser();
			CacheUtil.removeCacheObject(context, loggedinUser+"_QuestionsResponse");
		    boolean isExecuteInBackgroundOptionChecked =
            "TRUE".equalsIgnoreCase(emxGetParameter(request, "ExecuteInBackground"));
			StringBuffer treeUrl = null;
			if (!isExecuteInBackgroundOptionChecked) {
				treeUrl = new StringBuffer("../common/emxTree.jsp?AppendParameters=true" +
                "&treeNodeKey=node.ProjectSpace&suiteKey=eServiceSuiteProgramCentral&objectId=" +
                XSSUtil.encodeForURL(request.getParameter("newObjectId")) +
                "&DefaultCategory=PMCGateDashboardCommandPowerView" +
                "&emxSuiteDirectory=programcentral&treeTypeKey=type.Project");
    }
	%>
        <script language="javascript" type="text/javascript">
				 var isBackgroundChecked = "<%= isExecuteInBackgroundOptionChecked %>";
	...
	<%
			...
			session.setAttribute("objectList", mlSummaryTasks);
		    return;
			...
			session.setAttribute("objectList", finalObjectList);
			return;
		   ...
		   <%
				...
				if(!errorMassageMap.isEmpty()){
					if(errorMassageMap.containsKey("error")){
	    		 ...
	    		 return;
						...
						//IR-1287263
						Object errorNoticeMessage = errorMassageMap.get("notice");
						String errorNoticeMessageStr = (String)errorNoticeMessage;
						hasNotice = true;
						%>
		    		 	<script language="javascript" type="text/javaScript">
	 		 				alert("<%=errorNoticeMessageStr%>");
	 		 			</script>
		    		  <%
					}
				}
	...
	<%
		...
		try{
     	...
     	if(parentIdList.isEmpty()){
		            ...
		            if(topFrame.dataGridEnabled){
							...
							var index = 0;
								...
								if("moveUp"==action){
									...
									if (abvRow.getAllDescendants() != null){
										for(var i=0,len=abvRow.getAllDescendants().length;i<len;i++){
											impObjRowIdArr[count++] = topFrame.emxEditableTable.getAttribute(abvRow.getAllDescendants()[i],"rowId");
										}
										}
										if (childRows[index].getAllDescendants() != null){
											for(var i=0,len=childRows[index].getAllDescendants().length;i<len;i++){
												impObjRowIdArr[count++] = topFrame.emxEditableTable.getAttribute(childRows[index].getAllDescendants()[i],"rowId");
											}
											}
			...
			*/
			...
			if(!(ProgramCentralConstants.POLICY_PROJECT_TEMPLATE.equalsIgnoreCase(policy))){
				Map paramMap = new HashMap(1);
				paramMap.put("objectId",objectId);
       ...
       <%
        ...
        if((bookmarkWSId != null) && (!bookmarkWSId.equals(""))){
                String copyOfobjectId = objectId;
                ...
                else 
                {
                    context.execWithPrivilege(() -> {
                        ...
                        MqlUtil.mqlCommand(context,"modify bus $1 remove interface $2", copyOfobjectId, INTERFACE_DPMBOOKMARKROOT);
                    });
                }
        }
...
<%
			...
			searchURL +=":TSK_TASK_HAS_CONTEXT!=NO_CONTEXT";
				...
				searchURL +=":PRG_TASK_ROOT_CURRENT!=Inactive,Obsolete,Draft,In Approval";
    ...
    <%
           ...
           <%
				   ...
				   String policy = MqlUtil.mqlCommand(context,"print bus $1 select $2 dump", projectId, "policy");
				   String[] ids = emxGetParameterValues(request, "emxTableRowId");
				   ...
				   try{
				   	...
				   	%>
					<script language="javascript">
					//START: IR-1483997-3DEXPERIENCER2026x
					var portalCmdName 		= "<%=portalCmd%>";   
				    var contentFrameObj = findFrame(getTopWindow(), portalCmdName);
	   				if(contentFrameObj == null){
	   					contentFrameObj = findFrame(getTopWindow(), "PMCProjectMember");
	   					if(contentFrameObj == null){
		     				    topFrame = findFrame(getTopWindow(), "PMCProjectTemplatePeople");
	   					}
	   				}
					if(contentFrameObj !== null){
					...
					//END: IR-1483997-3DEXPERIENCER2026x
		            </script>
		       		<%
		    	...
		    	try{
		    		...
		    		boolean isCoOwnerUser = false;
		    		StringList coOwnersList = new StringList();
		    		int len = ids.length;
					StringList deletionList = new StringList();

		    		for(int i =0; i<len;i++){
						boolean isOwnerOrLoggedInUser = false;
						boolean isCoOwnerSelected = false;       
		    			...
		    			if(idList.size() > 2){
		    				...
		    				if(ProgramCentralUtil.isNotNullString(strProject) && ProgramCentralUtil.isNullString(strOrganization)){
		    					...
		    					if(users.size()==1 && strProject.endsWith("_PRJ")){		    					
									...
									String fullName=PersonUtil.getFullName(context, result);
									...
									if(policy.equalsIgnoreCase(ProgramCentralConstants.POLICY_PROJECT_TEMPLATE)){
										if( (relId == null || "".equals(relId)) && !isOwnerOrLoggedInUser) {
											deleteOwnershipIds.add(ids[i]);   //deleting ownership only if User isnt coOwner
										}else {											
											coOwnersList.add(fullName);
											isCoOwnerSelected = true;
										}
									}else if( relId != null && !"".equals(relId) && ! isOwnerOrLoggedInUser){
									...
									isCoOwnerUser = isCoOwnerSelected ? true : false;   //flag used for alert
		    			...
		    			if(!isCoOwnerSelected && policy.equalsIgnoreCase(ProgramCentralConstants.POLICY_PROJECT_TEMPLATE)){
		    				deletionList.addElement(ids[i]);  //require in case of project template, do not remove policy check
		    			}else if(!isOwnerOrLoggedInUser && !policy.equalsIgnoreCase(ProgramCentralConstants.POLICY_PROJECT_TEMPLATE)){
		    				deletionList.addElement(ids[i]);  //require in case of project, do not remove policy check
				        }
		    		}
		    		...
		    		if (/*If contextUser,above alert is shown*/!isOwnerOrContextUser && isCoOwnerUser)   //alert for co-owner
		    		{
		    			String sErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.Common.UnableToRemoveCoOwners", context.getLocale().toString());
		    			sErrMsg=sErrMsg+"\\n"+coOwnersList;
		    			%>
	    			     <script language="JavaScript" type="text/javascript">											
			        	    	    alert("<%=sErrMsg%>");			        	    	   
			...
			return;
       		...
       		<script language="javascript">
   			   ...
   			   var detailsDisplay_2 = findFrame(getTopWindow(), "PMCWhatIfProjectExperimentsList");
   				    detailsDisplay_1.refreshSBTable(detailsDisplay_1.configuredTableName);
		...
		return;
            ...
            try {
                ...
                if (projectTemplates != null) {
                    ...
                    for (int i = 0; i < 1; i++) {
                        ...
                        busSelects.addElement(DomainConstants.SELECT_ATTRIBUTE_TITLE);
                        ...
                        HashMap requestMap = new HashMap(9);
                        requestMap.put("objectId", projectTemplateId);
                        if (ProgramCentralUtil.isNotNullString((String)map.get(DomainConstants.SELECT_ATTRIBUTE_TITLE))){
                        requestMap.put("Name", map.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
                        }else{
						...
						boolean referBookmarkDocument = com.dassault_systemes.enovia.dpm.ProjectSpace.getExpressionValue(context, "DPM_ReferBookmarkDocuments", false); 
						if(referBookmarkDocument){
							//While ReviseTemplate same bookmark document should use - no need to create a copy.
							requestMap.put("ReferBookmarkDocument", "ReferBookmarkDocument");
						}
            ...
            <%
	        	...
	        	//String objectId = (String)(ProgramCentralUtil.parseTableRowId(context,selectedNodeId)).get("objectId");
	         	String programObjectId = emxGetParameter(request, "objectId");
	        	...
	        	String url = "?exportFormat="+exportFormat;
	        	url =url+"&Accept-Language="+language;
				url =url+"&programId="+programObjectId;

				String serviceRequestObjectIds = DomainConstants.EMPTY_STRING;
	        	for(String objectIdSelected : selectedItems){
					serviceRequestObjectIds += (String)(ProgramCentralUtil.parseTableRowId(context,objectIdSelected)).get("objectId") + ",";
	        		url += "&objectIds="+(String)(ProgramCentralUtil.parseTableRowId(context,objectIdSelected)).get("objectId");
	        	}
