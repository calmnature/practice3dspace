...
<%
	 ...
	 if("Checklist".equalsIgnoreCase(strMode)){ 
		 ...
		 String gateState = MqlUtil.mqlCommand(context,
				false,
				false,
				mqlCmd,
				false,
				objectId,
				"to[Checklist].from.current");
		
		
		 contentURL = "../common/emxIndentedTable.jsp?table=PMCChecklistSummaryTable&sortColumnName=Name,Type&Export=true&expandLevelFilter=true&header=emxProgramCentral.Common.Checklist.ChecklistSummary&suiteKey=ProgramCentral&HelpMarker=emxhelpgatechecklist&expandProgram=emxChecklist:expandChecklistSummaryTableObjects&objectId="+objectId;
		//If the parent gate is not completed then checklist is editable
		 if (!"Complete".equalsIgnoreCase(gateState)) {
			 contentURL += "&editLink=true";
			}
      ...
      }else if("ChecklistItem".equalsIgnoreCase(strMode)){ 
		 ...
		 String gateState = MqlUtil.mqlCommand(context,
				...
				false,
				objectId,
				"to[Checklist Item].from.to[Checklist].from.current");
		...
		//If the parent gate is not completed then checklist is editable
	 ...
	 }else if("ProjectTemplateTableViewNameColumn".equalsIgnoreCase(strMode)){
    	 ...
    	 try{
			 ...
			 if (isGate) { 	
						contentURL = "../common/emxNavigator.jsp?isPopup=false&headerLifecycle=hide&objectId="+objectId;
			 }else{
				 contentURL = "../common/emxNavigator.jsp?isPopup=false&treeMenu=type_TaskTemplate&headerLifecycle=hide&objectId="+objectId;
    	 	}
     ...
     }else if("PMCDeliverableURL".equalsIgnoreCase(strMode)) {
...
<%
		...
		CacheUtil.removeCacheObject(context, context.getUser()+"_RestrictAssigneeMap");
		...
		boolean isProjectExperimentTask = ProgramCentralConstants.TYPE_EXPERIMENT.equalsIgnoreCase(projectType);
		...
		if (isHoldOrCancelProject || isProjectBaselineTask) {
			...
			if(isProjectBaselineTask){
				contentURL +=	"&expandProgramMenu=PMCWBSExperimentTaskListMenu";
			}else{
				contentURL += "&expandProgramMenu=PMCWBSTaskListMenu";
			}
			...
			contentURL = "../common/emxIndentedTable.jsp?table=PMCWBSViewTable";
			if(isProjectExperimentTask){
				contentURL +=	"&expandProgramMenu=PMCWBSExperimentTaskListMenu";
			}else{
				contentURL += "&expandProgramMenu=PMCWBSTaskListMenu";
			}
			contentURL +=	 "&freezePane=Name" + "&selection=multiple"
...
<%
        ...
        selectables.add(ProgramCentralConstants.SELECT_POLICY);
        selectables.add(ProgramCentralConstants.SELECT_NAME);
        
		DomainObject dmoObject = DomainObject.newInstance(context, objectId);
		
		Map infoMap = dmoObject.getInfo(context, selectables);
		
		String projectPolicy = (String)infoMap.get(ProgramCentralConstants.SELECT_POLICY);
		String projectName = (String)infoMap.get(ProgramCentralConstants.SELECT_NAME);
		...
		if ("PMCWBSDataGridView".equalsIgnoreCase(subMode)) {
			...
			//IR-1266894
			if(ProgramCentralUtil.isNullString(treeLabel)){
				urlBuilder.append("&treeLabel="+XSSUtil.encodeForURL(context, projectName));
			} 
		...
		urlBuilder.append("&showPageHeader=false"); // IR-1467943
		if(!"PMCWBSDataGridView".equalsIgnoreCase(subMode)){
       ...
       <%
		...
		String objectId = emxGetParameter(request, "objectId");
 		Map paramMap = new HashMap(1);
   		...
   		//IR-1266894
   	 	DomainObject dmoObject = DomainObject.newInstance(context, objectId);
		String projectTemplateName = dmoObject.getInfo(context, ProgramCentralConstants.SELECT_NAME);
       ...
       String[] methodArgs = JPO.packArgs(paramMap);
		...
		if("PMCProjectTemplateWBSDataGridView".equalsIgnoreCase(subMode)) {
			...
			//IR-1266894
			if(ProgramCentralUtil.isNullString(treeLabel)){
				urlBuilder.append("&treeLabel="+XSSUtil.encodeForURL(context, projectTemplateName));
			}
        } else {
		...
		StatusReport report = (StatusReport)session.getAttribute(objectId+"_store");
		Method method = null;
		try{
...
<%
    	...
    	String[] governItemIdArray = new String[sTableRowId.length];
			...
			governItemIdArray[i] = toBeGovernedId;
		}
		StringList governItemIdToConnectList = ProgramCentralUtil.getGovernedItemIdListToConnect(context, governItemIdArray);
    	
		for(int i=0; i<governItemIdToConnectList.size(); i++){
			String toBeGovernedId = (String)governItemIdToConnectList.get(i);
				try {
...
<%
		...
		String mqlCmd = "print bus $1 select $2 $3 $4 $5 $6 $7 dump $8";
		...
		String iskindofExperiment = (String)objectInfoList.get(3);
		String hasIssue = (String)objectInfoList.get(4);
		String projectState = (String)objectInfoList.get(5);
		...
		if (!ProgramCentralConstants.POLICY_PROJECT_SPACE_HOLD_CANCEL.equalsIgnoreCase(projectPolicy) 
		&& "true".equalsIgnoreCase(hasModifyAccess)
		&& !"Released".equalsIgnoreCase(projectState) 
		&& !"Obsolete".equalsIgnoreCase(projectState)
        && !("true".equalsIgnoreCase(iskindofExperiment) && "true".equalsIgnoreCase(hasIssue)))	{
...
//  final String  RELATIONSHIP_RESOLUTION_PROJECT   = "Resolution Project";  //from
	 ...
	 try{
			...
			if(RiskOpArrayString.length > 0){
				DomainRelationship.connect(context,projectObject, RELATIONSHIP_RESOLUTION_PROJECT,false, RiskOpArrayString);
			}
			// IR-1460370-3DEXPERIENCER2026x
						if(IssueArrayString.length > 0){
							String errorIssueId = "";
							RelationshipType relationshipResolvedTo = new RelationshipType(RELATIONSHIP_RESOLVED_TO);
							for(int i = 0 ; i < IssueArrayString.length ; i++){
								DomainObject IssueObject = new DomainObject(IssueArrayString[i]);
								try{
									projectObject.connect(context, relationshipResolvedTo,false, IssueObject);
								}catch(Exception exp){
									//TODO - Fix the Issue Message
						             errorIssueId +=  "\n" + IssueObject.getName(context) ;
								}
							}
							if(errorIssueId.length() > 1){
								 %>
						         	<script language="javascript" type="text/javaScript">
						         	
						            	//alert("<emxUtil:i18nScript localize="i18nId">emxProgramCentral.Error.NoFromConnectAccess</emxUtil:i18nScript>" + " " +errorIssueId+" ");
						            	alert("<emxUtil:i18nScript localize="i18nId">emxProgramCentral.Error.NoFromConnectAccess</emxUtil:i18nScript>" + "<%=XSSUtil.encodeForJavaScript(context,errorIssueId)%>");
									</script>
								<%
							}
						}		
...
<%
		...
		String portalCmd 		= emxGetParameter(request, "portalCmdName");
		 String physicalProductVersionId = emxGetParameter(request,"physicalProductVersionId"); 
		  String typePatternPrj = emxGetParameter(request,"strType");
		...
		String strURL = "../programcentral/emxProjectManagementUtil.jsp?&mode=connectLatestRevProcess&objectId="+docObjectId+"&parentOID="+parentObjectId+
				"&relType="+relationType+"&portalCmdName="+portalCmd+"&relId="+relationshipId +"&physicalProductVersionId="+physicalProductVersionId +"&strType="+typePatternPrj;
    	...
    	if(ProgramCentralUtil.isNotNullString(physicalProductVersionId)) {
            ...
            String whereClause = "attribute[PLMReference.V_VersionID] == '"+physicalProductVersionId+"' && program[emxProgramCentralUtil -method isLastVersion ${OBJECTID}] == TRUE";
...
//				}
			 ...
			 try{
					...
					//System.out.println("toObjIdArr" + toObjIdArr);
					...
					if(toObjIdArr.length > 0){
						...
						//System.out.println("connectedObjMap " + connectedObjMap);
						
						   if(connectedObjMap != null && !connectedObjMap.isEmpty()){
                // Fetch task names for all connected tasks in one go (optimization)
						StringList slBusSelect = new StringList(DomainConstants.SELECT_NAME);
						BusinessObjectWithSelectList objectWithSelectList = ProgramCentralUtil.getObjectWithSelectList(context, toObjIdArr, slBusSelect, true);
						StringBuilder addedTasksNames = new StringBuilder();
                for (int i = 0, size=objectWithSelectList.size(); i < size; i++){
                    BusinessObjectWithSelect objectWithSelectPassiveTask = objectWithSelectList.get(i);
                    String taskName = objectWithSelectPassiveTask.getSelectData(DomainConstants.SELECT_NAME); // Fetch task name
                    addedTasksNames.append(taskName).append("\n");  // Add newline after each task for display
					
                }

                // Display the success message with all added task names
                if(addedTasksNames.length() > 0){
                    String successMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.PassiveSubtask.alertString", context.getSession().getLanguage());
                    successMessage += "\n" + addedTasksNames.toString();
                    %>
                    <script language="javascript">
                    ...
                    <%
						...
						//connectedObjMap will be a Map of passiveTaskIds (Keys) and their connectionIds(Values)
						//START - FUN146902
						HashMap programMap = new HashMap();
                        programMap.put("expressionName", "DPM_DefaultThresholdForFlexibleLinks");
                        String[] arrJPOArguments = JPO.packArgs(programMap);
						String warningLevelFromExpression = (String) JPO.invoke(context, "emxTask", null, "getExpressionValue", arrJPOArguments, String.class);
						//String warningLevelFromExpression = com.dassault_systemes.enovia.tskv2.Task.getExpressionValue(context,"DPM_DefaultThresholdForFlexibleLinks");
						
						int defaultWarningLevelFromExpression = 0;
				    	if(warningLevelFromExpression != null && !warningLevelFromExpression.isEmpty()) {
				    		defaultWarningLevelFromExpression = Integer.parseInt(warningLevelFromExpression);
				    	}
						if(defaultWarningLevelFromExpression != 0){
							for (String relId : connectedObjMap.values()){
								DomainRelationship.setAttributeValue(context,relId,"PassiveSubtaskWarningLevel",warningLevelFromExpression);
							}
						}
						//END - FUN146902
					}  else {
							// Show an error message if no tasks were connected
							String errorMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral", "emxProgramCentral.PassiveSubtask.NoTasksConnected", 			context.getSession().getLanguage());
                             %>
                         <script language="javascript">
                         alert("<%= errorMessage %>");
                         </script>
...
<%
	...
	else if("checkoutDocument".equalsIgnoreCase(strMode))
	{
		...
		objectId = XSSUtil.encodeURLForServer(context, objectId);
  		...
  		%>
  		  ...
  		  <%
  	   } else if("addUserGroups".equalsIgnoreCase(strMode)){
			String objectId = emxGetParameter(request, "objectId");
			String[] ids = emxGetParameterValues(request, "emxTableRowId");
			String defaultAccess = "";
			String OID = "";
			...
			<%
  	   }else if("addMemberFromUserGroup".equalsIgnoreCase(strMode)) {
    String objectId = emxGetParameter(request,"objectId");
    ...
    if(strRowId == null) {
        strRowId = emxGetParameterValues(request,"emxTableRowIdActual");
    }
    ...
    MapList mapList = DomainObject.getInfo(context,strRowId,busSelects);
    Map memberMap = (Map)mapList.get(0);

    String name = (String)memberMap.get(person.SELECT_NAME);
    String lastName = (String)memberMap.get(person.SELECT_LAST_NAME);
    String firstName = (String)memberMap.get(person.SELECT_FIRST_NAME);
    String personId = (String)memberMap.get(person.SELECT_ID);
    String latestOwner = lastName + " " + firstName;
    paramMap.put("objectId",objectId);
    paramMap.put("New Value", personId);

    Map argsMap = new HashMap();
    argsMap.put("paramMap", paramMap);
    JPO.invoke(context, "emxTaskBase", null, "updateOwner",JPO.packArgs(argsMap));
    String chooserType = emxGetParameter(request,"chooserType");
%>
<script language="javascript">
    var formWin = getTopWindow().getWindowOpener();
    if (formWin && formWin.document) {
        if (formWin.document.getElementById("projectMembers")) {
            formWin.document.getElementById("projectMembers").value = "<%= latestOwner %>";
        }
        if (formWin.document.getElementById("hiddenProjectMembers")) {
            formWin.document.getElementById("hiddenProjectMembers").value = "<%= personId %>";
        }
    }

    <% if ("group".equalsIgnoreCase(chooserType)) { %>
        getTopWindow().close();
    <% } %>
</script>
<%
