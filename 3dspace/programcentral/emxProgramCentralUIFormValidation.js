...
function previewTemplateTasksWBS() {
	var searchProjectTemplateId = document.getElementsByName("SeachProjectOID")[0].value;
	searchProjectTemplateId = searchProjectTemplateId.replaceAll("|",",");
...
function validateDurationFormField() {
	...
	//For Gate or Milestone 
	var fieldDisabled =  document.getElementById("Duration").disabled;
    ...
    var weeksUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Weeks";
    var weeks = emxUICore.getData(weeksUrl);
    var monthsUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Months";
    var months = emxUICore.getData(monthsUrl);
    
    var dayUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Day";
    var day = emxUICore.getData(dayUrl);
    var hourUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Hour";
    var hour = emxUICore.getData(hourUrl);
    var weekUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Week";
    var week = emxUICore.getData(weekUrl);
    var monthUrl = "../programcentral/emxProgramCentralUtil.jsp?mode=errorMessage&errorKey=emxProgramCentral.DurationUnits.Month";
    var month = emxUICore.getData(monthUrl);
    ...
    if (durationVal.indexOf(days) != -1 || durationVal.indexOf(day) != -1) {
        durationVal = durationVal.replace(days, "d");
        durationVal = durationVal.replace(day, "d");
    } else if (durationVal.indexOf(hours) != -1 || durationVal.indexOf(hour) != -1) {
        durationVal = durationVal.replace(hours, "h");
        durationVal = durationVal.replace(hour, "h");
    } else if (durationVal.indexOf(weeks) != -1 || durationVal.indexOf(week) != -1) {
        durationVal = durationVal.replace(weeks, "w");
        durationVal = durationVal.replace(week, "w");
    } else if (durationVal.indexOf(months) != -1 || durationVal.indexOf(month) != -1) {
        durationVal = durationVal.replace(months, "mon");
        durationVal = durationVal.replace(month, "mon");
    }

    /* The below condition will check if the entered duration contains Days / Hours. If not Its not a valid duration*/
    if (!durationVal.endsWith(' h') && !durationVal.endsWith(' d') && !durationVal.endsWith(' w') && !durationVal.endsWith(' mon') && isNaN(durationVal)) {
        alert(errmsg);
        document.getElementById("Duration").value = 1
        return false;
    }
        ...
        if (durationVal.lastIndexOf(' d') != -1) {
            ...
            durationUnit = 'h';
        } else if (durationVal.lastIndexOf(' w') != -1) {
            durationVal = durationVal.substring(0, durationVal.lastIndexOf(' w'));
            durationUnit = 'w';
        }else if (durationVal.lastIndexOf(' mon') != -1) {
            durationVal = durationVal.substring(0, durationVal.lastIndexOf(' mon'));
            durationUnit = 'mon';
        }
    ...
    if (durationVal != null) {
       ...
       if((durationUnit==='h' && durationVal >=80000) || ((durationUnit==='d' || durationUnit==='') && durationVal >=10000) || (durationUnit==='w' && durationVal >=2000) || (durationUnit==='mon' && durationVal >=500)) {
...
function getSubQuestionsInTemplate()
	{
				...
				if (topFrame === null || topFrame === "") {
				topFrame = findFrame(getTopWindow(), "_top");
			}
