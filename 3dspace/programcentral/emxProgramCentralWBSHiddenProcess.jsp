      ...
      if(projectSelected == "true"){
    	...
    	//START: IR-1388608 
 		 ...
 		 if(<%=isFromRMB%>){
 			 ...
 			 //if confirmation canceled then return.
 			 if(!result) {
 				 return;
 			 }
 		 }
              ...
              else 
              {
                  <%
                    if(!slFinalIdsList.isEmpty())
                    {
                    	...
                    	strFinalId=(String)slFinalIdsList.get(0);
                       %>
                             url="../common/emxForm.jsp?form=PMCTaskMarkAsDeletedComments&mode=edit&suiteKey=<%=suiteKey%>&formHeader=emxProgramCentral.Common.MarkDeleteSelected&HelpMarker=emxhelpmarkasdeleted&selectedObjectIds=<%=strRowId%>&objectId="+projId;
							  
							  var topFrame = findFrame(getTopWindow(), "PMCWBSDataGridView"); 
							  if(topFrame && topFrame.dataGridEnabled){
								url=url+"&submitAction=doNothing";  
							  }
							  url=url+"&postProcessURL=../programcentral/emxProgramCentralMandatoryDiscussionProcess.jsp";
                              getTopWindow().showSlideInDialog(url,true);
                       <%
                    }
                 %>
     ...
     <%
       ...
       if(!slParentCompletedList.isEmpty())
       {
           ...
           if(!slFinalIdList.isEmpty())
           {
               ...
               %>       
               var taskObjId = selectedIds;
               url = "emxProgramCentralMandatoryDiscussionProcess.jsp?fromWBS=true&objectId="+"<%=XSSUtil.encodeForURL(context,strFinalIds)%>"+"&mode=UndoMarkDeleted&objectIds=<%=XSSUtil.encodeForURL(context,strTaskIds)%>&rowIds=<%=XSSUtil.encodeForURL(context,selectedRowIds)%>&emxTableRowId=<%=XSSUtil.encodeForURL(context,emxTableRowId)%>";
