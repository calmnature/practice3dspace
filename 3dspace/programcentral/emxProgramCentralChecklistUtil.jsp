	...
	portalCmdName = XSSUtil.encodeURLForServer(context, portalCmdName);
        ...
        if(null != mode &&mode.equalsIgnoreCase("userRole")){
            try{
    			...
    			selectables.add(DomainConstants.SELECT_HAS_MODIFY_ACCESS);
    			
                DomainObject domObj = DomainObject.newInstance(context,objectId);
                Map gateInfo =domObj.getInfo(context, selectables);
                String gateState = (String)gateInfo.get(DomainConstants.SELECT_CURRENT);
            	boolean modifyAccess =   "true".equalsIgnoreCase((String)gateInfo.get(DomainConstants.SELECT_HAS_MODIFY_ACCESS));
                ...
                if((ProgramCentralUtil.isNotNullString(strReturnValue) && strReturnValue.equalsIgnoreCase("true")) && (!gateState.equalsIgnoreCase(STATE_COMPLETE))){
                	targetSearchPage += "&toolbar=PMCChecklistTopActionToolbar";
                		if(modifyAccess){
                			targetSearchPage += "&editLink=true";
                		}
                }
%>
                <script type="text/javascript" language="JavaScript">
