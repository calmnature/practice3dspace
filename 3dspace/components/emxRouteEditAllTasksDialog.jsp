...
relSelStmts.addElement("physicalid");
  ...
  selectStmts.addElement("attribute["+DomainObject.ATTRIBUTE_APPROVAL_STATUS+"]");
...
//Start: Resume Process modifications
    ...
    for (Iterator itrInboxTasks = InboxTaskList.iterator(); itrInboxTasks.hasNext();) {
        ...
        } else if("Auto Complete".equals(mapTempTaskInfo.get("attribute["+DomainObject.ATTRIBUTE_APPROVAL_STATUS+"]"))){
        	mlFilteredTasks.add(mapTempTaskInfo);
        }
    }
...
//function to open the RouteTaskAssignSelected dialog window.
 ...
 // javascript function shows calendar popup only if calendar radio is chosen. Else alert user.
  ...
  function showDeleteSelected() {
    ...
    for (var i=0; i < document.TaskSummary.elements.chkItem1.length; i++) {
        if (document.TaskSummary.elements.chkItem1[i].checked  )  {
           checkedFlag = "true";
           break;
        }
    }
...
//       String isKindOfGroupProxy = (String)taskMap.get("from[Route Node].to.type.kindof");
      ...
      String sRelId2 = (String)taskMap.get("physicalid");
	  //Added for the Bug No:350789 starts
...
%>
      ...
      %>
       ...
       boolean maintainExactTaskList = "Maintain Exact Task List".equals(strTemplateTaskSetting);
       boolean extendTaskList = "Extend Task List".equals(strTemplateTaskSetting);
    	   ...
    	   %>
    		...
    		<td style="padding-top:3px;">
				<table>
		           <tr>
			           <td>
							<textarea style="min-height:50px;width:250px;" rows="6" name="routeInstructions" readonly><xss:encodeForHTML><%=routeInstructionsValueStr%></xss:encodeForHTML></textarea>				       
					   </td>
			        </tr>
		          </table>
		       </td>
...
<tr>
           ...
           <input type="hidden" name="chooseUsersRouteFromUG<%= i1 %>" value=<%=SelectUsersUGCount %> />
        	
        <% } else {
    		SelectUsersUGCount=0;
                %>
                <td> 
