...
<%
  ...
  else if(routeNodeId != null && !routeNodeId.equals(""))
  {
    ...
    busSelects.addElement("from["+DomainObject.RELATIONSHIP_INITIATING_ROUTE_TEMPLATE+"].to.attribute["+DomainObject.ATTRIBUTE_TASKEDIT_SETTING+"]");
    ...
    taskEditSetting      = (String)routeMap.get("from["+DomainObject.RELATIONSHIP_INITIATING_ROUTE_TEMPLATE+"].to.attribute["+DomainObject.ATTRIBUTE_TASKEDIT_SETTING+"]");
    ...
    tempTask            = (String)taskMap.get(domainObject.ATTRIBUTE_TEMPLATE_TASK);
...
//till here
  ...
  int iTimeZone         = (new Double((new Double((String)session.getAttribute("timeZone"))).doubleValue() * -3600000)).intValue();
  double iTimeZoneDbl= (new Double((String)session.getAttribute("timeZone"))).doubleValue();
  //End modification
    ...
    <tr>
      ...
      <%
             boolean isTemplateTask = tempTask != null && tempTask.equals("Yes");
      		 boolean maintainExactTaskList = "Maintain Exact Task List".equals(taskEditSetting);
             boolean extendTaskList = "Extend Task List".equals(taskEditSetting);
             if(maintainExactTaskList || (extendTaskList && isTemplateTask)){
      %>
        <textarea name="instructions" cols="40" rows="5" wrap readonly><xss:encodeForHTML><%=taskInstructions%></xss:encodeForHTML></textarea>
      <%
	    	 }else{
       %>
