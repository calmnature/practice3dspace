    ...
    function loadDependencyContent()
     {
         ...
         {   
             require(['DS/PlatformAPI/PlatformAPI',
                     ...
                     ChgInfraService,ChgServiceGlobalVariable, CADependencyModel, ChgDependencyServices, ChgDataService){
                          ...
                          ChgInfraService.populateSecurityContext()
                            .then(function (securityContextDetails){
								...
								function(response){
									...
									//ChgInfraService.fetchCASubTypes();
									ChgInfraService.fetchChangeSubTypes(["Change Action", "Change Order", "Change Request", "Change Analysis"]);
					
									var that = this;

									var physicalID = '<%=strContextID%>';
                  var objID = physicalID.replace(/^pid:/, '');

                  // if (result && Object.prototype.hasOwnProperty.call(result, 'pid')) {
                  //         delete result.pid;          // result now has everything except pid
                  //       }
             
                  // ChgDataService.getChangeGroupData(physicalID)
                  //     .then(function (result) {
                  //       console.log('Change‑group data:', result);
                        
                  //       let changemodeAttrs = result.changeaction.attributes.filter(item => item.name === "attribute_ChangeAction.ChangeMode")
                  //       	if(changemodeAttrs.length > 0)
		              //         that.model.caChangeMode = changemodeAttrs[0].value
                  ...
                  ChgDataService.getChangeGroupData(objID)
                      .then(function (result) {
                        console.log('Change‑group data:', result);
                        
                        let changemodeAttrs = result.changeaction.attributes.filter(item => item.name === "attribute_ChangeAction.ChangeMode")
                        	if(changemodeAttrs.length > 0)
		                      that.CADependencyModelObj.caChangeMode = changemodeAttrs[0].value
                        CASpinner.endWait(widget.body);
                      });
                      ...
                      params.caChangeMode=that.CADependencyModelObj.caChangeMode;
