...
<%
	...
	currentCriteria  		= !changeUXUtil.isNullOrEmpty(currentCriteria) ? currentCriteria : ""; 
	...
	StringList excludeTypeList = !changeUXUtil.isNullOrEmpty(excludeTypes) ? FrameworkUtil.splitString(excludeTypes,",") : new StringList();
	...
	excludeOIDprogram		   = !changeUXUtil.isNullOrEmpty(excludeOIDprogram) ? excludeOIDprogram : ""; 
	String targetRelActualName = !changeUXUtil.isNullOrEmpty(targetRelName) ? PropertyUtil.getSchemaProperty(context,targetRelName) : ""; 
	...
	strProposedActivityList=changeUXUtil.getListOfAllowedTypesForGivenCategory(context, ChangeConstants.PROPOSEDACTIVITY);
	strProposedActivityList =strProposedActivityList.replace(", ", ",");

	if(!changeUXUtil.isNullOrEmpty(objectId)) {
	...
	try{
	    ...
	    if("AddToNewChange".equalsIgnoreCase(functionality)||
	    	...
	    	Map mapControlledItems = changeUXUtil.checkForChangeControlledItems(context, affeObjList);
		...
		{						
			...
			searchTypes  =  changeUXUtil.getRelationshipTypes(context,targetRelName,isSerachForToSide,false,excludeTypeList);
		...
		{	
			...
			StringList caIdsList = changeUXUtil.getStringListFromMapList(objectList,ChangeConstants.SELECT_FROM_ID); 	
		...
		{							
			...
			searchTypes  = "type_ChangeAction:CURRENT=policy_ChangeAction.state_Prepare,policy_ChangeAction.state_InWork:ECM_CHG_ON_HOLD!=TRUE:ECM_CA_CHANGE_MODE=Standard";
		...
		else if("MassRelease".equalsIgnoreCase(functionality)) 
		{
            ...
            Map ObjMap = changeUXUtil.getObjectIdsRelIdsMapFromTableRowID((String[])emxTableRowIds);
			StringList AffeObjList = (StringList)ObjMap.get("ObjId");
			changeUXUtil.checkForMassReleaseOrObsolete(context,changeUXUtil.getTypeNamePolicyForObjList(context,AffeObjList),ChangeConstants.FOR_RELEASE);
			session.setAttribute("sourceAffectedItemRowIds",AffeObjList);
		...
		else if("MassObsolete".equalsIgnoreCase(functionality)) 
		{
            ...
            Map ObjMap = changeUXUtil.getObjectIdsRelIdsMapFromTableRowID((String[])emxTableRowIds);
			StringList AffeObjList = (StringList)ObjMap.get("ObjId");
			changeUXUtil.checkForMassReleaseOrObsolete(context,changeUXUtil.getTypeNamePolicyForObjList(context,AffeObjList),ChangeConstants.FOR_OBSOLETE);
			MapList ObjInfoList = changeUXUtil.getTypeNamePolicyForObjList(context,AffeObjList);
			session.setAttribute("sourceAffectedItemRowIds",AffeObjList);
		...
		{
			session.setAttribute("functionality",functionality);
			searchTypes=changeUXUtil.getListOfAllowedTypesForGivenCategory(context, ChangeConstants.REFERENTIAL);
		...
		else if ("CreateNewCOUnderCO".equalsIgnoreCase(functionality)) 
		{
			 ...
			 if(emxTableRowIds != null && emxTableRowIds.length > 1){
				 ...
				 if(!changeUXUtil.isNullOrEmpty(objectId))
				 {
					 ...
					 String strUIContext = changeUXUtil.isNullOrEmpty(strParentOID)? "MyDesk" : "ChangeOrder" ;
...
errorMessage = !changeUXUtil.isNullOrEmpty(errorMessageProjectCheck)? errorMessageProjectCheck : errorMessage;
%>
<script language="Javascript">
//XSSOK
