...
<%	
	...
	if("refreshHeader".equals(sAction)){
		...
		paramsJPO.put("MCSURL", XSSUtil.encodeForURL(context, sMCSURL));
	...
	<script type="text/javascript">
		...
		try{
			...
			if("promote".equals(sAction)) {	
				...
				com.matrixone.apps.framework.lifecycle.LifeCycleUtil.checkAndCreateRoute(context, sOID);
				if(!isCloud){
				...
				com.matrixone.apps.framework.lifecycle.LifeCycleUtil.checkActiveRoutes(context, bo);
				if(!isCloud){
...
<%		
		...
		String errorMsgNLSForAppliesTo = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource", new Locale(sLanguage), "emxComponents.RouteMaturityController.RouteCreatedOrRestarted.NoticeMsg");
		...
		<%			
			...
			while (itrNotices.next()) {
				...
				if((sbMessages.toString()).isEmpty()){
					...
					}else if(UIUtil.isNotNullAndNotEmpty(mxErrMsg.getMessage())){
						String sErrorMsg = mxErrMsg.getMessage();
						sbMessages.append(sErrorMsg);
	                }
				}
...
%>
		...
		var errorMsgNLSForAppliesTo = "<%=errorMsgNLSForAppliesTo%>";
		if(errorMsg.includes(errorMsgNLSForAppliesTo)){
			var detailsDisplayFrame = emxUICore.findFrame(getTopWindow(), "detailsDisplay");
