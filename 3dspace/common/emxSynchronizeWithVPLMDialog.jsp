...
<%
    ...
    boolean isEBOMPSSimulEnabled = false;
    VPLMBusObject boToSync  = new VPLMBusObject(context,objectId);

		String objectType=boToSync.getBasicTypeName();
	    boolean isPartConfigured = MDCollabSynchronizerObject.isConfigured(context, boToSync.getIdPhysical());
		...
		// yes => we show the values
			String sAllowDepthChange = emxGetParameter(request,"enableDepthChange");
		...
		if (sEnableTransfer == null || sEnableTransfer.length() == 0 || bEnableTransfer) {
			...
			isEBOMPSSimulEnabled = MDCollabIntegrationUtilities.simulCollabActivated(context);
			String checkedSimulation="";
			boolean bEnableSimulation=false;
			if(isEBOMPSSimulEnabled){
				String sEnableSimulation = emxGetParameter(request, "enableSimulation");
				
			  	
			  	if (sEnableSimulation != null)
			  		bEnableSimulation = Boolean.parseBoolean(sEnableSimulation);
			  	if(bEnableSimulation==true){
			  		checkedSimulation="checked";
			  	}
...
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
        ...
        function getSimul()
        {
        	if(document.emxSyncDialog.MAPKEY_SIMUL.checked){
        ...
        function getdetailedReportOption()
        {
			if(!document.emxSyncDialog.DETAILED_REPORT.checked && !document.emxSyncDialog.MAPKEY_SIMUL.checked )
				return "false";
			else
				return "true";
            ...
            function updateGDetailedReportCheckbox(){
            	const detailedReport = document.getElementById("detailedReport");
            	const mapkeySimul = document.getElementById("mapkeySimul");
            	
            	mapkeySimul.addEventListener("change", ()=> {
            		if(mapkeySimul.checked){
            			detailedReport.checked = true;
            			detailedReport.disabled = true;
            		}else{
            			detailedReport.disabled = false;
            			detailedReport.checked = false;
            		}
            	});
            }
            ...
            function checkInput()
            {
         	...
         	var syncDepth = "-1";
          	...
          	//ANT4 - 3/28/2018 : IR-563472-3DEXPERIENCER2017x Fix issue with '&' in vplmContext 
          	var vplmContext = document.emxSyncDialog.vplmContext.value;
          	vplmContext = vplmContext.replace(/&/g, '%26');
           	//LUS - Begin 6/25/2008 for Bug: A0627779 
			var href = "<%=sreturnFramesetEncoded %>&vplmContext=" + vplmContext + "&SYNC_DEPTH=" + syncDepth+ "&SYNC_DEPTH_CUSTOM=" + getSyncDepthCustom();		
			if(<%=bEnableTransfer%>) 
			{				
			...
			href += "&DETAILED_REPORT=" + getdetailedReportOption();
...
<TABLE BORDER="0" CELLPADDING="5" CELLSPACING="2" WIDTH="100%">
	...
	<% } %>
	<TR>
		<TD CLASS="label"><emxUtil:i18n localize="i18nId">emxVPLMSynchro.DetailedReport.Title</emxUtil:i18n>
		</TD>
		<TD CLASS="inputField"><INPUT TYPE="CHECKBOX"
					NAME="DETAILED_REPORT" id="detailedReport" <%=checkedTransfer%> ></TD>
	</TR>
	<%
	...
	%>		
	<TR>
		<!--HLN4: simulation option chekbox  -->
		<TD CLASS="label"><emxUtil:i18n localize="i18nId">emxVPLMSynchro.Synchronization.Simulation</emxUtil:i18n>
		</TD>
		
		<TD CLASS="inputField"><INPUT TYPE="CHECKBOX"
					NAME="MAPKEY_SIMUL"  id="mapkeySimul" <%=checkedSimulation%>></TD>
	</TR>
...
<SCRIPT TYPE="text/javascript"> enableCustom();
	<!-- update generate detailed report option when simulate option checked -->
	updateGDetailedReportCheckbox();
