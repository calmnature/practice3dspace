        ...
        if (isIE) {
                while(objParent != null) {
						...
						if(objParent != null && objParent.tagName == "TABLE")
						{
							...
							{
								intTop -= ($(objParent).find('input[name="searchType"]').eq(0).parent().height() || 0);
							}
						}
                }
        } else {
...
function adjustAppName(appName){
	if(appName){
		...
		var totwidth = (clogoelem.innerWidth() || 0) + (getTopWindow().jQuery('li[id="AEFBrandLogoHolder"]').innerWidth() || 0) + (getTopWindow().jQuery('li[id="compassHoldout"]').innerWidth() || 0);
...
function launchAppMenu(){
	...
	if(menuDiv.is(":visible")){
		...
		menuDiv.css('top', ((jQuery('#pageHeadDiv').height() || 0) - (jQuery('div#tabBar').height() || 0)) + 'px');

		emxUICore.iterateFrames(function (objFrame) {
  	    	if(objFrame){
  	    		jQuery(objFrame).on("mousedown",function(e){
  	    			var target= e.target;
  	    			if(target && menuDiv.has(target).length == 0 && target.className != 'btn app-menu'){
  	    				menuDiv.hide();
  	    				jQuery(objFrame).off("mousedown");
  	    			}
  	    		});
  	    	}
  	    });
...
async function sha256Hex(text) {
  ...
  // Compute the digest (returns an ArrayBuffer)
...
async function getUserHashid(userId) {
  
  const userIdSha256 = await sha256Hex(userId);
  console.log('SHA‑256 of userId:', userIdSha256);
  return userIdSha256;
...
// Build json with the tenant, appid, user with hash code
async function prepareEventObject(appName,licensedAppList) {	
	const now = new Date().getTime();
	var envId = getTopWindow().curTenant;
	if(envId == '' ||  typeof(envId) == "undefined") {
		envId = "OnPremise";
	}
	//var useridSha256 = createSHA256Hash(getTopWindow().curUserId);
	const useridSha256 = await getUserHashid(getTopWindow().curUserId);
	//let listApps = [licensedAppList]

	let defaulfPayloadUpload = {
		 "r_schemaName" : "userroleusage",
		 "r_schemaVersion" : "1.0",
		 "r_service" : "3dspace",
		 "r_timestamp" : now,
		 "user__hash" : useridSha256,
		 "app__id" : licensedAppList,
		"tenant__id" : envId,
		"event__type" : "userroleconsumption"
		//"user_roles" :{listApps} //a json array of licenses currently granted to the user 
	}
	return defaulfPayloadUpload;
	
	
}

// Request to call upload api with the passed event
async function invokeEventUpload(evts) {
      let i = 1;
      const form = new FormData();
	  form.append('usage1', JSON.stringify(evts));
	  UWA.Data.request("/upload", {
                method: 'POST',
...
// updated to async and calling the upload for spefic cases of clusters
async function changeProduct(brandName, appName, appTrigram, appMenuList, licensedAppList, isFromIFWE){
	if(getTopWindow().curProd == "" || licensedAppList.indexOf(getTopWindow().curProd) < 0  && !getTopWindow().isfromIFWE){
		...
		try {			
			//example window.location.host = https://dsqal00112-euw1-8s3021923-ifwe.3dx-staging.3ds.com/
			const hostPattern = /(?:\b(?:int[abcde]|cst[abcde])\b|-8s\w+|devprol\w*|dsw\w*|renaultlci)/i;				
			if (
			 getTopWindow().curTenant != null &&                 // not null **and** not undefined
			  typeof getTopWindow().curTenant != 'null' &&
			  getTopWindow().curTenant.toLowerCase() != 'onpremise' && // ensure we can call toLowerCase() 
			  typeof window !== 'undefined' &&                     // window exists
			  window.location &&                                  // location object exists
			  typeof window.location.host != 'null' &&         // host is a string			 			  
			  hostPattern.test(window.location.host)         // pattern check    			 
			) {
				const evts = await prepareEventObject(appName, licensedAppList);
				invokeEventUpload(evts);
			}
		} catch(err) {
			console.log('error due to upload failed', err);
		}
...
function showMyDesk()
{
    ...
    if(jQuery("#mydeskpanel").hasClass("appMenu")) {
        ...
        jQuery("#mydeskpanel").off("mouseenter");
        jQuery("#mydeskpanel").off("mouseleave");                                                                           
	...
	jQuery("div#pageContentDiv").css('left', (jQuery("div#mydeskpanel").width() || 0) + (jQuery("div#panelToggle").width() || 0) + 'px');	
	...
	if(getTopWindow().isfromIFWE){
		...
		if(emxUIConstants.TOPFRAME_ENABLED){
			getTopWindow().toppos = jQuery('#topbar').height() || 0 + jQuery('#navBar').height() || 0;
		}else{
			getTopWindow().toppos = jQuery('#pageHeadDiv').height() || 0;
		}
	}
