...
var emxUIShortcut = {
    ...
    template: { 
        ...
        facet_head: function (facet_Info) {
        	...
        	if(facet_Info.showClearButton == true && facet_Info.items.length > 0){
	        	...
	        	jQuery("button",removeAll).on("mouseenter",function(){
	        		jQuery(this).attr("title", getTopWindow().emxUIConstants.SHORTCUT_REMOVEALL_BUTTON);
	        		jQuery(this).toggleClass("active");
	        	});
	        	jQuery("button",removeAll).on('click',function(e){
	            	...
	            	var url="emxShortcutGetData.jsp?action=removeObject&objectId="+encodeURIComponent(objectids);
	            	jQuery.ajax({  
	        		    url : url, 
	        		    cache: false
	        		}) 
        	...
        	jQuery("button",title).on('click',emxUIShortcut._toggleContent);
        	return head;
        },
    	...
    	facet_attr_head_title: function (itemObj) {
			...
			var link = jQuery('<label class="link" url="'+itemObj.url+'">'+getXSSEncodedValue({"value":itemObj.label, "encodeType":"html"})+'</label>');
        ...
        facet_attr_head_title_removeButton: function () {
        	...
        	jQuery("button",liButton).on( "mouseenter",function(e){
    ...
    _initializeOuterControls: function () { 
        jQuery('a.action-close').on('click',function (event) {
    ...
    _expand: function(elem){
    	...
    	if(elem.hasClass("facet"))
    	{
    		...
    		jQuery("div.facet-title button",elem).trigger("click");	 
    	}
    },
