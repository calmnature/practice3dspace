define('DS/StudioIV/StuXRRay',
	...
	function (STU, Behavior, Task, Tools, EP, DSMath, EventServices, XRManager) {
		...
		var XRRay = function () {
			...
			/**
			 * Ray visu from RenderManager
			 * @private
			 * @type {STU.VisuPrimitive}
			 */
			this._renderedLine = null;
		...
		*/
		XRRay.prototype.onActivate = function (oExceptions) {
			...
			this._lastRayTransform = new DSMath.Transformation();
		...
		*/
		...
		XRRay.prototype.onExecute = function (iContext) {
			...
			if (this.display === true) {
				...
				if (this._renderedLine) {
					this._renderedLine.life = 0;
					this._renderedLine = null //delete the ray if we dont want to display it anymore
				}
			}
		...
		XRRay.prototype.moveRay = function (iTransform) {
			...
			//create the center point of the ray
			var centerPoint = new DSMath.Point();
			centerPoint.copy(offsettedControllerCenterVector);
			centerPoint.addScaledVector(zDirection, this.length/2);

			//Actually draw the ray
			let rm = STU.RenderManager.getInstance();
			if (this._renderedLine) {
				//update ray visu if line exist
				var transfo = orientedWorldActorTransform.clone()
				transfo.vector = centerPoint
				this._renderedLine.transfo = transfo;
			} else {
				//create line if line doesnt exist
				this._renderedLine = rm.createLine(offsettedControllerCenterVector, endPoint, this.rayParams); //quel init aussi a faire ?
			}

			//added this param since there is a diffrence in the referential between win & web
			var startPointTransfo = orientedWorldActorTransform.clone()
				startPointTransfo.vector = offsettedControllerCenterVector
				this._renderedLine.startPointTransfo = startPointTransfo;

			//check we actually need to move the render line (not the case for xr emulation for instance IR-1465960)
			const originTransfo = orientedWorldActorTransform.matrix.getArray()
			const lastTransfo = this._lastRayTransform.matrix.getArray()
			const matrixChanged = !originTransfo.every((v, i) => v === lastTransfo[i])
			const vectorChanged = !orientedWorldActorTransform.vector.isEqual(this._lastRayTransform.vector);
			if (vectorChanged || matrixChanged) {
				this._lastRayTransform = orientedWorldActorTransform.clone();
				this._renderedLine.isLineTransformDirty = true; //set flag to actually move the ray in rendermanager
			}
		...
		*/
		XRRay.prototype._updateRayParams = function () {
			...
			{
				referential: "World",
				lifetime: -1, // The ray has a lifetime infite, and will be deleted if we want to hide it
		...
		XRRay.prototype._onDevicePressEvent = function (iDeviceEvent) {
			...
			let handID=0;
			if (this._actor.deviceID === 14)
				handID=1;
			if (this._actor.deviceID === 15)
				handID=2;

			if (iDeviceEvent.buttonName === XRManager.EDeviceInputName[this.clickInputID] + "_" + controllerID || iDeviceEvent.buttonName === "bPinch" + "_" + handID) {
