define('DS/StudioIV/StuXRManager', [
	...
	function (
		...
		XRManager.prototype.onInitialize = function (oExceptions) {
			...
			this.xrHandLeftPalmTransform = new DSMath.Transformation();
			this.xrHandRightPalmTransform = new DSMath.Transformation();
		};
		...
		XRManager.prototype.startXRViewer = function (iEmulationEnabled) {
			if (this._wrapper !== null) {
				...
				//set ref of this XRManager to C++
				this._wrapper.setJSXRManager(this);

				//setXRCamera as current Camera
		...
		XRManager.prototype.getDeviceTransform = function (iDeviceID) {
			...
			else if (iDeviceID === XRManager.EDeviceIdentifier.eRightHand && this.xrHandRightPalmTransform !== undefined) { // Case where XR Hand, as EP does not support them: they return the controller transform for hand
				deviceTransform = this.xrHandRightPalmTransform
			}
			else if (iDeviceID === XRManager.EDeviceIdentifier.eLeftHand && this.xrHandLeftPalmTransform !== undefined) { // Case where XR Hand, as EP does not support them: they return the controller transform for hand
		 ...
		 * @private
		 *
		 * @return {DBoolean} Transform of the headset device
		 */
		XRManager.prototype.isWorn = function () {
			if (this._wrapper !== null) {
				return this._wrapper.isWorn();
			}
			return false
		}
		...
		/**
		 * Dispatch Event From C++
		 * @method
		 * @private
		 */
		XRManager.prototype.onDispatchEventFromWrapper = function (iStringEventType) {
			let isLeftHand = false;
			if (iStringEventType === 'PinchRightPress') {
				//WARN supporting only pinch for now
				this.dispatchEvent(new STU.XRHandPinchBeginEvent(isLeftHand));
			}
			else if (iStringEventType === 'PinchRightRelease') {
				//WARN supporting only pinch for now
				this.dispatchEvent(new STU.XRHandPinchEndEvent(isLeftHand));
			}

			isLeftHand = true;
			if (iStringEventType === 'PinchLeftPress') {
				//WARN supporting only pinch for now
				this.dispatchEvent(new STU.XRHandPinchBeginEvent(isLeftHand));
			}
			else if (iStringEventType === 'PinchLeftRelease') {
				//WARN supporting only pinch for now
				this.dispatchEvent(new STU.XRHandPinchEndEvent(isLeftHand));
			}
		};
