			...
			for (let i = 0; i < posesListLength; i++) {
				...
				if (isGestureActive && !wasPoseActive) {
					...
					if (poseName === 'xrPinch2Bool') {
						...
						//this.dispatchEvent(new STU.XRHandPinchBeginEvent(isLeftHand)); //--updated with skill in visuxrtoolkit
					}
					...
					if (poseName === 'xrPinch2Bool') {
						//WARN supporting only pinch for now
						//this.dispatchEvent(new STU.XRHandPinchEndEvent(isLeftHand)); //--updated with skill in visuxrtoolkit
					}
				}
			}
		}
