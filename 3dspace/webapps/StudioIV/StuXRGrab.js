define('DS/StudioIV/StuXRGrab',
	...
	function (STU, Behavior, Task, Tools, EP, EventServices, DSMath, XRManager, WebPlayer) {
		...
		var XRGrab = function () {
			...
			/**
			 * Collection identifying all actors that can be manipulated through the XR Grab behavior.
			 * If a collection is set, only those actors can be grabbed.
			 * If no collection is set, then any clickable actor can be grabbed.
			 * @public
			 * @type {STU.Collection}
			 */
			this.manipulationTargets;
		...
		*/
		...
		XRGrab.prototype._onDevicePressEvent = function (iDeviceEvent) {
			...
			if (iDeviceEvent.buttonName === XRManager.EDeviceInputName[this.grabInputID] + "_" + controllerID || iDeviceEvent.buttonName === "bPinch" + "_" + handID) {
				...
				if (WebPlayer.getInstance().isAVPEnabled()) {
					...
					intersection = this._actor.XRRay.pick();
				}

				//check if there is a filter of the grabbed actor
				if (this.manipulationTargets !== null && this.manipulationTargets !== undefined) {
					//there is a filter that has been added, now check if picked actor needs to be filtered out
					if (intersection !== null && intersection !== undefined && this.manipulationTargets.contains(intersection.getActor()) === false) {
