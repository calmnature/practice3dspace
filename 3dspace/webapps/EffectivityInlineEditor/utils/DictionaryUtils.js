/**
 * @quickreview PPD4    25:11:11 Changes for getAvailableFeaturesAndOptions web service
 * @quickreview KR5     24:12:11 TSK11975782 : used onlyModelVersion check to use modelversion for getDictionary
 * @quickreview KR5     24:09:05 added Dictionary_Error for CfgUtility.getLegacyCfgDico web-service failure
 * @quickreview PPD4    24:11:07 IR-1325533 Migration to getAvailableFeaturesAndOptions web service.
 */
...
], function (CfgDictionaryTypeEnum, CfgData, CfgUtility, CfgXMLServices, CfgUXEnvVariablesJS, AppNLS, DicoV3Template) {
    'use strict';
    ...
    var DictionaryUtils = {
        ...
        convertDicoFromCFCO: function (dicoCFCO) {
            ...
            for (i = 0; i < features.length; i += 1) {
                ...
                if (currentFeature.current !== 'Preliminary' && currentFeature.current !== 'Obsolete') {
                    if (currentFeature.selectionType === 'Single') {
                        var variant = { kind: 'reference', attributes: {} };
                        // for configurationdictionary we are getting physical id in 'id' and for getAvailableFeaturesAndOptions we are getting physical id in 'physicalid' attribute
                        variant.id = currentFeature.id || currentFeature.physicalid;
                        ...
                        for (j = 0; j < currentFeature.options.length; j += 1) {
                            ...
                            varValue.id = currentFeature.options[j].id || currentFeature.options[j].physicalid;
                            ...
                            varValue.attributes._title = currentFeature.options[j].displayValue;
                        ...
                        dico.portfolioClasses.member[0].portfolioComponents.member[0].variants.member.push(variant);
                    } else if (currentFeature.selectionType === 'Multiple') {
                        var variabilityGroup = {
                            id: currentFeature.id || currentFeature.physicalid,
                        ...
                        for (j = 0; j < currentFeature.options.length; j += 1) {
                            options.push({
                                id: currentFeature.options[j].id || currentFeature.options[j].physicalid,
                        ...
                        dico.portfolioClasses.member[0].portfolioComponents.member[0].variabilityGroups.member.push(variabilityGroup);
                    }
                }
            }
        ...
        createMapsFromDico: function (dictionary) {
            var mapByName = {};
            var mapById = {};
            var dico_inside = dictionary.portfolioClasses ? dictionary.portfolioClasses.member[0].portfolioComponents.member[0] : dictionary;
            var flatVariants = dico_inside.variants ? dico_inside.variants.member : [];
            var flatVGs = dico_inside.variabilityGroups ? dico_inside.variabilityGroups.member : [];
        ...
        getModelDictionary: function (model, ite_options) {
            var that = this;

            return new Promise(function (resolve) {
                var dictionary = CfgData.getDictionaryByCriteria(model.id, 'features');
                let cfgUseGetAvailableFeaturesOptions = CfgUXEnvVariablesJS.getCfgUXEnvVariables().CFGUseGetAvailableFeaturesOptions;
                if (dictionary.features === null) {
                    var wParam = '';
                    let modelversion = false;
                    if (ite_options.onlyModelVersion) {
                        //Only Model version
                        modelversion = true;
                    }
                    ...
                    if (model.currentEvoExpression !== '')
                        wParam = {
                            evolutionExpression: JSON.stringify({
                                filterExpression: CfgXMLServices.getValidXmlWithoutDescId(model.currentEvoExpression),
                            }),
                        };
                    if (ite_options.isEvolutionBasedOn === 'xRevision' || cfgUseGetAvailableFeaturesOptions) {
                        let options = {
                            version: '1.0',
                            referencePID: ite_options.parentID,
                            modelPIDs: [model.id],
                        };
                        if (!ite_options.cfg_auth_ctx) {
                            let instance = {
                                instancePID: ite_options.instanceID,
                                effectivityCompiledForm: ite_options.evoEffXML ? '' : 'No Effectivity',
                            };
                            options.instances = [instance];
                        }
                        if (ite_options && ite_options.cfg_auth_ctx && ite_options.cfg_auth_ctx.AuthoringContextHeader) {
                            let key = Object.keys(ite_options.cfg_auth_ctx.AuthoringContextHeader)[0];
                            let wsHeaderOptions = {
                                operationheader: {
                                    key: key,
                                    value: ite_options.cfg_auth_ctx.AuthoringContextHeader[key],
                                },
                            };
                            options.wsHeaderOptions = wsHeaderOptions;
                        }
                        CfgUtility.getAvailableFeaturesAndOptions(options).then(function (response) {
                            if (response && response.result && response.result.length > 0) {
                                response.result.forEach((modelResponse) => {
                                    //In case of modelversion no need to check for model id as web service returns model instead of model id
                                    if (modelResponse.modelPID === model.id || ite_options.onlyModelVersion) {
                                        that.processResponse(model, modelResponse);
                        ...
                        CfgUtility.getLegacyCfgDico(model.id, wParam, modelversion).then(
                            function (response) {
                                that.processResponse(model, response);
                                resolve('dictionary loaded from server');
                            },
                            function () {
                                CfgUtility.showNotifs('error', '', AppNLS.Dictionary_Error);
                            }
                        );
                    }
                } else {
        ...
        processResponse: function (model, response) {
            let that = this;
            var optimized_features = [];
            for (var i = 0; i < response.features.length; i++) {
                var ftr = response.features[i];
                // IR-923384-3DEXPERIENCER2022x
                if (ftr.current === 'Preliminary' || ftr.current === 'Obsolete') continue;
                else optimized_features.push(ftr);
            }
            response.features = optimized_features;
            if (response.features.length == 0) CfgUtility.showwarning(AppNLS.NO_FEATURES, 'warning');
