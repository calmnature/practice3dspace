define('DS/PCsBulkViewPresenter/js/Utils/PCsBulkViewPresenterUtils', [
    ...
    PCsBulkViewPresenterUtils.buildSolverInputFormat = function (columnData, allcriteria, baseConfigurationId) {
        ...
        if (columnData && columnData.size > 0) {
            ...
            columnData.forEach((cellData, criteriaId) => {
                if (
                    (allcriteria && ['Chosen', 'Selected', 'Default', 'Dismissed'].includes(cellData.status)) ||
            ...
            columnData.forEach((cellData, id) => {
                if (UWA.is(cellData.numericalValue, 'number')) {
                    ...
                    selecetedValues.forEach((id) => {
                        let rel_id = this._valueRelIdInfo[id].rel_id;
                        let p_rel_id = this._valueRelIdInfo[id].p_rel_id;
                        configurationCriteria.push({
                            id: id,
                            rel_id: rel_id,
                            p_rel_id: p_rel_id,
    ...
    PCsBulkViewPresenterUtils.buildSelectedValues = function (cellData, variabilityValueMap) {
        ...
        this._valueRelIdInfo = {};

        values.forEach((value, id) => {
            if (
                value.status === ConfiguratorVariables.Chosen ||
    ...
    PCsBulkViewPresenterUtils.buildAllValues = function (cellData, variabilityValueMap, isValueRequired, forceChosen) {
        ...
        values.forEach((value, id) => {
            ...
            dropdownValue.state =
                forceChosen && value.status === ConfiguratorVariables.Selected
            ...
            dropdownValue.validate =
                !forceChosen && [ConfiguratorVariables.Selected, ConfiguratorVariables.Default].includes(value.status);
    ...
    PCsBulkViewPresenterUtils.clearSelection = function (cellData) {
        let clear = function (data) {
            if (data.status === ConfiguratorVariables.Default) data.status = ConfiguratorVariables.Dismissed;
            else if (data.status === ConfiguratorVariables.Chosen) data.status = ConfiguratorVariables.Available;
        };
        clear(cellData);
    ...
    PCsBulkViewPresenterUtils.setChosenValues = function (cellData, values) {
        cellData.values.forEach((cellValue, id) => {
            if (!values.includes(id)) {
                if (
                    cellValue.status === ConfiguratorVariables.Selected ||
                    cellValue.status === ConfiguratorVariables.Default
                ) {
    ...
    PCsBulkViewPresenterUtils.createCellDataInSolverFormat = function (id, cellData) {
        ...
        if (cellData.Criterion) {
            if (UWA.is(cellData.Value, 'boolean')) {
                // For options
                IdCriterion = id;
                Value = cellData.Value;
            } else if (UWA.is(cellData.numericalValue)) {
                // For parameters
            ...
            if (variabilityValuesMap[receivedData.id]) {
                ...
                if (variabilityValue && rowID >= 0) {
                    ...
                    if (receivedData.value) {
                        ...
                        if (valueformat.length > 0) cellObj.unit = valueformat[1];
                        else cellObj.unit = undefined;
    ...
    PCsBulkViewPresenterUtils.applySolverConfiguration = function (
        cellData,
        ...
        if (UWA.is(configurationCriteriaType)) {
            ...
            if (configurationCriteriaType === 'Value') {
                ...
                if (UWA.is(Value, 'number')) {
                    ...
                    if (Value === false && criteriaState === 'Incompatible')
                        // case of option required, solver answer is Value{false} Incompatible
                        criteriaState = 'Required';
                    else if (Value === false && criteriaState === 'Required')
                        // case of option incompatible, solver answer is Value{false} Required
                        criteriaState = 'Incompatible';
                }
            }
        } else {
            // ODT SPECIFIC
            // TODO: REFACTOR AFTER MIGRATION OF ODT TO 4.0
            criteriaId = configurationCriteria.Id;
            if (UWA.is(configurationCriteria.NumericalValue)) {
                cellData.numericalValue = configurationCriteria.NumericalValue;
            }
        }
...
define('DS/PCsBulkViewPresenter/js/PCsBulkViewPresenter', [
    ...
    var CONSTANT_GET_CRITERIA_CONFIGURATION_INSTANCE = 'getProductConfigurationCriteria';
    ...
    var PCsBulkViewPresenter = BulkTablePresenter.extend({
        init: function (options) {
            ...
            this._eventManager = new xModelEventManager(
                ...
                this._applicationChannel,
                this._privateChannelForRules
            );
            ...
            this._componentEventListeners = {
                ...
                'update-height': function (e) {
                    that._dataGridView.layout.resetRowHeight(e.options.id.rowID);
                }
            };
            ...
            if (this._modelMaturity === 'Obsolete') {
                if (this._showEditAction) {
                    ...
                    this._privateChannel.publish({
                        ...
                        data: { id: 'edit-pencil', flag: false }
                    });
            ...
            return new Promise((resolve, reject) => {
                if (arraySelectedIds.length) {
                    ...
                    if (UWA.is(that_._batchLoadingPromise)) {
                        ...
                        that_._batchLoadingPromise = that_.batchLoading(arraySelectedIds).then(
                            function () {
        ...
        batchLoading: function (arraySelectedIds) {
            ...
            var FinalPromise = new Promise((resolve) => {
                ...
                arraySelectedIds.forEach((selectedIds) => {
                    PromiseArray.push(this.loadElement(selectedIds));
                });

                if (PromiseArray.length) {
                    Promise.all(PromiseArray).then((values) => {
                        this._bulkTableLoadedPromise.then(() => {
                            this._dataGridView.getTreeDocument().prepareUpdate();
                            this._createColumnsForLoadedInfos(values, selectedPCs);
                            this._pauseCheckConflictCall = false;
                            if (this.checkAllPCs instanceof Function) this.checkAllPCs({ request: 'solve' });
                            this._dataGridView.getTreeDocument().pushUpdate();
                            resolve();
                        });
                    });
                } else {
            ...
            if (loadData) {
                for (var i = 0; i < loadData.length; i++) {
                    if (loadData[i].member && UWA.is(loadData[i].member, 'array')) {
                        ...
                        selectedPCs[i].modelID = this._modelId;
                        this._bulkTableModel.createColumn(selectedPCs[i]);
                        ...
                        if (columnID !== undefined) {
                            ...
                            for (var j = 0; j < dataReceived.length; j++) {
                                ...
                                if (UWA.is(cellData.disableControl, 'function')) cellData.disableControl();
                                cellData.status = dataReceived[j].status;
                                if (UWA.is(cellData.enableControl, 'function')) cellData.enableControl();
        ...
        _createColumnForPC: function (pcObj) {
            ...
            if (pcObj) {
                let VariabilityOpt = {
                    ...
                    setCellValue: function (cellInfos, value) {
                        if (value !== undefined) {
                            ...
                            if (that.pasted) {
                                ...
                                that.checkConflictOfPC(cellInfos.columnID - COLUMN_ID_INDEX).then(function (data) {
                                    that.applySolverAnswer(data, cellInfos.columnID - COLUMN_ID_INDEX);
                                    that.checkStatusOfPC(cellInfos.columnID - COLUMN_ID_INDEX).then(
                                        function (solverData) {
                                            that.applySolverAnswer(solverData, cellInfos.columnID - COLUMN_ID_INDEX);
                                            //update the UX
                                            that._updateDGVColumn(cellInfos.columnID);
                    ...
                    getCellValueForCopy: (cellInfos) => {
                        ...
                        if (cellData.values && cellInfos.nodeModel.options.grid.type === 'VariabilityGroup') {
                            let values = PCsBulkViewPresenterUtils.buildSelectedValues(
                                cellData,
                                this._bulkTableModel._variabilityMap
                            );
                    ...
                    processCellValueFromPaste: (cellInfos, textValue) => {
                        if (cellInfos.nodeModel) {
                            ...
                            if (parentIDAndValue[0] === '' && cellInfos.nodeModel.options.grid.type === 'Parameter') {
                                ...
                                // check same unit
                                if (
                                    (paramValueParts.length === 1 && targetMin.unit === '') ||
                                    (paramValueParts.length === 2 && targetMin.unit === paramValueParts[1])
                                ) {
                                    // check the range of the value
                                    var paramCopiedValue = parseFloat(paramValueParts[0]);
                                    if (
                                        paramCopiedValue >= parseInt(targetMin.value) &&
                                        paramCopiedValue <= parseInt(targetMax.value)
                                    ) {
                                        this.pasted = true;
                ...
                columnOptions = {
                    ...
                    children: [
                        {
                            text: pcObj.attributes.baseConfigurationTitle
                                ? pcObj.attributes.baseConfigurationTitle +
                                  ' ' +
                                  pcObj.attributes.baseConfigurationRevision
                                : '',
                ...
                var data = {
                    id: 'totalColumn',
                    text:
                        _NLS_PCsBulkViewPresenter.TotalProductConfigurations +
                        ' : ' +
                        Object.keys(this._loadedPCs).length
                };
        ...
        removeLoadedPC: function (pcsToRemove) {
            if (pcsToRemove) {
                ...
                for (var i = 0; i < arraySelectedPCs.length; i++) {
                    ...
                    var columnAsReference = this._bulkTableModel.getColumnHeader(
                        this._bulkTableModel.getColumnID(arraySelectedPCs[i].id)
                    );
        ...
        _setCellContent: function (cellInfos, isEditable) {
            ...
            if (this._compareFlag) {
                referenceCellData = this._bulkTableModel.getReferenceCellData(
                    cellInfos.nodeModel.getAttributeValue('id')
                );
            }
        ...
        _handleParameterCell: function (cellInfos, component, referenceCellData) {
            ...
            var paramData = this._bulkTableModel.getRowHeaderData(cellInfos.rowID);
            var paramUnit = paramData.range[0].min.unit;
            var paramMaxRangeValue = parseInt(paramData.range[0].max.value);
            var paramMinRangeValue = parseInt(paramData.range[0].min.value);
            var status = cellData.status;
            var value = cellData.numericalValue;
            component.setProperties({
                value: UWA.is(value, 'number') ? value : null,
                units: paramUnit,
                minValue: UWA.is(cellData.minValue, 'number') ? cellData.minValue : paramMinRangeValue,
                maxValue: UWA.is(cellData.maxValue, 'number') ? cellData.maxValue : paramMaxRangeValue,
                state: status,
        ...
        _handleVariabilityCell: function (cellInfos, component, referenceCellData) {
            ...
            component.setProperties({
                ...
                context: cellInfos,
                possibleValues: PCsBulkViewPresenterUtils.buildAllValues(
                    cellData,
                    this._bulkTableModel._variabilityMap,
                    ['Primary', 'Mandatory'].includes(usage)
                )
            });

            if (UWA.is(values, 'array') && values.length > 1) {
        ...
        _handleVariabilityViewCell: function (cellInfos, component, referenceCellData) {
            ...
            if(this._dataGridView.sortModel && this._dataGridView.sortModel.length > 0){
                properties.sort = [this._dataGridView.sortModel[0]];
            }
            ...
            if (cellData) {
                ...
                component.setProperties({
                    value: value.numericalValue
                        ? [{ title: value.numericalValue + ' ' + _NLS_xUnitLabelLong[cellData.unit] }]
                        : [],
