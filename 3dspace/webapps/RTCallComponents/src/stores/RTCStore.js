...
export const useRTCStore = defineStore('RTCStore', {
    ...
    searchedContacts: (state) => {
      ...
      if (state.searchFilter != '' && state.searchFilter.length > 2) {
        ...
        users = users.filter((user) => {
          if (
            privateConversation.includes(user.login) ||
            user.login === state.currentUserLogin ||
            user.login === 'svc_3dswym_copilot'
          ) {
            return false;
          }
          return (
        ...
        let favorites = uniqueFavoriteArray.map((fav) => {
          if (fav.convId) return state.conversations.get(String(fav.convId));
          else if (fav.extConvId) return state.conversations.get(String(fav.extConvId));
          else if (fav.userId) return state.users.get(fav.userId);
        });
  ...
  actions: {
    ...
    getV2ConversationsById: function (conversation_id) {
      if (
        conversation_id &&
        !this.getConversationById(conversation_id) &&
        conversation_id != '' &&
    ...
    addUser: function (user) {
      if (user.user_id) {
        ...
        user.isUser = true;
        if (!user.username)
          user.username = (user.first_name || user.firstName) + ' ' + (user.last_name || user.lastName);
    ...
    updateConversation: function (data) {
      if (data.convId) {
        ...
        if (data.members) conv.members = [...data.members];
        if ('title' in data) {
    ...
    changeUserInCall: function (user) {
      const additionalLabels = user.additionalLabels || (user.presence && user.presence.additionalLabels)
      if (user.login === this.currentUserLogin && additionalLabels) {
        this.currentUserInCall = additionalLabels && ['inACall', 'presenting'].some((label) => additionalLabels.includes(label));
      }
    },
    ...
    searchContacts: debounce(function (pattern) {
      ...
      if (pattern.length > 2) rtconv.searchContact(pattern);
    }, 500),
  },
});
