...
export const PlatformListeners = async (data) => {
  ...
  if (data.evenement == 'CONNECTED') {
    ...
    if (data.data.callIds) callHistoryStore.deleteCall(data.data.callIds);
    else callHistoryStore.deleteCall(data.data.callId);
    ...
    // }
  } else if (data.evenement == 'call' && data.data.action == 'setRingtone') {
    settings.setRingtone(data.data.ringtone);
  } else if (data.evenement == 'call' && data.data.action == 'updateCallSettings') {
    ...
    rtcStore.changeUserInCall(data.data);
