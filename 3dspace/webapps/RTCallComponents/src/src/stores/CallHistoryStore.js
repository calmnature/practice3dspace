...
export const useCallHistoryStore = defineStore('CallHistory', () => {
    ...
    if (filters.value && (filters.value.search || filters.value.category)) {
      filteredCallIds = callIdList.filter((id) => {
        ...
        if (filters.value.search && filters.value.search != '') {
          if (
            conversation &&
            normalizeString(conversation.title.toLowerCase()).includes(
              normalizeString(filters.value.search.toLowerCase())
            )
          )
            searchFound = true;
          else {
            const users =
              (c.users.length === 1 ? c.users : c.users.filter((el) => el.login !== rtcStore.currentUserLogin)) || [];
            users.forEach((user) => {
              if (
                (user.title &&
                  normalizeString(user.title.toLowerCase()).includes(filters.value.search.toLowerCase())) ||
                (user.username &&
                  normalizeString(user.username.toLowerCase()).includes(
                    normalizeString(filters.value.search.toLowerCase())
                  )) ||
                (user.login &&
                  normalizeString(user.login.toLowerCase()).includes(
                    normalizeString(filters.value.search.toLowerCase())
                  ))
              )
                searchFound = true;
        ...
        if (filters.value.category != '' && filters.value.search != '') return searchFound && categoryFound;
        else return searchFound || categoryFound;
      });
    }
  ...
  const startCall = function (data) {
    if (data.logins && data.logins.length > 0) {
      ...
      if (data.logins.length > 15) {
        messagesStore.displayAlert('morethan15Members');
      } else rtconv.startCall(data);
    }
  };
  ...
  const getUsersPhone = function (call) {
    if (call.users.length == 2) {
      for (let i = 0; i < call.users.length; i++) {
        if (call.users[i].login != rtcStore.currentUserLogin) call.mainPhone = call.users[i].mainPhone;
      }
    }
  };
  ...
  const updateOngoingCall = function (callEvent) {
    if (callEvent.evenement == 'inviteSent' || callEvent.evenement == 'inviteToCall') {
        ...
        */
        ...
        calls.forEach((call) => {
          ...
          if (currentUser) {
            ...
            if (currentUser.quitDate) endDate = new Date(currentUser.quitDate);
            ...
            // set call type
            if (call.isJoinable) call.category = CALL_TYPES.Ongoing;
            else if (currentUser.id == call.caller) call.category = CALL_TYPES.Outgoing;
            else if (currentUser.state == 1) call.category = CALL_TYPES.Incoming;
            else if (currentUser.state == 2) call.category = CALL_TYPES.Declined;
            else call.category = CALL_TYPES.Missed;

            if (!callsMap.has(call.call_id)) {
              // to add ongoing call at the top of the list
              if (call.isJoinable) callIdList.unshift(call.call_id);
        ...
        const currentUser = calls[0].users.find((user) => user.login === rtcStore.currentUserLogin);
        rtcStore.changeUserInCall(currentUser);
      }
    }
    return callHistory;
  };
