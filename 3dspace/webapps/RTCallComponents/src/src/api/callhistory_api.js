  ...
  RTInterface.send(
    {
      ...
      count: limit,
      dateOffset: leftDateOffset,
      dateOffset_left: rightDateOffset,
...
export const removeFromHistory = async (callId) => {
  ...
  const rtcStore = useRTCStore();
  let data = {};
  if (callId instanceof Array) data = { action: 'deleteCallHistory', callIds: callId, tenant: rtcStore.tenantId };
  else data = { action: 'deleteCallHistory', callId: callId, tenant: rtcStore.tenantId };
...
export const clearHistory = async (lastCallDate) => {
  ...
  const rtcStore = useRTCStore();
  RTInterface.send(
    { action: 'cleanCallHistory', lastCallDate: lastCallDate, tenant: rtcStore.tenantId },
    'callToRTC.im.ds.com'
  );
};

export const setRingtone = async (ringtone) => {
  const RTInterface = await getRTInterface();
  const rtcStore = useRTCStore();
  RTInterface.send({ action: 'setRingtone', tenant: rtcStore.tenantId, ringtone }, 'callToRTC.im.ds.com');
};

export const getRingtone = async () => {
  const RTInterface = await getRTInterface();
  const rtcStore = useRTCStore();
  const settingsStore = useSettings();
  if (!settingsStore.ringtone)
    RTInterface.send({ action: 'getRingtone', tenant: rtcStore.tenantId }, 'callToRTC.im.ds.com');
};
