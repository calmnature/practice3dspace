...
const topicRules = computed(() => [isNotTextEmptyRule]);

const validateTopic = () => {
  if (callHistoryStore.selectedContact.length > 1) topicInputref.value.validate();
    ...
    if (contactToAdd.login !== callHistoryStore.currentUserLogin && allUsersSize.value < maxUsers) {
      ...
      if (i == members.length - 1) {
        if (contactToAdd?.isUser && isUserAlreadyAdded(contactToAdd))
          callHistoryStore.selectedContact.push({
            value: contactToAdd,
            label: contactToAdd.username,
            dmId: dmId,
            dbConvId: dbConvId,
          }); // if user is already added, toggle will remove it.
        multiselectRef.value.toggle(
          { value: contactToAdd, label: contactToAdd.username, dmId: dmId, dbConvId: dbConvId },
          { fromOptionsClick: true }
        );
      } else {
        if (contactToAdd?.isUser && !isUserAlreadyAdded(contactToAdd)) {
          callHistoryStore.selectedContact.push({
...
const addToSelectedContact = (contact) => {
  if (contact.isConversation) {
    ...
    multiselectRef.value.toggle(
      { value: contact, label: contact.username, dmId: '', dbConvId: '' },
      { fromOptionsClick: true }
    );
  }
  ...
  <div v-if="translationsLoaded">
    <div style="display: flex; flex-direction: column; gap: 0.5rem; margin-bottom: 10px">
      ...
      <div v-if="showTopicInput" class="conversation-topic">
        <vu-input
          ...
          :placeholder="$i18n('conversationTopicPlaceholder')"
          :label="$i18n('conversationTopicLabel')"
          ref="topicInputref"
          lazyValidation="true"
        />
      </div>
    ...
    <div>
      <vu-scroller class="contactlist-scroller">
        <div
          v-if="props.contacts.value.favorites && props.contacts.value.favorites.length > 0"
        ...
        >
          ...
          <div v-for="contact in props.contacts.value.favorites">
            <div
              ...
              @click="addToSelectedContact(contact)"
        ...
        <br />
        <div
          v-if="props.contacts.value.suggested && props.contacts.value.suggested.length > 0"
