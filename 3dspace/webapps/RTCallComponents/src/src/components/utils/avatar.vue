...
const isTooltipActivated = computed(() => !checkIsMobileDevice() && props.isRightPanel);

const activateTooltip = function () {
  if (enterTimeout) clearTimeout(enterTimeout);
  enterTimeout = setTimeout(() => {
    if (!props.topic) isHovered.value = true;
  }, 700);
};
...
const usersInContact = computed(() => {
  if (props.contact?.isConversation) {
    if (props.contact.members.length === 1) return props.contact.members;
  ...
  <div v-if="usersInContact">
    <div v-if="usersInContact.length > 1" class="conv">
      ...
      <div v-if="contact.members.length > 5" class="flex justify-center items-center member-count">
        <span> +{{ contact.members.length - 4 }} </span>
      </div>
    </div>

    <div v-else-if="isTooltipActivated">
      <RichUserTooltipWrapper
        :displayed-user-login="usersInContact[0].login"
      ...
      >
        <template #default="{ setRef }">
          <UserPresence
            :login="usersInContact[0].login"
            class="medium"
            :swym-url="rtcStore.swymUrl"
            :ref="(el) => setRef(el)"
          />
        </template>
      </RichUserTooltipWrapper>
