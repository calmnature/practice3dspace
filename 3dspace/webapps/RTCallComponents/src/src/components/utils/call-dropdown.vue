...
const getFavoritesSubmenu = computed(() => {
  ...
  if (
    callHistoryStore.enableTelephony &&
    ((props.isLeftPanel && props.contact.mainPhone != 'null') ||
...
const getMainMenu = computed(() => {
  ...
  if (callHistoryStore.enableProfile && props.contact && props.contact.members.length === 2) {
    items.push({
      ...
      handler: () => {
        let data = {};
        data.userLogin = props.contact.isUser
          ? props.contact.login
          : props.contact.members.filter((el) => el.login !== callHistoryStore.currentUserLogin)[0].login;
  ...
  if (props.isRightPanel) {
    items.push(
      ...
      },
      {
        ...
        handler: () => {
          props.call.merged
            ? rtcStore.removeFromHistory(processMergedCalls(props.call))
            : rtcStore.removeFromHistory(props.call.call_id);
        },
      }
    );
  }
