...
const close = function () {
  ...
  rtcallcomponentsTracker.clearHistory().closeClearHistory({
    persDim: {
...
const clearHistory = function () {
  ...
  const rtcallcomponentsTracker = RTCallComponentsTracker();
  rtcallcomponentsTracker.clearHistory().clearHistory({
    persDim: {
      pd1: rtcStore.currentUserId,
      pd2: version,
    },
...
<template>
  <Teleport to="body">
    <div v-if="translationsLoaded">
      <vu-modal :title="$i18n('confirmation')" show @close="close" noMobile="false">
        <template #modal-body>
        ...
        <template #modal-footer>
          <vu-btn color="primary" @click="clearHistory">
            {{ $i18n('clearHistory') }}
          </vu-btn>
          <vu-btn @click="close">
            {{ $i18n('cancel') }}
