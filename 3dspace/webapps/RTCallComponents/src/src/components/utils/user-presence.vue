...
watch(
  ...
  async (element) => {
    if (!element) return;
    try {
      const [component, VueJS] = await requirejs(['DS/RTVueUserPresenceAPI/RTVueUserPresenceAPI', 'vuejs']);
...
watch(
  () => props.login,
  (v) => {
    if (presenceComponent.value) presenceComponent.value.login = v;
  }
);

onBeforeUnmount(() => {
  if (presenceComponent.value) presenceComponent.value.$destroy();
});
</script>
