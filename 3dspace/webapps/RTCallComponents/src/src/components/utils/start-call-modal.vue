...
import { message } from '@ds/platformkit/components';
...
const currentComponent = computed(() => {
  if (isDialerComponent.value)
    return { id: 'Dialer', component: CallDialer, props: { login: rtcStore.currentUserLogin } };
  else
    return {
      id: 'ContactList',
      component: startCallContactlist,
      props: { contacts: searchedContacts },
...
const showDialerComponent = () => {
  if (isDialerComponent.value) isDialerComponent.value = false;
  ...
  if (callHistoryStore.selectedContact.length) {
    ...
    if (callHistoryStore.selectedContact.length > 1) {
      if (!topicInput.topic.trim()) {
        ...
        return;
      } else conversationTopic = topicInput.topic;
    } else conversationTopic = '';
    ...
    for (let i = 0; i < callHistoryStore.selectedContact.length; i++) {
      ...
      if (
        callHistoryStore.selectedContact[i].dmId == '' || // if empty dmId => user that is not in conversation is added
        (dmId != '' && callHistoryStore.selectedContact[i].dmId != dmId)
      )
        // if change in dmId => user of different conversations added
...
const callDropdownItems = computed(() => {
  return [
    {
  ...
  <Teleport to="body">
    <div class="start-call-modal" v-if="translationsLoaded">
      <vu-modal :title="$i18n('newCall')" show @close="closeModal" noMobile="false">
        <template #modal-body>
          <div>
            <vu-message color="warning" :show.sync="showAlert">{{
              $i18n('notAllUsersAdded', { x: callHistoryStore.remainingUsersToAdd })
            }}</vu-message>
          </div>
