...
import { checkIsMobileDevice } from '@ds/platformkit/utils';
...
const isTooltipActivated = computed(() => !checkIsMobileDevice() && props.contact.members.length === 2);
...
const activateTooltip = function () {
  ...
  enterTimeout = setTimeout(() => {
    if (!convTopic.value) isHovered.value = true;
  }, 700);
};
...
const checkState = (call) => {
  ...
  if (
    ...
    (currentUser.state === null ||
      currentUser.state === '3' ||
      isOngoing ||
      (currentUser.state == '1' && currentUser.joinDate < currentUser.quitDate))
  )
    return true;
  else return false;
};
...
<template>
  <div class="call-items" :class="{ ongoingCallIitems: isOngoing }" v-if="translationsLoaded">
    <div
      class="contacts"
      :class="[{ ongoingContacts: isOngoing }, { threeColumn: isMerged || props.call.duration != '' }]"
    >
      <span class="contact-item user-icon">
        <avatar
          :contact="props.contact"
          :topic="!props.contact.hasCustomTitle"
          isRightPanel="true"
          @click="openContact"
        />
      </span>
      <div class="contact-item">
        <div>
          <RichUserTooltipWrapper
            v-if="isTooltipActivated"
            :displayed-user-login="userLogin()"
            :is-hovered="isHovered"
            @mouseenter="activateTooltip"
            @mouseleave="deActivateTooltip"
          >
            <template #default="{ setRef }">
              <a :ref="(el) => setRef(el)" class="contact-title" @click="openContact"
                >{{ props.contact.title }}&nbsp;</a
              >
            </template>
          </RichUserTooltipWrapper>
          ...
          <span class="call-category" :class="{ missed: isMissed }">{{
            $i18n(props.call.category.text.toLowerCase())
          }}</span>
          <span v-if="isMissed && props.call.merged && props.call.merged.length > 0" class="missed">{{
            ' (' + props.call.merged.length + ')'
          }}</span>
        </div>
        <div class="call-date">- {{ call.creation_date_string }}</div>
      </div>
      <div v-if="props.call.category.value == 'Ongoing' && checkState(props.call)" class="join-btn contact-item">
        <button class="btn btn-primary" @click="joinCall('audio', props.call)">
          <vu-icon icon="phone" class="call-icon"></vu-icon>{{ $i18n('joinCall') }}
        </button>
      </div>
    ...
    <div v-if="props.call.category.value !== 'Ongoing'" class="call-dropdown">
      <call-dropdown
        v-if="callHistoryStore.isRTCStoreLoaded"
        :isRightPanel="true"
        :call="props.call"
        :contact="props.contact"
      >
