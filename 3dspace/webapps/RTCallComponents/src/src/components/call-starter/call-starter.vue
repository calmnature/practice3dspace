...
const contactSearch = computed(() => {
  ...
  if (contact.value) {
    ...
    let filteredFavs = favorites.filter((favorite) => {
      ...
      if (favorite.members) {
        let members = favorite.members.filter((user) => {
          return user.login !== rtcStore.currentUserLogin;
        });
        ...
        let filteredLogins = membersLogin.filter((login) =>
          login.includes(normalizeString(contact.value.toLowerCase()))
        );
        let filteredUsernames = memberUserNames.filter((username) =>
          username.includes(normalizeString(contact.value.toLowerCase()))
        );

        // If we have any matches, return the favorite
        if (filteredLogins.length || filteredUsernames.length) return true;
      }
