	...
	let getRefinementToSnN = function(socket_id, title, multiSelect,recentTypes){	
		var searchSource=[];
			if(widget.getValue("x3dPlatformId") != "OnPremise"){
				searchSource=["3dspace","usersgroup"];
			}else{
				searchSource=["3dspace"];
			}
    ...
    */
    buildToolbarView : function()
    {
     ...
     // [0].options.grid.type
      ...
      that.typeButton = new WUXComboBox({ elementsList: objTypes, placeholder: NLS.selectType, selectedIndex: -1, enableSearchFlag: false, multiSelFlag: true}).inject(headerContainer);
      that.eventButton = new WUXComboBox({ elementsList: eventTypes, placeholder: NLS.selectEvent, selectedIndex: -1, enableSearchFlag: false, multiSelFlag: true }).inject(headerContainer);
...
/*
 * @module 'DS/ENOSubscriptionMgmt/Config/ViewSubscriptionGridViewToolbarConfig.js'
 * This config is used to create a toolbar for "Edit Subscription for Others" dialog
 */

define('DS/ENOSubscriptionMgmt/Config/ViewSubscriptionGridViewToolbarConfig',
[	
   	'i18n!DS/ENOSubscriptionMgmt/assets/nls/ENOSubscriptionMgmt'
   	],
  	function (
    ...
    let ViewSubscriptionGridToolbarConfig;
    
    let expandCollapseMenu = function(){
		let _expandCollapse =  {
    		menu:[
                {
				  id: "expand",
                  ...
                  fonticon: {
                      family:1,
                      content:"wux-ui-3ds wux-ui-3ds-flow-tree-expand-down"
                    },
                  action: {
                      module: 'DS/ENOSubscriptionMgmt/Model/ViewSubscriptionModel', 
                      func: 'expandTree',
                    }
                },
                {
	...
	let writetoolbarDefinition = function () {
    	
    	let expandCollapseMenuItems = expandCollapseMenu();
		let definition = {};
		let entries = [];
		...
		entries.push({
	        ...
	        "position": "far",
	        "action": {
				module: 'DS/ENOSubscriptionMgmt/Components/Wrappers/WrapperDataGridView',
				func: 'launchFindWindow',
			},
	        "tooltip": NLS.search,
	        "category": "action"
	    });     
	    ...
	    entries.push({
	        "id": "expandCollapse",
	        "domId": "expandCollapse",
	        "class": "expandCollapse",
	        "dataElements": {
	     	   "typeRepresentation": "functionMenuIcon",
	            "icon": {
	              "iconName": "flow-tree-expand-down",
	              "fontIconFamily": 1
	            },
	            "value": expandCollapseMenuItems
	        },
	        "position": "far",
	        "tooltip": NLS.viewControlCommands,
	        "category": "action"
	    });  
    ...
    ViewSubscriptionGridToolbarConfig = {
		writetoolbarDefinition: () => {return writetoolbarDefinition();}
    };

    return ViewSubscriptionGridToolbarConfig;
...
define('DS/ENOSubscriptionMgmt/Config/MySubscriptionGridViewToolbarConfig',
	[
		'i18n!DS/ENOSubscriptionMgmt/assets/nls/ENOSubscriptionMgmt'
	],
	function (
		...
		let MySubscriptionGridToolbarConfig;

		const menuTemplate = (title, type, index) => 
			({
				type: 'CheckItem',
				title: title,
				state: "selected",
				id: title,
				action: {
					module: 'DS/ENOSubscriptionMgmt/Views/MySubscriptionGridView',
				...
				tooltip: 'SeparatorItem'
			})

		function addFilterToolbarItems(filterPreference = {}) {
			const  { objTypes, eventTypes } = filterPreference
			const menu = [];
			objTypes && objTypes.forEach(type => {
				menu.push(menuTemplate(type, "type", menu.length))
			})
			menu.push({
				type: 'SeparatorItem',
				title: ''
			})
			eventTypes && eventTypes.forEach(event => {
				menu.push(menuTemplate(event, "event", menu.length))
			})
			widget
			return { menu };
		}
		...
		let writetoolbarDefinition = function (filterPreference) {
			...
			entries.push(
				{
				"id": "unsubscribe",
				"domId": "unsubscribeButton",
				"dataElements": {
					"typeRepresentation": "functionIcon",
					"icon": {
						"iconName": "bell-delete",
						fontIconFamily: WUXManagedFontIcons.Font3DS
					},
					"action": {
						"module": "DS/ENOSubscriptionMgmt/Components/Wrappers/WrapperDataGridView",
						"func": "unsubscribeObject"
					}
				},
				"position": "far",
				"category": "delete",
				"tooltip": { title: NLS.Unsubscribe }
			},
			{
				"id": "search",
				"domId": "search",
		...
		MySubscriptionGridToolbarConfig = {
			writetoolbarDefinition
		};

		return MySubscriptionGridToolbarConfig;
...
define('DS/ENOSubscriptionMgmt/Utils/ParseJSONUtil',
	...
	var ParseJSONUtil = UWAClass.extend({
		...
		getURLwithLanguage: function(url) {
            ...
            var quesAmp=url.indexOf('?') === -1 ? '?' : '&';
            
            var scValue = widget.getValue("xPref_CREDENTIAL") ? "ctx::"+widget.getValue("xPref_CREDENTIAL"): "";
            if(scValue){
            	url =url +quesAmp+'SecurityContext='+encodeURIComponent(scValue);
            }
            
            var tenantValue = widget.getValue("x3dPlatformId") ? widget.getValue("x3dPlatformId") : "";
            if(tenantValue){
            	url = url + (url.indexOf('?') === -1 ? '?' : '&') + 'tenant=' + tenantValue;
            }
			return url;
		}
...
function(NotificationsManagerUXMessages,NotificationsManagerViewOnScreen) {
   ...
   let SubscriptionNotify = function () {
    ...
    SubscriptionNotify.prototype.handler = function () {
    	...
    	}else if(document.getElementById("MySubscriptionsDialog")){
    		NotificationsManagerViewOnScreen.inject(document.getElementById("MySubscriptionsDialog"));
    	}
    	else{
...
define('DS/ENOSubscriptionMgmt/Components/Wrappers/WrapperDataGridView',
	...
	function (DataGridView, WAFData, CollectionViewStatusBar, DataGridViewLayout, WUXFind, Utils, ParseJSONUtil, ArrayUtils, SubscriptionNotify, NLS) {
		...
		let launchFindWindow = function () {
			...
			if (!_dataGrid.findWidget) {
				...
				if (_toolbar.elements.container.offsetWidth < 290) {
					...
					_dataGrid.findWidget.getContent().style.right = "75px";
				}

				_dataGrid.findWidget.getContent().style.bottom = "0px";
		...
		let unsubscribeObject = function () {
			let findParent = _toolbar.elements.container;
			const findSpan = findParent.querySelector(
				'.wux-controls-toolbar-mask-container .wux-tweakers .wux-ui-3ds-bell-delete'
			);
			const parent = findSpan.parentElement.parentElement;
			if (parent && parent.classList.contains('wux-controls-button')) {
				findParent = parent;
			...
			let selectedRows = _dataGrid.getTreeDocument().getSelectedNodes()
			...
			subscribeOptions.data = JSON.stringify(new ParseJSONUtil().createDataWithElementForRequest(selectedElements, widget.getValue('csrf')));
			...
			subscribeOptions.onComplete = function (serverResponse) {
				...
				//Update dialog table & title
				ArrayUtils.optimizedForEach(nodesToRemove, function (node) {
					node.remove();
				});
				if (widget.mySubscriptionDialog) {
					var countOfEvents = 0;
					_dataGrid.model.forEach(function (row) {
						if (row._options.grid.revision == "1")
							countOfEvents++;
					});
					if (!widget.mySubscriptionDialog.title.contains(countOfEvents)) {
						widget.mySubscriptionDialog.title = NLS.replace(NLS.MySubscriptionsTitle, {
							tag1: countOfEvents
						});
					}
				}
			...
			subscribeOptions.timeout = 0;
			var url = widget.getValue('url') + '/resources/v1/modeler/subscriptions';
	...
	let deleteRowModelByIndex = function(treeDocumentModel,index){
		...
		*/
		...
		WrapperDataGridView = {
			...
			launchFindWindow: () => { return launchFindWindow(); },
			unsubscribeObject: () => { return unsubscribeObject(); }
    ...
    'use strict';
	 ...
	 let onSubscriptionCellRequest= function (cellInfos) {
     		...
     		if (!cellInfos.isHeader) {
                 ...
                 if((cellInfos.nodeModel._options.grid.context=='User' && cellInfos.nodeModel._options.grid.Name != widget.data.dsUser && cellInfos.nodeModel._options.grid.relationship=='Subscribed Person') || (cellInfos.nodeModel._options.grid.context=='Group Member')){
                 	disableUnsubscribe=true;
                 }
                 //var imgIcon=EnoviaBootstrap.get3DSpaceURL() + '/webapps/ENOSubscriptionMgmt/assets/icons/22/I_ActionUnSubscribe_22.png';
                 var imgIcon;
				 let iconImage = {
						"iconName": "bell-delete",
						fontIconFamily: WUXManagedFontIcons.Font3DS
				 ...
				 if(cellInfos.nodeModel._options.grid.relationship=='Subscribed Person'){
                 	//imgIcon=EnoviaBootstrap.get3DSpaceURL() + '/webapps/ENOSubscriptionMgmt/assets/icons/22/I_ActionUnSubscribe_22.png';
					imgIcon = iconImage;
             	...
             	var buttonUnsubscribe = new WUXButton({ 
						...
						onClick: function(e){
							...
							cellInfos.nodeModel._parentNode._options.children.forEach(child => {
								if( child == cellInfos.nodeModel){
									child.remove();
								}
							});
							...
							subscribeOptions.onComplete = function (serverResponse) {
								...
								if (cellInfos.nodeModel.options.grid.event == '') {
									if (cellInfos.nodeModel.options.grid.revision == "1") {
										countOfEvents++;
										titleCount = cellInfos.nodeModel._parentNode.options.children.length - 1;
									}
									cellInfos.nodeModel.remove();
								} else if (cellInfos.nodeModel._parentNode.options.children.length == 1) {
									countOfEvents++;
									titleCount = cellInfos.nodeModel._parentNode._parentNode.__children.length - 1;
...
function(
	...
	var ObjectSubscribeEvents = {
		...
		showEditModalDialog : function(options) {
			...
			var localContext = PADContext.get() || that.context || that.options.context;
			let selectedNodes = localContext.getSelectedNodes(); 
			let titleKey = 'editSubscriptionsTitle'; 
			let tagValue = selectedNodes.length; 
			if (selectedNodes.length === 1) { 
				const node = selectedNodes[0]; 
				if (node?._options?.label) { 
					titleKey = 'editSubscriptionsOnlyTitle'; 
					tagValue = node._options.label; 
				} else { 
					titleKey = 'editSubscriptionsTitleOneObject'; 
				} 
			}
			widget.setValue('selectedObjectTitle', NLS.replace(NLS[titleKey], { tag1: tagValue }));
			that.onCompleteEdit = function(serverResponse) {
				...
				var objEventList = [], objEventNLSList = [], eventSubscriptionList = [], 
				buttonGroup = new WUXButtonGroup({ type: 'checkbox' , domId: 'EditSubscriptionButtonGroup'});
	            ...
	            that.dialog = new WUXDialog({
				   domId: "EditSubscriptionsDialog",
	               title: widget.getValue('selectedObjectTitle'),
	               modalFlag: true,
	               width:400,
	               height:250,
		       ...
		       CreatePushSubscribeDialog.autoCompleteAttendees= new WUXAutoComplete(
		     			{
		     				...
		     				elementsTree :  function (typedValue) {                                   //CreatePushSubscribeDialog.modelForAttendees,
								return new Promise(function (resolve, reject) {
									// Simulate an asynchronous server call to retrieve the AutoComplete possible values
									var getSuccess = function (data) {
										CreatePushSubscribeDialog.modelForAttendees = new TreeDocument();
										stringRoleFilter = BaseFilter.inherit({
			      publishedProperties: {},
			      ...
			      isDataFiltered: function(data) {
			        ...
			        else if (dataLabel && this.filterModel && this.filterModel.text) {
						var filter = this.filterModel.text.toLowerCase();
						var doesDataFitTheFilter = dataLabel.toLowerCase().indexOf(filter) > -1;
						return !doesDataFitTheFilter;
					}
					else{		// if the filter is empty, nothing should be filtered out
			         	return false;
			        }
			      },
    			...
    			var filterManager = CreatePushSubscribeDialog.modelForAttendees.getFilterManager();
											...
											var nodeForAttendee = new TreeNodeModel(
		     			{
														...
														value :  results[i].identifier,     //resp[i].value,
														name  : results[i].name,
														...
														name:results[i].name,
														label : results[i].label,
														value : results[i].identifier
														},
														id: results[i].id
													});
											CreatePushSubscribeDialog.modelForAttendees.addRoot(nodeForAttendee);
										}
										resolve(CreatePushSubscribeDialog.modelForAttendees);
									};
									...
									let queryStrPreCond =CreatePushSubscribeDialog.getPrecondForSearch();
									queryString = "([ds6w:label]:(*"+typedValue+"*) OR " +"[ds6w:identifier]:(*"+typedValue+"*)) AND ("+queryStrPreCond+")" ;
									var inputjson = { "with_indexing_date": true, "with_nls": false, "label": "yus-1515078503005", "locale": "en", "select_predicate": ["ds6w:label", "ds6w:type", "ds6w:description", "ds6w:identifier", "ds6w:responsible", "ds6wg:fullname"], "select_file": ["icon", "thumbnail_2d"], "query": queryString, "order_by": "desc", "order_field": "relevance", "nresults": 1000, "start": "0", "source": searchSource, "tenant": widget.getValue("x3dPlatformId") };
									...
									require(['DS/i3DXCompassPlatformServices/i3DXCompassPlatformServices'],function(CompassServices){
										CompassServices.getServiceUrl({ 
											...
											platformId : widget.getValue("x3dPlatformId"), 
											onComplete : function (data) {
												var url = data+"/search?xrequestedwith=xmlhttprequest";
												SubscriptionServices.makeWSCall(url, "POST", "enovia", 'application/json', inputjson, getSuccess, getFailure, options);
											},
											onFailure: function (data) {
												//reject(data);
											}
       ...
       onSearchUserClick:  function(){
        ...
        OnSearchComplete:function(result)
        {
        	for (var d = 0; d < result.length; d++) {
				...
				if(CreatePushSubscribeDialog.autoCompleteAttendees.selectedItems==undefined){
					CreatePushSubscribeDialog.autoCompleteAttendees.selectedItems = nodeArray;
				}else {
					CreatePushSubscribeDialog.autoCompleteAttendees.selectedItems = CreatePushSubscribeDialog.autoCompleteAttendees.selectedItems.concat(nodeArray);
								}
					CreatePushSubscribeDialog.autoCompleteAttendees.selectedItems.forEach(function(dataElem) {
								dataElem.select();
						});
...
define('DS/ENOSubscriptionMgmt/Views/ViewSubscriptionGridView',
		[ 
			...
			'DS/ENOSubscriptionMgmt/Config/ViewSubscriptionGridViewToolbarConfig'
			], function(
					PushSubscriptionGridViewConfig,
					WrapperDataGridView,
					ViewSubscriptionGridViewToolbarConfig
            	    ) {
	...
	let build = function(model){
        ...
        */
        ...
        let toolbar = ViewSubscriptionGridViewToolbarConfig.writetoolbarDefinition();
        let dataGridViewContainer = WrapperDataGridView.build(model, PushSubscriptionGridViewConfig, toolbar, gridViewDiv);
        dataGridViewContainer.getChildren()[0].style.top = "40px";
 ...
 * datagrid view for Agenda summary page
 */
define('DS/ENOSubscriptionMgmt/Views/MySubscriptionGridView',
		[ 
			'DS/ENOSubscriptionMgmt/Config/PushSubscriptionGridViewConfig',
			'DS/ENOSubscriptionMgmt/Components/Wrappers/WrapperDataGridView',
			'DS/ENOSubscriptionMgmt/Config/MySubscriptionGridViewToolbarConfig',
			'DS/Utilities/Array',
			], function(
					PushSubscriptionGridViewConfig,
					WrapperDataGridView,
					MySubscriptionGridViewToolbarConfig,
					ArrayUtils
            	) {
	...
	const getTypesAndEvents = rows => {
    	...
    	rows.forEach(function (row){
			...
			row.options.children.forEach(function (rowChild){
				if(rowChild.options.grid.event != "" && eventTypes.indexOf(rowChild.options.grid.event) == -1){
					eventTypes.push(rowChild.options.grid.event);
				}
			})  
    	});
		return { objTypes, eventTypes }
	}
	...
	let build = function(model){
		...
		data.model =model;
		data.columns=PushSubscriptionGridViewConfig;
		let rows = model.getChildren();

        var gridViewDiv = UWA.createElement("div", {id:'MySubscriptionsDialog','class': "mySubscription-gridView-View"});
        let toolbar = MySubscriptionGridViewToolbarConfig.writetoolbarDefinition(getTypesAndEvents(rows));
        let dataGridViewContainer = WrapperDataGridView.build(model, PushSubscriptionGridViewConfig, toolbar, gridViewDiv);
        dataGridViewContainer.getChildren()[0].style.top = "40px";
       	_toolbar = WrapperDataGridView.dataGridViewToolbar();
        return dataGridViewContainer;
	...
	let filterTreeListView = function(typesSelected, eventsSelected){
    		// if(!windowResized){
    		// 	treeListView.getContent().childNodes[0].childNodes[1].style.minWidth =  "150px";
    		// 	windowResized = true;
    		// }
	...
	const changeOwnerFilter = (view, option) => {
			console.log(view, option);
				var viewIcon = MySubscriptionGridView.getToolbar().getNodeModelByID("eventTypeFilter");
				console.log(viewIcon);

				var append = true;
				if (view.type == "type") {
					console.log("view.filter: ", view.filter);

					if (viewIcon.options.grid.data.menu[view.index].state == "selected") {
						viewIcon.options.grid.data.menu[view.index].state = "unselected";
						append = false;
					} else {
						viewIcon.options.grid.data.menu[view.index].state = "selected";
					}
				...
				var menuList= view.dsModel.value.menu;
				let typesSelected =[], eventsSelected =[];  
				menuList.forEach(function(menuItem){
					if(menuItem.type == 'SeparatorItem'){
						return;
					}
					if(menuItem.action.argument.type == "type" && menuItem.state == "selected")
						typesSelected.push(menuItem.action.argument.filter);
					else if (menuItem.action.argument.type == "event" && menuItem.state == "selected")
						eventsSelected.push(menuItem.action.argument.filter);
				});

				filterTreeListView(typesSelected, eventsSelected);
		}

    let MySubscriptionGridView={
            build : (model) => { return build(model);},
			// updateEventAndTypeDropdown : () => {return updateEventAndTypeDropdown();},
            getToolbar: () => {return getToolbar();},
			changeOwnerFilter
    };

    return MySubscriptionGridView;
});
...
[
	    ...
	    response.data.forEach(function(dataElem) {
			...
			for(var i=0; i< eventsSubscribed.length; i++){
				...
				for(var j=0; j< UserDetails.length; j++){
					...
					var userFullName = user.objectFullName;
					var objectRev = user.objectRev;
					var userLabel = user.objectFullName+' ('+username+')';
		            ...
		            if (user.objectType === "Group" || user.objectType === "Group Proxy") {
						var groupMembers = user.GroupMembersDetails;

						if(groupMembers && groupMembers[0]!== null){
							for (var k = 0; k < groupMembers.length; k++) {
								
								var member = groupMembers[k];
								var memberName = member.memberName;
								var memberFullName = member.memberFullName;
								var memberRev = member.memberRev;
								var memberLabel = member.memberFullName+' ('+memberName+')';
								var memberRel = user.objectRelationship;
	
								if (member.memberAttrTitle && member.memberAttrTitle != null && member.memberAttrTitle != "null") {
									memberLabel = member.memberAttrTitle;
								}
	
								var memberTree = new TreeNodeModel({
									label: memberLabel,
									id: dataElem.dataelements.id,
									grid: {
										Name: memberName,
	...
	let expandTree = function(){
		if(model.getSelectedNodes().length == 0) {
			model.expandAll();
		}
		else{
			model.getSelectedNodes().forEach((node) => node.expandAll());
		}
	};
	
	let collapseTree = function(){
		if(model.getSelectedNodes().length == 0) {
			model.collapseAll();
		}
		else{
			model.getSelectedNodes().forEach((node) => node.collapseAll());
		}
	};
	...
	let ViewSubscriptionModel = {
			...
			expandTree: () => {return expandTree();},
			collapseTree: () => {return collapseTree();}
...
//    'DS/ENOSubscriptionMgmt/Config/ViewSubscriptionGridViewToolbarConfig',
    'DS/ENOSubscriptionMgmt/Components/Wrappers/WrapperDataGridView',
   'DS/ENOSubscriptionMgmt/Views/MySubscriptionGridView',
   'i18n!DS/ENOSubscriptionMgmt/assets/nls/ENOSubscriptionMgmt',
   'css!DS/ENOSubscriptionMgmt/ENOSubscriptionMgmt'   
],
function(
...
//					}
			...
			that.drawListViewPage = function(serverResponse){
				...
				options.allowUnsafeHTMLContent = false;
				...
				// var gridViewDiv = UWA.createElement("div", {id:'dataGridViewContainer', 'class': "mySubscription-gridView-View"});
				// let toolbar = ViewSubscriptionGridViewToolbarConfig.writetoolbarDefinition();
				// let dataGridViewContainer = WrapperDataGridView.build(model, PushSubscriptionGridViewConfig, toolbar, gridViewDiv);
				// dataGridViewContainer.getChildren()[0].style.top = "40px";
				// _toolbar = WrapperDataGridView.dataGridViewToolbar();

				let datagridDiv = MySubscriptionGridView.build(model);

				const dataGridView = WrapperDataGridView.dataGridView();
				

				const onSelect = () => {
					const unsubscribeBtn = document.getElementById('unsubscribeButton');
					if(WrapperDataGridView.dataGridView().treeDocument.getSelectedNodes().length == 0){
						//unsubscribeBtn.disabled = true
						unsubscribeBtn.disabled = true;
						WrapperDataGridView.dataGridView()._toolbar.getContent().querySelectorAll('.wux-toolbar-items')[0].querySelector('.wux-tweakers').setAttribute('disabled', '');
						WrapperDataGridView.dataGridView()._toolbar.getContent().querySelectorAll('.wux-toolbar-items')[0].querySelector('.wux-tweakers > div').setAttribute('disabled', '');

					}else{
						unsubscribeBtn.disabled = false
						WrapperDataGridView.dataGridView()._toolbar.getContent().querySelectorAll('.wux-toolbar-items')[0].querySelector('.wux-tweakers').removeAttribute('disabled');
						WrapperDataGridView.dataGridView()._toolbar.getContent().querySelectorAll('.wux-toolbar-items')[0].querySelector('.wux-tweakers > div').removeAttribute('disabled');

					}
				}

				dataGridView.getTreeDocument().getXSO().onAdd(onSelect);
				dataGridView.getTreeDocument().getXSO().onRemove(onSelect); 

				let MySubscriptionToolbar = MySubscriptionGridView.getToolbar();
				...
				widget.setValue('csrf', that.csrf);
				widget.setValue('url', that.url);
				...
				let toolBarContainer = UWA.createElement('div', {id:'MySubscriptionToolbar', 'class':'mySubscription-toolbar-container'}).inject(containerDiv);
				MySubscriptionToolbar.inject(toolBarContainer);
        		datagridDiv.inject(containerDiv);
				//gridViewDiv.inject(containerDiv) //rki commented
	           ...
	           //    disabled:false,
				...
				widget.mySubscriptionDialog = dialog;
				...
				const eventTypeFilter1 = document.getElementById('eventTypeFilter');
				console.log('eventTypeFilter: ', eventTypeFilter1);
			...
			// that.filter123 = function(){
			// 	const eventTypeFilter1 = document.getElementById('eventTypeFilter');
			// 	console.log('eventTypeFilter: ', eventTypeFilter1);
			// 	eventTypeFilter1.addEventListener('change', function(e){
			// 		console.log("inside filter123()");
			// 		console.log(e);
			...
			// 	that.filter123();
			// }, 1000);
			const eventTypeFilter2 = document.getElementById('eventTypeFilter');
			console.log('eventTypeFilter: ', eventTypeFilter2);
...
define('DS/ENOSubscriptionMgmt/Views/ViewSubscriptionsDialog',
    ...
    let drawViewSubscriptionsDialogsView = function(containerDiv){
        ...
        let EditSubsForOthersToolbar = ViewSubscriptionGridView.getToolbar();
        let toolBarContainer = UWA.createElement('div', {id:'EditSubsForOthersToolbar', 'class':'editSubsOther-toolbar-container'}).inject(containerDiv);
        EditSubsForOthersToolbar.inject(toolBarContainer);
        datagridDiv.inject(containerDiv);
