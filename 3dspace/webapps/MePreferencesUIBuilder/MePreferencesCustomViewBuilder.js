define('DS/MePreferencesUIBuilder/MePreferencesCustomViewBuilder', [
    ...
    function (I18n, MePreferencesPageModelDataProvider, MePreferencesClientCacheHelper,
        ...
        mePrefWidgetTranslation, MePreferencesPlatformServicesUtils, MePreferencesClientCacheMgr) {
        //"use strict";
        ...
        var MePreferenceCustomViewBuilder = {
            buildCustomView: async function (appID, data) {
                ...
                var encryptedKey = await mePrefClientCacheMgr.retrieveKey(appID);
                ...
                if (prefConfigData) {
                    ...
                    var customDlg = await MePreferencesPlatformServicesUtils.getServiceUrl(tenantID, serviceName).then(async (url) => {
                        if (url) {
                            ...
                            let pref_id = appID.replace(/\./g, '_') + '.' + repoName + '.' + prefName;
                            return mePreferencesCustomViewObj.createCustomDialog(completeURL, jsonValue.value, pref_id, jsonValue.lockState, isStandaloneUI, isAdminView, displayName, dlgHeight, dlgWidth, tenantID, data.variation.type, encryptedKey);
                        }
                        else {
                            CreateErrorDialog();
                        }
