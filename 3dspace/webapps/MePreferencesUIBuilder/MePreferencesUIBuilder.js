    ...
    function (
       ...
       /* MePreferencesSelectionModel,*/ MePreferencesPlatformServicesUtils, mePrefWidgetTranslation,
        ...
        mePrefernceUIBuilder.prototype.createUI = function (UIJson, nlsJSON, preferenceValues, enumValues, rangeValues,
            ...
            function createRadioButtonGroup(appID, enumKey, isAdminView, lockValue, needrefresh) {

                let radioItemsValues = mepUIDataUtilsObj.getDataFromJsonResponse("enumValues", enumKey.split(".")[0], enumKey.split(".")[1], "values");
                let radioItemsArray = [];
                //Create button Group
                let buttonGroup = new WUXButtonGroup({ domId: appID + "." + enumKey, type: "radio" });

                if (radioItemsValues === null)
                    return buttonGroup;
                for (let i = 0; i < radioItemsValues.length; i++) {
                    radioItemsArray.push(getValueFromNLS(appID + "." + enumKey + "." + radioItemsValues[i] + ".Label", nlsJSON));
                }
                ...
                for (let nbItems = 0; nbItems < numberOfValues; nbItems++) {
                    let radioItem = new WUXToggle({ domId: appID + "." + enumKey, type: "radio", label: radioItemsArray[nbItems], value: radioItemsValues[nbItems], checkFlag: false });
                ...
                buttonGroup.addEventListener("buttonclick", function (e) {
                    ...
                    let type = "radio";
                  //  let needRefresh = needrefresh;
                    let data = [identifier, selectedvalue, type, needrefresh];
                    sendEvent(data);
                });
            ...
            function createCheckButtonGroup(preferenceItemKey, repoKey, needRefresh) {
                ...
                buttonItem = new WUXToggle({
                    domId: preferenceItemKey.split(".")[0] + "." + repoKey + "." + preferenceItemKey.split(".").pop(),
                    type: "checkbox",
            ...
            function createCustomControlButton(preferenceItemKey, repoKey, preferenceObj, lockValue, needRefresh) {
                ...
                customButton.addEventListener("buttonclick", function (e) {
                    let selectionModelValue = MePreferencesSelectionModel.getUpdatedPreference(preferenceItemKey.split(".")[0], repoKey, preferenceObj.name);
                    let selectionModelLockstate = (MePreferencesSelectionModel.getUpdatedPreferenceLockState(preferenceItemKey.split(".")[0], repoKey, preferenceObj.name));
                    ...
                    createCustomUI(preferenceObj, customButtonvalue, customButton.domId, lockValue, displayName, needRefresh);
                });
                return customButton;
            ...
            async function createCustomUI(preferenceObj, customButtonvalue, type_id, lockValue, displayName, needRefresh) {
                ...
                return await MePreferencesPlatformServicesUtils.getServiceUrl(tID, serviceName).then(async (serviceUrl) => {
                    if (serviceUrl) {
                        ...
                        await MePreferencesCustomViewObj.createCustomDialog(completeUrl, customButtonvalue, type_id, lockValue, false, isAdminView, displayName, dlgHeight, dlgWidth, tID, null, null, false, needRefresh); //false parameter for isStandaloneUI
                    }
                    else {
                        CreateErrorDialog();
                    }
            ...
            function createComboBox(appID, enumKey, sortmethod, needRefresh) {
                ...
                if (comboBoxValues) {
                    for (let i = 0; i < comboBoxValues.length; i++) {
                        comboBoxItems[i] = [];
                        comboBoxItems[i]["label"] = getValueFromNLS(appID + "." + enumKey + "." + comboBoxValues[i] + ".Label", nlsJSON);
            ...
            function createColorPicker(preferenceItemKey, repoKey, needRefresh) {
                ...
                let colorPicker = new WUXColorPicker({ domId: preferenceItemKey.split(".")[0] + "." + repoKey + "." + preferenceItemKey.split(".").pop(), value: colorValue });
            ...
            function createSpinBox(repoKey, preferenceItemKey, stepSize, tweakerType, nbDecimal, needRefresh) {
            ...
            function createLineEditor(preferenceItemKey, repoKey, needRefresh) {
            ...
            function createMagnitudeSpinBox(preferenceItemKey, repoKey, paramType, stepvalue, nbDecimal, needRefresh) {
                let suffix = null;
                if (paramType.toLowerCase() === "length") {
                    suffix = "mm";
            ...
            function createImageComboBox(appID, enumKey, iconlist, sortmethod, needRefresh) {
                let comboBoxItems = [];
                let selIndex = -1;
                let comboBoxValues = mepUIDataUtilsObj.getDataFromJsonResponse("enumValues", enumKey.split(".")[0], enumKey.split(".")[1], "values");
            ...
            function createPreferenceItem(preferenceItemKey, preferenceObj) {
                ...
                // Check if preferenceObj.Refresh exists
                let needRefresh = preferenceObj.needrefresh ? preferenceObj.needrefresh : null;
                ...
                if (preferenceType.toLowerCase() === "radio") {
                    ...
                    element = createRadioButtonGroup(preferenceItemKey.split(".")[0], enumKey, isAdminView, lockValue, needRefresh);
                    ...
                    let buttonItem = createCheckButtonGroup(preferenceItemKey, repoKey, needRefresh);
                    ...
                    let comboBox = createComboBox(preferenceItemKey.split(".")[0], enumKey, sortmethod, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", enumKey.split(".")[0], enumKey.split(".")[1], "lockState");
                    ...
                    let repoKey = getRepository(preferenceObj);
                    let enumKey = getRepository(preferenceObj) + "." + preferenceObj.name;
                    let colorPicker = createColorPicker(preferenceItemKey, repoKey, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    ...
                    let repoKey = getRepository(preferenceObj);
                    let enumKey = getRepository(preferenceObj) + "." + preferenceObj.name;
                    let spinbox = createSpinBox(repoKey, preferenceItemKey, stepValue, "Int", nbDecimal, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    ...
                    let repoKey = getRepository(preferenceObj);
                    let enumKey = getRepository(preferenceObj) + "." + preferenceObj.name;
                    let spinbox = createSpinBox(repoKey, preferenceItemKey, stepValue, "decimal", nbDecimal, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    spinbox.inject(element);
                    LockButton = createItemLockButton(spinbox, lockValue, enumKey, isAdminView);
                    infoIcon = createInfoIcon(preferenceItemKey, nlsJSON);
                    ...
                    let repoKey = getRepository(preferenceObj);
                    let enumKey = getRepository(preferenceObj) + "." + preferenceObj.name;
                    let lineEditor = createLineEditor(preferenceItemKey, repoKey, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    ...
                    let repoKey = getRepository(preferenceObj);
                    let enumKey = getRepository(preferenceObj) + "." + preferenceObj.name;
                    let spinbox = createSpinBox(repoKey, preferenceItemKey, stepValue, "percent", needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    spinbox.inject(element);
                    LockButton = createItemLockButton(spinbox, lockValue, enumKey, isAdminView);
                    infoIcon = createInfoIcon(preferenceItemKey, nlsJSON);
                    ...
                    if (preferenceObj.sortmethod && (preferenceObj.sortmethod.toLowerCase() === 'atoz' || preferenceObj.sortmethod.toLowerCase() === 'ztoa' || preferenceObj.sortmethod.toLowerCase() === 'desc' || preferenceObj.sortmethod.toLowerCase() === 'aesc'))
                        sortmethod = preferenceObj.sortmethod;
                    let comboBox = createImageComboBox(preferenceItemKey.split(".")[0], enumKey, iconlist, sortmethod, needRefresh);
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", enumKey.split(".")[0], enumKey.split(".")[1], "lockState");
                    element = createDiv("me-preference-control-right ");
                    labelOnRightFlag = false;
                    comboBox.inject(element);
                    ...
                    let lockValue = mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", repoKey, preferenceItemKey.split(".").pop(), "lockState");
                    let dialogButton = createCustomControlButton(preferenceItemKey, repoKey, preferenceObj, lockValue, needRefresh);
            ...
            function CreateGroupContent(groupKey, childrenObj) {
                ...
                if (childrenObj.tweakertype.toLowerCase() === "expander") {
                    ...
                    if (childrenObj["expanded"] && childrenObj["expanded"] == "false")
                        groupexpander.getExpanderFromIndex(0).expandedFlag = false;
                    else
                        groupexpander.getExpanderFromIndex(0).expandedFlag = true;

                    groupexpander.getExpanderFromIndex(0).getContent().setAttribute('data-mepreferences-po-use-only', groupKey);
                    groupexpander.getExpanderFromIndex(0).getContent().setStyle("background-color", "white");
            ...
            async function CreatePageContent(pageContentArray) {
                ...
                for (let sectionCnt = 0; sectionCnt < numberOfSections; sectionCnt++) {
                    ...
                    sectionKey = sectionObj.appID + "." + sectionObj.name;
        ...
        mePrefernceUIBuilder.prototype.createSections = function (UIJson, nlsJSON) {
            ...
            for (let sectionCnt = 0; sectionCnt < numberOfSections; sectionCnt++) {
                ...
                let sectionObj = UIJson.UIModel[sectionCnt];
                let sectionKey = sectionObj.appID + "." + sectionObj.name;
            ...
            for (let sectionCnt = 0; sectionCnt < numberOfSections; sectionCnt++) {
                ...
                if (sectionObj["expanded"] && sectionObj["expanded"] == "false")
                    sectionAccordeon.getExpanderFromIndex(sectionCnt).expandedFlag = false;
                else
                    sectionAccordeon.getExpanderFromIndex(sectionCnt).expandedFlag = true;
                sectionAccordeon.getExpanderFromIndex(sectionCnt).getContent().setAttribute('data-mepreferences-po-use-only', sectionKey);
            }
