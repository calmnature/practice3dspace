    ...
    function (
        ...
        mePreferencesTableBuilder.prototype.createTableDGV = function (headerName, headericons, tablemodel, tableKey, nlsJSON) {
                ...
                for (let i = 1; i < headerName.length; i++) {
                    ...
                    columns[i]["setCellValue"] = function (cellInfos, value) {
                        if (cellInfos.nodeModel) {
                            ...
                            let identifier = cellInfos.nodeModel._options.appID + "." + cellInfos.nodeModel.getAttributeValue("column" + cellInfos.columnID + "repository") + "." + cellInfos.nodeModel.getAttributeValue("column" + cellInfos.columnID + "Name");
                            let type = cellInfos.nodeModel.getAttributeValue("coltype" + cellInfos.columnID);
                            var selectedvalue = value;
                            let needRefresh = cellInfos.nodeModel.getAttributeValue("column" + cellInfos.columnID + "needRefresh");
                    ...
                    columns[i]["getCellClassName"] = function (cellInfos) {
                        if (cellInfos.nodeModel) {
                            let appID = cellInfos.nodeModel._options.appID;
                            ...
                            var className = "data-me-preferences-dgv-po-class-" + appID + "." + repoName + "." + prefName;
                            return className;
                        }
            ...
            let myDataGridView = new DataGridView({
                identifier: "MePrefDataGridViewPOUseOnly",
                columns: col,
        ...
        mePreferencesTableBuilder.prototype.getNodeData = function (preferenceItemKey, Obj, mepUIDataUtilsObj) {
            ...
            var createNode = function (preferenceItemKey, rowdata) {
                ...
                let gridObject = {};
                let SemanticsObject = {};
                let appID = preferenceItemKey.split(".")[0];
                ...
                while (i < rowdata.length) {
                    ...
                    let needRefresh = columnID + "needRefresh";
                    
                    gridObject[columnName] = rowdata[i];
                    gridObject[columnName] = rowdata[i];
                    gridObject[columnType] = rowdata[i + 1];
                    if (gridObject[columnType] === "enum")
                        gridObject[columnValue] = getValueFromNLS(appID + "." + rowdata[i + 3] + "." + rowdata[i] + "." + mepUIDataUtilsObj.getDataFromJsonResponse("preferenceValues", rowdata[i + 3], rowdata[i], "value") + ".Label", mepUIDataUtilsObj.nlsJSON);
                    ...
                    gridObject[needRefresh] = rowdata[i + 4];
            ...
            for (let i = 0; i < Obj.CellData.length; i++) {
                ...
                rowdata.push(Obj.CellData[i].needrefresh);
                ...
                if (Obj.CellData[i].tweakertype === "enum") {
                    ...
                    let enumKey = preferenceItemKey.split(".")[0] + '.' + getRepository(Obj.CellData[i]) + '.' + Obj.CellData[i].name;
        ...
        mePreferencesTableBuilder.prototype.getEnumMap = function () {
            return enumMap;
        }
