/**
 * @name DS/StudioUIActorModelWeb/extensions/CATE3DWebViewer3DGeoVisu
 ...
 */
define("DS/StudioUIActorModelWeb/extensions/CATE3DWebViewer3DGeoVisu", [
  "UWA/Core",
  "UWA/Class/Listener",
  ...
  let CATE3DWebViewer3DGeoVisu = CATE3DActor3DGeoVisu.extend(
    Listener,
    /** @lends DS/StudioUIActorModelWeb/extensions/CATE3DWebViewer3DGeoVisu.prototype **/
    {
      init: function () {
		this._parent();
      ...
      _Fill: function (iNode3D) {
        ...
        refreshPropertiesPromise.then(() => {
          ...
          let eventsToListen = [
            ...
            "height.CHANGED",
            "normalImage.CHANGED",
            "disabledImage.CHANGED",
            "hoveredImage.CHANGED",
            "pressedImage.CHANGED",
          ];

          for (const event of eventsToListen) {
            this.listenTo(expObject, event, function () {
              self.frameVisuChanges.push(self._refreshViewerFromModel);
      ...
      _RefreshVisibility: function () {
        ...
        this.frameVisuChanges.push(this._refreshViewerFromModel);
      ...
      _createButton: function () {
        ...
        bsphere.radius = Math.sqrt(
          Math.pow(this._width * 0.5, 2) + Math.pow(this._height * 0.5, 2),
        );
        let bbox = button.getBoundingBox();
        button.forceBoundingElements(
          true,
      ...
      computePow: function () {
        ...
        if (this._textureImage && this._textureSize) {
          maxSize = Math.min(
            maxSize,
            this._textureSize.w * this._textureSize.h,
          );
        }
        ...
        return (
          Math.log(maxSize / (this._height * this._width)) / (2 * Math.log(2))
        );
      },
      ...
      _refreshViewerFromModel: function () {
        let expObject = this.QueryInterface("CATI3DExperienceObject");
        if (!expObject) return false;
        let refreshPropertiesPromise = this._refreshProperties(expObject);
  ...
  return CATE3DWebViewer3DGeoVisu;
});
