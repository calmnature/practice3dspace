...
define('DS/RDFVocabsImport/model/Vocabulary', [
  ...
  Vocabulary = Model.extend(Debuggable, {
    ...
    /**
     * E65 (13-01-25)
     * This function download the contents of the given vocabulary, as an RDF\XML file.
     * @param {*} options 
     * @returns 
     */
    download: function (options) {
      ...
      //E65: As seen with SDM, Import+Delete+Export should not connect to tagsURL, since users should have access to 3DSpace to perform them/
      // if (tagsURL !== '' && tagsURL !== SpaceURL) {
      var url =
        // this.collection.Tagsurl() +
        SpaceURL +
        '/DownloadVocabulary?' +
        UWAUtils.toQueryString(this.pick(this.idAttribute)) + //the name of the vocabulary
        '&' +
        UWAUtils.toQueryString({
          tenant: this.collection.collabSpaceId
        });
      ...
      WAFData.authenticatedRequest(csrf.url, csrf);
      // }
      return this._parent(Core.merge({ operation: 'download' }, options));
    },
...
define('DS/RDFVocabsImport/model/Vocabularies', [
  ...
  'DS/WAFData/WAFData'
], function(Utils, PlatformAPI, Vocabulary, Collection, Debuggable, Core, UWAUtils, Settings, SixwDictionaryAccessAPI, WAFData) {
  var Vocabularies;
  ...
  Vocabularies = Collection.extend(Debuggable, {
    ...
    url: function() {
      ...
      return this.baseUrl + '/resources/6WVocab2/access';
    },
    Tagsurl: function() {
      if (!this.base6WTagsUrl) {
        return this.baseUrl + '/resources/6WVocab2/access';
      }
      return this.base6WTagsUrl + '/resources/6WVocab2/access';
    },
    getTagUrl: function() {
      if (!this.base6WTagsUrl) {
        return this.baseUrl;
    ...
    sync: function(method, model, options) {
      ...
      SixwDictionaryAccessAPI.get6wVocabularies(callbackObj);
    }
  });
...
define(
	...
	function (NLSKeys, i18n, View, Debuggable, Button, SuperModal, Spinner, Alert) {
		...
		//E65 (23-12-24): support exporting vocabularies
		vocabHTML = '<td>{name}</td><td>{description}</td>'
				+ '<td style="display: flex; column-gap: 10px;">'
				+ '<button type="button" class="download btn btn-info btn-xs">'
				+ '<span class="fonticon fonticon-download"></span>{0}</button>'
				+ '<button type="button" class="delete btn btn-warning btn-xs">'
				+ '<span class="fonticon fonticon-trash"></span>{1}</button>'
				+ '</td>';
		...
		vocabHTML = vocabHTML.format(NLSKeys.download_btn_label, NLSKeys.delete_btn_label);//.format(NLSKeys.delete_btn_label);
			...
			.extend(
				Debuggable,
				{
					...
					domEvents: {
						...
						'click button.download': '_onDownload' //E65 (23-12-24): support exporting vocabularies
					},
					...
					render: function () {
						...
						if (that.model.get('nlsName') === undefined) {
							...
							vocabHTML = vocabHTML
								.format(NLSKeys.download_btn_label, NLSKeys.delete_btn_label);
								// vocabHTML = '<td>{name}</td><td>{description}</td>'
								// + '<td><button type="button" class="delete btn btn-warning btn-xs">'
								// + '<span class="fonticon fonticon-trash"></span>{0}</button></td>';
							// vocabHTML = vocabHTML
					...
					_onDownload: function () {
						let vocabName, that = this;

						that.options.appView.mask(NLSKeys.download_vocab_mask);
						that.model.download({
							onComplete: function (fileContents) {
								that.options.appView.unmask();
								// let's cache the vocab name:
								vocabName = that.model.get('name');

								let link = UWA.createElement('a');
								let fileBlob = new Blob([fileContents], { type: 'text' });
								let blobUrl = window.URL.createObjectURL(fileBlob);
								link.href = blobUrl;
								link.download = vocabName + '.rdf';
								link.click();							
							},
							onFailure: function () {								
								that.options.appView.unmask();
            					...
            					alert.show();
								// that.log('Failed to download vocab "{0}" from the server', vocabName);
							}, wait: true
						});
					}
				});
