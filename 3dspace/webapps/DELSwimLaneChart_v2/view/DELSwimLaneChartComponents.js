...
define("DS/DELSwimLaneChart_v2/view/DELSwimLaneChartComponents", ["require", "exports", "DS/DELSwimLaneChart_v2/utils/StaticAttributes", "DS/DELSwimLaneChart_v2/utils/TextUtils"], function (require, exports, StaticAttributes_1, TextUtils_1) {
  ...
  <div id="root__${viewId}" style="width:100%;height:100%;overflow:hidden;position:relative"> <div class="parentContainer" style="width:100%;max-height:100vh;height:100%;overflow:auto" id="parent__${viewId}" >
          ...
          <div class="pxpsandboxcontainer"></div>
          `;
    };
    ...
    const nodeTemplate = (node) => {
        ...
        if (node.content) {
            ...
            if (node.content.fragmentElement) {
                let fragmentElement = document.getElementById("fragmentElementParent__" + node.id);
                if (!fragmentElement) {
                    fragmentElement = document.createElementNS(StaticAttributes_1.nameSpace, 'foreignObject');
                    fragmentElement.setAttribute('id', "fragmentElementParent__" + node.id);
                    fragmentElement.setAttribute('class', "foreignNode");
                    fragmentElement.innerHTML = `
        <div id="fragmentElement__${node.id}" xmlns="http://www.w3.org/1999/xhtml" class="fragmentElement" 
        ></div>
        `;
                    nodeGroupElement.appendChild(fragmentElement);
                }
                fragmentElement.setAttribute('height', `${Math.abs(contentBodyHeight)}`);
                ...
                const fragmentElement = document.getElementById("fragmentElementParent__" + node.id);
                if (fragmentElement)
                    nodeGroupElement.removeChild(fragmentElement);
            }
            if (node.content.childElement) {
                // update the text color of the child element
                if (node.content.textColor)
                    node.content.childElement.style.color = node.content.textColor;
  ...
  `);
            ...
            // node header with a child html Element
            if (node.header.childElement) {
                // update the text color of the child element
                if (node.header.textColor)
                    node.header.childElement.style.color = node.header.textColor;
                let headerContentBody = document.getElementById("headerForeignNode__" + node.id);
                if (!headerContentBody) {
                    headerContentBody = document.createElementNS(StaticAttributes_1.nameSpace, 'foreignObject');
                    headerContentBody.setAttribute('id', "headerForeignNode__" + node.id);
                    headerContentBody.setAttribute('class', "foreignNode");
                    headerGroup.appendChild(headerContentBody);
                }
                if (!node.header.childElement.classList.contains("childElement"))
                    node.header.childElement.classList.add("childElement");
                headerContentBody.setAttribute('height', `${headerHeight}`);
                headerContentBody.setAttribute('width', `${headerTitleWidth}`);
                headerContentBody.setAttribute('x', `${headerTitleLeft}`);
                headerContentBody.setAttribute('y', `${headerTitleTop}`);
                headerContentBody.setAttribute("visibility", !node.isHidden ? "visible" : "hidden");
                headerContentBody.setAttribute("display", !node.isHidden ? "block" : "none");
                if (node.header.highlightedchildElement)
                    node.header.highlightedchildElement.style.fontSize = ((_r = node.header.fontSize) !== null && _r !== void 0 ? _r : 12) + "px";
                else
                    node.header.childElement.style.fontSize = ((_s = node.header.fontSize) !== null && _s !== void 0 ? _s : 12) + "px";
                const headerChildElement = document.getElementById("headerChildElement__" + node.id);
                if (!headerChildElement) {
                    // handle the image nodes inside the child element
                    const listOfImgElts = node.header.childElement.getElementsByTagName("img");
                    for (let i = 0; i < listOfImgElts.length; i++) {
                        const imgElt = listOfImgElts[i];
                        const imgSrc = imgElt.src;
                        if (imgSrc.includes("https://")) {
                    ...
                    if (node.header.highlightedchildElement) {
                        node.header.highlightedchildElement.setAttribute('id', "headerChildElement__" + node.id);
                        headerContentBody.innerHTML = "";
                        headerContentBody.appendChild(node.header.highlightedchildElement);
                    }
                    else {
                        node.header.childElement.setAttribute('id', "headerChildElement__" + node.id);
                        headerContentBody.innerHTML = "";
                        headerContentBody.appendChild(node.header.childElement);
                    }
                }
                else {
                    if (node.header.highlightedchildElement)
                        headerChildElement.replaceWith(node.header.highlightedchildElement);
                    else
                        headerChildElement.replaceWith(node.header.childElement);
                }
            }
            else {
                const headerContentBody = document.getElementById("headerForeignNode__" + node.id);
                if (headerContentBody)
                    headerGroup.removeChild(headerContentBody);
            }
            if (node.header.text && !node.header.childElement) {
                ...
                headerTitle.innerHTML = `<span id=${"nodeHeaderResponsiveText__" + node.id} class="responsiveText"  style="font-size:${(_t = node.header.fontSize) !== null && _t !== void 0 ? _t : 12}px;height:${StaticAttributes_1.header_height}px;color:${(_u = node.header.textColor) !== null && _u !== void 0 ? _u : "black"};">${(0, TextUtils_1.encodeTextInput)(node.header.text)}</span>`;
