...
define("DS/DELSwimLaneChart_v2/model/DELSwimLaneChartModel", ["require", "exports", "DS/DELSwimLaneChart_v2/model/SwimLaneChartColumn", "DS/DELSwimLaneChart_v2/model/SwimLaneChartNode", "DS/DELSwimLaneChart_v2/model/SwimLaneChartGroup", "DS/DELSwimLaneChart_v2/utils/StaticAttributes", "DS/DELSwimLaneChart_v2/utils/TextUtils"], function (require, exports, SwimLaneChartColumn_1, SwimLaneChartNode_1, SwimLaneChartGroup_1, StaticAttributes_1, TextUtils_1) {
    ...
    class SwimLaneChartModel {
        ...
        addNodesToColumn(colId, nodes, groups = []) {
            const childElements = [];
            const collapsedNodesIds = [];
            const fakeCollapsedNodesIds = [];
            nodes.forEach((n, index) => {
                ...
                if (!this._nodes.has(n.id)) {
                    ...
                    this._nodes.set(n.id, {
                        node: new SwimLaneChartNode_1.default(n.id, "", { x: 0, y: 0 }, 0, (_a = n.height) !== null && _a !== void 0 ? _a : 0, n.header, n.content, n.isCollapsible, n.isCollapsed, n.isPinnable),
                    ...
                    if (n.isCollapsed) {
                        if (!n.parentId)
                            collapsedNodesIds.push(n.id);
                        fakeCollapsedNodesIds.push(n.id);
                    }
                }
            });
            for (let i = 0; i < childElements.length; i++) {
                ...
                if (n.parentId) {
                    ...
                    // collapsed side-effects
                    if (n.isCollapsed && !fakeCollapsedNodesIds.includes(n.parentId))
                        collapsedNodesIds.push(n.id);
                }
            }
            // collapsedNodes
            for (let i = 0; i < collapsedNodesIds.length; i++) {
                this.collapseSwimLaneChartNode(collapsedNodesIds[i]);
            }
        }
        ...
        collapseSwimLaneChartNode(id, recursiveFlag = false) {
            ...
            if (!elt || !elt.node.isCollapsible)
                return elt;
            const nodehasHeader = elt.node.header; // && elt.node.height >= elt.node.header.height (when added for the first time: elt.node.height==0)
            if (nodehasHeader)
                elt.node.isCollapsed = true;
            if (elt.node.type === "CompositeNode") {
                const children = [...this.nodes.values()].filter((n) => n.node.parentId === id);
                children.forEach((child) => {
                    if (recursiveFlag || !nodehasHeader)
        ...
        hideSwimLaneChartNode(id, shouldHideInnerGroup = true) {
            ...
            if (id === "8")
                console.log("hide node", elt.node, elt.node.isHidden);
        ...
        computeNodeHeightFromHtmlContent(node, width) {
            ...
            let headerHeight = 0;
            if (node.header) {
                const headerLeftIconLeft = ((_a = node.header) === null || _a === void 0 ? void 0 : _a.collapsibleIcon) ? StaticAttributes_1.imgSize + StaticAttributes_1.margin : StaticAttributes_1.margin;
                const headerTitleLeft = ((_b = node.header) === null || _b === void 0 ? void 0 : _b.leftIcon) ? StaticAttributes_1.imgSize + StaticAttributes_1.margin + headerLeftIconLeft : headerLeftIconLeft;
                const headerWidth = ((_c = node.header) === null || _c === void 0 ? void 0 : _c.rightIcon) ? Math.max(node.width - headerTitleLeft - StaticAttributes_1.imgSize - 2 * StaticAttributes_1.margin, 2 * StaticAttributes_1.margin) : Math.max(node.width - headerTitleLeft - StaticAttributes_1.margin, 2 * StaticAttributes_1.margin);
                if (node.header.childElement && node.header.textWrap === "wrap") {
                    headerHeight = Math.max((0, TextUtils_1.computeHtmlContentHeight)(node.header.childElement, headerWidth), StaticAttributes_1.header_height);
                    node.header = { ...node.header, height: headerHeight }; // move elsewhere
                }
                else if (!node.header.childElement && node.header.text && node.header.textWrap === "wrap") {
                    const headerSpan = document.createElement("span");
                    headerSpan.classList.add("spanText");
                    headerSpan.style.fontFamily = "Arial";
                    headerSpan.style.fontSize = "12px";
                    headerSpan.innerHTML = ((_e = (_d = node.header.text) === null || _d === void 0 ? void 0 : _d.trim()) !== null && _e !== void 0 ? _e : "");
