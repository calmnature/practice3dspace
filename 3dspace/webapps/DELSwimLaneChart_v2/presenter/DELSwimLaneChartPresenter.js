...
define("DS/DELSwimLaneChart_v2/presenter/DELSwimLaneChartPresenter", ["require", "exports", "DS/DELSwimLaneChart_v2/utils/StaticAttributes", "DS/DELSwimLaneChart_v2/utils/LayoutUtils", "DS/DELSwimLaneChart_v2/services/presenterServices/LayoutManager", "DS/DELSwimLaneChart_v2/model/SwimLaneChartColumn"], function (require, exports, StaticAttributes_1, LayoutUtils_1, LayoutManager_1, SwimLaneChartColumn_1) {
    ...
    class SwimLaneChartPresenter {
        constructor(model, view, props) {
            // fragment elements height observer
            this._heightResizeObserver = new ResizeObserver(entries => {
                requestAnimationFrame(() => {
                    ...
                    for (const entry of entries) {
                        ...
                        const id = elt.id.split("__")[1];
                        const nodeElt = this._swimLaneChartModel.nodes.get(id);
                        if (!nodeElt)
                            return;
                        const { width, height } = entry.contentRect;
                        const heightPlus = (_b = (_a = nodeElt.node.header) === null || _a === void 0 ? void 0 : _a.height) !== null && _b !== void 0 ? _b : 0;
                        this._swimLaneChartModel.updateSwimLaneChartNode({ id: id, height: height + heightPlus + 2 * StaticAttributes_1.margin });
                    }
                    this.updateHeight(); // recompute-the-layout-called-once
                });
            });
            // attributes
        ...
        */
        ...
        handleDblClick(e) {
            ...
            //collapse/minimize
            const headerElts = ["nodeHeaderResponsiveText", "nodeHeader", "nodeHeaderText", "headerLeftIcon", "headerRightIcon", "headerChildElement", "headerForeignNode"];
            const collapseExpandCondition = headerElts.includes(type) || headerElts.includes(parenttype);
            if (collapseExpandCondition) {
                if (targetElt.isCollapsed)
                    this.expand(nodeId);
                else
                    this.collapse(nodeId);
        ...
        handlePointerOver(e) {
            ...
            if (!targetElt || targetElt.highlighted)
                return;
            if (!["node", "childElement", "nodeHeaderGroup", "headerChildElement", "headerForeignNode", "foreignNode", "bodyTextContent", "nodeHeaderText"].includes(type))
                return;
        ...
        /**
         * Method to get all the swimlanechart groups
         * @returns list of swimlane groups
         */
        getAllGroups() {
            return this._swimLaneChartModel.listOfGroups;
        }
        /**
        ...
        collapseAll(id) {
            if (id) {
                ...
                this._swimLaneChartModel.getSwimLaneChartNodes().forEach((n) => {
                    const shouldBeCollapsed = !n.isCollapsed && !n.isHidden && !n.parentId; //
        ...
        */
        ...
        /**
         * Method to watch the fragment element height changes
         * @param id
         */
        observeFragmentElement(id) {
            var _a;
            const fragmentElement = (_a = this._swimLaneChartView.getParentContainer()) === null || _a === void 0 ? void 0 : _a.querySelector('#fragmentElement__' + id);
            if (!fragmentElement)
                return;
            // Firefox may trigger the observer before the element is fully rendered !
            if (StaticAttributes_1.isFireFox) {
                const interval = setInterval(() => {
                    if (fragmentElement.children.length > 0) {
                        clearInterval(interval);
                        this._heightResizeObserver.observe(fragmentElement);
                    }
                }, 50);
            }
            else
        ...
        /**
        * Method to unwatch the fragment element height changes
        * @param id
        */
        unObserveFragmentElement(id) {
            var _a;
            const fragmentElement = (_a = this._swimLaneChartView.getParentContainer()) === null || _a === void 0 ? void 0 : _a.querySelector('#fragmentElement__' + id);
            if (!fragmentElement)
                return;
            this._heightResizeObserver.unobserve(fragmentElement);
        }
    }
