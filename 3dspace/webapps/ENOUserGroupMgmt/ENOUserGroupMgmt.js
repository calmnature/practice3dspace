...
/* global define, widget */
/**
 * @overview User Groups Management - persistency utilities
 * @licence Copyright 2006-2020 Dassault Systemes company. All rights reserved.
 * @version 1.0.
 * @access private
 */
define('DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil', [
	...
	function() {
		...
		let getViewPersistency = function(fromPage) {
			...
			if(fromPage == "UserGroupSummary")
				persistedView = widget.getValue('UserGroupSummary-ShowView');
			else if(fromPage == "MembersTab")
				persistedView = widget.getValue('MembersTab-ShowView');
			else if(fromPage == "DetailsMembersTab")
				persistedView = widget.getValue('groupDetailsViewer-MembersTab-ShowView');
			else if(fromPage == "Access Rights")
				persistedView = widget.getValue('Access Rights-ShowView');

			if (persistedView === undefined || persistedView === "")
				persistedView = "TileView"; //default view (Tile View) is set
				
			return persistedView;			
		}
		
		let getHomePanelPersistency = function() {
			let homePanelState = widget.getValue('UserGroupHomePanelPersistedState');
			
			if (homePanelState === undefined || homePanelState === "")
				homePanelState = "Collapsed"; //default state (Collapsed) will be set

			return homePanelState;
		}

		let getIDCardPersistency = function() {
			let idCardState = widget.getValue('UserGroupIDCardPersistedState');
		...
		let getTabPersistency = function(fromPage) {
			...
			if(fromPage == "UserGroup")
				activeTab = widget.getValue('UserGroupTabToPersist');
			else if(fromPage == "GroupDetailsViewer")
				activeTab = widget.getValue('GroupDetailsViewerTabToPersist');
		...
		let getSummaryFilterPersistency = function(filterType) {
			let summaryFilter = "";
			
			if(filterType == "resp")
				summaryFilter = widget.getValue("respFilterToPersist");
			else if(filterType == "visibility")
				summaryFilter = widget.getValue("visibilityFilterToPersist");
			
			if(summaryFilter === undefined) //default filter is all
				summaryFilter = "all";
			
			return summaryFilter;
		}
		
		let UserGroupPersistencyUtil = {
			getViewPersistency: (fromPage) => { return getViewPersistency(fromPage); },
			getHomePanelPersistency: () => { return getHomePanelPersistency(); },
			...
			getSummaryFilterPersistency: (filterType) => { return getSummaryFilterPersistency(filterType); }
		};

		return UserGroupPersistencyUtil;
	});
...
define('DS/ENOUserGroupMgmt/Utilities/HelpUtil',
	[
	...
	function _addToolbarBtnTooltip(homeToolbar, btnId, resourceName) {
		...
		tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', resourceName);
	...
	function getTooltip(command) {
		var tooltip = new WUXTooltipModel();
		tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', command);
		return tooltip;
	};

	let HelpUtil = {
		_addToolbarBtnTooltip: (homeToolbar, btnId, resourceName) => { return _addToolbarBtnTooltip(homeToolbar, btnId, resourceName) },
		getTooltip: (command) => {return getTooltip(command)}
	}
	return HelpUtil;
...
define('DS/ENOUserGroupMgmt/Components/Wrappers/WrapperDataGridView',
	...
	function(DataGridView, CollectionViewStatusBar, DataGridViewLayout, WUXFind, UserGroupBootstrap, Utils) {
		...
		let initDataGridView = function(treeDocument, colConfig, toolBar, dummydiv, datagridID) {
			//buildLayout();
		...
		WrapperDataGridView = {
			build: (treeDocument, colConfig, toolBar, dummydiv, datagridID) => { return initDataGridView(treeDocument, colConfig, toolBar, dummydiv, datagridID); },
	...
	let registerEvents = function(){
		if(widget.ugMgmtMediator){
			...
			widget.ugMgmtMediator.subscribe('usergroup-back-to-summary', function (data) {
				...
				widget.setValue("userGroupIDToPersist", undefined);      	  
	        });
...
define('DS/ENOUserGroupMgmt/Config/Toolbar/UsersGroupMembersTabToolbarConfig',
  ['DS/ENOUserGroupMgmt/Utilities/RequestUtils',
	  'DS/ENOUserGroupMgmt/Model/UsersGroupModel',
	  'DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil',
	  'DS/CoreBase/WebUXGlobalEnums',
   'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'],
  function (RequestUtils, UsersGroupModel, UserGroupPersistencyUtil, WebUXGlobalEnums, NLS) {
	...
	let writetoolbarDefination = function (model) {
		var tileViewIconState;
		let groupDetailsViewer = document.getElementById("GroupDetailsViewer");
		if(groupDetailsViewer)
			tileViewIconState = UserGroupPersistencyUtil.isPersistedViewSelected("DetailsMembersTab", "TileView");
		else
			tileViewIconState = UserGroupPersistencyUtil.isPersistedViewSelected("MembersTab", "TileView");
		var gridViewIconState = tileViewIconState === "selected" ? "unselected" : "selected";
		...
		if(RequestUtils.isAdmin || 
				...
				entries.push({
					...
					"position": "far",
					"category": "create",
					"tooltip": {title: NLS.addExistingMembers}
				});
		}
		...
		entries.push({
	        ...
	        "position": "far",
	        "tooltip": {title: NLS.manageSelection},
	        "category": "action"
	    });  
	    } 
        ...
        entries.push({
            ...
            "dataElements": {
            	...
            	"icon": {
            		"iconName": tileViewIconState == "selected" ? "view-small-tile" : "view-list",
            		"fontIconFamily": 1
            	},                
            	"value":_viewData
            },
            "position": "far",
            "tooltip": tileViewIconState == "selected" ? {title: NLS.tileView} : {title: NLS.gridView},
            "category": "action" 
        });
        if(!widget.readOnlyUG) {
        if(isAdmin ||  
	        ...
	        entries.push({
	        	...
	        	"position": "far",
	        	"category": "action",
	        	"tooltip": {title: NLS.removeMember }
	        });
        }
        }
...
['DS/ENOXSplitView/js/ENOXSplitView','DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil','i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'],
function(ENOXSplitView,UserGroupPersistencyUtil,NLS) {
  ...
  ENOXSplitViewWrapper.prototype.addRightPanelExpander = function () {
    if (this._rightPanel != null) {
	  ...
	  let toolTip = NLS.homeRightPanelExpand;
	  var showIcon = "fonticon-expand-left";
	  let groupHomePanelPersistency = UserGroupPersistencyUtil.getHomePanelPersistency();
	  if(groupHomePanelPersistency == "Expanded") {
		showIcon = "fonticon-expand-right";
		toolTip = NLS.homeRightPanelCollapse;
	  }
      ...
      closer.onclick = function (e) {
        ...
        if (expandLeft) {
		  ...
		  widget.setValue('UserGroupHomePanelPersistedState', "Expanded");
        } else {
		  ...
		  widget.setValue('UserGroupHomePanelPersistedState', "Collapsed");
        }
      }
    }
  }
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupDetailsTabsView', [
  ...
  function (WUXTabBar,UsersGroupTabsConfig,UserGroupPersistencyUtil) {
	'use strict';
	...
	let ontabClick = function(args){
		...
		widget.setValue('GroupDetailsViewerTabToPersist',_currentTabIndex);		
	};
	...
	let build = function(container,usergroupInfoModel){
		...
		UsersGroupTabsConfig.forEach((tab) => {
			if(tab.id == "accessRights")
					return;
			//isSelected to be set based on groupDetailsTabPersisted value
			let groupDetailsTabPersisted = UserGroupPersistencyUtil.getTabPersistency("GroupDetailsViewer");
			if(tab.id == ntabs[groupDetailsTabPersisted])
				tab.isSelected = true;
			else
	...
	let initializeUserGroupTabs = function(){		
		new Promise(function (resolve, reject){
			...
			args.options.value = groupDetailsTabPersisted;
			ontabClick(args);
...
define('DS/ENOUserGroupMgmt/Views/Tile/UsersGroupDetailMembersTileView',
    ...
    let build = function(model){
		...
		let containerClass = 'GroupDetail-tileView-View showView nonVisible';
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("DetailsMembersTab");
		if(persistedView == "GridView")
			containerClass = 'GroupDetail-tileView-View hideView';
...
define('DS/ENOUserGroupMgmt/Config/Toolbar/UserGroupAccessRightsToolbarConfig',
		...
		'DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil',
   'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'],
  function (RequestUtils, UsersGroupModel, UserGroupPersistencyUtil, NLS) {
    let UGMemberTabToolbarConfig, 
    _viewData =  {
            menu:[
                {
	...
	let writetoolbarDefination = function (model) {
		var tileViewIconState = UserGroupPersistencyUtil.isPersistedViewSelected("Access Rights", "TileView");
		var gridViewIconState = tileViewIconState === "selected" ? "unselected" : "selected";
		_viewData.menu[0].state = gridViewIconState;
		_viewData.menu[1].state = tileViewIconState;   	
		...
		if( RequestUtils.isAdmin ||
				...
				entries.push({
					...
					"tooltip":{title: NLS.addAccessRight},
					"category": "addRemove"
				  });
				...
				entries.push({
					...
					"tooltip":{title: NLS.RevokeAccess},
					"category": "addRemove"
				  });
		}
...
define('DS/ENOUserGroupMgmt/Config/Toolbar/UsersGroupSummaryToolbarConfig',
  [ 	'DS/ENOUserGroupMgmt/Utilities/RequestUtils',
		...
		'DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil',
	  'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'],
  function (RequestUtils, WebUXGlobalEnums, UserGroupPersistencyUtil, NLS) {
	...
	var tileViewIconState = UserGroupPersistencyUtil.isPersistedViewSelected("UserGroupSummary", "TileView");
	var gridViewIconState = tileViewIconState === "selected" ? "unselected" : "selected";
	_viewData.menu[0].state = gridViewIconState;
	_viewData.menu[1].state = tileViewIconState;
    let selectMenu = getSelectMenu(model)
    ...
    if(RequestUtils.isAdmin || RequestUtils.isAuthor || RequestUtils.isLeader ) {
    	entries.push({
            ...
            "position": "far",
            "category": "create",
            "tooltip": {title: NLS.newUserGroup,
            			shortHelp: 'creates new user group',
            			//longHelp: 'long help..'
            			}
          });
        ...
        entries.push({
            ...
            "position": "far",
            "category": "create",
            "tooltip": {title: NLS.toolbarAddMembers}
          });
        ...
        entries.push({
            ...
            "position": "far",
            "category": "create",
            "tooltip": {title: NLS.toolbarRemoveMembers}
          });
    ...
    //if(isAdmin || filterPreference=="owned"){
    	 entries.push({
    	        ...
    	        "position": "far",
    	        "category": "create",
    	        "tooltip": {title: NLS.deleteSelectedGroups }
    	      });	
    //}
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupTabsView', [
  ...
  function (WUXTabBar,UserGroupTabsConfig,UserGroupPersistencyUtil) {
	'use strict';
    ...
    let ontabClick = function(args){
		...
		_currentTabIndex = seltab;
		widget.setValue('UserGroupTabToPersist',_currentTabIndex);		
	};
	...
	UserGroupTabsView.prototype.init = function(){		
		...
		UserGroupTabsConfig.forEach((tab) => {
			//isSelected to be set based on groupTabPersisted value
			let groupTabPersisted = UserGroupPersistencyUtil.getTabPersistency("UserGroup");
			if(tab.id == ntabs[groupTabPersisted])
				tab.isSelected = true;
			else
				tab.isSelected = false;
		    _usergroupTabs.add(tab); //adding the tabs
	...
	let initializeUserGroupTabs = function(){		
		new Promise(function (resolve, reject){
			...
			let groupTabPersisted = UserGroupPersistencyUtil.getTabPersistency("UserGroup");
			//Passing value only loads tab content; for tab selection isSelected must be set as true
			args.options.value = groupTabPersisted;
			ontabClick(args);
			//event to be called when clicked on any tab
			_usergroupTabs.addEventListener('tabButtonClick', function(args){
				ontabClick(args);
...
define('DS/ENOUserGroupMgmt/Views/Grid/UsersGroupSummaryDataGridView',
	...
	let build = function(model){	
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("UserGroupSummary");
		let containerClass = 'data-grid-container hideView';
			if(persistedView && persistedView == "GridView")
				containerClass = 'data-grid-container showView';		
		var _container=UWA.createElement('div', {id:'dataGridViewContainer', 'class':containerClass/*,styles: {
            'width': "100%",
            'height': "calc(100% - 40px)",
            'position': "relative"
          }*/});
		...
		let dataGridViewContainer = WrapperDataGridView.build(model, DatagridViewConfig, toolbar, _container, "UsersGroupSummaryView");
...
define('DS/ENOUserGroupMgmt/Views/Menu/UsersGroupContextualMenu', [
		...
		'DS/ENOUserGroupMgmt/Utilities/RequestUtils',
		'DS/Controls/TooltipModel',
		'DS/ENOUserGroupMgmt/Utilities/HelpUtil',
		'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt',
		'css!DS/ENOUserGroupMgmt/ENOUserGroupMgmt.css' ], 
	function(WUXMenu,  UserGroupActions, UserGroupModel,RemoveUsersGroup,DuplicateUsersGroup,RequestUtils, WUXTooltipModel, HelpUtil, NLS){
		'use strict';
		let Menu;
		...
		/*let getDeleteTooltip = function() {
			var tooltip = new WUXTooltipModel();
			tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', 'DeleteContextualMenu');
			return tooltip;
		};*/
		...
		/* 	if(removeDetails.data.length === 1 && removeDetails.data[0].options.grid.DeleteAccess != "TRUE"){
			...
			if(RequestUtils.isAdmin  || (removeDetails.data[0].options.grid.owner== RequestUtils.contextUser  && (RequestUtils.isLeader || RequestUtils.isAuthor))){
				 menu.push({
		                ...
		                title: NLS.Delete,
		                type: 'PushItem',
		                tooltip: HelpUtil.getTooltip('DeleteContextualMenu')/*getDeleteTooltip()*/,
		                fonticon: {
		...
		/*let getOpenMenuTooltip = function() {
			var tooltip = new WUXTooltipModel();
			tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', 'OpenContextualMenu');
			return tooltip;
		};*/
		...
		let openMenu = function(Details){
            ...
            menu.push({
                ...
                title:NLS.Open,
                type: 'PushItem',
                tooltip: HelpUtil.getTooltip('OpenContextualMenu') /*getOpenMenuTooltip()*/,
                fonticon: {
        ...
        /*let getDuplicateMenuTooltip = function() {
			var tooltip = new WUXTooltipModel();
			tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', 'DuplicateContextualMenu');
			return tooltip;
		};*/
        ...
        let duplicateMenu = function(userGroupDetails){
            ...
            var cmd =  {
	                ...
	                title:NLS.duplicate,
	                type: 'PushItem',
	                tooltip: HelpUtil.getTooltip('DuplicateContextualMenu') /*getDuplicateMenuTooltip()*/,
	                fonticon: {
...
define('DS/ENOUserGroupMgmt/Views/Tile/UsersGroupSummaryTileView',
    ...
    let build = function(model){
		...
		let containerClass = 'tile-view-container showView nonVisible';
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("UserGroupSummary");
		if(persistedView == "GridView")
			containerClass = 'tile-view-container hideView';        
...
define('DS/ENOUserGroupMgmt/Views/UserGroupAccessRightsView',
    ...
    let drawUserGroupAccessRightsView = function(containerDiv){
	    ...
	    HelpUtil._addToolbarBtnTooltip(accessrightsTabToolbar, "addContext", 'AddResponsibilities');
	    HelpUtil._addToolbarBtnTooltip(accessrightsTabToolbar, "revokeResp", 'RevokeResponsibility');
	    HelpUtil._addToolbarBtnTooltip(accessrightsTabToolbar, "selection", 'ResponsibilitiesManageSelection');
	    HelpUtil._addToolbarBtnTooltip(accessrightsTabToolbar, "view", 'ViewResponsibilities');
	    
	    /*_addToolbarBtnTooltip(accessrightsTabToolbar, "addContext", 'AddResponsibilities');
	    _addToolbarBtnTooltip(accessrightsTabToolbar, "revokeResp", 'RevokeResponsibilities');
	    _addToolbarBtnTooltip(accessrightsTabToolbar, "selection", 'ResponsibilitiesManageSelection');
	    _addToolbarBtnTooltip(accessrightsTabToolbar, "view", 'ViewResponsibilities');*/
   ...
   /* function _addToolbarBtnTooltip(accessrightsTabToolbar, btnId, resourceName) {
		var tooltip = new WUXTooltipModel();
		tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', resourceName);
		_setToolbarBtnTooltip(accessrightsTabToolbar, btnId, tooltip);
	}

	function _setToolbarBtnTooltip(accessrightsTabToolbar, btnId, tooltip) {
		if (tooltip.isEmpty()) {
			return setTimeout(_setToolbarBtnTooltip.bind(this, accessrightsTabToolbar, btnId, tooltip), 1000);
		}
		accessrightsTabToolbar.updateNodeModel(btnId, { tooltip: tooltip });
	}*/
	...
	//Method to execute callback function to the latest event in case of multiple events of same type triggered
	...
	//Wrap the real update in a debounced function
	const updateSelectAllCount = debounce(() => {
		let respToolbar = UsersGroupAccessRightsDataGridView.getGridViewToolbar();
	    UserGroupCommands.updateSelectAllCount(
	        UserGroupAccessRightsModel.getModel(),
	        respToolbar
	    );
	});
    ...
    let registerListners = function(){
    	...
    	tileView.onSelectionAdd(updateSelectAllCount);
    	tileView.onSelectionRemove(updateSelectAllCount);  
    };
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupIDCardView', [
  ...
  function (UserGroupHeaderView, UserGroupTabsView, IdCardUtil, DataFormatter, UserGroupPersistencyUtil, UsersGroupModel) {
	'use strict';
	...
	var UserGroupIDCardWrapper = function(rightPanel){
		...
		let idCardView = UserGroupPersistencyUtil.getIDCardPersistency();
	...
	UserGroupIDCardWrapper.prototype.init = function(data){	
		destroyViews(); //to destroy any pre-existing views
		widget.setValue("userGroupIDToPersist", data.model.id);
...
function(SplitViewWrapper, UserGroupIDCardWrapper, RightPanelSplitView, UserGroupPersistencyUtil, IdCardUtil) {
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupSummaryView',
		...
		DataGridView,TreeDocument,TreeNodeModel,UsersGroupModel,UsersGroupSummaryTileView,UserGroupServicesController,UserGroupAccessRightsModel,UsersGroupMemberModel,DataFormatter,RequestUtils,SearchUtil,UserGroupPersistencyUtil,ArrayUtils,PointerEvents,AutoCompleteUtil,WUXComboBox,WUXLineEditor,
		WUXButton,WUXTooltipModel,WebUXGlobalEnums,PlaceHolder,UserGroupSpinner,CollectionViewStatusBar,UserGroupCommands,DateUtils,WebUAUtils,ImmersiveFrame,HelpUtil,/*TagNavigator,*/NLS
			) {
	...
	let build = function(){		
		...
		return new Promise(function(resolve, reject) {
			if (widget.data.contentId) { //To show single UG
				...
				let allowPrivateUG = RequestUtils.AllowPrivateUserGroup.toLowerCase();				
				let userName = RequestUtils.contextUser;				
				let respFilter = UserGroupPersistencyUtil.getSummaryFilterPersistency("resp");
				if(allowPrivateUG != "true") {
					if(respFilter == "viewer") {
						widget.setValue("respFilterToPersist",undefined);
						respFilter = UserGroupPersistencyUtil.getSummaryFilterPersistency("resp");
					}
					widget.setValue("visibilityFilterToPersist","Active");
					widget.setValue("privateUGDisabled",true);
				}
				else {
					if(widget.getValue("privateUGDisabled")) {
						widget.setValue("visibilityFilterToPersist",undefined);
						widget.setValue("privateUGDisabled",false);
					}
				}
				let visibilityFilter = UserGroupPersistencyUtil.getSummaryFilterPersistency("visibility");
				
				if((allowPrivateUG != "true" && respFilter != "all") ||
					(allowPrivateUG == "true" && (respFilter != "all" || visibilityFilter != "all"))) {
					UserGroupServicesController.filterUserGroups(userName, respFilter, visibilityFilter, "").then(
						success => {
							let containerDiv = _fetchUserGroupSuccess(success);
							resolve(containerDiv);
						},
	...
	let applyFilter = async function(userNameFilter, respFilter, visibilityFilter, titleToSearch){
		widget.setValue("respFilterToPersist",respFilter);
		widget.setValue("visibilityFilterToPersist",visibilityFilter);
		let username;
	...
	let drawFilters = function(){
		...
		_filterToolbar.filterType = new WUXComboBox({
			...
			enableSearchFlag: false,
			value: UserGroupPersistencyUtil.getSummaryFilterPersistency("resp")
		});
		...
		_filterToolbar.visibilityFilter = new WUXComboBox({
			...
			value: UserGroupPersistencyUtil.getSummaryFilterPersistency("visibility")
		});
		...
		_filterToolbar.search = new WUXButton({
		    ...
		    tooltipInfos: new WUXTooltipModel({
					        ...
					        //shortHelp: 'Search Filter shorthelp'
					      })
		});
	...
	let drawUsersGroupSummaryView = function(serverResponse){
		...
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'createUserGroup', 'NewUserGroup');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'addMembersToUserGroup', 'AddMembers');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'removeMembersFromUserGroup', 'RemoveMembers');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'deleteUserGroup', 'DeleteSelectedGroups');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'selection', 'UserGroupManageSelection');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'exportUserGroup', 'Export');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'view', 'ViewUserGroup');
		HelpUtil._addToolbarBtnTooltip(homeToolbar, 'back', 'Home');
	...
	let openSelectedUserGroup = function(id){
		...
		if(!uID)
			uID = widget.getValue("userGroupIDToPersist");
		...
		if(uID){
			...
			else if(widget.getValue("userGroupIDToPersist"))
				backToUserGroupSummary();
			else {
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupHeaderView', [
  ...
  function (WUXMenu, UserGroupContextualMenu, WebappsUtils, IdCardUtil, RequestUtils, UserGroupPersistencyUtil, LifecycleServices, UsersGroupModel, RemoveUsersGroup, UsersGroupSummaryView, OpenWith, HelpUtil, NLS) {
	'use strict';
	let userGroupIdCard;
		...
		if(readOnly) {
			...
			if(RequestUtils.contextUser == usergroupDetails.data[0].options.grid.owner || RequestUtils.isAdmin){
				...
				if(data.model.state=="Private"){
					...
					changeVisibilityDiv.tooltipInfos = HelpUtil.getTooltip('ChangeVisibility');
					changeVisibilityDiv.inject(infoHeaderSecAction);
				}
				...
				deleteGroupDiv.tooltipInfos = HelpUtil.getTooltip('DeleteSelectedGroups');
				deleteGroupDiv.inject(infoHeaderSecAction);
			}
	
			//default is maximized
			let viewClass = "wux-ui-3ds wux-ui-3ds-2x wux-ui-3ds-expand-up fonticon-display";
			let toolTip = NLS.idCardHeaderActionCollapse;
			//fetching persisted info of collapse/expand
			let groupIDCardPersistency = UserGroupPersistencyUtil.getIDCardPersistency();
			if(groupIDCardPersistency && groupIDCardPersistency == "minimized") {
				viewClass = "wux-ui-3ds wux-ui-3ds-2x wux-ui-3ds-expand-down fonticon-display";
    	...
    	var expandCollapse = document.querySelector('#expandCollapse');
		 ...
		 let accessrightsGridView = document.querySelector('.accessrights-gridView-View');		
		  ...
		  if(expandCollapse.className.indexOf("wux-ui-3ds-expand-up") > -1){
			...
			widget.setValue('UserGroupIDCardPersistedState', "minimized");
			...
			// handle the hide thumbnail case //
			  ...
			  widget.setValue('UserGroupIDCardPersistedState', "maximized");
...
define('DS/ENOUserGroupMgmt/Views/Widget/InitializeSummaryDetails', [
    ...
    'DS/WebUAUtils/WebUAUtils',
    'DS/ENOUserGroupMgmt/Views/UsersGroupHomeSplitView'],
    function (Promise, UWA, UserGroupApplicationFrame, WUXImmersiveFrame, UsersGroupSummaryView, WrapperDataGridView, RequestUtils, UserGroupBootstrap, UsersGroupModel, ImmersiveFrame, WebUAUtils, UsersGroupHomeSplitView) {
        'use strict';
        var container = null;
...
define('DS/ENOUserGroupMgmt/Views/Grid/UsersGroupMembersDataGridView',
    ...
    let _membersToolbar;
    let build = function(model){
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("MembersTab");
		let containerClass = 'Members-gridView-View hideView';
		if(persistedView && persistedView == "GridView")
			containerClass = 'Members-gridView-View showView';	
        ...
        let dataGridViewContainer = WrapperDataGridView.build(model, UserGroupMembersGridViewConfig, toolbar, gridViewDiv, "UsersGroupMembersView");
...
define('DS/ENOUserGroupMgmt/Views/Grid/UsersGroupAccessRightsDataGridView',
    ...
    let build = function(model){
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("Access Rights");
		let containerClass = 'accessrights-gridView-View hideView';
			if(persistedView && persistedView == "GridView")
				containerClass = 'accessrights-gridView-View showView';		
        ...
        let dataGridViewContainer = WrapperDataGridView.build(model, UsersGroupAccessRightsGridViewConfig, toolbar, gridViewDiv, "UsersGroupAccessRightsView");
...
define('DS/ENOUserGroupMgmt/Views/Tile/UsersGroupMembersTileView',
    ...
    let build = function(model){
		...
		let containerClass = 'Members-tileView-View showView nonVisible';
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("MembersTab");
		if(persistedView == "GridView")
			containerClass = 'Members-tileView-View hideView';
        var tileViewDiv = UWA.createElement("div", {id:'tileViewContainer',
            styles: {
                'width': "100%",
                'height': "calc(100% - 40px)",
...
define('DS/ENOUserGroupMgmt/Config/UsersGroupDetailMembersGridViewConfig',
	[
		'DS/ENOUserGroupMgmt/Views/Grid/UserGroupMembersGridCustomColumns',
		'i18n!DS/ENOUserGroupMgmt/assets/nls/ENOUserGroupMgmt'],
	function(UserGroupMembersGridCustomColumns, NLS) {
		...
		let UserGroupDetailMembersGridViewConfig = [{
			text: NLS.MemeberFullName,
			dataIndex: 'tree',
			editableFlag: false,
			'onCellRequest': UserGroupMembersGridCustomColumns.onMemberUserNameCellRequest
		...
		return UserGroupDetailMembersGridViewConfig;
...
define('DS/ENOUserGroupMgmt/Views/Grid/UsersGroupDetailMembersDataGridView',
        [   
            'DS/ENOUserGroupMgmt/Config/UsersGroupDetailMembersGridViewConfig',
            'DS/ENOUserGroupMgmt/Components/Wrappers/WrapperDataGridView',
			'DS/ENOUserGroupMgmt/Utilities/UserGroupPersistencyUtil',
            'DS/ENOUserGroupMgmt/Config/Toolbar/UsersGroupMembersTabToolbarConfig'
            ], function(
            		UserGroupDetailMembersGridViewConfig,
                    WrapperDataGridView,
					UserGroupPersistencyUtil,
                    UserGroupMembersTabToolbarConfig ) {
    ...
    let build = function(model){
		let persistedView = UserGroupPersistencyUtil.getViewPersistency("DetailsMembersTab");
		let containerClass = 'GroupDetail-gridView-View hideView';
		if(persistedView && persistedView == "GridView")
			containerClass = 'GroupDetail-gridView-View showView';	
        var gridViewDiv = UWA.createElement("div", {id:'dataGridViewContainer',
            styles: {
                'width': "100%",
                'height': "calc(100% - 40px)",
        ...
        let dataGridViewContainer = WrapperDataGridView.build(model, UserGroupDetailMembersGridViewConfig, toolbar, gridViewDiv, "UsersGroupDetailMembersView");
        _membersToolbar = WrapperDataGridView.dataGridViewToolbar();
        return dataGridViewContainer;
    };  
    let getGridViewToolbar = function(){
...
define('DS/ENOUserGroupMgmt/Views/ToggleViews',
    ...
    var ToggleViews = {
            ...
            doToggleView: function(args) {
				...
				let groupDetailsViewer = document.getElementById("GroupDetailsViewer");
				if(groupDetailsViewer && args.curPage == "MembersTab")
					widget.setValue("groupDetailsViewer-" + args.curPage + "-ShowView", args.view);
				else
...
define('DS/ENOUserGroupMgmt/Views/UsersGroupMembersView',
    ...
    let drawUserGroupMembersView = function(containerDiv){
        ...
        /*_addToolbarBtnTooltip(membersTabToolbar, "createContent", 'AddMemberTab'); 
        _addToolbarBtnTooltip(membersTabToolbar, "selection", 'MembersManageSelection'); 
        _addToolbarBtnTooltip(membersTabToolbar, "view", 'ViewMembers'); 
        _addToolbarBtnTooltip(membersTabToolbar, "removeContent", 'RemoveMemberTab'); */
        
        HelpUtil._addToolbarBtnTooltip(membersTabToolbar, "createContent", 'AddMemberTab');
        HelpUtil._addToolbarBtnTooltip(membersTabToolbar, "selection", 'MembersManageSelection');
        HelpUtil._addToolbarBtnTooltip(membersTabToolbar, "view", 'ViewMembers');
        HelpUtil._addToolbarBtnTooltip(membersTabToolbar, "removeContent", 'RemoveMemberTab');
    ...
    /*function _addToolbarBtnTooltip(membersTabToolbar, btnId, resourceName) {
		var tooltip = new WUXTooltipModel();
		tooltip.loadFromHelpRscFile('ENOUserGroupMgmt/help', resourceName);
		_setToolbarBtnTooltip(membersTabToolbar, btnId, tooltip);
	}

	function _setToolbarBtnTooltip(membersTabToolbar, btnId, tooltip) {
		if (tooltip.isEmpty()) {
			return setTimeout(_setToolbarBtnTooltip.bind(this, membersTabToolbar, btnId, tooltip), 1000);
		}
		membersTabToolbar.updateNodeModel(btnId, { tooltip: tooltip });
	}*/
...
define('DS/ENOUserGroupMgmt/Views/Tile/UsersGroupAccessRightsDataTileView',
            ...
            'DS/ENOUserGroupMgmt/Components/Wrappers/WrapperTileView'
            		...
            		ContextualMenu,UserGroupAccessRightsModel,UserGroupPersistencyUtil,WrapperTileView
    ...
    let build = function(model){
		...
		let containerClass = 'accessrights-tileView-View showView nonVisible';
				let persistedView = UserGroupPersistencyUtil.getViewPersistency("Access Rights");
				if(persistedView == "GridView")
					containerClass = 'accessrights-tileView-View hideView';        
