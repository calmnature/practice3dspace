...
let genToolbarTemplate = Handlebars.compile(html_ENOXGeneratedToolBar);
	...
	ENOXGeneratedToolBar.prototype._displayMenu = function (menu, container, displayOption) {
  		let that = this;
  		...
  		let pos = {
    		   ...
    		   forceTouchMode: displayOption?.touchMode ? displayOption.touchMode : that._options.touchMode,
 		};
  		Menu.show(menu, pos);
	};
...
//************************************* SORT PARTY ******************************************//
	...
	ENOXGeneratedToolBar.prototype._updateSortDropdownStyle = function(notificationContainer, dropdownComponent){
		...
		let dropdownBoundary = dropdownComponent.getBoundingClientRect()
		let sortHeight = dropdownComponent.clientHeight + dropdownBoundary.top;
		...
		// Get the right boundary of the document body
		let bodyRightBoundary = document.body.getBoundingClientRect().right;

		// Check if the dropdown boundary exceeds the body's right boundary
		if (dropdownBoundary.right > bodyRightBoundary) {
			// Calculate the difference between the dropdown boundary and the body's right boundary
			let overflowAmount = dropdownBoundary.right - bodyRightBoundary;

			// Adjust the left position of the notification container to prevent overflow
			notificationContainer.style.left = (Number(notificationContainer.style.left.split('px')[0]) - overflowAmount) + 'px';
		}
	};
