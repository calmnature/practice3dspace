    ...
    var ConfiguratorSolverFunctions = {
        ...
        initialize: function (configModel, modelEvents, version) {
            ...
            this.modelEvents.subscribe({ event: 'OnRuleAssistanceLevelChange' }, (data) => {
                if (data.value !== ConfiguratorVariables.NoRuleApplied) this.setSelectionModeOnSolver(data.value, true);
            });
        ...
        initSolver: function (modelId, configModel, modelEvents, configCriteria, parentContainer, options) {
            ...
            this.displayErrorSolverNotification =
                options.displayErrorSolverNotification !== undefined ? options.displayErrorSolverNotification : true;
            ...
            if (this.version === 'V1' || this.version === 'V2' || this.version === 'V3') {
                return CfgSolverServices.initSolver({
                    modelId: modelId,
                    data: { configurationCriteria: configCriteria },
                    tenant: that.tenant
                }).then(function (data) {
                ...
                return CfgSolverServices.create(options).then(async function (data) {
                    ...
                    return CfgSolverServices.initialize({ solverKey: data.solverKey, jsonData: solverDico }).then(
                        function () {
        ...
        getFeatureStatusEnforceBaseConfiguration: function (options) {
            ...
            return Utilities.sendRequestPromise({
                ...
                method: 'POST',
                data: JSON.stringify({
                    features: ['enforceBaseConfiguration']
                }),
                tenant: options.tenant,
                requestType: 'application/json'
            }).then((data) => {
                try {
                    ...
                    return data && data.features && data.features.enforceBaseConfiguration
                        ? data.features.enforceBaseConfiguration
                        : false;
                } catch (error) {
                    return false;
                }
        ...
        CallSolveMethodOnSolver: function (options) {
            ...
            if (this.solverCreated) {
                ...
                if (!this.doNotUseSolver) {
                    ...
                    if (this.configModel.getRulesActivation() === 'true') {
                        if (this.configModel.getRulesMode() === 'RulesMode_SelectCompleteConfiguration') {
                            ...
                            if (this.configModel.getFirstSelection()) {
                                ...
                                listConfigurationCriteria = this.configModel.convertToNewSolverFormat(
                                    this.configModel.getConfigurationCriteria()
                                );
                                requestData = {
                                    configurationCriteria: this.configModel._bcId
                                        ? Array.prototype.concat(listConfigurationCriteria, [
                                              { Criterion: this.configModel._bcId, State: 'Chosen' }
                                          ])
                                        : listConfigurationCriteria
                                };
                                if (this.configModel._manageDefaultVersion === 'V2') {
                                    requestData.timeoutForDefaultComputation =
                                        this.configModel.getDefaultComputationTimeout();
                                }
                                CfgSolverServices.CallSolveWithoutDignoseMethodOnSolver(
                                    requestData,
                                    that.solverKey,
                                    that.modelEvents
                                ).then(
                                ...
                                Utilities.displayNotification({
                                    eventID: 'info',
                                    msg: nlsConfiguratorKeys.Selection_Required_Complete
                                });
                                //
                            }
                        } else {
                            ...
                            CfgSolverServices.CallSolveMethodOnSolver(
                                requestData,
                                that.solverKey,
                                that.modelEvents
                            ).then(
                    ...
                    var message = UWA.String.unescapeHTML(
                        '<b>' +
                            nlsConfiguratorKeys.BC_conflict_title +
                            '</b>' +
                            '<br>' +
                            nlsConfiguratorKeys.BC_conflict_message
                    );
                    ...
                    if (options && options.firstCall) {
                        ...
                        var configCriteria = this.updateConfigCriteriaWithoutSolver(
                            this.configModel.getAllConfigCriteria(),
                            this.configModel.getBCSelectedCriteria()
                        );
        ...
        callSolverOnSelectedIDsMethodOnSolver: function (options) {
            ...
            if (this.solverCreated && !this.doNotUseSolver) {
                ...
                if (this.configModel.getRulesActivation() === 'true') {
                    ...
                    var configurationCriteria = this.configModel._bcId
                        ? Array.prototype.concat(listConfigurationCriteria, [
                              { Criterion: this.configModel._bcId, State: 'Chosen' }
                          ])
                        : listConfigurationCriteria;
                    var requestData = {
                        idsToDiagnose: options.idsToDiagnose,
                    ...
                    if (!this.abortNotificationDisplayed) {
                        abortRequired = setTimeout(function () {
                            that.counterNotification = Utilities.displayNotification({
                                eventID: 'warning',
                                msg: nlsConfiguratorKeys.Abort_Notification,
                                action: {
                                    label: 'Abort',
                                    callback: function (counterNotification) {
                                        ...
                                        that.configModel.setSelectedConfigurationCriteria(
                                            _solveCriteria,
                                            options.idsToDiagnose
                                        );
                    ...
                    CfgSolverServices.CallSolverOnSelectedIDsMethodOnSolver(
                        requestData,
                        that.solverKey,
                        that.modelEvents
                    ).then(
        ...
        setSelectionModeOnSolver: function (newMode, callSolveAfterSolverResult) {
            ...
            if (this.solverCreated) {
                ...
                if (newMode !== ConfiguratorVariables.NoRuleApplied) {
                    if (newMode === ConfiguratorVariables.RulesMode_DisableIncompatibleOptions) newMode = 'Select_None';
                    ...
                    abortRequest.then(function () {
                        CfgSolverServices.setSelectionModeOnSolver(newMode, that.solverKey, that.modelEvents).then(
                            function (data) {
                                if (callSolveAfterSolverResult) {
        ...
        getResultingStatusOriginators: function (optionId) {
            var that = this;
            CfgSolverServices.getResultingStatusOriginators(optionId, this.solverKey, that.modelEvents).then(
                function (data) {
        ...
        CheckRulesConsistency: function () {
            var that = this;
            if (this.solverCreated) {
                return CfgSolverServices.CheckRulesConsistency(this.solverKey, that.modelEvents).then(
                    function (data) {
                        that.ApplyAnswer(data);
                    },
                    function () {}
        ...
        ApplyAnswer: function (input, in_callSolveAfterSolverResult, idsToDiagnose) {
            ...
            if (input !== undefined && input !== '' && UWA.typeOf(input) === 'string') {
                ...
                if (answerMethod === 'getResultingStatusOriginators') {
                        ...
                        if (this.configModel.setRulesActivation) {
                            ...
                            if (listOfInconsistentRules.length > 0) {
                                ...
                                for (var i = 0; i < listOfInconsistentRules.length; i++) {
                                    message +=
                                        "<blocquote style='padding-left:20px;'>" +
                                        listOfInconsistentRules[i] +
                                        '<br></blocquote>';
                                }
                            }
                            if (answerRC == 'ERROR') {
                    ...
                    answerMethod === 'solveAndDiagnoseAll' ||
                    answerMethod === 'solve' ||
                    answerMethod === 'solveAndDiagnose'
                ) {
                    var answerConfigCriteria =
                        inputbis._version === '4.0'
                            ? this.configModel.convertFromNewSolverFormat(
                                  answerData.configurationCriteria,
                    ...
                    if (answerConflicts) {
                        if (answerRC === 'ERROR') {
                            Utilities.displayNotification({
                                eventID: 'info',
                                msg: nlsConfiguratorKeys.InfoComplexityOfRules
                            });
                        }
                        var listOfListOfConflictingIds =
                            inputbis._version === '4.0'
                                ? this.configModel.convertFromNewSolverFormat(answerConflicts, 'conflictCriteria')
                                : answerConflicts;
                        ...
                        this.configModel.setRulesCompliancyStatus('Invalid');
                    } else if (answerConfigCriteria) {
                        // No conflict found during resolution
                        this.configModel.setConfigurationCriteria(answerConfigCriteria, answerMethod);
                    ...
                    if (answerDefaults) {
                        var listOfDefaultCriteria =
                            inputbis._version === '4.0'
                                ? this.configModel.convertFromNewSolverFormat(answerDefaults, 'defaultCriteria')
                                : answerDefaults;
                        this.configModel.setDefaultCriteria(listOfDefaultCriteria);
                    }

                    if (answerModifiedAssumptions && answerModifiedAssumptions.length > 0) {
                        // Some assumptions have been modified during resolution
                        var listOfModifiedAssumptions =
                            inputbis._version === '4.0'
                                ? this.configModel.convertFromNewSolverFormat(
                                      answerModifiedAssumptions,
                                      'modifiedAssumtionsCriteria'
                                  )
                                : answerModifiedAssumptions;
                        ...
                        for (var i = 0; i < listOfModifiedAssumptions.length; i++) {
                            ...
                            message +=
                                "<blocquote style='padding-left:20px;'>" +
                                this.configModel.getFeatureDisplayNameWithId(listOfModifiedAssumptions[i].Id);
                            if (UWA.is(this.configModel.getOptionDisplayNameWithId(listOfModifiedAssumptions[i].Id))) {
                                message +=
                                    '[' +
                                    this.configModel.getOptionDisplayNameWithId(listOfModifiedAssumptions[i].Id) +
                                    ']<br></blocquote>';
                            } else {
                                message += '</blocquote>';
                            }
                    ...
                    if (answerRC === 'DEFAULT_ABORTED') {
                        Utilities.displayNotification({
                            eventID: 'warning',
                            msg: nlsConfiguratorKeys.DefaultAbortWarning
                        });
                    }
                    ...
                    if (callSolveAfterSolverResult.firstCall) this.CallSolveMethodOnSolver(callSolveAfterSolverResult);
                    else this.CallSolveMethodOnSolver({ refresh: true });
...
define('DS/ConfiguratorPanel/scripts/Models/DictionaryDataUtil', ['UWA/Core'], function (UWA) {
    'use strict';
    ...
    dictionaryDataUtil.prototype.isZoneVariant = function (id) {
        return this._criteriDetails[id].type === 'Zone Variant';
    };
...
define('DS/ConfiguratorPanel/scripts/Presenters/ParameterSelectionView', [
    ...
    var ParameterSelectionView = Abstract.inherit({
        ...
        addUnitInCell: function (unit) {
            if (this.status == 'Not Available' || this.status == 'Incompatible') {
...
define('DS/ConfiguratorPanel/scripts/Presenters/CriteriaSelectionEditor', [
    ...
    var CriteriaSelectionEditor = Abstract.inherit({
        ...
        buildView: function () {
            ...
            this._autoComplete = new CfgAutocomplete({
                ...
                onClickIcon: (e, cellInfos) => {
                    ...
                    value.forEach((item) => {
                        if (item.id === cellInfos.id) that.fire('show-state-tooltip', { value: item, id: cellInfos.id });
                    });
                },
                    ...
                    if (that.sort) {
                        if (that.sort.attribute === 'displayName') {
                            return nodeModelA
                                ...
                                .localeCompare(nodeModelB.getAttributeValue('title'));
                        } else if (that.sort.attribute === 'sequenceNumber') {
                            return (
                                nodeModelA.getAttributeValue('sequenceNumber') -
                                nodeModelB.getAttributeValue('sequenceNumber')
                            );
                        }
                    }
                    return 0;
        ...
        handleEvents: function () {
            ...
            that.getContent().addEventListener(
                'click',
                function (e) {
            ...
            that.getContent().addEventListener('focusout', function () {
        ...
        _getChipInfo: function (nodeModel) {
            ...
            className += nodeModel.getAttributeValue('state')
                ? ' criteria-chip-' + STATE_ICONS[nodeModel.getAttributeValue('state')]
                : '';
        ...
        _applyMultiValue: function () {
            if (this.enableEasySelection) {
                this._autoComplete.easySelectionMode = this.multiValue
                    ? CfgAutocomplete.SELECT_MODE_MULTI
                    : CfgAutocomplete.SELECT_MODE_SINGLE;
            } else {
        ...
        _applyValidationAction: function () {
...
define('DS/ConfiguratorPanel/scripts/Models/PCDataUtil', [
    'UWA/Core',
    'DS/ConfiguratorPanel/scripts/Models/ConfiguratorVariables',
    'DS/ConfiguratorPanel/scripts/ServiceUtil',
    'i18n!DS/ConfiguratorPanel/assets/nls/ConfiguratorPanel.json',
    'i18n!DS/xDimensionManager/assets/nls/xUnitLabelLong.json'
], function (UWA, ConfiguratorVariables, ServiceUtil, nlsConfiguratorKeys, nlsUnitLabelLongKeys) {
    'use strict';
    ...
    pCDataUtil.prototype.isSelectable = function (id) {
        return (
            this._configModel.getStateWithId(id) !== ConfiguratorVariables.Incompatible &&
            this._configModel.getStateWithId(id) !== ConfiguratorVariables.Required
        );
    };
    ...
    pCDataUtil.prototype.getSelectedValue = function (id) {
        ...
        if (
            UWA.is(id, 'string') &&
            id !== '' &&
            (this._dictionaryUtil.isVariant(id) ||
                this._dictionaryUtil.isOptionGroup(id) ||
                this._dictionaryUtil.isZoneVariant(id))
        ) {
            ...
            value = [];
            if (
                !this.isMultiValue(id) &&
                this._configModel.getRulesMode() === ConfiguratorVariables.RulesMode_SelectCompleteConfiguration
            ) {
    ...
    pCDataUtil.prototype.hasRejectAction = function (id) {
        if (
            this._configModel.getRulesActivation() === ConfiguratorVariables.str_false ||
            this._configModel.getStateWithId(id) === ConfiguratorVariables.Chosen ||
            this._configModel.getStateWithId(id) === ConfiguratorVariables.Default ||
            this._configModel.getStateWithId(id) === ConfiguratorVariables.Dismissed
        ) {
            return true;
    ...
    pCDataUtil.prototype.isMultiValue = function (id) {
        if (
            this._configModel.getFeatureSelectionMode(id) ===
                ConfiguratorVariables.feature_SelectionMode_EnforceMultiple ||
    ...
    pCDataUtil.prototype.isUpdateRequired = function (id) {
        if (
            this._configModel.getRulesMode() === 'RulesMode_SelectCompleteConfiguration' &&
        ...
        if (
            ...
            value.forEach((valueId) => {
                ...
                if (this.isDismissValue(id)) {
                    ...
                    if (oldValue === ConfiguratorVariables.Dismissed) {
                        if (
                            this._configModel.getDefaultCriteria() &&
                            this._configModel.getDefaultCriteria().indexOf(valueId) !== -1
                        ) {
    ...
    pCDataUtil.prototype.validateValue = function (id, value) {
        var oldValue = this._configModel.getStateWithId(value);
        if (
            oldValue === ConfiguratorVariables.Default ||
            oldValue === ConfiguratorVariables.Required ||
            oldValue === ConfiguratorVariables.Selected
        ) {
            this._configModel.setStateWithId(value, ConfiguratorVariables.Chosen);
            if (UWA.is(this._chosenCriteria[id], 'object')) this._chosenCriteria[id][value] = true;
            else {
                this._chosenCriteria[id] = {};
    ...
    pCDataUtil.prototype._searchCall = function (data, requestID, nresults) {
        return ServiceUtil.performSearchRequest(data, requestID)
            .then((output) => {
    ...
    pCDataUtil.prototype._parseSearchResult = function (result) {
        ...
        var pcs = result.map((item) => {
            var details = {};
            var attributes = UWA.Json.path(
                item,
                '$..attributes[?(@.name=="resourceid" || @.name=="ds6w:label" || @.name=="ds6w:identifier" || @.name=="pc_feature_options" || @.name=="preview_url" || @.name=="revision"|| @.name=="pc_options_selected" || @.name=="pc_options_rejected" )]'
            );
            if (attributes) {
    ...
    pCDataUtil.prototype._prepareSearchQuery = function () {
        ...
        var parentCheck =
            'AND (based_95_on_95_model_95_version_95_pid:"' +
            mvID +
            '")' +
            (pcID ? ' AND (NOT physicalid:"' + pcID + '") ' : '');
        var selectionModeCheck =
            this._configModel.getSelectionMode() == ConfiguratorVariables.selectionMode_Refine
                ? 'AND (pc_95_selection_95_mode:"Refine")'
                : 'AND (pc_95_selection_95_mode:"Build") ';
        var criteriaCheck = '';
                ...
                filteredPC = pcList.filter((id) => {
                    if (this._pcDetails.hasOwnProperty(id)) {
                        ...
                        if (this._pcDetails[id][selectionType] /*.selections*/) {
                            ...
                            if (
                                this._pcDetails[id][selectionType] /*.selections*/
                                    .hasOwnProperty(parent)
                            ) {
        ...
        for (var criteriaId in this._chosenCriteria) {
            ...
            for (var valueId in criteria) {
                if (criteria.hasOwnProperty(valueId)) {
                    pcList = updateMatchingPC(
                        { id: valueId, state: this._configModel.getStateWithId(valueId) },
                        pcList,
                        criteria[valueId] ? 'selections' : 'rejections'
                    );
                }
            }
            ...
            if (output && UWA.is(output.member, 'array')) {
                if (this._pcDetails.hasOwnProperty(id)) {
                    ...
                    output.member.forEach((criteria) => {
                        if (criteria.status) {
                            states[criteria.id] = STATES.hasOwnProperty(criteria.status)
                                ? STATES[criteria.status]
                                : criteria.status;
    ...
    pCDataUtil.prototype._getInfoCall = function (id) {
        var url = UWA.String.format(
            '/resources/v1/modeler/dspfl/dspfl:ModelVersion/{0}/dspfl:ProductConfiguration/{1}',
            this._configModel._modelID,
            id
        );
        var data = {};
        return ServiceUtil.performServiceRequest(url, data, 'pc' + id, { method: 'GET', data: '' }).then((output) => {
            if (output && UWA.is(output.member, 'array') && output.member.length === 1) {
                if (this._pcDetails.hasOwnProperty(id)) {
                    ...
                    var rulesMode =
                        evalMode === 'CompleteConfiguration'
                            ? 'RulesMode_SelectCompleteConfiguration'
                            : evalMode === 'Dependencies'
                              ? 'RulesMode_EnforceRequiredOptions'
                              : 'RulesMode_DisableIncompatibleOptions';
        ...
        if (searchCriteria) {
            ...
            if (searchKeys.length !== oldSearchKeys.length) {
                ...
                for (var i = 0; i < searchKeys.length; i++) {
                    ...
                    if (
                        !this._searchCriteria.hasOwnProperty(searchKeys[i]) ||
                        !UWA.equals(searchCriteria[searchKeys[i]], this._searchCriteria[searchKeys[i]])
                    ) {
                        hasChanged = true;
                        break;
                    }
    ...
    pCDataUtil.prototype.applyPCSelections = async function (pcId) {
        ...
        this._modelEvents.publish({
            event: 'OnConfigurationModeChange',
            data: { mode: this._configModel.getSelectionMode() }
        });
    };
        ...
        if (this._pcDetails.hasOwnProperty(pcId)) {
            ...
            if (this._pcDetails[pcId].states) {
                for (var id in this._pcDetails[pcId].states) {
                    if (
                        this._pcDetails[pcId].states[id] === ConfiguratorVariables.Chosen ||
        ...
        if (this._pcDetails && this._pcDetails.hasOwnProperty(pcId)) {
            if (this._pcDetails[pcId].selections || this._pcDetails[pcId].rejections) {
                ['selections', 'rejections'].forEach((selectType) => {
                    if (
                        this._pcDetails[pcId][selectType] &&
                        this._pcDetails[pcId][selectType].hasOwnProperty(criteriaId)
                    ) {
                        ...
                        if (UWA.is(values, 'array')) {
                            selectedValues = values.map((item) => {
                                ...
                                var icon = '';
                                if (
                                    this._pcDetails[pcId].states &&
                                    this._pcDetails[pcId].states.hasOwnProperty(valueDetails.id)
                                ) {
    ...
    pCDataUtil.prototype._getConflictingMessage = function (id) {
        ...
        listOfListOfConflictingIds.every((ConflictingId) => {
            ...
            if (ConflictingId !== id) {
                ...
                message +=
                    this._configModel.getFeatureDisplayNameWithId(ConflictingId) +
                    '[' +
                    that._getDisplayOption(ConflictingId) +
                    ']';
                addAlso = true;
            }
            return true;
                ...
                if (data) {
                    ...
                    if (causingAssumptions.length === 0) {
                        ...
                        for (var i = 0; i < causingAssumptions.length; i++) {
                            if (i > 0) message += ' ' + UWA.i18n('and because') + ' ';

                            state = model.getStateWithId(causingAssumptions[i]);
                            var msgStr2 = UWA.i18n('#OPTION# is #STATUS#');
                            msgStr2 = msgStr2.replace(
                                '#OPTION#',
                                model.getFeatureDisplayNameWithId(causingAssumptions[i]) +
                                    '[' +
                                    that._getDisplayOption(causingAssumptions[i]) +
                                    ']'
                            );
...
define('DS/ConfiguratorPanel/scripts/Presenters/CriteriaSelectorUtil', [
    ...
    criteriaSelectorUtil.prototype.getProperties = function (id) {
        ...
        return {
            ...
            enableEasySelection:
                this._pcDataUtil.isMultiValue(id) ||
                this._configModel.getRulesMode() !== ConfiguratorVariables.RulesMode_SelectCompleteConfiguration,
    ...
    criteriaSelectorUtil.prototype.getNumericalProperties = function (id) {
        ...
        if (UWA.is(paramdef, 'object')) {
            ...
            return {
                ...
                state: paramvalue.NumericalValue ? this._configModel.getStateWithId(id) : '',
    ...
    criteriaSelectorUtil.prototype.updateViewComponent = function (component, id) {
        const item = this._pcDataUtil.getSelectedValue(id);
...
define('DS/ConfiguratorPanel/scripts/Models/FilterExpressionXMLServices', [
    ...
    function FilterExpressionXMLServices(xmlType) {
        ...
        function initXMLType() {
            if (xmlType == 'FilterExpression') XML_FILE_TYPE = FILTER_EXPRESSION;
        }
        ...
        function jsonXmlElemCount() {
            ...
            for (var it in ConfigCriteriaObj) {
                if (ConfigCriteriaObj[it] instanceof Object) {
                    if (
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Chosen ||
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Required ||
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Default ||
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Selected ||
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Dismissed ||
                        ConfigCriteriaObj[it].State == ConfiguratorVariables.Incompatible
                    )
                        elementsCnt++;
                }
            }
...
*/
        ...
        function generateFilterXML(options) {
            ...
            if (ModelVersionInfoObj !== undefined) result += getProdStateXML();
            ...
            if (elementsCnt > 0) {
                let chosenCrit = options.configModel.getChosenConfigurationCriteriaAsMap(true);
                ...
                options.configModel._dictionaryJson.features.forEach((feature) => {
                    if (
                        options.configModel.getFeatureSelectionMode(feature) === 'Dismiss' &&
                        options.configModel.hasDismissedOptions(feature, true)
                    ) {
                        result += addFeatureOptions(feature, options.configModel, true);
                    } else if (options.configModel.hasChosenOptions(feature, true, chosenCrit)) {
            ...
            for (var prop in listOfRejectedOptionsForEachSingleFeature) {
                if (listOfRejectedOptionsForEachSingleFeature.hasOwnProperty(prop)) empty = false;
            }

            if (!empty) result += addListOfRejectedOptionsForSingleFeaturesInTheXML();
        ...
        function getProdStateXML() {
            ...
            if (
                ModelVersionInfoObj.modelName &&
                ModelVersionInfoObj.modelVersionName &&
                ModelVersionInfoObj.modelVersionRevision
            ) {
                ...
                prodStateXml +=
                    '<Single Name="' + escapeXmlChars(prodName) + '" Revision="' + escapeXmlChars(prodRevsion) + '" />';
        ...
        function addFeatureOptions(Feature, configModel, isRejected) {
            ...
            Feature.options.forEach((coElem) => {
                ...
                if (coName != undefined && coName != null) {
                    ...
                    if (
                        (isRejected && (coState === 'Dismissed' || coState === 'Incompatible')) ||
                        (!isRejected &&
                            (coState === 'Required' ||
                                coState === 'Chosen' ||
                                coState === 'Selected' ||
        ...
        function featureTag(elemName, elemDisplayName, closed, addSelectedByAttribut, optionState) {
            ...
            if (addSelectedByAttribut && optionState) {
                if (optionState == ConfiguratorVariables.Default) resXml += ' SelectedBy="Default"';
                else if (
                    optionState == ConfiguratorVariables.Required ||
                    optionState == ConfiguratorVariables.Incompatible
                )
                    resXml += ' SelectedBy="Rule"';
                else resXml += ' SelectedBy="User"';
            }
...
define('DS/ConfiguratorPanel/scripts/Presenters/TopbarPresenter', [
    'UWA/Core',
    ...
    TouchUtils,
    ConfigVariables,
    ENOXCollectionToolBarV2,
    NLS_ConfiguratorPanel,
    html_TopbarPresenter
) {
    'use strict';
    ...
    TopbarPresenter.prototype._init = function (options) {
        ...
        this.sort = [
            ...
            { id: 'sequenceNumber', type: 'integer' }
        ];
    ...
    TopbarPresenter.prototype._initToolBar = function () {
        ...
        if (this.add3DButton === 'yes') {
            actions.push({
                id: '3d-view',
                text: that._3dViewActionActivated
                    ? NLS_ConfiguratorPanel.Hide_3D_View
                    : NLS_ConfiguratorPanel.Show_3D_View, //NLS_ConfiguratorPanel.Show_3D_View, // Hide_3D_View
                icon: {
                    iconName: 'compass-3d',
                    fontIconFamily: 1
                },
                value: function (options) {
                    ...
                    that.modelEvents.publish({
                        event: 'Request3DFromConfigPanel',
                        data: { value: that._3dViewActionActivated === true ? 'show' : 'hide' }
                    });
                    that._toolbarModelEvent.publish({
                        event: 'enox-collection-toolbar-icon-update',
                        data: { id: '3d-view', icon: { className: updatedClassName } }
                    });
                    that._toolbarModelEvent.publish({
                        ...
                        data: {
                            ...
                            text: that._3dViewActionActivated
                                ? NLS_ConfiguratorPanel.Hide_3D_View
                                : NLS_ConfiguratorPanel.Show_3D_View
                        }
                    });
                },
                handler: function () {
        ...
        if (this.enableMatchingPC) {
            actions.push({
                ...
                tooltip: {
                    title: that._matchingPCActionActivated
                        ? NLS_ConfiguratorPanel.Tooltip_hideMatchingPC
                        : NLS_ConfiguratorPanel.Tooltip_showMatchingPC
                },
                selected: true,
                icon: {
                ...
                value: function (options) {
                    ...
                    that._toolbarModelEvent.publish({
                        event: 'enox-collection-toolbar-icon-update',
                        data: { id: 'find-matching-pc', icon: { className: updatedClassName } }
                    });
                    that._toolbarModelEvent.publish({
                        event: 'enox-collection-toolbar-change-text-tooltip-action',
                        data: {
                            id: 'find-matching-pc',
                            text: that._matchingPCActionActivated
                                ? NLS_ConfiguratorPanel.Tooltip_hideMatchingPC
                                : NLS_ConfiguratorPanel.Tooltip_showMatchingPC
                        }
                    });
        ...
        if (that.configModel.isRuleSelectionEnabled()) {
            actions.push({
            ...
            actions.push({
                id: 'rule-level-action',
                text: that._currentRuleObj.title,
                fonticon: that._getRuleLevelIcon(that._currentRuleAssistanceLevel)
            });
        }
        ...
        var toolbarOptions = {
            ...
            touchMode: TouchUtils.getTouchMode() == true,
            ...
            stableActions: [
                ...
                },
                {
                    id: 'expand-all',
                    title: 'Expand All',
                    text: NLS_ConfiguratorPanel.Expand_All,
                    fonticon: 'flow-tree-expand-down',
                    handler: function () {
                        that.modelEvents.publish({
                            event: 'OnExpandAll',
                            data: { activated: true }
                ...
                },
                {
                    id: 'collapse-all',
                    title: 'Collapse All',
                    text: NLS_ConfiguratorPanel.Collapse_All,
                    fonticon: 'flow-tree-expand-up',
                    handler: function () {
                        that.modelEvents.publish({
                            event: 'OnCollapseAll',
                            data: { activated: true }
    ...
    TopbarPresenter.prototype.hideCollapseExpandAll = function () {
        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-visibility-action',
            data: { id: 'expand-all', flag: false }
        });
        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-visibility-action',
            data: { id: 'collapse-all', flag: false }
        });
    };

    TopbarPresenter.prototype.showCollapseExpandAll = function () {
        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-visibility-action',
            data: { id: 'expand-all', flag: true }
        });

        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-visibility-action',
            data: { id: 'collapse-all', flag: true }
        });
    };
        ...
        var callbackFunction = function () {
            that._resetFeaturesForCompleteMode(this.id).then((confirmed) => {
                ...
                that._toolbarModelEvent.publish({
                    ...
                    data: { id: 'rule-level-action', menu: that._getRuleEvalLevelMenu() }
                });
                that._toolbarModelEvent.publish({
                    event: 'enox-collection-toolbar-icon-update',
                    data: {
                ...
                action: {
                    callback: function () {
                        ...
                        if (that.configModel.getCurrentFilter() === ConfigVariables.Filter_Chosen) {
                            ...
                            that._toolbarModelEvent.publish({
                                event: 'enox-collection-toolbar-icon-update',
                                data: { id: 'criteria-filter', icon: { className: '' } }
                            });
                        } else {
                            ...
                            that._toolbarModelEvent.publish({
                                ...
                                data: {
                                    id: 'criteria-filter',
                                    icon: { className: 'enox-collection-toolbar-filter-activated' }
                                }
                            });
                        }
                        that._toolbarModelEvent.publish({
                            event: 'enox-collection-toolbar-update-menu',
                            data: { id: 'criteria-filter', menu: that._getFilterActionMenu() }
                        });
                    }
                }
            },
    ...
    TopbarPresenter.prototype._subscribeToEvents = function () {
        ...
        this.modelEvents.subscribe({ event: 'deactivate-3dbutton' }, function () {
            ...
            that._toolbarModelEvent.publish({
                event: 'enox-collection-toolbar-icon-update',
                data: { id: '3d-view', icon: { className: '' } }
            });
            that._toolbarModelEvent.publish({
                event: 'enox-collection-toolbar-change-text-tooltip-action',
                data: { id: '3d-view', text: NLS_ConfiguratorPanel.Show_3D_View }
            });
        });
        ...
        this._toolbarModelEvent.subscribe(
    ...
    TopbarPresenter.prototype._findRuleAssistanceLevels = function (options) {
        ...
        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-update-menu',
            data: { id: 'rule-level-action', menu: this._getRuleEvalLevelMenu() }
        });
        this._toolbarModelEvent.publish({
            event: 'enox-collection-toolbar-icon-update',
            data: {
                id: 'rule-level-action',
                icon: { iconName: this._getRuleLevelIcon(this._currentRuleAssistanceLevel) }
            }
        });
        ...
        var confirmPromise = new Promise((resolve) => {
            if (newRulesMode === 'RulesMode_SelectCompleteConfiguration') {
                var superModal = new SuperModal({
                    className: 'confirm-changAssistanceLevel-modal',
                    renderTo: document.body
                });
                superModal.confirm(
                    NLS_ConfiguratorPanel.Message_Confirm_CompleteModeSwitch,
                    NLS_ConfiguratorPanel.Title_Confirm_CompleteModeSwitch,
                    function (confirmed) {
                        resolve(confirmed);
                    }
                );
        ...
        if (newRulesMode === 'RulesMode_SelectCompleteConfiguration') {
            ...
            if (UWA.is(features, 'array')) {
                for (var i = 0; i < features.length; i++) {
                    if (
                        this.configModel.getFeatureSelectionMode(features[i]) ===
                        ConfigVariables.feature_SelectionMode_Dismiss
                    ) {
                        if (this.configModel.hasDismissedOptions(features[i])) {
                            callbackPromises.push(
                                confirmUserPromptPromise.promise.then(() => {
                        ...
                        features[i].selectionType === ConfigVariables.feature_SelectionMode_Single &&
                        this.configModel.hasMultiSelectVariantValues(features[i])
                    ) {
                        callbackPromises.push(
                            confirmUserPromptPromise.promise.then(() => {
    ...
    TopbarPresenter.prototype._setRulesAssistanceLevel = function (options) {
        ...
        var ruleActivation =
            this._currentRuleAssistanceLevel === ConfigVariables.NoRuleApplied
                ? ConfigVariables.str_false
                : ConfigVariables.str_true;
        ...
        if (solveCallback) {
            this.modelEvents.publish({
                event: 'OnRuleAssistanceLevelChange',
                data: { value: this._currentRuleAssistanceLevel, callsolve: solveCallback }
            });
        }
...
/*jshint esversion: 6 */
define('DS/ConfiguratorPanel/scripts/Models/ConfiguratorModel', [
    'UWA/Core',
    'UWA/Controls/Abstract',
    'DS/ConfiguratorPanel/scripts/Models/ConfiguratorVariables',
    ...
    Abstract,
    ConfiguratorVariables,
    FilterExpressionXMLServices,
    FilterExpressionXMLServicesWithDisplayName,
    Utilities,
    Utils
) {
    'use strict';
    ...
    function _unselectCriteria(model, feature, criteria, min, newState, excludedTypes) {
        ...
        let unselect = function (elt) {
            if (UWA.is(elt, 'object')) {
                let selectedoptions = [];
                if (UWA.is(criteria, 'array')) {
                    selectedoptions = elt.options.filter(
                        (option) => criteria.filter((crit) => option.ruleId === crit.Id).length > 0
                    );
                } else {
                    if (UWA.is(criteria, 'object')) {
                        selectedoptions = elt.options.filter((option) => criteria.hasOwnProperty(option.ruleId));
                    }
                }
        ...
        if (UWA.is(feature)) {
            ...
            model.getDictionary().features.forEach(function (elt) {
                if (excludedTypes && excludedTypes.length > 0 && excludedTypes.indexOf(elt.type) !== -1) {
                    return;
                }
                unselect(elt);
            });
    ...
    var ConfiguratorModel = Abstract.extend({
        ...
        init: function (options) {
            ...
            this._withZoneVariants = false;
            this._parameterMandStatus = {};
            this._parameterMandStatusPrevious = {};

            //TODO : initialize appFunc and appRulesParams with the ones given as parameters

            if (options.appFunc) {
                if (options.appFunc.multiSelection) this._appFunc.multiSelection = options.appFunc.multiSelection;
                ...
                if (options.appFunc.rulesMode_ProposeOptimizedConfiguration)
                    this._appFunc.rulesMode_ProposeOptimizedConfiguration =
                        options.appFunc.rulesMode_ProposeOptimizedConfiguration;
                if (options.appFunc.rulesMode_SelectCompleteConfiguration)
                    this._appFunc.rulesMode_SelectCompleteConfiguration =
                        options.appFunc.rulesMode_SelectCompleteConfiguration;
                ...
                if (options.appFunc.rulesMode_DisableIncompatibleOptions)
                    this._appFunc.rulesMode_DisableIncompatibleOptions =
                        options.appFunc.rulesMode_DisableIncompatibleOptions;
            }
            if (options.appRulesParams) {
                ...
                this._appRulesParams.rulesMode =
                    options.appRulesParams.rulesMode &&
                    options.appRulesParams.rulesActivation === ConfiguratorVariables.str_true
                        ? options.appRulesParams.rulesMode
                        : ConfiguratorVariables.RulesMode_EnforceRequiredOptions;
            ...
            if (options.modelEvents) this._modelEvents = options.modelEvents;
            ...
            if (options.configuration) {
                ...
                this.setConfigurationCriteria(
                    Utilities.convertPersistedStatesToStatesInConfigCriteria(options.configuration),
                    '',
                    '',
                    true
                );
            }
            if (options.pcId) this._pcId = options.pcId;

            if (options.bcId) this._bcId = options.bcId;
            ...
            if (options.readOnly) this.setReadOnlyFlag(options.readOnly);
            ...
            that._enableRuleSelection = options.enableRuleSelection !== false;
        },
        ...
        setMultiSelectionState: function (newState) {
            //newState needs to be "true" or "false" strings
        ...
        getSelectedOptionsModelAsMap: function () {
            let that = this;
            let JSONModel = new Map();
            this.getDictionary().features.forEach(function (feature) {
                ...
                if (
                    ...
                    feature.type === 'Zone Variant'
                ) {
                    feature.options.forEach((option) => {
                        if (
                            (UWA.is(that._cacheIdWithState[option.ruleId]) &&
                                that.getFeatureSelectionMode(feature) ===
                                    ConfiguratorVariables.feature_SelectionMode_Dismiss &&
                            ...
                            JSONModel.set(option.ruleId, that._cacheIdWithState[option.ruleId]);
                        }
                    });
                } else if (
                    feature.type === 'Parameter' &&
                    UWA.is(that.getValueWithId(feature.ruleId)) &&
                    UWA.is(that.getValueWithId(feature.ruleId).NumericalValue, 'number') &&
                    ...
                    JSONModel.set(feature.ruleId, value);
                }
            });
        ...
        getSelectedOptionsJSONModel: function (PCSave) {
            ...
            this.getDictionary().features.forEach(function (feature) {
                ...
                if (
                    ...
                    feature.options.forEach((option) => {
                        if (
                            ...
                            if (PCSave) {
                                JSONModel.push({
                                    Id: option.ruleId,
                                    rel_id: option.rel_id,
                                    p_rel_id: feature.p_rel_id,
                    ...
                    // Variant Param
                    var value = that.getValueWithId(feature.ruleId).NumericalValue;
                    if (feature.unit) value += ' ' + feature.unit;
                    if (PCSave && value) {
                        JSONModel.push({
                            Id: feature.ruleId,
        ...
        getChosenConfigurationCriteriaAsMap: function (selected) {
            let chosenCriteria = {};
            for (var id in this._cacheIdWithState) {
                if (this._cacheIdWithState.hasOwnProperty(id)) {
                    if (
                        this._cacheIdWithState[id] === ConfiguratorVariables.Chosen ||
                        (selected &&
                        ...
                        chosenCriteria[id] = this._cacheIdWithState[id];
                    }
                }
            }
            return chosenCriteria;
        ...
        getDismissedConfigurationCriteria: function (incompatible) {
            ...
            for (var id in this._cacheIdWithState) {
                if (this._cacheIdWithState.hasOwnProperty(id)) {
                    if (
                        this._cacheIdWithState[id] === ConfiguratorVariables.Dismissed ||
                        (incompatible && this._cacheIdWithState[id] === ConfiguratorVariables.Incompatible)
                    ) {
        ...
        isStatusUpdated: function (criteria) {
            ...
            if (!this._cacheIdWithState[criteria.Id]) {
        ...
        setConfigurationCriteria: function (newConfigCriteria, answerMethod) {
            var that = this;
            this._configurationCriteria = [];
            this._allConfigCriteria = [];
            this._variantWithSelection = [];
            ...
            for (var i = 0; i < newConfigCriteria.length; i++) {
                ...
                if (newConfigCriteria[i].State) {
                    ...
                    var criteriaData = {};
                    if (featureType === 'Value' || featureType === 'Zone Variant Value') {
                        ...
                        if (
                            this._parameterMandStatusPrevious[criteriaData.Id] !==
                            this._parameterMandStatus[criteriaData.Id]
                        ) {
                            updateFlag = true;
                        }
                    } else {
                        // Variant or Option Group or featureType undefined (first open)
                        criteriaData.Id = newConfigCriteria[i].Id;
                        criteriaData.State = this._cacheIdWithState[newConfigCriteria[i].Id];
                    ...
                    if (
                        this._allConfigCriteria[i].State === ConfiguratorVariables.Required ||
                        this._allConfigCriteria[i].State === ConfiguratorVariables.Chosen ||
                        this._allConfigCriteria[i].State === ConfiguratorVariables.Default ||
                        this._allConfigCriteria[i].State === ConfiguratorVariables.Selected
                    ) {
                    ...
                    if (
                        UWA.is(newConfigCriteria[i].MaxValue, 'number') &&
                        UWA.is(newConfigCriteria[i].MinValue, 'number')
                    ) {
                        ...
                        this._cacheIdWithValue[newConfigCriteria[i].Id] = newConfigCriteria[i];
                        if (
                            this._parameterMandStatusPrevious[newConfigCriteria[i].Id] !==
                            this._parameterMandStatus[newConfigCriteria[i].Id]
                        ) {
                            this.setUpdateRequired(newConfigCriteria[i].Id, true);
                        }
                    }
                }
            }
            this._cachePreviousCriteria = this._cacheIdWithState;
            this._parameterMandStatusPrevious = this._parameterMandStatus;
        ...
        isAnOption: function (id) {
            ...
            if (UWA.is(this._cacheOption[id])) return true;
            return false;
        ...
        setSelectionMode: function (newMode) {
            ...
            if (
                this.getCompletenessStatus() === ConfiguratorVariables.Hybrid &&
                this._appRulesParams.selectionMode === ConfiguratorVariables.selectionMode_Build
            )
                this.setCompletenessStatus(ConfiguratorVariables.Partial);
            else if (
                this.getCompletenessStatus() === ConfiguratorVariables.Partial &&
                this._appRulesParams.selectionMode === ConfiguratorVariables.selectionMode_Refine
            )
        ...
        setRulesActivation: function (newMode) {
            if (newMode === ConfiguratorVariables.str_false) {
                for (var i = 0; i < this._configurationCriteria.length; i++) {
                    if (
                        this._configurationCriteria[i].State !== ConfiguratorVariables.Chosen &&
        ...
        getFeatureIdWithMandStatus: function (id) {
            ...
            let feature = this.getDictionary().features.find((feature) => feature.ruleId === id);
            if (!UWA.is(feature)) {
                return false;
            }
            var variantState = this.getStateWithId(id),
                mandatory =
                    (feature.type === 'Parameter' &&
                        this._parameterMandStatus[feature.ruleId] &&
                        variantState !== ConfiguratorVariables.Chosen) ||
                    feature.selectionCriteria === 'Mandatory' ||
                    feature.selectionCriteria === true ||
                    variantState === ConfiguratorVariables.Required ||
                    feature.selectionCriteria === 'Primary',
                that = this,
            ...
            feature.options.forEach((option) => {
                ...
                if (
                    ((selectionmode === ConfiguratorVariables.feature_SelectionMode_Single ||
                        selectionmode === ConfiguratorVariables.feature_SelectionMode_Multiple ||
                        selectionmode === ConfiguratorVariables.feature_SelectionMode_EnforceMultiple) &&
                        (state === ConfiguratorVariables.Chosen ||
                            state === ConfiguratorVariables.Selected ||
                            state === ConfiguratorVariables.Required ||
                            state === ConfiguratorVariables.Default)) ||
        ...
        updateFeatureIdWithChosenStatus: function (id) {
            var that = this,
                WithChosenStatus = false,
                feature = this.getDictionary().features.find((elt) => elt.ruleId === id),
                selectionmode = this.getFeatureSelectionMode(feature.ruleId);
                ...
                feature.options.forEach((option) => {
                    ...
                    if (
                        ((selectionmode === ConfiguratorVariables.feature_SelectionMode_Single ||
                            ...
                            state === ConfiguratorVariables.Chosen) ||
                        (selectionmode === ConfiguratorVariables.feature_SelectionMode_Dismiss &&
                            state === ConfiguratorVariables.Dismissed)
                    )
                        WithChosenStatus = true;
                });
        ...
        setDictionary: function (newDictionary) {
            if (newDictionary) {
                let chosenCrit = this.getChosenConfigurationCriteriaAsMap(true);
                ...
                for (var i = 0; i < Features.length; i++) {
                    ...
                    this._cacheOptionIdWithFeatureId[feature.ruleId] = feature.ruleId;
                    for (var j = 0; j < feature.options.length; j++) {
                        this._cacheOptionIdWithFeatureId[feature.options[j].ruleId] = feature.ruleId;
                        this._cacheOptionIdWithFeatureId[feature.options[j].rel_id] = feature.ruleId;
                    }
                    // init feature selection mode
                    this.resetFeatureSelectionMode(feature, chosenCrit);
                }
            }
        },
        ...
        setRules: function (dictionary) {
            ...
            for (var i = 0; i < flatRules.length; i++) {
                this._cacheRuleIdWithName[flatRules[i].id] =
                    flatRules[i].attributes._title || flatRules[i].attributes._name || '';
            }
        },
        _getCompatibleDicoInV1: function (dictionary) {
            ...
            var flatVGs =
                dico_inside.variabilityGroups && dico_inside.variabilityGroups.member
                    ? dico_inside.variabilityGroups.member
                    : [];
            var flatParamaters =
                dico_inside.parameters && dico_inside.parameters.member ? dico_inside.parameters.member : [];
            var zoneVariants =
                dico_inside.zoneVariants && dico_inside.zoneVariants.member ? dico_inside.zoneVariants.member : [];
            this._withZoneVariants = zoneVariants.length > 0;
            for (let i = 0; i < flatVariants.length; i++) {
                this.addMinifiedVariantAndValueInRes(features, flatVariants[i], 'Single');
            }

            for (let j = 0; j < zoneVariants.length; j++) {
                this.addMinifiedZoneVariantsAndValues(features, zoneVariants[j], 'Single');
            }

            for (let i = 0; i < flatVGs.length; i++) {
            ...
            for (let i = 0; i < flatParamaters.length; i++) {
        ...
        addMinifiedParametersInRes: function (features, parameter) {
            if (parameter.kind === 'instance') {
                ...
                feature.defaultValue = parameter.attributes._defaultValue
                    ? parameter.attributes._defaultValue.inputvalue
                    : undefined;
                feature.defaultUnit = parameter.attributes._defaultValue
                    ? parameter.attributes._defaultValue.inputunit
                    : '';
        ...
        addMinifiedVariantAndValueInRes: function (features, variant, defaultSelectionType) {
            if (variant.kind === 'instance') {
                ...
                feature.p_rel_id = variant.id; //Connection relID between ModelVersion and Variant
                ...
                for (var i = 0; i < values.length; i += 1) {
                    ...
                    option.sequenceNumber = values[i].attributes._sequenceNumber
                        ? values[i].attributes._sequenceNumber
                        : 1;
        ...
        addMinifiedZoneVariantsAndValues: function (features, zoneVariant, defaultSelectionType) {
            var feature = {};
            feature.ruleId = zoneVariant.id; // physical id of zone variant
            feature.options = [];
            feature.name = zoneVariant.attributes._name || '';
            feature.displayName = zoneVariant.attributes._title || '';
            feature.description = zoneVariant.attributes._description || '';
            feature.type = zoneVariant.attributes._type ? zoneVariant.attributes._type : 'Zone Variant';
            feature.selectionType = zoneVariant.attributes._selectionType || defaultSelectionType;
            feature.image = zoneVariant.attributes._image || '';

            feature.optionPhysicalIds = [];
            var values = zoneVariant.values.member;
            for (var i = 0; i < values.length; i++) {
                var option = {};
                option.ruleId = values[i].id; // note: ruleId term not appropriate : tobe changed
                option.rel_id = values[i].rel_id; // TODO: CfgBase to stop usage of rel_id
                option.name = values[i].attributes._name ? values[i].attributes._name : '';
                option.displayName = values[i].attributes._title
                    ? values[i].attributes._title
                    : values[i].attributes._name;
                option.description = values[i].attributes._description || '';
                option.image = values[i].attributes._image ? values[i].attributes._image : '';
                option.type = values[i].attributes._type ? values[i].attributes._type : ' Zone Variant Value';

                feature.optionPhysicalIds.push(values[i].id);
                feature.options.push(option);
            }
            features.push(feature);
        },
        ...
        addMinifiedVGInRes: function (features, variant, defaultSelectionType) {
            ...
            feature.p_rel_id = variant.rel_id;
            feature.sequenceNumber = variant.attributes._sequenceNumber ? variant.attributes._sequenceNumber : 1;
            feature.commercial = variant.attributes._commercial ? variant.attributes._commercial : false;
            feature.options = [];
            feature.name = variant.attributes._name || '';
        ...
        isConfigWithZoneVariants: function () {
            return this._withZoneVariants;
        },

        setReadOnlyFlag: function (booleanValue) {
            this._readOnly = booleanValue === true;
        },
        ...
        getAppRulesParam: function () {
            return this._appRulesParams;
        },

        getFeatureIdWithOptionId: function (Id) {
            return this._cacheOptionIdWithFeatureId[Id];
        },
        getCacheOptionsIdWithFeatureId: function () {
            return this._cacheOptionIdWithFeatureId;
        },
        setRefineSelectionModeForAllFeatures: function (newMode) {
            const keys = Object.keys(this._listOfRefineSelectionMode);
        ...
        setStateWithId: function (Id, NewState, NumericalValue) {
            ...
            if (NewState === ConfiguratorVariables.Range) return 0;
            ...
            for (var i = 0; i < this._configurationCriteria.length; i++) {
                if (this._configurationCriteria[i].Id === Id) {
                    ...
                    else if (UWA.is(this._configurationCriteria[i].NumericalValue))
                        delete this._configurationCriteria[i].NumericalValue;
                    set = true;
                    break;
                }
            }
        ...
        getValueWithId: function (Id) {
            if (this._cacheIdWithValue[Id] && this.getRulesMode() === 'No rule applied') {
        ...
        setValueWithId: function (Id, value) {
            if (this._cacheIdWithValue[Id]) {
                if (UWA.is(value, 'number')) this._cacheIdWithValue[Id].NumericalValue = value;
        ...
        CalculateCompletenessStatus: function () {
            ...
            if (this.hasMultiSelectVariantValues(undefined, false, ['VariabilityGroup'])) {
                //passing excludedTypes - which should not emptying the option groups
            ...
            for (var i = 0; i < features.length; i++) {
                if (
                    features[i].selectionCriteria === 'Mandatory' ||
                    this.getFeatureIdWithMandStatus(features[i].ruleId)
                ) {
                    //Only consider the Must features
                    ...
                    for (var j = 0; j < features[i].options.length; j++) {
                        if (
                            cacheConfigCriteria[features[i].options[j].ruleId] === ConfiguratorVariables.Chosen ||
                            ...
                            cacheConfigCriteria[features[i].options[j].ruleId] === ConfiguratorVariables.Dismissed
                        ) {
                    ...
                    if (
                        this._appRulesParams.selectionMode === ConfiguratorVariables.selectionMode_Refine &&
                        cacheFeaturesIdWithSelections[features[i].ruleId].SelectionsNb === 0
                    ) {
                        cacheFeaturesIdWithSelections[features[i].ruleId].SelectionsNb =
                            features[i].options.length - cacheFeaturesIdWithSelections[features[i].ruleId].UserRejectNb;
                    }
                }
            }

            for (var feature in cacheFeaturesIdWithSelections) {
                if (cacheFeaturesIdWithSelections.hasOwnProperty(feature)) {
                    if (
                        !hybrid &&
                        cacheFeaturesIdWithSelections[feature].SelectionsNb > 1 &&
                        cacheFeatureTypes[feature] === 'Single'
                    ) {
                        hybrid = true;
                    }
        ...
        getFeatureDisplayNameWithId: function (id) {
            ...
            for (var i = 0; i < features.length; i++) {
                if (features[i].ruleId === id) return features[i].displayName;
                for (var j = 0; j < features[i].options.length; j++) {
                    if (features[i].options[j].ruleId === id) return features[i].displayName;
                }
            }
        },

        getOptionDisplayNameWithId: function (id) {
            const features = this.getDictionary().features;

            const idStr = Array.isArray(id) ? id[0] : id;

            for (let i = 0; i < features.length; i++) {
                for (let j = 0; j < features[i].options.length; j++) {
                    if (features[i].options[j].ruleId === idStr) return features[i].options[j].displayName;
                }
            }
        },
        ...
        getXMLExpression: function (options) {
            ...
            var json = {
                ...
                configModel: this,
                includeDisplayName:
                    options && options.includeDisplayName !== undefined ? !!options.includeDisplayName : true
            };
                    ...
                    if (
                        ((selectionmode === ConfiguratorVariables.feature_SelectionMode_Single ||
                            ...
                            (state === ConfiguratorVariables.Required ||
                                ...
                                state === ConfiguratorVariables.Selected)) ||
                        (selectionmode === ConfiguratorVariables.feature_SelectionMode_Dismiss &&
                            state === ConfiguratorVariables.Incompatible)
                    )
                        WithRules = true;
                });
        ...
        updateFeatureIdWithCommercialStatus: function (id) {
            var feature = this.getDictionary().features.find((elt) => elt.ruleId === id);
        ...
        updateFeatureIdWithUserSelection: function (id) {
            var that = this,
        ...
        getValidateVariant: function (id) {
        ...
        getVariantForDiagnosis: function () {
        ...
        isConfigCriteriaUpdated: function () {
        ...
        emptyDismissedOptions: function (feature) {
            return _unselectCriteria(
                this,
                feature,
                this.getDismissedConfigurationCriteria(),
                1,
                ConfiguratorVariables.Unselected
            );
        },
        ...
        emptyChosenOptions: function (feature) {
            return _unselectCriteria(
                this,
                feature,
                this.getChosenConfigurationCriteria(),
                1,
                ConfiguratorVariables.Unselected
            );
        },
        ...
        switchExcludeOptions: function (feature) {
            return _unselectCriteria(
                ...
                this.getChosenConfigurationCriteria(),
                1,
                ConfiguratorVariables.Dismissed
            );
        },
        ...
        switchAllUnselectedToExclude: function () {
            ...
            if (UWA.is(features, 'array')) {
                for (var i = 0; i < features.length; i++) {
                    if (
                        this._featureLevelSelectionMode[features[i].ruleId] !==
                            ConfiguratorVariables.feature_SelectionMode_Parameter ||
                        this._featureLevelSelectionMode[features[i].ruleId] !==
                            ConfiguratorVariables.feature_SelectionMode_Dismiss
                    ) {
                    ...
                    if (
                        ...
                        if (!this.hasDismissedOptions(features[i], false)) {
                            if (features[i].selectionType === ConfiguratorVariables.feature_SelectionMode_Multiple) {
                                this._featureLevelSelectionMode[features[i].ruleId] =
                                    ConfiguratorVariables.feature_SelectionMode_Multiple;
                            } else {
                                this._featureLevelSelectionMode[features[i].ruleId] =
                                    ConfiguratorVariables.feature_SelectionMode_Single;
                            }
                        }
                    }
                }
        ...
        hasMultiSelectVariantValues: function (feature, selected, excludedTypes) {
            if (feature) {
                return _checkCriteriaState(
                    this,
                    feature,
                    [ConfiguratorVariables.Chosen, ConfiguratorVariables.Default, ConfiguratorVariables.Required],
                    [ConfiguratorVariables.Dismissed],
                    2
                );
            }
            return _unselectCriteria(
                this,
                feature,
                this.getChosenConfigurationCriteria(selected),
                2,
                undefined,
                excludedTypes
            );
        ...
        hasIncompactibleOptions: function (feature) {
            var hasIncompatible = _checkCriteriaState(
                this,
                feature,
                [ConfiguratorVariables.Incompatible],
                [ConfiguratorVariables.Chosen],
                1
            );
            var isRequired = _checkCriteriaState(
                this,
                feature,
                [ConfiguratorVariables.Incompatible, ConfiguratorVariables.Required],
                [],
                feature.options.length
            );
            return hasIncompatible && !isRequired;
        },
        hasSingleSelectVariantValues: function (feature) {
            return _checkCriteriaState(
                this,
                feature,
                [ConfiguratorVariables.Chosen, ConfiguratorVariables.Default, ConfiguratorVariables.Required],
        ...
        hasChosenOptions: function (feature, selected, chosenCrit) {
            if (!chosenCrit) chosenCrit = this.getChosenConfigurationCriteria(selected);

            return _unselectCriteria(this, feature, chosenCrit, 1);
        },
        ...
        getFeatureSelectionMode: function (feature) {
            ...
            if (UWA.is(feature, 'string')) return this._featureLevelSelectionMode[feature];
            if (UWA.is(feature, 'object')) return this._featureLevelSelectionMode[feature.ruleId];
        },
        // replace param feature with id
        setFeatureMultiSelectionMode: function (feature) {
            if (
                this._featureLevelSelectionMode[feature.ruleId] ===
                    ConfiguratorVariables.feature_SelectionMode_Single ||
        ...
        resetFeaturesForCompleteMode: function (confirmUserPrompt) {
            ...
            if (UWA.is(features, 'array')) {
                for (var i = 0; i < features.length; i++) {
                    let feature = features[i];
                    let selectMode = this.getFeatureSelectionMode(feature);
                    if (selectMode === ConfiguratorVariables.feature_SelectionMode_Dismiss) {
                        let hasDismissedOpts = this.hasDismissedOptions(feature);
                        if (hasDismissedOpts) {
                            confirmUserPrompt().then(() => {
                                this.emptyDismissedOptions(feature);
                            });
                        }
                        this._featureLevelSelectionMode[feature.ruleId] = feature.selectionType;
                    } else if (
                        feature.selectionType === ConfiguratorVariables.feature_SelectionMode_Single &&
                        this.hasMultiSelectVariantValues(feature)
                    ) {
                        this.emptyMultiSelectedVariants(feature);
                    }
                }
            }
        },
        ...
        resetFeatureSelectionMode: function (feature, chosenCrit) {
            if (feature.selectionType === ConfiguratorVariables.feature_SelectionMode_Parameter) {
            ...
            } else if (this.hasChosenOptions(feature, true, chosenCrit)) {
                ...
                this.hasDismissedOptions(feature, true) &&
                this._appFunc.selectionMode_Refine === ConfiguratorVariables.str_yes
            ) {
                this._featureLevelSelectionMode[feature.ruleId] = ConfiguratorVariables.feature_SelectionMode_Dismiss;
            } else {
                this._featureLevelSelectionMode[feature.ruleId] = feature.selectionType;
        ...
        isRuleSelectionEnabled: function () {
            return this._enableRuleSelection;
        },
                ...
                if (UWA.is(criteria.Criterion)) {
                    if (UWA.is(criteria.Value)) {
                        ...
                        if (UWA.is(criteria.Value, 'string')) {
                            if (criteria.Value === 'ANY_NUMBER') {
                                if (answerDataType === 'configurationCriteria') {
                                    configCriteriaData.Id = criteria.Criterion;
                                    configCriteriaData.State = criteria.State ? criteria.State : '';
                                    var paramValue = that.getValueWithId(criteria.Criterion);
                                    var state =
                                        that.getStateWithId(criteria.Criterion) === ConfiguratorVariables.Chosen
                                            ? ConfiguratorVariables.Chosen
                                            : '';
                                    if (paramValue) {
                                        configCriteriaData.NumericalValue = paramValue.NumericalValue;
                                        configCriteriaData.State = configCriteriaData.State || state;
                                    }
                        ...
                        } else if (UWA.is(criteria.Value, 'number')) {
                            // numerical parameter
                            configCriteriaData.Id = criteria.Criterion;
                        ...
                        } else if (UWA.is(criteria.Value, 'boolean')) {
                            //options OR Mandatory criterion OR No Value to be selected
                            featureType = that.getFeatureType(criteria.Criterion);
                            if (
                                ['VariabilityGroup', 'Option', 'Variant', 'Parameter', 'Zone Variant'].includes(
                                    featureType
                                )
                            ) {
                                if (featureType === 'Parameter') {
                                    if (criteria.Value === false) {
                                        if (criteria.State === 'Incompatible') {
                                            that._parameterMandStatus[criteria.Criterion] = true;
                                            // } else if(criteria.State === 'Required') {
                                        } else {
                                            that._parameterMandStatus[criteria.Criterion] = false;
                                        }
                                    }
                                } else {
                                    configCriteriaData.Id = criteria.Criterion;
                                    ...
                                    if (answerDataType === 'configurationCriteria') {
                                        ...
                                        else if (criteria.Value === false && criteria.State === 'Required')
                                            // case of Variant, solver answer is Value{false} Incompatible
                                            configCriteriaData.State = 'Incompatible';
                                        else configCriteriaData.State = criteria.State;
                                    }
                                }
                            }
                        }
                    } else {
                        // parameter without a value, and check if it has min and max value
                        if (answerDataType === 'configurationCriteria') {
                            configCriteriaData.Id = criteria.Criterion;
                            if (UWA.is(criteria.ValueMin) && UWA.is(criteria.ValueMax)) {
                                configCriteriaData.MinValue = criteria.ValueMin;
        ...
        convertToNewSolverFormat: function (selectedCriteria) {
            ...
            selectedCriteria.forEach((criteria) => {
                if (criteria.State === 'Chosen' || criteria.State === 'Dismissed') {
                    ...
                    } else if (featureType === 'Value' || featureType === 'Zone Variant Value') {
                        // for variant values & options
...
define('DS/ConfiguratorPanel/scripts/Presenters/MultipleValueAutocompletePresenter', [
    'UWA/Core',
    'DS/Handlebars/Handlebars',
    'DS/ConfiguratorPanel/scripts/Models/ConfiguratorVariables',
    ...
    nlsConfiguratorKeys,
    html_MultipleValueAutocompletePresenter
) {
    'use strict';
    ...
    MultipleValueAutocompletePresenter.prototype._init = function (options) {
        const _options = options ? UWA.clone(options, false) : {};
        UWA.merge(this, _options);
        this._subscribeEvents();
        this._initDivs();
        this._render();
        ...
        this.modelEvents.subscribe(
            ...
            function (data) {
                that._criteriaselection.setProperties({
                    sort: { attribute: data.sortAttribute, order: data.sortOrder }
                });
            }
        );
    };
    ...
    MultipleValueAutocompletePresenter.prototype._render = function () {
        ...
        this._criteriaselection = this.criteriaSelection;
        if (!this._criteriaselection)
            this._criteriaselection = new CriteriaSelectionEditor({ context: this.variant.ruleId });
        ...
        this._criteriaselection.addEventListener('show-criteria', function () {
        ...
        this._criteriaselection.addEventListener('show-state-tooltip', function (e) {
            ...
            if (
                that.configModel.getRulesActivation() === ConfiguratorVariables.str_true &&
                that.configModel.isConflictingOption(optionId)
            ) {
        ...
        this._criteriaselection.setProperties({
            ...
            enableEasySelection:
                this.pcDataUtil.isMultiValue(this.variant.ruleId) ||
            ...
            sort: {
                attribute: this.configModel.sortPreference
                    ? this.configModel.sortPreference.sortAttribute
                    : 'sequenceNumber',
                order: this.configModel.sortPreference ? this.configModel.sortPreference.sortOrder : 'ASC'
            }
        });

        this._criteriaselection.inject(this._container);
    };

    /********************************FUNCTIONALITIES - CALL SOLVER BASED ON RULES SELECTION******************************/

    MultipleValueAutocompletePresenter.prototype.callSolver = function () {
        this.modelEvents.publish({
    ...
    MultipleValueAutocompletePresenter.prototype.enforceRequired = function (data) {
        if (
            data &&
            data.answerMethod === 'solveAndDiagnose' &&
            this.configModel.getRulesMode() === 'RulesMode_SelectCompleteConfiguration'
        ) {
    ...
    MultipleValueAutocompletePresenter.prototype._updateImage = function (selectedItems) {
        ...
        else this.imageContainer.removeClassName('cfg-image-selected');

        if (option && option.image !== '') this.imageContainer.style.backgroundImage = 'url("' + option.image + '")';
...
define('DS/ConfiguratorPanel/scripts/Presenters/CriteriaSelectionView', [
...
], function (UWA, Abstract, Handlebars, ConfiguratorVariables) {
    'use strict';
    //build Criteria Selection view

    class Badge {
        constructor(title = '', image = null, hasConflict = false, icon = null, sequenceNumber = null) {
            this._title = title;
            this._icon = icon;
            this._hasConflict = hasConflict;
            this._image = image;
            this._sequenceNumber = sequenceNumber;
            this._buildContent(title, image, hasConflict, icon);
        }
        ...
        update(title = '', image = null, hasConflict = false, icon = null) {
            if (image) {
                this._image.src = image;
                this._image.show();
            } else this._image.hide();

            this._title.textContent = title;
            this._title.title = title;

            if (hasConflict) {
                this._icon.addClassName('wux-ui-3ds-rule-status-conflict item-state-conflict');
                this._icon.show();
            } else if (icon) {
                this._icon.addClassName(`wux-ui-3ds-${icon}`);
                this._icon.show();
            } else {
                this._icon.hide();
            }
        ...
        // Method to render the badge HTML
        _buildContent(title, image, hasConflict, icon) {
            this._container = UWA.createElement('div', {
                "class": 'value-badge'
            });
            this._image = UWA.Element('img', {
                "class": 'value-image',
                src: image
            }).inject(this._container);
            this._icon = UWA.createElement('span', {
                "class": 'wux-ui-3ds'
            }).inject(this._container);

            if (hasConflict) {
                this._icon.addClassName('wux-ui-3ds-rule-status-conflict item-state-conflict');
            } else if (icon) this._icon.addClassName(`wux-ui-3ds-${icon}`);
            else this._icon.hide();

            this._title = UWA.createElement('span', {
                "class": 'value-title',
                title: title,
                text: title
            }).inject(this._container);
        }
        // Method to clean up and destroy the badge
        destroy() {
    ...
    var CriteriaSelectionView = Abstract.inherit({
        ...
        buildView: function () {
            this._valueContainer = UWA.createElement('div', {
                "class": 'value-container multi-value-content'
            });
            this._badges = [];
        },
        ...
        _applyValue: function () {
                if (UWA.is(this.value, 'array')) {
                    if (this.sort.length > 0 && this.sort[0].id == 'sequenceNumber') {
                        if(this.sort[0].order == 'asc'){
                            ...
                            this.value.sort( (val1, val2)=>{
                                return val2.sequenceNumber - val1.sequenceNumber;
                            });
                        }
                    } else if(this.sort.length > 0 && this.sort[0].id == 'tree'){
                        if(this.sort[0].order == 'asc'){
                            this.value.sort( (a,b)=>{
                ...
                this.value.forEach((item, index) => {
                    if (!this._badges[index]) {
                        this._badges.push(new Badge(item.title, item.image, item.hasConflict, item.icon, item.sequenceNumber));
                        this._badges[index].getContent().inject(this._valueContainer);
                    } else this._badges[index].update(item.title, item.image, item.hasConflict, item.icon, item.sequenceNumber);
                });

                for (let i = this.value.length; i < this._badges.length; i++) this._badges[i].destroy();
                // Remove any extra badges beyond the length of this.value
                if (this._badges.length > this.value.length) {
                    this._badges.splice(this.value.length);
                }
            }
        },
        ...
        _applyReferenceValue: function () {
            ...
            if (this.referenceValue) {
                ...
                if (this.referenceValue.length === this.value.length) {
                    let differences = this.referenceValue.filter((obj1) => {
                        return !this.value.some((obj2) => {
                            return obj1 === obj2.id || obj1 === obj2.numericalValue; // numericalValue => Parameter
                        });
                    });
                    if (differences.length === 0) {
                        highlightDiff = false;
...
define('DS/ConfiguratorPanel/scripts/Presenters/VariantComponentPresenter', [
    ...
    VariantComponentPresenter.prototype._init = function (options) {
        ...
        if (this.variant.unit) this.variant.nlsUnit = xUnitLabelLong[this.variant.unit];
    ...
    VariantComponentPresenter.prototype._toggleVariantMultiSelect = function (activate, needConfirm) {
        ...
        return new Promise((resolve) => {
            ...
            if (
                (currSelection === ConfiguratorVariables.feature_SelectionMode_Single &&
                    (!UWA.is(activate) || activate !== false)) ||
                (currSelection === ConfiguratorVariables.feature_SelectionMode_Dismiss &&
                    (!UWA.is(activate) || activate !== false)) ||
                activate === true
            ) {
                //R14 need to confirm ?
                that.modelEvents.publish({
                    event: 'pc-confirm-variant-includeSelect',
                    data: {
    ...
    VariantComponentPresenter.prototype._toggleVariantDismiss = function (activate, needConfirm) {
        ...
        return new Promise((resolve) => {
            that.modelEvents.publish({
                event: 'pc-confirm-variant-excludeSelect',
                data: {
                    feature: that.variant,
                    needConfirm: needConfirm,
                    callback: function (confirmed) {
    ...
    VariantComponentPresenter.prototype._initDivs = function () {
        ...
        options.isMandatory =
            this.variant.selectionCriteria === 'Primary' || this.variant.selectionCriteria === 'Mandatory';
        this._container.innerHTML = template({
            variant: options
        });
    ...
    VariantComponentPresenter.prototype._updateVisibity = function (activated) {
        ...
        var flag = false;
        var conflictOptions = this.configModel.getConflictingFeatures(),
            conflictingFeatures = [];
        ...
        if (
            this.variant.type === 'Parameter' &&
            this.configModel.getStateWithId(this.variant.ruleId) === ConfiguratorVariables.Incompatible
        ) {
            isAllOptionsIncompatible = true;
        } else if (this.variant.options.length > 0) {
            isAllOptionsIncompatible = true;
        ...
        if (!isMand && !isConflicting && isAllOptionsIncompatible) {
            isVisible = false;
        }
        if (isVisible && activated) flag = true;
    ...
    VariantComponentPresenter.prototype._subscribeToEvents = function () {
        ...
        this.modelEvents.subscribe({ event: 'OnConflictIconClick' }, function (data) {
            var conflictOptions = that.configModel.getConflictingFeatures(),
                conflictingFeatures = [];
        ...
        this.modelEvents.subscribe({ event: 'applyDefaultSearch' }, function (data) {
            ...
            for (var i = 0; i < that.variant.options.length; i++) {
                attributesList['value_' + that.variant.options[i].ruleId + '_DisplayName'] =
                    that.variant.options[i].displayName;
            }
                    ...
                    for (var attr in attributesList) {
                        if (Object.prototype.hasOwnProperty.call(attributesList, attr)) {
                            ...
                            for (var j = 0; j < searchValue.length; j++) {
                                if (
                                    AttribValue.toUpperCase().contains(searchValue[j].toUpperCase()) ||
                                    searchValue[j] === ''
                                ) {
                                    matchFound = true;
                                    break;
            ...
            this._selectionModeAction.addEvent('onClick', function () {
                ...
                if (value === nlsConfiguratorKeys.Exclude_criteria) {
                    if (that.variant.selectionType === ConfiguratorVariables.feature_SelectionMode_Single) {
                        that._toggleVariantMultiSelect(false).then(() => {
                            this.setValue(nlsConfiguratorKeys.Include_criteria);
                        });
                    } else if (that.variant.selectionType === ConfiguratorVariables.feature_SelectionMode_Multiple) {
                        that._toggleVariantMultiSelect(true).then(() => {
                            this.setValue(nlsConfiguratorKeys.Include_criteria);
                        });
                    }
                } else {
                    that._toggleVariantDismiss().then(() => {
                        this.setValue(nlsConfiguratorKeys.Exclude_criteria);
                    });
                }
            });
        }
    ...
    VariantComponentPresenter.prototype._setVisibility = function (criteria, flag) {
        ...
        if (criteria) {
            if (this._container.style.display !== 'inline-flex') {
                flag
                    ? this._container.classList.add('animated-variant')
                    : this._container.classList.remove('animated-variant');
        ...
        if (
            this.configModel.getSearchStatus() &&
            values &&
            values.searchValue &&
            values.searchValue.length > 0 &&
            values.searchValue[0] !== ''
        ) {
    ...
    VariantComponentPresenter.prototype._getAutocomplete = function () {
        ...
        if (this.variant.selectionType === 'Parameter') {
            ...
            if (!this.parameterPresenter) this.parameterPresenter = new ParameterPresenter(options);
        } else {
            //Unconstrained mode
            // rely on ConfigModel
    ...
    VariantComponentPresenter.prototype.updateView = function (data) {
        if (this.autocompletePresenter) {
            if (this._selectionModeAction) {
                if (this.configModel.getRulesMode() === ConfiguratorVariables.RulesMode_SelectCompleteConfiguration) {
                    ...
                    this._selectionModeAction.getContent().style.visibility = 'hidden';
                } else {
                    this._selectionModeAction.getContent().style.visibility = 'visible';
        ...
        if ('Optional' === this.variant.selectionCriteria) {
            this._variantLableContainer.setAttribute(
                'is-mandatory',
                this.configModel.getFeatureIdWithMandStatus(this.variant.ruleId)
            );
        }
    };
...
define('DS/ConfiguratorPanel/scripts/Presenters/ConfigGridPresenter', [
    'UWA/Core',
    'DS/CoreEvents/ModelEvents',
    'DS/TreeModel/TreeNodeModel',
    ...
    ConfigGridPresenter.prototype.init = function (modelEvents, applicationChannel, options) {
        ...
        this._dictionary = options.dictionary
            ? options.dictionary
            : options.configModel
              ? options.configModel.getDictionary() || { features: [] }
              : undefined;
        ...
        var typeLabelMap = {
            ...
            'Value': NLS.valueType,
            'Zone Variant': NLS.zoneVariantType,
            'Zone Variant Value': NLS.zoneVariantValueType,
            ...
            _columns: [
                {
                    ...
                    getCellIcons: function (cellInfos) {
                        if (cellInfos.nodeModel && cellInfos.nodeModel.getIcons) {
                            if (
                                cellInfos.nodeModel.options.id &&
                                that._pcDataUtil.isMandatory(cellInfos.nodeModel.options.id)
                            ) {
                ...
                },
                {
                    ...
                    getCellValue: function (cellInfos) {
                        if (
                            ['Variant', 'VariabilityGroup', 'Parameter'].includes(
                                cellInfos.nodeModel.getAttributeValue('type')
                            )
                        ) {
        ...
        if (this._configModel.getAppFunc().selectionMode_Refine === ConfiguratorVariables.str_yes) {
            ...
            this.options._columns.push({
                ...
                getCellValue: function (cellInfos) {
                    if (cellInfos.nodeModel) {
                        if (
                            ['Variant', 'VariabilityGroup', 'Zone Variant'].includes(
                                cellInfos.nodeModel.getAttributeValue('type')
                            )
                        ) {
                            var currentState = NLS_ConfiguratorPanel.Include_criteria; //Exclude_criteria
                            var currentSelectionMode = that._configModel.getFeatureSelectionMode(
                                cellInfos.nodeModel.getAttributeValue('ruleId')
                            );
                            if (currentSelectionMode === ConfiguratorVariables.feature_SelectionMode_Dismiss) {
                                currentState = NLS_ConfiguratorPanel.Exclude_criteria;
                            }
                            if (
                                cellInfos.nodeModel.getAttributeValue('selectionCriteria') === 'Primary' &&
                                !that._configModel.isPrimaryEditionEnabled()
                            ) {
                                return {
                                    label: currentState
                                };
    ...
    ConfigGridPresenter.prototype._subscribeEvents = function () {
        ...
        that._eventManager.subscribe(that._modelEvents, 'OnExpandAll', function () {
            that._applyExpandAll();
        });

        that._eventManager.subscribe(that._modelEvents, 'OnCollapseAll', function () {
            that._applyCollapseAll();
        });
        ...
        that._eventManager.subscribe(that._modelEvents, 'show-maching-pc-action', function (activate) {
            ...
            that._dataGridView.elements.statusBar.updateNodeModel('statusLoading', {
                visibleFlag: that._matchingPCActivated
            });
        });
    ...
    ConfigGridPresenter.prototype._applyExpandAll = function () {
        this._treeDocument.prepareUpdate();
        this._treeDocument.expandAll();
        this._treeDocument.pushUpdate();
    };

    ConfigGridPresenter.prototype._applyCollapseAll = function () {
        this._treeDocument.prepareUpdate();
    ...
    ConfigGridPresenter.prototype._initializeDataGridView = function () {
        ...
        this._dataGridView.onReady(() => {
            ...
            this._dataGridView.elements.statusBar.updateNodeModel('totalRows', {
                grid: {
                    data: NLS.totalItemsLabel + ' : ' + count
                }
            });
        });
        ...
        that._dataGridView.addEventListener('click', function (e, cellInfos) {
            if (cellInfos && cellInfos.nodeModel && cellInfos.nodeModel.options.id) {
                ...
                if (href === '#selectionMode') {
                    if (
                        that._configModel.getFeatureSelectionMode(cellInfos.nodeModel.getAttributeValue('ruleId')) ===
                        ConfiguratorVariables.feature_SelectionMode_Dismiss
                    ) {
                        that._updateSelectionMode({
                            id: id,
                            selectionMode: cellInfos.nodeModel.getAttributeValue('selectionType')
                        });
                    } else {
                        that._updateSelectionMode({
                            id: id,
    ...
    ConfigGridPresenter.prototype._updateTooltip = function (id, value) {
        ...
        var that = this;
        if (
            configModel.getRulesActivation() === ConfiguratorVariables.str_true &&
            configModel.isConflictingOption(optionId)
        ) {
            message = this._pcDataUtil._getConflictingMessage(optionId);
        } else {
            message = NLS_ConfiguratorPanel.get('Loading');
            this._pcDataUtil
                ._getReasonMessage(optionId)
                .then((msg) => that._criteriaSelectorUtil.updateTooltip(id, msg));
        }
        // currentNode.getModelEvents().publish({ 'event' : 'state-message-update', data : message });
    ...
    ConfigGridPresenter.prototype.callSolverOnSelectedIDs = function (id) {
        ...
        if (this._configModel && modelEvent) {
            if (this._configModel.getRulesActivation() === ConfiguratorVariables.str_true) {
                ...
                modelEvent.publish({
                    event: 'SolverFct_CallSolveOnSelectedIDsMethodOnSolver',
                    data: { idsToDiagnose: idsToDiagnose }
                });
            } else {
                this.updateFilters();
                modelEvent.publish({ event: 'solveAndDiagnoseAll_SolverAnswer' });
    ...
    ConfigGridPresenter.prototype._loadModel = function () {
        ...
        if (this._dictionary) {
            ...
            if (UWA.is(this._dictionary.features, 'array')) {
                if (this._configModel.isConfigWithZoneVariants()) {
                    zoneVariantNode = new TreeNodeModel({
                        label: NLS.zoneVariantType,
                        icons: [],
                        grid: {
                            type: 'Zone Variant Root'
                        }
                    });
                }
                this._dictionary.features.forEach(function (feature) {
                    var featureNode = that._createTreeNode(feature);
                    if (feature.type === 'Zone Variant') {
                        if (UWA.is(zoneVariantNode)) {
                            zoneVariantL.push(feature);
                            zoneVariantNode.addChild(featureNode);
                        }
                    } else {
                        if (featureNode) {
                ...
                if (UWA.is(zoneVariantNode)) {
                    zoneVariantNode.updateOptions({
                        label: NLS.zoneVariantType + ' ( ' + zoneVariantL.length + ' ) '
                    });
                    this._treeDocument.addChild(zoneVariantNode);
                }
            }
            this._treeDocument.pushUpdate();
            this._isLoaded = true;
                ...
                for (var optionKey in contentOptions) {
                    if (contentOptions.hasOwnProperty(optionKey)) {
                        if (optionKey === 'options') {
                            contentOptions[optionKey].forEach((contentOption) => {
                                let optionGridValue = [];
                                for (var contentOptionKey in contentOption) {
                                    if (contentOption.hasOwnProperty(contentOptionKey))
                                        optionGridValue[contentOptionKey] = contentOption[contentOptionKey];
                                }
                                childrenNode.push(
                                    new TreeNodeModel({
                                        id: contentOption.ruleId,
                                        label: contentOption.displayName,
                                        icons: contentOption.image ? [contentOption.image] : [],
                                        grid: optionGridValue
                                    })
                                );
                            });
    ...
    ConfigGridPresenter.prototype.refresh = function (forceUpdate) {
        ...
        if (this._dataGridView) {
            ...
            this._dataGridView.layout.setColumnVisibleFlag(
                'selectionMode',
                this._configModel.getRulesMode() !== ConfiguratorVariables.RulesMode_SelectCompleteConfiguration
            );
        }
    ...
    ConfigGridPresenter.prototype.loadConfigurationInfo = function (pcDetails, configurations) {
        if (pcDetails && pcDetails.id) {
            ...
            this._criteriaSelectorUtil = new CriteriaSelectorUtil({
                configModel: this._configModel,
                modelEvents: this._modelEvents,
                pCDataUtil: this._pcDataUtil,
                dictionaryDataUtil: this._dictionaryUtil
            });
    ...
    ConfigGridPresenter.prototype._createColumnForPC = function (pcObj, pcStatusInfo) {
        ...
        var columnOptions = {
            ...
            typeRepresentation: 'string', //'selectionStateType',
            ...
            getCellValue: function (cellInfos) {
                ...
                if (
                    [
                        ...
                        'Option',
                        'Zone Variant Value'
                    ].includes(cellInfos.nodeModel.getAttributeValue('type'))
                ) {
                    return '';
                } else if (cellInfos.nodeModel.options.grid.type === 'Parameter') {
                    return configModel.getValueWithId(cellInfos.nodeModel.options.id)
                        ? configModel.getValueWithId(cellInfos.nodeModel.options.id).NumericalValue
                        : null;
                }
                state = configModel.getStateWithId(cellInfos.nodeModel.options.id);
                if (
                    configModel.getRulesActivation() === ConfiguratorVariables.str_true &&
                    configModel.isConflictingOption(cellInfos.nodeModel.options.id)
                ) {
            ...
            onCellRequest: function (cellInfos) {
                let component;
                if (
                    cellInfos.nodeModel.getAttributeValue('type') === 'Variant' &&
                    cellInfos.nodeModel.getAttributeValue('selectionCriteria') === 'Primary'
                ) {
                    component = that._dataGridView.reuseCellContent('criteriaSelectionView');

                    that._criteriaSelectorUtil.updateViewComponent(component, cellInfos.nodeModel.options.id);
                } else if (
                    ['Variant', 'VariabilityGroup', 'Zone Variant'].includes(
                        cellInfos.nodeModel.getAttributeValue('type')
                    )
                ...
                } else if ('Parameter' === cellInfos.nodeModel.getAttributeValue('type')) {
            ...
            getCellEditableState: function (cellInfos) {
                if (
                    ['Value', 'Option', 'Zone Variant Value', 'Zone Variant Root'].includes(
                        cellInfos.nodeModel.getAttributeValue('type')
                    )
                ) {
                    return false;
            ...
            getCellSemantics: function (cellInfos) {
                ...
                if (cellInfos.nodeModel && cellInfos.nodeModel.getAttributeValue('type') === 'Parameter') {
                    options.minValue = that._configModel.getValueWithId(
                        cellInfos.nodeModel.getAttributeValue('ruleId')
                    ).MinValue;
                    options.maxValue = that._configModel.getValueWithId(
                        cellInfos.nodeModel.getAttributeValue('ruleId')
                    ).MaxValue;
            ...
            setCellValue: function (cellInfos, value) {
                if (
                    cellInfos.nodeModel &&
                    cellInfos.nodeModel.getAttributeValue('type') === 'Parameter' &&
                    UWA.is(value, 'number')
                ) {
    ...
    ConfigGridPresenter.prototype._updateSelectionMode = function (options) {
        ...
        if (
            options.selectionMode === ConfiguratorVariables.feature_SelectionMode_Single ||
            options.selectionMode === ConfiguratorVariables.feature_SelectionMode_Multiple
        ) {
            if (currentSelectionMode === ConfiguratorVariables.feature_SelectionMode_Dismiss) {
                this._modelEvents.publish({
                    event: 'pc-confirm-variant-includeSelect',
                    data: {
                        ...
                        callback: function (confirmed) {
                            var isSolverCalled = false;
                            if (confirmed) {
                                if (that._configModel.switchIncludeOptions(criteria)) {
                                    //emptyDismissedOptions
                                    isSolverCalled = true;
                                    that.callSolver();
                                }
            ...
            if (currentSelectionMode != ConfiguratorVariables.feature_SelectionMode_Multiple) {
                this._modelEvents.publish({
                    event: 'pc-confirm-variant-multiSelect',
                    data: {
                        feature: criteria,
                        needConfirm: true,
                        callback: function (confirmed) {
                    ...
                    onCellRequest: (cellInfos) => {
                        if (cellInfos.nodeModel) {
                            if (
                                ...
                                let component = this._dataGridView.reuseCellContent('criteriaSelectionView');
                                var value = cellInfos.cellModel;
                                if (UWA.is(value, 'array')) {
                                    component.setProperties({ value: value });
                                } else if (value) {
                                    component.setProperties({ value: [value] });
                                } else {
...
define('DS/ConfiguratorPanel/scripts/Presenters/VariantListPresenter', [
    'UWA/Core',
    'DS/Handlebars/Handlebars',
    ...
    Handlebars,
    Expander,
    VariantComponentPresenter,
    ConfiguratorVariables,
    nlsConfiguratorKeys,
    html_VariantListPresenter
) {
    'use strict';
    ...
    VariantListPresenter.prototype._render = function () {
        let dico = this.configModel.getDictionary();
        let featureList = dico.features ? dico.features : [];
        this.configModel.setVariants(featureList.length);
        this.variantCompList = [];
        this.zoneVariantList = [];
        var zoneElement = UWA.createElement('div', { "class": 'config-editor-zone-variant-list-container' });

        this.docFragment = document.createDocumentFragment();
        for (let i = 0; i < featureList.length; i++) {
            if (featureList[i].type === 'Zone Variant') {
                this.zoneVariantList.push(
                    new VariantComponentPresenter({
                        variant: featureList[i],
                        parentContainer: zoneElement,
                        configModel: this.configModel,
                        modelEvents: this.modelEvents,
                        version: this.version,
                        pcDataUtil: this.pcDataUtil,
                        criteriaSelection: this.criteriaSelections
                            ? this.criteriaSelections[featureList[i].ruleId]
                            : undefined
                    })
                );
            } else {
                this.variantCompList.push(
                    new VariantComponentPresenter({
                        variant: featureList[i],
                        parentContainer: this.docFragment,
                        configModel: this.configModel,
                        modelEvents: this.modelEvents,
                        version: this.version,
                        pcDataUtil: this.pcDataUtil,
        ...
        if (this.configModel.isConfigWithZoneVariants()) {
            this._zoneVariantsExpander = new Expander({
                header: nlsConfiguratorKeys.ZONE_VARIANT_LABEL + ' ( ' + this.zoneVariantList.length + ' )',
                body: zoneElement,
                headerTooltipInfos: {
                    shortHelp: nlsConfiguratorKeys.ZONE_VARIANT_LABEL
                } /*,
    				icon: { 
        				iconName: "Zone Variants"
    				}*/
            });
            this._zoneVariantsExpander.inject(this.docFragment);
        }
        ...
        if (this.zoneVariantList && this.zoneVariantList.length > 0) {
            this.variantCompList = this.variantCompList.concat(this.zoneVariantList);
        }
    };
    ...
    VariantListPresenter.prototype._subscribeToEvents = function () {
        ...
        this.modelEvents.subscribe({ event: 'updateVariantList' }, function () {
            that.updateVariantList();
        });
        ...
        if (listIncompatibilities.length === 0) {
            ...
            for (var i = 0; i < listIncompatibilities.length; i++) {
                ...
                for (var j = 0; j < listIncompatibilities[i].length; j++) {
                    ...
                    msgStr2 = msgStr2.replace(
                        '#OPTION#',
                        model.getFeatureDisplayNameWithId(listIncompatibilities[i][j]) +
                            '[' +
                            model.getOptionDisplayNameWithId(listIncompatibilities[i][j]) +
                            ']'
                    );
                    msgStr2 = msgStr2.replace('#STATUS#', UWA.i18n(state));
                    message += ' ' + msgStr2;
    ...
    VariantListPresenter.prototype.attachComponentsToDOM = function () {
        if (this.docFragment && this.docFragment.childElementCount > 0) {
            this._extendedVariantList.appendChild(this.docFragment);
        }
    };
    ...
    VariantListPresenter.prototype.updateFilters = function (data) {
        ...
        if (that.variantCompList && that.variantCompList.length > 0) {
            var mandVariants = 0,
                unselectedVariants = 0,
                rulesVariants = 0,
                chosenVariant = 0;
            var mandvariant, conflicts, mandIcon;
            ...
            for (var i = 0; i < that.variantCompList.length; i++) {
                let variantComp = that.variantCompList[i];
                let variantRuleId = variantComp.variantId;
                let isUpdateRequired = that.configModel.getUpdateRequired(variantRuleId);
                // var variantIncompatible = that.configModel.getStateWithId(variantRuleId)=== "Incompatible" ? true : false;
                if ((data && data.updateView && isUpdateRequired) || (data && data.refresh)) {
                    variantComp.updateView(data);
                }
                let featureState = that.configModel.getStateWithId(variantRuleId);
                let userSelected = that.configModel.getUserSelectVariantIDs(variantRuleId);
                let variantIncompatible = featureState === 'Incompatible' && !userSelected;

                mandIcon = UWA.extendElement(document.getElementById('variantRightIcon_' + variantRuleId));
                let featureIdWithStatus = that.configModel.getFeatureIdWithStatus(variantRuleId);
                if (!featureIdWithStatus && !variantIncompatible) {
                    unselectedVariants++;
                }

                let featureIdWithRulesStatus = that.configModel.getFeatureIdWithRulesStatus(variantRuleId);
                if (rules && featureIdWithRulesStatus && !variantIncompatible) {
                    rulesVariants++;
                }

                let featureIdWithChosenStatus = that.configModel.getFeatureIdWithChosenStatus(variantRuleId);
                if (featureIdWithChosenStatus) {
                    chosenVariant++;
                }

                mandvariant = that.configModel.getFeatureIdWithMandStatus(variantRuleId);
                if (mandvariant && !variantIncompatible) {
                    mandVariants++;
                }

                if (mandIcon) {
                    mandvariant
                        ? mandIcon.addClassName('fonticon-attention')
                        : mandIcon.removeClassName('fonticon-attention');
                }

                if (variantComp && !variantIncompatible) {
                    countVariants++;
                }
            }

            conflicts = rules ? that._conflicts : 0;
...
define('DS/ConfiguratorPanel/scripts/Presenters/ConfigEditorPresenter', [
    'DS/ResizeSensor/js/ResizeSensor',
    'DS/UIKIT/Mask',
    'UWA/Core',
    'DS/Handlebars/Handlebars',
    ...
    ConfigEditorPresenter.prototype._subscribeEvents = function () {
        ...
        this.modelEvents.subscribe({ event: 'pc-interaction-complete' }, function () {
			...
			that.VLPresenter.attachComponentsToDOM();
        ...
        this.modelEvents.subscribe({ event: 'onTopbarSwitchView' }, function (evtData) {
            if (evtData && evtData.view) {
                if (evtData.view === ConfiguratorVariables.TILE_VIEW) {
                    ...
                    that.configuratorToolbar.hideCollapseExpandAll();
                    ...
                    //hide collapse-all & expand-all
                    that.configuratorToolbar.showCollapseExpandAll();
        ...
        this.modelEvents.subscribe({ event: 'pc-confirm-variant-singleSelect' }, function (options) {
            if (options && options.feature && UWA.is(options.callback, 'function')) {
                if (
                    (that.configModel.hasDismissedOptions(options.feature) &&
                        (!UWA.is(options.needConfirm) || options.needConfirm)) ||
                    (that.configModel.hasMultiSelectVariantValues(options.feature) &&
                        (!UWA.is(options.needConfirm) || options.needConfirm))
                ) {
                    var superModal = new SuperModal({
                        className: 'confirm-variantSingleSelect-modal',
                        renderTo: document.body
                    });
                    superModal.confirm(
                        NLS_Configurator.Text_Confirm_EmptySelection,
                        NLS_Configurator.Title_Confirm_SingleSelect,
                        function (confirmed) {
        ...
        this.modelEvents.subscribe({ event: 'pc-confirm-variant-multiSelect' }, function (options) {
            if (options && options.feature && UWA.is(options.callback, 'function')) {
                if (
                    ...
                    var superModal = new SuperModal({
                        className: 'confirm-variantMultiSelect-modal',
                        renderTo: document.body
                    });
                    superModal.confirm(
                        NLS_Configurator.Text_Confirm_EmptySelection,
                        NLS_Configurator.Title_Confirm_MultiSelect,
                        function (confirmed) {
                            confirmed = !UWA.is(confirmed) ? false : confirmed;
                            options.callback(confirmed);
                        }
        ...
        this.modelEvents.subscribe({ event: 'pc-confirm-variant-dismissSelect' }, function (options) {
            if (options && options.feature && UWA.is(options.callback, 'function')) {
                if (
                    that.configModel.hasChosenOptions(options.feature) &&
                    (!UWA.is(options.needConfirm) || options.needConfirm)
                ) {
                    var superModal = new SuperModal({
                        className: 'confirm-variantDismissSelect-modal',
                        renderTo: document.body
                    });
                    superModal.confirm(
                        NLS_Configurator.Text_Confirm_EmptySelection,
                        NLS_Configurator.Title_Confirm_DismissSelect,
                        function (confirmed) {
                            confirmed = !UWA.is(confirmed) ? false : confirmed;
                            options.callback(confirmed);
                        }
        ...
        this.modelEvents.subscribe({ event: 'pc-confirm-variant-includeSelect' }, function (options) {
            if (options && options.feature && UWA.is(options.callback, 'function')) {
                if (
                    ...
                    var superModal = new SuperModal({
                        className: 'confirm-variantIncludeSelect-modal',
                        renderTo: document.body
                    });
                    superModal.dialog({
        ...
        this.modelEvents.subscribe({ event: 'pc-confirm-variant-excludeSelect' }, function (options) {
            if (options && options.feature && UWA.is(options.callback, 'function')) {
                if (
                    ...
                    var superModal = new SuperModal({
                        className: 'confirm-variantExcludeSelect-modal',
                        renderTo: document.body
                    });
                    superModal.dialog({
    ...
    ConfigEditorPresenter.prototype._resize = function () {
        ...
        var zoneVariantComponents = this.VLPresenter ? this.VLPresenter.zoneVariantList : [];
        components = components.concat(zoneVariantComponents);
    ...
    ConfigEditorPresenter.prototype._setMultiselection = function () {
        //set multiselection in case of Unconstrained mode
        //R14: does not need to set Multi Selection Mode on Solver anymore
        if (
            this.configModel.getAppFunc().multiSelection === ConfiguratorVariables.str_yes ||
            this.configModel.getAppFunc().selectionMode_Refine === ConfiguratorVariables.str_yes
        ) {
    ...
    ConfigEditorPresenter.prototype._initConfigEditor = function () {
        var that = this;
        ...
        this._dictionaryUtil = new DictionaryDataUtil({ dictionary: dictionary });
        this._pcDataUtil = new PCDataUtil({
            configModel: this.configModel,
            dictionaryUtil: this._dictionaryUtil,
            modelEvents: this.modelEvents,
            enableMatchingPC: this.enableTableView
        });
        ...
        this.VLPresenter = new VariantListPresenter({
            ...
            version: this.version,
            pcDataUtil: this._pcDataUtil,
            criteriaSelections: this.criteriaSelections
        });
        //GridPresenter init//
        if (this.enableTableView) {
            setTimeout(function () {
                ...
                that._cfgGridPresenter.init(that.modelEvents, undefined, {
                    id: pcId,
                    configModel: that.configModel,
                    pCDataUtil: that._pcDataUtil,
                    dictionaryDataUtil: that._dictionaryUtil
                });
        ...
        if (this.allowSave === 'yes') {
            this._saveBtn = new Button({ value: NLS_Configurator.save, className: 'primary' }).inject(
                this._savePCContainer
            );
...
define('DS/ConfiguratorPanel/scripts/Presenters/PCPanel', [
    'UWA/Core',
    'DS/ConfiguratorPanel/scripts/ConfiguratorSolverFunctionsV2',
    'DS/ConfiguratorPanel/scripts/Models/ConfiguratorModel',
        ...
        if (this.isWebInWin === true && options.tenant !== 'OnPremise') {
            ...
            if (window.dscef !== undefined) {
                ...
                if (window['COMPASS_CONFIG'] === undefined || window['COMPASS_CONFIG'].myAppsBaseUrl === undefined) {
                    // Get the base url
                    window.dscef.getMyAppsURL().then(
                        function (resolve) {
    ...
    PCPanel.prototype._initSolverDictionary = function () {
        var that = this;
        var tempOptions = {
            version: this.version,
            ...
            displayErrorSolverNotification: this.displayErrorSolverNotification
        };
        this.modelEvents.subscribe({ event: 'solver_init_complete' }, function () {
            that.modelEvents.publish({ event: 'solveAndDiagnoseAll_SolverAnswer', data: { refresh: true } });
            that.modelEvents.publish({ event: 'pc-loaded', data: {} });
        });
        ...
        if (this._configModel.getDictionary()) {
            ...
            ConfiguratorSolverFunctionsV2.initSolver(
                this.mvId,
                this._configModel,
                this.modelEvents,
                this.configCriteria,
    ...
    PCPanel.prototype._initSolver = function () {
        var that = this;
        var tempOptions = {
            version: this.version,
            dictionary: this._dictionary,
    ...
    PCPanel.prototype._getConfigModel = function () {
        return new ConfiguratorModel({
            ...
            appFunc: {
                ...
                rulesMode_ProposeOptimizedConfiguration: this.ruleLevels['RulesMode_ProposeOptimizedConfiguration']
                    ? 'yes'
                    : 'no',
                rulesMode_SelectCompleteConfiguration: this.ruleLevels['RulesMode_SelectCompleteConfiguration']
                    ? 'yes'
                    : 'no',
                rulesMode_EnforceRequiredOptions: this.ruleLevels['RulesMode_EnforceRequiredOptions'] ? 'yes' : 'no',
                rulesMode_DisableIncompatibleOptions: this.ruleLevels['RulesMode_DisableIncompatibleOptions']
                    ? 'yes'
                    : 'no'
            },
            enablePrimaryEdition: this.enablePrimaryEdition,
            enableRuleSelection: this.enableRuleSelection,
            modelEvents: this.modelEvents,
            readOnly: false,
