define('DS/ENOXEmptyContent/js/ENOXEmptyContent',
    ...
    function (ModelEvents, SentenceDetails, WUXContextualMenu, NLS, css_wux_fonticons, css_wux, css_ENOXEmptyContent) {
        'use strict';

        var ENOXEmptyContent = function (param, emptyListContextualOption) {
            this._init(param, emptyListContextualOption);
        };
        ...
        ENOXEmptyContent.prototype._init = function (param, emptyListContextualOption) {
            ...
            this._initDiv(param);
            if (emptyListContextualOption) {
                this._addEmptyListContextualMenu(emptyListContextualOption);
            }
            this._subscribeToEvents();
        };
        ENOXEmptyContent.prototype._addEmptyListContextualMenu = function (emptyListContextualActions) {

            if (emptyListContextualActions) {
                if (emptyListContextualActions.length > 0) {
                    var getMenuOptions = function (menu, actionOptions) {
                        var actionItem = {
                            id: actionOptions.id,
                            type: actionOptions.type ? actionOptions.type : 'PushItem',
                            title: actionOptions.title ? actionOptions.title : actionOptions.text,
                            state: actionOptions.state ? actionOptions.state : 'enabled'
                        };

                        if ((actionOptions.multiselect !== undefined) && multipleItemSelected && !actionOptions.multiselect) {
                            actionItem.state = 'disabled';
                        }

                        if (actionOptions.handler) {
                            actionItem.action = {
                                callback: actionOptions.handler
                            };
                        }

                        if (actionOptions.id == "createOrDerive") {
                            actionItem.action = {
                                callback: function () {
                                    actionOptions.handler(true);  // Call the handler with parameters
                                }
                            };
                        }

                        if (actionOptions.fonticon) {
                            actionItem.icon = actionOptions.fonticon;
                        }
                        if (actionOptions.content) {
                            var content = actionOptions.content;
                            actionItem.submenu = [];
                            content.reduce(getMenuOptions, actionItem.submenu);
                        } else if (actionOptions.submenu) {
                            actionItem.submenu = actionOptions.submenu;
                        }
                        menu.push(actionItem);
                        return menu;
                    };
                    var reducedContextualMenu = emptyListContextualActions.reduce(getMenuOptions, []);
                    WUXContextualMenu.addEventListener(this._containerEmptyContent, {
                        callback(data) {
                            // if (!that._contentViews['DataGridView'].elements.poolContainer.contains(data.data.target))
                            return reducedContextualMenu;
                        }
                    });
                }
            }
