    ...
    class EndpointClientBase {
        ...
        _handleResponse(message) {
            ...
            if (correlationID === undefined)
                return false;
            const requestHandler = this._requestMap.get(correlationID);
            if (requestHandler === undefined)
                return false;
            const success = requestHandler.onCheckSuccess ? requestHandler.onCheckSuccess(message) : message instanceof Transport_1.Transport;
        ...
        send(message, policies) {
            const msg = createBaseMessage(message);
            try {
        ...
        async publish(message, policies) {
            ...
            try {
                if (BusType_1.enInterceptResult.Intercept == this._doOnInterceptReceptMessage(msg)) {
                    ...
                    throw new Error('Message ' + msg + ' intercepted !');
                }
                if (this._impl.send(msg)) {
                    return;
                }
                throw new Error("failed to publish message '" + message + "' (encoding error).");
            }
            catch (err) {
                throw new Error("failed to publish message '" + message + "':" + err);
            }
        }
        sendRequest(parameters) {
            ...
            try {
                const myMsg = createBaseMessage(parameters.msg);
                const cid = myMsg.withHeader(Transport_1.HeaderKey.URI, parameters.uri).withCorrelationID().flagAsRequest().getCorrelationID();
                ...
                const handler = (0, RequestHandler_1.CreateRequestHandler)(parameters);
        ...
        async connect(id) {
            ...
            if (id) {
                this.endpointAddress.identifier = id;
            }
            try {
                // connection ExperienceKernel
                ...
                _logger.info("Client '" +
                    ...
                    (((_b = this.endpointAddress) === null || _b === void 0 ? void 0 : _b.identifier) ? this.endpointAddress.endpoint + '|' + this.endpointAddress.identifier : (_c = this.endpointAddress) === null || _c === void 0 ? void 0 : _c.endpoint) +
                    "'.");
                // send Hello steps...
                await this._helloConnect();
                // subscribe Signals...
                if (this._signal > 0) {
                    await this._subscribeSignal();
                }
                // subscribe Topics...
                const topicPromises = [];
                this._topics.forEach((count, topic) => {
                    for (let i = 0; i < count; ++i) {
                        topicPromises.push(this._subscribeTopics(topic));
                    } //endfor
                });
                if (topicPromises.length > 0) {
                ...
                _logger.error("Client '" +
                    ...
                    (((_d = this.endpointAddress) === null || _d === void 0 ? void 0 : _d.identifier) ? this.endpointAddress.endpoint + '|' + this.endpointAddress.identifier : (_e = this.endpointAddress) === null || _e === void 0 ? void 0 : _e.endpoint) +
                    "'.");
                throw reason;
            }
        }
