...
define(["require", "exports", "DS/Logger/Logger", "DS/DELPXPFoundations/PXPRegistry", "DS/DELPXPFoundations/PXPPassport"], function (require, exports, Logger, PXPRegistry_1, PXPPassport_1) {
    ...
    async function ConnectToPXPBackend(params) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            // retrieve 'pxpserver' in LocalStorage
            const pxpServer = localStorage.getItem('pxpserver');
            if (pxpServer !== null) {
                try {
                    connection.hypervisorUrl = new URL(pxpServer);
                }
                catch (error) {
                    reject(new Error('PXPBackend- invalid Hypervisor URL in local storage (' + pxpServer + ' invalid: ' + error + ')'));
                }
            }
            else {
                ...
                if (hypervisorLocal) {
                    try {
                        ...
                        reject(new Error('PXPBackend- invalid Hypervisor URL (' + hypervisorLocal + ' invalid: ' + error + ')'));
                    }
                }
            }
            if (isServer) {
