        ...
        var FoundationV2Data = {
            ...
            __copyModifiedObject: function (source, target) {
                ...
                for (var i = 0; i < propertyList.length; i++) {
                    ...
                    if (propertyList[i] === "id" && source['type'] === 'Group') { //bcc: see if instead of having our internal type we can use the official one foaf:Group
                        // task WS for candidate assignees require groups to be sent in non standard format
                        // this is because groups are special case of objects which are really proxy for others
                        // need to make sure that any future 6W services which manipulate groups do it the same way.
                        if (!target.dataelements) {
                            target.dataelements = {};
                        }
                        if (source.dataelements && source.dataelements.groupURI) {
                            target.dataelements.groupURI = source.dataelements.groupURI;
            ...
            getParams: function () {
                ...
                if(lParamsParsed.scopeType == 'Project Space'){
                    lParamsParsed.showProjectTasks = true;
                }
        ...
        var FoundationV2Model = Model.extend({
            ...
            syncRead: function(inMemoryData, iServiceName, options) {
                ...
                if (requestFetch) {
                  ...
                  FoundationData.loadServiceData(lServiceName, (data) => {
                    this.syncFromServer(data);
                    if (data.success) {
                      options && options.onComplete && options.onComplete();
                    } else {
                      options && options.onFailure && options.onFailure();
                    }
                  }, undefined, undefined, undefined, urlParam);
                }
            },
            ...
            dateForJSON: function(iAttributeName, iAttributeValue, iFieldsDef, iOptions) {
                ...
                if ((iAttributeName === 'dueDate' || iAttributeName === 'estimatedStartDate' || iAttributeName === 'routeTaskDueDate') && !(iOptions && iOptions.dataForRendering)) {
                    ...
                    if(iAttributeName == 'estimatedStartDate'){
                        lDateClone.setHours(8, 0, 0, 0);
                    } else {
...
define('DS/Foundation2/Models/FoundationBaseModel', [
        'UWA/Core',
        'UWA/Class/Model',
        'UWA/Class/Collection',
        //UWA Prereqs
        'DS/Foundation2/FoundationV2Data',
        'DS/Foundation2/Models/FoundationV2Model',
    ...
    function(UWA, Model, Collection, FoundationData, FoundationV2Model, SecurityContextUtils) {
        ...
        var FoundationBaseModel = FoundationV2Model.extend({
             ...
             * @param {object} iObject object to add, this should be a row object
             ...
             * @param {object} iRelationshipName the relationship to use
             * @param {string} options  options to pass down.  In our case we are interested by existingObject which if true will
             ...
             * @return {object} the new added model
             */
            ...
            getParentIdForRelationship: function() {
...
define('DS/Foundation2/Models/ReferenceModel', ['UWA/Core','DS/Foundation2/FoundationV2Data',
    ...
    function(UWA, FoundationData, FoundationBaseModel, UIHelper, Search, PlatformManager, FedSearchUtil) {
        'use strict';
        ...
        var ReferenceModel = FoundationBaseModel.extend({
            ...
            init: function(attrs) {
                if (attrs && attrs.source) this._origSourceUrl = attrs.source;
                this._parent.apply(this, arguments);
            },
            ...
            getTransferObject: function(options) {
                ...
                if (type && type === 'URL') {
                    lUIHelperOptions.linkURL = this.get('linkURL');
                    lUIHelperOptions.description = this.get('description');
                }
            ...
            syncRead: function(data, serviceName, options) {
                ...
                if (this.serviceId === '3DSwym') { //case of a swym model
                    ...
                    if (personMatch) {
                        FoundationData.loadServiceData('PersonSearchByName?searchStr=' + this.id.substr(personMatch[0].length), function(resp) {
            ...
            get: function(iAttrName, options) {
                if (iAttrName === 'name') {
                ...
                } else if (iAttrName === 'nameAndAccount') {
                    // eg. Benoit Cerrina (bcc)
                    return this.get('dsaccess:accountName') ? this.get('name') + ' (' + this.get('dsaccess:accountName') + ')' : this.get('name');
                    ...
                    return this._parent('name');
                } else if (iAttrName === '_isFromSourcing') {
                    return this._isFromSourcing();
                }
            ...
            toJSON: function(options) {
                var lOptions = UWA.clone(typeof options === 'object' ? options : {}, false);
                if (!Object.prototype.hasOwnProperty.call(lOptions, 'noMapping')) {
                ...
                ret.nameAndAccount = this.get('nameAndAccount');
                return ret;
            },
            /*
            ...
            dataForRendering: function(options) {
                ...
                ret.source = this.serviceHosting.toLowerCase();
                ret.physicalId = ret.id;
                ret.revision = UIHelper.getFilteredRevision(ret.revision);
                return ret;
            },
            canBeRemoved: function() {
                return true;
            },
            canShowProperties: function() {
                return !this._isFromSourcing();
            },
            _isFromSourcing: function() {
                return this._origSourceUrl && this._origSourceUrl.includes("sourcing");
            }
        });
...
/*global enoviaServer*/
define(
    'DS/Foundation2/WidgetTagNavInit', ['DS/ENOjquery/ENOjquery', 'DS/Foundation2/FoundationV2Data'],
    function(jQuery, FoundationV2Data) {
        'use strict'; //need to know if tags should be a local or global variable

        const MAX_TAG_IDS = 2000;
        ...
        var WidgetTagNavInit = {
            ...
            loadTagData: function(inMemoryData) {
                ...
                const sendTagRequest = () => {
                    const lIds = ids.splice(0, MAX_TAG_IDS); // IR-1433716 server times out with too many ids fetching tags
                    if (lIds.length) {
                        var postData = this.getTagPostData(inMemoryData, lIds);
                        ...
                        FoundationV2Data.loadServiceData(
                            ...
                            /*callback*/
                            (data) => {
                                data.data && (resp.data = resp.data.concat(data.data));
                                sendTagRequest();
                            },
                            /*isPost*/
                            true,
                        ...
                        this.loadTagDataDone(resp, inMemoryData);
                    }
                }
                sendTagRequest();
            },
            loadTagDataDone: function(data, inMemoryData) {
        ...
        var FoundationBaseCollection = Collection.extend({
            ...
            setup: function FoundationCollectionSetup(models, options) {
                if (options) {
                    ...
                    if(options && options.convertToInMemoryData) {
                        this._data = FoundationData._turnDataIntoObject(this._data)
                        FoundationData._setInternalProperty(this._data, '_serviceName', this._serviceName);
                    }
                }
                ...
                if (this.model && lRelations) {
                    ...
                    for (var lCurRelIdx = 0; lCurRelIdx < lNbRelations; lCurRelIdx++) {
                        ...
                        var newChildCollectionType = new ChildCollectionType(null, {
            ...
            fetchObjectsInfo: function() {
                ...
                FoundationData.loadServiceData('ObjectInfo', callback, true, '$ids=' + lKeys, 'application/x-www-form-urlencoded', '$fields=islast');
                return lRet;
            },
            /**
             * override fetch.
                    ...
                    if (!options.localOnly || options.server) {
                        if (collection instanceof Model) {
                            ...
                            if (!options.delayedSave) {
                                ...
                                this.pendingApplyUpdatesCallbacks.push(applyUpdatesCallback); // used in case of multi-edit
                            }
            ...
            save: function(options) {
                ...
                var lData = this.data();
                var applyUpdatesCallback = function applyUpdatesCallback(success, data, clonedInfo) {
                    if (that.pendingApplyUpdatesCallbacks.length > 0) { // In case of multi-edit
                        that.pendingApplyUpdatesCallbacks.forEach(function(callback) {
            ...
            complete: function(iToComplete) {
                ...
                if (UWA.is(iToComplete, 'object')) {
                    ...
                    lToComplete = iToComplete.filter((iElem) => { //filter out non foundation models like RDF ones for instance
                         return iElem instanceof FoundationBaseModel && this.contains(iElem);
                    });
                }
