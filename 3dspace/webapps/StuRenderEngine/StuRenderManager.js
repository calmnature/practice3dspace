...
define('DS/StuRenderEngine/StuRenderManager', [
	...
	function (STU, EP, Manager, Task, DSMath, Actor3D, VisuPrimitive, Color, ColorRGB) {
		'use strict';
		...
		RenderManagerPostProcessTask.prototype.updatePostProcess = function () {
			...
			// Apply the glow to the viewer (not supported by the viewer...)
			// var glowEffect = viewer.getEffect("Bloom");
			// if (glowEffect) {
			// 	if (cameraExpObject.GetValueByName('enableGlow')) {
			// 		glowEffect.setBloomFactor(startupCam.glowIntensity);
			// 	} else {
			// 		glowEffect.setBloomFactor(0);
			// 	}
			// }

			var toneMappingEffect = viewer.getEffect("ToneMapping");
			if (toneMappingEffect) {
				// Apply the exposure to the viewer
				if (startupCam.enableExposure) {
					toneMappingEffect.setBrightness(startupCam.exposure);
				} else {
					toneMappingEffect.setBrightness(0);
				}

				// Apply the tonemapping to the viewer
				if (startupCam.enableToneMapping) {
					switch (startupCam.toneMapping) {
						case 0:
							toneMappingEffect.setToneMapping(6, {
								crushblacks: startupCam.blacks,
								burnhighlights: startupCam.whites,
								saturation: startupCam.saturation,
							});
						break;
						case 1:
							toneMappingEffect.setToneMapping(4, {
								A: startupCam.shoulderScale,
								B: startupCam.linearScale,
								C: startupCam.linearAngle,
								D: startupCam.toeScale,
								E: startupCam.toeNumerator,
								F: startupCam.toeDenominator,
								W: startupCam.whiteLevel
							});
							break;
						default:
							break;
					...
					// The exposure parameter is used by the Photographic tonemapping mode
					toneMappingEffect.setGamma(startupCam.gamma);
				} else {
					toneMappingEffect.setToneMapping(1);
					// setToneMapping(1) => gamma reset to ColorCorrectionEnum.NONE;
				}
			}
		...
		RenderManager.prototype.setCurrentCamera = function (iActorCamera) {
			...
			//IR-1473836: if camera is XRHeadset then we are locked in the XR Session
			if (STU.XRHeadsetActor && previousCam instanceof STU.XRHeadsetActor) {
				console.warn("Experience is in XR Session, current camera is bound to the XR Headset Actor.")
				return;
			}
		 ...
		 * @public
		 *
		 * @param {DSMath.Transformation | DSMath.Point | DSMath.Vector3D} iTransform The origin of the box.
		 ...
		 * @param {STU.Referential} [iOptions.referential] The referential in which the inputs information are given.
		...
		RenderManager.prototype.createBox = function (iTransform, iSize, iOptions) {
			...
			else if (iTransform instanceof DSMath.Point || iTransform instanceof DSMath.Vector3D) {
				params.position = new DSMath.Transformation();
				params.position.setVector(iTransform.x, iTransform.y, iTransform.z);
			}
			else {
				throw new TypeError('iTransform parameter must be a Transformation, Vector3D or Point');
			}
		 ...
		 * @public
		 *
		 * @param {DSMath.Transformation|DSMath.Vector3D|DSMath.Point} iTransform The origin of the sphere.
		...
		/**
		 * Projects a 3D point to the screen space, returning its position in pixels.
		 *
		 * Notes: calling this function is valid only when playing the experience on a screen. Playing it in VR or AR mode will return undefined results.
		 *
		 * @example
		 *  // This script centers a UI Actor on top of 3D actor on each frame
		 *	// Prerequisites:
		 *	// - the script is attached to a 2D UI actor
		 *	// - the script has a 'target' property pointing to a 3D Actor
		 *	beScript.execute = function (context) {
		 *		let rm = STU.RenderManager.getInstance();
		 *		let targetScreenPos = rm.projectPointToScreen(this.target.getPosition());
		 *		let x = targetScreenPos.x - this.actor.dimension.width/2;
		 *		let y = targetScreenPos.y - this.actor.dimension.height / 2;
		 *		this.actor.offset = new DSMath.Vector2D(x, y);
		 *	};
		 *
		 *
		 * @method
		 * @public
		 *
		 * @param {DSMath.Point} iPoint3D the point to project, expressed in the given referential (or world if none specified).
		 * @param {STU.Referential} [iReferential] The referential in which the input coordinates are expressed. If undefined, coordinates are experessed in world context.
		 * @return {DSMath.Point2D} the 2D coordinates of the projected point in pixels, in screen space.
		 */
		RenderManager.prototype.projectPointToScreen = function (iPoint3D, iReferential) {

			if (iReferential === null || iReferential === undefined) {
				iReferential = "World";
			}

			// express iPoint3D in Visu referential
			var ReferentialTransfo_Visu = STU.Experience.getCurrent().getReferentialTransform(iReferential, "Visu");
			iPoint3D.applyTransformation(ReferentialTransfo_Visu);

			var point2D = new DSMath.Point2D();
			this.main3DViewer.ProjectPointToScreen(iPoint3D, point2D);
			return point2D;
		};
		...
		RenderManager.prototype._pickGroundFromPosition = function (iParams) {
			...
			if (iParams !== null && iParams !== undefined) {
				...
				if (env !== null && env !== undefined) {
					...
					if (STU.Planets !== undefined && env instanceof STU.Planets) { //checking like that instead of having to preq the whole StuPlanets to define
		...
		RenderManager.prototype.updateDebugPrimitives = function (iContext) {
			...
			for (var p in this.map) {
				...
				if (obj.life >= 0.0) {
					...
					if (obj.isLineTransformDirty) {
						//update position
						this.StuGeomPrimitive_ISOManager.setLinePosition(obj.id, obj.transfo, obj.startPointTransfo); //in win its origin is in the middle, in web it is at the beginning
						obj.isLineTransformDirty = false;
					}
				}
			}
		};
