                    ...
                    for (var i = 0; i < primitiveGroup.nbPrimitives; i++) {
                        ...
                        if (component !== undefined) {
                            if (component.indices == null) {
                                ...
                                primitiveGroup.forEachBuffer((buffer, id) => {

                                    if (!buffer) { return; }

                                    if (buffer.component == Mesh.VertexComponentEnum.POSITION) {
                                        component.bufferId = id;
                                        return;
                                    }
                    ...
                    if (tmp !== undefined) {
                        ...
                        for (var i = 0; i < strips.length; i++) {
                            ...
                            if (item.length > 0) {
                                ...
                                // unstrip and concatenate the indices in a temporary index buffer
                                
                                for (var j=0; j<item.length-2; j++) {
                                    if (j % 2) {
                                        currentIndexBufferData[ibOffset++] = item[j];
                                        currentIndexBufferData[ibOffset++] = item[j+2];
                                        currentIndexBufferData[ibOffset++] = item[j+1];
                                    } else {
                                        currentIndexBufferData[ibOffset++] = item[j];
                                        currentIndexBufferData[ibOffset++] = item[j+1];
                                        currentIndexBufferData[ibOffset++] = item[j+2];
                        ...
                        prim.nbIndices = realNbIndices;
                        vcomp.nbVertices = realNbIndices;
