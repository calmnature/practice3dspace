...
function(NotificationsManagerUXMessages,NotificationsManagerViewOnScreen) {
    ...
    MeetingNotify.prototype.handler = function () {
		...
		} else if(document.getElementById("Meeting-massAgenda-container")){
			NotificationsManagerViewOnScreen.inject(document.getElementById("Meeting-massAgenda-container"));
    ...
    Utils.getMeetingDataUpdated = function (meetingId) {
    	...
    	MeetingController.fetchMeetingById(meetingId).then(
				success => {
					...
					let strMeetingDuration = success[0].duration;
					let totalDur = parseInt(widget.getValue('SumOfAgendaDuration'));
...
define('DS/ENXMeetingMgmt/Utilities/DataFormatter',
	...
	let gridData = function(dataElem){
		...
		var response =
		{
			...
			typeIcon: dataElem.typeIcon,
			thumbnailIcon: dataElem.thumbnailIcon,
...
function(core, Alone, NLS) {
    ...
    let MeetingFacets = [{ 
        ...
        loader : 'DS/ENXMeetingMgmt/View/Facets/MeetingPropertiesView'
	}, { 
    	label: NLS.MeetingContextAndAccess, 
    	id: "MeetingAssociatedInfo",
    	isSelected: false, 
    	icon: { 
    		iconName: "attributes-cog",
    		fontIconFamily: WUXManagedFontIcons.Font3DS
    	},
    	content: Alone.createElement('div', {id:'MeetingAssociatedInfoContainer', 'class':'meeting-associated-info-container'}),
    	...
    	allowClosableFlag : false,
    	content: Alone.createElement('div', {id:'MeetingCommentContainer', 'class':'meeting-Comment-container'}),
        loader : 'DS/ENXMeetingMgmt/View/Facets/MeetingCommentFacet'
    }];
    ...
    ENOXWidgetPreferences
            ...
            EnoviaBootstrap = UWACore.merge(UWAListener, {
                ...
                authenticatedRequest : function(url, options, preferedSecuritycontext) {	//preferedSecuritycontext: "ctx::VPLMProjectLeader.MyCompany.DemoStandard"
                    ...
                    const meetingTypes = widget.getValue("meetingTypes");
					const decisionTypes = widget.getValue("decisionTypes");
                    if(x3DContent && x3DContent.data.items && ((meetingTypes && meetingTypes.includes(x3DContent.data.items[0].objectType)) || (decisionTypes && decisionTypes.includes(x3DContent.data.items[0].objectType)))){
                        if (decisionTypes.includes(x3DContent.data.items[0].objectType)) {
                        ...
                        else if (meetingTypes.includes(x3DContent.data.items[0].objectType)){
				...
				getTenantValue : function(){
					var tenantValue="";
                	tenantValue = widget.getValue("x3dPlatformId");
                	
					return tenantValue;
                },

				getRegistryUrl : function(){
					return PlatformAPI.getApplicationConfiguration('app.urls.registry');
				},
...
define('DS/ENXMeetingMgmt/Services/MeetingServices',
    ...
    let MeetingServices,_deleteMeetingAgenda, _fetchAllMeetings,_fetchMeetingById,_fetchMeetingAgendas, _deleteMeeting,_deleteAttachment,_addAttachment,
		_fetchMeetingAttachments,_updateMeetingAgenda,_createMeetingAgenda,_fetchMembers,_addMembers,_updateMeetingAttendance,_deleteMember,_updateMeetingProperties,
		_getContentInfo,_fetchAllowedAttachmentTypesForMeeting;
...
//            };
	...
	/* Get all the subtypes to show in the create dialog  */
    const _getTypesForCreate = function(type) {
		
		return new Promise(function(resolve, reject) {	
			let typeDerivativesServiceUrl = EnoviaBootstrap.getMeetingServiceBaseURL() + `/creatableTypes?type=${type}`;
	       	let options = {};
           	options.method = 'GET';
           	options.headers = {
            	'Content-Type' : 'application/ds-json',
   			...
   			options.onComplete = function(serverResponse) {
        		...
        		if (response.success) {
                	var returnInfo = [];
                   	response.data.forEach(typeInfo => {
                    	const typeData = {
							label: typeInfo.dataelements.typeNLS,
							value: typeInfo.dataelements.type
						}
                       	returnInfo.push(typeData);
                   	});
	...
	/* Get all the attributes for a given type */
    const _getTypeAttributes = function(type) {
	    return new Promise(function(resolve, reject) {
	        let postURL = EnoviaBootstrap.getMeetingServiceBaseURL() + `/creatableAttributes?type=${type}`;
    ...
    MeetingServices={
			...
			getTypesForCreate: (parentType) => {return _getTypesForCreate(parentType);},
			getTypeAttributes: (type, detailed) => {return _getTypeAttributes(type, detailed);}
    };
	    ...
	    response.forEach(function(dataElem) {	
	       var assigneesDiv = new UWA.Element("div", {
	        ...
	        var root = new TreeNodeModel({
	            ...
	            width: 300,
	            grid: gridData,
	            "thumbnail" : dataElem.thumbnailIcon,
...
define('DS/ENXMeetingMgmt/View/Form/MeetingCommentsView', [

	"DS/ENXMeetingMgmt/Controller/EnoviaBootstrap",
	"DS/SocialToolbarVue/SocialToolbarVueUWAView"
	
], function(EnoviaBootstrap, SocialToolbarVueUWAView) {

	let MeetingCommentsView = {
		renderComments: function(_appInstance, data) {
			var that = this;
			this.elements = _appInstance;
			let meetingPhysicalId = data.model.id;
			...
			var x3dcontent = {
				...
				'data': {
					'items': [{
						'envId': EnoviaBootstrap.getTenantValue(),
						'serviceId': "3DSpace",
						'objectId': meetingPhysicalId,
						'objectType' : data.model.type,
						'facet': 'isSocial'
					}]
				}
			};
			let appForcedLinkUri = '/#app:ENOLCMT_AP/content:X3DContentId=' + encodeURIComponent(JSON.stringify(x3dcontent));

			const opts = {
			    props: {
					subjectUri: 'pid:' + meetingPhysicalId,
			        disableFeatures: {
			            disableShare: true,
			            disableLike : false,
			            disableShowUserProfile : false,
						disableComment: false,
						disableLike: false,
						disableCommentLike: false,
						disableReply: false
			        },
					platformId: EnoviaBootstrap.getTenantValue(),
					urls: {
			            service: EnoviaBootstrap.get3DSpaceURL() + '/resources',
			            serviceCSRF: {
			                name: "X-DS-CSRFTOKEN",
			                //token: "1WEH-GJ3W-VBP6-T8T1-R9D5-MWEY-VDXS-51GA",
			            },
			            registry: EnoviaBootstrap.getRegistryUrl(),
			        },
					atInit: {
			            nbCommentsToLoad: 8,
			        },
					appForcedLinkUri : appForcedLinkUri
			    }
			}
			that.comments = new SocialToolbarVueUWAView();
			that.comments.render(opts);
			that.comments.inject(this.elements.container);
		}
	}

	return MeetingCommentsView;
});


define('DS/ENXMeetingMgmt/View/Facets/MeetingCommentFacet', [
	
	'DS/ENXMeetingMgmt/View/Form/MeetingCommentsView'
	
], function (MeetingCommentsView) {
    'use strict';
	
	let _appInstance = {};
	const buildContainer = function() {
		_appInstance.container = new UWA.Element('div', { id: 'meetingCommentContainer', 'class': 'meeting-Comment-container' });
        _appInstance.container.inject(document.querySelector('.MeetingPropertiesFacet-facets-container'));
	};

    const init = function (data = {}) {
        if (!showView()) {
			buildContainer();
			render.call(_appInstance, data)
        }
    };
	
	const render = function(data){
		this.elements = _appInstance;
		MeetingCommentsView.renderComments(_appInstance, data);
	};

    const hideView = function () {
        if (document.getElementById('meetingCommentContainer') != null) {
            document.getElementById('meetingCommentContainer').style.display = 'none';
        }
    };

    const showView = function () {
        if (document.querySelector('#meetingCommentContainer') != null) {
            document.getElementById('meetingCommentContainer').style.display = 'block';
            return true;
        }
        return false;
    };
    ...
    const destroy = function () {
        document.querySelector('#meetingCommentContainer').destroy();
    };
			...
			DecisionController.fetchDecisionById(e.result[0].identifier).then(
							success => {
								...
								if(widget.decisionSummary){
                                    widget.meetingEvent.publish('decision-info-close-click');
                                }			
							},
							failure =>{
								if(failure.error){
...
define('DS/ENXMeetingMgmt/View/Properties/MeetingPropWidget', [
	...
	function(
		...
		const MeetingPropWidget = {
			...
			render: function (propContainer, data) {
				...
				const options_panel = {
					typeOfDisplay: EditPropConstants.ONLY_EDIT_PROPERTIES, // Properties with ID card - ALL. ONLY_EDIT_PROPERTIES for only properties
	 				...
	 				'facets': facets,
	 				'editMode': false,
	 				'readOnly': "Complete" == data.state ? true : false,
	 				'context': { 
						getSecurityContext: function () { 
							return { SecurityContext: collabSpace }
		 				}
	 				},
	 				'extraNotif': true,
				...
				widget.setValue('initEditProp', true);
				if (data.id) {
			...
			getPropModel: function (objModel) {
				...
				selection.addEvent('onSave', function (event) {
					Utils.getMeetingDataUpdated(objModel.objectId);
				});
			...
			loadModel: function (objId, propContainer) {
				let that = this;
				let results = that.getPropModel({objectId: objId});
	    ...
	    return MeetingPropWidget;
		
	}
);


define('DS/ENXMeetingMgmt/View/Facets/MeetingPropertiesView',
       	[  
			'DS/ENXMeetingMgmt/View/Properties/MeetingPropWidget'
		], 
		function(
            MeetingPropWidget
       	) {
    ...
    let build = function(data){
        if(!showView()){
        	let containerDiv = UWA.createElement('div', {id: 'MeetingPropertiesView', 'class': 'meeting-properties-container',
				styles: {"height": "calc(100% - 50px)", "width":"100%"}}); 
        	containerDiv.inject(document.querySelector('.MeetingPropertiesFacet-facets-container'));
        	MeetingPropWidget.render(containerDiv, data.model);
        }
    };

    let hideView = function(){
        if(document.getElementById('MeetingPropertiesView') != null){
            document.getElementById('MeetingPropertiesView').style.display = 'none';
    ...
    let showView = function(){
        if(document.getElementById('MeetingPropertiesView') != null){
            document.getElementById('MeetingPropertiesView').style.display = 'block';
            return true;
        }
        return false;
    };
    ...
    let destroy = function() {
    	document.getElementById('MeetingPropertiesView').destroy();
    };
	
    let MeetingPropertiesView = {
            init : (data) => { return build(data);},        
            hideView: () => { hideView(); },
            destroy: () => { destroy(); }
    };

    return MeetingPropertiesView;
...
function(MeetingServices,CreateMeetingServices, MeetingWidgetUtilServices, Promise, NLS) {
    ...
    MeetingController = {
			...
			getTypesForCreate: function(parentType) {
				return new Promise(function(resolve, reject) {
    				MeetingServices.getTypesForCreate(parentType).then(
    				success => {
    					resolve(success);
    		    	},
    		    	failure => {
			...
			getTypeAttributes: function(type) {
				return new Promise(function(resolve, reject) {
    				MeetingServices.getTypeAttributes(type).then(
    				success => {
    					resolve(success);
    		    	},
    		    	failure => {
...
define('DS/ENXMeetingMgmt/Utilities/DragAndDropManager',
	...
	let _onDropMassAgendaManager = function(e, info, target) {
		if(target == "meetingAgendaTab")
			_manageMassAgendaDrop(e, info, target);
	};
	
	let _manageMassAgendaDrop = function(e, access, target) {	
		if(access) {
			const maxAgendaItemsLimit = 20;
			let data = getDroppedData(e);
			if(data) {
				if(data.length <= maxAgendaItemsLimit) {
					require(['DS/ENXMeetingMgmt/Utilities/WrapperUtilities'], function(WrapperUtilities) {
						let editedStateValues = WrapperUtilities.getAgendaEditedState();						
						if (editedStateValues && editedStateValues.editedState && editedStateValues.requiresConfirmationDialog) {
							...
							data.meetinginfo = getCurrentMeetingModel();
							if(data.length > 0) {
								let droppedObjectIds = [];
						    	data.every(function(obj) {
									let objectId = obj.objectId;
						        	droppedObjectIds.push(objectId);
						        	return true;
								});
								widget.meetingEvent.publish('meeting-massAgenda-create-click', { model: MeetingAgendaModel.getModel(), meetinginfo: MeetingAgendaModel.meetingInfo(), itemsId: droppedObjectIds });
								// DropZone._removeDroppableStyle(); to remove the style can be added along vid warning notif later
							}						
						}
					});					
				}
				else {
					let errorMsg = NLS.massAgendaMaxLimit;
					errorMsg = errorMsg.replace("{count}", maxAgendaItemsLimit);
					widget.meetingNotify.handler().addNotif({
		                level: 'error',
		                subtitle: errorMsg,
		             	sticky: true
			...
			widget.meetingNotify.handler().addNotif({
                level: 'error',
                subtitle: NLS.errorAccessAddAgenda,
             	sticky: true
             });
		}		
	};	
	  ...
	  let onDropManagerContext = function(e, info, target){
	   ...
	   let _manageContextDrop = function(e, _meetingProperties, target, allowedTypes){
		...
		let DragAndDropManager ={
			...
			onDropMassAgendaManager: (e, info, target) => { return _onDropMassAgendaManager(e, info, target);},
...
function(SearchUtil,MeetingController,MeetingModel,ParseJSONUtil,Utils, WUXLineEditor, WUXEditor, WUXToggle, WUXButtonGroup, WUXButton, WUXComboBox, WUXDatePicker, SelectionChipsEditor, SelectionChips, CustomFieldUtil, NLS) {    
		...
		let MeetingUtil = {
				...
				meetingPropertiesUpdate : function(meetingData,meetingProperties){
					...
					if (meetingData.model.Description === initiateJson.data[0].dataelements.description) {
						delete initiateJson.data[0].dataelements.description;
					}
			...
			validateAgenda: function(agendaModel, massAgendaFlag){
				var agendaflag = "true";
    			  agendaModel.forEach((agenda) => {
					  ...
					  var strAgendaDur = massAgendaFlag ? agenda.duration.value : agenda.info.duration.value;
        			  if(strAgendaDur == ""){
        				  widget.meetingNotify.handler().addNotif({
  							level: 'error',
        			 ...
        			 var strAgendaDuration = strAgendaDur.trim();
			...
			getDefaultUnit: function(ele) {
				...
				if(ele.units){
					let unitItems = [];
					if(typeof ele.units === "string") {
						unitItems = JSON.parse(ele.units).item;
					}
					else if(ele.units){
						unitItems = ele.units.item;
					}
			...
			getRangeElementsList: function(field) {
				let rangeList = [];
				if(typeof field.range === "string") {
					rangeList = JSON.parse(field.range).item;
				}
				else if(field.range){
					rangeList = field.range.item;
				} 
			...
			renderMultivalueFieldInCreateView2: function(eleDataType, eleType, ele, requiredFlag, meetingProperties, source, rowNumber, parentContainer, childContainer) {
				...
				let eleViewConfig = ele.viewConfig ? JSON.parse(ele.viewConfig) : {};
				const isEditable = Boolean(ele.editable || eleViewConfig.readWrite);
				...
				if (!source) { //this block will run only once in create: on render
					...
					elem = this.renderFieldInCreateView(eleDataType, eleType, requiredFlag, eleDefaultValue, ele);							
					if(elem) {
						container = new UWA.Element("div", { "id": ele.name+"-"+rowNum, "class":"multiVal", "styles":{'display': 'flex'}}).inject(childContainer);
						let CustomFieldUtilObj = CustomFieldUtil.getCustomFieldUtilObj();
						let buttonData = {
					...
					if(elem) {
						let totalRows = rowNumber + 1;					
						container = new UWA.Element("div", { "id": ele.name+"-"+rowNumber, "class":"multiVal", "styles":{'display': 'flex'}}).inject(childContainer);
						let buttonData = {
							eleDataType: eleDataType,
							eleType: eleType,
							ele: ele,
			...
			renderFieldInCreateView: function(eleDataType, eleType, requiredFlag, eleDefaultValue, ele) {
				...
				const isMultiLine = eleViewConfig.multiline;
...
*/
			...
			agendaActionCreate: function(agendadata, _agendaProperties, meetnginfo, e) {
				...
				*/
				...
				var topicItemsInfo = [];
				var	agendaCount = 1;
				if(_agendaProperties.massAgenda) {
					if(!_agendaProperties.elements.agendaData)
							return;						
					topicItemsInfo = _agendaProperties.elements.agendaData;
					agendaCount = topicItemsInfo.length;
				}
				else {				
				if (_agendaProperties.autoCompleteComponent != undefined) {
					if (_agendaProperties.autoCompleteComponent.selectedItems != undefined && _agendaProperties.autoCompleteComponent.selectedItems.length != 0) {
				...
				let autoDecision;
				if(!_agendaProperties.massAgenda)
					autoDecision = _agendaProperties.elements.autoDecision.checkFlag;

				if (topicItemsInfo.length == 0) {
					topicItemsInfo.push(meetnginfo.model.id);
				}
				
				let agendaDeciFlag = false;

				topicItemsInfo.forEach(function(topicItem, index) {
					var info = {}					
					if(_agendaProperties.massAgenda) {
						//info.id = topicItem.id;
						info.id = agendadata.topicItems[index].id;
						info.relelements = {};
						info.relelements.topic = _agendaProperties.elements.agendaData[index].topic.value;
						info.relelements.topicDuration = _agendaProperties.elements.agendaData[index].duration.value;
						info.relelements.topicDescription = "";
						info.relelements.sequence = (meetnginfo.nextSequence).toString();
						meetnginfo.nextSequence = meetnginfo.nextSequence + 1;
						if (_agendaProperties.elements.agendaData[index].speaker.options.agendaSpeakerUsername) {
							info.relelements.responsibility = _agendaProperties.elements.agendaData[index].speaker.options.agendaSpeakerUsername;
							info.relelements.responsibileOID = _agendaProperties.elements.agendaData[index].speaker.options.speakerId;
						}
						autoDecision = _agendaProperties.elements.agendaData[index].autoDecision.checkFlag;
						if(autoDecision)
							agendaDeciFlag = true;
					}
					else {
				...
				MeetingController.createMeetingAgenda(data, agendadata, meetnginfo).then(
					success => {
						...
						if(_agendaProperties.massAgenda) {
							successMsg = NLS.massAgendaCreateSuccessMsg;
							successMsg = successMsg.replace("{count}", agendaCount);	
						}
						else
						...
						var totalDur = parseInt(widget.getValue('SumOfAgendaDuration'));
						if(_agendaProperties.massAgenda) {
							success.data.forEach(function(agendaItem) {
								totalDur = totalDur + parseInt(agendaItem.relelements.topicDuration);
							});
						}
						else
						...
						if(autoDecision || agendaDeciFlag){
...
*/
		...
		let selected_Objects_search = function(_agendaProperties, data) {
			...
			if (data[0]["ds6w:type_value"] == "Person") {
				...
				if(_agendaProperties.mode == "massAgenda") {
					_agendaProperties.speaker.selectedItems = node;
					_agendaProperties.speaker.selectedItems.select();
				}
				else {	
			...
			if(_agendaProperties.mode == "massAgenda") {
				_agendaProperties.speaker.options.speakerId = data[0].id;
				if (data[0]["ds6w:type"].includes("Person"))
					_agendaProperties.speaker.options.speakerType = data[0]["ds6w:type_value"].unescapeHTML();
				_agendaProperties.speaker.options.agendaSpeakerUsername = data[0]["ds6w:label"];
			}
			else {
...
define('DS/ENXMeetingMgmt/View/Form/AgendaView',
	...
	function(WUXLineEditor, WUXEditor, WUXButton, WUXToggle, WUXButtonGroup, WUXComboBox, WUXDatePicker,
		...
		let _agendaProperties, _massAgendaProperties = {};
		...
		let massAgendabuild = function(agendaModel, container, meetnginfo) {
			meetingInfoVar = meetnginfo;
			_massAgendaProperties = {};
			_massAgendaProperties.currentValue = {};
			destroy();
			var data = { model: agendaModel.model.options.grid };
			if (!data.model) {
				data.model = {};
				_massAgendaProperties.mode = agendaModel.mode;
			}
			
			_massAgendaProperties.elements = {};
			_massAgendaProperties.formBody = new UWA.Element('div', { id: 'MassAgendaFormBody', class: 'agenda-prop-body' });
			_massAgendaProperties.formBody.inject(container);				
			_massAgendaProperties.formFields = new UWA.Element('div', { id: 'MassAgendaFormContainer', class: 'agenda-prop-container' });
			_massAgendaProperties.formFields.inject(_massAgendaProperties.formBody);
			_massAgendaProperties.closeEventName = "meeting-massAgenda-close-click"; //Used in AgendaViewUtil after creation success
			
			// Mass Agenda Contents //
			var massAgendaContentsDiv = new UWA.Element("div", {
				"id": "massAgendaContents"
			}).inject(_massAgendaProperties.formFields);
			
			var massAgendaHeaderDiv = new UWA.Element("div", {
				"id": "massAgendaHeader",
				"class": "massAgendaItemList"
			}).inject(massAgendaContentsDiv);
			
			// Agenda Topic Header //
			var topicHeaderDiv = new UWA.Element("div", {
				"id": "massAgendaTopicHeader",
				"title": NLS.topic
			}).inject(massAgendaHeaderDiv);				
			labelAttr = new UWA.Element("h5", { "class": "required", text: NLS.topic }).inject(topicHeaderDiv);
			UWA.createElement("div", { "class": "required-label-agenda fonticon fonticon-asterisk-alt" }).inject(labelAttr);
								
			// Agenda Speaker Header //
			var agendaSpeakerDiv = new UWA.Element("div", {
				"id": "massAgendaSpeakerHeader",
				"title": NLS.speaker
			}).inject(massAgendaHeaderDiv);				
			labelAttr = new UWA.Element("h5", { "class": "", text: NLS.speaker }).inject(agendaSpeakerDiv);

			// Agenda Duration Header  //
			var durationHeaderDiv = new UWA.Element("div", {
				"id": "massAgendaDurationHeader",
				"title": NLS.durationInMinutes
			}).inject(massAgendaHeaderDiv);
			labelAttr = new UWA.Element("h5", { "class": "required", text: NLS.durationMin }).inject(durationHeaderDiv);
			UWA.createElement("div", {
				"class": "required-label-agenda fonticon fonticon-asterisk-alt"
			}).inject(labelAttr);
			
			// Agenda Duration Apply-All //
			var durationApplyAllDiv =	new UWA.Element("div", {
				"id": "massAgendaDurationApplyAll",
				"title": NLS.applyAll
			}).inject(durationHeaderDiv);				
			let durationApplyAll = new WUXLineEditor({
				placeholder: NLS.placeholderAgendaItemDuration,
				value: "10",
				sizeInCharNumber: 10,
				pattern: '([0-4]?\\d{0,2}([.]\\d+)?)|500(.[0]+)?'
			}).inject(durationApplyAllDiv);
			_massAgendaProperties.elements.massDurationApplyAll = durationApplyAll;
			
			_massAgendaProperties.elements.massDurationApplyAll.addEventListener('change', function(e) {
				widget.meetingEvent.publish('create-massAgenda-ApplyAll-duration', _massAgendaProperties);
			});
			
			// Agenda Auto-Decision Header //
			var agendaDeciDiv = new UWA.Element("div", {
				"id": "massAgendaDecisionHeader",
				"title": NLS.autoDecision
			}).inject(massAgendaHeaderDiv);
			let autoMassDecisionCheck = new WUXToggle({ type: "checkbox", label: NLS.decisions, value: 0 }).inject(agendaDeciDiv);
			_massAgendaProperties.elements.massDeciSelectAll = autoMassDecisionCheck;
			
			let isDeciSelectAllChange = false;
			let isSingleDeciChange = false;

			_massAgendaProperties.elements.massDeciSelectAll.addEventListener('change', function(e) {
				if(isDeciSelectAllChange || isSingleDeciChange) return;
				isDeciSelectAllChange = true;
				widget.meetingEvent.publish('create-massAgenda-toggle-decisionSelectAll', _massAgendaProperties);
				isDeciSelectAllChange = false;
			});

			// Agenda Remove Item Header //
			var removeHeaderDiv = new UWA.Element("div", {
				"id": "massAgendaRemoveHeader"
			}).inject(massAgendaHeaderDiv);			
			
			_massAgendaProperties.elements.agendaData = [];
			
			agendaModel.topicItems.forEach(function(agendaItem, index) {
				// Mass Agenda Item //
				var massAgendaItemDiv = new UWA.Element("div", {
					"id": "massAgendaItem-"+index,
					"class": "massAgendaItemList"
				}).inject(massAgendaContentsDiv);
								
				// Agenda Item Topic //
				var topicItemDiv = new UWA.Element("div", {
					"id": "massAgendaItemTopic-"+index,
					"title": agendaItem.name,
					"class": "massAgendaItemTopic"
				}).inject(massAgendaItemDiv);
				
				if(!agendaItem.name)
					agendaItem.name = "";
				let topicTitle = new WUXLineEditor({
					placeholder: NLS.placeholderItemTopic,
					pattern: '[^\./#,\\[\\]\\$\\^@\\*\\?%:\'"\\\\<>]+',
					value: agendaItem.name,
					sizeInCharNumber: 45
				}).inject(topicItemDiv);
				
				_massAgendaProperties.elements.agendaData[index] = {};
				_massAgendaProperties.elements.agendaData[index].mode = "massAgenda";
				_massAgendaProperties.elements.agendaData[index].topic = topicTitle;
				
				_massAgendaProperties.elements.agendaData[index].topic.addEventListener('uncommittedChange', function(e) {
					widget.meetingEvent.publish('create-massAgenda-toggle-dialogbuttons', _massAgendaProperties);
				});
				
				// Agenda Item Speaker //
				var speakerItemDiv = new UWA.Element("div", {
					"id": "massAgendaItemSpeaker-"+index,
					"class": "massAgendaItemSpeaker"
				}).inject(massAgendaItemDiv);
				...
				let acOptions = {
					allowFreeInputFlag: false,
					elementsTree: asyncModelForSpeaker,
					placeholder: NLS.typeSearchPlaceHolder,
					multiSearchMode: false,
					minLengthBeforeSearch: 3,
					keepSearchResultsFlag: false
				};
				...
				autoCompleteSpeaker.addEventListener('change', function(e) {
					...
					if (typeof e.dsModel.elementsTree != 'function') {
						e.dsModel.elementsTree = autocompleteCB;
						if (!autoCompleteSpeaker.selectedItems) {
							_massAgendaProperties.elements.agendaData[index].speaker._model.removeRoots();
							_massAgendaProperties.elements.agendaData[index].speaker.options.speakerId = "";
							_massAgendaProperties.elements.agendaData[index].speaker.options.agendaSpeakerUsername = "";
						}
					}
				});

				_massAgendaProperties.elements.agendaData[index].speaker = autoCompleteSpeaker;
				_massAgendaProperties.speakerField = new UWA.Element('div', { id: 'SearchAgendaSpeakerField', class: "fonticon-chooser-display searchAgendaSpeaker" });
				_massAgendaProperties.elements.agendaData[index].speaker.inject(_massAgendaProperties.speakerField);
				
				new UWA.Element('div', { html: "&nbsp;" }).inject(_massAgendaProperties.speakerField);
				var agendaSpeakerChooser = new WUXButton({ displayStyle: "lite", icon: { iconName: "search" } });
				agendaSpeakerChooser.inject(_massAgendaProperties.speakerField);

				agendaSpeakerChooser.getContent().addEventListener('buttonclick', function() {
					AgendaViewUtil.launchSpeakerSearch(event, _massAgendaProperties.elements.agendaData[index], meetnginfo);
				});
				
				_massAgendaProperties.speakerField.inject(speakerItemDiv);					
				
				// Agenda Item Duration //
				var durationItemDiv = new UWA.Element("div", {
					"id": "massAgendaItemDuration-"+index,
					"class": "massAgendaItemDuration"
				}).inject(massAgendaItemDiv);
				_massAgendaProperties.elements.agendaData[index].duration = new WUXLineEditor({
					placeholder: NLS.placeholderAgendaItemDuration,
					value: "10",
					sizeInCharNumber: 10,
					pattern: '([0-4]?\\d{0,2}([.]\\d+)?)|500(.[0]+)?'
				}).inject(durationItemDiv);
				
				_massAgendaProperties.elements.agendaData[index].duration.addEventListener('uncommittedChange', function(e) {
					widget.meetingEvent.publish('create-massAgenda-toggle-dialogbuttons', _massAgendaProperties);
				});					
				
				// Agenda Item Auto Decision checkbox //
				var autoDeciItemDiv = new UWA.Element("div", {
					"id": "massAgendaItemDecision-"+index,
					"class": "massAgendaItemDecision",
					"title": NLS.autoDecision
				}).inject(massAgendaItemDiv);

				let autoDecisionCheck = new WUXToggle({ type: "checkbox", label: NLS.CreateAgenda, value: 0 }).inject(autoDeciItemDiv);
				_massAgendaProperties.elements.agendaData[index].autoDecision = autoDecisionCheck;
				
				_massAgendaProperties.elements.agendaData[index].autoDecision.addEventListener('change', function(e) {
					if (isDeciSelectAllChange) return;
					isSingleDeciChange = true;
					widget.meetingEvent.publish('create-massAgenda-toggle-decisionSelect', _massAgendaProperties);
					isSingleDeciChange = false;
				});	

				// Agenda Item Remove //
				var durationItemDiv = new UWA.Element("div", {
					"id": "massAgendaItemRemove-"+index,
					"class": "massAgendaItemRemoveBtn",
					"title": NLS.removeItem,
				}).inject(massAgendaItemDiv);
				let removeAgendaItem = new WUXButton({
				...
				removeAgendaItem.addEventListener('click', function(e) {
					const itemDiv = e.target.className.contains("wux-controls-button") ? e.target.parentElement : e.target.parentElement.parentElement;						
					widget.meetingEvent.publish('create-massAgenda-remove-agendaItem', {properties: _massAgendaProperties, removedItem: itemDiv});
					isSingleDeciChange = true;
					widget.meetingEvent.publish('create-massAgenda-toggle-decisionSelect', _massAgendaProperties);
					isSingleDeciChange = false;
				});			
		...
		let _ondrop = function(e, target) {
			if(target.id == "InitiateAgendaPropertiesBody")
				DragAndDropManager.onDropTopicItemsManager(e, _agendaProperties, "Agenda topic");
			else if(target.id == "meetingAgendaContainer") {
				let access = meetingInfoVar.model.ModifyAccess == "TRUE" ? true : false;
				DragAndDropManager.onDropMassAgendaManager(e,access,"meetingAgendaTab");
			}				
		};
			
		let AgendaView = {
			init: (data, container, meetnginfo, mode) => { return build(data, container, meetnginfo, mode); },
			initMassAgenda: (data, container, meetnginfo) => { return massAgendabuild(data, container, meetnginfo); },
			hideView: () => { hideView(); },
			getAgendaProperties: () => { return _agendaProperties },
			getMassAgendaProperties: () => { return _massAgendaProperties },
...
define('DS/ENXMeetingMgmt/View/Form/MeetingProperties',
	...
	function(MeetingUtil, WUXLineEditor, WUXEditor, WUXButton, WUXToggle, WUXAccordeon, WUXButtonGroup, WUXComboBox, WUXPopup, WUXDatePicker, ModalLoader, TreeDocument, 
		TreeNodeModel, MeetingController, NewMeetingContextChooser, SearchUtil, AutoCompleteUtil, DragAndDrop, DragAndDropManager,EnoviaBootstrap, DataFormatter, NLS) {
		...
		let build = function(container, data, mode) {
			ModalLoader.displayModalLoader(document.getElementById("initiateMeeting"), "Loading");
			const attrFetch = [
				MeetingController.getTypesForCreate("Meeting"),
				MeetingController.getTypeAttributes("Meeting")
			];
			Promise.allSettled(attrFetch).then(function(results) {
				
				let errorMsg = "";
				
				if(results[0].status === "fulfilled"){
					data.types = results[0].value;
				}
				else{
					console.log(results[0].reason);
					errorMsg = NLS.errorMeetingCreateDialog;
				}
				
				data.selectedType = "Meeting"
				data.tooltipInfos = {
					shortHelp: NLS.meeting
				};
				
				let customFields = widget.getValue("customFields");
				let customAttributes = [];
				if(customFields && customFields.items){
					customAttributes = customFields.items.filter(
						attr => Object.prototype.hasOwnProperty.call(attr, "extension") && attr.extension === "XP_Meeting_Ext"
					);
				}
				...
				if(results[1].status === "fulfilled"){
					let attributesData = results[1].value.data;
					attributesData.forEach(attribute => {
						let attrDetails = attribute.dataelements;
						data.additionalAttributes.groupAttributes.push(attrDetails);
					});
				}
				else{
				...
				if(errorMsg != ""){
					ModalLoader.removeModalLoader(document.getElementById("initiateMeeting"), "Loading");
					widget.meetingNotify.handler().addNotif({
						level: 'error',
						subtitle: errorMsg,
						sticky: false
				...
				let properties = buildPropertiesBody(container, data);
				ModalLoader.removeModalLoader(document.getElementById("initiateMeeting"), "Loading");
				return properties;
			})
		};
		
		let buildPropertiesBody = function(container, data, mode) {
			...
			_properties.customFields = {
				"items": Object.values(data.additionalAttributes).flat()
			}
			...
			if(data.types && data.types.length > 1) {
				let typeSelectionDiv = new UWA.Element('div', {
					id: 'type', class: "typeSelection-field"
				}).inject(_properties.formFields);
	
				let typeSelectionDivLabel = new UWA.Element("h5", {"class": "fieldRequired", text: "Type"}).inject(typeSelectionDiv);
				if (fieldRequired) {
					UWA.createElement("div", {
						"class": "required-label-meetings fonticon fonticon-asterisk-alt"
					}).inject(typeSelectionDivLabel);
				}
	
				_properties.elements.typeSelector = new WUXComboBox({
					domId: "typeSelector",
					class: "create-meeting-form-combobox",
					elementsList: data.types,
					enableSearchFlag: false,
					popupSizeAdjustPolicy: WUXPopup.SizeAdjustPolicyEnum.NoAdjust,
					value: data.selectedType,
					tooltipInfos: data.tooltipInfos
				}).inject(typeSelectionDiv);
				document.getElementById("typeSelector").classList.add("create-meeting-form-combobox");
	
				_properties.elements.typeSelector.addEventListener('change', function(e) {
					const selectedType = e.dsModel._properties.value.value;
					ModalLoader.displayModalLoader(document.getElementById("initiateMeeting"), "Loading");
					MeetingController.getTypeAttributes(selectedType).then(function(attributes){
						let createPropertiesDiv = document.getElementById("MeetingPropertiesBody");
						if(createPropertiesDiv && createPropertiesDiv.remove) {
							createPropertiesDiv.remove();
						}
						
						data.additionalAttributes.typeAttributes = [];
						data.additionalAttributes.groupAttributes = [];
						let attributesData = attributes.data;
						attributesData.forEach(attribute => {
							let attrDetails = attribute.dataelements;
							if(attrDetails.extension == undefined){
								data.additionalAttributes.typeAttributes.push(attrDetails);
							}
							else{
								data.additionalAttributes.groupAttributes.push(attrDetails);
							}
						...
						data.tooltipInfos = {
							shortHelp: e.dsModel.label
						};
						buildPropertiesBody(container, data);
						ModalLoader.removeModalLoader(document.getElementById("initiateMeeting"), "Loading");
					});
				});
			}
			...
			_properties.elements.collaborativeSpace.inject(CollabSpaceDiv);
			...
			if (organizationsData.length < 2) {
				OrganizationDiv.classList.add('hideOrganization');
			};
			_properties.elements.collaborativeSpace.addEventListener('change', function() {
				...
				if (organizationsData.length > 1) {
					OrganizationDiv.classList.remove('hideOrganization');
			...
			// Start of the more/advanced form fields section
			...
			//custom attributes and group attributes (group attributes on cloud only)
			if (_properties.customFields) {
				_properties.customFields.items.forEach((ele) => {
					drawAdditionalAttributes(_properties, ele);
				});
			}
			
			new UWA.Element("h5", { "class": "" }).inject(_properties.formFields);
			
			// "More" expandable accordean section to add in the end
		...
		const drawAdditionalAttributes = function(_properties, ele) {
						
			let containerDiv;	
			
			let required = (ele.mandatory === true || ele.mandatory === "Yes") ? "required" : "";
			let requiredFlag = (ele.mandatory === true || ele.mandatory === "Yes") ? true : false;
			
			let parentDiv = requiredFlag ? _properties.formFields : _properties.advFormFields;
...
function (WUXLineEditor, WUXEditor, WUXButton, WUXToggle, WUXAccordeon, WUXButtonGroup, WUXComboBox, WUXDatePicker,
	...
	let build= function (data,container,mode) {
		...
		access = data.model.ModifyAccess == "TRUE" ? true : false;
		if(!showView()){
			// Create the container in which all Meeting properties details will be rendered //
			_meetingProperties.elements = {};
			
			// Layout - Header Body Footer //
			_meetingProperties.formContainer = new UWA.Element('div', {id: 'MeetingAssociatedInfoContainer','class':'meeting-associated-info-container'});
			_meetingProperties.formContainer.inject(container);
			
			_meetingProperties.formHeader = new UWA.Element('div', {id: 'MeetingAssociatedInfoHeader','class':'meeting-associated-info-header'});
			_meetingProperties.formHeader.inject(_meetingProperties.formContainer);
			_meetingProperties.formFields = new UWA.Element('div', {id: 'MeetingAssociatedInfoBody','class':'meeting-prop-body meeting-properties-form-field'});
			_meetingProperties.formFields.inject(_meetingProperties.formContainer);
			_meetingProperties.formFooter = new UWA.Element('div', {id: 'MeetingAssociatedInfoFooter','class':'meeting-prop-footer'});
			...
			var meetingEditButtonContainer = new UWA.Element('div', {
				id: 'meetingEditButtonContainer', class: ""
		    ...
		    if(fieldViewOnly && data.model.ModifyAccess  == "TRUE"){
				UWA.createElement('div',{
					...
					"title" : NLS.Edit,
					"class" : "wux-ui-3ds wux-ui-3ds-1x wux-ui-3ds-pencil fonticon-display fonticon-color-display",
					styles : {"font-size": "21px","float":"right"},
					events : {
		                click: function (event) {
		                	document.querySelector('#MeetingAssociatedInfoContainer').destroy();
		                	build(data,container,"edit");	
		                }
					}
				}).inject(meetingEditButtonContainer);
			}
			...
			if (fieldViewOnly) {
				var meetingContextDiv = new UWA.Element('div', {
					id: 'contextId', class: 'associated-info-field'
				...
				var meetingContextDiv = new UWA.Element('div', {
					id: 'contextId-dropzone', class:"droppable show meeting-context-dropzone associated-info-field",
				}).inject(_meetingProperties.formFields);
			...
			if (fieldViewOnly) {
				...
				if(!data.model.CoOwners){
					...
					let coOwnerAndSearchDiv = UWA.createElement('div', {
						'class': 'coOwner-field-and-search-button'
					}).inject(coOwnerField);
	...
	var selected_text = function(agndaProperties, result){
 	...
 	let hideView = function(){
        if(document.getElementById('MeetingAssociatedInfoContainer') != null){
            document.getElementById('MeetingAssociatedInfoContainer').style.display = 'none';
        }
    };
	
	let getProperties = function() {
		return _meetingProperties;
	};
	...
	let showView= function(){
	   if(document.querySelector('#MeetingAssociatedInfoContainer') != null){
	       document.getElementById('MeetingAssociatedInfoContainer').style.display = 'flex';
	       return true;
	   }
	   return false;
	};
	...
	onDrop = function(e, target) {
		if(target.id == "MeetingAssociatedInfoBody")
	...
	function(MeetingController, WAFData, UWACore, EnoviaBootstrap, ParseJSONUtil, MeetingUtil, NLS) {
		...
		let _createMeeting = function(properties, agenda, members, attachments) {
			...
			return new Promise(function(resolve, reject) {
				...
				initiateJson = new ParseJSONUtil().createDataForRequest(initiateJson);
				initiateJson.data[0].type = properties.elements.typeSelector ? properties.elements.typeSelector.value : "Meeting";
				initiateJson.data[0].relateddata = {};
...
define('DS/ENXMeetingMgmt/View/Dialog/MassAgendaDialog', [
  'DS/Windows/Dialog',
  'DS/Windows/ImmersiveFrame',
  'DS/Controls/Button',
  ...
  function (
	...
	let dialog, agendaTopics;
	
	let createMassAgendaDialog = function(data) {
	
		let agendaContentContainer = new UWA.Element('div',{"id":"Meeting-massAgenda-container"});
		
		let meetingInfo = data.meetinginfo;
		let itemIds = data.itemsId;
		
		if(document.querySelector('#MassAgendaFormBody'))
			document.querySelector('#MassAgendaFormBody').destroy();

		let _massAgendaProperties;
		
		agendaTopics = [];
		MeetingController.getContentInfo(itemIds).then(function(resp) {
			resp.data.forEach(function (objectInfo) {
				var name = (objectInfo.title == undefined) ? objectInfo.name : objectInfo.title;																							
				agendaTopics.push({ id: objectInfo.id, name : name });
			});
			data.topicItems = agendaTopics;
			
			_massAgendaProperties = AgendaView.initMassAgenda(data, agendaContentContainer, meetingInfo);
		
			var immersiveFrame = new WUXImmersiveFrame();
		    immersiveFrame.inject(document.body);
			
			let okBtnLabel = NLS.AddMassAgenda;
			okBtnLabel = okBtnLabel.replace("{count}", resp.data.length);
			...
			dialog = new WUXDialog({
				  ...
				  width:550,
	              height:300,
				  title: NLS.addAgendaItems,
				  content: agendaContentContainer,
	              immersiveFrame: immersiveFrame,
				  buttons: {
	                Ok: new WUXButton({
						//disabled: true,	//Un-comment if no default value for duration
	                    label: okBtnLabel,
	                    onClick: function (e) {							
							_massAgendaProperties.elements.agendaData.forEach(function(agendaItem) {
								let autoCompleteSpeaker = agendaItem.speaker;
								if(autoCompleteSpeaker.selectedItems != undefined && autoCompleteSpeaker.selectedItems.options.id){						
									agendaItem.speaker.options.speakerId = autoCompleteSpeaker.selectedItems.options.id;
									agendaItem.speaker.options.agendaSpeakerUsername = autoCompleteSpeaker.selectedItems.options.label;									
								}							
							});
							
							if((MeetingUtil.validateAgenda(_massAgendaProperties.elements.agendaData,true)) == "false")
			      				return;
							
							e.dsModel.disabled = true;
							_massAgendaProperties.massAgenda = true;
							AgendaViewUtil.agendaActionCreate(data,_massAgendaProperties,meetingInfo,e);
	                    }
	                }),
	                Cancel: new WUXButton({
	                    onClick: function (e) {
							widget.meetingEvent.publish('meeting-massAgenda-close-click');
	                    }
	                })
	            }
			});
			...
			widget.meetingEvent.subscribe('meeting-massAgenda-close-click', function () {
				if(agendaTopics)	agendaTopics = [];
				if(_massAgendaProperties) _massAgendaProperties = {};
				if(dialog!=undefined){
	...
	let registerAgendaDialogButtonEvents = function(){
		widget.meetingEvent.subscribe('create-massAgenda-toggle-dialogbuttons', function (_massAgendaProperties) {
			if(_massAgendaProperties) {
				let enableAddBtn = true;
				_massAgendaProperties.elements.agendaData.forEach(function(agendaItem) {
					let topic = agendaItem.topic._properties.valueToCommit.value;
					let duration = agendaItem.duration._properties.valueToCommit.value;
					if(duration.trim() == "" || topic.trim() == "")
						enableAddBtn = false;
				});
				if(enableAddBtn)
		...
		widget.meetingEvent.subscribe('create-massAgenda-ApplyAll-duration', function (_massAgendaProperties) {
			if(_massAgendaProperties) {
				const durationAll = _massAgendaProperties.elements.massDurationApplyAll.value.trim();
				if(durationAll && durationAll != "") {
					_massAgendaProperties.elements.agendaData.forEach(function(agendaItem) {
						agendaItem.duration.value = durationAll;
					});
				}
			}
		});

		widget.meetingEvent.subscribe('create-massAgenda-toggle-decisionSelectAll', function (_massAgendaProperties) {
			if(_massAgendaProperties) {
				const selectAllFlag = _massAgendaProperties.elements.massDeciSelectAll.checkFlag;
				_massAgendaProperties.elements.agendaData.forEach(function(agendaItem) {
					if(agendaItem.autoDecision.checkFlag != selectAllFlag)
						agendaItem.autoDecision.checkFlag = selectAllFlag;
				});
			}
		});

		widget.meetingEvent.subscribe('create-massAgenda-toggle-decisionSelect', function (_massAgendaProperties) {
			if(_massAgendaProperties) {
				let selectedCount = 0;
				let deSelectedCount = 0;
				_massAgendaProperties.elements.agendaData.forEach(function(agendaItem) {
					agendaItem.autoDecision.checkFlag ? ++selectedCount : ++deSelectedCount;
				});
				
				if(selectedCount == 0) {
					_massAgendaProperties.elements.massDeciSelectAll.checkFlag = false;
					_massAgendaProperties.elements.massDeciSelectAll.mixedState = false;
				}
				else {
					_massAgendaProperties.elements.massDeciSelectAll.checkFlag = true;					
					_massAgendaProperties.elements.massDeciSelectAll.mixedState = (deSelectedCount > 0) ? true : false;
				}
			}
		});
		
		widget.meetingEvent.subscribe('create-massAgenda-remove-agendaItem', function (massAgendaInfo) {
			if(massAgendaInfo && massAgendaInfo.removedItem) {
				let removeBtnDivs = document.getElementsByClassName(massAgendaInfo.removedItem.className);
				if(removeBtnDivs) {
					let removedIndex = Array.from(removeBtnDivs).indexOf(massAgendaInfo.removedItem);
					if(removedIndex != -1 && massAgendaInfo.properties.elements.agendaData[removedIndex]) {
						massAgendaInfo.removedItem.parentElement.destroy();	
						massAgendaInfo.properties.elements.agendaData.splice(removedIndex, 1);
						agendaTopics.splice(removedIndex, 1);
						let updatedOkBtnLabel = NLS.AddMassAgenda;
						updatedOkBtnLabel = updatedOkBtnLabel.replace("{count}", massAgendaInfo.properties.elements.agendaData.length);
						dialog.buttons.Ok.label = updatedOkBtnLabel;
						if(removeBtnDivs.length == 0)
							widget.meetingEvent.publish('meeting-massAgenda-close-click');											
					}
				}			
			}
    ...
    let MassAgendaDialog = {
		init: (data) => { return createMassAgendaDialog(data) },
		getDialog: () => { return dialog; }
	};
	
	return MassAgendaDialog;
});
...
define('DS/ENXMeetingMgmt/View/Facets/MeetingAgenda',
    ...
    let build = function(_meetingInfoModel,massupdate){
		access = _meetingInfoModel.model.ModifyAccess == "TRUE" ? true : false;
    	 if(!showView() || massupdate){			
    		 ...
    		 containerDiv.inject(document.querySelector('.meeting-facets-container'));
			 if(access)
			 	DropZone.makeDroppable(containerDiv, _onAgendaDrop);
			 ...
			 //drag and drop initialize to store Meeting data
			 DragAndDropManager.init(_meetingInfoModel);
    	 }
    };
	
	_onAgendaDrop = function(e, target){
		if(target.id == "meetingAgendaContainer")
			DragAndDropManager.onDropMassAgendaManager(e,access,"meetingAgendaTab");
		else if(target.id == "InitiateAgendaPropertiesBody") {
			let _agendaProperties = AgendaView.getAgendaProperties();
			DragAndDropManager.onDropTopicItemsManager(e, _agendaProperties, "Agenda topic");
		}
		else if(target.id == "MeetingAssociatedInfoBody") {
			let meetingProperties = MeetingView.getProperties();
			DragAndDropManager.onDropManagerContext(e, meetingProperties, target);
		}		
	};
	...
	function _setToolbarBtnTooltip(homeToolbar, btnId, tooltip) {
		...
		homeToolbar.updateNodeModel(btnId, { tooltip: tooltip, "tooltipInfos" : tooltip });
	}
    ...
    let showView= function(){
        if(document.querySelector('#meetingAgendaContainer') != null){
			...
			DropZone.makeDroppable(document.getElementById('meetingAgendaContainer'), _onAgendaDrop); 
            return true;
        }
        return false;
    };
    ...
    let MeetingSummaryGridViewConfig = {
		...
		setMeetingSummaryGridViewConfig: function() {
			...
			if (customFields && customFields.items && customFields.items.length && customFields.items.length>0) {
				...
				customFields.items.forEach((ele) => {
					if (ele.name != 'extensions' && ele.extension == "XP_Meeting_Ext") {
						let tempObj = {};
    ...
    var _nameInWidgetTitle, _storages;
    	 ...
    	 let init = function(){
    		 return new Promise(function(resolve, reject) {
				...
				if(!widget.getValue("meetingTypes")){
					MeetingController.getTypesForCreate("Meeting").then((meetingTypes) => {
	  					const meetingTypesList = meetingTypes.filter(obj => 'value' in obj)
	 	 				  					.map(obj => obj.value);
	 	 				widget.setValue("meetingTypes", meetingTypesList);
	 	 			})
	 	 			.catch((err) => { console.log(err); });
				} 

				if(!widget.getValue("decisionTypes")){
					MeetingController.getTypesForCreate("Decision").then((decisionTypes) => {
	 					const decisionTypesList = decisionTypes.filter(obj => 'value' in obj)
		 				  					.map(obj => obj.value);
		 				widget.setValue("decisionTypes", decisionTypesList);
		 			})
		 			.catch((err) => { console.log(err); });
				}
...
function(MeetingAgendasDataGridView, MeetingAgendaActions, MeetingMemberDataGridView, MemberActions) {
   ...
   let getAttendanceEditedState = function () {
    ...
    let WrapperUtilities = {
		...
		getAgendaEditedState: () => {return getAgendaEditedState()},
		...
		var thumbnailSec = new UWA.Element('div',{
			...
			"html":[
				  UWA.createElement('div',{
					  ...
					  styles:{
						  "background-image": "url(" + data.model.thumbnailIcon + ")"
					  }
				  	})]
		});
...
define('DS/ENXMeetingMgmt/View/Facets/MeetingPropertiesTabs', [
  ...
  function (WUXTabBar,MeetingInfoFacets,MeetingPersistencyUtil,NLS) {
    ...
    var ntabs = ["MeetingPropertiesInfo", "MeetingAssociatedInfo", "MeetingRelationshipInfo", "MeetingHistory", "MeetingComments"];
	let ontabClick = function(args){
		let seltab = args.options.value;
		if(typeof seltab == 'undefined'){
			seltab = args.dsModel.buttonGroup.value[0]; //this is to get the selected tab from the model
...
define('DS/ENXMeetingMgmt/View/Dialog/CreateMeetingDialog', [
	...
	function(InitiateMeetingTabs,
		...
		let InitiateDialog = function(contentData, credentialResponseData) {
			...
			let myContent = new UWA.Element('div', { html: "", id: "initiateMeeting", styles: {position: "relative", height: "100%"}});
...
define('DS/ENXMeetingMgmt/View/RightSlideIn/PanelView', [
	...
	PanelView.prototype.init = function(data,loadFor) {
		destroyViews(); // to destroy any pre-existing views
		if(document.querySelector('#decisionInfoDisplayContainer')){
					document.querySelector('#decisionInfoDisplayContainer').destroy();
				}
		...
		if (editedStateValues && editedStateValues.editedState && editedStateValues.requiresConfirmationDialog && ((editedStateValues.unsavedEdit!="attendance" && (loadFor=="agendaPreview" || loadFor=="agendaEditView")) || loadFor=="meetingInfo")) {
			...
			if(loadFor == "meetingInfo"){
				...
				let decisionIDCardFacet = new DecisionIDCardFacets(displayContainer,loadFor);
				decisionIDCardFacet.init(data.model,loadFor);
			} /*else if(loadFor == "DecisionInfoIcon"){//acc2 to check
				
				var displayInfoContainer = new UWA.Element('div',{	
													"id":"decisionInfoDisplayContainer",
													styles:{"height":"100%"}
												});
				//containerDiv.inject(document.querySelector('.decision-facets-container'));
        		DecisionPropWidget.render(displayInfoContainer,data.models,data.mode);
        		displayContainer.appendChild(displayInfoContainer);
...
'DS/ENXMeetingMgmt/View/Dialog/MassAgendaDialog',
'DS/ENXMeetingMgmt/View/Dialog/OpenDialog',
'DS/ENXDecisionMgmt/Utilities/DecisionPersistencyUtil',
'DS/ENXMeetingMgmt/View/Dialog/NewOpenDialog'
	],
function(SplitView,PanelView,IdCardUtil,IdCardUtilDecision,DecisionModel,AgendaDialog,DecisionDataGridView,MassAgendaDialog,OpenDialog,DecisionPersistencyUtil,NewOpenDialog) {
    ...
    //    sView.setContextForGesture();
      ...
      RightPanelSplitView.prototype.setSplitviewEvents = function(rightContainer, splitView, midRightContainer){
		...
		widget.meetingEvent.subscribe('meeting-massAgenda-create-click', function (data) {
			IdCardUtil.infoIconInActive();
        	widget.setValue("meetPropOpen", false);
        	if (widget.sidePanelOpen)
        		widget.sidePanelOpen = false;
        	widget.meetingEvent.publish('meeting-agenda-close-click-view-mode');
			MassAgendaDialog.init(data);
        });
		...
		widget.meetingEvent.subscribe('meeting-agenda-create-click', function (data) {
          ...
          widget.meetingEvent.subscribe('meeting-header-info-click', function (data) {
			  ...
			  //To destroy decision edit prop when decision info button is active and meeting info is clicked 
			  //and also to inactivate decision info button on meeting info button click.
			  if (document.querySelector('#decisionInfoDisplayContainer')) {
				  document.querySelector('#decisionInfoDisplayContainer').destroy();
			  }
			  //Inactivate Info icon
			  if (DecisionDataGridView.getGridViewToolbar()) {
				  let infoIcon = DecisionDataGridView.getGridViewToolbar().getNodeModelByID("decisionInfoIcon");
				  if (infoIcon && infoIcon._associatedView && infoIcon._associatedView.elements?.container.querySelector('.wux-controls-button')) {
					  infoIcon._associatedView.elements.container.querySelector('.wux-controls-button').setStyle("color", 'rgb(119, 121, 124)');
					  //widget.setValue("propWidgetOpen", 0);
					  widget.setValue("isDecInfoIconActive", false);
					  widget.setValue("DecisionIdSelected", undefined);
				  }
			  }
        ...
        //   });
		  ...
		  widget.meetingEvent.subscribe('decision-delete-refresh-agenda', function () {
			  require(['DS/ENXMeetingMgmt/View/Facets/MeetingAgenda',
			  			'DS/ENXMeetingMgmt/Model/MeetingAgendaModel'
			  		], function(MeetingAgenda, MeetingAgendaModel) {
			  	if (document.querySelector('#meetingAgendaContainer') != null) {
			  		let meetInfo = MeetingAgendaModel.meetingInfo();
			  		if(meetInfo && meetInfo.model) {
			  			MeetingAgenda.destroy();
						widget.setValue('opAgendaResequence', false);
			  			MeetingAgenda.init(meetInfo, "false");
			  			if (document.querySelector('#meetingAgendaContainer') != null)
			  				document.getElementById('meetingAgendaContainer').style.display = "none";
			  		}
			  	}
          ...
          widget.meetingEvent.subscribe('decision-create-click', function (data) {
        	  IdCardUtil.infoIconInActive();
        	  widget.setValue("meetPropOpen", false);
        	  if(!DecisionPersistencyUtil.getInfoIconPersistency()){
				...
				//Inactivate Info icon
				 if (DecisionDataGridView.getGridViewToolbar()) {
					 ...
					 if (infoIcon && infoIcon._associatedView && infoIcon._associatedView.elements?.container.querySelector('.wux-controls-button')) {
						 ...
						 //widget.setValue("propWidgetOpen", 0);
						 if (widget.decisionSummary) {
							 widget.setValue("isDecInfoIconActiveSummary", false);
							 widget.setValue("DecisionIdSelectedSummary", undefined);
						 } else {
							 widget.setValue("isDecInfoIconActive", false);
							 widget.setValue("DecisionIdSelected", undefined);
						 }
...
define('DS/ENXMeetingMgmt/Components/Wrappers/MeetingApplicationFrame',
    ...
    function (Core, TriptychWrapper, DecisionPropWidget,DecisionDataGridView ,WelcomePanel, NLS) {
        'use strict';
        ...
        MeetingApplicationFrame.prototype._initDom = function () {
         ...
         MeetingApplicationFrame.prototype._subscribeRightPanelEvents = function () {
            ...
            //  rightPanelInfoView.destroyEditPropWidget();
             DecisionPropWidget.destroyEditPropWidget();
             let triptychManager = that._triptychWrapper._getTriptych();
             
             
             
             that._listRightPanelSubscription.push(that._applicationChannel.subscribe({ event: 'decision-info-click' }, function (data) {
				
				let containerDiv = document.querySelector('#meeting-right-panel');
				var displayContainer;
				
				if(document.querySelector('#rightPanelDisplayContainer')){
					document.querySelector('#rightPanelDisplayContainer').destroy();
				}
				
				/*if(document.querySelector('#decisionInfoDisplayContainer')){
					document.querySelector('#decisionInfoDisplayContainer').destroy();
				}
				var displayContainer = new UWA.Element('div',{	
													"id":"decisionInfoDisplayContainer",
													styles:{"height":"100%"}
												});*/
				if(document.querySelector('#decisionInfoDisplayContainer')){
					displayContainer =document.querySelector('#decisionInfoDisplayContainer');
				}else{
					displayContainer = new UWA.Element('div',{	
						"id":"decisionInfoDisplayContainer",
						styles:{"height":"100%"}
				...
				//containerDiv.inject(document.querySelector('.decision-facets-container'));
        		DecisionPropWidget.render(displayContainer,data.models,data.mode,data.fromEditProp);
				 ...
				 if (DecisionDataGridView.getGridViewToolbar()) {
					 ...
					 if (infoIcon && infoIcon._associatedView && infoIcon._associatedView.elements?.container.querySelector('.wux-controls-button')) {
						 infoIcon._associatedView.elements.container.querySelector('.wux-controls-button').setStyle("color", "rgb(54, 142, 196)");
					 	 //widget.setValue("propWidgetOpen", 1);
						 if (widget.decisionSummary) {
							 widget.setValue("isDecInfoIconActiveSummary", true);
						 }
						 else {
							 widget.setValue("isDecInfoIconActive", true);
						 }
             ...
             that._listRightPanelSubscription.push(that._applicationChannel.subscribe({ event: 'decision-info-close-click' }, function (data) {
		...
		_initializeComponent : function(options) {
			...
			return new Promise(
				...
				return new Promise(function(resolve, reject) {
					...
					let parentContainer = UWA.createElement('div', { 'id': 'usergroup-parent-div', 'styles': { 'height': '100%' } });
					let mainContainer = UWA.createElement('div', { 'id': 'meeting-main-div', 'styles': { 'height': '100%' } });

					var myImmersiveFrame = new ImmersiveFrame({});
							
							UWA.extendElement(mainContainer);
							UWA.extendElement(parentContainer);
					...
					mainContainer.inject(myImmersiveFrame.getFreeZone(), "top");
					myImmersiveFrame.inject(parentContainer);
					container = new WUXImmersiveFrame();
					container.setContentInBackgroundLayer(parentContainer);
					...
					if(widget.getValue('openedDecisionId') || widget.getValue('openedMeetingId')) {
						widget.setValue("landingPageToPersist", undefined);
						widget.setValue("meetIDToPersist", undefined);
						widget.setValue("widgetDeciIDToPersist", undefined);
					}
					widget.setValue("showinfoIconforDecision",true);
