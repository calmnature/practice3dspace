 ...
 * @quickreview KR5     26:01:07 IR-1483438 : added PADContext.get().getPADTreeDocument undefined check in init method
 * @quickreview KR5     25:10:29 IR-1460275 considered work under Authoring Context in updateStaticMappingDefinition method
 * @quickreview KR5     25:09:16 IR-1440411 considered selected nodes title in getData method used for Replace Instantiated Configuration By Revision dialog
...
], function (CfgController, CfgUtility, Utils, PADContext, AFRCommand, TouchUtils, CfgDialog, CfgTracker, CfgData, ModelEvents, CfgAuthoringContext, CfgReplacePCVersionExplorerView, CfgBaseUXNLS, ReplaceInstantiatedConfigurationNLS) {
    'use strict';
    ...
    var CfgReplaceInstantiatedConfigurationByRevisionCmd = AFRCommand.extend({
        ...
        init: function (options) {
            ...
            if (response != undefined) {
                ...
                //[IR-1057931 01-Mar-2023] Security context required for readConfigSettings web-service for StaticMappingActivation value.
                ...
                CfgUtility.populate3DSpaceURL().then(function () {
                    CfgUtility.populateSecurityContext().then(function () {
                        CfgUtility.readConfigSettings().then(function () {
            ...
            if (PADContext.get !== undefined && PADContext.get() !== null
                && PADContext.get().getPADTreeDocument !== undefined && PADContext.get().getPADTreeDocument() !== null)
                this._SelectorManager = PADContext.get().getPADTreeDocument().getXSO();
                ...
                for (let i = 0; i < length; i++) {
                    if (selectednodes[i].isRoot()) {
                        let item = {
                            ...
                            isRoot: true,
                            title: typeof selectednodes[i].getLabel === 'function' ? selectednodes[i].getLabel() : '',
                        };
        ...
        createReplaceProductConfigurationDialog: function (options) {
            ...
            let dialogTitle = options.dialogTitle ? options.dialogTitle : pcName;
            ...
            let dialogOption = {
                ...
                dialogue: {
                    ...
                    header: dialogTitle ? ReplaceInstantiatedConfigurationNLS.CFG_Replace_Product_Configuration_By_Revision_Header + ' - ' + dialogTitle : ReplaceInstantiatedConfigurationNLS.CFG_Replace_Product_Configuration_By_Revision_Header,
        ...
        getProductConfigrationInfo: async function (iOptions) {
            ...
            let dialogTitle = iOptions.data.selectedNodes[0].title;
                ...
                if (key != 'version') {
                    if (iResponse.hasOwnProperty(key)) {
                        if (iResponse[key].hasStaticMapping == 'true' && iResponse[key].content != undefined && iResponse[key].content.results != undefined && iResponse[key].filterID != undefined) {
                            ...
                            //IR-1440411 considered selected reference title which is different for Effectivity based REUSE scenario
                            if (dialogTitle != undefined) dialogTitle = dialogTitle + ' ' + revisionValue;

                            CfgReplaceInstantiatedConfigurationByRevisionCmd.productConfigrationData = { instanceId: instanceId, pcId: iResponse[key].filterID, pcName: pcStaticMappingValue, dialogTitle: dialogTitle };
                        }
                    }
                }
            }
