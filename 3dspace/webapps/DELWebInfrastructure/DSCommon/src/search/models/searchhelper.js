        ...
        getUnifiedSearchObjectsFromDrop: function(params, success, error) {
            ...
            if (objlist && objlist.length) {
                ...
                for (var kk = 0; kk < objlist.length; kk++) {
                    var obj = objlist[kk];
                    //if it's a VPMReference (or ErgoHuman for Worker objects), then we can query for the resource details. Otherwise, just save what we have.
                    if (obj.objectId && obj.objectType && (['VPMReference', 'ErgoHuman'].indexOf(obj.objectType) >= 0)) {
