...
if(window && window.jQuery && window.widget && window.widget.getValue) {
    ...
    if(jQuery.ajaxPrefilter) {
        ...
        ****/
        jQuery.ajaxPrefilter(function( options, originalOptions, jqXHR ){
            ...
            //before trying to figure it out for ourself, see if it is already being set from outside
            var context = (options && options.headers && options.headers.SecurityContext) ||
                          (originalOptions && originalOptions.headers && originalOptions.headers.SecurityContext);

            if (!context) {
                //it was not explicitly set, so grab whatever the widget preferences have it set as
                context = window.widget.getValue("pad_security_ctx") || window.widget.getValue('collabSpace') || window.widget.getValue('Credentials');

                if (!context && window.Foe && window.Foe.Model && window.Foe.Model.Session) {
                    context = Foe.Model.Session.get('security_ctx');
                    if (context && (context.indexOf('ctx::ctx::') >= 0)) {
                ...
                if (!context && window.widget.data && window.widget.data.X3DContentId) {
                    ...
                    if (X3DContentId && X3DContentId.data && X3DContentId.data.items && X3DContentId.data.items.length && X3DContentId.data.items[0].contextId) {
                        ...
                        if (window.Foe && window.Foe.Model && window.Foe.Model.Session) {
                            window.Foe.Model.Session.set('security_ctx', context); //just set this locally for now for the benefit of others
                        }
                    }
                }
            }
