    ...
    jQuery.ajaxPrefilter(function( options, originalOptions, jqXHR ){
      ...
      if(jqXHR && options && window.widget && window.widget.environment){
          ...
          if(window.widget.environment.name != 'uwa'){
            ...
            var secContext = (options && options.headers && options.headers.SecurityContext) ||
                             (originalOptions && originalOptions.headers && originalOptions.headers.SecurityContext);
            var strUser = (window.COMPASS_CONFIG ? window.COMPASS_CONFIG.userId : null);
            if (!secContext || !strUser) {
                secContext = secContext || window.widget.getValue("pad_security_ctx") || window.widget.getValue('collabSpace') || window.widget.getValue('Credentials');
                ...
                if (!secContext && window.widget.data && window.widget.data.X3DContentId) {
                    ...
                    if (X3DContentId && X3DContentId.data && X3DContentId.data.items && X3DContentId.data.items.length && X3DContentId.data.items[0].contextId) {
                        //NWT 2/15/2024 - IR-1233648-3DEXPERIENCER2024x (from jquery.uwa.securityContext.js)
                        ...
                        secContext = X3DContentId.data.items[0].contextId;
                        if (secContext && (secContext.indexOf('ctx::ctx::') >= 0)) {
                        ...
                        if (window.Foe && window.Foe.Model && window.Foe.Model.Session) {
                            window.Foe.Model.Session.set('security_ctx', secContext); //just set this locally for now for the benefit of others
                        }
                    }
                }
            }
            while (secContext && (secContext.indexOf('ctx::ctx::') >= 0)) {
