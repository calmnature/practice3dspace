...
const useNotificationsStore = defineStore('notifications', () => {
  ...
  const setUnreadTotal = (nbUnread) => {
    ...
    if (typeof nbUnread === 'number') {
      ...
      counter = Number.parseInt(nbUnread);
    }
  ...
  const setUnreadTotalFederation = ({ unread, clusterId }) => {
    ...
    if (!find) {
      unreadTotalFederation.value.push({ unread, clusterId });
    }
    else {
      find.unread = unread;
    }
  ...
  const getUnreadTotalFederation = computed(() => {
    return unreadTotalFederation.value.map(n => n.unread)
      .reduce((a, b) => a + b, 0);
  });
   ...
   * @param {number} id
   * @returns { Notification} return the notification witd the id.
   */
   ...
   * @returns { Notification} return the notification
   */
   ...
   * @param {number} id
   * @returns {Notification} return a merge depenfing of the id.
   */
  ...
  const getMergeByGroupId = (groupId) => {
    for (const [, notification] of merges) {
      if (notification.GROUPID === groupId)
        return notification;
    }
    return null;
  ...
  const addFilterNotifIds = (notifIds, clusterId, archive) => {
    for (const id of notifIds) {
      ...
      if (!find)
        filterNotifIds.value.push({ id, archive, clusterId });
    }
  };
   ...
   * @param {*} notification
   */
  const setMerge = ({ notification, options }) => {
