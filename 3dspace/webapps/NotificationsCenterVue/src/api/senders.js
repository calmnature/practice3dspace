 ...
 */
function setAnalyticsStartTime() {
...
export function notificationRead({
  ...
  if (notifId) {
    id = Number.parseInt(notifId.id);
  }

  if (params && !!params.id) {
    const paramId = checkIfIdHasHyphen(params.id);
    if (paramId) {
      params.id = Number.parseInt(paramId.id);
    }
  }
...
export function getAppInfos({ appID, notifID, clusterId }) {
  ...
  if (notifId) {
    notifID = Number.parseInt(notifId.id);
  }
...
export function getMerge({ parentNotif, filterData }) {
  ...
  if (notifId) {
    parentNotif.id = Number.parseInt(notifId.id);
  }
  const { filterOptions, clusters } = filterData ? setFilterOptionsPayload(filterData) : {}; ;
 ...
 * @param {object} options.subscribe - Indicates whether to subscribe to notifications.
 * @param {number} options.id - The ID of the sender.
 * @param {object} options.notif_by_browser - Indicates whether to receive notifications via browser.
 * @param {object} options.notif_by_email - Indicates whether to receive notifications via email.
 * @param {object} options.notif_by_ui - Indicates whether to receive notifications via UI.
 ...
 * @s {Promise} A promise that resolves when the settings are updated.
 */
export function updateServiceSettings({ subscribe, id, notif_by_email, tenantAware }) {
  driver.send({
    action: 'updateServiceSettings',
    data: { subscribe, id, notif_by_email, tenantAware },
  });
}
 ...
 * @s {Promise} A promise that resolves with the settings data.
 */
export function getServicesSettingsOfTenant() {
  // setAnalyticsStartTime();
  driver.send({ action: 'getServicesSettingsOfTenant', data: {} });
}

/**
 * Retrieves the DND status.
 */
export function getDNDStatus() {
  driver.send({ action: 'getDNDStatus', data: {} });
}
