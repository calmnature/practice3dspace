...
import { getDNDStatus, getServicesSettingsOfTenant, getSettings, resetSettingUnsubscriptionDate } from '~/api/senders';
...
export function usePreferencesManagement() {
   ...
   *
   * @param {number} [debounce] - The debounce time in milliseconds for the watcher.
   * @param {number} [maxWait] - The maximum wait time in milliseconds for the watcher.
   */
  const watchSettingWithUnsubDate = (debounce = 60000, maxWait = 60000) => {
    watch(
      ...
      (list) => {
        ...
        if (stopSettingUnsubDateWatcher)
          stopSettingUnsubDateWatcher();
        // start a new watcher
        if (list.length) {
          stopSettingUnsubDateWatcher = watchDebounced(
            ...
            (realTimeCurrentDate) => {
              for (let i = 0; i < list.length; i++) {
                ...
                if (new Date(setting.unsubscribe_date)
                  .getTime() < new Date(realTimeCurrentDate)
                  .getTime()) {
                  // reset the unsubscribe date
   ...
   */
  const loadSettings = () => {
    const settings = store.isPlatformPreferences
      ? { method: getServicesSettingsOfTenant, name: 'servicesSettings' }
      : { method: getSettings, name: 'settings' };
    if (!store.settings.size) {
      store.setSettingLoadingState(settings.name, true);
      settings.method();
    }
    else {
      store.setSettingLoadState(settings.name, true);
      store.setSettingLoadingState(settings.name, false);
    }
    if (!store.isDNDRefreshed)
      getDNDStatus();
  };
  ...
  const messageNotice = (msg, timeout) => {
    ...
    if (typeof timeout === 'number' && typeof msg === 'string') {
      notifStore.dispose = message.create({ text: msg, timeout });
    }
    else {
   ...
   * @returns {Promise<boolean>} Return true if the user confirm, false otherwise.
   */
  const alertConfirmNotice = async ({ title, msg, okLabel, cancelLabel }) => {
    try {
      await modal.confirm({
        title,
        message: msg,
  ...
  /**
   * return true if the setting can be update.
   *
   * @param {object} setting - The setting object.
   * @param {string} attribut - The attribute to update.
   * @returns {Promise<boolean>} true if setting can be update.
   */
  const settingUpdateVerification = async (setting, attribut) => {
    const needConfirm = setting.getNotifOptionEnabled(attribut);
    const needBrowserPermission = attribut === 'browser' && !setting.getNotifOptionEnabled(attribut);
