...
import { addAndSortIdsDesc, addAndSortNotifsDateDesc, checkIfIdHasHyphen, linkIds, replaceParenthesisStrong } from
  '~/functions/utils';
...
export function useNotificationManagement() {
   ...
   * @param {boolean} [withSettings] - Indicates whether to fetch settings.
   */
  const refreshCenterView = (withSettings = true, withServicesSettings = false) => {
  ...
  const addNotificationToSection = (notification) => {
    ...
    if (!store.sectionList.has(sectionId))
      store.sectionList.set(sectionId, section);
  };
   ...
   * @param {boolean} [withSettings] - Indicates whether to fetch settings.
   * @param {object} groupIdToExclude - ids to exluede.
   */
  ...
  const deleteNotification = (id, editTotal = true, fetchMore = true) => {
    ...
    //
    if (editTotal) {
      if (!notification.isRead && !notification.FROM_ARCHIVE)
        store.setUnreadTotal(store.unreadTotal - 1);
      if (isFilterApplied())
        store.setFilterTotal(store.filterTotal - 1);
    }
   ...
   * @param {object} options - The notification object to be added.
   * @param {object} options.notification - The notification object to be added.
   * @param {object} options.options - The options for the notification.
   * @param {boolean} [options.group] - Indicates whether the notification should be grouped.
   * @param {string} options.oldSectionId - The ID of the old section.
   * @param {string} options.oldNotifId - The ID of the old notification.
   */
  ...
  const addHistory = (data) => {
    ...
    if (isFilterApplied()) {
      ...
      if (data.archive)
        store.setUnreadTotal(store.unreadTotal + (data.notifications?.length ?? 0));
    }
    ...
    if (hasArchiveNotifications(data.notifications))
   ...
   * @param {boolean} [merge] - Indicates whether to merge the notification with the group.
   * @param {boolean} [last] - Indicates whether the copied notification is the last one.
   */
  ...
  const replaceGroupMessage = (notification) => {
    ...
    if (!newGroup)
      newGroup = store.getNotificationByGroupId(notification.GROUPID);
  ...
  const addNotificationToGroup = (notification, merge = false, last = false) => {
    ...
    addNotification({ notification: group, options, group: true, oldSectionId, oldNotifId: oldID });
    // replace the number of persons in message
    if (!last)
      replaceGroupMessage(notification);
  ...
  const DeleteNotificationGroup = (id) => {
    console.log('function not implement', id);
  };
   ...
   * @returns true if the notification is a group.
   */
   ...
   * @param {string} options.id - The ID of the notification to delete.
   * @param {boolean} [options.merge] - Indicates whether the notification is a merge notification.
   * @param {boolean} options.group - Indicates whether the notification is a group notification.
   * @param {boolean} options.archive - Indicates whether the notification is from the archive.
   * @param options.withCancelation
   */
  const tryNotificationDeletion = ({ id, merge = false, group = false, archive = false, withCancelation = true }) => {
    ...
    let notification;
    if (group || !merge)
      notification = store.getNotificationById(id);
  ...
  const setNotificationReadState = (notifData, data) => {
    ...
    if (data.read !== undefined) {
      ...
      if (!Object.hasOwn(notifData, 'hasMerges')) {
        ...
        notifData.READ_DATE = notifData.mergesRead === notifData.COUNT
          ? new Date()
              .toISOString()
          : null;
      }
  ...
  const setMergeReadState = (mergeData, data) => {
    ...
    if (data.read !== undefined) {
      ...
      if (data.read) {
        ...
        if (!mergeData.isRead)
          mainNotification.mergesRead = mainNotification.mergesRead + 1;
      }
      else {
    ...
    if (mainNotification.mergesRead === mainNotification.COUNT) {
      if (!mainNotification.isRead) {
        mainNotification.READ_DATE = new Date()
          .toISOString();
        if (!mainNotification.FROM_ARCHIVE)
          store.setUnreadTotal(store.unreadTotal - 1);
      }
    }
   ...
   */
  const unsubscribeNotification = ({ setting }) => {
    //
  ...
  const setActioned = (params) => {
    ...
    if (params.isMerge) {
      ...
      notification = store.getNotificationById(params.id);
    }
    // set the action date
    ...
    const inSearchesRange
      = filter.searches.length === 0
        || filter.searches.some((search) => {
          return notification.MESSAGE.nls.msg.toLowerCase()
   ...
   * @param {string} id - The ID of the notification.
   * @param {boolean} [merge] - Indicates whether the notification is a merge.
   */
  ...
  const loadMoreNotifications = () => {
    if (store.loadingMore || store.isLoading)
      return;
  ...
  const rollbackPopupAction = async (data) => {
    if (Object.keys(data.params).length) {
      ...
      // show the popup
      alertConfirmNotice({ title, msg: $i18n('rollbackPopupMsg') })
        .then((result) => {
