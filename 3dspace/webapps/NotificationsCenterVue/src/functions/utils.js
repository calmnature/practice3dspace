 ...
 */
function addAndSortNotifsDateDesc(array, el) {
  if (array.find(item => item.id === el.id))
    return;
  let index = 0;
 ...
 */
function addAndSortIdsDesc(array, id) {
  if (array.find(idItem => idItem === id))
    return;
  let index = 0;
  for (const idItem of array) {
    // -1 because it means it's a section with archive title
    if (id >= idItem && idItem !== -1)
      break;
    index++;
  }
 ...
 */
function addAndSortMerge(array, merge) {
  if (array.find(item => item.notification.id === merge.notification.id))
    return;
  let index = 0;
  for (const item of array) {
    if (merge.notification.id >= item.notification.id)
      break;
    index++;
  }
 ...
 * @returns {Promise<string>} The merge format of the date.
 */
async function getDateMergeFormat(date) {
 ...
 */
async function getDateSectionFormat(date) {
  if (timeDiffByDays(date) === 0) {
    return $i18n('today')
      .toUpperCase();
  }
    ...
    const I18n = await getI18n();
    return date.toLocaleDateString(I18n.getCurrentLanguage(), format)
      .toUpperCase();
  }
}
 ...
 * @returns Date + 1 hour.
 */
function getDateAfter1Hour(date) {
 ...
 */
function getDateAfter5Min(date) {
 ...
 * @returns {string} The date and time after adding 5 minutes, in ISO 8601 format.
 */
function getDateAfterXMin(date, x) {
  if (typeof x === 'number') {
 ...
 */
function checkIfIdHasHyphen(id) {
  try {
    if (id.includes('-')) {
      return {
        id: id.split('-')[0],
