 ...
 * @returns {Promise<boolean>} - Returns true if the notifications store has at least one notification, otherwise false.
 */
export async function setHistory(data = null) {
  let internalData;
  if (!data) {
    internalData = {
      appName: '3dswym',
  ...
  await eventHandlers.setHistory(internalData);
