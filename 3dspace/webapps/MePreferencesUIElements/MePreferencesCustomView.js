...
define('DS/MePreferencesUIElements/MePreferencesCustomView',
    ...
    function (WUXButton, WUXImmersiveFrame, WUXDialog, MePreferencesPlatformServicesUtils, MePreferencesWriter,
        ...
        MePreferenceCustomUIObj.prototype.createCustomDialog = async function (completeUrl, jsonValue, type_id, lockValue,
            isStandaloneUI, isAdminView, displayName, dialogHeight, dialogWidth, tID, dlgType, encryptedKey, readOnly = false, needrefresh) {
            ...
            if (customUIDialog == null) {
                ...
                if ((lockValue == "true" || readOnly === true) && !isAdminView) {
                ...
                async function onPreferenceUpdate(e) {
                    ...
                    if (iframeRemovalRequested == true && message == "com.ds.mep:OnPrefUpdated_msg" && (!isStandaloneUI)) {
                        ...
                        let data = [type_id, selectedvalue, type, needRefresh];
                        ...
                        var prefObj = {};
                        prefObj["name"] = type_id.split('.')[2];
                        prefObj["value"] = msgObj.jsonData;
                        prefArray.push(prefObj);

                        var repoObject = {};
                        repoObject["name"] = type_id.split('.')[1];
                        ...
                        return MePreferencesPlatformServicesUtils.getServiceUrl(tenantID, "mepreferences").then(async (url) => {
                            const Refresh = await mePreferencesWriterObj.save(url, repoObj, encryptedKey);
                            if (customUIDialog)
                                customUIDialog.close();
                        });
