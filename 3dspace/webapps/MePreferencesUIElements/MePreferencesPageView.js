define('DS/MePreferencesUIElements/MePreferencesPageView',
    ...
    function (MePreferencesUIBuilder, MePreferencesPageModelDataProvider, MePreferencesSelectionModel,
            ...
            if (appID === "3DSCommon.Language") {
                ...
                if (response) {
                    ...
                    for (var i = 0; i < ComJson.mePrefUIData.length; i++) {
                        ...
                        const currentAppID = (ComJson.mePrefUIData[i].appID).replace(/\./g, '_');

                        function preparePrefstyleDataWithAppID(dataArray, appID) {
                            const preparedPrefstyleArray = [];

                            for (let k = 0; k < dataArray.length; k++) {
                                const item = dataArray[k];
                                const newItem = { ...item };
                                newItem.appID = appID;
                                preparedPrefstyleArray.push(newItem);
                            }
                            return preparedPrefstyleArray;
                        }

                        function prepareNLSDataWithAppID(nlsObj, appID) {
                            const preparedNLSObj = {};
                            for (const key in nlsObj) {
                                if (Object.prototype.hasOwnProperty.call(nlsObj, key)) {
                                    const newKey = `${appID}.${key}`;
                                    preparedNLSObj[newKey] = nlsObj[key];
                                }
                            }

                            return preparedNLSObj;
                        }

                        if (appitem.prefStyle != "empty") {
                            for (let j = 0; j < appitem.prefStyle.length; j++) {
                                const rawPrefstyleData = JSON.parse(appitem.prefStyle[j].data);
                                const finalPrefstyleData = preparePrefstyleDataWithAppID(rawPrefstyleData, currentAppID);
                                style.push(appitem.prefStyle[j].fileName.split("_")[0])
                                if (UIObj["UIModel"])
                                    UIObj["UIModel"] = [...UIObj["UIModel"], ...finalPrefstyleData];
                                else
                                    UIObj["UIModel"] = finalPrefstyleData;
                            }
                        }
                        ...
                        if (appitem.nlsJSON != "empty") {
                            for (let j = 0; j < appitem.nlsJSON.length; j++) {
                                ...
                                const rawNLSData = JSON.parse(nls.data);
                                const finalNLSData = prepareNLSDataWithAppID(rawNLSData, currentAppID);
                                if (nls && style.includes((nls.fileName).split("_")[0])) {
                                    if (nlsObj)
                                        nlsObj = { ...nlsObj, ...finalNLSData };
                                    else
                                        nlsObj = finalNLSData;
                                }
                            }
        ...
        mePreferencesPageView.prototype.fetchDefaultValues = async function (DEFAULT_REQUEST_BODY) {
            return await readDefaultValues(DEFAULT_REQUEST_BODY);
        };
