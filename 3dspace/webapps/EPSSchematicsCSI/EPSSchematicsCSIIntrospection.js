    ...
    (function (CSIIntrospection) {
        ...
        var generateCSIFunctionBlockDocCode = function (iCSIFunction) {
            ...
            if (iCSIFunction.progress !== undefined && (iCSIFunction.progress.parameters !== undefined ||
                ...
                if (iCSIFunction.implementation.name === 'executionGraph') {
                    blockDocObject.dataPorts.Progress += ' <br>Can be filled by the graph function implementation with links or by the graph engine with progress events.';
                    if (iCSIFunction.implementation.version === 1) {
                        blockDocObject.dataPorts.Progress += ' <br>The data returned is always a CSIExecGraphProgress (even when the progress is coming from the implementation, contained in the data property).';
                    }
                    else if (iCSIFunction.implementation.version === 2) {
                        blockDocObject.dataPorts.Progress += ' <br>The implementation data is passed as is and the progress events are sent as CSIExecGraphProgress. ';
                    }
                    blockDocObject.dataPorts.Progress += ' <br>For more information please read the following <a href="https://executionfw.dsone.3ds.com/docs/graph/getting_started/#manage-progress">documentation</a>.';
                }
            ...
            if (iCSIFunction.throwError !== undefined && (iCSIFunction.throwError.parameters !== undefined ||
                ...
                if (iCSIFunction.implementation.name === 'executionGraph') {
                    blockDocObject.dataPorts.Error += ' <br>Can be filled by the graph function implementation with links or by the graph engine when an internal error occurs.';
                    if (iCSIFunction.implementation.version === 1) {
                        blockDocObject.dataPorts.Error += ' <br>The data returned is always a CSIExecGraphError (even when the error is coming from the implementation, contained by the data property).';
                    }
                    else if (iCSIFunction.implementation.version === 2) {
                        blockDocObject.dataPorts.Error += ' <br>The implementation data is passed as is and the internal graph errors are sent as CSIExecGraphError.';
                    }
                    blockDocObject.dataPorts.Error += ' <br>For more information please read the following <a href="https://executionfw.dsone.3ds.com/docs/graph/getting_started/#understand-error">documentation</a>.';
                }
        ...
        var generateCSIFunctionBlockCode = function (iArguments, iCSIFunction) {
            ...
            if (iCSIFunction.implementation.name === 'ecmaScript' || iCSIFunction.implementation.name === 'python') {
                parentBlockPath = 'DS/EPSSchematicsCSI/EPSSchematicsCSIScriptFunctionBlock';
                parentBlockClass = 'CSIScriptFunctionBlock';
            }
            else if (iCSIFunction.implementation.name === 'executionGraph') {
