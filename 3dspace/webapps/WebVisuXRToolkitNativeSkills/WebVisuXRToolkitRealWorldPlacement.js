...
define("DS/WebVisuXRToolkitNativeSkills/WebVisuXRToolkitRealWorldPlacement", ["require", "exports", "DS/Visualization/ThreeJS_DS", "DS/WebVisuGLTF/GLTFLoader", "DS/WebappsUtils/WebappsUtils", "DS/Visualization/Node3D", "DS/Visualization/SceneGraphFactory", "DS/WebVisuXRToolkit/WebVisuXRToolkit", "i18n!DS/WebVisuXRToolkit/assets/nls/WebVisuXRToolkitNativeSkills"], function (require, exports, THREE, GLTFLoader, WebappsUtils_1, Node3D, SceneGraphFactoryStatic, WebVisuXRToolkit_1, NLS) {
    ...
    function getTouchIntersection(touchAxes, planeHeight, fov, aspect) {
        // Touch normalized device coords (already between -1 and 1)
        const ndcX = touchAxes[0];
        const ndcY = -touchAxes[1];
        // Get camera world transform from head
        const headMatrix = (0, WebVisuXRToolkit_1.getHeadWorldMatrix)();
        const camPos = new THREE.Vector3().getPositionFromMatrix(headMatrix);
        const camQuat = new THREE.Quaternion().setFromRotationMatrix(headMatrix);
        // Build camera basis
        const forward = new THREE.Vector3(0, 1, 0).applyQuaternion(camQuat);
        const up = new THREE.Vector3(0, 0, 1).applyQuaternion(camQuat);
        const right = new THREE.Vector3(1, 0, 0).applyQuaternion(camQuat);
        // Convert NDC to view direction
        const tanFov = Math.tan(fov / 2);
        const dir = new THREE.Vector3()
            .copy(forward)
            .add(right.multiplyScalar(ndcX * tanFov * aspect))
            .add(up.multiplyScalar(ndcY * tanFov))
            .normalize();
        // Construct ray
        ...
        onActivate(input) {
            ...
            if (input.userData.RealWorldHitTest.isMoving) {
                ...
                if (mat) {
                    ...
                    if (!cameraProp) {
                        cameraProp = (0, WebVisuXRToolkit_1.getCameraProperties)();
                    }
                    const intersection = getTouchIntersection(input.touchAxes, 0, cameraProp.fovInRad, cameraProp.aspectRatio);
                    if (intersection) {
                        this.cursor.setMatrix(this.cursor.getMatrix().setPosition(intersection));
                        this.cursor.setVisibility(!input.userData.RealWorldHitTest.isFingerSelectingCross);
                    }
                }
            }
