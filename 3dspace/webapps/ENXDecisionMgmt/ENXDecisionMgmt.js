    ...
    Utils.getDecisionDataUpdated = function (decisionId) {
    	...
    	DecisionController.fetchDecisionById(decisionId).then(
				success => {
					...
					if(!widget.getValue('openedDecisionId')) //Need to skip decision onSave -> decision-preview-close-click call for open-with flow
        ...
        Utils.getDecisionDataUpdatedFromSummary = function (decisionId) {
    	require(['DS/ENXDecisionMgmt/Controller/DecisionController','DS/ENXDecisionMgmt/Model/DecisionModel','DS/ENXDecisionMgmt/Utilities/DataFormatter'], function(DecisionController,DecisionModel,DataFormatter) {
    	DecisionController.fetchDecisionById(decisionId).then(
				success => {
					// Refresh id card header and summary page //
					//var datadecision = DataFormatter.gridData(success[0]);
					DecisionModel.updateRows(success[0]);
...
define('DS/ENXDecisionMgmt/Utilities/DecisionPersistencyUtil', [
	...
	function() {
		...
		let getInfoIconPersistency = function() {
			if (widget.decisionSummary) {
				return widget.getValue("isDecInfoIconActiveSummary");
			}
			else {
				return widget.getValue("isDecInfoIconActive");
			}
		...
		let DecisionPersistencyUtil = {
			...
			getInfoIconPersistency: () => { return getInfoIconPersistency(); },
		};
...
function(core, Alone, NLS) {
	...
	{ 
    	...
    	allowClosableFlag : false,
    	content: Alone.createElement('div', {id:'decisionCommentContainer', 'class':'decision-Comment-container'}),
        loader : 'DS/ENXDecisionMgmt/View/Facets/DecisionCommentFacet'
    }
    ];
...
define('DS/ENXDecisionMgmt/Utilities/VersionFacetIntegration', [
	...
	let launchVersionControlApp = function(id, version){
        ...
        _versionModel._myVersionExplorer.publishEvent('ENOXVersionExplorerLoadVersionModel', {
            ...
            securityContext: collabSpace.split("ctx::")[1],
            onComplete: function () { },
...
function(NotificationsManagerUXMessages,NotificationsManagerViewOnScreen) {
    ...
    DecisionNotify.prototype.handler = function () {
    	...
    	} else if (document.getElementById("chg-Decision-container")) {
            NotificationsManagerViewOnScreen.inject(document.getElementById("chg-Decision-container"));
        }else{
...
define('DS/ENXDecisionMgmt/Utilities/DataFormatter',
	...
	let decisionGridData = function(dataElem){
		...
		const finalResponse = addCustomAttributesToModel(dataElem, response);
	...
	let addCustomAttributesToModel = function(dataElem, currentModelData) {
		
		let customFields = (widget.getValue('decisionCustomFields')) || null;
		if (customFields && customFields.items && customFields.items.length && customFields.items.length > 0) {
			...
			customFields.items.forEach((ele) => {
				...
				if (ele.name != 'extensions' && ele.extension == "XP_Decision_Ext") {
						...
						if (viewConfig.multivalue===true) {
							if (!customFieldValue) {
								...
								for (let i=0; i<dateStringArray.length; i++) {
									...
									let formattedDate = Utils.formatDateTimeString(date);
									customFieldValue.push(formattedDate);
								}
							}
						}
						else {
							let date = (!customFieldValue || customFieldValue=="") ? "": new Date(customFieldValue);
							let formattedDate = Utils.formatDateTimeString(date);
							customFieldValue = formattedDate;
						}
						
					}
					
					currentModelData[ele.name] = customFieldValue;
				}
			});
		}
	...
	let prepareTreeDocumentModel = function(response){      
 	...
 	let setModelFilter = function(model) {
		...
		if (customFields && customFields.items && customFields.items.length && customFields.items.length>0) {
			customFields.items.forEach((ele) => {
				if (ele.name != 'extensions' && ele.extension == "XP_Decision_Ext") {
					let eleConfig, eleType;
					if(ele.viewConfig) {
						eleConfig = JSON.parse(ele.viewConfig);
						eleType = eleConfig.type;
					}
					if(eleType == "date"){
						columnFilterConfig[ele.name] = { filterId : "date" }
					}
					else {
	    ...
	    if(data){    		
    		...
    		data.forEach(function(dataElem, idx) {	
	        	...
	        	var root = new TreeNodeModel({
     	            ...
     	            width: 300,
     	           "thumbnail" : dataElem.dataelements.thumbnailIcon,
...
define('DS/ENXDecisionMgmt/Services/DecisionServices',
    ...
    let DecisionServices, _createDecision, _fetchMeetingDecisions, _fetchDecisionAppliesTo, _removeRefDocsItems, _addReferenceDocument,
        _fetchDecisionRefDocs, _fetchAllDecisison, _fetchDecisionById, _addDecisions, _deleteDecision, _addAppliesToDecision, _removeAppliesToItems,
		_reviseDecision, _removeDecision, _fetchAllowedRefDocsTypesForDecision, _getCustomPropertiesFromDB;
  ...
  _reviseDecision = function(inputData,decisionId){
   ...
   let _makeWSCall  = function (URL, httpMethod, authentication, ContentType, ReqBody, userCallbackOnComplete, userCallbackOnFailure, options) {
	...
	_fetchAllowedRefDocsTypesForDecision = function(){
   	...
   	/*makes service call to bps.decision i.e., /decisions to retrieve data definitions only
   	@args: none
   	@params: $definitions=nodata
   	@ret: {items:[{...}]}
   	*/
	_getCustomPropertiesFromDB = function() {
   		return new Promise(function(resolve, reject) {

   			let getURL = DecisionBootstrap.getDecisionServiceBaseURL() + "?$definition=nodata";
   			let options = {};
   			options.method = 'GET';
   			options.headers = {
   				'Content-Type': 'application/json'
   			...
   			options.onComplete = function(data) {
   				...
   				if (res) {
   					let customFields = res.find((ele) => ele.name==='decision-custom-fields');
   					if (customFields && customFields.items) 
   						tempObj.items = customFields.items; 
   				}
   				else {
   					console.log("no object definition found for /decision?");
   					reject({});
   					return;
   				}
   			...
   			options.onFailure = function(err) {
   				console.log("fetching decision object properties - ERR");
   				reject(err);
   			}
   			
   			DecisionBootstrap.decisionAuthReq(getURL, options);
								     ...
								     try {
									     require(['DS/ENOXAttachmentsUX/AttachmentsPane'],function(AttachmentsPane) {
											...
											documentsPane.inject(containerDiv);
												if(typeof dscef !== 'undefined'){
													documentsPane.modelEvents.subscribe({ event: 'user-clicks-attach' }, function (data) {
														widget.decisionDocumentsPane = documentsPane
														dscef.sendString("NativeChgGenericSearch" + '=' + "flattenedtaxonomies:\"types/DOCUMENTS\" OR flattenedtaxonomies:\"types/Document\" OR flattenedtaxonomies:\"types/Service Content\" OR flattenedtaxonomies:\"types/mxsysDSFAHolder\" OR flattenedtaxonomies:\"types/Specification\" OR flattenedtaxonomies:\"types/Engineering Specification\" OR flattenedtaxonomies:\"types/Material Specification\" OR flattenedtaxonomies:\"types/Derived Output\" OR flattenedtaxonomies:\"types/Report Template\" OR flattenedtaxonomies:\"types/Report Document\" OR flattenedtaxonomies:\"types/CONTROLLED DOCUMENTS\" OR flattenedtaxonomies:\"types/Quality System Document\" OR flattenedtaxonomies:\"types/Controlled Document Template\" OR flattenedtaxonomies:\"types/Technical Document\" OR flattenedtaxonomies:\"types/Rendition Document Template\" OR flattenedtaxonomies:\"types/Rendition Configuration\" OR flattenedtaxonomies:\"types/Requirement Specification\" OR flattenedtaxonomies:\"types/Design Specification\" OR flattenedtaxonomies:\"types/Drawing Print\" OR flattenedtaxonomies:\"types/Functional Specification\" OR flattenedtaxonomies:\"types/Sketch\" OR flattenedtaxonomies:\"types/Test Specification\" OR flattenedtaxonomies:\"types/CAD Drawing\" OR flattenedtaxonomies:\"types/CAD Model\" OR flattenedtaxonomies:\"types/Markup\" OR flattenedtaxonomies:\"types/Document Sheet\" OR flattenedtaxonomies:\"types/Generic Document\" OR flattenedtaxonomies:\"types/ECAD Model\" OR flattenedtaxonomies:\"types/MCAD Model\" OR flattenedtaxonomies:\"types/MCAD Assembly\" OR flattenedtaxonomies:\"types/MCAD Assembly Instance\" OR flattenedtaxonomies:\"types/MCAD Component\" OR flattenedtaxonomies:\"types/MCAD Component Instance\" OR flattenedtaxonomies:\"types/MCAD Parameterized Model\" OR flattenedtaxonomies:\"types/MCAD Parameterized Assembly Model\" OR flattenedtaxonomies:\"types/MCAD Parameterized Component Model\" OR flattenedtaxonomies:\"types/Technical Specification\" OR flattenedtaxonomies:\"types/Product Specification\" OR flattenedtaxonomies:\"types/Build Specification\" OR flattenedtaxonomies:\"types/Customer Requirement Specification\" OR flattenedtaxonomies:\"types/Customer Response Requirement Specification\" OR flattenedtaxonomies:\"types/Customer Specification\" OR flattenedtaxonomies:\"types/Feature Requirement Specification\" OR flattenedtaxonomies:\"types/Feature Specification\" OR flattenedtaxonomies:\"types/Process Technical Specification\" OR flattenedtaxonomies:\"types/Product Requirement Specification\" OR flattenedtaxonomies:\"types/Software Specification\" OR flattenedtaxonomies:\"types/System Requirement Specification\" OR flattenedtaxonomies:\"types/System Specification\" OR flattenedtaxonomies:\"types/Use Case Specification\" OR flattenedtaxonomies:\"types/PTArchive\" OR flattenedtaxonomies:\"types/Part Specification\" OR flattenedtaxonomies:\"types/MCAD Drawing\" OR flattenedtaxonomies:\"types/MCAD Drawing Template\" OR flattenedtaxonomies:\"types/MCAD Model Template\" OR flattenedtaxonomies:\"types/Sub System Requirement Specification\" OR flattenedtaxonomies:\"types/IEF Family\" OR flattenedtaxonomies:\"types/MCAD Component Template\" OR flattenedtaxonomies:\"types/MCAD Assembly Template\" OR flattenedtaxonomies:\"types/IEF Assembly Family\" OR flattenedtaxonomies:\"types/IEF Component Family\" OR flattenedtaxonomies:\"types/IEF Versioned Assembly Family\" OR flattenedtaxonomies:\"types/IEF Versioned Component Family\" OR flattenedtaxonomies:\"types/IDF Model\" OR flattenedtaxonomies:\"types/IDF Versioned Model\" OR flattenedtaxonomies:\"types/ECAD FPGA Design\" OR flattenedtaxonomies:\"types/ECAD PCB Design\" OR flattenedtaxonomies:\"types/ECAD Versioned FPGA Design\" OR flattenedtaxonomies:\"types/ECAD Versioned PCB Design\" OR flattenedtaxonomies:\"types/Part Marking Specification\" OR flattenedtaxonomies:\"types/MCAD Representation\" OR flattenedtaxonomies:\"types/Document Proxy\" OR flattenedtaxonomies:\"types/Manufacturing Program Document\"|DS/ENXDecisionMgmt/Utilities/SearchUtil|nativeExistingAttachmentCb");
													});
													documentsPane.modelEvents.subscribe({ event: 'context-menu-requested' }, function (data) {
														//Need to add code for attachments RMB menu changes
													});
												}
											}, 
											function(e){
...
define('DS/ENXDecisionMgmt/Utilities/SearchUtil',
	...
	const nativeExistingDecisionsCb = (decIDs, action) => {
		const idArray = (typeof decIDs == 'string' && decIDs.split(',')) || []
		const currentModel = { 
			TreedocModel: {
				meetingId: widget.nativeParentObject
			}
		}
		idArray.forEach(id => {
			const data = [{ id }]
			DecisionController.addDecisions(currentModel, data).then(function(resp) {
				widget.meetingNotify.handler().addNotif({
					level: 'success',
	...
	const nativeAppliesTosCb = (decIDs, action) => {
		const idArray = (typeof decIDs == 'string' && decIDs.split(',')) || []
		const currentModel = {
			TreedocModel: {
				decisionId: widget.nativeParentObject
			}
		}
		idArray.forEach(id => {
		const data = [{ id }]
			DecisionController.addAppliesToDecision(currentModel,data).then(function(resp) {
				DecisionAppliesToModel.appendRows(resp);
	...
	const nativeExistingAttachmentCb = (id) => {
		let decAttachmentPane= widget.decisionDocumentsPane
		if (decAttachmentPane) {
			let selectedObjects = id.split(",");
				let ObjectIds = [];
				for (let i = 0; i < selectedObjects.length; i++) {
					ObjectIds.push(selectedObjects[i]);
				}
				decAttachmentPane.attachAttachments(ObjectIds);
		}
	}
	...
	let SearchUtil = {
			...
			nativeExistingDecisionsCb,
			nativeAppliesTosCb,
			nativeExistingAttachmentCb
            ...
            DecisionAppliesToActions={ 
                    ...
                    onSearchClick: function(){
                        ...
                        require(['DS/ENXDecisionMgmt/Model/DecisionAppliesToModel'], function(appliesToModel) {
                            ...
                            var that = this;
                            if (typeof dscef !== 'undefined') {
								widget.nativeParentObject = data.TreedocModel.decisionId
								dscef.sendString("NativeChgGenericSearch" + '=' + "|DS/ENXDecisionMgmt/Utilities/SearchUtil|nativeAppliesTosCb");
							} else if (!UWA.is(searchcom_socket)) {
...
define('DS/ENXDecisionMgmt/Config/Toolbar/DecisionTabToolbarConfig',
  ...
  function (DecisionPersistencyUtil, WUXTooltipModel, NLS) {
    ...
    let writetoolbarDefination = function (filterPreference) {
      ...
      var showinfoIconforDecision=widget.getValue("showinfoIconforDecision");
      if(showinfoIconforDecision) {
       		let infoIcon = {
      			"id": "decisionInfoIcon",
	             "dataElements": {
	               "typeRepresentation": "functionIcon",
	               "icon": {
	                   "iconName": "info",//acc2 need to change
	                   fontIconFamily: WUXManagedFontIcons.Font3DS
	                 }
	             },
	             "position": "far",
	             ...
	             "action": {
	                 module: 'DS/ENXDecisionMgmt/Actions/DecisionActions',
	                 func: 'showDecisionInfoPanel'
	               },
	             "tooltip": {title: NLS.idCardHeaderActionInfo}
        	}
        	defination.entries.push(infoIcon);
...
define('DS/ENXDecisionMgmt/Config/DecisionGridViewConfig',
    ...
    let actionTypeUrl = (typeof dscef !== 'undefined') ? "string" : "url"
    
	let decisionViewConfig;
	
    let DecisionGridViewConfig = 	{
		
		init: function() {
			this.setDecisionGridViewConfig();
		},
	    
		getDecisionGridViewConfig: function() {
			if (!this.decisionViewConfig) {
				this.setDecisionGridViewConfig();
			}
			
			return this.decisionViewConfig;
		},
		
		setDecisionGridViewConfig: function() {
			this.decisionViewConfig = [
				{
		        	text: NLS.title,
		          	dataIndex: 'tree',
		          	pinned: 'left',
		        ...
		        }, 
		        {
					...
					dataIndex: "actions",
					editableFlag: false,
					typeRepresentation: actionTypeUrl
				...
				customFields.items.forEach((ele) => {
					if (ele.name != 'extensions' && ele.extension == "XP_Decision_Ext") {
						...
						try {
							...
							this.decisionViewConfig.push(tempObj);
						} catch(e) {
...
define('DS/ENXDecisionMgmt/View/Grid/DecisionDataGridView',
	...
	let build = function(model){
		...
		DecisionGridViewConfig.init();
		let decisionColumnsConfig = DecisionGridViewConfig.getDecisionGridViewConfig();
		_gridOptions.cellDragEnabledFlag = true;
        let dataGridViewContainer = WrapperDataGridView.build(model, decisionColumnsConfig, toolbar, gridViewDiv, _gridOptions, "DecisionView");
		...
		var thumbnailSec = new UWA.Element('div',{
			...
			"html":[
				  UWA.createElement('div',{
					  ...
					  styles:{
						  "background-image": "url(" + data.thumbnailIcon + ")"
					  }
				  	})]
		});
...
define('DS/ENXDecisionMgmt/View/Facets/DecisionAllTabs', [
  ...
  function (WUXTabBar,DecisionFacets,DecisionPersistencyUtil) {
	...
	var ntabs =["DecisionFacetProperties", "DecisionAttachments","DecisionAppliesTo","DecisionApplicability","DecisionRelations", "DecisionHistory", "DecisionComments"];
...
define('DS/ENXDecisionMgmt/View/Form/DecisionCommentsView', [

	"DS/ENXDecisionMgmt/Controller/DecisionBootstrap",
	"DS/SocialToolbarVue/SocialToolbarVueUWAView"
	
], function(DecisionBootstrap, SocialToolbarVueUWAView) {

	let DecisionCommentsView = {
		renderComments: function(_appInstance, data) {
			var that = this;
			this.elements = _appInstance;
			let decisionPhysicalId = data.id;
			let swymUrl = DecisionBootstrap.getSwymUrl();
			var x3dcontent = {
				'protocol': '3DXContent',
				'version': '',
				'source': '',
				...
				'data': {
					'items': [{
						'envId': DecisionBootstrap.getTenantValue(),
						'serviceId': "3DSpace",
						'objectId': decisionPhysicalId,
						'objectType' : data.type,
						'facet': 'isSocial'
					}]
				}
			...
			const opts = {
			    props: {
					subjectUri: 'pid:' + decisionPhysicalId,
			        disableFeatures: {
			            disableShare: true,
			            disableLike : false,
					...
					platformId: DecisionBootstrap.getTenantValue(),
					urls: {
			            service: DecisionBootstrap.get3DSpaceURL() + '/resources',
			            serviceCSRF: {
			                name: "X-DS-CSRFTOKEN",
			                //token: "1WEH-GJ3W-VBP6-T8T1-R9D5-MWEY-VDXS-51GA",
			            },
			            registry: DecisionBootstrap.getRegistryUrl(),
			        },
					atInit: {
			            nbCommentsToLoad: 8,
			        },
	...
	return DecisionCommentsView;
});


define('DS/ENXDecisionMgmt/View/Facets/DecisionCommentFacet', [
	
	'DS/ENXDecisionMgmt/View/Form/DecisionCommentsView'
    
], function (DecisionCommentsView) {
	'use strict';

	let _appInstance = {};
	const buildContainer = function() {
		_appInstance.container = new UWA.Element('div', { html: "", id: "decisionCommentContainer", 'class': 'decision-Comment-container' });
		_appInstance.container.inject(document.querySelector('.decision-facets-container'));
	};
	...
	const render = function(data){
		//this.elements = _appInstance;
		DecisionCommentsView.renderComments(_appInstance, data);
	};

    const hideView = function () {
        if (document.getElementById('decisionCommentContainer') != null) {
            document.getElementById('decisionCommentContainer').style.display = 'none';
        }
    };

    const showView = function () {
        if (document.querySelector('#decisionCommentContainer') != null) {
            document.getElementById('decisionCommentContainer').style.display = 'block';
            return true;
        }
        return false;
    };
    ...
    const destroy = function () {
        document.querySelector('#decisionCommentContainer').destroy();
    };
...
let build	= function(data){
	 if(!showView()){
		 ...
		 containerDiv.inject(document.querySelector('.decision-facets-container'));
		 const environment = typeof dscef !== 'undefined' ? "Native" : "Dashboard";
		 var options = { 'mode': "Applicability", 'objectid': data.id, 'parent': containerDiv, environment }
        ...
        'DS/ENXDecisionMgmt/Controller/DecisionController',
         ...
         function(
            ...
            DecisionActions={
            		...
            		addExistingDecisionDialog: function(){
                        ...
                        require(['DS/ENXDecisionMgmt/Model/DecisionModel'], function(decisionModel) {
                            ...
                            var that = this;
                            if (typeof dscef !== 'undefined') {
								widget.nativeParentObject = data.TreedocModel.meetingId
								dscef.sendString("NativeChgGenericSearch" + '=' + "flattenedtaxonomies:\"types/Decision\" AND current:\"Active\"|DS/ENXDecisionMgmt/Utilities/SearchUtil|nativeExistingDecisionsCb");
							} else if (!UWA.is(searchcom_socket)) {
                                require(['DS/SNInfraUX/SearchCom'], function(SearchCom) {
                                    searchcom_socket = SearchCom.createSocket({
                                        socket_id: socket_id
                 	   ...
                 	   return new Promise(function(resolve, reject) {
                 		  DecisionController.deleteDecision(meetingId,ids).then(
                 				success => {
									...
									if(widget.decisionSummary == false)
										widget.meetingEvent.publish('decision-delete-refresh-agenda');
									if(widget.decisionSummary)
           ...
           reviseDecision : function(decisionId){
	        ...
	        showDecisionInfoPanel : function(from,fromEditProp){
				
				//MeetingPropertiesFacetFacetsContainer
				var decModel=DecisionModel.getModel();
				var decisions = decModel.getSelectedNodes();
				//var data=decModel.getSelectedNodes()[0].options.grid;
				if(IdCardUtil.infoIconIsActive()){
					IdCardUtil.infoIconInActive();
					widget.setValue("meetPropOpen", false);
				}
				
				if (DecisionPersistencyUtil.getInfoIconPersistency() && from != "ForRefresh") {
					widget.meetingEvent.publish('decision-info-close-click');
				} else {
					widget.meetingEvent.publish('decision-info-click', { models: decisions, mode: "SummaryInfoPanel",fromEditProp:fromEditProp });
				}
				/*if(decModel.getSelectedNodes().length > 1){
					widget.meetingEvent.publish('decision-preview-click', {model:decModel ,mode:"multiSelect"});
					widget.meetingEvent.publish('decision-multi-select');//call only if multi objects are selected
				}else{
					var data=decModel.getSelectedNodes()[0].options.grid;
					widget.meetingEvent.publish('decision-preview-click', {model:data});
				}*/
			}
          };
...
define('DS/ENXDecisionMgmt/View/Properties/DecisionPropWidget', [
    ...
    var DecisionPropWidget = {
    	render: function (propContainer,data,mode,fromEditProp) {
			var that = this;
	        ...
	        if(mode == "SummaryInfoPanel"){
			var options_panel = {
					 typeOfDisplay: EditPropConstants.ALL, // Properties with ID card - ALL. ONLY_EDIT_PROPERTIES for only properties
			...
			/*if (Array.isArray(data)) {
				// Iterate over each object ID in the array
				data.forEach(function (obj) {
					//var objectId = obj.options.grid.id;
					if("Active"!=obj.options.grid.state ){
						options_panel.readOnly = true;
					}
				});
			}*/
			if(!that.EditPropWidget || widget.getValue('initEditProp')) { 
				widget.setValue('initEditProp', false);
				propContainer.empty();
		...
		/*	if(data && data.length >= 1 && !document.querySelector('.modal-header')){
				propContainer.empty();
				this.EditPropWidget = new EditPropWidget(options_panel);
				}*/
			//this.EditPropWidget = new EditPropWidget(options_panel);
			
			that.EditPropWidget.elements.container.inject(propContainer);
			
			that.loadModel(data, propContainer);
			...
			if (Array.isArray(objModel)) {
				...
				objModel.forEach(function (obj) {
					...
					selection.addEvent('onSave', function (event) {
						Utils.getDecisionDataUpdatedFromSummary(objectId);
						DecisionActions.showDecisionInfoPanel("ForRefresh",true);
					});

					resultElementSelected.push(selection);
				});
			} else {
				// Handle the case where objModel is a single object ID
				var selection = new EditPropModel({
					metatype: 'businessobject',
					objectId: objModel,
					source: "3DSpace",
				...
				selection.set('isTransient', false);
				selection.addEvent('onSave', function (event) {
					Utils.getDecisionDataUpdated(objModel);
				});
		...
		loadModel: function (objId,propContainer) {
			...
			let that = this, results = that.getPropModel(objId);
			that.EditPropWidget.initDatas(results);
			
			//this.EditPropWidget.elements.container.inject(propContainer);			
		},
		destroyEditPropWidget: function(){
				let that = this;
				that.EditPropWidget = undefined;
		}
    };
...
define('DS/ENXDecisionMgmt/View/Facets/DecisionTab',
        [ 'DS/ENXDecisionMgmt/Model/DecisionModel',
		  ...
		  'i18n!DS/ENXDecisionMgmt/assets/nls/ENXDecisionMgmt'], function(
        		 DecisionModel, DecisionActions, DecisionDataGridView ,DecisionTileView, WrapperDataGridView,WrapperTileView, WAFData, DecisionBootstrap, CollabStorageCollection, 
				 DecisionServices, DecisionController,DecisionEvent,DecisionNotify, DataFormatter, TagNavigator, ArrayUtils, WUXButton, PointerEvents, ModalLoader, 
				 TreeDocument, AutoCompleteUtil, PlaceHolder, WUXTooltipModel, ImmersiveFrame,DecisionPersistencyUtil, WebUAUtils, DecisionWidgetComServices, NLS
            ) {
    ...
    let  initDecisionBootstrap = function(parentInfoModel,storages){
    	return new Promise(function(resolve, reject) {
			...
			DecisionServices.getCustomPropertiesFromDB().then((decisionCustomProps) => {
				if (decisionCustomProps && Object.keys(decisionCustomProps).length && Object.keys(decisionCustomProps).length>0 && decisionCustomProps[Object.keys(decisionCustomProps)[0]].length && decisionCustomProps[Object.keys(decisionCustomProps)[0]].length>0) {
					widget.setValue('decisionCustomFields', decisionCustomProps);
				}
			})
    		...
    		initDecisionBootstrap(_parentInfoModel,storages).then(function(success) {
				...
				if (_parentInfoModel.from && _parentInfoModel.from === 'openwith') {
    				...
    				showOrHideLoader(false);
						...
						if (widget.getValue("isDecInfoIconActive")) {
							DecisionActions.showDecisionInfoPanel("ForRefresh");
							if (widget.getValue("DecisionIdSelected")) {
								const decIds = new Set(widget.getValue("DecisionIdSelected").split(','));
								decIds.forEach((id) => {
									if(DecisionModel.getRowModelById(id)){
										DecisionModel.getRowModelById(id).select();
									}
								});
							}
    		...
    		initDecisionBootstrap(_parentInfoModel,storages).then(function(success) {
    			...
    			DecisionController.fetchAllDecisison().then(
    				success => {
	    				...
	    				showOrHideLoader(false);
	    					//Add logic to check for info icon status
						if (widget.getValue("isDecInfoIconActiveSummary")) {
							DecisionActions.showDecisionInfoPanel("ForRefresh");
							if (widget.getValue("DecisionIdSelectedSummary")) {
								const decIds = new Set(widget.getValue("DecisionIdSelectedSummary").split(','));
								decIds.forEach((id) => {
									if(DecisionModel.getRowModelById(id)){
										DecisionModel.getRowModelById(id).select();
									}
    ...
    let drawDecisionsView = function(containerDiv){
		...
		_addToolbarBtnTooltip(decisionTabToolbar, "decisionInfoIcon", 'SummaryPageInformation');
    ...
    let openContextualMenu = function (e, cellInfos) {
        if (cellInfos && cellInfos.nodeModel && cellInfos.nodeModel.options.grid) {
			...
			let onSelect = function(e, cellInfos) {
				//persistency
				var newSelectedObjId = e.options.grid.id;
				var key;
				if (widget.decisionSummary) {
					key = "DecisionIdSelectedSummary";
				} else {
					key = "DecisionIdSelected";
				}
				if (widget.getValue(key)) {
					const decIds = new Set(widget.getValue(key).split(','));
					decIds.add(newSelectedObjId);
					widget.setValue(key, Array.from(decIds).join(','));
				} else {
					const selectedObjIds = new Set();
					selectedObjIds.add(newSelectedObjId);
					widget.setValue(key, Array.from(selectedObjIds).join(','));
				}
			...
			let deSelect = function(e, cellInfos) {
				...
				if (widget.getValue(key)) {
					const decIds = new Set(widget.getValue(key).split(','));
					decIds.delete(newSelectedObjId);
					widget.setValue(key, Array.from(decIds).join(','));
				}
