...
define('DS/ENOXCADTypeMappingUI/views/SelectionChipsView', [
    ...
    var selectionChipsView = View.extend({
        ...
        _addSelectionChips: function(selectionChips) {
            selectionChips._preventEventsCpt++; // Disable Events
            selectionChips.removeAllChips();

            this.selectionChipsBackup.forEach(function(selectionChip) {
                selectionChips.addChip({
                    label: selectionChip.label,
                    value: selectionChip.value
                });
            });
        ...
        render: function() {
            ...
            selectionChips.addEventListener('onPostRemove', function(e) {
				...
				const mode = that.model.get('mode');

                if (addedChipsValues.includes(parentEnoviaType) || (null != mode && mode.includes('customTypeList') && addedChipsValues.length > 0)) {
                    ...
                    CheckObjectsForType.isObjectsPresentForEnoviaType(subEnoviaType, parentEnoviaType)
                        .then(response => {
                            ...
                            if (!isObjectPresent) {
                                ...
                                that._addSelectionChips(selectionChips);
                            }
